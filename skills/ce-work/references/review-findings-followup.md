...
`ce-code-review` is invoked here with `mode:agent`, so it is **review-only** in this context — it reports findings and writes artifacts and does not mutate the working copy, describe or finalize a Jujutsu change, push, or file tickets. **The caller owns apply/fix policy.** Standalone review is also report-only unless local apply was explicitly authorized.
...
- `base:` — when the diff base is already resolved in the current workspace; omit when reviewing a PR number/URL or standalone current bookmark/change.
...
- Stable finding `#` — reuse in Jujutsu descriptions, residual sinks, and subagent prompts

## What to apply

Default to applying every actionable finding. Applying is a reversible edit to the working copy; diffs are reviewed before the change is described and tests run after, so leaving a clear, reversible fix unapplied "to be safe" is the failure mode, not the safe choice. Bias to act:
...
There is no precondition safety checklist and no deny-list — a code-review fix is a reversible edit, so downside is controlled after the fact through diff review, tests, and the Jujutsu change boundary, not by gating the apply.
...
**Orchestrator owns:** parse review output → **eligibility filter on JSON fields only** → build batches → dispatch fix subagents → review diffs → tests → record accepted Jujutsu changes → Residual Work Gate.
...
3. **Parallel waves:** batches with **disjoint file sets** may run in parallel under the same isolated-workspace/shared-directory rules as Phase 1 Step 4 in `ce-work` `SKILL.md`.
...
- Shared-directory fallback: do not describe, split, squash, or finalize a Jujutsu change — return which `#` were applied or skipped and which files changed

**After each wave:** the orchestrator reviews diffs (scope = assigned `#` only) and runs tests (`requires_verification: true` on any applied finding -> at least targeted tests; multi-file -> broader suite). Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Derive the Jujutsu description from the fixes actually applied, the project's active instructions, and syntax observed with `jj log`; those local sources take precedence, and compatible Go guidance applies without a fixed prefix, type, scope, template, example, or identity footer. Record the accepted fixes as a Jujutsu change unless provider-isolated workers require import/integration per Phase 1. Repeat until all batches complete.
