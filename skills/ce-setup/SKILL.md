...
Ask each question below using the active host's blocking question mechanism. Fall back to a numbered list in chat only when no blocking mechanism exists or the call errors. Never silently skip or auto-configure.
...
4. Check whether `.rocketclaw/config.local.yaml` exists and, if it does, whether `jj file list .rocketclaw/config.local.yaml` omits it after the working copy is snapshotted. A listed path is already tracked and is not safely ignored.
...
**User-runnable invocation rendering.** In setup summaries, use the invocation syntax documented by the active host, defaulting to `/ce-setup`. Render only the invocation as inline code and output one form only.
...
If `.rocketclaw/config.local.yaml` exists and is tracked by Jujutsu, offer to add this ignore pattern to `.gitignore`:

```text
.rocketclaw/*.local.yaml
```

Append the entry to the workspace-root `.gitignore` only if the user approves. If the file was already tracked, also run `jj file untrack .rocketclaw/config.local.yaml`. Do not overwrite unrelated `.gitignore` content.
