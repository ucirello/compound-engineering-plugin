...
5. **Precise Implementation**: Make the necessary code changes to fix all identified differences:
   ...
   - Ensure changes follow the project's coding standards already supplied in context; if dispatched without them, request the applicable conventions from the caller
...
- **Code Quality**: Follow the project's frontend conventions already supplied in context; if absent, request them from the caller
