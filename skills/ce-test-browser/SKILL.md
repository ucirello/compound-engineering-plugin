...
1. **Select the driver** per the policy above and record it. This also requires a JJ workspace with changes to test. Resolve the workspace root with `jj workspace root` and run every later `jj` command with cwd at that root (`(cd "$workspace_root" && jj ...)`); do not use `jj -R`.
2. **Determine test scope** from the argument, at that workspace root: a PR number → `GIT_DIR=$(jj git root) gh pr view [number] --json files -q '.files[].path'`; otherwise `jj diff --from main --to <rev> --name-only`, with `<rev>` = `@` when the argument is `current` or empty, and the named bookmark otherwise. File lists are repo-relative.
