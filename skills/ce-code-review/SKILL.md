...
argument-hint: "[mode:agent] [blank to review current change, or provide PR link]"
---
...
| `mode:report-only` | `mode:report-only` | **Deprecated — ignored.** Former no-artifacts mode; default behavior is review-only without switching the working copy |
| `base:<change-or-ref>` | `base:abc1234` or `base:trunk()` | Diff base in the **current workspace** (explicit; skips auto base detection) |
...
- **Explicit mutations only.** Never run commands that switch the working copy, including `jj edit` or `jj new`. Passing a PR number, URL, or bookmark name selects **review scope**, not permission to mutate the working copy. To review local work on another change, edit that change yourself (or stay on it) and pass `base:` or no target.
- **Smart defaults.** JJ snapshots eligible new files into the working-copy change; ignored files remain out of scope. Plan: use `plan:` when passed; otherwise discover conservatively from PR body or bookmark keywords. Weak advisory P2/P3 from testing/maintainability alone: demote to `testing_gaps` / `residual_risks` per Stage 5.
...
| **`mode:agent`** | One JSON object (see ### JSON output format below) + the same `<workspace-root>/.tmp/rocketclaw/code-review/<run-id>/` artifacts |
...
13 reviewer personas in layered conditionals, plus RocketClaw local prompt assets. Quick roster with one-line triggers below; the persona catalog in `references/persona-catalog.md` (read it at Stage 3) has the full per-persona selection criteria and spawn gates. Each selected reviewer is a generic subagent seeded with a local prompt file from `references/personas/`; do not dispatch standalone agents by type/name.

**Always-on (full review):** local prompt assets `correctness-reviewer`, `testing-reviewer`, `maintainability-reviewer`, `project-standards-reviewer`, plus RocketClaw local prompt assets `agent-native-reviewer` and `learnings-researcher`. (Stage 3c may reduce this set to a lite roster for trivial, low-risk diffs.)
...
A full review spawns generic subagents for all 4 always-on personas plus the 2 RocketClaw always-on local prompt assets, then adds whichever cross-cutting and stack-specific conditionals fit the diff (Stage 3c can collapse this to a lite roster for trivial, low-risk diffs). The model naturally right-sizes: a small config change triggers 0 conditionals = 6 reviewers. A Rails auth feature might trigger security + reliability + adversarial = 9 reviewers.

## Protected Artifacts

The following paths are pipeline artifacts and must never be flagged for deletion, removal, or ignore rules by any reviewer:

- `docs/brainstorms/*` -- legacy requirements documents
- `docs/plans/*.{md,html}` -- unified requirements and implementation plans (decision artifacts; execution progress is derived from JJ, not stored in plan bodies)
...
The caller already knows the comparison base. Skip trunk detection and remote resolution. Resolve the provided value as a JJ revset, require exactly one revision, then use its unique fork point with the working-copy revision when one exists; otherwise fall back to the resolved revision itself. This preserves the explicit-base contract: `base:` names the comparison line, while `BASE` normally spans the whole current stack from its common ancestor.

```
BASE_ARG="{base_arg}"
if [ "$(jj log -r "$BASE_ARG" --count 2>/dev/null)" != "1" ]; then
  echo "base: must resolve to exactly one JJ revision" >&2
  exit 1
fi
BASE_REF=$(jj log -r "$BASE_ARG" --no-graph -T 'commit_id ++ "\n"')
if [ "$(jj log -r "fork_point(@ | $BASE_REF)" --count 2>/dev/null)" = "1" ]; then
  BASE=$(jj log -r "fork_point(@ | $BASE_REF)" --no-graph -T 'commit_id ++ "\n"')
else
  BASE="$BASE_REF"
fi
```
...
```
echo "BASE:$BASE" && echo "FILES:" && jj diff --from "$BASE" --to @ --name-only && echo "DIFF:" && jj diff --from "$BASE" --to @ --git --context 10
```

This path works with any JJ revision — a change ID, commit ID, `trunk()`, local bookmark, or remote bookmark. Callers reviewing the current workspace should pass explicit `base:` when auto-detection is unnecessary. **Do not combine `base:` with a PR number or bookmark target.** If both are present, stop with an error: "Cannot use `base:` with a PR number or bookmark target — `base:` implies the current workspace already contains the reviewed change. Pass `base:` alone, or pass the target alone and let scope detection resolve the base."

**If a PR number or GitHub URL is provided as an argument:**

Do **not** edit the PR head. Scope comes from GitHub read APIs plus optional local alignment when the working-copy ancestry already contains the PR head.
...
- **Trivial-PR judgment**: spawn a lightweight sub-agent on the platform's cheapest capable model when a known override exists; otherwise omit the model override and inherit. Give it the PR title, body, and changed file paths. The agent's task: "Is this an automated or trivial PR that does not warrant a code review? Consider: dependency lock-file or manifest-only bumps, automated release changes, chore version increments with no substantive code changes. When in doubt, answer no — false negatives (skipped reviews that should have run) are more costly than false positives (unnecessary reviews)." If the judgment returns yes: stop with reason `PR appears to be a trivial automated PR; not reviewing. Run without a PR argument to review the current change, or pass base:<revision> if review is intended.`
...
If no skip rule fires, fetch PR metadata **without switching the working copy**:

```
GIT_DIR="$(jj git root)" gh pr view <number-or-url> --json title,body,baseRefName,headRefName,headRefOid,isCrossRepository,url,files,reviews,comments,headRepository --jq '{title, body, baseRefName, headRefName, headRefOid, isCrossRepository, url, headRepository, files: [.files[].path], hasPriorComments: ((.reviews | map(select(.state != "APPROVED" or .body != "")) | length) > 0 or (.comments | length) > 0)}'
```

Resolve `BASE_REMOTE` and `HEAD_REMOTE` before any fetch. Derive the base repository URL from the PR `url` and the head repository URL from `headRepository.nameWithOwner` (use `gh api` for the PR when that identity is unavailable), normalize SSH and HTTPS forms, and match them against `jj git remote list`. Require exactly one configured remote for each repository. For a same-repository PR, `HEAD_REMOTE=BASE_REMOTE`; for a fork, resolve the head repository independently. Never assume a remote is named `origin`. If either required repository has zero or multiple matches, use `pr-remote` with the GitHub diff and omit the corresponding JJ ref rather than fetching or diffing the wrong repository.

Set `BASE:` to `pr:<number-or-url>` (logical marker, not a JJ revision). JJ snapshots eligible new files into `@`; ignored files remain outside review scope.

**PR scope mode.** Classify as **`local-aligned`** only when **all** of these hold; otherwise use **`pr-remote`**. A matching bookmark name alone is not enough — a fork PR or a stale local bookmark can share a name with the PR head while pointing at unrelated code, and trusting the name would diff and inspect the wrong change.

1. The PR is **not** cross-repository (`isCrossRepository` is false).
2. The PR head repository maps to exactly one configured JJ remote, and after `jj git fetch --remote "$HEAD_REMOTE" --branch <headRefName>`, set `PR_HEAD_REF=<headRefName>@<head-remote>` and require it to resolve to exactly one revision matching `<headRefOid>`.
3. `jj log -r "@ & $PR_HEAD_REF::" --count` returns exactly `1`. This one-revision membership check confirms the working copy carries the PR head, allowing local changes layered on top; do not compare `commit_id` output to the literal string `@`.

- **`local-aligned`** — all checks pass. Local Read/Grep/`jj file annotate` against workspace files are valid for PR changed paths.
...
- **`local-aligned`:** Fetch `baseRefName` with `jj git fetch --remote "$BASE_REMOTE" --branch <baseRefName>` when needed. Resolve `BASE` with `jj log -r 'fork_point(@ | <baseRefName>@<base-remote>)' --no-graph -T 'commit_id ++ "\n"'`, substituting the resolved remote name, then set `FILES:` from `jj diff --from "$BASE" --to @ --name-only` and `DIFF:` from `jj diff --from "$BASE" --to @ --git --context 10`. Do **not** call `gh pr diff` or append remote hunks — when local fixes exist, the working-copy change is canonical. Note in Coverage: `scope: local-aligned (PR; local workspace diff)`.
- **`pr-remote`:** Set `FILES:` from the PR `files` array. Set `DIFF:` from `GIT_DIR="$(jj git root)" gh pr diff <number-or-url> --color=never`. If `gh pr diff` fails, stop with an actionable error — do not switch the working copy.

When **`pr-remote`**, before Stage 4:

1. Best-effort fetch PR head without editing it when `HEAD_REMOTE` was resolved: `jj git fetch --remote "$HEAD_REMOTE" --branch <headRefName>`.
2. When fetch succeeds, set `PR_HEAD_REF=<headRefName>@<head-remote>` using that resolved remote for reviewers and validators. When mapping or fetch fails, omit `PR_HEAD_REF` and note in Coverage — reviewers must rely on diff hunks only.
3. Best-effort fetch the PR base when `BASE_REMOTE` was resolved: `jj git fetch --remote "$BASE_REMOTE" --branch <baseRefName>`. When it succeeds, resolve `PR_BASE_REF` with `jj log -r 'fork_point(<baseRefName>@<base-remote> | <PR_HEAD_REF>)' --no-graph -T 'commit_id ++ "\n"'`, substituting the resolved remote name — a concrete fork-point revision reviewers and validators use for file-level diffs (for example, `data-migration-reviewer` runs `jj diff --from <PR_BASE_REF> --to <PR_HEAD_REF>` with dump filesets). The `pr:<number-or-url>` logical marker in `BASE:` stays the scope marker; `PR_BASE_REF` is the diffable base. When mapping, fetch, or fork-point resolution fails, omit `PR_BASE_REF` and note in Coverage — schema-drift and other JJ diff checks fall back to diff hunks only and must **not** assume a fixed default bookmark.
...
If `jj log -r '@ & <bookmark>::' --count` returns exactly `1`, use the **standalone (current change)** path below — same ancestry, explicit bookmark name; do not use remote-only diff. Treat zero or an unresolved/multi-revision result as not aligned; do not compare `commit_id` output to the literal string `@`.

Otherwise diff the remote/local bookmark **without switching the working copy**:

1. Try `GIT_DIR="$(jj git root)" gh pr view <bookmark> --json baseRefName,url,headRefName` — if a PR exists, prefer the **PR number/URL path** above (same remote diff rules).
2. Else map the repository that owns `<bookmark>` to exactly one configured remote by normalizing its GitHub URL against `jj git remote list`, as in PR mode. When mapped, run `jj git fetch --remote <bookmark-remote> --branch <bookmark>` and resolve `<bookmark-ref>` as `<bookmark>@<bookmark-remote>`. If no remote maps unambiguously, fall back to the local `<bookmark>` only when it resolves unambiguously; never guess a remote name.
3. Resolve the default base as `trunk()` unless GitHub metadata names another base. Compute `BASE` with `jj log -r 'fork_point(<base-ref> | <bookmark-ref>)' --no-graph -T 'commit_id ++ "\n"'` and diff with `jj diff --from "$BASE" --to <bookmark-ref> --git --context 10`.
4. If `<bookmark-ref>` cannot be resolved, stop: "Cannot diff bookmark `<bookmark>` without editing it. Edit that change, pass its open PR URL/number, or review the current change with `base:`."
...
```
echo "BASE:$BASE" && echo "FILES:" && jj diff --from "$BASE" --to <bookmark-ref> --name-only && echo "DIFF:" && jj diff --from "$BASE" --to <bookmark-ref> --git --context 10
```

**If no argument (standalone on current change):**

Apply the same base-detection logic as bookmark mode above. List local bookmarks on the current stack with `jj bookmark list -r 'heads(::@ & bookmarks())' -T 'name ++ "\n"'`, exclude the default bookmark, and identify the unique feature bookmark relevant to the current line. When exactly one is relevant, try `GIT_DIR="$(jj git root)" gh pr view <bookmark> --json baseRefName,url`; never use argumentless/current-branch PR discovery. Because this skill has no blocking prompts, when zero or multiple feature bookmarks are relevant, stop and require the caller to pass the PR number or URL explicitly; do not guess a bookmark or silently fall back to `trunk()`. Resolve the discovered base to exactly one local or remote JJ revision and save it as `BASE_REF`; do not discard a PR-provided `baseRefName` by reverting to `trunk()` afterward.

If no base can be resolved, **stop**. Do not fall back to `jj diff -r @` — a standalone review without the base would show only the working-copy change and silently miss earlier changes in the stack.
...
Using `jj diff --from "$BASE" --to @` diffs the fork point against the working-copy commit, including the full local stack and current working-copy changes. Eligible new files are included automatically, while ignored files remain out of scope.
...
- **`local-aligned` / standalone / `base:`** — `DIFF_A="$BASE"` (a single JJ revision), `DIFF_B` empty (diffs base vs `@`).
...
# Validate BOTH endpoints as single JJ revisions. An unfetched remote bookmark must
# not make a failed diff look like an empty diff and clear the lite gate.
if [ -z "${DIFF_A:-}" ] || [ "$(jj log -r "$DIFF_A" --count 2>/dev/null)" != "1" ] \
   || { [ -n "${DIFF_B:-}" ] && [ "$(jj log -r "$DIFF_B" --count 2>/dev/null)" != "1" ]; }; then
  ...
  CODE_FILESET='root-glob:"**/*.rb" | root-glob:"**/*.py" | root-glob:"**/*.js" | root-glob:"**/*.jsx" | root-glob:"**/*.ts" | root-glob:"**/*.tsx" | root-glob:"**/*.go" | root-glob:"**/*.rs" | root-glob:"**/*.java" | root-glob:"**/*.swift" | root-glob:"**/*.kt" | root-glob:"**/*.c" | root-glob:"**/*.cc" | root-glob:"**/*.cpp" | root-glob:"**/*.cs" | root-glob:"**/*.php" | root-glob:"**/*.ex" | root-glob:"**/*.exs" | root-glob:"**/*.scala"'
  # Count added and removed executable lines from JJ's Git-format patch, excluding
  # file headers. A modified line counts as one removal plus one addition.
  EXEC_LINES=$(jj diff "${DIFF_ARGS[@]}" --git "$CODE_FILESET" 2>/dev/null | awk '/^[+-]/ && !/^(---|\+\+\+)/ {n++} END{print n+0}')
...
**Bookmark mode:** Run `jj log -r "$BASE..<bookmark-ref>" --no-graph -T 'commit_id.short() ++ " " ++ description.first_line() ++ "\n"'` using the resolved fork point and remote or local bookmark from Stage 1. Use `<bookmark-ref>` (prefer `<bookmark>@<bookmark-remote>` with the mapped configured remote), not the raw argument, so a stale same-named local bookmark cannot supply the wrong history.

**Standalone (current change):** Run:

```
echo "BOOKMARKS:" && jj bookmark list -r @ && echo "CHANGES:" && jj log -r "$BASE..@" --no-graph -T 'commit_id.short() ++ " " ++ description.first_line() ++ "\n"'
```
...
1. Stage 1 gathered PR metadata (PR number or URL was provided as an argument, or `gh pr view` returned metadata for the nearest current bookmark).
...
All other persona subagents and RocketClaw local prompt assets use the platform's mid-tier model to reduce cost and latency. See the Spawning subsection below for the exact dispatch-time override.
...
Pass `{run_id}` and `{workspace_root}` to every persona sub-agent so they can write their full analysis to `{workspace_root}/.tmp/rocketclaw/code-review/{run_id}/{reviewer_name}.json`.
...
For each selected reviewer, read the corresponding local prompt asset from `references/personas/<reviewer-name>.md` and spawn a generic subagent using the subagent template. Do not use `subagent_type`, typed `Agent` names, or platform-level RocketClaw agent registration. Each persona subagent receives:
...
3. **Spawn validators with bounded parallelism.** One sub-agent per finding, dispatched independently using the validator template and the same bounded scheduler from Stage 4. Each validator receives:
   ...
   - `why_it_matters` when available — loaded from the per-agent artifact file at `{workspace_root}/.tmp/rocketclaw/code-review/{run_id}/{reviewer_name}.json`; omit when the file is absent or the artifact write failed. The validator proceeds without it, using the diff and cited code directly.
   - The full diff
   - The scope mode and remote head ref, mirroring the Stage 4 reviewer bundle: inject `<pr-scope-mode>local-aligned | pr-remote | bookmark-remote</pr-scope-mode>` and, when set, `<pr-head-ref>...</pr-head-ref>` or `<bookmark-head-ref>...</bookmark-head-ref>`. The validator template defaults to local-aligned workspace inspection when these are absent, so omitting them in `pr-remote`/`bookmark-remote` makes validators verify findings against the stale working copy — dropping valid findings or confirming false ones on the wrong revision.
   - Inspection access scoped by mode: in `local-aligned`, Read/Grep/`jj file annotate` the cited code, callers, guards, framework defaults, and history; in `pr-remote`/`bookmark-remote`, inspect via `jj file show -r <remote-head-ref> <path>` or the provided diff hunks only — do not Read/Grep workspace paths for files in scope.
...
**Commit when the pre-review working-copy change was empty.** Before applying, record whether `jj diff -r @ --summary` has any entries. The permanence gate is the **push**, not the local change boundary; `jj commit` is private and reversible with `jj undo`.

- **Empty before the review:** after applying and verifying, commit the fixes as one isolated change. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Apply the project's active instructions first, then the syntax established by `git log`, and use Go guidance only when compatible. Derive the message syntax dynamically; do not impose a fixed prefix, type, scope, subject, body structure, template, or example. Apply the composed description with `jj commit -m <message>`, returning the working copy to a new empty change.
- **Non-empty before the review:** apply but do **not** commit — the fixes remain in the user's in-flight working-copy change. The Applied section lists what changed.
...
2. **Applied (default mode only).** When Stage 5c applied fixes, list them first — before the findings — in an Applied section (see review output template); each entry carries `#`, file, the fix, and reviewer (a multi-file fix is one row with one `#`), then a one-line validation outcome (e.g. "pin tests 4 -> 6; suite 94 pass, lint clean") and change status (committed as an isolated change when `@` was initially empty, or left in the existing change when it was non-empty). Flag green-but-unverifiable edits (auth/contract/concurrency) prominently. Omit this section in `mode:agent` and when nothing was applied. Applied findings appear here, not in the severity tables.
...
Emit **one raw JSON object** as the primary response — a single bare JSON value, **no markdown code fence**. A leading ```` ```json ```` fence makes the response start with backticks and breaks naive `JSON.parse` consumers, so never wrap it. Also write `review.json` under `<workspace-root>/.tmp/rocketclaw/code-review/<run-id>/` with the same payload.
...
```json
{
  ...
  "scope": {
    "base": "<fork-point commit ID, pr:NNN marker, or base revision>",
    "bookmarks": ["<local bookmarks pointing to @>"],
...
5. **Protected artifacts are respected.** Discard any findings that recommend deleting or ignoring files in `docs/brainstorms/`, `docs/plans/`, or `docs/solutions/`.
...
After Stage 6, stop. Never push, open PRs, or file tickets from this skill. In default (interactive) mode, Stage 5c has already applied and (when the initial working-copy change was empty) committed the safe fixes; in `mode:agent` the review mutates nothing — the downstream caller and the user apply fixes, file tickets, or accept residual risk using the report and artifact.
...
- Include the run-artifact path when one was written: `<workspace-root>/.tmp/rocketclaw/code-review/<run-id>/`
...
Always write run artifacts under `<workspace-root>/.tmp/rocketclaw/code-review/<run-id>/`. Resolve `<workspace-root>` with `jj workspace root`; when that fails, use the current directory so the fallback remains local at `./.tmp/rocketclaw/code-review/<run-id>/`:
...
{
  "run_id": "<run-id>",
  "bookmarks": ["<local bookmarks pointing to @ at dispatch time>"],
  "head_commit_id": "<JJ commit ID for @ at dispatch time>",
  "verdict": "<Ready to merge | Ready with fixes | Not ready>",
  "completed_at": "<ISO 8601 UTC timestamp>"
}
```

Capture `bookmarks` with `jj bookmark list -r @` and `head_commit_id` with `jj log -r @ --no-graph -T 'commit_id'` at dispatch time (no in-skill fixes will land afterward).
