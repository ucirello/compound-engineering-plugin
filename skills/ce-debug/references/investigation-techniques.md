...
The test command should exit 0 for good, 125 to skip, 127 to abort, and any other non-zero status for bad. `jj bisect run` exposes the revision being tested as `$JJ_BISECT_TARGET`.

When you need manual judgment, pass a shell and exit it with the right status after testing the selected revision:
