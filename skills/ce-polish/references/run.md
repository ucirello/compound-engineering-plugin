...
If the user named a PR or bookmark, first locate whether a workspace already has that bookmark as its working copy. Use `jj workspace list` and `jj workspace root --name <name>` for membership; never read `.jj/` or `.git/`. Enter that existing workspace when the harness can; if it cannot, report the blocker and stop. Only update the current workspace's working copy when no other workspace owns the target. With no argument, stay in the current workspace.

Confirm the working-copy change `@` is named by a feature bookmark, not the repository's default bookmark. Report and stop when a safe feature-bookmark workspace cannot be reached; do not create another workspace behind the harness or move working-copy user changes.
