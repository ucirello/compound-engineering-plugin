...
- Write under `$(jj workspace root)/.tmp/rocketclaw/pov/`, falling back to `.tmp/rocketclaw/pov/` in the current directory when no JJ workspace root is available or the workspace path cannot be created, or under `docs/` when the user wants it kept; announce the absolute path. Do **not** introduce a new mandated `docs/` location — that store is deferred.
