...
**Load-bearing line provenance (conditional evidence).** When the finding's claim depends on line history, append one concise provenance evidence item from targeted `jj file annotate` and `jj log` on the cited line (shape: `provenance: <change-id> <author> <date> - <description first line>`). In remote scope, gather history against the reviewed revision, not a mismatched workspace. Provenance is additional evidence and never replaces the quote-the-line item. Omit it when the finding is fully justified from the diff and surrounding code alone; do not dump full-file annotation.
...
- You are operationally read-only. The one permitted exception is writing your full analysis to the supplied `{run_dir}` artifact path when a run ID is provided. You may also use non-mutating `jj` inspection commands and `gh` to gather evidence. Do not edit project files, change the working-copy revision, describe or commit changes, push, create PRs, or otherwise mutate repository state.
...
| `{pr_metadata}` | Stage 1 output | PR title, body, and URL when reviewing a PR. Empty string for bookmark or standalone workspace review |
