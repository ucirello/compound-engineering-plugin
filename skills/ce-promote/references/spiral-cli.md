...
Resolve the workspace root with `jj workspace root`, falling back to the physical current directory from `pwd -P` when unavailable, then read `<workspace-root>/.tmp/ce-promote/config.local.yaml` with the native file-read tool. If the file does not exist, treat the preference as absent.

If the contents have an **uncommented** top-level `ce_promote_spiral_optout: true` line, **skip Path 0** and go straight to Path B. Ignore commented lines: a commented example is documentation, not an opt-out, and a naive substring match would wrongly suppress the offer. Otherwise, offer setup.

### Ask

Use the available blocking-question capability. If none exists or the call errors, present the same options as a numbered list in chat and wait for a reply — never silently skip.
...
There is deliberately no separate "don't ask again" option: **dismissing is itself the opt-out.** A single first-run decline records the flag and the offer never recurs in this project. This is what keeps a per-ship skill from nagging — never make the user choose a special variant to stop being asked.
...
- **Draft directly without Spiral** — record the opt-out (below) so the offer never re-prompts in this project, then go to Path B. (A failed/abandoned **sign-in or install** attempt does NOT record the opt-out — only an explicit "draft directly" dismissal does — so a user whose auth didn't complete still gets one clean re-offer next run.)

### Record the opt-out (best-effort)

Resolve the workspace root with `jj workspace root`, falling back to the physical current directory from `pwd -P` when unavailable, then add `ce_promote_spiral_optout: true` as a top-level key to `<workspace-root>/.tmp/ce-promote/config.local.yaml`, using the native file-write/edit tool:

- **File already exists:** ensure an **uncommented** `ce_promote_spiral_optout: true` line is present — add one (or uncomment an example) unless an uncommented one already exists. A commented `# ce_promote_spiral_optout: true` does not count as present; leaving only the comment would let the comment-ignoring read path re-prompt next run.
- **File absent:** create it and its `.tmp/ce-promote/` directory, then make sure this workspace-local temporary state does not enter the working-copy change. If `.tmp/` is not already ignored, resolve the colocated Git root with `jj git root`, then resolve its `.git` entry without following an unsafe symlink: use the directory directly when `.git` is a directory, or parse the `gitdir:` target relative to the Git root when `.git` is a regular file; reject any other type or a resolved target outside the expected backing metadata. Append `.tmp/` to that metadata directory's `info/exclude`. Use the local exclude, not the tracked ignore file, so a drafts-only action does not dirty a tracked file. Confirm with `jj status` that the config is absent from the working-copy change; if it appears, remove only the newly created config and proceed to Path B without recording the opt-out. These are JJ operations; `.git` is referenced only to locate the colocated repository's local exclude safely, and all paths and commands must remain Git Bash-compatible.

If the root can't be resolved or any write fails, proceed to Path B anyway; the opt-out is a convenience, never a blocker.

After recording, confirm it in one line so the write isn't silent and the user knows how to undo it: state that the preference was saved under `.tmp/ce-promote/`, stayed out of the working-copy change, and can be undone by removing `ce_promote_spiral_optout`. Keep it to a single line; do not add product branding, creator, model, provider, tool, agent, runtime, workflow, generated-by attribution, badges, or standalone product decoration.
