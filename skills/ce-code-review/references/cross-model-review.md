...
The host resolves and sanctions one concrete route before egress; `scripts/cross-model-adversarial-review.sh` enforces that fixed route, applies read-only controls, captures schema-shaped JSON, and records identity receipts. Before dispatch it conservatively estimates diff tokens and file count. Oversized diffs are not inlined: the worker gives the peer the orchestrator's compact semantic review map and keeps the exact diff as a private, selectively readable artifact under the workspace run directory. Selective routes use `jj diff --from <base> --to @ <path>` under the existing read-only sandbox. A failed route writes no artifact and never switches recipients internally.
...
2. Scope is `local-aligned` or standalone: the working copy is the reviewed head. Skip in `pr-remote` / `bookmark-remote` because the peer would otherwise review the wrong revision.
...
- **Dispatch-infrastructure failure** — the runner or worker itself crashed: a non-zero exit before any job starts, a preflight/detach failure, or an unresolved `$SKILL_DIR`/script path. This is distinct from a gate-not-met skip. Recover a no-job-id preflight failure only at Stage 3d, before the local roster is materialized; do not re-attempt it during fold-in after the in-process fallback may have run. For a job-id-returned-then-failed crash, re-run the same resolved fixed route by hand while holding the target, model, `jj diff --from <base-ref> --to @` read scope, and adversarial persona brief fixed. Continue only while each failure is plausibly recoverable and the shared deadline holds. Stop on a repeated failure or exhausted deadline, report the lens as degraded, and never substitute a recipient, widen scope, or relax read-only controls.
...
The peer reviews the **current working copy** read-only against `jj diff --from <base-ref> --to @`. Reviewed code/diff content is sent to the resolved external provider. `CROSS_MODEL_PEERS` restricts which providers may receive content.
