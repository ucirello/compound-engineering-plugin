...
**Shared-branch topology** (`sweep_shared_branch: true`): before any source-side write, commit and push the state file. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Repository-local commit-message syntax from project instructions and `git log` ALWAYS wins when it differs from the Go guidance. The message should identify this lease-acquisition write of the state file; do not use a fixed prefix, type, scope, or subject template. If the state file is untracked, `jj file track` it. Then `jj commit -m "<message composed from the standards above>"` that fileset, and `jj git push --bookmark <name>` it (`<name>` from `jj bookmark list -r @`). A rejected push means another writer won the bookmark. `jj git fetch` and `jj rebase`, re-run `lease-acquire`, and if the lease is still not yours, back off (record `aborted-locked` and stop). Only once your lease is pushed and confirmed do you touch a source.
...
- `GIT_DIR="$(jj git root)" gh pr view "<validated-number>" --json mergedAt,baseRefName` (merged, base is the default branch), or `(cd "$(jj workspace root)" && jj log -r '"<validated-sha>" & ::"<default-branch-head>"' --no-graph -n 1)` (non-empty output means the SHA is an ancestor of the default-branch head).
...
- **Commit.** Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Repository-local commit-message syntax from project instructions and `git log` ALWAYS wins when it differs from the Go guidance. Apply compatible Go guidance to quality/clarity/structure without replacing repository-local syntax. The message should identify this feedback-sweep run (the plan plus repo-internal state); do not use a fixed prefix, type, scope, or subject template. Name ONLY `<root>/plans/feedback-sweep-plan.md` plus `<state>` when it is repo-internal (never other paths; machine-local state under `.tmp` is never committed). If those files are untracked, `jj file track` them. Then `jj commit -m "<message composed from the standards above>"` those filesets. A commit failure is reported, not fatal. In local-commit mode, never push. In shared-branch mode (`sweep_shared_branch: true`), `jj git fetch`, `jj rebase`, and `jj git push --bookmark <name>` the final change (`<name>` from `jj bookmark list -r @`).
