...
Dispatch context supplies: `{window}` (a date range, relative window, or since-revset), `{workspace-root}`, and `{run-dir}` (workspace-local temporary path for your output file).

## What to gather

Work through these sources for the window, cheapest first:

1. **Local revision activity** — use `jj log -r <revset>` with an explicit revset covering the window; plain `jj log` shows only its configured subset. Capture subjects, change IDs or commit IDs, dates, and authors, plus a stat-level view for substantial revisions. Group obviously related revisions in the same stack rather than listing them flat.
...
3. **Project docs** — plans, brainstorms, and solution docs added or modified in the window (`<root>/plans/`, `docs/brainstorms/`, `<root>/solutions/`, or wherever this workspace keeps them). These carry the *why* behind the revision activity — quote the decision or problem statement, not the whole doc.
