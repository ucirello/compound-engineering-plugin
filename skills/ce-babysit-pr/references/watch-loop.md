...
- **Report success only when** `all_checks_ok` is true, the actionable backlog and canonical `needs_human_residuals` set are empty, `mergeability_certain` is true, `merge_state_status == "CLEAN"`, `base_ref_blocker` and `stack_blocker` are null, `bookmark_currency_blocker` is null, and `unrequested_base_merge` is null. A red check, a human decision, an empty check rollup, uncertain merge/base/stack state, or an open/claimed currency item is not success: keep ticking until it clears or the time budget expires; or
...
- **A current decision is a hard blocker with one lifetime condition.** It makes the PR *not* merge-ready until the shared answer transition consumes it or any frozen source observation changes or disappears. The latter invalidates the whole decision automatically and makes surviving ordinary sources actionable; it never counts as the human's answer. An explicit source `--disposition open` may cancel a covering decision when the agent has separately proved the observation is no longer the question, but no stream owns a route-specific reopen rule.
...
```json
{
  ...
  "bookmark_currency_state": {
    ...
    "head_sha": "abc123",
    "items": { "<currency-key>": { "status": "BEHIND|DIRTY", "disposition": "open|claimed|confirmed", "host_bookmark_update_capability": true, "recovery_state": "claimed|mutation-observed|ambiguous|retry-authorized|retry-exhausted", "semantic_conflict_fingerprint": "<paths-and-conflict-identities>" } },
...
- **CI checks.** A failing check on the current head is actionable until you `mark --check <key>`. A typed decision enters the same canonical set through the shared residual mark, never through check-specific decision state. A new head commit ID clears dispatch state, invalidates check-sourced residuals, and re-evaluates the decision against the new commit.

- **Current-base merge identity.** `base.historical_oid` is historical PR metadata. `base.graphql_oid` and the independent exact host-ref API read must agree; a mergeable result also requires GitHub's generated test merge to name the observed base/head parents. A race whose only mismatch is the test merge's base parent degrades to `stale-computation` after `STALE_MERGE_COMPUTATION_SECONDS` of stable head/base identity and carries `merge_computation_stale: true`. Other race, pending, and probe-error results conservatively block. There is no secondary VCS transport fallback.
- **Bookmark currency.** `bookmark_currency` is the third attention stream. Its identity binds host-qualified live base repository/ref/commit ID, head commit ID, merge status, and route. "Mergeable against the current base" and "head contains the latest base" are separate: only `BEHIND`, `DIRTY`, a branch-protection requirement, or an explicitly selected always-current policy requires maintenance. Ordinary historical base movement with GitHub reporting `CLEAN` does not. `UNKNOWN` mergeability or any non-null `base_ref_blocker` emits no item. Managed stacks and manager/relationship `probe-error` are excluded; never mutate dependent bookmarks.
  ...
  Re-entry into `claimed` is reconciliation-only, never a direct resubmission. Record the observed recovery as `--currency-outcome mutation-observed`, `--currency-outcome proven-no-mutation`, or `--currency-outcome ambiguous`. Continue autonomously only through the recovery the recorded outcome leaves safe: reconcile an observed mutation toward confirmation. Exactly one retry follows only conclusive no-mutation proof and the engine backoff. Ambiguous recovery never retries or resubmits. Confirm the same exact item with `--currency-disposition confirmed`. Every terminal currency result with no safe autonomous continuation includes one complete `--residual-file` in that same locked mark with source `{ id: <currency_key>, kind: "currency" }`. Answers use the shared decision-ID transition above; bookmark currency has no separate answer state. Never transfer a claim or decision to a moved head/base observation.

`human_decisions`, `answered_human_decisions`, `ci_dispatched`, the thread and feedback dispositions, and `bookmark_currency_state` share the locked state journal. There is no second decision ledger or route-specific crash-recovery record. An unmarked review or CI item stays actionable, a current decision covers its exact observations, and a claimed currency item stays reconciliation-only until explicitly confirmed or handed to the shared decision boundary.
...
Sequential babysitting is available only while a fresh probe positively reports `manager_status == "confirmed"` for the active PR and the run posture is `stack-ready` or `stack-land` (or `target` after the one-time offer is accepted, which selects `stack-ready`). It uses one active PR target and one watcher, never one watcher per stack layer. On an authorized transition, stop the old watcher, re-read `gh stack view --json`, require the next PR to be the manager's immediate open entry and either non-draft or explicitly included by the user, run `jj git fetch --remote <remote>`, and move a clean JJ workspace to the next bookmark with `jj edit <next-bookmark>`. Initialize its state directory with `--continue-invocation`, the same invocation ID/start/budget, `--continue-dead-time-seconds <prior layer's invocation_dead_time_seconds>`, and the same posture. One fixed budget covers traversal. Under `stack-ready`, transition on quiescence: zero actionable backlog, no standing residual, and no delegate in flight. Landing under `stack-land` still requires the full settled gate. Arm the active watcher with `--downstack-pr <N>` for every open lower layer so new lower-layer work wakes `downstack-actionable`; also recheck downstack quiescence at transitions, immediately before mutation, and at readiness. Return to the lowest re-opened layer rather than writing to both concurrently. Loss of manager confirmation ends continuation.
...
- **Managed stack:** stale/unknown manager currency becomes `stack-blocked`. Record manager-ordered open bookmarks and remote bookmark revisions before delegation. After a target push, re-confirm manager ownership, require a clean JJ workspace, run `jj git fetch --remote <remote>`, and verify the target bookmark plus `<target-bookmark>@<remote>` still select the pushed revision. Rebase open dependent changes with `jj rebase`, advance only their bookmarks, and push each explicitly with `jj git push --bookmark <bookmark> --remote <remote>`. Re-fetch, preserve partial progress, and surface the first rejected/divergent layer. Preserve conflicts for inspection or restore only the proven JJ operation; never guess at stack membership or overwrite moved bookmarks.
...
- **Normal-base `DIRTY`:** prove jj bookmark-push authority without mutating, require a clean PR-head workspace, fetch with `jj git fetch --remote <remote>`, and create a disposable, undescribed `jj new <pr-head-revision> <base-revision>` merge preview. Fingerprint conflicted paths/materialized conflict identities, excluding the base ID. Restore the prior working-copy change and abandon the preview unless applying the proven mechanical repair.
  ...
  A mechanical resolution requires positive intent evidence with no reasonable alternative. Otherwise restore or abandon the preview and record the shared typed currency residual with options, tradeoffs, and a lean. For a mechanical repair, inspect the project's active instructions and change descriptions visible in `jj log` before composing the actual merge-change description. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. The project's active instructions and the description syntax observed at runtime in `jj log` win. Apply compatible Go guidance only to quality, clarity, and structure; it does not prescribe fixed syntax. Create the actual merge change with `jj new -m "<description-composed-from-runtime-conventions>" <pr-head-revision> <base-revision>`, resolve only the previewed conflict, validate, advance `<pr-bookmark>`, and run `jj git push --bookmark <pr-bookmark> --remote <remote>`.
...
- **No bookmark-push access / fork PR:** a delegated `jj git push` will fail. Detect that from the delegated skill's result, report it, and stop; the loop cannot make progress it has no permission to make.
...
`references/watch-loop.md` covers these in full. The non-negotiable ones: classify `pr_chain` and consume an exact claimed `bookmark_currency` item before any base-movement mutation; use the positive host-capability route for `BEHIND` and the clean-workspace exact-base route for bounded mechanical `DIRTY` repairs; semantic, stale, ambiguous, or unauthorized outcomes park rather than retrying or guessing; a pre-existing managed target currency problem becomes `stack-blocked`, never an ordinary base merge; after an owned target push, maintain a locally confirmed managed upstack through Step 7 and preserve JJ conflict evidence; external head movement -> re-snapshot and reconcile rather than clobber unrelated work; PR closed out from under the loop -> clean exit; `needs-human` feedback -> record it, keep doing independent CI work, never auto-resolve someone else's thread; no bookmark-push access / fork PR -> prove the appropriate route before mutation or park it; rate limits -> honor reset headers and back off.
