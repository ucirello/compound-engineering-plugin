...
After all actions are executed and the report is generated, handle describing the changes. Skip this phase if no files were modified (all Keep, or all writes failed).

### Detect JJ context

Before offering options, check:
1. Which bookmark is currently active (main/master vs feature bookmark)
2. Whether the working copy has other changes beyond what compound-refresh modified
3. Recent change descriptions to match the repo's style
...
Use path-limited JJ operations only for the files that compound-refresh modified — not other dirty files in the working copy. JJ has no staging area; use `jj split`, path-limited `jj commit`, or `jj squash` as appropriate.

### Interactive mode

First, run `jj bookmark list --tracked` to determine the current bookmark. Then present the correct options based on the result. Include only compound-refresh files regardless of which option the user picks; JJ has no staging area.
...
1. Describe only the compound-refresh changes on `{current bookmark name}` using path-limited JJ operations — other dirty files stay untouched
2. Don't describe
