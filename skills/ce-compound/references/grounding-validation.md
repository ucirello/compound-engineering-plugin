...
| Commit ID reachable from `@` only | Local-only commit; its ID may change when rewritten or squash-merged | Replace with the PR number |
...
If the script cannot be resolved on this platform, apply its checks manually at the same scope — scan the body for cited paths that don't exist, hex commit IDs, `Learning(s) N` / `{{…}}` scaffold, and broken relative links — and note in the run output that the check was manual. Do not silently skip.
...
2. MERGE-STATE CLAIMS — assertions that a change landed ("fixed in",
   ...
   revset reachability from the default branch's remote bookmark. Verdict: verified,
