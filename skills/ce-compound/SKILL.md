...
Coordinate multiple AI Assistant tasks in parallel to document a recently solved problem.
...
- `references/agents/session-historian.md` — skill-local synthesis prompt for optional session-history context (read only when the automatic probe escalates)
...
- If there are **multiple candidates in the same area**, ask the user via the Interaction Method whether to run a targeted refresh for that module, category, or pattern set
...
4. If not:
   ...
   b. Draft the smallest addition that communicates the three things. Match the file's existing style and density. The addition should describe the knowledge store itself, not the plugin — an agent without the plugin should still find value in it.
      ...
      ```
   c. In full interactive mode, explain why this matters: fresh sessions and collaborators without the plugin will not know to check `docs/solutions/` unless the instruction file surfaces it. Show the proposed change and location, then get consent via the Interaction Method before editing. In lightweight mode, output a one-line note and move on. In full headless mode, **do not edit instruction files**; report `Instruction-file edit: gap noted, not applied`.
