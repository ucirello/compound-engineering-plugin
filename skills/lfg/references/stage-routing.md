...
- **Planning** routes to `ce-plan` as a `plan_model:<alias>` carrier — the plan-authoring **model** (model elevation) — and, when a planning-stage directive names a harness, as a `plan_harness:<harness>` carrier alongside it. Example aliases: `fable`, `opus`. Example harnesses: `codex`, `claude`, `grok`, `cursor`, `composer`, `opencode`, `opencode2`. `opencode2` is its own harness, not compatible with `opencode`; do not reuse opencode flags or fall back from an `opencode2` selection to `opencode`. A model pin in `provider/modelname#variant` form preserves provider, model, and variant in `plan_model`. Never encode a harness name as `plan_model:<harness>`.
...
- `target`: exactly one of `codex`, `claude`, `grok`, `cursor`, `composer`, `opencode`, or `opencode2` — a **harness** name, never a model name. `opencode2` is not `opencode`; do not reuse opencode flags or fall back from an `opencode2` selection to `opencode`.
- `model`: the explicit model pin, otherwise `null`. A pin in `provider/modelname#variant` form preserves provider, model, and variant.
- `source`: caller-visible provenance identifying the current LFG instruction

A directive that names a bare **model** with no harness (e.g. "use fable", "with opus") is a model *pin*, not a target: encode it as the harness that serves that model family with the alias in `model` — a Claude-family model (`fable`, `opus`, `sonnet`, `haiku`) is `{"target":"claude","model":"<alias>"}`. Never put a model name in `target`; if you cannot map the named model to one of the listed harnesses, that is a routing-carrier blocker, not a `null` binding that silently drops the user's instruction.
