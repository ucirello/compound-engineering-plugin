...
This is **diff-scoped**, not whole-app exploration. You test what *this change* introduced or modified versus the trunk.
...
For every delegated JJ change description: Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Repository-local instructions and `git log` syntax always win; apply compatible Go quality guidance, and derive any prefix, type, scope, subject, or template dynamically instead of imposing a fixed one.

## Workflow

```
0. Scope        Pick the change, get onto it (offer a workspace), never edit the trunk change
1. Analyze      Diff the change vs trunk, understand every change
...
1. **Identify the target — keep PR identity; do not create or edit a JJ change yet.**
   ...
   - **Blank:** the target is the current JJ change (`@`).
2. **Refuse to run on the trunk — bookmark/blank targets only.** If a *bookmark-name or blank* target resolves to `trunk()`, stop — there is no diff to dogfood. A **PR is always diffable** (it has a base), so this check never applies to a PR target; never refuse `/ce-dogfood <number>` just because the PR's head bookmark happens to be named `main`.
3. **Decide isolation by what you're testing; let `ce-worktree` own the JJ workspace mechanics.** Do not re-derive workspace detection or creation here — `ce-worktree` handles existing-isolation detection, the harness-native tool, attaching to a revision, and reports its decision back. The only call this skill makes is *whether to ask for isolation at all*:
   - **Blank / current-change target:** do **not** isolate — dogfood in place. You are already on the change under test and the fix changes belong in its stack. If you are already in an isolated JJ workspace, continue there.
   - **A PR or a different named bookmark:** this is an existing revision to test without disturbing your current workspace. Offer isolation (platform's blocking question tool). On **yes**, invoke `ce-worktree` to isolate **that target revision** and act on its verdict; the primary workspace is never switched. On **no**, first run `jj status` and confirm if existing work would be disturbed, then run `jj git fetch` and create a working-copy child with `jj new <target-revision>`. For a named bookmark, resolve its local or `@<remote>` bookmark. For a same-repository PR, use its fetched head bookmark; for a fork PR, read its head repository and bookmark with `gh pr view`, add a dynamically named JJ Git remote with `jj git remote add <dynamic-remote> <fork-url>`, fetch it with `jj git fetch --remote <dynamic-remote>`, and use `<head-bookmark>@<dynamic-remote>` as the target revision.
...
Derive the trunk revision once, fetch it, then pull the full JJ diff against it and read it. Do not hard-code `main`; use the PR base when targeting a PR and `trunk()` otherwise.

```bash
# For a PR target, replace trunk() with the fetched base bookmark reported by:
...
2. **Drive it** with agent-browser — navigate, snapshot for interactive refs, click, fill, submit, follow the journey to its real end state:
   ...
   ROOT=$(jj workspace root 2>/dev/null || pwd); SCRATCH="$ROOT/.tmp/ce-dogfood"; mkdir -p "$SCRATCH"; agent-browser screenshot "$SCRATCH/<scenario>.png"
   agent-browser errors      # check console/page errors
   ```

   Write transient screenshots below `$(jj workspace root)/.tmp/ce-dogfood`, creating the directory first; when no JJ repo exists, fall back to local `.tmp/ce-dogfood`. Only copy a screenshot into the report's location if you intend to embed it in the final report. Do not edit `.gitignore`; the orchestrator owns it.
...
4. Describe the JJ change (use `ce-commit`). One logical fix per change. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Repository-local instructions and `git log` syntax always win; apply compatible Go quality guidance, and derive any prefix, type, scope, subject, or template dynamically instead of imposing a fixed one.
