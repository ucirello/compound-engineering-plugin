...
- Write ephemeral output under `$(jj workspace root)/.tmp/pov/`; if there is no Jujutsu workspace, use `.tmp/pov/` under the current project folder. Use `docs/` when the user wants it kept, and announce the absolute path. Do **not** introduce a new mandated `docs/` location — that store is deferred.
...
- Omit creator-credit metadata, visual credit marks, standalone product branding, and actor or display-name fields.
