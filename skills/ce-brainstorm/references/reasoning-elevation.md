...
Hand over the working context as **file paths the subagent reads itself**, never a re-narrated prose brief. Create **one private per-run handoff directory** at `<workspace-root>/.tmp/rocketclaw/ce-elevation/<run-id>/`; when `jj workspace root` cannot resolve a workspace, use the current directory's `.tmp/rocketclaw/ce-elevation/<run-id>/`. Claim the run directory without reusing an existing one, set mode `0700`, and write the prompt-file and every evidence file into *that* directory. On the Claude CLI route the worker grants the elevated model read access to only that one directory (via `--add-dir` on the prompt-file's parent), so the handoff files stay readable while unrelated workspace scratch is not exposed:
