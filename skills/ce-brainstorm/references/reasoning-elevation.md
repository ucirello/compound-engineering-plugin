...
Resolve the per-skill **model choice immediately before adapter selection**, so the decision reflects the current conversation rather than an intake snapshot. The value is a model alias (e.g. `fable`, `opus`), not a boolean. When `brainstorm_harness` is `opencode2`, the model is `provider/modelname#variant` — preserve provider, model, and variant; do not fall back to V1 `opencode`.
...
2. **Caller carrier** — when live user intent does not decide the choice, an automatic orchestrator may pass a structured `<per-skill-key>:<model-alias>` carrier (LFG passes `plan_model:<alias>` to ce-plan; the analogous `brainstorm_model:<alias>` to ce-brainstorm). Strip it from the request text and never reconstruct it from product prose. It is honored in pipeline / `disable-model-invocation` runs. The alias must match `^[A-Za-z0-9._-]{1,64}$`; when the resolved harness is `opencode2`, `/` and `#` are also allowed so `provider/modelname#variant` matches. A malformed carrier is absent, not guessed.
3. **Config** — otherwise use the per-skill keys: `plan_model` / `plan_harness` for ce-plan, `brainstorm_model` / `brainstorm_harness` for ce-brainstorm. Read them the **same way this skill's Phase 0.0 resolves `plan_output` / `brainstorm_output`**: reuse the workspace root already resolved, else run `jj workspace root`, then apply the ordinary-key rule (`config.local.yaml` then `config.yaml`). Reuse the Phase 0.0 reads if still in hand. Ignore commented (`#`-prefixed) lines. A model alias selects the model; missing / commented / invalid / no file selects none. The only valid harness value is `opencode2` (not `opencode`); any other value is invalid and continues to the next layer, then none. When the harness is `opencode2`, the model is `provider/modelname#variant` — alias validation allows `/` and `#`.
...
2. **opencode2.** Distinct from `opencode` and from the Claude CLI adapter — not compatible, not a fallback, and not V1 flags. When the resolved harness is `opencode2` and `opencode2` is on PATH, dispatch `opencode2 run --standalone --format json --file "$PROMPT_FILE" --model "$MODEL"` with cwd = the handoff directory (the parent CLI takes `[<directory>]`; there is no `--dir`), without `--auto` (cooperative read-only). `$MODEL` is the full `provider/modelname#variant` string. If the binary is missing, fall through to the remaining adapters.
...
Hand over the working context as **file paths the subagent reads itself**, never a re-narrated prose brief. Resolve the workspace with `jj workspace root`, falling back to the current directory outside JJ, then create **one private, exclusively named per-run handoff directory** under `<resolved-root>/.tmp/ce-elevation/` and write the prompt-file and every evidence file into *that* directory. On the Claude CLI route the worker grants the elevated model read access to only that one directory (via `--add-dir` on the prompt-file's parent), so the handoff files stay readable while other same-user workspace scratch is not exposed:
