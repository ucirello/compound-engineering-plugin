...
| commit ID reachable from `@` only | Local-only commit; ID may change when revisions are rewritten | Replace with the PR number |
...
| commit ID exists but is unreachable | Rewritten-away commit | Replace with the PR number |
