...
**Worktree backend** (JJ workspaces; script name kept for invocation):
...
3. Write the filled prompt to a temp file under the workspace `.tmp` root
4. Dispatch via Codex:
   ```bash
   tmp_dir="$(jj workspace root)/.tmp/rocketclaw/ce-optimize"
   mkdir -p "$tmp_dir"
   PROMPT_FILE=$(mktemp "$tmp_dir/optimize-exp-XXXXXX.txt")
...
3. **If `decide.mjs` returns `keep` for that winner: KEEP**
   - Finish the experiment change first so the winning diff exists as a real change before any merge or duplicate. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Repo-local syntax from project instructions and `git log` ALWAYS wins when it differs from Go guidance. Apply compatible Go guidance to quality/clarity/structure without replacing local syntax. Preserve these semantic constraints in the composed message: the spec name, the kept hypothesis, and that only mutable-scope paths are included.
   ...
   - From the experiment workspace:
     ```bash
     (cd "$experiment_workspace_root" && jj commit -m "<message composed from the standards above>" -- <mutable-scope-paths>)
     ```
   - Merge the finished experiment bookmark into the optimization bookmark from the default workspace:
     ```bash
     (cd "$(jj workspace root)" && jj new "optimize/<spec-name>" "optimize-exp/<spec-name>/exp-<NNN>")
     (cd "$(jj workspace root)" && jj bookmark set "optimize/<spec-name>" -r @)
     ```
...
4. **Check file-disjoint runners-up** (up to `max_runner_up_merges_per_batch`):
   ...
   - Otherwise abandon the duplicate (`jj abandon`), log it as "promising alone but neutral/harmful in combination" (outcome: `runner_up_reverted`), then clean up the runner-up's experiment workspace and bookmark
