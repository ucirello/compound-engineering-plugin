...
Run this once at the start of this invocation, before any subagent dispatch, and follow the directives it prints — except where one conflicts with this skill's own rules on asking the user questions, whether those rules are scoped to a non-interactive mode or apply in every mode, in which case this skill's rules win and no blocking question is asked. Run the fence exactly as written, as its own command: do not pipe or filter it (no `head`, `tail`, or `grep`), do not truncate its output, and do not bundle it into a batch with other commands. Its output opens with a `=== skill context` header and ends with `PLAN_CONTEXT_END`; if you received one of those lines without the other, the output was truncated — rerun the fence verbatim once. That recovery is the only rerun: otherwise do not rerun it within the same invocation; a later invocation of this or any other skill runs its own. If no Node runtime is available the skill proceeds unchanged.
...
- **Read** `docs_root` from `<workspace-root>/.rocketclaw/config.yaml` only (`<workspace-root>` = `jj workspace root`, with `pwd -P` fallback when the command fails). Do not read it from `config.local.yaml`. Unset -> `<root>` is `docs`, exactly as before.
...
- Workspace-relative file paths for the work being proposed (never absolute paths — see Planning Rules)
...
**Read config.** Resolve `<workspace-root>` at runtime by running `jj workspace root` with the shell tool; if it fails, use `pwd -P`. Then apply the ordinary-key rule below and read both files when they exist. If neither file exists, fall through to the defaults below.
...
**Resolve ordinary config keys from the two workspace files.**

- **Read** `<workspace-root>/.rocketclaw/config.local.yaml`, then `config.yaml` (`<workspace-root>` = `jj workspace root`, with `pwd -P` fallback). Missing files are skipped. Ignore rules do not change resolution.
...
- **Bug-shaped prompt** (user describes broken behavior — "fix the bug where X", error message, regression, "doesn't work"). Surface `ce-debug` as a route-out option alongside continuing with `ce-plan` whenever the bug surface is reachable (in cwd OR named repo found at another local path). Stay in `ce-plan` silently when the named code can't be found anywhere local — paper-planning is the only useful output for unreachable surfaces.
  ...
  **When the bug is at another local path (not cwd):**
  - Announce the target explicitly **before** any cross-workspace investigation: which path will be read AND where plan outputs will land (default: target workspace's `<root>/plans/`, not cwd's).
  - Default: proceed from the target workspace for both investigation and plan-write. The user can interrupt to redirect (switch context, paper-plan, abandon, etc.). No location menu — the announcement makes the cross-workspace nature visible, and the user can speak up if they want something unusual.
  - **After** announcing and proceeding, fire the standard ce-debug routing menu (continue with `ce-plan` vs switch to `ce-debug`) — same shape as the in-cwd case. Cross-workspace location and ce-debug skill routing are orthogonal decisions; do not merge them into a single question.

  Reading code at another path is fine in principle — that's just file access. The harm to avoid is silent operation on the wrong workspace, especially writing the plan doc somewhere it won't be discovered (a plan landing in `<other-workspace>/<root>/plans/` is a discoverability failure). The announcement requirement makes the target visible; defaulting to the target workspace for both investigation and outputs respects the user's stated intent; the orthogonal ce-debug menu keeps the skill-choice question clean.
...
- If `CONCEPTS.md` exists at the workspace root, read it — its definitions are the canonical names for domain entities, named processes, and status concepts. Plan with those terms rather than synonyms.
...
- Draft a clear, searchable title without a fixed description prefix, such as `<descriptive plan title>`.
- Determine the plan type from the project's active instructions and conventions already in context, then from description patterns visible in `jj log`; use a neutral intent label only when neither supplies a convention.
- Build the filename following the repository convention: `<root>/plans/YYYY-MM-DD-HHMM-<type>-<descriptive-name>-plan.md`
  ...
  - Example shapes: `2026-01-15-0915-<type>-user-authentication-flow-plan.md`, `2026-02-03-1642-<type>-checkout-race-condition-plan.md`
...
Break the work into logical implementation units. Each unit should represent one meaningful change that an implementer could typically land as an atomic JJ change.
...
- Do not include JJ command choreography, change descriptions, commit messages, or exact test command recipes
...
Surface plan-time call-outs to the user before Phase 5.2 writes the plan to disk — the latest cheap moment to catch plan-time scope errors. The brainstorm already validated WHAT to build; this phase surfaces HOW the plan will execute on the forks that matter.
...
1. **Start `ce-work`** - Build and ship the plan in this session — subagent-driven development with simplification, code review, and JJ changes. Implementation-ready code plans only.
