...
3. **Check whether the issue already appears fixed on trunk.** Once you know the affected surface, use read-only `jj log` / `gh` on that area (files, routes, components the symptom touches) to see whether a recent change or merged PR already addresses it. Report this as a field in your finding so the orchestrator does not re-file resolved work.
...
- **Already fixed on trunk?** -- `yes` / `no` / `unclear`, with the change or PR reference you checked, or a note that you could not determine it.
