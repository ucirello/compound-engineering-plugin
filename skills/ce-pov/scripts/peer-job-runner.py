...
Environment overrides (defaults in parentheses):
  CE_PEER_JOBS_ROOT         base dir (<workspace-root>/.tmp/peer-jobs)
...
JJ workspace's `.tmp`, or the physical current directory's `.tmp` when no JJ
workspace is available. Every read of job state opens the file first (no-follow) and
...
POSIX path is behaviorally unchanged:
  ...
  jobs root defaults under the workspace-local `.tmp`, with a
            hardened owner-private ACL.
...
DEFAULT_ROOT = os.path.join(DEFAULT_SCRATCH_ROOT, "peer-jobs")
