...
1. **Resolve the target directory and extension.**
   ...
   - **Otherwise (no repo, or elsewhere with no `docs/ideation/`):** write into the run's workspace-local scratch area — the `<scratch-dir>` resolved in Phase 1 (`.tmp/ideate/<run-id>/`). Do **not** write directly into the user's current working directory, and do **not** create a `docs/ideation/` tree for a subject unrelated to the repo. Announce the absolute path and note it is temporary scratch that the user should move to keep permanently.
...
- the deliverable was written automatically in both modes (Phase 4) — to `docs/ideation/` when present, else the workspace-local `.tmp/` area, never directly in the user's CWD
