...
3. **Unpushed changes vs base bookmark.** If the working copy is clean, review `jj diff $(jj log @ <base>)..@` where `<base>` is the default branch (main or master).
...
- Prefer `jj show <remote-head-ref>:<path>` when `<pr-head-ref>` or `<branch-head-ref>` is provided in context.
