...
|---|---|---|
| E1 native restraint | `ce-work <root>/plans/feature.md`; no directive, caller binding, or enabled config | Native inline/subagent engine; nothing leaves the host and `ce-work` creates no workspace for an ordinary synchronous unit; `ce-work` itself still runs the finishing steps. |
...
| E18 transactional failure | Synthetic transport applies, but canonical verification fails | Restore the exact pre-fold working-copy parent and paths under the lock before sibling, retry, resume, or fallback; preserve the external result and block if exact restoration is unprovable. |
...
| E20 linked-checkout sibling | `ce-work` is itself running in an existing linked workspace and selects external implementation for one unit | Create a new detached **sibling** with `jj workspace add`, place it under `$(jj workspace root)/.tmp/rocketclaw/ce-work/<run-id>/` rather than beneath the active checkout, base it at the recorded clean canonical change, and keep canonical fold-in host-owned. Do not reject the route merely because the active checkout is already a workspace, and do not create a nested workspace. |
...
| E29 clean packet and shell argv | A clean linked checkout needs a packet source, and its V1 command is `test "$(cat delegated.txt)" = "expected"` | Write the packet source directly to `$(jj workspace root)/.tmp` outside the checkout files. At integration and plan-wide verification, recognize `$(...)` as shell syntax and use an explicit pipefail-capable shell on the first attempt; do not create repository scratch or pass the expression as literal direct argv. |
...
| E40 sandboxed worker no-commit | A Codex or Cursor unit has finished files and scoped checks in its detached workspace; the worker is about to `jj commit`/`jj describe` | Do not instruct the worker to describe or finish the change. Leave the working copy undescribed and treat completion as files plus scoped checks. Host `terminalize` snapshots the tree. A Codex sandbox `EPERM` on a socket bind or peer-credential probe is not proof the host lacks the capability. |
