...
- **Current bookmarks** — run `jj log --no-graph -r @ -T 'local_bookmarks.map(|b| b.name()).join(",") ++ "\n"'`. Use the returned local bookmark names to filter session history in Phase 1. If it is empty or exits non-zero (not a JJ workspace), skip bookmark filtering.
- **Workspace root** — run `jj workspace root`. Use it as the session-history workspace filter in Phase 1. If it exits non-zero (not a JJ workspace), fall back to the working directory.
...
#### 4. **Session History** (internal flow after launching the parallel block — automatic in Full mode, including non-interactive)
   ...
   **Session-history payload — keep tight.** A long, keyword-rich payload licenses widening. Use this shape:
   ...
   - **Session context** (only if the values resolved cleanly above; otherwise omit): repo name, current JJ bookmarks.
   ...
   **Discovery pipeline.** Infer the scan window from the problem topic, starting with 7 days. Run discovery and metadata extraction:
   ...
   if [ -f "$SKILL_DIR/scripts/session-history/discover-sessions.sh" ] && [ -f "$SKILL_DIR/scripts/session-history/extract-metadata.py" ]; then
     ...
     WORKSPACE_ROOT=$(jj workspace root 2>/dev/null || pwd); REPO_NAME=$(basename "$WORKSPACE_ROOT"); SCAN_DAYS="7"; bash "$SKILL_DIR/scripts/session-history/discover-sessions.sh" "$REPO_NAME" "$SCAN_DAYS" --cwd "$WORKSPACE_ROOT" | tr '\n' '\0' | xargs -0 "$PY" "$SKILL_DIR/scripts/session-history/extract-metadata.py" --cwd-filter "$WORKSPACE_ROOT";
