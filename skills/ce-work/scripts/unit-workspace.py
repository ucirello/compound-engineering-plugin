#!/usr/bin/env python3
"""CLI entrypoint for the Jujutsu external-unit workspace controller."""
...
from unit_workspace_integration import cmd_composition_acquire, cmd_composition_release, cmd_mark_composed, cmd_mark_described, cmd_mark_verified, cmd_preflight, cmd_restore, cmd_wave_advance
...
def build_parser():
    ...
    p = sub.add_parser("init"); p.add_argument("--run-id", required=True); p.add_argument("--repo", required=True)
    source = p.add_mutually_exclusive_group(required=True); source.add_argument("--plan"); source.add_argument("--prompt-brief")
    p.add_argument("--plan-digest"); p.add_argument("--prompt-digest"); p.add_argument("--binding-json", default="{}"); p.add_argument("--egress-json", default="{}")
    p = sub.add_parser("checkpoint-plan"); p.add_argument("--run-id", required=True); p.add_argument("--change-description", required=True)
    p = sub.add_parser("prepare"); p.add_argument("--run-id", required=True); p.add_argument("--unit-id", required=True); p.add_argument("--base", required=True); p.add_argument("--packet", required=True); p.add_argument("--attempt-id", default="attempt-1"); p.add_argument("--activity-posture", choices=("incremental", "hard-only"), default="hard-only"); p.add_argument("--dependency", action="append", default=[]); p.add_argument("--wave-id"); p.add_argument("--wave-position", type=int, default=0)
    for name in ("record-job", "authorize-dispatch"):
        p = sub.add_parser(name); p.add_argument("--run-id", required=True); p.add_argument("--unit-id", required=True); p.add_argument("--attempt-id", required=True); p.add_argument("--job-id", required=True)
...
COMMANDS = {
    "init": cmd_init, "checkpoint-plan": cmd_checkpoint_plan, "prepare": cmd_prepare,
    "authorize-dispatch": cmd_authorize_dispatch, "record-job": cmd_record_job,
    "sync-job": cmd_sync_job, "terminalize": cmd_terminalize,
    "composition-acquire": cmd_composition_acquire, "preflight": cmd_preflight,
    "mark-composed": cmd_mark_composed, "mark-verified": cmd_mark_verified,
    "mark-described": cmd_mark_described, "integrate": cmd_integrate,
    "verify-run": cmd_verify_run, "wave-advance": cmd_wave_advance,
...
def main(argv):
    ...
    try:
        ...
        print("UNREADABLE"); sys.stderr.write(f"unit-workspace: {exc}\n"); return 4
    except Operational as exc:
        print(exc.word)
        if exc.detail: print(json.dumps(exc.detail, sort_keys=True, separators=(",", ":")))
