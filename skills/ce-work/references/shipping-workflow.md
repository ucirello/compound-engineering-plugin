...
1. **Run Core Quality Checks**
   ...
   # Use linting-agent before publishing a bookmark with jj git push
   ```

2. **Simplify** (conditional — separate from code review)

   Before code review, invoke **`ce-simplify-code`** when `jj diff --git` is non-mechanical and large enough to benefit (default: **>=30 changed lines**). Skip when the change is purely mechanical (formatting, dependency bumps, lint-only fixes, generated artifacts).

   This step refines reuse, quality, and efficiency in the **current JJ change** so any later review sees cleaner code. It is not a substitute for code review.
...
4. **Residual Work Gate** (REQUIRED when `ce-code-review` ran and left actionable residuals)
   ...
   **Interactive sessions:** Ask the user using the platform's blocking question tool (`AskUserQuestion` in Claude Code with `ToolSearch select:AskUserQuestion` pre-loaded if needed, `request_user_input` in Codex, `ask_question` in Antigravity CLI (`agy`), `ask_user` in Pi (requires the `pi-ask-user` extension)). Fall back to numbered options in chat only when the harness genuinely lacks a blocking tool. Never silently skip the gate.
   ...
   - `Apply/fix now` — load `references/review-findings-followup.md`, dispatch batched fix subagents for remaining eligible findings, run tests, and finalize a JJ change if needed; optionally re-run `ce-code-review` only after `jj diff --git` changed materially. If a change description is composed or edited, apply this guidance: Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. At that site, inspect the project's active instructions and run `git log`; the project's active instructions take precedence over `git log`, and both take precedence over compatible Go guidance. `jj log` may provide supplemental context but never replaces `git log`. Derive the message syntax dynamically; do not impose a fixed syntax, prefix, type, scope, subject, body structure, template, or example.
...
1. **Prepare Validation Context**
   ...
   Do not launch a dedicated evidence-capture workflow. Modern harnesses provide their own browser, screenshot, terminal recording, and artifact capture tools; use those directly only when the user asks or when the artifact already exists.
...
2. **Finalize JJ Changes and Create Pull Request**
   ...
   Load the `ce-commit-push-pr` skill to handle JJ change description/splitting, bookmark creation or movement, `jj git push`, and PR creation. The skill must retain `gh`/GitHub integration with `GIT_DIR="$(jj git root)"` while following JJ semantics. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. At that site, inspect the project's active instructions and run `git log`; the project's active instructions take precedence over `git log`, and both take precedence over compatible Go guidance. `jj log` may provide supplemental context but never replaces `git log`. Derive the message syntax dynamically; do not impose a fixed syntax, prefix, type, scope, subject, body structure, template, or example. Do not add product attribution, badges, bylines, branded prefixes, generated-by footers, tool/model names, or identity metadata to the change description or PR.
   ...
   If the user prefers to finalize JJ changes without creating a PR, load the `ce-commit` skill instead and require this guidance: Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. At that site, inspect the project's active instructions and run `git log`; the project's active instructions take precedence over `git log`, and both take precedence over compatible Go guidance. `jj log` may provide supplemental context but never replaces `git log`. Derive the message syntax dynamically; do not impose a fixed syntax, prefix, type, scope, subject, body structure, template, or example.
...
- [ ] JJ change descriptions satisfy this guidance: Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. At that site, inspect the project's active instructions and run `git log`; the project's active instructions take precedence over `git log`, and both take precedence over compatible Go guidance. `jj log` may provide supplemental context but never replaces `git log`. Derive the message syntax dynamically; do not impose a fixed syntax, prefix, type, scope, subject, body structure, template, or example
- [ ] PR description includes Post-Deploy Monitoring & Validation section (or explicit no-impact rationale)
- [ ] Simplify: `ce-simplify-code` when `jj diff --stat` shows >=30 changed lines (or skipped with reason)
- [ ] Code review: `ce-code-review` ran against `jj diff --git` (self-sized), or skipped (mechanical change / unavailable — noted in summary); residuals handled via the Residual Work Gate
- [ ] PR description includes summary, testing notes, and evidence when captured

## Code Review

Single portable path: **`ce-code-review`** self-sizes from `jj diff --git` (lite roster for small low-risk code-only changes, full roster otherwise). No harness-native review detection, no escalation tiers — the size/sensitive-surface judgment lives inside `ce-code-review` now.

**Skip** only for a purely mechanical change (formatting, dep-bumps, lint-only, generated). Everything else is reviewed through `jj diff --git`.
...
**If `ce-code-review` can't run** (no subagent dispatch): interactive → harness-native review if present, fix inline; non-interactive → skip-with-note + manual `jj diff --git` scan in Final Validation. Never silently ship a non-mechanical change unreviewed.
