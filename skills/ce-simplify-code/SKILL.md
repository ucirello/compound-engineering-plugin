...
2. **Otherwise, in a JJ repository**, default to the diff between the current change and its base bookmark (e.g., `jj diff --from main` or the repository's configured integration bookmark). This covers the common case of "simplify everything I've added on this feature bookmark before opening a PR." If no base bookmark is clear, fall back to the current working-copy change (`jj diff`).
