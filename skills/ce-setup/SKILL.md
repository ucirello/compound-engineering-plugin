...
4. Check whether `.compound-engineering/config.local.yaml` exists and, if it does, whether the path is covered by the repo ignore rules.
...
- `.compound-engineering/config.local.yaml` exists but is not safely repo-ignored
...
### Step 7: Ensure Local Config Is Repo-Ignored

If `.compound-engineering/config.local.yaml` exists and is not covered by the repo ignore rules, offer to add:
