...
- Analyze only. You are read-only except for the single write to {scratch_artifact_path}.
  ...
  project files, change bookmarks, commit, push, or open PRs. Pair every gh invocation
  that talks to the underlying git repo with GIT_DIR=$(jj git root) (run jj from the
  workspace root).
