...
- If the current bookmark is the default branch, ask whether to create a feature bookmark first using the platform's blocking question tool (see Phase 2 for the per-platform names). To detect the default branch, compare against `main`, `master`, or forge metadata when available. Default to creating one; derive a name from the bug, run `jj new <base>`, then `jj bookmark set <name>`. On any other bookmark/change, proceed.
