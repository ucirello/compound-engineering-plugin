...
GitHub only — **including GitHub Enterprise**, which the mode references handle by deriving the host and targeting it on every call rather than defaulting to `github.com`. Pair every `gh` invocation that talks to the underlying git repo with `GIT_DIR=$(jj git root)` (and when a workspace cwd is in play, also run `gh` with that workspace root as cwd). Before fetching, confirm the repo is GitHub: `GIT_DIR=$(jj git root) gh repo view` succeeding is the positive signal, and it covers a GHE host transparently. If it fails, check the remote — a `gitlab.*` or `bitbucket.*` host means an unsupported forge, so stop and tell the user this skill is GitHub-only rather than proceeding into `gh` calls that will error confusingly.
