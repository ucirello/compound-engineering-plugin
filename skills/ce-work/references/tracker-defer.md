...
2. **Probe the named tracker when one was found.** For forge issues, run `gh auth status` and `gh repo view --json hasIssuesEnabled`. For Linear or other connector/MCP-backed trackers, first discover available tools via the platform's tool-discovery primitive rather than assuming absence from an unloaded tool, then verify the discovered tool is responsive. For API-backed trackers, verify credentials wherever the platform exposes them (environment, connector auth, or a documented secrets location), not only shell environment variables. Set `named_sink_available` from the probe result.
3. **Probe the forge-issues fallback to compute `any_sink_available`.** Even when the named tracker was found and probed, `gh` matters for the `no_sink` bucket decision so that a run with no documented tracker but working `gh` still offers Defer.
   ...
   - Otherwise, probe forge issues via `gh auth status` plus `gh repo view --json hasIssuesEnabled` (skip if already probed in step 2). If it works, `any_sink_available = true`.
...
2. **Forge issues via `gh`** — when `gh auth status` succeeds and the current repository has issues enabled (`gh repo view --json hasIssuesEnabled` returns `true`)
...
| Forge issues | `gh issue create` | Pass the repository explicitly. Use `--label` for severity tags when labels exist; omit `--label` if the repository has no matching labels. Fall back to a label-less issue on first failure. | Markdown | `--label P0` / `--label P1` / etc. when labels exist |
