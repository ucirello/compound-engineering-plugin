...
- **Cluster by root assumption.** If one source makes the same kind of claim across several threads and you find it doesn't hold in one place, scrutinize the siblings: a systematically-wrong premise produces a cluster of plausible-but-wrong findings. This is the single biggest advantage of judging centrally instead of per-isolated-agent.
...
- **The fix would undo a *deliberate* design choice (evidence-gated, rare)** -- the finding is otherwise correct and low-risk, but implementing it would reverse a design/product decision the author made on purpose, *and* competent engineers could reasonably disagree about which is right. This does **not** fire just because a change alters behavior -- every fix alters behavior. It fires **only when both** of these hold, and you can name them:
  1. **Positive evidence of intent** -- a concrete artifact showing the current behavior is a choice, not an accident: a comment/docstring stating it, a test asserting it, a PR/change rationale, or a sentinel/guard that only makes sense as a decision. "The code currently does X" is **not** evidence (all code does something). If you cannot point to a specific artifact, there is no deliberate choice to protect -> **fix it**.
...
Compose these now under the Composition Standard -- you have the evidence. Every reply quotes the specific sentence being addressed, not the whole comment if it is long, followed by prose that carries the verdict's semantics without a mandatory label:

- For `replied`, directly answer the question, explain the design decision, or state why the immaterial change is unwarranted.
- For `not-addressing`, state the concrete evidence that the concern does not hold or is no longer relevant.
- For `declined`, state the specific harm the proposed change would cause.
- For `needs-human`, write a natural acknowledgment in the project's voice that identifies the unresolved tradeoff without generic generated-text framing. Do not resolve the thread.

The **decision_context** is presented to the user rather than posted in full. Compose it under the Composition Standard and preserve these facts in whatever structure local conventions support: the specific quoted ask, investigation findings with code locations, the exact ambiguity requiring a decision, concrete options and tradeoffs, and a recommendation with its basis or the missing context that would change it. Do not force fixed headings or option labels.
