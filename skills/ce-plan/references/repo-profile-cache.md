...
- `<root-sha>` = lexicographically-first commit ID from `jj log -r 'roots(::@- & ~root())' --no-graph -T 'commit_id ++ "\n"'` — the repository identity.
- `<head-sha>` = the commit ID from `jj log -r @- --no-graph -T commit_id` — the working-copy parent's state.

JJ workspaces in the same repository use their workspace-root-local cache. Lookup is JJ metadata only; on a hit, only this one file is read.
