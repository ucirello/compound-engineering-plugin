...
5. Do NOT describe or commit (the orchestrator will describe the winning diff before integration if this experiment succeeds)
...
- For Codex dispatch: write the filled template to a temp file under the workspace `.tmp` (`mktemp "$(jj workspace root)/.tmp/optimize-exp-XXXXXX.txt"` with fallback `mktemp ".tmp/optimize-exp-XXXXXX.txt"`) and pipe via stdin (`cat "$PROMPT_FILE" | codex exec --skip-git-repo-check - 2>&1`).
