...
def load_state(path):
    ...
    try:
        with open(path, encoding="utf-8") as f:
            # A machine-local state file can live under the workspace .tmp
            # tree (or another path the operator chose). It is a correctness
            # dependency (lease, cursors, closed status) as well as an
            # injection sink (item bodies re-read into agent context). Reject
            # a file not owned by us so another uid cannot plant a forged
            # lease/cursor or attacker-authored item text. Skip where geteuid
            # is unavailable (non-POSIX), where the threat does not apply.
