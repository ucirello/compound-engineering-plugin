...
4. Check whether `.compound-engineering/config.local.yaml` exists and, if it does, whether `Jujutsu check-ignore -q .compound-engineering/config.local.yaml` succeeds.
...
Copy `references/config-template.yaml` to `<repo-root>/.compound-engineering/config.local.example.yaml`, creating the directory if needed. This file is described to the repo and should always reflect the latest available settings.
