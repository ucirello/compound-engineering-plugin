...
5. Review context: intent summary, file list, diff, scope mode (`local-aligned` | `pr-remote` | `bookmark-remote`), and reviewed remote revision (`PR_HEAD_REVISION` or `<bookmark-head-revision>`) when set
...
8. **For `data-migration` only:** the resolved review-base JJ revision from Stage 1, wrapped in `<review-base>` so schema drift checks never assume a bookmark
...
Read-only here means **non-mutating**, not "no shell access." Reviewer sub-agents may use non-mutating `jj` inspection and `gh pr view`. Relevant JJ tools include `jj diff`, `jj file show`, `jj file annotate`, and `jj log`; consult matching local help or the official JJ references when syntax is uncertain. History-dependent findings carry one concise change-ID provenance line in addition to quoted code. In **`pr-remote`** or **`bookmark-remote`** scope, inspect changed files at the supplied reviewed revision or use diff hunks; do not Read/Grep workspace paths for files in scope. They must not edit project files, change the working-copy revision, describe or commit changes, push, create PRs, or otherwise mutate repository state.
