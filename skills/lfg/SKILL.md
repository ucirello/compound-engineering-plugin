...
3. Invoke the `ce-simplify-code` skill on the current JJ diff.
   ...
   Do not describe/split the change in this step. `ce-simplify-code` leaves its changes in the working tree; step 4's review scopes the working tree, and step 8's `ce-commit-push-pr` persists whatever remains. Describing here would sweep any still-unfinished `ce-work` edits into a misleading `refactor` change and could stall on a tree that never goes clean.
...
6. **Autonomous residual handoff** (only when step 4 reported one or more actionable `downstream-resolver` findings not applied in step 5; skip when it reported `Actionable findings: none.`)
   ...
   6. If no open PR exists, create a tracked fallback file at `docs/residual-review-findings/<bookmark-or-change-id>.md` containing the composed section and the source PR-review run context. Include only that file in the residual-record change, describe it with `docs(review): record residual review findings`, and push the current bookmark **when a remote is configured** (per the shipping precondition). If the bookmark is already tracked, run `jj git push`. If no tracked bookmark exists but a remote is configured, resolve a writable remote dynamically: prefer `origin` when present, otherwise use `jj git remote list` and choose the first configured remote, then run `jj git push --remote <remote> --bookmark <bookmark>`. If there is no remote at all, do not push — the local described change is the durable sink. This is the durable no-PR sink. Do not output DONE until the residual findings are durable: either the existing PR body has been updated, or this fallback file change has been made (pushed when a remote exists, described locally when none). A push that fails when a remote exists is a stop-and-report; never retry a push, or block DONE, when no remote exists.
...
8. Invoke the `ce-commit-push-pr` skill with `mode:pipeline`.
   ...
   This describes any remaining changes, pushes the current bookmark, and opens a pull request — non-interactively, per the mode token. If it prints a `New concepts:` trailer after the PR URL, record the concept name(s) for step 10. If step 6 already opened a PR (check with `gh pr view --json number,url,state 2>/dev/null`), skip PR creation but still describe and push any remaining changes. **Per the shipping precondition, when no remote is configured, do NOT invoke `ce-commit-push-pr` if it would push unconditionally. Instead describe any remaining changes locally yourself (`jj describe -m "<message>"`) and skip the push and PR creation entirely.**
...
9. **CI watch and autofix loop** (only when an open PR exists for the current bookmark)
   ...
   4. Describe only the fix you made, then push:

      ```bash
      jj diff -- <changed-files>
