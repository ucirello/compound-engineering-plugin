...
1. If a PR number or bookmark name was provided, activate it with `gh pr checkout <number>` for PRs or `jj edit <bookmark>` / `jj new <bookmark>` as appropriate for bookmarks (probe for existing JJ workspaces first).
2. If blank, use the current change.
3. Verify the current change is not directly on `main`, `master`, `main@origin`, or `master@origin`.
...
- When the user says they're done → describe the fixes and stop
