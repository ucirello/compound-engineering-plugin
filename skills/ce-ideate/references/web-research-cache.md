...
- **Repo mode:** the focus hint plus a stable repo discriminator. This keeps the cache key meaningful when focus is empty — two bare-prompt invocations in the same repo legitimately share research, while copied or relocated cache material remains distinguishable. Resolve the discriminator with this fallback chain and hash the result (first 8 hex chars of sha256 is sufficient):
    1. Match the provider repository URL to the normalized nonempty URLs from `jj git remote list` and hash the unique match. When provider identity is unavailable, normalize and sort the complete set of nonempty remote URLs and hash that set without privileging any remote name.
