...
3. **Unpublished changes vs base bookmark.** If the working-copy change is empty, review `jj diff --from <base>` where `<base>` is the resolved default bookmark.
...
In `pr-remote` / `branch-remote` scope these tiers inspect the working copy, which is not the reviewed head — apply the Remote scope rules above (`jj file show` / `jj file search -r <remote-head-ref>`) instead of local search.
