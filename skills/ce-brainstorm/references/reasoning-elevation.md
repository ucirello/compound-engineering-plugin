...
3. **Config** — otherwise the per-skill key: `plan_model` for ce-plan, `brainstorm_model` for ce-brainstorm. Read it the **same way this skill's Phase 0.0 resolves `plan_output` / `brainstorm_output`**: reuse the repo root already resolved, else run `jj workspace root`, then apply the ordinary-key rule (`config.local.yaml` then `config.yaml`). Reuse the Phase 0.0 reads if still in hand. Ignore commented (`#`-prefixed) lines. A model alias → elevate to it; missing / commented / invalid / no file → off.
...
Hand over the working context as **file paths the subagent reads itself**, never a re-narrated prose brief. Create **one private per-run handoff directory** under `$(jj workspace root)/.tmp/rocketclaw/ce-elevation/<run-id>/`, falling back to `$PWD/.tmp/rocketclaw/ce-elevation/<run-id>/` when no Jujutsu workspace resolves, and write the prompt-file and every evidence file into *that* directory. Reserve a unique run ID, reject symlinks or foreign-owned paths, and create the directory with mode `0700`. On the Claude CLI route the worker grants the elevated model read access to only that one directory (via `--add-dir` on the prompt-file's parent), so the handoff files stay readable while the rest of the workspace scratch root — other same-user scratch and credentials — is not exposed:
