...
(`<workspace-root>/.tmp/rocketclaw/ce-brainstorm/<run-id>/grounding.md`, or the same path under the current directory when no JJ workspace exists) — it gives
...
independent code review and applied fixes, describe the JJ change, run
`jj bookmark set`, run `jj git push`, open a PR, and watch CI to green. Do not also start a `/goal` or load `ce-work` directly — `lfg`
orchestrates them. Unlike a goal tool, `lfg` is host-agnostic: it works wherever
skills run (plus `jj git fetch`, `jj git push`, and `gh` for the PR/CI tail,
which it guards when absent).
