...
while (tasks remain):
  ...
  - **If any part of the unit's completion depends on out-of-repo state** (a console setting, DNS record, CMS object, live-system rows), that part has no Jujutsu-derived completion signal: decide it from the observed state of the deliverable, never from a clean tree or a tracker write. Mark it complete only when that state is already satisfied; execute only when it is observably unsatisfied and re-applying is safe or the user has authorized it; otherwise ask or block.
...
# 2. Finish only files related to this logical unit (JJ auto-snapshots tracked files; do not `jj file track` a whole tree)
...
**Note:** Incremental changes use the composed message without attribution footers. The final Phase 4 handoff does not add product branding to the PR.
