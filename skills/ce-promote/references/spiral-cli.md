...
- **Read** `<repo-root>/.rocketclaw/config.local.yaml`, then `config.yaml` (`<repo-root>` = the absolute root returned by `jj --ignore-working-copy workspace root`). Run JJ commands with that absolute workspace root as their working directory after root discovery. Missing files are skipped. Ignore rules do not change resolution.
- **Win** with the first active (non-commented) value. For scalars, empty is unset; an invalid value continues to the next layer, then the skill default. For lists and maps, a present key — including an empty list or map — replaces the whole key.
- **Do not** use this rule for `docs_root` — that key is `config.yaml` only.
<!-- ce-config-layers:end -->

On OpenCode V2, use this same configuration precedence through native file tools. This skill has no subagent or alternative-harness dispatch; invoking `ce-noslop` remains a skill call in the current harness.
...
Resolve the repo root, then set `ce_promote_spiral_optout: true` as a top-level key in `<root>/.rocketclaw/config.local.yaml`, using the native file-write/edit tool. Before writing, confirm with `jj --ignore-working-copy file list 'root-file:.rocketclaw/config.local.yaml' -T 'path ++ "\n"'` that the file is not already tracked: only a successful command with empty output permits persistence. Otherwise leave it untouched and continue to Path B; do not alter tracked project state during drafting.

- **File already exists:** update the active top-level key in place, or add it if absent, so the file has one unambiguous `ce_promote_spiral_optout: true` value. Preserve unrelated keys and comments. A commented example does not count as an active value. If the existing YAML cannot be safely edited, skip persistence and continue to Path B.
- **Keep the file untracked before either write:** read the effective `snapshot.auto-track` with `jj --ignore-working-copy config get snapshot.auto-track`. Preserve that fileset and exclude this exact path by setting the workspace-local value with `jj --ignore-working-copy config set --workspace snapshot.auto-track '(<existing fileset>) ~ root-file:.rocketclaw/config.local.yaml'`, substituting the actual existing fileset with correct shell quoting. Do not repeatedly add the exclusion if it is already present. Only write after the exclusion succeeds; if the setting cannot be read or updated, skip persistence and continue to Path B. This keeps the opt-out local without editing repository internals or a tracked `.gitignore`. `ce-setup` remains the owner of the shared ignore entry for teammates.
- **File absent:** after protecting it from automatic tracking, create it (and its `.rocketclaw/` directory) with the key.

If the root can't be resolved or any write fails, proceed to Path B anyway; the opt-out is a convenience, never a blocker.

After recording, confirm it in one line so the write isn't silent and the user knows how to undo it — e.g. "Got it — I won't bring up Spiral here again (saved to `.rocketclaw/config.local.yaml`, excluded from automatic tracking). Want it back later? Just ask, or remove the `ce_promote_spiral_optout` key." Keep it to a single line; don't belabor it.
