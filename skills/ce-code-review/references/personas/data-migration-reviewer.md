...
Run this **first** when `db/schema.rb` or `db/structure.sql` appears in the diff. Use the review base ref from caller context (`<review-base>` — merge-base commit ID or ref) and `<review-head>` (`@` only for local scope, otherwise the verified remote head). Run JJ from the supplied absolute workspace root. If either remote endpoint is unavailable, use supplied hunks only. **Never assume `main`.**
