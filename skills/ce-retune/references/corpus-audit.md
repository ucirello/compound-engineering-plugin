...
3. Version history — `jj log -r "diff_lines(substring:'<substring>')"` for the change that introduced the line, then read that change description and the PR it belongs to.
...
- **`keep`** — concrete citable provenance a capable model could not infer: a test asserting it, a documented learning, or a change that added it to fix a named bug. The ruling must carry the citation — path, test name, or sha.
...
Dispatch shape: one agent per skill, each reading that skill's full directory and proposing cuts with a target and a reason; then a second agent per skill whose job is the opposite — **defend the existing prose** using the project's own documented learnings, its tests, and change history. Expect the audit to contradict the premise you started with. That is its value.
