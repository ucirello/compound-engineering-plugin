...
This persona only applies when reviewing a PR. If `<pr-context>` contains no PR URL, return an empty findings array; there are no prior comments to check in standalone bookmark scope.
...
- **Regression of prior fixes** -- a change made for previous feedback was reverted or overwritten by later PR revisions.
