...
Environment overrides (defaults in parentheses):
  ROCKETCLAW_PEER_JOBS_ROOT         base dir inside <workspace-root>/.tmp
  ROCKETCLAW_WORK_RUNS_ROOT         parent work-run dir inside the same .tmp
...
the current Jujutsu workspace's `.tmp`. Every read opens the file first (no-follow) and
...
POSIX path is behaviorally unchanged:
  ...
  jobs root defaults under the current workspace's `.tmp`, with local `.tmp`
            as the fallback before workspace discovery.
...
def _workspace_tmp() -> str:
    try:
        proc = subprocess.run(
            ["jj", "--no-pager", "--ignore-working-copy", "workspace", "root"],
            capture_output=True,
            text=True,
            check=False,
        )
        ...
        root = os.getcwd()
    return os.path.abspath(os.path.join(root, ".tmp"))


def jobs_root_base() -> str:
    configured = os.environ.get("ROCKETCLAW_PEER_JOBS_ROOT")
    root = _workspace_tmp()
    candidate = os.path.abspath(configured) if configured else root
    if os.path.commonpath([root, candidate]) != root:
        raise RunnerError("ROCKETCLAW_PEER_JOBS_ROOT must remain inside the workspace-root .tmp directory")
    return candidate
...
def skill_runs_root(skill: str) -> str:
    if skill == "ce-work" and os.environ.get("ROCKETCLAW_WORK_RUNS_ROOT"):
        candidate = os.path.abspath(os.environ["ROCKETCLAW_WORK_RUNS_ROOT"])
        root = _workspace_tmp()
        if os.path.commonpath([root, candidate]) != root:
            raise RunnerError("ROCKETCLAW_WORK_RUNS_ROOT must remain inside the workspace-root .tmp directory")
        return candidate
...
def ensure_owned_dirs(base: str, path: str) -> None:
    ...
    if IS_WINDOWS:
        ...
        ours = created_base or os.path.basename(cur) == ".tmp" or (
...
def write_atomic(path: str, data: bytes) -> None:
    tmp = os.path.join(os.path.dirname(path), f".atomic-{secrets.token_hex(8)}")
