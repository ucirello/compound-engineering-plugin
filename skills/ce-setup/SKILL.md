...
4. Check whether `.compound-engineering/config.local.yaml` exists and, if it does, whether the workspace ignore rules cover it.
...
- `.compound-engineering/config.local.yaml` exists but is not safely ignored by workspace rules
...
If `.compound-engineering/config.local.yaml` exists and is not covered by the workspace ignore rules, offer to add:

```text
.compound-engineering/*.local.yaml
```

Append the entry to the repo-root ignore file only if the user approves. Do not overwrite unrelated ignore content.
