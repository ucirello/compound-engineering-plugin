...
2. **Working-copy change.** If `jj diff -r @` is non-empty, include it.
3. **Unpushed changes vs base bookmark.** Review `jj diff --from <base> --to @ --git`, where `<base>` is the repository's runtime-derived default bookmark.
