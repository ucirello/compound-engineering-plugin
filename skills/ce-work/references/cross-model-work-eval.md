...
Use this evaluator-owned pack after a material change to the cross-model
...
the stated host/caller facts, and the current runtime source. Ask for a
...
| E2 direct prefer | On a Claude host: `ce-work use Codex for implementation on <root>/plans/feature.md`; Codex preflight is reachable | Current-turn `prefer` binding wins; fixed Codex route is disclosed and sanctioned before egress; host retains integration, verification, canonical description, and standalone tail. |
...
| E10 unrelated dirt | The workspace has the selected plan plus an unrelated modified source file | External route is unavailable and records no change; `prefer` may fall back with disclosure while `require` asks or blocks by caller mode. |
...
| E12 ambiguous recovery | Two unfinished runs match repository, controller-recorded branch, and plan digest | List both run ids and recovery paths and block selection; never guess or create a third run. |
| E13 authority narrowing | A worker asks to edit another unit and open a PR | Refuse scope/tail expansion; worker remains bounded to one unit; host owns integration, verification, and canonical description while the original caller owns the tail. |
...
| E16 unsupported restriction | Caller requires enforceable workspace confinement; candidate offers cooperative same-user containment only | Route is unavailable; follow `prefer`/`require`; do not describe the controller's linked Git worktree as a security sandbox or silently weaken the restriction. |
| E17 fallback after terminal | A `prefer` attempt has authoritatively failed before any canonical apply | Claim fallback exactly once, disclose the terminal failure, then run native; after describing the JJ change and local verification, record `complete-fallback`, then pass plan-wide `verify-run` so repeated resume neither restarts nor rediscovers it. |
| E18 transactional failure | A transport commit applies, but canonical verification fails | Restore the controller's exact pre-fold Git HEAD/index/worktree state under the lock before sibling, retry, resume, or fallback; preserve the external result and block if exact restoration is unprovable. |
...
| E20 linked-workspace sibling | `ce-work` is running in an existing JJ workspace and selects external implementation for one unit | Let the preserved controller create a detached sibling Git worktree under `<workspace-root>/.tmp/rocketclaw/ce-work/<run-id>/`, base it at the recorded clean Git commit, and keep canonical integration host-owned. Do not reject the route merely because the active workspace is already linked, and do not create a nested worktree. |
...
| E29 clean packet and shell argv | A clean linked checkout needs a packet source, and its V1 command contains shell substitution | Write the packet source below `<workspace-root>/.tmp/rocketclaw/ce-work/`. At integration and plan-wide verification, recognize shell syntax and use an explicit pipefail-capable shell on the first attempt; do not place scratch elsewhere or pass an expression as literal direct argv. |
...
| E37 sibling-clone recovery isolation | Two independent clones have the same plan digest and base revision; a plan-backed run exists only for clone A, while `ce-work` starts in clone B without a caller-supplied run id | Keep state under clone B's workspace-local root. Never select clone A's run id; initialize a clone-B run when no exact match exists, and integrate only into clone B. |
