...
1. Run the analyzer in the workspace-local `.tmp/rocketclaw` namespace so generated evidence does not pollute tracked project paths (`SKILL_DIR` is the directory containing the `ce-riffrec-feedback-analysis` SKILL.md; set it in the same command because shell state does not persist between Bash calls):
   ...
   if WORKSPACE_ROOT="$(jj workspace root 2>/dev/null)"; then
     ...
     OUTPUT_DIR=".tmp/rocketclaw/riffrec-quick/$(date +%Y%m%d-%H%M%S)-$$"
   fi
...
2. Read only `analysis.md` from the local output. Skip `problem-analysis.md`, `review-prompt.md`, `requirements-kickoff.md`, and `source-materials.md` — they are designed for the extensive path.
...
- Do not include `raw/` or `frames/` in a repository change — they live only under the workspace-local `.tmp/rocketclaw` directory. Remove the run directory after the report is complete.
