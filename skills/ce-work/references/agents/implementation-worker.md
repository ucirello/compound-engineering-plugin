...
- You may edit and test in this workspace. Do not run `jj describe`, `jj commit`, or another change-description write. Leave the completed working copy undescribed beyond the host snapshot. Report `completed` when files and scoped checks are done; the host snapshots the tree. Do not push, open a PR, ship, or integrate into another checkout.
...
- Before returning `completed`, inspect the complete working-copy delta (`jj diff --name-only`), against the packet's expected scope. Remove only disposable artifacts created by your own checks. If an unexplained or non-disposable path remains, return `blocked` or `scope_expansion`; otherwise list every remaining changed path in `changed_files`.
- Your changed-file list and prose are evidence only. The host independently derives the complete tree and alone decides whether to integrate it.
