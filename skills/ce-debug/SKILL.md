...
**Trivial-bug fast-path:** Once the problem is clear, decide whether the framework is needed at all. If the cause is immediately readable from the input (single-file typo, missing import, obvious null deref or off-by-one with a one-line fix) and verification doesn't require deep tracing, present the cause and the proposed one-line fix and run Phase 2's **Fix it now / Diagnosis only** user-choice gate before editing — the fast-path saves investigation ceremony, not the user's choice over whether to apply a fix. If the user picks fix, run Phase 3's **Workspace and bookmark check** (unsaved-work confirmation and default-bookmark bookmark-creation prompt), apply the fix, leave a one-line note explaining the cause, and skip to Phase 4's structured summary. If diagnosis only, write the summary and stop. When in doubt, run the full framework; getting the wrong root cause costs more than the few minutes of ceremony.
...
- Correct bookmark/ref checked out; no unintended unsaved changes
...
- No stale build artifacts (`dist/`, `.next/`, compiled binaries from an earlier bookmark/ref)
...
- If the bug looks like a regression ("it worked before"), use `jj bisect run` when automatable (see `references/investigation-techniques.md`)
...
- The GitHub remote/forge (`gh` if available).
...
**Run a few targeted queries** on the symptom, the error string, and the affected file/area — not an exhaustive sweep. Weight the search toward what `jj log` cannot show you; do not re-derive what the Phase 1.3 history check already surfaced. Look for:
...
- Check for unsaved changes (`jj st`). If the user has in-progress work in files that need modification, confirm before editing — do not overwrite in-progress changes.
- If the current bookmark/ref is the default bookmark/ref, ask whether to create a feature bookmark first using the platform's blocking question tool (see Phase 2 for the per-platform names). Detect the workspace root with `jj workspace root`; inspect current bookmarks with `jj log -r 'bookmarks() & @::'`. Default to creating one; derive a name from the bug and use the repo's JJ bookmark workflow (or `/ce-commit-push-pr`) rather than branch-oriented commands. On any other bookmark/ref, proceed.
- Record the pre-fix scope before editing: current change/revision (`@`), whether `jj st` is clean, and any pre-existing changed files. During Phase 3, keep a list of fix-owned files (the tests and implementation files changed for this bug). Phase 4 uses this to keep simplify/review from touching unrelated bookmark work.
...
#### Post-fix polish/review tail (before saving or PR)

Run this tail after Phase 3 ran and before the bookmark-based save/PR handoff. The goal is to leave the fix PR-ready, not merely locally green.
...
2. **Save the fix (`/ce-commit`)** — local JJ change only
...
When offering, use the blocking question tool described above. If the user accepts, run `/ce-compound`, then save the resulting learning doc to the same bookmark and push so the open PR picks up the new change.
