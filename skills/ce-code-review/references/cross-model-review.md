...
2. Scope is `local-aligned` or standalone — the current workspace IS the reviewed head. Skip in `pr-remote` / `bookmark-remote`: the peer reviews the local workspace, which is not the PR/bookmark head.
