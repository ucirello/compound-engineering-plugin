...
For each selected reviewer, read the corresponding local prompt asset from `references/personas/<reviewer-name>.md` and spawn a generic subagent using the subagent template. Do not use `subagent_type`, typed `Agent` names, or platform-level branded agent registration. Each persona subagent receives:
...
Read-only here means **non-mutating**, not "no shell access." Reviewer sub-agents may use non-mutating inspection commands when needed to gather evidence or verify scope, including read-oriented `jj` / `gh` usage such as `jj diff`, `jj file show`, `jj file annotate`, `jj log`, and `gh pr view`; an exact `git log` read is also allowed. When a finding's claim depends on line history (`pre_existing`, intent, introduced-by-this-diff, or P0/P1 confidence that depends on authorship/age), reviewers attach one concise provenance evidence line from targeted annotation/log on the cited line, additional to the quote-the-line gate and omitted when the finding is justified from the diff alone. In **`pr-remote`** or **`branch-remote`** scope, inspect changed files via `jj file show -r <remote-head-ref> <path>` or diff hunks; do not Read/Grep workspace paths for files in scope. They must not edit project files, change working-copy revisions, describe or create changes, push bookmarks, create PRs, or otherwise mutate workspace or repository state.
