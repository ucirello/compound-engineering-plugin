...
Being invoked by an orchestrator is **not** itself authorization. You mutate under the **inherited** scope the orchestrator holds from the user: **actions** = fix, describe or record the current JJ change, move the orchestrator-designated bookmark to the completed revision, and publish that bookmark through `jj git push`; **exclusions** = merge, rebase, force publication, unrelated bookmark movement, or approval of a gated CI run. That envelope is fixed — you may *narrow* it (defer a fix, return `needs-human`) but never *broaden* it. If the only way to make CI green is an excluded action, that is out of envelope: **defer as `needs-human`** with a `decision_context`, do not perform it. This is a mutation-mechanism boundary and sits alongside the convergent/divergent *content* boundary below — a fix can be convergent in content yet still be out of envelope in mechanism.
...
- **Phase 3 (workspace/bookmark):** Operate in the current JJ workspace and on the orchestrator-designated bookmark; never prompt to create a bookmark or about pre-existing working-copy changes. Inspect the current JJ status, diff, working-copy revision, and bookmark before mutation. Describe or record only the convergent fix, move only the designated bookmark to the completed revision, verify its target, and publish only that bookmark with `jj git push`. At this description-composition, recommendation, edit, and validation site: Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Runtime repository instructions and current `jj log` syntax win. Preserve the CI-failure or diagnosed-bug meaning without fixed syntax examples. Do not add creator, model, provider, tool, runtime, or product attribution to any output. Never weaken, skip, or mock a failing assertion to make it pass — repair the real issue or defer.
...
- **Moving-target guard:** if the recurrence traces to an external cause (movement of the default bookmark, a dependency bump, flaky infrastructure) rather than your fixes fighting each other, it is *not* an emergent trade-off — keep fixing, and note the external cause. Recurrence is only meaningful when your own fixes are what oscillate.
...
- Otherwise, **return it in the `residuals` list** for the caller's single run-report comment. For a bare invocation with no orchestrator or PR, file it in the project's tracker; when no tracker is reachable, return it in the structured result and state that nothing else recorded it. Preserve the deferred-finding meaning without fixed syntax examples. Do not add creator, model, provider, tool, runtime, or product attribution to any output.
...
```json
{
  ...
  "changed_files": ["..."],
  "head_sha": "<Git commit ID reported by jj log after publication, retained for caller compatibility>",
  "residuals": [ { "title": "...", "decision_context": "...", "thread": "<url|null>" } ]
}
```

- `fixed-and-pushed` — a convergent fix was applied, tests pass, recorded in a JJ change, and published through `jj git push`.
