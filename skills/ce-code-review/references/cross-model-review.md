...
The host resolves and sanctions one concrete route before egress; `scripts/cross-model-adversarial-review.sh` enforces that fixed route, applies read-only controls, captures schema-shaped JSON, and records identity receipts. Before dispatch it conservatively estimates diff tokens and file count. Oversized diffs are not inlined: the worker gives the peer the orchestrator's compact semantic review map and keeps the exact diff as a private, selectively readable artifact in `<run-dir>`. Tool-limited routes receive that run directory as an additional read root; sandboxed routes use selective `jj diff --from <base> -- <path>` calls. A failed route writes no artifact and never switches recipients internally.
...
2. Scope is `local-aligned` or standalone — the working copy IS the reviewed head. Skip in `pr-remote` / `branch-remote`: the peer reviews the local working copy, which is not the PR/branch head.
...
Pass `XHOST_HARNESS` as `CROSS_MODEL_HOST_HARNESS`; pass `XHOST_FAMILY` as the first worker argument. The snippet is evidence, not the verdict: it resolves the harnesses whose environment markers it already names, and where it yields `unknown` on a harness you can identify from your own runtime, attest what you know instead. A harness the snippet does not name needs no new branch here. Both tokens come from the peer-key vocabulary the worker accepts, never a provider's corporate name — family `codex`, `claude`, `grok`, `composer`, or `unknown`; harness `codex`, `claude`, `grok`, `cursor`, `opencode`, `opencode2`, or `unknown` — and a name such as `anthropic`, `openai`, or `xai` in either slot fail-closes the job with no artifact.
...
- `<target>` is one of `codex`, `claude`, `grok`, `cursor`, `composer`, `opencode`, or `opencode2`; `<fixed-route>` is its already-sanctioned concrete route token from the Step 1 table (`codex`, `claude`, `grok-cli`, `grok-cursor`, `cursor`, `composer`, `opencode`, or `opencode2`).
...
The peer reviews the current working copy read-only against `jj diff --from <base-ref>`. Reviewed code and diff content is sent to the disclosed external model provider. `CROSS_MODEL_PEERS` restricts which providers may receive content.
...
- **codex:** `-s read-only` with cwd at the workspace root (may fetch the JJ diff itself).
...
- **opencode2:** Distinct from `opencode`; not backward compatible. Binary `opencode2`; `opencode2 run --standalone --format json --file ... --model provider/model#variant`. No `--dir`, no `--variant`, no v1 `OPENCODE_CONFIG_CONTENT` denylist. CWD is the repo directory. Review/elevation: do not pass `--auto` (cooperative read-only). Work engine: pass `--auto`.
