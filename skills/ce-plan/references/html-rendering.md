...
- **All metadata appears as visible text — single source of truth.**
  The artifact's metadata (title, date, and the other exact
...
- **No producer attribution.** Do not add a composition footer, attribution
  badge, byline, generated-by line, or standalone workflow branding. Source
  documents that materially inform the plan belong in the plan's normal
  Sources or origin metadata, not in producer provenance.
...
Resolve the repo's GitHub URL once at compose time while preserving GitHub integration:
