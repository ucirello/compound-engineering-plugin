...
The host resolves and sanctions one concrete route before egress; `scripts/cross-model-adversarial-review.sh` enforces that fixed route, applies read-only controls, captures schema-shaped JSON, and records identity receipts. Before dispatch it conservatively estimates diff tokens and file count. Oversized diffs are not inlined: the worker gives the peer the orchestrator's compact semantic review map and keeps the exact diff as a private, selectively readable artifact under the workspace `.tmp/rocketclaw` directory. Tool-limited routes receive that directory as an additional read root; Codex uses selective `jj diff --from <base> -- <path>` calls under its existing read-only sandbox. A failed route writes no artifact and never switches recipients internally.
...
2. Scope is `local-aligned` or standalone; the working copy is the reviewed revision. Skip remote scope because the peer's local workspace does not contain the reviewed revision.
...
**Resolve ordinary checkout YAML keys from the two repository files.**
...
**Checkout-configured model and effort.** After the target is resolved, read `cross_model_model:` and `cross_model_effort:` from the same two checkout config files under the ordinary-key rule. When `cross_model_model` is set, pass `CROSS_MODEL_MODEL_OVERRIDE_TARGET=<resolved-target>` and `CROSS_MODEL_MODEL_OVERRIDE=<value>`; when `cross_model_effort` is set, pass `CROSS_MODEL_EFFORT_OVERRIDE=<value>`. Both ride the `env` prefix of the start invocation below. The worker validates each against the route it actually runs; preserve the operational provider/model mappings. An incompatible value fails the pass closed with a named skip reason rather than substituting. A model or effort the user states in conversation outranks the config keys.
...
- **Dispatch-infrastructure failure** -- the runner or worker itself crashed. Keep the target, model, `jj diff --from <base-ref>` read scope, and adversarial brief fixed for any same-route recovery. Stop when a failure repeats or the shared deadline is spent; report the adversarial lens as degraded. Recovery may not substitute a recipient, widen read scope, or relax the read-only boundary.
...
The peer reviews the **current working copy** read-only against `jj diff --from <base-ref>`. Reviewed code/diff content is sent to the resolved external model provider. `CROSS_MODEL_PEERS` restricts which providers may receive content.
...
- **codex:** `-s read-only` with cwd at the repository root (may run selective JJ diffs itself).
