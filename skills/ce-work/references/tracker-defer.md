...
- **Body:**
  ...
  - Source: a link to the PR carrying this change when one already exists at filing time; otherwise the bookmark and Jujutsu commit id from `jj log`, so the ticket points at the code before a PR is opened. When the same run opens a PR after filing, back-fill the PR link best-effort.
