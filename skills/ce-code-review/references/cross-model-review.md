...
2. Scope is `local-aligned` or standalone — the working tree IS the reviewed head. Skip in `pr-remote` / `bookmark-remote`: the peer reviews the local tree, which is not the PR/bookmark head.
