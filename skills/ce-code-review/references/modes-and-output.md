...
| **`mode:agent`** | One JSON object (see ### JSON output format below) + the same `<jj-workspace>/.tmp/rocketclaw/ce-code-review/<run-id>/` artifacts; outside Jujutsu, use `<current-directory>/.tmp/rocketclaw/...` |
