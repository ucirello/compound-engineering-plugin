...
|-------|---------|--------|
| `diff:<ref-or-revset>` | `diff:qpvuntsm`, `diff:main@origin..@`, `diff:PR#42` | Forces diff mode on that change |
...
- `diff:main@origin..@`, or `audience:team` leading a request — genuine flags: nothing is left to garble.
...
- An unrecognized `<word>:<word>` token (including project-specific change-description prefixes like `feat:` appearing inside a topic) is not a flag — it passes through verbatim as request text. The same holds for a *recognized* token that fails the reads-as-a-flag test above.
...
- **Diff** — the request names a resolvable Jujutsu change or range: a change ID, bookmark, revset, PR, "the parent change", "what you just did", "this change".
