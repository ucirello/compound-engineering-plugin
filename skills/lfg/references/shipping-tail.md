...
This records any remaining changes, pushes the bookmark, and opens a pull request — non-interactively, per the mode token. If it prints a `New concepts:` trailer after the PR URL, record the concept name(s) for step 10. If a PR already exists for the bookmark (check with `GIT_DIR` set to the output of `jj git root` and `gh pr view --json number,url,state`), skip PR creation but still record and push any remaining working-copy changes.

**Per the shipping precondition, when no remote is configured, do NOT invoke `ce-commit-push-pr` or a project-defined shipping process** — the default's commit step pushes unconditionally (`jj git push`), so a literal invocation would still hit the impossible push. Instead record any remaining working-copy changes locally yourself. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in git log, compose commit messages adherent to the present standards. Repository-local commit-message syntax from project instructions and `git log` ALWAYS wins when it differs from Go guidance. Apply compatible Go guidance to quality/clarity/structure without replacing repository-local syntax. Run `jj commit -m "<message composed from the standards above>"` and skip the push and PR creation entirely.
...
- Do **not** start a second bare `mode:pipeline` babysit on the current-bookmark PR URL (that can supersede the stack-aware run as target-only or watch the wrong layer).
...
When step 8/9 used a stack handoff, render that invocation for the **bottom open non-draft** PR URL with the same `posture:stack-ready` or `posture:stack-land` token — never a bare current-bookmark PR URL that would supersede stack scope.
