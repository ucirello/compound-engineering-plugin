...
Severity, confidence, and cross-reviewer agreement tell you what to do first and what to flag loudly -- they do not gate the decision. There is no deny-list: downside is controlled after the fact by `jj undo`, a visible diff, and an isolated change checkpoint, not by a precondition.
...
**Scope invariant.** Apply only when the working copy is what was reviewed: `local-aligned` or standalone. In remote scope the working copy is not the reviewed revision; report instead.
...
**Review the autofix diff before finishing.** Before describing or reporting applied fixes, diff only the changes introduced during Stage 5c against the pre-apply operation. Run one self-review pass over that diff:
...
**Describe and advance when the pre-review change was empty.** Before applying, record whether `jj diff` was empty. The permanence gate is the **push**, not a local JJ change, and `jj undo` can reverse the latest repository operation.

Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards.

Repository-local syntax wins. Apply only compatible Go quality guidance. Derive the description from the applied fixes and nearby repository history; do not use a fixed message shape.

- **Empty before the review:** after applying and verifying, set the current change description with `jj describe -m "<repository-derived description of the applied fixes>"`, then run `jj new` to leave an empty working-copy change. The described change is isolated and reversible.
- **Non-empty before the review:** apply but do **not** alter the description or create a new change; the fixes remain with the user's in-flight work. The Applied section lists what changed.
...
2. **Applied (explicit local apply only).** When Stage 5c applied fixes, list them first before the findings in an Applied section; each entry carries `#`, file, the fix, and reviewer, followed by validation outcome and change status. Report either the repository-derived JJ description used for an initially empty change or that the existing in-flight change and description were preserved. Flag green-but-unverifiable edits prominently. Omit this section when local apply was not authorized or nothing was applied.
...
```json
{
  ...
  "scope": {
    "base": "<common-ancestor revision, pr:NNN marker, or explicit base revision>",
...
5. **Protected artifacts are respected.** Discard any finding that recommends deleting or ignoring a workflow artifact under a `plans/`, `solutions/`, or legacy `brainstorms/` directory whose immediate parent is the artifact root. Categories nest; a `references/personas/` skill asset is not protected by this rule.
...
- `plans/` under the artifact root -- unified decision artifacts; execution progress is derived from JJ history, not stored in plan bodies
- `solutions/` under the artifact root -- solution documents; categories may nest, for example `solutions/<category>/foo.md`
- the legacy `brainstorms/` -- requirements documents retained for compatibility
...
After Stage 6, stop. When local apply was explicitly authorized, Stage 5c may already have applied and isolated verified fixes in a described JJ change. Otherwise the caller or user decides what to apply from the report and artifacts.
