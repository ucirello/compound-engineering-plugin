...
Use the current JJ workspace root when available; outside a JJ repository, use `.tmp` under the current directory. JJ snapshots working-copy changes directly, so there is no staging step.
...
On `HIT`, load the profile JSON — stack, conventions, vocabulary — and take orientation from it. On `MISS`, dispatch a generic subagent with `references/agents/repo-profiler.md` to derive the profile, write its JSON to a file, then persist with `python3 "$SKILL_DIR/scripts/repo-profile-cache.py" put <file>` (re-set `SKILL_DIR` in that call — shell vars don't persist between Bash invocations). On `NO-CACHE` — or if the call errors — derive orientation inline and skip the `put`. The cache is an optimization, never a correctness dependency. The topic-specific evidence (the diff, the concept's call-sites, the window's revisions) is always gathered fresh.

- **Diff mode:** resolve the change (the `diff:` ref, or the most recent substantial change when the request points at one implicitly), use `jj status` for the high-level working-copy state when relevant, and gather its evidence with `jj diff` — the diff itself, the files it touches, any plan or solution doc that motivated it. Gather silently: nothing learned here is narrated to the user until Phase 3's ordering rule is satisfied.
- **Recap mode:** dispatch a generic subagent seeded with `references/agents/work-recap-scout.md` (extraction tier), passing the resolved window, the JJ workspace root, and `$RUN_DIR`. It returns an evidence summary with change IDs or commit IDs and `file:line` pointers. **Empty window** (no JJ revision activity, no doc changes): say so, offer to widen the window, write no artifact, and end the run after the user responds.
...
- **Leave it** — report the `$RUN_DIR` path and state that it remains in the workspace-local scratch area; nothing else is written.
