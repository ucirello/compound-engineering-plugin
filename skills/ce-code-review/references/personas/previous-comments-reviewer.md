...
This persona only applies when reviewing a PR. The orchestrator passes PR metadata in the `<pr-context>` block. If `<pr-context>` is empty or contains no PR URL, return an empty findings array immediately -- there are no prior comments to check on a standalone JJ change-stack review.
...
- **Regression of prior fixes** -- a change that was made to address a previous comment has been reverted or overwritten by later revisions in the same PR.
