...
4. **Contributor Mapping**: Run `jj log <path>` and summarize authors to identify key contributors and their relative involvement.

5. **Historical Pattern Extraction**: Use `jj log <path>` with content search over relevant revisions to find when specific code patterns were introduced or removed.
