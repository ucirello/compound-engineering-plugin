...
- **Machine-local under the workspace `.tmp`** (solo setups; keeps sweep bookkeeping out of the committed tree, no commit noise). Resolve the path immediately with this shell block, substituting a sanitized repository slug:
  ...
  SWEEP_STATE_DIR="$(dirname "$SWEEP_STATE_PATH")"; mkdir -p "$SWEEP_STATE_DIR"
