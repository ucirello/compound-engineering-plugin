#!/usr/bin/env python3
"""Validate cited claims in a solution doc against the JJ working copy.
...
citations against the repository:
    ...
    1. Cited repo-relative paths (backticked, containing at least one '/')
       ...
       at `@-` or the remote default line still count as real paths and
...
def main(argv: list[str]) -> int:
    ...
    if in_jj:
        ...
        if code == 0 and len(resolved.splitlines()) == 1:
        ...
        if remote_bookmark:
            ...
            if behind_count > 0:
                infos.append(
                    f"INFO: workspace is {behind_count} changes behind {remote_bookmark} — "
        ...
        infos.append(
            "INFO: not a JJ repository — path and commit-ID classification skipped "
    ...
    def revision_is_ancestor(revision: str, descendant: str) -> bool:
        if not in_jj:
            return False
        code, out = jj(
            [
    ...
    for raw in BACKTICK_RE.findall(body):
        ...
        if token.startswith("../") or "/../" in token:
            ...
            if check.startswith(".."):
                continue  # escapes the repository — not checkable as a repository path
        ...
        present_at_remote = bool(remote_bookmark) and revision_has_path(
            remote_bookmark, check
        )
        if not (present_at_current or present_at_remote) and not is_path_shaped(
            check, base
        ):
        ...
        if present_at_current:
            ...
            flags.append(
                ...
                f"{remote_bookmark}: stale workspace? Annotate or verify against the remote bookmark."
            )
        else:
            where = (
                f"working copy or {remote_bookmark}"
                if remote_bookmark
                else "working copy"
            )
            flags.append(
    ...
    if in_jj:
        for m in COMMIT_ID_RE.finditer(body):
            ...
            if code != 0 or len(resolved.splitlines()) != 1:
                flags.append(
                    f"FLAG commit ID {commit_id}{loc} — does not resolve to a revision in this "
                    "repository. Replace with the PR number, or drop it."
                )
                continue
            in_current = revision_is_ancestor(commit_revset, "@")
            in_remote = bool(remote_bookmark) and revision_is_ancestor(
                commit_revset, remote_bookmark
            )
            if in_current and (in_remote or remote_bookmark is None):
                continue
            if in_current and not in_remote:
                flags.append(
                    f"FLAG commit ID {commit_id}{loc} — reachable from @ but not {remote_bookmark}: "
                    "local-only revision whose commit ID may change when the revision is rewritten. "
                    "Prefer citing the PR number."
                )
            elif in_remote:
                flags.append(
                    f"FLAG commit ID {commit_id}{loc} — not reachable from @ but reachable "
                    f"from {remote_bookmark}: this workspace predates the merge. Add a "
