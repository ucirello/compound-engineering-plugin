...
- **Merge-state claims** ("fixed in #1608", "landed", "shipped") verify against **remote truth** — the workspace may differ from the published state, so `gh pr view` (or the tracker equivalent) is primary and JJ reachability through `trunk()` is only the fallback. If `trunk()` is unavailable, treat verification as degraded.
...
| JJ change/commit ID does not resolve | Fabricated, stale, or from another repository | Replace with a durable review reference, or drop |
| revision ID reachable from `@` only | Publication may rewrite identity | Prefer a durable review reference |
| revision ID reachable from `trunk()` only | Workspace differs from trunk | Keep with a temporal qualifier; verify the landed claim via `gh` |
...
native read/search tools, read-only `jj` commands, and `gh` when available.
