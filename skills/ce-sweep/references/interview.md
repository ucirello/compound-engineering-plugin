...
Loaded by `SKILL.md` when `ce-sweep` runs with no `feedback_sources` configured. Captures the setup that will be merged into `<workspace-root>/.rocketclaw/config.local.yaml` (the unified RocketClaw local config, ignored and machine-local) and re-read on every subsequent run.
...
**User-runnable invocation rendering.** Whenever this interview prints or registers a `ce-sweep` invocation, default to `/ce-sweep` (plus any arguments); use `$ce-sweep` only when the active host explicitly documents dollar-prefixed skill invocation. Render only the invocation as inline code and output one form only.

## Interaction Method

Ask **one question at a time** using the platform's blocking question tool: `AskUserQuestion` in Claude Code (call `ToolSearch` with `select:AskUserQuestion` first if its schema isn't loaded), `request_user_input` in Codex, `ask_question` in Antigravity CLI (`agy`), `ask_user` in Pi (requires the `pi-ask-user` extension). Fall back to numbered options in chat only when no blocking tool exists in the harness or the call errors (e.g., Codex edit modes) — not because schema loading is required. Never silently skip a question or assume a default without surfacing it.
...
- **Committed to the repository** (recommended when multiple writers or machines share a bookmark — one source of truth everyone reads and writes). Sets `sweep_state_path` to the committed default `docs/feedback-sweep/state.yml`.
- **Machine-local under the workspace `.tmp` directory** (solo setups; keeps sweep bookkeeping out of changes). Resolve the path immediately with this shell block. Its global base is `$(jj workspace root)/.tmp`; when `jj workspace root` fails, use the current directory's local `.tmp` instead:
  ...
  SWEEP_STATE_PATH="$SCRATCH_ROOT/ce-sweep/state.yml";
...
Let the user override the path if they want a different location. If they pick machine-local, note that a fresh workspace or a teammate's machine will not see this state — it is per-machine by design.
...
**Ask:** "Is this a multi-writer setup where several Jujutsu workspaces push the sweep state to a shared docs bookmark? Answer yes only if more than one machine or writer describes and pushes changes to the same bookmark. Default is no — a single workspace recording changes locally."

- **No** (default) -> `sweep_shared_branch: false`. The single-writer lease serializes overlapping sweeps within one workspace.
- **Yes** -> `sweep_shared_branch: true`. Explain: the lease becomes **push-gated** — before any source-side write, the sweep records and pushes the lease acquisition on the shared bookmark and confirms its writer won, making the lease a repository-wide mutex across machines.
...
Merge the captured settings into `<workspace-root>/.rocketclaw/config.local.yaml`. Resolve the workspace root with `jj workspace root`; if that fails, use the current directory as the local fallback.
...
- If `.rocketclaw/config.local.yaml` is not already covered by the repository's active ignore rules, offer to add the entry using the repository's established ignore-file syntax before writing.
...
sweep_state_path: docs/feedback-sweep/state.yml   # committed (multi-writer) or workspace .tmp path (solo)
