...
2. Bind presence with the assistant identity defaults
...
Run the shell recipe with its working directory set to the absolute workspace root. Outside JJ, use the absolute project directory; scratch then stays in its local `.tmp`.
...
STATE_TMP=$(mktemp "$WORKSPACE_ROOT/.tmp/proof-state.XXXXXX")
...
TMP=$(mktemp "$WORKSPACE_ROOT/.tmp/proof-sync.XXXXXX")
jq -jr '.markdown' "$STATE_TMP" > "$TMP" && mv "$TMP" "$LOCAL"
rm "$STATE_TMP"
```

`jq -jr` streams markdown bytes without going through a shell variable, so trailing newlines survive. Both temporary files stay under the workspace's `.tmp`. The final `mv` is atomic only when the destination is on the same filesystem.
