...
allowed-tools: Bash(gh *), Bash(jj *), Bash(bash *), Read
---
...
| PR URL (e.g., `https://HOST/OWNER/REPO/pull/123`, no comment fragment) | **Full** -- all unresolved threads on that PR; parse `HOST`, `OWNER/REPO`, and the number from the URL so a fork-to-upstream PR targets the right host and base repository |
...
- Valid fixes described in an isolated JJ change and pushed through the PR head bookmark
