...
Polish reads `.rocketclaw/launch.json` at the repo root to resolve the dev-server start command. The schema is a subset of VS Code's `launch.json` format so the same process fields remain familiar across supported editors and agent harnesses.
...
`.rocketclaw/launch.json` is not yet a fully unified standard across Claude Code, Cursor, VS Code, and Codex. Polish leads with `.rocketclaw/launch.json` because:
- RocketClaw owns the configuration while preserving familiar launch fields across providers
