...
2. **Otherwise, in a Jujutsu repository**, default to the diff between the current change stack and its base. Prefer the repository's local revset aliases and conventions; otherwise use `jj diff --from 'trunk()' --to '@'`. This covers the common case of "simplify everything I've added on this feature stack before opening a PR." If no base revision can be resolved, fall back to the current working-copy change (`jj diff`).
