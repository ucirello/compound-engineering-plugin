...
When `<pr-scope-mode>pr-remote</pr-scope-mode>` or `<pr-scope-mode>branch-remote</pr-scope-mode>` is in context, do **not** Read/Grep the workspace copy of {finding_file}. In `pr-remote`, use GitHub data from `gh` or diff hunks only. In `branch-remote`, inspect via `jj file show -r <branch-head-ref> {finding_file}` when a remote head revision is set; otherwise use diff hunks only.
...
- Do not edit files, alter JJ changes/workspaces, push, or otherwise mutate repository state. You are operationally read-only.
