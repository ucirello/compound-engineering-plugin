...
**Large shared context -- pass paths, not contents.** The diff and file list go to every reviewer and validator. When inlining them into each prompt would be wasteful, write them once into the run directory and pass those paths in the diff and changed-files slots. The AI Assistant and validator templates instruct the child to read a stored path. Inline a small diff directly.
...
- **Mid-tier** -- every other persona and local prompt actor: preserve the platform's operational balanced-tier mapping. When the dispatch primitive exposes no supported override, inherit the parent model.
...
Before assembling any spawn prompt, read these three files from this skill's directory now: `references/subagent-template.md`, `references/diff-scope.md`, and `references/findings-schema.json`. They define the dispatch shape and JSON contract every AI Assistant needs. Read them and all selected persona prompt assets in one parallel read-tool wave.

For each selected reviewer, and only for those, read the corresponding local prompt asset from `references/personas/<reviewer-name>.md` and spawn a generic subagent using the subagent template. Do not use `subagent_type`, typed `Agent` names, or platform-level actor registration. Each persona subagent receives:
...
Read-only here means **non-mutating**, not "no shell access." Reviewer subagents may use non-mutating `jj` and `gh` inspection commands. When a finding depends on line history, attach one concise provenance item from targeted `jj file annotate` and `jj log` evidence, additional to the quote-the-line gate. In remote scope, inspect changed files via `jj file show -r <remote-head-ref> <path>` or diff hunks; do not inspect workspace copies. They must not edit files, change the working-copy revision, alter descriptions, push, create PRs, or otherwise mutate repository state.
...
The artifact file **must** carry the full detail-tier fields (`why_it_matters`, `evidence`); the compact *return* omits all detail-tier fields **except `first_evidence`**, but writing the compact shape to the artifact silently strips detail. However review context is delivered, each reviewer still receives the full AI Assistant template output contract. `suggested_fix` is optional in both tiers. If the file write fails, the compact return still provides everything the merge needs.
