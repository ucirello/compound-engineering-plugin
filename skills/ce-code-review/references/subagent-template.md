...
(For a large prepared review, `{file_list}` and `{diff}` may be **file paths** rather than inline content. When a value above is a path, Read that file to get the full list/diff before reviewing — never treat the path string itself as the content to review.)
</review-context>
```
...
| `{pr_metadata}` | Stage 1 output | PR title, body, and URL when reviewing a PR. Empty string when reviewing a branch or standalone workspace |
| `{file_list}` | Stage 1 output | Changed-file list — inline, or a prepared file path to Read for a large review |
| `{diff}` | Stage 1 output | The diff to review — inline hunks, or a prepared file path to Read for a large review |
