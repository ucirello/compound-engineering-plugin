...
Run this once at the start of this invocation, before any subagent dispatch, and follow the directives it prints — except where one conflicts with this skill's own rules on asking the user questions, whether those rules are scoped to a non-interactive mode or apply in every mode, in which case this skill's rules win and no blocking question is asked. Run the fence exactly as written, as its own command: do not pipe or filter it (no `head`, `tail`, or `grep`), do not truncate its output, and do not bundle it into a batch with other commands. Its output opens with a `=== skill context` header and ends with `WORK_CONTEXT_END`; if you received one of those lines without the other, the output was truncated — rerun the fence verbatim once. That recovery is the only rerun: otherwise do not rerun it within the same invocation; a later invocation of this or any other skill runs its own. If no Node runtime is available the skill proceeds unchanged.
...
- **Read** `docs_root` from `<workspace-root>/.rocketclaw/config.yaml` only (`<workspace-root>` = `jj workspace root`, with the current directory fallback when no workspace is available). Do not read it from `config.local.yaml`. Unset -> `<root>` is `docs`, exactly as before.
...
**Repository workspace availability.** Workspace-local implementation writes require a writable Jujutsu workspace. Before treating the current working directory as the project, confirm `jj workspace root` resolves to a workspace you can edit. If this session has no writable workspace, but the user named a repository and the harness exposes a remote repository surface with a writable workspace, run the implementation work on that surface and treat it as canonical for verification, changes, and handoff. Otherwise skip workspace-local writes and report that no writable workspace is available; do not synthesize file changes from a non-repository scratch directory.
...
When a valid `implementation_engine:` binding is present without recovery, **pre-controller discovery is read-only**. Do not run baseline, test, build, format, install, or generation commands in the canonical workspace before resolving the binding and initializing the external controller: those commands can create ignored or untracked artifacts before the controller records its clean starting point. Limit triage to reads such as metadata, source, configuration, bookmark, status, and command-availability probes. If a non-read probe is genuinely required to decide whether the route can start, run it only with artifact suppression and prove the canonical Jujutsu snapshot is byte-for-byte unchanged before continuing; otherwise stop with a route blocker.
...
- **Read** `<workspace-root>/.rocketclaw/config.local.yaml`, then `config.yaml` (`<workspace-root>` = `jj workspace root`, with the current directory fallback when no workspace is available). Missing files are skipped. Ignore rules do not change resolution.
...
1. **Read Plan and Clarify** _(skip if arriving from Phase 0 with a bare prompt)_
   ...
   - **Do not edit the plan body during execution.** The plan is a decision artifact; progress lives in Jujutsu changes and the task tracker, not the plan. `ce-work` does not mutate the plan — whether it shipped is derived from `jj log`, not recorded in the doc. Legacy plans may contain `- [ ]` / `- [x]` marks on unit headings or a `status:` field — ignore them as state; per-unit completion is determined during execution by reading the current file state.

2. **Setup Environment**

   Two things must hold before the first edit: the work has a dedicated Jujutsu change and bookmark for publication, and nothing the user did not offer up gets described, finalized, or published by this run. Neither is a question for the user — Jujutsu operations are recoverable through the operation log, so do it and say so in one line.

   **Bookmark and change.** Determine the default bookmark from tracked remote bookmarks; otherwise use what the host reports as the default, such as `gh repo view --json defaultBranchRef`; otherwise use `main` or `master` when present. Refresh with `jj git fetch --remote origin` when a remote exists. If the working-copy change is at the default bookmark, has no suitable local bookmark, or cannot be classified, start a dedicated change from the fresh remote bookmark when the current change has no unique work beyond it; otherwise start from the current change so existing work remains visible to idempotency and shipping checks. Create a neutral bookmark name derived from the plan or work description and verify it with `jj bookmark list`. Otherwise continue on the invoked bookmark without renaming it. A separate workspace is used only when the user asked for one this session (`ce-worktree`); the default bookmark is moved or published only when the user explicitly said so this session.

   **Pre-work scope.** Before editing, record `jj diff --summary -r @` and whether `jj log -r '<remote-default>..@'` shows changes not on the remote default (`unknown` without a remote). Nothing in that set is yours to describe, finalize, or publish, and it remains untouched in its existing change. No stash is needed. It is enforced without a menu:
   - Incremental changes include only work-owned files through path-limited `jj commit` (Phase 2), so untouched work never enters the finalized change; the Phase 4 handoff passes every pre-work file this run did not finalize as `exclude:<paths>`, and stays local via `ce-commit` when pre-existing unpublished changes are on the bookmark (`references/shipping-workflow.md`).
   - A unit that must edit a file already changed in `@` cannot be separated without moving content between changes. Ask once, covering every such file, before the first finalization that would include one: include those edits, or leave them in the working-copy change as an exclusion for the rest of the run and name them as unshipped. In Return-to-Caller Mode do not ask and do not edit the file; return `status: blocked` naming it so the user can split or move the prior work and rerun.
...
4. **Choose Execution Engine, then Strategy**
   ...
   **Route resolution is a mandatory pre-write gate.** Before any implementation write, native worker dispatch, or implementation change finalization, read `references/execution-engines.md`; inspect applicable live/session/project intent, any typed caller binding, and `.rocketclaw/config.local.yaml` then `config.yaml` when they exist; then resolve and record the engine. Do not infer native execution merely because no typed carrier was supplied. Native is eligible only after this gate finds no higher-authority cross-model selection or exhausts a `prefer` route under the reference's fallback contract.
   ...
   **For ordinary native workers, isolation is the harness's job, never ce-work's** — never run `jj workspace add` yourself for inline/subagent, goal-mode, or dynamic-workflow execution. The only exception is the external cross-model controller, which owns sibling Jujutsu workspaces under `<workspace-root>/.tmp/rocketclaw/ce-work/<run-id>/`. Probe what your native subagent mechanism provides and pick the parallel path:
   - **Harness-native isolated workers** — each worker edits an isolated workspace the harness manages. This works even when you are already inside a linked workspace because harness-managed workspaces are peers, not nested. Parallelize only units that pass the Safety Check; isolation makes recovery possible, not overlap safe.
   ...
   **Fresh worker invariant (native subagent dispatch only):** When dispatching an implementation unit to a native subagent worker, create a new worker context with no prior implementation-unit transcript. Bind the worker handle to exactly that unit: it may continue or recover the same unit, but never receive a different unit. Retire each handle after its unit is integrated; never retask it or retain idle implementation workers for reuse. Invoke an explicit close/release operation only when the active harness exposes one and assigns that lifecycle action to the caller. Inline execution creates no worker context or handle, so it has nothing to retire.
   ...
   - **Do not finalize or publish.** Ordinary native workers implement and may run their *own unit's* focused tests in isolation as a self-check, but the **orchestrator owns Jujutsu descriptions, finalization, bookmarks, and authoritative test runs**. An external cross-model worker must not run `jj describe`, `jj commit`, `jj bookmark`, or `jj git push`. Leave its working-copy change for the host to snapshot through the controller's Git-backed transport interoperability. Transport objects are never canonical Jujutsu changes.

   **Shared-workspace constraints** — when subagents share your working directory (no isolation), they must not describe, commit, move bookmarks, publish, or run the full test suite concurrently; the orchestrator does all of that after the batch. A worker may run a single focused unit test only if it touches no shared state.
   ...
   **After a parallel inline/subagent batch — the orchestrator integrates; never trust the handoff summary alone:**
   ...
   2. **Inspect the actual tree, not reported paths.** Determine what each worker really changed with `jj diff --summary` and `jj diff` in its workspace or the shared directory. Reported paths are a hint; declared `Files:` are often incomplete — workers create or modify files the plan did not anticipate.
   ...
   **Per-harness integration (examples — the universal flow above is the contract):**
   - **Harness-owned workspace/change:** integrate one change in dependency order, verify, and finalize before the next; on conflict re-run or explicitly resolve that unit against the advanced change.
   ...
   - **External cross-model workspace:** follow the conditionally loaded cross-model parallel-wave protocol and controller receipts; ordinary bookmark or change-integration shortcuts do not apply.
...
2. **Incremental Changes**
   ...
   | Finalize when... | Keep the change open when... |
   |----------------|---------------------|
   | Logical unit complete (model, service, component) | Small part of a larger unit |
   | Tests pass + meaningful progress | Tests failing |
   | About to switch contexts (backend → frontend) | Purely scaffolding with no behavior |
   | About to attempt risky/uncertain changes | A complete description is not yet possible |

   **Heuristic:** Can the current change receive a description of a complete, valuable result? If yes, finalize it; otherwise keep working without applying a placeholder description.

   If the plan has Implementation Units, use them as a starting guide for change boundaries, adapting to implementation evidence. A unit may need multiple changes, or small related units may land together. Runtime project instructions and history read via `jj log` win. Apply compatible Go quality guidance. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards.
   ...
   **Change workflow:**
   ...
   # 2. Inspect only files related to this logical unit
   jj diff --summary -- <work-owned-paths>

   # 3. Finalize those paths with the dynamically composed description
   jj commit -m "<description derived from active instructions and history>" <work-owned-paths>
   ```

   The fileset on `jj commit` is load-bearing: it keeps unrelated working-copy changes in the new `@` while finalizing only work-owned paths in the current change.

   **Handling conflicts:** If conflicts arise during rebasing or integration, resolve them immediately. Focused Jujutsu changes keep conflict resolution bounded.

   **Parallel subagent mode:** Commit ownership is split by isolation mode (see Phase 1 Step 4):
   - **Workspace-isolated:** subagents leave their Jujutsu working-copy changes open; the orchestrator integrates and finalizes them in dependency order after the batch.
   - **Shared-directory fallback:** subagents do not finalize changes; the orchestrator finalizes each unit after the entire parallel batch completes.
