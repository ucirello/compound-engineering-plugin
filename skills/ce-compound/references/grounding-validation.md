...
- **Merge-state claims** ("fixed in #1608", "landed", "shipped") verify against **remote truth** — the workspace may predate a merge, so `gh pr view` (or the tracker equivalent) is primary and local JJ reachability is only the fallback. The script's `INFO: workspace is N revisions behind …` line tells you how much to distrust the local working-copy revision for this category.
...
| path missing here, exists at `trunk()` | Stale workspace | Verify the claim against `trunk()`; annotate if the doc implies the file is present in the working copy |
...
| SHA reachable from `trunk()` only | Workspace predates the merge | Keep, with a temporal qualifier; verify the landed claim via `gh` |
