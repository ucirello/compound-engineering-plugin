...
Skip if nothing changed. Otherwise describe a change that contains **only** the files this refresh modified, in the repo's convention. **Read `references/commit.md`** for the per-mode bookmark decision and the jj-failure fallback.
