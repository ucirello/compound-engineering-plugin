...
argument-hint: "[PR number, comment URL, or blank for current working copy's PR]"
...
| Argument | Mode |
|----------|------|
| No argument | **Full** -- all unresolved threads on the current working copy's PR |
...
Keep scratch files under `$(jj workspace root)/.tmp`. If the workspace root cannot be resolved, use `.tmp` relative to the current directory. Do not use external temporary directories.
