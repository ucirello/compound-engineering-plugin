...
When `<pr-scope-mode>pr-remote</pr-scope-mode>` or `<pr-scope-mode>branch-remote</pr-scope-mode>` is in context, do **not** Read/Grep the workspace copy of {finding_file}. Inspect via `jj show <pr-head-ref>:{finding_file}` or `jj show <branch-head-ref>:{finding_file}` when a remote head ref is set; otherwise use diff hunks only.
...
- Do not edit, change, push, or modify any files. You are operationally read-only.
