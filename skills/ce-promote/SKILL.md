...
- **The diff** — use the PR base when available, otherwise the project's default bookmark, then run `jj diff --from '<base>@origin' --stat` and skim notable changes to ground the claim in what actually changed.
