...
- **Body:**
  ...
  - Source: a link to the PR carrying this change when one already exists at filing time; otherwise the bookmark and head commit id (`jj log -r @ -T 'commit_id' --no-graph`), so the ticket points at the code even before a PR is opened. When the same run opens a PR after the ticket is filed, back-fill the PR link into the ticket (best-effort; never block shipping on the update).
