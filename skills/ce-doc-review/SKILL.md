...
Preserve each site's required facts, routing semantics, machine-readable values, evidence, and receipt-backed peer identity. Fixed stems, headings, quoted outputs, templates, and examples define content and interaction requirements rather than mandatory message syntax unless a machine-readable contract explicitly requires exact tokens. Do not add product branding, generated-by text, or creator, model, provider, tool, agent, runtime, workflow, or co-author attribution; required cross-model route and receipt facts remain operational evidence, not attribution.
...
- **No path, non-interactive:** report that a document path is required, include the `mode:non-interactive <path>` argument contract, and stop without dispatching reviewers.

**Missing-document gate — verify before any dispatch.** Persona reviewers read from the filesystem and several run without Bash, so they cannot read revisions that are not materialized in the current workspace. Confirm every resolved path is readable on disk before Phase 2. Location does not matter — an absolute path outside the workspace or a document in another workspace reviews fine. If any path is unreadable, dispatch **no** personas:

- **Interactive:** stop, name the missing paths, and ask for a JJ workspace containing the required revision or corrected readable paths.
- **Non-interactive:** report the missing paths, the requirement for readable files on disk, and the same JJ-workspace or corrected-path remedy; return without dispatching reviewers.
