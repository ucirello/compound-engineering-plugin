...
2. **Working-copy revision.** If `jj diff -r @` is non-empty, review that change together with its reviewed stack.
3. **Unpublished stack vs base bookmark.** If the working-copy revision is empty, review `jj diff --from <base> --to @`, where `<base>` is the resolved default bookmark.

The scope step in the SKILL.md handles discovery and passes you the resolved diff. You do not need to run Jujutsu commands yourself unless remote scope mode requires it below.

## Remote scope (`pr-remote` and `bookmark-remote`)

When the review context includes `<pr-scope-mode>pr-remote</pr-scope-mode>` or `<pr-scope-mode>bookmark-remote</pr-scope-mode>`, the working copy is **not** the reviewed head. Do **not** use Read/Grep on workspace paths for files in the changed-file list; they may not match the bookmark or PR under review.
...
In `pr-remote` / `bookmark-remote` scope these tiers inspect the working copy, which is not the reviewed head — apply the Remote scope rules above (`jj file show -r <remote-head-ref>`, then search the returned revision content) instead of local search.
