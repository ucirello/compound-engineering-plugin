...
Build under `.context/ce-prototype/<date>-<slug>/`, so the prototype survives for the implementation that follows. Fall back to `.tmp/ce-prototype/<date>-<slug>/` under the workspace, where survival is best-effort. `references/build.md` names every case that forces the fallback root.

The `.context` path has to be gitignored first. Probe it from the workspace root by reading the repo-root `.gitignore` for a rule covering `.context/`; the trailing slash is load-bearing. Do not call git. When it is not covered, the offer to add that line comes before the root is resolved, or accepting it cannot help this run.
