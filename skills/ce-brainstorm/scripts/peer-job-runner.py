...
Environment overrides (defaults in parentheses):
  ...
  ROCKETCLAW_WORKSPACE_ROOT         explicit workspace root before local fallback
...
the workspace-root `.tmp/rocketclaw` path, with the current directory as a local
fallback. Every read of job state opens the file first (no-follow) and
...
environment overrides: ROCKETCLAW_PEER_JOBS_ROOT, ROCKETCLAW_WORKSPACE_ROOT,
ROCKETCLAW_WORK_RUNS_ROOT, ROCKETCLAW_PEER_IDLE_SECS, ROCKETCLAW_PEER_HARD_SECS,
ROCKETCLAW_PEER_LOG_MAX_BYTES, ROCKETCLAW_PEER_RESULT_MAX_BYTES,
ROCKETCLAW_PEER_POLL_SECS, ROCKETCLAW_PEER_GRACE_SECS (defaults in the module
docstring).
"""
...
def jobs_root_base() -> str:
    ...
    workspace_root = os.environ.get("ROCKETCLAW_WORKSPACE_ROOT")
    if not workspace_root:
        try:
            ...
            workspace_root = probe.stdout.strip()
        except (OSError, subprocess.CalledProcessError):
            workspace_root = os.getcwd()
    return os.path.abspath(os.path.join(workspace_root, ".tmp", "rocketclaw"))
...
def _require_posix_detach() -> None:
    ...
    if not hasattr(os, "fork") or not hasattr(os, "setsid"):
        ...
        raise RunnerError(
            ...
            "started. On native Windows, run this skill under WSL or wait for "
            "a Windows-native detach path."
        )
