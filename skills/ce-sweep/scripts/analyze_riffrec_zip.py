...
video frames when available, and portable markdown artifacts.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        help="Directory for extracted evidence/kickoff artifacts. Defaults to workspace .tmp/rocketclaw/ce-sweep/analyzer/<source-stem>; durable ce-brainstorm outputs live in the plans artifact directory.",
    )
...
def write_requirements_kickoff(
    ...
    lines.extend(
        [
            ...
            "## Next Steps",
            "",
            "-> Invoke `ce-brainstorm` through the active harness to confirm candidate findings and replace generic R-items with product-specific requirements.",
        ]
    )
    output_path.write_text("\n".join(lines) + "\n")
...
def write_source_materials(
    ...
    lines = [
        ...
        "- Recording policy: raw media, audio chunks, zip contents, session dumps, and extracted screenshots are local-only by default; record generated Markdown/JSON/manifests in a JJ change only when useful for brainstorm/planning traceability.",
    ...
    if chunk_files:
        ...
        lines.append(f"  - retained locally in `{link(raw_dir / 'transcription_chunks')}`; not suitable for a recorded change by default.")

    lines.extend(["", "## Local-Only Frames", ""])
    lines.append("Extracted screenshots are retained locally for agent inspection and should not be recorded in a JJ change by default.")
    lines.append("")
    if moments:
    ...
    lines.append("Raw files are intentionally local-only by default. Do not record these in a JJ change unless the user explicitly asks and privacy/security is acceptable.")
    lines.append("")
...
def main() -> int:
    ...
    print(f"Brainstorm handoff: invoke ce-brainstorm through the active harness with {display_path(kickoff_md, workspace_root)}")
