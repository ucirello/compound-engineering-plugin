...
2. **Working-copy change.** If `jj diff -r @` is non-empty, review that current change when the supplied scope selects it.
3. **Stack vs base bookmark.** When the current change is empty, review the full stack from the resolved merge base to `@`; do not lose the non-empty parent `@-`. Stage 1 supplies the exact base and target endpoints.

The scope step in the SKILL.md handles discovery and passes you the resolved diff. You do not need to run JJ commands yourself unless PR scope mode requires it (below). Run every JJ command with cwd at the supplied absolute workspace root; set `GIT_DIR=$(jj git root)` there before `gh` calls.

The ignored-file inventory helper owned by `ce-work` has a scoped compatibility exception: `unit_workspace_ignored.py` may execute only the read-only `git ls-files --others --ignored --exclude-standard -z --`, with cwd and `GIT_WORK_TREE` set to the absolute workspace root and `GIT_DIR` obtained there through `jj git root`. Preserve its before/after ignored metadata and reporting. Do not flag that authorized command for removal; executable Git operations outside that exception remain disallowed.
...
In `pr-remote` / `branch-remote` scope these tiers inspect the working tree, which is not the reviewed head — apply the Remote scope rules above (`jj file show` / `jj file search -r <remote-head-ref>`) instead of local search.
