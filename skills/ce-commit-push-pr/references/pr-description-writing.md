...
For current-bookmark mode, resolve `<base>` in priority order: `jj bookmark list -r trunk()` (parse the trunk bookmark name) → `GIT_DIR="$(jj git root)" gh repo view --json defaultBranchRef --jq '.defaultBranchRef.name'` → try `main`/`master`/`develop` via `jj log -r 'present(<candidate>@origin)'`. If none resolve, ask the user. `<head>` is `@`.
...
jj git fetch --remote <base-remote> --branch <head-bookmark>   # PR mode only, when the head is a named bookmark
jj log --no-graph -T builtin_log_oneline -r '<base>@<base-remote>..<head>'
jj log --no-graph -T builtin_log_compact_full_description -r '<base>@<base-remote>..<head>'   # full commit messages for related-reference discovery
jj diff -r '<base>@<base-remote>..<head>'
```

If the commit list is empty, report "No commits to describe" and stop.

**Fallback** — use `GIT_DIR="$(jj git root)" gh pr diff <ref>` and `GIT_DIR="$(jj git root)" gh pr view <ref> --json commits` when local Jujutsu can't reach the refs (fork PR with no matching remote, shallow clone, offline, unrelated histories). There is no public Jujutsu command to fetch `refs/pull/<number>/head` or to read FETCH_HEAD; do not probe `.git/` or `.jj/`. Use the `gh` fallback when the head commit is not already present.
...
Compose the title from the scope map's umbrella outcome, not one cluster or mechanism, matching repository-local title conventions (project instructions in context, then recent change descriptions). With program context, the title may name this PR's contribution under the program without restating the whole series — it must not make another material outcome sound incidental. Imperative, lowercase, under 72 chars, no trailing period, when that matches local convention. Do not choose a type, scope, prefix, or subject template. **Never use `!` or `BREAKING CHANGE:` without explicit user confirmation.**
...
- **non-closing reference** — related, partial, investigative, follow-on, validation-only, or tracker semantics unknown.
...
- Good: "This covers the duplicate-row retry path; concurrent cancellation remains follow-on work."
- Good: "Related: #123"
...
```bash
jj file search --name-only -r '<base>@<base-remote>' --pattern "<term>"
```
