...
- [ ] Validation/evidence context passed to `ce-commit-push-pr` when the change has observable behavior. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. For validation of this change-description input, repository-local syntax from active project instructions and runtime history inspected with `jj log -r '::@' --limit 20 --no-graph -T 'description.first_line() ++ "\n"'` wins over incompatible Go guidance. Only compatible Go guidance for quality, clarity, and structure applies. No fixed prefix, type, scope, subject, body, layout, template, or example was imposed, and any required placeholders are neutral.
- [ ] Commit messages were composed and validated under this rule: Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Repository-local syntax from active project instructions and runtime history inspected with `jj log -r '::@' --limit 20 --no-graph -T 'description.first_line() ++ "\n"'` wins over incompatible Go guidance. Only compatible Go guidance for quality, clarity, and structure applies. No fixed prefix, type, scope, subject, body, layout, template, or example was imposed, and any required placeholders are neutral.
- [ ] PR description includes Post-Deploy Monitoring & Validation section (or explicit no-impact rationale), validated under this rule: Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Repository-local syntax from active project instructions and runtime history inspected with `jj log -r '::@' --limit 20 --no-graph -T 'description.first_line() ++ "\n"'` wins over incompatible Go guidance for the change description. Only compatible Go guidance for quality, clarity, and structure applies. No fixed prefix, type, scope, subject, body, layout, template, or example was imposed, and any required placeholders are neutral.
