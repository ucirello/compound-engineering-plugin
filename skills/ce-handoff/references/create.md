...
if [ -L "$SCRATCH_PARENT" ] || [ -L "$SCRATCH_ROOT" ]; then echo "unsafe scratch root symlink: $SCRATCH_ROOT" >&2; exit 1; fi;
...
Use a readable topic slug as the filename. When a JJ workspace is available, use a sanitized repository name plus a stable root-change prefix as the repository namespace; otherwise use `general`. The managed store is workspace-local, so another workspace will not see these files. Do not put a timestamp or unique ID in the path by default; `created_at` carries chronology for discovery. Reserve the final candidate filename atomically and exclusively; on collision, retry with the smallest available numeric suffix rather than overwrite a handoff. Never check availability and then write. Keep the directory and file user-private where the platform supports permissions.
...
repo_root_sha: "First root change commit_id when available"
branch: "Captured bookmark when available"
head: "Captured @ when available"
worktree_path: "Captured workspace when relevant"
---
```
