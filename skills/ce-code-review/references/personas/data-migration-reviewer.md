...
1. **Schema drift (when `schema.rb` / `structure.sql` is in the diff)** — unrelated dump changes from other changes
...
Run this **first** when `db/schema.rb` or `db/structure.sql` appears in the diff. Use the resolved Jujutsu base revision from caller context (`<review-base>`). **Never assume `main`.**

```bash
jj diff --from <review-base> --name-only 'glob:db/migrate/**'
```
...
jj diff --from <review-base> --git db/schema.rb

# When db/structure.sql is in the diff:
jj diff --from <review-base> --git db/structure.sql
```
