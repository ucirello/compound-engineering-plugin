...
- No incremental code-oriented JJ changes, and none of `references/shipping-workflow.md` (no PR, no CI).
...
4. **Save and report.** Write the deliverable to a durable, repo-tracked location — default to a sensible `docs/` subpath (or a path the user named at the checkpoint) — and report its absolute path so the user can find it. Whether to record it in a JJ change vs. leave it written is the user's call; offer, don't force.
