...
- `<root-id>` = lexicographically-first `jj log -r 'root()+' --no-graph -T 'commit_id ++ "\n"'` — the repo identity (stable, shared across workspaces and clones).
- `<current-id>` = `jj log -r @ --no-graph -T 'commit_id'` — the working state.
