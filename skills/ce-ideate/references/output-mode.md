...
**Read config.** Resolve `<repo-root>` with `jj workspace root`, then apply the ordinary-key cascade block in `SKILL.md`, including its OpenCode V2 settings precedence. Read both files when they exist. If the root cannot be resolved, fall through to the defaults below.
