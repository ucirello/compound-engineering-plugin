...
Loaded by `SKILL.md` at the start of Phase 1. Captures the configuration that will be merged into `.compound-engineering/config.local.yaml` (the unified CE local config, VCS-ignored, machine-local) as `pulse_*` keys and re-read on every subsequent run.
...
If the file does not yet exist, create the directory and file. If `.compound-engineering/config.local.yaml` is not already covered by the repo's shared ignore rules, offer to add the entry before writing.
