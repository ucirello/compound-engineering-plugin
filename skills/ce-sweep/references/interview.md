...
- **Committed to the repository** (recommended when multiple assistants or machines share a bookmark — one source of truth everyone reads and writes). Sets `sweep_state_path` to the committed default `docs/feedback-sweep/state.yml`.
- **Machine-local under the workspace `.tmp` directory** (solo setups; keeps sweep bookkeeping out of changes). Resolve the path immediately with this shell block, using the workspace root resolved by `jj root` or the current directory as the local fallback:
...
**Ask:** "Is this a multi-assistant setup where several Jujutsu workspaces push the sweep state to a shared docs bookmark? Answer yes only if more than one machine or assistant describes and pushes changes to the same bookmark. Default is no — a single workspace recording changes locally."
...
Merge the captured settings into `<workspace-root>/.rocketclaw/config.local.yaml`. Resolve the workspace root with `jj root`; if that fails, use the current directory as the local fallback.
...
sweep_state_path: docs/feedback-sweep/state.yml   # committed (multi-assistant) or workspace .tmp path (solo)
