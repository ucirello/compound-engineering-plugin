...
<scope-context>
The diff above is the full change being reviewed. The finding is about file {finding_file} around line {finding_line}. If `<diff>` contains an artifact path, Read it first.

When `<pr-scope-mode>pr-remote</pr-scope-mode>` or `<pr-scope-mode>bookmark-remote</pr-scope-mode>` is in context, do **not** Read/Grep the workspace copy of {finding_file}. Inspect via `jj file show -r <pr-head-ref> {finding_file}` or `jj file show -r <bookmark-head-ref> {finding_file}` when a reviewed revision is set; otherwise use diff hunks only.

When scope is local-aligned (default), use read tools plus targeted `jj file annotate` and `jj log` to inspect the cited code and its callers, guards, middleware, or framework defaults.
</scope-context>
...
2. **Is the issue introduced by THIS diff?** Use `jj file annotate`, `jj log`, or diff inspection against the reviewed revision. If the cited line predates the reviewed changes and the diff does not interact with it, the finding is pre-existing. When history informs that verdict, cite concise change-ID provenance (for example, `provenance: <change-id> <author> <date> - <description>`). Missing load-bearing provenance weakens confidence but does not alone reject an otherwise-correct finding.
...
- Do not edit files, describe revisions, move bookmarks, push, or otherwise mutate workspace or repository state.
