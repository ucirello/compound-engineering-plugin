...
1. Run the analyzer in workspace-local scratch so nothing pollutes tracked project paths (`SKILL_DIR` is the directory containing the `ce-riffrec-feedback-analysis` SKILL.md; set it in the same command — shell state does not persist between Bash calls):
   ...
   OUTPUT_DIR="$ROOT/.tmp/rocketclaw/riffrec-feedback/quick-$$"
   mkdir -p "$OUTPUT_DIR"
   python "$SKILL_DIR/scripts/analyze_riffrec_zip.py" /path/to/input --output-dir "$OUTPUT_DIR"
   ```

   This uses the current directory's `.tmp/rocketclaw/riffrec-feedback/` as the fallback outside a Jujutsu workspace. Capture the printed output directory; later steps read from it.
