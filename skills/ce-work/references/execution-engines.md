...
`ce-work` has four implementation engines: inline/subagent, goal-mode, dynamic-workflow, and cross-model execution. The engine decides *how* implementation runs; it never changes *who* owns the shipping tail. Native inline/subagent execution remains selected unless applicable live intent, a caller binding, or an enabled standing preference selects the fourth engine.
...
| Applicable live intent, a caller binding, or enabled config resolves a qualified fixed external route | **Cross-model execution** | A fixed external worker handles bounded units while the host retains canonical jj integration, verification, descriptions, bookmarks, and tail ownership. |
...
Follow the dispatch strategy in `SKILL.md` Phase 1 Step 4 and the Phase 2 execution loop. `ce-work` owns task creation, unit sequencing, dispatch, verification, change descriptions, and bookmark advancement.
...
Tail ownership decides who opens the final PR; it does not forbid progress signals during a long run. For multi-hour goals, described jj changes and an optional artifact under the workspace-local `.tmp/rocketclaw/ce-work/` run directory keep progress observable. Only final PR creation is gated: a standalone top-level goal may open a draft PR only when it explicitly owns that channel; in return-to-caller mode `ce-work` must not open any PR, but may finalize local changes and return a progress report. Never write progress into the plan body; jj history and the envelope carry it.
