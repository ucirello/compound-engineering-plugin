...
- **`pr-snapshot watch`** is that detector: it performs the same fetch-to-diff operation without AI Assistant reasoning, prints one `BABYSIT_WAKE {reason,url,...}` line only on an actionable change or stop condition, then exits.
- AI Assistant backgrounds `watch`, waits with the harness's background-and-wake capability, runs a tick, and re-arms. The loop stays in the current session, preserving prior decisions and live steering.
...
| Harness | Background-and-wake capability used by AI Assistant | Durable beyond the session? |
|---------|-----------------------------------------|-----------------------------|
...
- `--settle-seconds` (default 300) is the quiet window before a `merge-ready` wake, so AI Assistant is roused only after the PR cools off. Leave it unset on the normal arm; only the post-rejection re-arm changes it.
...
A loop can churn without finishing: CI ping-pong, a review treadmill, or wrong-approach whack-a-mole. A raw attempt counter cannot distinguish these from legitimate progressive repair, so the decision is **AI Assistant judgment over the trajectory**.
...
The rule that makes ticks idempotent and crash-safe: **the snapshot never marks an item handled just from observing it.** An item leaves the actionable set only when AI Assistant confirms action through `mark` or remote truth removes it. A failed pass therefore remains actionable next tick.
...
- **Lock.** The script takes a file lock around each state read/write. It cannot span AI Assistant mutations between script calls, so it is necessary but not sufficient.
