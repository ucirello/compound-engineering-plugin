# Dogfood Report — <bookmark/change>

> Diff-scoped browser QA of `<bookmark/change>` vs the trunk. Generated by `/ce-dogfood` on <YYYY-MM-DD>.
...
<What changed between the bookmark/change and the trunk: new features, modified behavior, new/changed routes, views, components, data flows. 2-6 bullets.>
...
### <Short issue title> — `<change/commit-id>`
...
- **<Persona>** — <paper cut> — <severity> — <fixed `<change/commit-id>` / deferred>
...
<Overall readiness verdict for the bookmark/change. Ready to ship? Caveats? Outstanding blocked items? Record the result of the Phase 5 automated test suite run — a green matrix with a red suite is not "ready.">
