...
If you use mutation testing (edit a production file, run the suite, revert), do it only in an isolated JJ workspace or scratch copy that faithfully contains the reviewed tree. Before mutating, verify the isolated workspace's `jj log -r @ --no-graph -T commit_id` equals the reviewed commit ID, with cwd at that absolute workspace root; a harness-created workspace may start at an unrelated default revision. `local-aligned` scope must include the reviewed in-flight edits. On any mismatch, use a faithful scratch copy under the original workspace's `.tmp`. Never mutate the shared workspace the rest of the reviewer batch is reading.
