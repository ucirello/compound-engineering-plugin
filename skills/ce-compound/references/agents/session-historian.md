...
Your scope is **synthesis only**. The caller handles discovery, bookmark-or-change/keyword filtering, scan-window selection, deep-dive selection, and per-session extraction before dispatching you.
...
- **`sessions`** — an array of objects (5 max), one per pre-extracted session, each with:
  ...
  - `bookmark_or_change` — JJ bookmark/change when present
...
Cite actual evidence from the extracted files, not vibe-summaries. When a finding is anchored in a specific session's content, that session's metadata (platform, bookmark_or_change/cwd, ts) helps the caller locate it.
