...
- **Merge policy (Ulderico's fork):** Pull requests and upstream merge gates apply to `main@upstream` only. For this automated port, publish directly to `main@origin` after the prompt's mandatory validation passes, with exactly one non-empty commit above the starting upstream release and no merge commit. Do not open a PR for `origin`. Preserve existing origin-only work in the validated result. The prompt's focused validation replaces upstream CI/test requirements for this port; all unrelated repository guidance remains applicable.
