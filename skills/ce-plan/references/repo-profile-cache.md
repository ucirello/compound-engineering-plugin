...
- `<root-revision>` = lexicographically-first JJ root ancestor commit ID from `jj log -r 'roots(::@ & ~root())'` — the repo identity.
- `<current-revision>` = `jj log -r @- -T commit_id` — the parent commit; dirty profile inputs are handled by the status guard below.

Two workspaces at the same current revision share the same entry. Lookup is VCS metadata only; on a hit, only this one file is read.
