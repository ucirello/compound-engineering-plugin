...
- **No blocking prompts.** Never use `AskUserQuestion`, `request_user_input`, `ask_user`, or other blocking question tools. Infer intent, plan, and scope from explicit tokens, jj workspace state, PR metadata, and conversation. Note uncertainty in Coverage or the verdict — do not stop to ask.
- **Explicit mutations only.** Never run `gh pr checkout`, `jj edit`, `jj new` onto another bookmark, or similar working-copy switch commands. Passing a PR number, URL, or bookmark name selects **review scope**, not permission to mutate the working copy. Undescribed work can only be reviewed from the workspace that holds it, so to review it on a feature bookmark, stay on that bookmark (or edit it yourself) and pass `base:` or no target.
