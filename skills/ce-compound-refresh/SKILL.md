...
6. **Fold in or create a follow-up change when the check produces edits.** If step 4 or step 5 edited an instruction source after Phase 5 recorded the refresh, use `jj squash --into <refresh-revision> <instruction-paths>` when that revision has not been published. If it has been published, describe the instruction edits as a follow-up revision. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. The project's active instructions and syntax observed via `jj log` win; do not impose a fixed prefix, type, scope, template, or example. Move the same bookmark to the new tip, and publish it with `jj git push --bookmark <bookmark>` so the open review request includes the update. If the user chose to leave the refresh undescribed, leave these edits in the same working-copy change; no separate revision logic is needed.
