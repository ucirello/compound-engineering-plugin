...
1. Search the folder or collection the user supplied; otherwise resolve the workspace-local managed root in the current shell call with this block, then enumerate candidate files beneath `$SCRATCH_ROOT/ce-handoff/`. Bound the candidate set before inspecting content; prefer recent files and current Jujutsu workspace or working-directory affinity without making workspace affinity mandatory.
