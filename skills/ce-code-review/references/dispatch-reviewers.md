...
All other persona subagents and RocketClaw local prompt assets use the platform's balanced model tier to reduce cost and latency. See the Spawning subsection below for the dispatch-time override.
...
- **Balanced tier** — every other persona and RocketClaw prompt asset: pass the active platform's balanced model tier only when the dispatch primitive exposes an explicit supported selector. Otherwise omit the override and inherit the parent model; task wording alone does not select a model.
...
For each selected reviewer, read the corresponding local prompt asset from `references/personas/<reviewer-name>.md` and spawn a generic subagent using the subagent template. Do not use typed actor names or platform-level registration. Each persona subagent receives:
...
4. PR metadata: title, body, URL, and the resolved `<gh-repo>[HOST/]OWNER/REPO</gh-repo>` selector when reviewing a PR (empty string otherwise). Passed in a `<pr-context>` block so reviewers can verify code against stated intent and address the forge repository without relying on workspace colocation
...
Read-only here means **non-mutating**, not "no shell access." Reviewer sub-agents may use `jj workspace root`, `jj status`, `jj diff`, `jj log`, `jj file show`, `jj file annotate`, `jj file search`, and `gh pr view` to gather evidence. Every delegated `gh pr` call must pass `-R <gh-repo>` using the selector from `<pr-context>`; never let `gh` infer repository identity from the workspace directory. When a claim depends on line history, attach one concise provenance line from targeted annotation/history inspection. In **`pr-remote`** or **`bookmark-remote`** scope, inspect changed files with `jj file show -r <remote-head-ref> <path>` or diff hunks, never workspace paths. Reviewers must not edit project files, run `jj new`, `jj describe`, `jj split`, `jj squash`, `jj rebase`, mutate bookmarks, run `jj git fetch` or `jj git push`, create PRs, or otherwise mutate workspace or repository state.
...
**RocketClaw generic conditional local prompt assets** (`agent-native-reviewer`, `learnings-researcher`) are dispatched only when selected by Stage 3, through the same deterministic foreground batch dispatch as the structured personas. Read their prompt files from `references/personas/`, then give them the same review context bundle the personas receive: entry mode, any PR metadata gathered in Stage 1, intent summary, review base bookmark when known, `BASE:` marker, file list, and diff scope notes. Do not invoke them with a generic "review this" prompt. Their output is unstructured and synthesized separately in Stage 6.

**RocketClaw conditional local prompt assets** (`deployment-verification-assistant` only) are dispatched as generic subagents through the same deterministic foreground batch dispatch when the migration-artifact gate applies. Read the prompt file from `references/personas/`, then pass the same review context bundle plus the applicability reason. Its output is unstructured and must be preserved for Stage 6 synthesis just like the other selected local prompt assets. Schema drift is handled by the `data-migration` persona as structured findings, not here.
