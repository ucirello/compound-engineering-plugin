...
- **Moving-target guard:** if the recurrence traces to an external cause (a base-bookmark merge, a dep bump, flaky infra) rather than your fixes fighting each other, it is *not* an emergent trade-off. Keep fixing, and note the external cause. Recurrence is only meaningful when your own fixes are what oscillate.
