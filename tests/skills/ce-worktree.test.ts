...
describe("ce-worktree SKILL.md", () => {
  ...
  test("detects existing isolation before creating a workspace", () => {
    expect(
      ...
      "ce-worktree/SKILL.md must inspect the current JJ workspace root to detect existing isolation (Step 0).",
    ).toBe(true)
    expect(
      SKILL_BODY.includes("jj root"),
      "ce-worktree/SKILL.md must compare the workspace root against the repo root so a normal checkout is not misread as isolated.",
    ).toBe(true)
    expect(
      SKILL_BODY.includes("jj workspace list"),
      "ce-worktree/SKILL.md must list JJ workspaces so a named non-root workspace is detected.",
    ).toBe(true)
    expect(
  ...
  test("prefers the harness's native workspace tool before the JJ fallback", () => {
    expect(
      ...
      "ce-worktree/SKILL.md must instruct the agent to prefer the harness's native workspace tool before falling back to jj workspace (avoids phantom state).",
    ).toBe(true)
  })
  ...
  test("documents an inline JJ fallback under .workspaces with ignore safety", () => {
    ...
    expect(
      ...
      "ce-worktree/SKILL.md must require a trailing-slash `.workspaces/` ignore rule before creating JJ workspaces.",
    ).toBe(true)
  })
  ...
  test("anchors the JJ fallback at the repo root before using relative paths", () => {
    expect(
      ...
      "ce-worktree/SKILL.md must `cd \"$(jj root)\"` in the JJ fallback so relative `.workspaces`/`.gitignore` paths resolve at the repo root, not a subdirectory CWD.",
    ).toBe(true)
    expect(
  ...
  // PR #948 review: remote fetches can fail with no remote
