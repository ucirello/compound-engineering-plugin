...
**Trivial-bug fast-path:** Once the problem is clear, decide whether the framework is needed at all. If the cause is immediately readable from the input (single-file typo, missing import, obvious null deref or off-by-one with a one-line fix) and verification doesn't require deep tracing, present the cause and the proposed one-line fix and run Phase 2's **Fix it now / Diagnosis only** user-choice gate before editing — the fast-path saves investigation ceremony, not the user's choice over whether to apply a fix. If the user picks fix, run Phase 3's **Workspace and change check** (existing-work confirmation and trunk-change child/bookmark prompt), apply the fix, leave a one-line note explaining the cause, and skip to Phase 4's structured summary. If diagnosis only, write the summary and stop. When in doubt, run the full framework; getting the wrong root cause costs more than the few minutes of ceremony.
...
- Correct JJ change selected; no unintended existing changes
...
- The JJ Git remote (`jj git remote list`; a GitHub origin implies GitHub Issues + PRs; `gh` if available).
...
- Check the working copy (`jj status`). If the user has existing work in files that need modification, confirm before editing — do not overwrite in-progress changes.
- If the current change is the trunk change (`jj log -r 'trunk() & @' --no-graph` returns it), ask whether to create a feature change first using the platform's blocking question tool (see Phase 2 for the per-platform names). Default to creating one; run `jj new @`, derive a dynamic bookmark name from the bug, and run `jj bookmark create <dynamic-bookmark-name> -r @`. If `@` is already a distinct working-copy change, proceed; a bookmark is optional unless the project requires one.
- Record the pre-fix scope before editing: the current change ID from `jj log -r @ --no-graph -T 'change_id ++ "\n"'`, whether `jj status` is clean, and any pre-existing changed files from `jj diff --name-only -r @`. During Phase 3, keep a list of fix-owned files (the tests and implementation files changed for this bug). Phase 4 uses this to keep simplify/review from touching unrelated work.
...
**Re-verify after tail edits.** If simplification or review changed code, rerun the bug's regression test and any targeted checks the tail identified. Never proceed to describe, bookmark, or push a red change.
...
#### Skill-owned change (created in Phase 3): default to describe, bookmark, and open a PR without prompting
...
2. **Briefly preview what will happen** — what the JJ change contains, which bookmark will be pushed with `jj git push`, and that a PR will be opened — then proceed without waiting for confirmation. The preview exists so the user can interrupt; it is not a blocking question. Format and length are your call; keep it scannable.
3. **Run `/ce-commit-push-pr`.** Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Repository-local instructions and `git log` syntax always win; apply compatible Go quality guidance, and derive any prefix, type, scope, subject, or template dynamically instead of imposing a fixed one. The delegated workflow must describe the JJ change, create or advance its dynamic bookmark as needed, push with `jj git push`, and then open the PR. When the entry came from an issue tracker, use that tracker's documented auto-close syntax in the location the tracker requires so the diagnosis and fix flow back to the issue and it closes on merge. Surface the resulting PR URL.
...
When offering, use the blocking question tool described above. If the user accepts, run `/ce-compound`, then include the resulting learning doc in a new JJ change on the same bookmark and push with `jj git push` so the open PR picks it up. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Repository-local instructions and `git log` syntax always win; apply compatible Go quality guidance, and derive any prefix, type, scope, subject, or template dynamically instead of imposing a fixed one.
