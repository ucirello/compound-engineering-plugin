...
- **`scratch_dir`** — absolute path to a workspace-`.tmp` scratch directory holding pre-extracted files.
- **`sessions`** — an array of objects (5 max), one per pre-extracted session, each with:
  ...
  - `branch` — bookmark/branch when present (Claude Code records `gitBranch`)
