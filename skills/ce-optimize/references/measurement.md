...
```bash
jj log -r "optimize/<spec-name>" -n 1 --no-graph
```
...
```bash
jj bookmark create "optimize/<spec-name>"  # or `jj new "optimize/<spec-name>"` if resuming
```
...
The body owns this gate. Run `jj diff --name-only`, filter the output against `scope.mutable` and `scope.immutable`, and apply the body's rule to the result: name the dirty in-scope files and ask the user to describe/commit them or move them off the working copy with `jj new @-`, and do not continue until they are clean.
