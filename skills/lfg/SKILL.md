---
name: lfg
description: "Run the full autonomous shipping pipeline end-to-end, hands-off with no check-ins: plan, implement, review and fix, describe changes, push a bookmark, open a PR, and watch CI to green. Use only when the user explicitly asks to build or ship something autonomously all the way to an open PR, or invokes lfg directly — it pushes and opens a PR without stopping. Not for in-the-loop work where the user reviews each step: use ce-plan to plan, ce-work to implement a plan, ce-debug to fix a bug, or ce-commit-push-pr to describe changes and open a PR for existing work."
...
When invoking any skill referenced below, resolve its name against the available-skills list the host platform provides and use that exact entry. Some platforms list skills under a provider namespace; others list the bare name. Invoking a short-form guess that isn't in the list will fail — always match a listed entry verbatim before calling the Skill/Task tool.
...
Keep every temporary or scratch artifact under `<workspace-root>/.tmp/`, using `<workspace-root>/.tmp/rocketclaw/` when a namespace is needed. Resolve `<workspace-root>` with `jj workspace root`; if that fails because there is no Jujutsu workspace, use the current directory. Do not use OS-global temporary directories or `TMPDIR`.

<!-- docs-root:start -->
**Resolve the artifact root `<root>` before composing any artifact path.**

- **Read** `docs_root` from `<repo-root>/.rocketclaw/config.yaml` only (`<repo-root>` = `jj workspace root`, with the current directory as the no-workspace fallback). Do not read it from `config.local.yaml`. Unset -> `<root>` is `docs`, exactly as before.
- **Validate** a set value: a repo-relative directory whose real, symlink-resolved path stays inside the repo and is neither the repo root nor inside version-control metadata. Otherwise stop with an error naming `docs_root` and the value -- never fall back to `docs`.
...
2. Invoke the `ce-work` skill with `mode:return-to-caller <plan-path-from-step-1>` when no scalar transient carrier exists, including when a retained ordered current-task assignment is still active in context. When the scalar carrier exists, use the exact string-host form `mode:return-to-caller implementation_engine:<compact-json> <plan-path-from-step-1>`.
   ...
   For Go changes, require the implementation and verification evidence to follow the project's current instructions and history first, then compatible Go quality guidance for formatting, focused tests and static analysis, idiomatic APIs, error handling, and documentation. Select the actual tools and checks from the project rather than imposing a fixed command list.
...
3. Invoke the `ce-simplify-code` skill on the active Jujutsu change-set diff.

   This runs before review so the code-review in step 4 covers the simplified code. **Skip** this step when the change is docs-only (only markdown/docs paths changed) or trivial (roughly under 10 changed lines). Otherwise let `ce-simplify-code` resolve the active Jujutsu change-set scope from `jj diff` and the relevant revset; it preserves behavior and runs the test suite. Pass the plan path from step 1 as structure-pin context, not as the simplification scope (the active change-set diff remains the scope), with a one-line constraint: `session-settled:`-labeled KTDs are structure pins the simplification must preserve (deliberate duplication stays duplicated).

   Do not finalize a change description or start a new change in this step. `ce-simplify-code` leaves its edits in the working-copy revision; step 4 reviews that revision, and step 8's `ce-commit-push-pr` describes and ships whatever remains. Finalizing here could combine still-active `ce-work` edits under a misleading description.
...
**Shipping precondition (steps 5–9).** Run `jj git remote list` once before the shipping steps. If it lists **no remote** (for example, a sandbox or throwaway Jujutsu workspace with no Git remote), shipping is **local-only**: finalize every local change the steps below call for, but **skip every push, PR create/edit, and CI-watch action** — the pushes in steps 5 and 6, the push and PR creation in step 8, and step 9 in full. A missing remote is a terminal local-only state, not an error: never retry a push or hunt for a remote — finalize the local changes and proceed to step 10. Run steps 5–9 normally when a remote exists.

5. **Apply and persist review fixes** (REQUIRED after step 4, before residual handoff)

   Load `references/review-followup.md` and execute its apply step (mechanical apply + describe/push when changes exist). Do not proceed to the residual handoff, run browser tests, or output DONE while eligible review fixes remain in an undescribed working-copy revision.
...
6. **Autonomous residual handoff** (only when step 4 reported one or more actionable `downstream-resolver` findings not applied in step 5; skip when it reported `Actionable findings: none.`)
   ...
   Do not output DONE until the residuals are durable: tracker tickets filed, and — when a PR exists — the run-report comment posted. Never block DONE on tracker filing failures once the comment is posted. When no PR exists at all (no remote, per the shipping precondition), the run output is the record: state the residuals in the DONE report rather than writing a file nobody will read.

7. Invoke the `ce-test-browser` skill with `mode:pipeline`.

8. Ship: the goal is the remaining work described, bookmarked, pushed, and in an open PR whose URL you hold. If the project's active instructions name a process that owns that handoff (a named skill or command, a stacking tool, or documented steps — not change-description or PR-title conventions, which the default honors; a skill directory alone is not a directive), run it non-interactively with the same plan path and context below. It is done only when the bookmark is pushed and you hold the URL of an open PR containing it — one the process opened, or the existing PR from step 6; if it cannot run headlessly, is unavailable, or ends short of that state, stop as **blocked** naming the process — do not fall through to the default or to step 9. Otherwise: Invoke the `ce-commit-push-pr` skill with `mode:pipeline`, explicitly identifying the repository as Jujutsu-backed and requiring Jujutsu-native description, bookmark, and `jj git push` operations while preserving `gh` for GitHub. Thread the recorded plan path from step 1 into the invocation, along with any proceeded-and-flagged `settled_decision_conflicts` entries from step 2, so the PR body's settled-decisions provenance line and its proceed-under-flag clause can fire.

   Any process that composes, edits, validates, or recommends the Jujutsu change description must follow the project's active instructions and current `jj log` history. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Use a neutral description derived from the actual change; do not impose a fixed syntax, message, or template.

   This describes any remaining changes, advances or creates the appropriate bookmark, pushes it with `jj git push`, and opens a pull request — non-interactively, per the mode token. If it prints a `New concepts:` trailer after the PR URL, record the concept name(s) for step 10. Once the PR URL is known, back-fill it into any residual tickets filed in step 6 (the `filed` list) so each ticket links to the PR carrying the finding — best-effort, and never block DONE on a failed ticket update. If step 6 already opened a PR, detect it with `gh pr view --json number,url,state`, skip PR creation, but still describe, bookmark, and push any remaining work. **Per the shipping precondition, when no remote is configured, do NOT invoke `ce-commit-push-pr` or a project-defined shipping process that requires a push. Instead describe the working-copy revision with the dynamically composed description, start a new empty change with `jj new`, and skip bookmark push and PR creation entirely.**
