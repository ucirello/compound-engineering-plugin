...
**Pre-resolve the scratch directory.** Generate a `<run-id>` once (8 hex chars) and reuse it for the V15 cache and the Phase 2/4 checkpoints so they share one per-run directory. Scratch lives under `$(jj workspace root)/.tmp`, or cwd-relative `.tmp` when not in a JJ repository — never `.context/`. Run this to resolve the root, create the run directory, and capture its absolute path:

```bash
REPO_ROOT="$(jj workspace root 2>/dev/null)" || REPO_ROOT="";
if [ -n "$REPO_ROOT" ]; then SCRATCH_ROOT="$REPO_ROOT/.tmp"; else SCRATCH_ROOT="$(pwd)/.tmp"; fi;
