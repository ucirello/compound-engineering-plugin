...
1. **Resolve the target directory and extension.**
   ...
   - **Otherwise (no repo, or elsewhere with no `docs/ideation/`):** write into the run's workspace-local scratch area — the `<scratch-dir>` resolved in Phase 1 (`<jj-workspace-root>/.tmp/rocketclaw/ideate/<run-id>/`, with `$PWD/.tmp/rocketclaw/ideate/<run-id>/` fallback). Do **not** write directly into the user's current working directory, and do **not** create a `docs/ideation/` tree for a subject unrelated to the repo. Announce the absolute path and note that `.tmp` is scratch storage that the user must move elsewhere to keep permanently.
