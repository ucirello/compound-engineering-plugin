...
- No code-change/bookmark or isolated-workspace setup (Phase 1 Step 2).
...
4. **Save and report.** Write the deliverable to a durable, tracked location, defaulting to a sensible `docs/` subpath, and report its absolute path. Whether to describe/finalize the JJ change or leave it in the working-copy change is the user's call; offer, do not force.

## Stay scoped to non-code deliverables

The carve-out is for knowledge-work output. If producing the deliverable requires code, route that sub-step through the normal code path so Test Discovery, review, fileset isolation, description, and bookmark safeguards apply. The deliverable itself stays non-code.
