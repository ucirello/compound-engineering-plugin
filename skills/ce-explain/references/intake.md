...
|-------|---------|--------|
| `diff:<ref-or-range>` | `diff:<change-id>`, `diff:<bookmark-range>`, `diff:PR#42` | Forces diff mode on that change |
...
- An unrecognized `<word>:<word>` token appearing inside a topic is not a flag — it passes through verbatim as request text.
...
- **Diff** — the request names a resolvable JJ revision: a change ID, commit ID, bookmark, PR, "the last revision", "what you just did", "this change".
