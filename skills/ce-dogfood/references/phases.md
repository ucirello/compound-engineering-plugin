...
Parse the arguments you were invoked with: a PR number, a bookmark/revision, or blank (use `@`). Strip `--port PORT` if present.

1. **Identify the target — keep PR identity; do not switch the working-copy change yet.**
   - **PR number:** the target *is the PR* — carry the number through every later step (trunk check, isolation, revision selection). Read its head only for display (`GIT_DIR="$(cd "$(jj workspace root)" && jj git root)" gh pr view <number> --json headRefName,isCrossRepository`), but do **not** reduce it to a bare bookmark name: a fork PR's head can even be named `main`/`master`. Do not `jj edit` yet.
   - **Bookmark/revision:** the target is that revision. Resolve it with `jj log -r <revision>` and retain the user's expression.
   - **Blank:** the target is `@`.
2. **Refuse to run on the trunk — bookmark/blank targets only.** If a *bookmark-name or blank* target resolves to `trunk()` (the default-bookmark head, typically `main`/`master`, or an empty working-copy change whose parent is `trunk()`), stop — there is no diff to dogfood. A **PR is always diffable** (it has a base), so this check never applies to a PR target; never refuse a `ce-dogfood <number>` invocation just because the PR's head bookmark happens to be named `main`.
3. **Decide isolation by what you're testing; let `ce-worktree` own the workspace mechanics.** Do not re-derive workspace detection or creation here — `ce-worktree` handles existing-isolation detection, the harness-native tool, attaching to a revision, and the "already isolated" constraint, and reports its decision back. The only call this skill makes is *whether to ask for isolation at all*:
   - **Blank / current-change target:** do **not** isolate — dogfood in place. You are already on the change under test, and the fix changes belong on it. (If you happen to already be in a JJ workspace, that is fine — you are simply dogfooding here.)
   - **A PR or a different named revision:** this is an existing ref to test without disturbing your current workspace. Offer isolation (platform's blocking question tool). On **yes**, invoke `ce-worktree` to isolate **that target ref** — it attaches a workspace to the revision (or, if already isolated, selects it in place; or reports "already isolated at `<path>` — work there" when the ref is live elsewhere). Act on `ce-worktree`'s verdict; the primary workspace is never switched. On **no**, `jj edit` the target revision in place (`GIT_DIR="$(cd "$(jj workspace root)" && jj git root)" gh pr view <number> --json headRefOid` then `(cd "$(jj workspace root)" && jj edit <oid>)` for a PR, `(cd "$(jj workspace root)" && jj edit <bookmark>)` for a named bookmark); JJ retains the prior working-copy change instead of discarding it.
...
Resolve the workspace root, then derive the trunk revision once, pull the full diff against it, and read it. Do not hard-code `main` — `trunk()` is the default-bookmark head (`main`/`master`/`trunk` on `origin`/`upstream`).

Run every `jj` command with cwd set to that workspace root so name-only paths stay workspace-relative:

```bash
WORKSPACE_ROOT=$(jj workspace root);
(cd "$WORKSPACE_ROOT" && jj diff --name-only -r 'trunk()..@')
(cd "$WORKSPACE_ROOT" && jj diff -r 'trunk()..@')
```

After a declined-isolation `jj edit`, `@` is the target revision, so the same `trunk()..@` range is the feature diff. For a PR that has not been edited into `@`, substitute the PR head commit ID for `@`. Pair `gh` with `GIT_DIR="$(cd "$WORKSPACE_ROOT" && jj git root)"`.
...
2. **Drive it** with agent-browser — navigate, snapshot for interactive refs, click, fill, submit, follow the journey to its real end state:
   ...
   SCREENSHOT_DIR="$WORKSPACE_ROOT/.tmp/ce-dogfood/<run-id>";
   mkdir -p "$SCREENSHOT_DIR";
   agent-browser screenshot "$SCREENSHOT_DIR/<scenario>.png"
   agent-browser errors      # check console/page errors
   ```

   Write transient screenshots under `<workspace-root>/.tmp/ce-dogfood/<run-id>/` (`<workspace-root>` = `jj workspace root`; fall back to `./.tmp/ce-dogfood/<run-id>/` when not in a JJ repository), never the workspace root itself. Only copy a screenshot into the report's location if you intend to embed it in the final report.
