...
**Exception: tree-mutating reviewers.** A persona whose method mutates the tree (`testing`, when it runs mutation testing) operates only on a faithful snapshot of the reviewed tree, never the shared checkout. Mutation testing on the shared tree is forbidden: a concurrent reviewer can observe a transient write as if it were the diff. When the reviewed tree is the working-copy change `@`, dispatch that persona with `isolation: "workspace"` and pass it the reviewed commit id. A harness-supplied workspace is not automatically faithful: when the session itself runs inside a managed workspace, the harness may cut it from the primary checkout or default bookmark instead (observed live). So before mutating, the persona verifies its copy's `@` equals the reviewed commit and falls back to a scratch copy on any mismatch. When the reviewed tree includes uncommitted changes (`local-aligned` scope), or the host cannot isolate, have the persona create a scratch copy that preserves those changes and mutate only the copy. Read-only siblings stay on the shared checkout.
