...
1. Create `OUTPUT_DIR` under the workspace `.tmp` directory: `ROOT="$(jj workspace root 2>/dev/null || echo .)"; mkdir -p "$ROOT/.tmp" && OUTPUT_DIR="$(mktemp -d "$ROOT/.tmp/riffrec-quick-XXXXXX")"`. Set `INPUT_PATH` to the supplied capture, and use the invocation in `references/analyzer.md`. Capture the analyzer's printed output directory; later steps read from it.
...
- No commit of `raw/` or `frames/` — they live only under workspace `.tmp` and must not be tracked.
