...
| `{pr_metadata}` | Stage 1 output | PR title, body, and URL when reviewing a PR. Empty string when reviewing a bookmark or standalone change |
...
| `{workspace_root}` | Stage 4 output | `jj workspace root`, or the current directory when no JJ workspace exists |
