...
6. If this check edited an instruction file after Commit already ran, fold the edit into that change with `jj squash` / `jj describe` (same bookmark, not yet pushed) or add a small follow-up change, composing the description per `references/commit.md`, and `jj git push --bookmark <name>` if the bookmark was already pushed so an open PR includes it. If the user chose "don't commit", leave the edits undescribed alongside the rest.
