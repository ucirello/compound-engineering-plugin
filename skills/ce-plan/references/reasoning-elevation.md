...
2. **Caller carrier** — a structured `<per-skill-key>:<model-alias>` token that an automatic orchestrator may pass in the invocation (LFG passes `plan_model:<alias>` to ce-plan; the analogous `brainstorm_model:<alias>` to ce-brainstorm). Use it when live user intent does not decide the choice. Strip it from the request text and never reconstruct it from product prose. It is honored in pipeline / `disable-model-invocation` runs. The alias must match `^[A-Za-z0-9._-]{1,64}$`, except when the resolved harness is `opencode2`, in which case the model is `provider/modelname#variant` and must match `^[A-Za-z0-9][A-Za-z0-9._-]*/[A-Za-z0-9._-]+(?:#[A-Za-z0-9._-]+)?$`. A malformed carrier counts as absent, not guessed.
3. **Config** — otherwise use the per-skill keys: `plan_model` (and optional `plan_harness`) for ce-plan, `brainstorm_model` (and optional `brainstorm_harness`) for ce-brainstorm. Read them the **same way this skill's Phase 0.0 (output-mode resolution) resolves `plan_output` / `brainstorm_output`**: reuse the workspace root already resolved, else run `jj workspace root`, then apply the ordinary-key rule (`config.local.yaml` then `config.yaml`). Reuse the Phase 0.0 reads if still in hand. Ignore commented (`#`-prefixed) lines. A model alias selects it; missing / commented / invalid / no file selects none.

   `plan_harness` / `brainstorm_harness` of `opencode2` is a distinct harness from `opencode` and is not a fallback to it. When the harness is `opencode2`, the matching model key is `provider/modelname#variant` (slash and `#variant` are required shape, not a malformed alias). Dispatch is the `opencode2` binary: `opencode2 run --standalone --auto --model provider/model#variant` (no `--variant` flag; variant is `#variant` on `--model`).
...
2. **`opencode2` CLI (own branch; not `opencode`).** When the resolved harness is `opencode2`, run the bundled `scripts/elevation-dispatch.sh` worker as a detached job with `ELEVATION_HARNESS=opencode2` (see Off-host dispatch). Available when `opencode2` is on PATH. The worker execs `opencode2 run --standalone --auto --model provider/model#variant`. Do not fall through to an `opencode` binary or reuse an `opencode` argv. If `opencode2` is absent, this adapter is unmet and the next adapter runs.
...
Hand over the working context as **file paths the subagent reads itself**, never a re-narrated prose brief. Create **one private per-run handoff directory** under the workspace `.tmp` (`ROOT="$(jj workspace root 2>/dev/null || echo .)"; mkdir -p "$ROOT/.tmp" && mktemp -d "$ROOT/.tmp/ce-elevation-XXXXXX"`) and write the prompt-file and every evidence file into *that* directory. On the Claude CLI route the worker grants the elevated model read access to only that one directory (via `--add-dir` on the prompt-file's parent), so the handoff files stay readable while the rest of the workspace `.tmp` root — other same-user scratch and credentials — is not exposed:
...
When the resolved harness is `opencode2`, prefix that same `start` environment with `ELEVATION_HARNESS=opencode2` so the worker takes its own `opencode2` branch (`opencode2 run --standalone --auto --model …`). Do not set it to `opencode` and do not rewrite the worker onto an `opencode` argv.
