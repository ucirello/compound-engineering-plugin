...
- **Recover the author's intent before overriding deliberate-looking code.** Inspect nearby Jujutsu history with `jj log` and line provenance with `jj file annotate`, then read the PR description and surrounding code. The intent the author had is the thing an isolated reviewer lacked; weigh it against the finding rather than assuming the reviewer saw more.
