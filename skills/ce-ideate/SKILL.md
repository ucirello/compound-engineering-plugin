...
- **Repo mode surprise-me:** the codebase-scan sub-agent samples a few representative files per top-level area (not just reads the top-level layout + AGENTS.md), surfaces recent PR/change activity from `jj log` as signal about what's actively being worked on, and — when issue intelligence runs — passes issue themes as first-class input rather than footnote. Keep the scan bounded: representative, not exhaustive.
...
**Pre-resolve the scratch directory path.** Scratch lives under the current Jujutsu workspace's `.tmp/rocketclaw/ideate/` namespace. When no workspace root is available, use `$PWD/.tmp/rocketclaw/ideate/`. Run one bash command to create the directory and capture its absolute path for downstream use.
