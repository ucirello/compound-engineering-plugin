...
3. **Config** — otherwise use the per-skill keys: `plan_model` and `plan_harness` for ce-plan, `brainstorm_model` and `brainstorm_harness` for ce-brainstorm. Read them the **same way this skill's Phase 0.0 (output-mode resolution) resolves `plan_output` / `brainstorm_output`**: reuse the repo root already resolved, else run `jj workspace root`, then apply the ordinary-key rule (`config.local.yaml` then `config.yaml`). Reuse the Phase 0.0 reads if still in hand. Ignore commented (`#`-prefixed) lines. A model alias or `provider/modelname#variant` selects the model; `plan_harness: opencode2` / `brainstorm_harness: opencode2` selects the opencode2 binary route (not opencode). Missing / commented / invalid / no file selects none.
...
2. **opencode2 CLI** when `plan_harness` / `brainstorm_harness` is `opencode2`. Run the bundled `scripts/elevation-dispatch.sh` worker as a detached job with harness `opencode2` (see Off-host dispatch). This invokes the `opencode2` binary, never `opencode`. Available when `opencode2` is on PATH. Pass the model whole in `provider/modelname#variant` form to `opencode2 run --model`. Do not preflight authentication in the host command context: the detached worker's provider-capable call is authoritative, and an authentication failure there follows Recovery.
3. **Claude CLI.** Run the bundled `scripts/elevation-dispatch.sh` worker as a detached job (see Off-host dispatch). Available when `claude` is on PATH. Do not preflight authentication in the host command context: the detached worker's provider-capable call is authoritative, and an authentication failure there follows Recovery.
...
- On the **opencode2 CLI** route the worker invokes `opencode2` (not `opencode`) as `opencode2 run --standalone --auto --format json --file <prompt-file> --model <provider/modelname#variant>`. The `#variant` segment stays on `--model`; do not split it into a v1 `--variant` flag. `--auto` auto-approves permissions that are not explicitly denied. This CLI does not expose the Claude `--tools` denylist, so write/shell denial is an **instruction** in the prompt-file, not a hard guarantee.
...
Hand over the working context as **file paths the subagent reads itself**, never a re-narrated prose brief. Create **one private per-run handoff directory** under the workspace scratch root:

```bash
ROOT="$(jj workspace root 2>/dev/null || pwd)"
mkdir -p "$ROOT/.tmp"
mktemp -d "$ROOT/.tmp/ce-elevation-XXXXXX"
```

Write the prompt-file and every evidence file into *that* directory. On the Claude CLI route the worker grants the elevated model read access to only that one directory (via `--add-dir` on the prompt-file's parent), so the handoff files stay readable while the rest of the scratch root — other same-user scratch and credentials — is not exposed:
...
## Off-host dispatch (Claude CLI or opencode2 CLI route)
...
```
   ...
   CE_PEER_HARD_SECS=5400 CE_ELEVATION_HARD_SECS=5400 CE_PEER_LOG_MAX_BYTES=52428800 \
     ...
     -- bash "$SKILL_DIR/scripts/elevation-dispatch.sh" "<model>" "<prompt-file>" "<result-path>" "<harness>"
   ```

`<harness>` is `opencode2` when `brainstorm_harness` / `plan_harness` is `opencode2` (invokes the `opencode2` binary; `<model>` is `provider/modelname#variant` passed whole). Otherwise omit it or pass `claude` for the Claude CLI route. Never pass `opencode` as a stand-in for `opencode2`.
