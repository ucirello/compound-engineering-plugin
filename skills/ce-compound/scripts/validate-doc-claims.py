...
citations against the repository:
    ...
    1. Cited repo-relative paths (backticked, containing at least one '/')
       ...
       doc's directory (those escaping the repo are skipped). Misses tracked
         at the parent revision or `trunk()` still count as real paths and are
       ...
       identifiers (bookmark names, Jujutsu revisions, provider/model IDs) are skipped.
...
The script never touches the network; classification uses Jujutsu revisions
already present locally. Run a best-effort `jj git fetch --all-remotes` first when freshness
matters. Pure stdlib.
"""
import os
import re
...
def revset_has_match(revset: str, cwd: str) -> bool:
    code, output = jj(
        ["log", "-r", revset, "--no-graph", "-T", "commit_id"], cwd
    )
...
def is_path_candidate(token: str) -> bool:
    ...
    if token.startswith(("origin/", "upstream/", "refs/")):
        return False  # remote/revision identifiers, not repo paths
...
def is_path_shaped(token: str, base: str) -> bool:
    ...
    (bookmark name, provider/model ID) among tokens absent from all revisions."""
...
def main(argv: list[str]) -> int:
    ...
    if in_jj:
        if revset_has_match("trunk()", repo_root):
            upstream = "trunk()"
        if upstream:
            code, behind_ids = jj(
            ...
            behind = len(behind_ids.splitlines()) if code == 0 and behind_ids else 0
            if behind > 0:
                infos.append(
            ...
            infos.append(
                ...
                "path/commit-ID classification limited to the current revision"
            )
    else:
        infos.append(
    ...
    if in_jj:
        for m in COMMIT_ID_RE.finditer(body):
            ...
            in_up = (
                ...
                and jj(
                    ["log", "-r", f"{commit_id} & ::{upstream}", "--no-graph", "-T", "commit_id"],
                    repo_root,
                )[1] != ""
            )
            ...
            if in_current and not in_up:
                flags.append(
                    f"FLAG commit ID {commit_id}{loc} — reachable from @ but not {upstream}: "
                    "local-only commit whose ID may be rewritten by history edits "
                    "or squash merge. Prefer citing the PR number."
                )
            elif in_up:
                flags.append(
