...
if [ -f "bin/dev" ]; then
  PORT=${PORT} bin/dev > "$SCRATCH_ROOT/dev-server-${PORT}.log" 2>&1 &
elif [ -f "bin/rails" ]; then
  bin/rails server -p ${PORT} > "$SCRATCH_ROOT/dev-server-${PORT}.log" 2>&1 &
elif [ -f "package.json" ]; then
  PORT=${PORT} npm run dev > "$SCRATCH_ROOT/dev-server-${PORT}.log" 2>&1 &
fi
...
if ! lsof -i ":${PORT}" -sTCP:LISTEN -t >/dev/null 2>&1; then
  ...
  tail -20 "$SCRATCH_ROOT/dev-server-${PORT}.log" 2>/dev/null
  exit 1
fi
```
