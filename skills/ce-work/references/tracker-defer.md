...
- On total chain exhaustion (every tier failed or no sink available), return findings in the `no_sink` bucket so the caller can route them to another surface (e.g., inline them in a pull-request description).
...
2. **Probe the named tracker when one was found.** For GitHub Issues, run `gh auth status` and `gh repo view --json hasIssuesEnabled`. For Linear or other connector/MCP-backed trackers, discover available tools through the provider's tool-discovery primitive, such as Claude Code `ToolSearch`, rather than assuming absence from an unloaded tool, then verify the discovered tool is responsive. For API-backed trackers, verify credentials wherever the provider exposes them. Set `named_sink_available` from the probe result.
...
- **Body:**
  - Plain-English problem statement — reads the persona-produced `why_it_matters` from `<workspace-root>/.tmp/rocketclaw/ce-code-review/<run-id>/{reviewer}.json`, resolving `<workspace-root>` with `jj workspace root` and falling back to `pwd -P`. Match with `file + line_bucket(line, +/-3) + normalize(title)`. Fall back to the merged finding's `title`, `severity`, `file`, and `suggested_fix` when no artifact match is available.
...
- **Length cap:** when the composed body would exceed a tracker's body length limit, truncate with `... (continued in review run artifact: <workspace-root>/.tmp/rocketclaw/ce-code-review/<run-id>/)` and include the finding ID in both the truncated body and metadata block.
...
The question-tool mapping varies by provider: Claude Code `AskUserQuestion`, Codex `request_user_input`, Antigravity CLI `ask_question`, and Pi `ask_user` with `pi-ask-user`. Use tool discovery before treating a mapping as unavailable. Fall back to numbered options in chat only when the provider genuinely lacks a blocking tool or the tool call explicitly fails. A pending schema load is not a fallback trigger. Never silently skip the question.
