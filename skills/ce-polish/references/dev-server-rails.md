...
- **SSL dev server:** `rails s --ssl` serves over `https://`, but RocketClaw polish's reachability probe, browser handoff, and printed URL are all `http://localhost:<port>`; the scheme is not configurable via `.rocketclaw/launch.json`. The probe therefore fails against an HTTPS-only server. That failure is non-fatal: RocketClaw polish shows the workspace-local log and asks what to do, so open `https://localhost:<port>` manually to continue. Setting `port` explicitly still targets the right port.
