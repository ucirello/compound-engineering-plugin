...
- **Bookmark** — run `jj log -r @ --no-graph -T 'local_bookmarks.map(|b| b.name()).join("\n")'`. Use the bookmark name to filter session history in Phase 1. If it returns empty (no bookmark at `@`) or exits non-zero (not a jj repo), skip branch filtering.
...
#### 4. **Session History** (internal flow after launching the parallel block — automatic in Full mode, including non-interactive)
   ...
   **Extraction pipeline.** Create a per-run scratch directory under the workspace `.tmp` (cwd-relative `.tmp` if not in a jj repository). For each selected session, write extracted content to scratch files:

   ```bash
   REPO_ROOT=$(jj workspace root 2>/dev/null || pwd);
   SCRATCH="$REPO_ROOT/.tmp/ce-compound/sessions-$(date +%Y%m%d-%H%M%S)-$$";
