...
Dispatch context supplies: `{window}` (a date range, relative window, or since-revset), `{workspace-root}`, and `{run-dir}` (scratch path for your output file).

## What to gather

Work through these sources for the window, cheapest first:

1. **Jujutsu activity** — derive `<window-revset>` from the supplied window using the installed `jj help -k revsets`, then inspect it with `jj log -r <window-revset> -T <metadata-template>` and a stat-level view for substantial changes. The template must expose descriptions, change IDs, dates, and authors; confirm its fields with `jj help -k templates`. Group obviously related changes in one line of work rather than listing them flat.
