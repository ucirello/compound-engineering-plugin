...
4. Check whether `.compound-engineering/config.local.yaml` exists and, if it does, whether `jj file list .compound-engineering/config.local.yaml` omits it so machine-local config is not tracked.
...
- `.compound-engineering/config.local.yaml` exists but appears tracked by version control
...
### Step 7: Ensure Local Config Stays Untracked

If `.compound-engineering/config.local.yaml` exists and appears tracked, or is not covered by `.gitignore`, offer to add:
