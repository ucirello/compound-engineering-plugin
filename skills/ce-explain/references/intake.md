...
|-------|---------|--------|
| `diff:<ref-or-range>` | `diff:<change-id>`, `diff:trunk()..@`, `diff:PR#42` | Forces diff mode on that change |
...
- An unrecognized `<word>:<word>` token (including a repository-defined message prefix appearing inside a topic) is not a flag — it passes through verbatim as request text.
...
- **Diff** — the request names a resolvable JJ change ID, commit ID, bookmark, revset, GitHub PR, "the last change", "what you just did", or "this change".
