...
```bash
(cd "$(jj workspace root)" && jj bookmark list "exact:optimize/<spec-name>")
```
...
```bash
# Missing bookmark (fresh): create it at the working copy.
(cd "$(jj workspace root)" && jj bookmark create "optimize/<spec-name>")
# Existing bookmark (resume): continue from it. Do not also create.
(cd "$(jj workspace root)" && jj new "optimize/<spec-name>")
```
...
The SKILL.md body states this gate. Run `jj status` from the workspace root, filter the output against `scope.mutable` and `scope.immutable`, and apply the body's rule to the result. Name the dirty in-scope files, ask the user to `jj commit` them or move them aside with `jj new @-` (working copy remains a sibling), and do not continue until they are clean.
