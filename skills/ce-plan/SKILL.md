...
**Token-parsing convention:** only literal-prefix flag tokens (`output:`, `mode:`, the exact `confirm:auto`/`confirm:ask` forms, `delegate:` where applicable) are consumed and stripped. Other `<word>:<word>` tokens — including repository-specific change-description syntax and any unrecognized `confirm:<value>` (e.g., a `confirm: delete-account modal` feature description) — pass through verbatim.
...
- Draft a clear, searchable title in the repository's established plan-title syntax; do not add a fixed prefix or type.
...
- **Publish to Proof — shareable link** — Load the `ce-proof` skill to publish the plan: create a shared Proof doc from the plan file (title = plan title; identity `ai:assistant` / `AI Assistant`), surface the share URL to the user, then return to this menu. Do not add an attribution badge, byline, generated-by line, or standalone workflow branding. One-way publish — the local plan file stays canonical, nothing syncs back. If the upload fails, see the graceful-fallback note in `references/plan-handoff.md`.
