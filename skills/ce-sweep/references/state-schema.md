...
| field | meaning |
| --- | --- |
| `fix_ref` | Reference to the fix (PR/JJ change/Git commit/issue link). |
| `verified_commit_id` | Git commit ID, as imported by JJ, that landed on the default remote bookmark. |
...
| --- | --- | --- |
| local-change mode (default) | Single writer **per JJ workspace**. | The lease serializes overlapping sweeps in the same workspace (for example, a scheduled sweep and a manual one). The file may be tracked in a local JJ change. No cross-machine guarantee. |
| published shared bookmark | One writer **per repository**. | The state file lives in changes published through one shared bookmark. `lease-acquire` must be isolated in a JJ change, described, published with `jj git push`, fetched, and confirmed **before any source-side write**. This makes the lease a repository-wide mutex across machines. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Repository-local instructions and syntax observed in `git log` take precedence; use Go guidance only when compatible. No message form or example is prescribed here. |
...
and never tracked (the skill isolates only the state file and plan in the
described JJ change).
