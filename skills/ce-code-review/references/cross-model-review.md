...
**Workspace-configured model and effort.** After resolving the target, read `cross_model_model:` and `cross_model_effort:` from the same `.rocketclaw` files. Pass valid values through the existing override variables; the worker validates target family and route support and fails closed on incompatibility. Live user values outrank config.
...
The `ce-code-review` invocation authorizes the selected configured/allowlisted route after disclosure. Skip for an explicit prohibition, workspace `cross_model_review_mode: off` without live opt-in, or an observed route failure.
...
- **Interactive host, no peer resolved** (host family unattested, no different provider available, or disabled by workspace config): one quiet line that the pass was skipped and why.
...
- **Never started / not run** — note the unmet gate in Coverage, including disabled workspace config when applicable. Ignore raw intermediates; they are not fold-in artifacts.
