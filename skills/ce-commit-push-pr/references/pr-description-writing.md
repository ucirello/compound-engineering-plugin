...
**Fallback** — use `gh pr diff <ref>` and `gh pr view <ref> --json commits` when local JJ/Git interop can't reach the refs (fork PR with no matching remote, shallow clone, offline, unrelated histories). For GHES configurations that reject SHA fetch but allow `refs/pull/`:
