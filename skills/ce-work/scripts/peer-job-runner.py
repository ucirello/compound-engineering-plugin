...
the current JJ workspace's `.tmp`. Every read of job state opens the file first (no-follow) and
...
POSIX path is behaviorally unchanged:
  ...
  jobs root defaults under <workspace-root>\\.tmp\\rocketclaw-peer-jobs,
            owner-private and local to the current JJ workspace.
...
def _workspace_root() -> str:
    ...
    if not root:
        try:
            root = subprocess.run(
                ["pwd", "-P"], capture_output=True, text=True, check=False,
            ).stdout.strip()
        except OSError:
            root = ""
...
def _workspace_local_root(path: str, label: str) -> str:
    root = os.path.abspath(path)
    allowed = os.path.join(_workspace_root(), ".tmp")
    try:
        inside = os.path.commonpath([allowed, root]) == allowed
    except ValueError:
        inside = False
    if not inside:
        raise RunnerError(f"{label} must stay under the current workspace .tmp")
    return root
...
def jobs_root_base() -> str:
    ...
    if configured:
        return _workspace_local_root(configured, "CE_PEER_JOBS_ROOT")
    os.makedirs(os.path.dirname(DEFAULT_ROOT), mode=0o700, exist_ok=True)
...
def skill_runs_root(skill: str) -> str:
    if skill == "ce-work" and os.environ.get("CE_WORK_RUNS_ROOT"):
        return _workspace_local_root(os.environ["CE_WORK_RUNS_ROOT"], "CE_WORK_RUNS_ROOT")
...
def _require_detach_support() -> None:
    ...
    if not hasattr(os, "fork") or not hasattr(os, "setsid"):
        raise RunnerError(
            ...
            "the documented native-Windows process contract)."
        )
