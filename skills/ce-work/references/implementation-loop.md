...
while (tasks remain):
  ...
  - **If the unit's work is already present and matches the plan's intent**, inspect the Jujutsu graph and current files, verify it, mark the task complete, and move on. Do not silently reimplement.
  ...
  - Evaluate whether the current change is complete enough to describe and start the next change
```

For a parallel wave, the loop pauses at a host-owned integration stop after every canonical result. Inspect the actual result rather than its declared scope, re-run the independence judgment against the advancing change, and recompute readiness from integrated prerequisites. Affected dependents remain queued. An unaffected sibling may continue only after any failed integration or verification has been restored to the exact recorded Jujutsu operation and the prior lock released. Rebase or re-dispatch a stale result explicitly; never treat a conflict-free result as semantic proof.
