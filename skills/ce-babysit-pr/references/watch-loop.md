...
- **`pr-snapshot watch`** is that detector: it performs the same fetch-to-diff operation without agent reasoning, prints one `BABYSIT_WAKE {reason,url,...}` line only on an actionable change or stop condition, then exits.
- The agent backgrounds `watch`, waits with the harness's background-and-wake capability, runs a tick, and re-arms. The loop stays in the current session, preserving prior decisions and live steering.
...
| Harness | Background-and-wake capability used by the agent | Durable beyond the session? |
|---------|-----------------------------------------|-----------------------------|
...
- `--settle-seconds` (default 300) is the quiet window before a `merge-ready` wake, so the agent is roused only after the PR cools off. Leave it unset on the normal arm; only the post-rejection re-arm changes it.
...
A loop can churn without finishing: CI ping-pong, a review treadmill, or wrong-approach whack-a-mole. A raw attempt counter cannot distinguish these from legitimate progressive repair, so the decision is **agent judgment over the trajectory**.
...
State lives at `$(jj workspace root)/.tmp/rocketclaw/babysit-pr/<host>-<owner>-<repo>-<pr>/state.json`, or local `$PWD/.tmp/rocketclaw/babysit-pr/<host>-<owner>-<repo>-<pr>/state.json` when there is no JJ repository. This stable workspace-local path is shared by later ticks. The `<host>` segment is load-bearing for GitHub Enterprise: without it, two PRs sharing `owner/repo#N` on different hosts would cross-contaminate dispositions. The `pr-snapshot` script owns all reads and writes under a file lock. Shape:
...
The rule that makes ticks idempotent and crash-safe: **the snapshot never marks an item handled just from observing it.** An item leaves the actionable set only when the agent confirms action through `mark` or remote truth removes it. A failed pass therefore remains actionable next tick.
...
- **Lock.** The script takes a file lock around each state read/write. It cannot span agent mutations between script calls, so it is necessary but not sufficient.
...
- **Merge conflict mid-flight** (`mergeable == "CONFLICTING"`): create a JJ merge working-copy change from the head and base revisions and immediately save its exact full change ID from `jj log -r 'exactly(@, 1)' --no-graph -T 'change_id ++ "\n"'`. Resolve mechanical conflicts as first-class JJ conflicts. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Inspect the actual history with `GIT_DIR="$(jj git root)" git log -n 10 --format=full`; repository-local instructions and the syntax visible in that output win over incompatible Go guidance. Apply compatible Go guidance only for quality, clarity, and structure. Do not impose any fixed prefix, type, scope, subject, body, layout, template, or example. Apply a neutral description composed dynamically from those conventions, then validate it with `jj show -r @`. Move the retained explicit head bookmark to `@` with `jj bookmark set "$BOOKMARK" -r @` and publish only it with `jj git push --remote "$REMOTE" --bookmark "exact:$BOOKMARK"`. For a semantic conflict, explicitly move the workspace to a clean child of the head revision with `jj new <head-revision>`, abandon only the saved unpublished merge change with `jj abandon <saved-merge-change-id>`, verify that `@` is clean and based on the head, and surface `needs-human`. Never use operation-global `jj undo` for cleanup; never rewrite published history.
