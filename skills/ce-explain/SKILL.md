...
RUN_DIR="$REPO_ROOT/.tmp/rocketclaw/explain/$(date +%Y%m%d)-$(openssl rand -hex 3)"
mkdir -p "$RUN_DIR"
echo "$RUN_DIR"
```
...
- **Recap mode:** dispatch a generic subagent seeded with `references/agents/work-recap-scout.md` (extraction tier), passing the resolved window, the repo root, and `$RUN_DIR`. It returns an evidence summary with change and commit IDs plus `file:line` pointers. **Empty window** (no JJ activity, no doc changes): say so, offer to widen the window, write no artifact, and end the run after the user responds.
