...
4. **Default:** use the OpenCode V2 native route below when its capability condition holds. Otherwise choose the first available attested-different target in `codex -> claude -> grok -> composer`; Cursor-default participates only when explicitly preferred.
...
**Checkout-configured model and effort.** After the target is resolved, read `cross_model_model:` and `cross_model_effort:` from the same two repo RocketClaw config files under the ordinary-key rule. When `cross_model_model` is set, pass `CROSS_MODEL_MODEL_OVERRIDE_TARGET=<resolved-target>` and `CROSS_MODEL_MODEL_OVERRIDE=<value>`; when `cross_model_effort` is set, pass `CROSS_MODEL_EFFORT_OVERRIDE=<value>`. Both ride the `env` prefix of the start invocation below. The worker checks each value against the route it actually runs. A model must belong to the resolved target's own family: an alias such as `fable` or a full id such as `claude-opus-5-5` for `claude`; `gpt-*` for `codex`, optionally namespace-qualified such as `openai.gpt-6-sol` when that CLI routes through a non-default `model_provider`. OpenCode V2 takes a discovered `provider/model#variant` through `--model`; it has no `--variant` flag. An effort must be a level that CLI documents. cursor-agent routes accept no effort override. An incompatible value stops the pass with a named skip reason; the worker never substitutes. Unset keys leave the script's editorial mapping unchanged except OpenCode, which requires an exact discovered model rather than a harness default. Announce the configured model and effort in the Step 3 (Announce) line exactly as requested. A model or effort the user states in conversation outranks the config keys.

**OpenCode V2 native route.** Resolve transport before testing CLI availability. Explicit effective subagent/harness settings and alternative-recipient `cross_model_peer` routing win. With no explicit subagent or alternative-harness route, when the current host is OpenCode and `opencode.models` plus native `subagent(model)` are available, discover exact IDs and variants with `opencode.models`, then dispatch with `subagent(model: "<exact discovered reference>")`. CLI presence is not required. `cross_model_peer: opencode` permits this native transport; a configured peer model or effort constrains model selection rather than selecting shell transport. Preserve the peer's required reasoning tier and different-serving-family intent; never guess or silently reuse the host model.

Apply the same checkout policy, egress allowlist, disclosure, scope, adversarial brief, trusted constraints, and untrusted semantic map. Launch a read-only native peer before finalizing the local roster; its accepted native launch receipt replaces the detached job ID for exclusive-lens selection. Collect through native bounded completion, using the same overall peer budget, and release through the host's native lifecycle. Do not invoke the shell worker or runner lifecycle commands while this native-route condition holds. Missing model/tier or read-only enforcement means named degraded coverage and the in-process adversarial fallback. A started failure follows the same degraded-coverage and bounded recovery conditions, retaining native transport.

Normalize the native result into `adversarial-opencode.json` with the ordinary finding schema and receipt fields: `cross_model_route: opencode-native`, `cross_model_harness` and `cross_model_target: opencode`, `model_requested` including the exact variant, `effort_requested`, `model_actual` and `effort_actual` as `unverified` unless attested, `receipt_supported`, `serving_family`, and `independence_verified` only with evidence of different serving families. Downgrade `safe_auto` to `gated_auto`. Save the artifact and peer outcome for the finish leaves exactly as on the shell route. If the native capabilities are unavailable, use the resolved shell route; do not invent a native primitive.
...
For OpenCode's shell route, bind the discovered reasoning variant in the model reference or through a matching `cross_model_effort`. A missing variant cannot establish this pass's required reasoning tier and fails closed. The dispatched `--model` and the receipt's `model_requested` carry the same combined reference.
...
## Step 4 — Start the peer before local dispatch

`cross_model_peer: opencode` selects the OpenCode recipient, not shell transport. It remains eligible for the native route above when no explicit subagent or alternative-harness setting overrides that route. The native contract replaces the shell lifecycle in Steps 4–5, including runner cleanup; retain the same artifact, recovery, and coverage requirements.
...
- `<host-serving-family>` is `codex`, `claude`, `grok`, `composer`, or `unknown`; `<host-harness>` is `codex`, `claude`, `grok`, `cursor`, `opencode`, or `unknown`.
...
- `<base-ref>` = the Stage 1 `BASE` (the diff base the peer reviews via `jj diff --from <base-ref> --to @`, with cwd at the absolute workspace root).
