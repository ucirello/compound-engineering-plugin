...
- `<workspace-root>` = `jj workspace root`; outside a JJ repo, the helper uses local `.tmp` only as the fallback base and returns `NO-CACHE` because no revision key exists.
- `<root-sha>` = lexicographically-first commit selected by `roots(::@ ~ root())` — the repo identity (stable across workspaces and clones).
- `<revision-sha>` = the commit ID of `@` — the working-copy state.

Two workspaces at the same revision share the same key within their workspace-local cache. Lookup is JJ metadata only; on a hit, only this one file is read.
