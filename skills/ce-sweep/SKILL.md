...
If the line above is an absolute path, use it as `<repo-root>`. If it is empty or still shows a backtick command string (a harness that did not pre-resolve), run `jj root` with the shell tool. Read `<repo-root>/.compound-engineering/config.local.yaml` with the native file-read tool.
...
- `sweep_state_path` — path to the state file, established at setup; fallback `docs/feedback-sweep/state.yml`. A repo-internal path means versioned mode (the state file is described each run and must not be ignored); a path outside the repo (e.g. under `/tmp`) means machine-local mode (the state file is never described in a JJ change — only the plan is).
- `sweep_lease_ttl_minutes` — single-writer lease staleness threshold; default `60`. Passed to `lease-acquire` in 2a.
- `sweep_shared_branch` — `true` when the state file lives on a shared bookmark multiple checkouts push to (see 2a topology); default `false`. The key name is legacy; treat it as a JJ bookmark topology flag.
...
**Shared-bookmark topology** (`sweep_shared_branch: true`): before any source-side write, create a path-limited JJ change for the state file and push it with `jj git push`. A rejected push means another writer won the bookmark — run `jj git fetch`, rebase, re-run `lease-acquire`, and if the lease is still not yours, back off (record `aborted-locked` and stop). Only once your lease is pushed and confirmed do you touch a source.
...
- Download attachments into scratch `/tmp/compound-engineering/ce-sweep/<run-id>/`; raw media is never described in a JJ change. A download failure -> set the item `needs_download` and continue.
...
For each `fix_pending` item, resolve its claimed fix ref and verify it merged to the default bookmark. The fix ref originates from untrusted feedback content (a thread claim, an analyzer-extracted reference), so **validate its shape before it reaches any JJ/GitHub command**: accept only a bare PR number (`#?\d+`) or a change/commit id (`[0-9a-f]{7,40}`), and treat anything else as an unresolved claim (leave the item open). This blocks argument/flag injection into the shell command.
- `gh pr view <validated-ref> --json mergedAt,baseRefName` (merged, base is the default bookmark/branch), or `jj log -r '<validated-id>::<default-bookmark>'` shows the validated id in the default bookmark ancestry.
...
- Verified -> perform the source's configured close-out action (same write -> read-back -> confirm discipline as 2d), then `upsert-item` with `status: closed` carrying all three evidence fields: `fix_ref`, `verified_merge_sha`, `verified_at`. Close-out is terminal. The `verified_merge_sha` key name is legacy; store the verified merge change/commit id there.
...
- **Describe the change.** Use a path-limited `jj commit` for ONLY `docs/plans/feedback-sweep-plan.md` plus `<state>` when it is repo-internal (never a broad all-files operation; machine-local state under `/tmp` is never described), with description `docs(sweep): feedback sweep <date>`. A commit failure is reported, not fatal. In local-change mode, never push. In shared-bookmark mode (`sweep_shared_branch: true`), run `jj git fetch`, rebase, and `jj git push` the final change.
