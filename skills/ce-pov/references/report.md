...
- Write temporary output under `<workspace-root>/.tmp/rocketclaw/ce-pov/`, resolving the root with `jj workspace root` and falling back to the current local directory, or under `docs/` when the user wants it kept; announce the absolute path. Do **not** introduce a new mandated `docs/` location — that store is deferred.
