...
Environment overrides (defaults in parentheses):
  CE_PEER_JOBS_ROOT         base dir (<workspace-root>/.tmp/rocketclaw-jobs;
                             local .tmp outside JJ)
...
POSIX path is behaviorally unchanged:
  ...
  jobs root uses the same workspace-local .tmp location with owner-private ACLs.
...
def _workspace_root() -> str:
    ...
    try:
        proc = subprocess.run(["jj", "workspace", "root"], cwd=cwd,
...
DEFAULT_ROOT = os.path.join(_workspace_root(), ".tmp", "rocketclaw-jobs")
...
def _private_root_usable(path: str) -> bool:
    ...
    Creation is the probe; a denied or unowned root is not usable.
    """
    try:
        os.mkdir(path, 0o700)
...
def candidate_jobs_root_bases() -> list:
    """The configured root or the stable workspace-local root."""
...
def cmd_start(args, worker_argv) -> int:
    ...
    base = jobs_root_base()
    if not os.environ.get("CE_PEER_JOBS_ROOT"):
        ensure_owned_dirs(os.path.dirname(base), base)
