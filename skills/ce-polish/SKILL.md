...
Start the dev server, open the feature in a browser, and iterate. The user tries the feature and says what feels off; the AI Assistant applies the fixes.
...
This is the core loop. The user browses the feature and tells the AI Assistant what to improve. The AI Assistant fixes it. Repeat until the user is happy.
...
- At the single change-description site, follow the repository-local instructions first. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Apply compatible Go guidance: summarize the user-visible effect in imperative form, keep the first line concise, and add a body only when it explains motivation or non-obvious constraints. Describe each finalized change with `jj describe -r <revision> -m "<message derived from repository conventions and the actual diff>"`, then use `jj new` to start a fresh change.
