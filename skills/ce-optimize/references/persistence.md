...
The scratch space under `.context/` is listed in `.gitignore`: it survives a local resume but does not travel with the bookmark, so anything needed durably must be exported to a tracked path.
...
2. Scan experiment workspace directories under `.tmp/ce-optimize/workspaces/` for `result.yaml` markers not yet in the log
