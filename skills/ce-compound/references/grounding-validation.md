...
| commit ID reachable from `@` only | Local-only change; commit ID may change during history rewriting or merge | Replace with the PR number |
| commit ID reachable from upstream only | Checkout predates the merge | Keep, with a temporal qualifier; verify the landed claim via `gh` |
...
Grep, Glob, non-mutating JJ commands, and gh when available.
