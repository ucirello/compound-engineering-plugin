...
When the repo has no remote (`jj git remote` from the workspace root), the run finishes locally. That is a final state, not an error: never retry a push or hunt for a remote. Step 5 still makes every `jj commit` it calls for; only the `jj git push` calls and the records that would live on the PR are dropped. With no PR to hold the unapplied findings, step 6 files them as tracker tickets and the DONE report states the rest. Never write them to a finished change nobody will read.
...
Do not `jj commit` in this step. `ce-simplify-code` leaves its edits in the working copy (`@`); step 4's review scopes the working copy, and step 9's `ce-commit-push-pr` finishes whatever remains. Finishing `@` here would sweep remaining working-copy `ce-work` edits into a change whose description would misstate them as simplification-only, and could stall on a working copy that never goes clean.
...
4. If `jj status` shows working-copy changes, finish only the review-driven files (JJ has no index; never invent a staging step). Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Repo-local syntax from project instructions and `git log` ALWAYS wins when it differs from Go guidance. Apply compatible Go guidance to quality/clarity/structure without replacing local syntax. The description's job is to record that review findings were applied; do not use a fixed Conventional Commit template. From the workspace root (do not use `jj -R`): `jj commit -m "<message composed from the standards above>" -- <review-driven-paths>`. Then push before step 6 **when a remote is configured** (per LFG's shipping precondition) and, on the defect route, when everything the push would publish is work the user offered (`references/debug-return.md`). To push: from the workspace root, identify this work's bookmark (`jj bookmark list`). Prefer remote `origin` when present, otherwise run `jj git remote` and choose the first configured remote. Create the bookmark on `@` if it does not exist (`jj bookmark create <name>`), then run `jj git push --bookmark <name>`. If there is no remote at all, do not push; the local change suffices. If no eligible fixes were applied, say so explicitly and skip the `jj commit`.
...
Invoke `ce-compound` with `mode:non-interactive` when the run produced durable reasoning that the final code, tests, and plan or diagnosis do not carry, and losing it would plausibly cause recurrence or substantial rediscovery. The plan or `root_cause`, the review residuals, and the fixes applied in step 5 are the evidence for that judgment. `ce-compound` writes into the repository's tracked learnings store on the bookmark and asks nothing; `Documentation skipped` is a successful outcome, not a stop. Step 9 finishes and pushes (`jj git push --bookmark <name>`) whatever it wrote with the rest of the change, so the learning is in the PR at the moment it opens and CI runs against a bookmark tip that contains it. Do not run it after the babysit result: a push after "CI decided" leaves an unwatched bookmark tip.
