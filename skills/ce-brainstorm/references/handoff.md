...
independent code review and applied fixes, describe with `jj describe`, push with
`jj git push`, open a PR, and watch CI to green. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Repository-local instructions and `git log` syntax always win; apply compatible Go quality guidance, and derive any prefix, type, scope, subject, template, or example dynamically from those local standards rather than fixing one here. Do not also start a `/goal` or load `ce-work` directly — `lfg`
orchestrates them. Unlike a goal tool, `lfg` is host-agnostic: it works wherever
skills run (plus `jj git`/`gh` for the PR/CI tail, which it guards when absent).
