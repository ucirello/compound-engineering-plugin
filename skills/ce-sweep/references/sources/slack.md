...
You are seeded at dispatch with: the channel id, the cursor timestamp (Slack `ts`) to fetch after, the sweep's `source` config-entry id, the configured acknowledgment reaction emoji plus the platform user id for protocol identity `ai:assistant` (`AI Assistant`), and the configured close-out reaction (if the source defines one).
...
| `existing_ack` | Boolean, scoped to protocol identity `ai:assistant` (`AI Assistant`): true only when the configured ack reaction is present AND was placed by its configured platform user. Any other user reacting with the same emoji does NOT set this true. |
...
- Fill `existing_ack` / `existing_closeout` by reading reactions and checking the reactor identity against the configured platform user id for `ai:assistant` — never by inferring "this looks handled."
