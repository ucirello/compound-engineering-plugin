...
(`<workspace-root>/.tmp/rocketclaw/ce-brainstorm/<run-id>/grounding.md`, or the same path under the current directory outside Jujutsu) — it gives
...
independent code review and applied fixes, describe changes, push a Jujutsu bookmark, open a PR, and CI watch to
...
skills run (plus `jj git` for Git remote interoperability and `gh` for the GitHub PR/CI tail, which it guards when absent). In a non-colocated workspace, GitHub CLI may need `GIT_DIR` set from `jj git root`, as documented by Jujutsu.
