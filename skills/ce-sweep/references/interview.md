...
Loaded by `SKILL.md` when `ce-sweep` runs with `feedback_sources` unset after the ordinary-key cascade. Captures setup that will be merged into `<workspace-root>/.rocketclaw/config.local.yaml`. Subsequent runs re-read those keys through the ordinary-key cascade (local then `config.yaml`).
...
- **Committed to the workspace** (recommended when multiple agents or machines share a bookmark). Sets `sweep_state_path` to the committed default under the artifact root's `feedback-sweep/`; resolve `<root>` to its concrete value first (for example, the default `docs`), so the persisted value is `<resolved-root>/feedback-sweep/state.yml`, never the literal `<root>` placeholder.
- **Checkout-local in `.tmp`** (solo setup; keeps bookkeeping unpublished). Resolve the path immediately with this shell block:
...
Let the user override the path if they want a different location. If they pick checkout-local, note that another workspace will not see this state.
...
**Skip this section entirely if the user chose checkout-local state in section 4.**

**Ask:** "Will multiple jj workspaces publish sweep state through one shared bookmark? If yes, enter the bookmark name. Leave it unset for local-only commits."

- **Unset** -> omit `sweep_shared_bookmark`. The lease serializes overlapping sweeps within one workspace; nothing is pushed.
- **Bookmark name** -> `sweep_shared_bookmark: <name>`. Explain that the lease becomes publication-gated: before any source-side write, the sweep commits only the state, moves and pushes that bookmark, fetches, and confirms its writer won.
...
- If `.rocketclaw/config.local.yaml` is not already covered by the workspace's ignore rules, offer to add the entry before writing.
...
- `sweep_shared_bookmark` — from section 6 when set; only meaningful with committed state.
...
After the interview completes, merge these flat keys into `<workspace-root>/.rocketclaw/config.local.yaml`, preserving unrelated keys already present.
...
sweep_state_path: <resolved-root>/feedback-sweep/state.yml   # concrete path (<root> resolved before persisting); committed or workspace-local under .tmp/ce-sweep
...
sweep_shared_bookmark: feedback-sweep             # omit for local-only commits
~~~
