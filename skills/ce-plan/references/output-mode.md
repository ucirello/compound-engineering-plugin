...
5. **Pipeline override.** When invoked from LFG or any `disable-model-invocation` context, force `OUTPUT_FORMAT=md` regardless of steps 1-4. Pipeline mode forces markdown and skips interactive questions but does **not** disable model elevation — `plan_model` / `plan_harness` config (and a `plan_model:<alias-or-id>` caller carrier) is still honored (see the model-elevation sub-step below and `references/reasoning-elevation.md`).

**Token-parsing convention:** only literal-prefix flag tokens (`output:`, `mode:`, the exact `confirm:auto`/`confirm:ask` forms, `plan_model:<alias-or-id>`, `plan_harness:opencode2`, `delegate:` where applicable) are consumed and stripped. Other `<word>:<word>` tokens — including unrecognized `confirm:<value>` — pass through verbatim into the feature description. A stripped `plan_model:` / `plan_harness:` carrier is retained for the Phase 5.2 model-elevation step.
...
**Model-elevation visibility.** Treat a stripped `plan_model:<alias-or-id>` / `plan_harness:` carrier or a surfaced `plan_model` / `plan_harness` config value as a pending Phase 5.2 input, not a resolved choice. Phase 5.2 resolves the choice from the current conversation, carrier, and config immediately before authoring, so later user intent cannot be lost.
