...
PROBE_DIR="$SCRATCH_ROOT/rocketclaw/ce-brainstorm-visual/<run-id>"; (umask 077; mkdir -p "$PROBE_DIR") || exit 1; chmod 700 "$PROBE_DIR" || exit 1;
...
```text
<scratch-root>/rocketclaw/ce-brainstorm-visual/<run-id>/
  screens/
    001-<decision>.html
  state/
    display-info.json
...
Do not copy probes into `.context`; keep disposable sketches under workspace `.tmp`. The durable artifact is the Phase 3 requirements-only unified plan under `<root>/plans/`.
