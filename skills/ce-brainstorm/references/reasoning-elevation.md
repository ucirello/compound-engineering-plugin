...
3. **Config** — otherwise the per-skill key: `plan_model` for ce-plan, `brainstorm_model` for ce-brainstorm. Read it the **same way this skill's Phase 0.0 resolves `plan_output` / `brainstorm_output`**: reuse the workspace root already resolved, else run `jj workspace root`, then read `<repo-root>/.rocketclaw/config.local.yaml` with the native file-read tool, reusing the Phase 0.0 read if still in hand. Ignore commented (`#`-prefixed) lines. A model alias → elevate to it; missing / commented / invalid / no file → off.
