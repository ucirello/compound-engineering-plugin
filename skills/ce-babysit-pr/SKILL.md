...
4. **Working copy must carry the PR's head bookmark with matching upstream** before any delegated mutation; default `GIT_DIR=$(jj git root) gh pr checkout <ref>`; no push access or dirty working copy → stop, say so.
