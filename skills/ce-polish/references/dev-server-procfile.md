...
- **Socket files:** `overmind` writes a socket to `.overmind.sock` by default. Polish's kill-by-port logic reclaims the port but does not clean up the socket. If overmind is already running and polish restarts it, the new process may fail with "connection refused" until the stale socket is removed. If redirecting `OVERMIND_SOCKET`, create and use a per-run path under `$(jj workspace root)/.tmp`, falling back to local `.tmp` when no JJ workspace exists.
