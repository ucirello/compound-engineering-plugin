...
> Diff-scoped browser QA of `<target>` vs the trunk/default bookmark. Generated by `/ce-dogfood` on <YYYY-MM-DD>.
...
<What changed between the target change/bookmark and the default bookmark: new features, modified behavior, new/changed routes, views, components, data flows. 2-6 bullets.>
...
<Overall readiness verdict for the target bookmark/change. Ready to ship? Caveats? Outstanding blocked items? Record the result of the Phase 5 automated test suite run — a green matrix with a red suite is not "ready.">
