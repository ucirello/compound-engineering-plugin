...
argument-hint: "[blank to simplify current JJ workspace changes, or describe what to simplify]"
---
...
2. **Otherwise, in a JJ workspace**, default to the diff between the current workspace change and its base bookmark (e.g., `jj diff --from main` or against the configured upstream bookmark). This covers the common case of "simplify everything I've added in this feature workspace before opening a PR." If no base bookmark or upstream ref is available, fall back to the current change diff (`jj diff`).
