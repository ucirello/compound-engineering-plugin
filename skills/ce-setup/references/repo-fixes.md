...
2. If `jj workspace root` succeeds, use that as the repo root.
3. Check for obsolete `rocketclaw.local.md` at the repo root.
4. Check whether `.rocketclaw/config.yaml` exists.
5. Check whether `.rocketclaw/config.local.yaml` exists and, if it does, whether the repo-root `.gitignore` contains a rule covering `.rocketclaw/config.local.yaml` (read `.gitignore` only; do not call git or read exclude files).
...
Skills that keep local scratch write it under `.context/`. Probe coverage by reading the repo-root `.gitignore` for a rule covering `.context/` — the exact `.context/` entry or a broader parent rule, with the trailing slash so an existing directory-only rule counts before the directory exists, and anchored to the repo root, since that is where the entry is appended. Do not call git or read exclude files. When it is not covered, offer to add:
