...
- `<head-id>` = `jj log -G -r @ -T 'change_id ++ " " ++ commit_id ++ " " ++ parents.map(|p| p.commit_id()).join(" ") ++ "\n"'` — the current JJ change identity and parent commit state.

Two workspaces at the same JJ head identity share the same entry. Lookup is JJ metadata only; on a hit, only this one file is read.
