...
2. Scope is `local-aligned` or standalone — the working copy IS the reviewed head. Skip in `pr-remote` / `branch-remote`: the peer reviews the local workspace, which is not the PR/branch head.
...
- `<base-ref>` = the Stage 1 `BASE` (the JJ revision the peer reviews via `jj diff --from <base-ref> --to @`).
- `<run-dir>` = the Stage 4 workspace-local run dir (`<workspace>/.tmp/rocketclaw/ce-code-review/<run-id>/`, with `<cwd>/.tmp/rocketclaw/...` fallback outside JJ). The script writes `adversarial-<peer>.json` and its short-lived intermediates there.
...
- Self-locates the persona + schema via `BASH_SOURCE` (works from any CWD); derives the workspace root from `jj workspace root`.
- Composes the peer prompt from the canonical persona brief + a JSON-only contract. Codex obtains its own diff with read-only `jj` inside its sandbox; Claude (which has no sandbox) is hard-denied `Bash`, so it gets the diff embedded and needs no shell. After capture, the script forces `reviewer = adversarial-<peer>` (the persona's example name `adversarial` would otherwise collide with the in-process reviewer and erase the cross-model agreement signal).
