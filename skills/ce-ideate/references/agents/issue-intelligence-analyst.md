...
1. **JJ workspace** — confirm the current directory is a JJ workspace using `jj workspace root`
2. **GitHub remote** — resolve the intended issue repository from explicit user/config context or GitHub metadata, normalize its URL, and require exactly one match from `jj git remote list`. Pass that URL to `gh repo view <url> --json nameWithOwner`; stop when repository identity is ambiguous instead of selecting by remote name.
