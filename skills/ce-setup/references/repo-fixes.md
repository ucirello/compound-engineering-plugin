...
2. If inside a Jujutsu workspace, resolve the repo root with `jj workspace root`.
3. Check for obsolete `rocketclaw.local.md` at the repo root.
4. Check whether `.rocketclaw/config.yaml` exists.
5. Check whether `.rocketclaw/config.local.yaml` exists and, if it does, whether `(cd "<repo-root>" && jj file list -- .rocketclaw/config.local.yaml)` prints that path. A printed path means the file is in the working copy and is not ignored.
...
Resolve the repository root (`jj workspace root`). All paths below are relative to the repo root, not the current working directory. Run every `jj` command with cwd at that absolute root; do not use `jj -R`.
...
Copy `references/config-template.yaml` to `<repo-root>/.rocketclaw/config.example.yaml`, creating the directory if needed. This file is tracked in the repo and should always reflect the latest available settings.
