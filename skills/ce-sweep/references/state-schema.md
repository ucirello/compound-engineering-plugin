...
| `verified_merge_sha` | The merge change SHA the fix landed on. |
...
| --- | --- | --- |
| local-change mode (default) | Single writer **per checkout**. | The lease serializes overlapping sweeps in the same working tree (e.g. a cron sweep and a manual one). The file is written in-tree (and may be described locally). No cross-machine guarantee. |
...
and never described (the skill's change step adds only the state file and the
plan, never `-A`).
