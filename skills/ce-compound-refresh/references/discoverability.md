...
6. If this check edited an instruction file after Commit already ran, fold the late edit into the current change (same bookmark, not yet pushed) or add a small follow-up change. Compose that follow-up description from the standards in `references/commit.md` (`jj describe -m "<message composed from the standards above>"`). Push with `jj git push` if the bookmark was already pushed so an open PR includes it. Pair `gh` with `GIT_DIR=$(jj git root)` when talking to GitHub. If the user chose "don't describe", leave the edits in the working copy alongside the rest.
