...
Hand over the working context as **file paths the subagent reads itself**, never a re-narrated prose brief. Resolve the workspace with `jj workspace root`, falling back to the current directory when the command fails, create one private per-run handoff directory under `<workspace-root>/.tmp/ce-elevation/<run-id>/`, and write the prompt-file and every evidence file into *that* directory. On the Claude CLI route the worker grants the elevated model read access to only that one directory (via `--add-dir` on the prompt-file's parent), so the handoff files stay readable while unrelated workspace-local scratch is not exposed:
