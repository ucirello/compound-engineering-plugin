...
```bash
ROOT=$(jj workspace root 2>/dev/null) || { echo '__NO_CONFIG__'; exit 0; }
cat "$ROOT/.rocketclaw/config.local.yaml" 2>/dev/null || echo '__NO_CONFIG__'
```
...
- **File absent:** create it (and its `.rocketclaw/` directory) with the key. Do not edit `.gitignore` or any local exclude file; the orchestrator owns ignore configuration for machine-local state.
