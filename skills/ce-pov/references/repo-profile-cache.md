...
- `<root-id>` = lexicographically-first commit ID selected by `jj log --no-graph -r 'children(root()) & ::@' -T 'commit_id ++ "\n"'` — the repository identity.
- `<revision-id>` = the current working-copy commit ID from `jj log --no-graph -r @ -T commit_id` — the working state.

Two JJ workspaces at the same revision in the same repository use content-equivalent entries under their respective workspace-local cache roots. Lookup is JJ metadata only; on a hit, only this one file is read. If `jj workspace root` is unavailable or its cache directory cannot be created, path resolution falls back to `.tmp/rocketclaw/repo-profile/` in the current directory, but key resolution returns `NO-CACHE` outside a JJ repository.
