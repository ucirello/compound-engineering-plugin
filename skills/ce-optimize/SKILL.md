...
The files under `.context/optimize/<spec-name>/` are local scratch state. They are ignored through `.gitignore`, so they survive local resumes on the same machine but are not preserved in Jujutsu changes or remote pushes unless the user exports them separately.
...
Verify the new orchestration change is empty and the recorded parent is the fixed baseline for this run:

```bash
jj diff --summary -r @
```

If the orchestration change is not empty, report the modified files and move those edits into an intentional baseline or separate change before proceeding. Do NOT continue until the orchestration change is empty; experiment workspaces must start from `best.change_id`, initially `baseline_change_id`.
...
- **Baseline snapshot status**: orchestration change confirmed empty and baseline change ID recorded
...
3. **If best improves on current best: KEEP**
   ...
   - The project's active instructions and the description syntax observed at runtime in `jj log` win. Apply compatible Go guidance only to quality, clarity, and structure; it does not prescribe imperative mood, casing, punctuation, line wrapping, subject/body shape, or any fixed syntax. Use that composition as the Jujutsu change description with `jj describe -r <change-id> -m <description>`.
...
4. **Check file-disjoint runners-up** (up to `max_runner_up_merges_per_batch`):
   ...
   - If disjoint: rebase the runner-up change onto the new baseline and re-run full measurement in its workspace
   - If combined measurement is strictly better, retain it as the next current-best parent.
   - Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards.
   - For that retained runner-up, the project's active instructions and the description syntax observed at runtime in `jj log` win. Apply compatible Go guidance only to quality, clarity, and structure; it does not prescribe imperative mood, casing, punctuation, line wrapping, subject/body shape, or any fixed syntax. Use the composition with `jj describe` and retain it as the next current-best parent using the same new/record sequence (outcome: `runner_up_kept`) before cleaning up that experiment workspace with `keep`
...
6. **Revert all others**: abandon their experiment changes, clean up their workspaces, and log as `reverted`
...
1. **Run `ce-code-review`** on the cumulative diff (baseline to final). Load the `ce-code-review` skill on the optimization stack (interactive or `mode:agent`). To land eligible fixes before the next option, apply the mechanical-apply bar below.
...
The project's active instructions and the description syntax observed at runtime in `jj log` win. Apply compatible Go guidance only to quality, clarity, and structure; it does not prescribe imperative mood, casing, punctuation, line wrapping, subject/body shape, or any fixed syntax. Use the composition with `jj describe`, then set or advance the provider-facing bookmark to the stack head before using the provider workflow.
