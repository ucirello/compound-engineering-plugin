...
When the cross-model judgment pass ran (see `references/cross-model-review.md`), each return enters synthesis as reviewer `<reviewer-name>-<provider>`. A return promotes a merged finding by one anchor step only when its receipt verifies independence and at least one in-process reviewer independently raised the same issue. Peer-only agreement never promotes, the bonus never stacks, and a peer-only finding never grants unattended-apply authority. User-facing output identifies the cross-model review by resolved provider and route without model branding or generated-by attribution; the stored reviewer field retains internal provenance.
