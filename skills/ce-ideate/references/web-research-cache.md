...
Files live under `<scratch-dir>/web-research-cache.json`, where `<scratch-dir>` is `<workspace-root>/.tmp/ce-ideate/<run-id>`, resolved once in SKILL.md Phase 1.
...
SCRATCH_ROOT="$WORKSPACE_ROOT/.tmp/ce-ideate"
...
- **Repo mode:** the focus hint plus a stable repo discriminator. This keeps the cache key meaningful when focus is empty — two bare-prompt invocations in the same repo legitimately share research, but the key still differentiates repos. Resolve the discriminator with this fallback chain and hash the result (first 8 hex chars of sha256 is sufficient):
    1. `jj git remote list` origin URL — stable across machines, correct for collaborators on the same remote.
    2. `jj workspace root` — absolute workspace path; machine-local but always available in a JJ repo.
