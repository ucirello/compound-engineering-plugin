...
- **Merged/active PR** — `gh pr view --json title,body,url 2>/dev/null` (and `gh pr view` for the current checkout). The title and body usually state the user-facing value.
...
- **Recent commits** — `jj log -r 'ancestors(@, 15)' --no-graph -T 'commit_id.short() ++ " " ++ description.first_line() ++ "\n"'` for the arc of the change.
