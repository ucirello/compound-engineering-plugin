...
1. Run the analyzer under workspace-local `.tmp/rocketclaw/` storage so nothing pollutes durable workspace content (`SKILL_DIR` is the directory containing the `ce-riffrec-feedback-analysis` SKILL.md; set it in the same command — shell state does not persist between Bash calls):
   ...
   OUTPUT_DIR="$WORKSPACE_ROOT/.tmp/rocketclaw/riffrec-quick-$(date +%s)-$$"
   mkdir -p "$OUTPUT_DIR"
   python "$SKILL_DIR/scripts/analyze_riffrec_zip.py" /path/to/input --output-dir "$OUTPUT_DIR"
   ```

   Capture the printed output directory; later steps read from it. Outside a JJ workspace, the command falls back to a unique local `.tmp/rocketclaw/riffrec-quick-*` directory under the current directory.
...
- Do not retain `raw/` or `frames/` in durable JJ revisions; they live only under workspace-local `.tmp/rocketclaw/` storage.
