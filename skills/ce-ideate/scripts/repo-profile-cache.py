...
Validity (HIT) requires ALL of:
  ...
  - no profile-input path differs from the current change's parent per
...
# an omitted manifest means a dependency bump in the current change can serve a stale
...
def changed_paths() -> "list[str] | None":
    ...
    Includes newly tracked entries so a newly-added profile input is seen.
...
def cache_path(workspace: str, root: str, current: str) -> str:
    return os.path.join(
        workspace, ".tmp", "rocketclaw", "repo-profile", root, f"{current}.json"
    )


def resolve_keys() -> "tuple[str, str, str] | None":
    """The workspace and cache key, or None if not a usable JJ repo."""
    workspace = jj("workspace", "root")
    root = root_commit_id()
    current = jj("log", "-r", "@", "--no-graph", "-T", "commit_id")
    if not workspace or not root or not current:
        return None
    return os.path.abspath(workspace), root, current
...
def do_put(profile_file: str) -> int:
    ...
    # Do not cache a profile derived from a current change that modifies profile
    # inputs. The next stable change should derive its own keyed profile.
    changed = changed_paths()
    if changed is None or any(is_profile_input(p) for p in changed):
        sys.stderr.write(
    ...
    try:
        ...
        # Atomic write: a unique file inside the workspace-local cache directory
