...
description: Run browser tests for pages affected by the current JJ change, bookmark, or PR. Use when asked to run or check browser tests for the current change.
argument-hint: "[PR number, bookmark name, 'current', or --port PORT]"
---

# Browser Test Skill

Run end-to-end browser tests on pages affected by a PR or JJ change using the best approved browser driver available in the active harness.
...
1. **Select the driver** per the policy above and record it. This also requires a JJ workspace with changes to test. Resolve its absolute root with `jj workspace root`, then set that root as the working directory for every JJ subprocess and scope command.
2. **Determine test scope** from the argument: a PR number → resolve `GIT_DIR` with `jj git root` successfully before running `GIT_DIR="$GIT_DIR" gh pr view [number] --json files -q '.files[].path'`; `current` or empty → `jj diff --name-only --from 'fork_point(main | @)' --to @`; a bookmark name → `jj diff --name-only --from 'fork_point(main | <bookmark>)' --to '<bookmark>'`. Use the project's actual base bookmark in place of `main`. An empty `@` still includes the work at `@-` in this base-to-current comparison; when identifying its PR or bookmark, inspect `@-` as well. If scope or Git-backend resolution fails, report the blocker rather than testing a guessed scope or letting `gh` select another repository.
