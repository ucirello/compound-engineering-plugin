...
1. **Resolve the target directory and extension.**
   ...
   - **Otherwise (no repo, or elsewhere with no `<root>/ideation/`):** write into the run's `.tmp` scratch area — the `<scratch-dir>` resolved in Phase 1 (`<scratch-root>/ce-ideate/<run-id>/`). Do **not** write into the user's current working directory, and do **not** create a `<root>/ideation/` tree for a subject unrelated to the repo. Announce the absolute path and note it lives under `.tmp` (not the durable ideation tree — move it to keep it with the rest of the project).
...
- **Inside a JJ repository:** offer to record only the ideation doc as a change (do not create a bookmark, do not push; if the user declines, leave the working copy undescribed). From the workspace root (`jj workspace root`), run `jj commit -m "<message composed from the standards above>"` with the ideation doc path as the fileset so only that file is in the change.
...
- the deliverable was written automatically in both modes (Phase 4) — to `<root>/ideation/` when present, else the `.tmp` scratch area, never the user's CWD
