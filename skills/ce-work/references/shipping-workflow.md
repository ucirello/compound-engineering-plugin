...
2. **Describe Changes and Create Pull Request**
   ...
   Resolve `heads(trunk()..@ & ~empty())` and require exactly one publishable tip. Select `origin` when present or the sole entry from `jj git remote list`; stop if multiple remotes remain ambiguous. Fetch that remote, advance the bookmark to the publishable tip, and push only that bookmark with `jj git push --remote "$REMOTE" --bookmark "exact:$BOOKMARK"`. Resolve `REPOSITORY` from that remote. Before writing the PR body, resolve the workspace root with `jj workspace root`, falling back to the current directory when JJ is unavailable; preserve existing `.gitignore` entries while ensuring `.tmp/` is ignored, refuse `.tmp` when it is a symlink or non-directory, and create `<resolved-root>/.tmp/`. Create or update the PR with explicit bookmark and repository context: `gh pr view "$BOOKMARK" --repo "$REPOSITORY" --json number,url,state`; write the body to `<resolved-root>/.tmp/pr-body.md`, then use `gh pr create` or `gh pr edit` with explicit repository, bookmark, base, title, and body-file values. Never rely on implicit repository or bookmark discovery.
