...
**Read config.** Resolve `<workspace-root>` at runtime by running `jj root` with the shell tool. Then read `<workspace-root>/.rocketclaw/config.local.yaml` with the native file-read tool. If the root cannot be resolved (not a Jujutsu workspace) or the file does not exist, fall through to the defaults below.
...
*Topic Scan (grounding scout)* — Create and retain the absolute scratch directory under the workspace root, falling back to the current directory when `jj root` cannot resolve a workspace. Substitute a short unique run slug:
