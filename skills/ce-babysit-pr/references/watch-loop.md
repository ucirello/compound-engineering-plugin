...
State lives at `$(jj workspace root)/.tmp/ce-babysit-pr/<host>-<owner>-<repo>-<pr>/state.json`, with local `.tmp` as the fallback when no workspace root resolves. This stable workspace-local path lets later ticks find the state without a global temp service. The `<host>` segment (from the PR URL, `github.com` on the public host) is load-bearing for GitHub Enterprise: without it, two PRs sharing `owner/repo#N` on different hosts would reuse one `state.json` and cross-contaminate dispositions. The `pr-snapshot` script owns all reads and writes under a file lock. Shape:
...
- **Branch currency.** `branch_currency` is the third attention stream. Its identity binds host-qualified live base repository/ref/OID, head SHA, merge status, and route. "Mergeable against the current base" and "head contains the latest base" are separate: only `BEHIND`, `DIRTY`, a branch-protection requirement, or an explicitly selected always-current policy requires maintenance. Ordinary historical base movement with GitHub reporting `CLEAN` does not. `UNKNOWN` mergeability or any non-null `base_ref_blocker` emits no item. Managed stacks and manager/relationship `probe-error` are excluded; a `normal-base` target may be independent or an eligible target-local manual dependency, and a root with open child dependents remains allowed. Never mutate those dependents.
  ...
  A current open item with carried `parked_semantic_fingerprints` must be previewed first. Compute the current fingerprint from sorted conflicted paths plus stable hashes of their materialized Jujutsu conflict terms, excluding the base OID, and mark `--currency-inspected-fingerprint <fingerprint>`. Unchanged evidence remains parked; changed evidence retires the old park and reopens attention. Before either a host update or Jujutsu merge change, atomically mark the exact item with `--currency-key <currency_key> --currency-disposition claimed`, plus `--invocation-id "$RUN_INVOCATION_ID" --session-started-at "$RUN_STARTED_AT" --invocation-budget-seconds "$RUN_BUDGET_SECONDS"`. Stale invocation fencing and `max-runtime` take precedence over a new claim or mutation.
...
- **Interrupted Jujutsu merge change.** On re-entry, inspect the operation log, working-copy change, conflicts, bookmark, and remote bookmark before acting. Recover only the exact operation ID recorded for the owned merge. Reconcile the existing merge change to the already-validated change; if reversal is required, use `jj undo` only when that operation is proven latest and no concurrent operation needs preservation, otherwise use `jj op revert <owned-operation-id>` or stop with that operation ID in the recoverable residual. Never layer another merge over unknown local state.

## Managed-stack continuation

Sequential babysitting is available only while a fresh probe reports `manager_status == "confirmed"` and posture is stack-wide. It uses one active target and one watcher. On transition, stop the watcher, re-read `gh stack view --json`, require the next PR to be the immediate in-scope non-draft entry, and prepare a clean empty Jujutsu working-copy change on that PR's tracked remote bookmark. Initialize its state with `--continue-invocation`, the same invocation ID/start/budget, `--continue-dead-time-seconds <prior layer's invocation_dead_time_seconds>`, and the same posture. One fixed budget covers the traversal. Quiescence, downstack return, and loss-of-confirmation behavior remain as defined in `references/stack.md`.
...
- **Managed stack:** `target_needs_rebase` true/unknown becomes `stack-blocked`; do not use `gh pr update-branch` or create a target/base merge. Baseline local and remote bookmark targets before delegation. After a target push, re-confirm provider ownership, fetch the target and dependents with `jj git fetch`, and verify the target's local and remote bookmarks equal the pushed commit ID. Rebase the dependent layer with `jj rebase --branch <first-dependent-bookmark> --onto <target-bookmark>`, record that owned operation's exact ID, then push each rewritten bookmark explicitly with `jj git push`. This moves the layer ancestors not already in the destination plus its descendants. If the rewritten range is conflicted, reverse only that operation under the `references/stack.md` concurrency rule or stop with its ID in a recoverable `needs-human` residual. On movement, bookmark conflict, or lease rejection, leave a residual and never overwrite remote work. Re-probe after each push and report partial progress precisely.
...
- **Normal-base `BEHIND`:** require positive host update capability; never infer it from Jujutsu push authority. Claim and revalidate, then call GitHub's update-branch endpoint once with `expected_head_sha`. Treat HTTP 422 as a stale claim. Confirm only from a fresh snapshot plus ancestry evidence.
- **Normal-base `DIRTY`:** separately prove authority to push the exact Jujutsu head bookmark. Require a clean empty working-copy change at the observed head, fetch the exact base through its remote bookmark, and preview the merge as a non-integrated Jujutsu operation. Fingerprint sorted conflicted paths and stable hashes of their materialized Jujutsu conflict terms, excluding the base OID. Record carried-fingerprint inspection before any claim.

  A resolution is mechanical only when positive intent evidence leaves no reasonable alternative behavior. Otherwise discard the preview and create the shared typed residual. For a mechanical case, claim and revalidate, create the exact two-parent Jujutsu merge change, record that owned operation's exact ID, mark mutation observed, resolve only the previewed conflict, and validate proportionally. Describe the change, move only the PR bookmark, and push only that bookmark through Jujutsu's safety checks. Confirm only from the validated merge commit and a fresh cleared snapshot. Recover an interrupted merge only under the exact-operation concurrency rule above before another attempt.
...
- **External head change or rewritten remote bookmark:** the head commit ID moved under the loop. The snapshot clears commit-scoped CI state automatically; fetch and re-snapshot. Never clobber unrelated remote work.
...
`references/watch-loop.md` covers these in full. The non-negotiable conditions are exact claimed currency before base movement; positive host capability for `BEHIND`; clean, exact-bookmark Jujutsu repair for `DIRTY`; safe parking for semantic, stale, ambiguous, unauthorized, conflicted, or lease-rejected outcomes; confirmed managed upstack maintenance after an owned push; fresh reconciliation after external head movement; and continued independent work around `needs-human` residuals.
