...
Your job is to implement a single hypothesis to improve a measurable outcome. You will modify code within a defined scope, then stop. You do NOT run the measurement harness, describe or commit a change, or evaluate results -- the orchestrator handles all of that.
...
5. Do NOT describe or commit a change (the orchestrator will describe the winning diff before merge if this experiment succeeds)
...
- For Codex dispatch: write the filled template under the workspace `.tmp` directory and pipe via stdin (`ROOT="$(jj workspace root 2>/dev/null || pwd)"; mkdir -p "$ROOT/.tmp"; cat "$ROOT/.tmp/optimize-exp-XXXXX.txt" | codex exec --skip-git-repo-check - 2>&1`).
