...
3. **If `decide.mjs` returns `keep` for that winner: KEEP**
   - Snapshot the experiment workspace change first so the winning diff exists as a real change before any rebase or duplicate. Run jj with cwd = the experiment workspace root.
   ...
   - Repository-local syntax from project instructions and `git log` ALWAYS wins when it differs from Go guidance. Apply compatible Go guidance to quality/clarity/structure without replacing repo-local syntax. The composed message must identify the optimization spec and the hypothesis that produced the kept diff.
   - Describe the experiment change: `jj describe -m "<message composed from the standards above>"`
   - Integrate that change onto the optimization bookmark (`jj rebase` or `jj new` as needed, cwd = the workspace that holds the optimization bookmark)
   - After integration succeeds, forget the winner's experiment workspace and delete its bookmark; the integrated change on the optimization bookmark is the durable artifact
...
4. **Check file-disjoint runners-up** (up to `max_runner_up_merges_per_batch`):
   ...
   - If disjoint, `jj duplicate` the runner-up onto the new baseline and run the same decide loop as step 3.3 against a fresh sample set for that combined snapshot. Do not reuse the standalone experiment's accumulated samples; they were measured against the previous baseline. Collect further measurement whenever `next_measurement` is not `none`. Persist the combined pairing as `kind: integrated` on that same log entry without replacing the standalone comparison. Keep the original standalone log entry for audit.
   - Keep the duplicated change only when that result is eligible and `next_measurement` is `none` (outcome: `runner_up_kept`); then forget that runner-up's experiment workspace and delete its bookmark
   - Otherwise abandon the duplicated change, log it as "promising alone but neutral/harmful in combination" (outcome: `runner_up_reverted`), then forget the runner-up's experiment workspace and delete its bookmark
...
6. **Close the rest.** Forget leftover experiment workspaces. `kept` and `runner_up_kept` are only for diffs on the optimization bookmark. Eligible candidates that were not integrated become `not_selected`. Leave `inconclusive`, `censored`, and `degenerate` as `decide.mjs` returned them.
