...
- Analyze only. You are read-only except for the single write to {scratch_artifact_path}.
  Running the bundled analyzer, read-oriented `jj`, and provider reads through `gh` are
  permitted; do not edit project files, mutate changes or bookmarks, publish, or open PRs.
- The media paths point at already-downloaded files in scratch. Open them; do not expect
  media bytes inline.
- Do NOT invoke other RocketClaw skills or agents. Perform the analysis directly.
