...
4. Check whether `.compound-engineering/config.local.yaml` exists and, if it does, whether `.gitignore` covers `.compound-engineering/*.local.yaml`.
...
- `.compound-engineering/config.local.yaml` exists but is not safely ignored by `.gitignore`
...
Copy `references/config-template.yaml` to `<repo-root>/.compound-engineering/config.local.example.yaml`, creating the directory if needed. This file is versioned in the repo and should always reflect the latest available settings.
