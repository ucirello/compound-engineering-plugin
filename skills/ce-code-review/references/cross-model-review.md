...
2. Scope is `local-aligned` or standalone — the working copy IS the reviewed head. Skip in `pr-remote` / `branch-remote`: the peer reviews the local workspace, which is not the PR/bookmark head.
