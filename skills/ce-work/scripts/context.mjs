...
function jj(...args) {
  try {
    return execFileSync('jj', ['--no-pager', '--config', 'snapshot.auto-track="~glob:.tmp/**"', ...args], { stdio: ['ignore', 'pipe', 'ignore'] }).toString().trim();
  } catch {
    return '';
  }
}
...
function buildResolvedContext() {
  return [
    'RESOLVED_CONTEXT:',
    `cwd: ${process.cwd()}`,
    `workspace_root: ${jj('--ignore-working-copy', 'workspace', 'root') || '(not a Jujutsu workspace)'}`,
    `working_copy: ${jj('log', '--no-graph', '-r', '@', '-T', 'change_id.short() ++ " " ++ commit_id.short()') || '(none)'}`,
  ].join('\n');
}
...
function cli() {
  ...
  // Both boundaries are load-bearing: field transcripts
