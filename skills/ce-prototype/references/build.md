...
Use workspace-root `.tmp/prototype/<date>-<slug>/` (local `.tmp` outside JJ) for the prototype and decisions capsule. If that root fails the safety checks, stop and report the blocker. Survival is best-effort, so do not promise it a lifetime. Explain the workspace-local location if the user asks not to leave the run in their repo. Calling the prototype throwaway describes the code; it is not a request to delete a kept prototype.
...
Scale into the existing app only as a throwaway overlay when the user asks or the question is density or chrome on an existing page — an isolated page will hide that. That overlay is not the shipped feature. Do not include prototype code in the product change or move a product bookmark onto it; JJ may automatically snapshot working-copy edits. Undo those edits when the try ends — restore only the edits you made, never work you did not make. An overlay run therefore leaves no artifact behind; nothing survives it. If you cannot undo them cleanly, name the files you left modified rather than handing off a dirty tree.
