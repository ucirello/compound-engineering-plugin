...
PROBE_DIR="$SCRATCH_ROOT/ce-brainstorm-visual/<run-id>"; mkdir -p "$PROBE_DIR" || exit 1;
...
Use the workspace `.tmp` directory by default because visual probes are disposable scratch:
