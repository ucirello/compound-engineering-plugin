...
2. **Otherwise, in a JJ workspace**, inspect `jj status`, identify bookmarks pointing to the working-copy change with `jj bookmark list --revisions @`, and inspect the change stack with `jj log -r 'trunk()..@'`. Default to `jj diff --from 'trunk()' --to @`. This covers the common case of "simplify everything I've added on this bookmarked change stack before opening a PR." If `trunk()` cannot be resolved or the stack diff is empty, fall back to the working-copy change with `jj diff -r @`.
