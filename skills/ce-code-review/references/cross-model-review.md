...
The host resolves and sanctions one concrete route before egress; `scripts/cross-model-adversarial-review.sh` enforces that fixed route, applies read-only controls, captures schema-shaped JSON, and records identity receipts. Before dispatch it conservatively estimates diff tokens and file count. Oversized diffs are not inlined: the worker gives the peer the orchestrator's compact semantic review map and keeps the exact diff as a private, selectively readable artifact. Tool-limited routes receive the workspace `.tmp/rocketclaw` directory as an additional read root; Codex uses selective `jj diff --from <base>` calls restricted by filesets under its existing read-only sandbox. A failed route writes no artifact and never switches recipients internally.
...
2. Scope is `local-aligned` or standalone — `@` is the reviewed working-copy commit. Skip in `pr-remote` / `bookmark-remote`: the peer would otherwise review a different local revision.
...
**Checkout egress policy — evaluate first.** Read `cross_model_review_mode:` from the same two RocketClaw config files under the ordinary-key rule. Valid values are `auto` (default) and `off`; anything else is invalid and continues to the next layer, then `auto`. When it resolves to `off`, skip the automatic cross-model pass here — before peer resolution, disclosure, or any job start — unless the user explicitly asked for a cross-model peer for this run in conversation. Record the skip reason as **disabled by checkout config**; the in-process `adversarial-reviewer` keeps the lens. A live user prohibition still overrides `auto`.

Resolve the preference in this order:

1. A preference the user **states in conversation** (e.g. "use grok for the cross-model pass").
2. `cross_model_peer:` from the two RocketClaw config files (`config.local.yaml` then `config.yaml`). Apply the ordinary-key rule: first active supported target wins; an invalid value continues to the next layer, then step 3.
...
**Checkout-configured model and effort.** After the target is resolved, read `cross_model_model:` and `cross_model_effort:` from the same two RocketClaw config files under the ordinary-key rule. Pass configured values through the documented override variables. The worker validates them against the selected route and fails closed on incompatibility rather than substituting. Unset keys leave the script's editorial mapping unchanged. Announce configured values exactly as requested. A model or effort the user states in conversation outranks the config keys.
...
- `<base-revision>` = the Stage 1 `BASE` JJ revision (the peer reviews its diff to `@`).
...
**Single-reap finish.** The runner detaches the worker into its own supervised session. Capture the epoch time right after `start` (`date +%s`) and do not poll while local reviewers are active. After local returns are collected, check status once. If still running, issue bounded `wait` slices until the job is terminal or the shared deadline has elapsed. Never begin a slice that would cross the deadline, and do not interleave status reads or no-op turns. At the deadline, reap once and perform one final bounded wait. The worker publishes its normalized artifact only after reviewing the working-copy commit against `<base-revision>`; transport captures the exact diff while the orchestrator owns semantic divisions.
...
- **Dispatch-infrastructure failure** — the runner or worker itself crashed before or after returning a job ID. Recover a no-job-ID failure only at Stage 3d, before the local roster is materialized; never retry it during fold-in. For a job-ID-returned failure, retry only the same sanctioned route while holding target, model, JJ base-to-`@` scope, and adversarial brief fixed. Stop when a failure repeats or the shared deadline is spent. Never substitute a recipient, widen scope, or relax the read-only boundary.
...
The peer reviews the **current working-copy commit** (read-only) against the supplied JJ base revision. Reviewed code/diff content is sent to an external model provider (OpenAI, Anthropic, xAI, or Cursor, depending on the resolved peer). `CROSS_MODEL_PEERS` restricts which providers may receive content.
...
- **codex:** `-s read-only` with cwd at the repo root (may run the scoped JJ diff itself).
