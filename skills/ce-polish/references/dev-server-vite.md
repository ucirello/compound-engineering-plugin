...
- **HMR websocket port:** Vite's HMR uses a separate websocket that inherits the dev-server port by default. If the project pins `server.hmr.port` in `vite.config.ts`, the RocketClaw polish reachability probe against the dev-server port still works, but the embedded browser may need additional configuration to reach HMR.
