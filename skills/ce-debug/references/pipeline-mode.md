...
- **Phase 3 (workspace/bookmark):** Operate on the caller's PR bookmark. The orchestrator decides that context, so never prompt to create a branch or about pre-existing work. Resolve the bookmark and its remote tip with `jj bookmark list` and `jj log`; an empty `@` can sit above the PR head at `@-`. Keep the fix in a new descendant of the caller's tip, commit only fix-owned files, advance only that bookmark, and push with `jj git push --remote <remote> --bookmark <bookmark>`. Do not rewrite the published tip or absorb unrelated working-copy files. If that separation or the caller's bookmark cannot be established, defer as `needs-human`. Never weaken, skip, or mock a failing assertion to make it pass. Repair the real issue or defer.
...
**Commit message:** "Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards." Here the quoted history command refers to history inspected using `jj log` for this JJ workflow. Active project instructions and observed history syntax override the Go guidance. Keep the bug, causal fix, and relevant issue/PR references in the message; do not impose a fixed prefix. The message argument for any JJ commit/description command is `<message composed from the standards above>`.

Run every JJ subprocess from the absolute workspace root and repository-dependent `gh` calls with `GIT_DIR` set from a successful `jj git root` there. Return the full commit ID of the actual fix/bookmark tip as `head_sha`, not an empty working-copy child or a change ID. Verify the remote bookmark reached that commit before reporting `fixed-and-pushed`.
