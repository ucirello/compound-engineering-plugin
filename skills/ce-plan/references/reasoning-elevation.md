...
Hand over the working context as **file paths the subagent reads itself**, never a re-narrated prose brief. Create **one private per-run handoff directory** under `<jj-workspace-root>/.tmp/rocketclaw/`, falling back to `<cwd>/.tmp/rocketclaw/` when there is no Jujutsu repository, and write the prompt-file and every evidence file into *that* directory. Create the local root with owner-only permissions and reserve the run directory atomically with `mkdir`; do not use OS-global temporary storage or temp APIs. On the Claude CLI route the worker grants the elevated model read access to only that one directory (via `--add-dir` on the prompt-file's parent), so the handoff files stay readable without exposing sibling scratch files:
