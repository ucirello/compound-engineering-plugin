...
- **Apply locally; never push.** Never push, open PRs, or file tickets in any mode — push is the outward step the user owns. In **default (interactive)** mode the review applies safe, verified fixes and changes them when the pre-review tree was clean (Stage 5c owns the full rule). In **`mode:agent`** it never mutates the tree — it reports and the caller applies.
...
- **Explicit mutations only.** Never run `gh pr checkout`, `jj restore --from`, `Jujutsu switch`, or similar branch-switch commands. Passing a PR number, URL, or branch name selects **review scope**, not permission to mutate the working tree. To review local uncommitted work on a feature bookmark, check out that branch yourself (or stay on it) and pass `base:` or no target.
...
- `docs/plans/*.{md,html}` -- unified plan artifacts created by ce-brainstorm or ce-plan (decision artifacts; execution progress is derived from Jujutsu, not stored in plan bodies)
...
```
BASE_ARG="{base_arg}"
BASE=$(jj log @ "$BASE_ARG" 2>/dev/null) || BASE="$BASE_ARG"
```

Then produce the same output as the other paths:

```
echo "BASE:$BASE" && echo "FILES:" && jj diff --name-only $BASE && echo "DIFF:" && jj diff --context 10 $BASE && echo "UNTRACKED:" && jj st
```

This path works with any ref — a SHA, `main@origin`, a branch name. Callers reviewing the current checkout should pass explicit `base:` when auto-detection is unnecessary. **Do not combine `base:` with a PR number or branch target.** If both are present, stop with an error: "Cannot use `base:` with a PR number or branch target — `base:` implies the current checkout is already the correct branch. Pass `base:` alone, or pass the target alone and let scope detection resolve the base."

**If a PR number or GitHub URL is provided as an argument:**

Do **not** check out the PR branch. Scope comes from GitHub read APIs plus optional local alignment when @ already matches the PR head branch.
...
Set `BASE:` to `pr:<number-or-url>` (logical marker — not a Jujutsu SHA). Set `UNTRACKED:` from `jj st` on the **current** checkout (usually empty during PR-remote review).
...
1. `jj log -r @ --no-graph --template 'bookmarks'` equals `headRefName`.
2. The PR is **not** cross-repository (`isCrossRepository` is false).
3. The PR head change is contained in the local checkout: `jj log --is-ancestor <headRefOid> @` exits 0. This confirms the working tree actually carries the PR head (allowing unpushed local fixes layered on top) rather than an unrelated same-named branch.
...
- **`local-aligned`:** Resolve `<resolved-base-ref>` from `baseRefName` (fetch if needed). Compute `BASE=$(jj log @ <resolved-base-ref>)`, then set `FILES:` from `jj diff --name-only $BASE` and `DIFF:` from `jj diff --context 10 $BASE` (includes committed, staged, and unstaged changes on the PR branch). Do **not** call `gh pr diff` or append remote hunks — when unpushed fixes exist, the local tree is canonical. Note in Coverage: `scope: local-aligned (PR; local tree diff)`.
...
1. Best-effort fetch PR head without checkout: `jj git fetch origin <headRefName>:refs/review/pr-<number>-head` (substitute PR number from metadata).
...
3. Best-effort fetch the PR base without checkout: `jj git fetch origin <baseRefName>`. When it succeeds, resolve a concrete ref with `the fetched JJ revision` and set `PR_BASE_REF` to that SHA — a **real Jujutsu base ref** reviewers and validators use for file-level Jujutsu diffs (e.g. `data-migration-reviewer` runs `jj diff <PR_BASE_REF> -- db/schema.rb`/`structure.sql`). The `pr:<number-or-url>` logical marker in `BASE:` stays the scope marker; `PR_BASE_REF` is the diffable base. When the fetch fails, omit `PR_BASE_REF` and note in Coverage — schema-drift and other jj-diff checks fall back to diff hunks only and must **not** assume `main`.
...
Reviewers and Stage 5b validators in **`pr-remote`** mode must **not** Read/Grep workspace paths for files in `FILES:`. Inspect via `jj show <PR_HEAD_REF>:<path>` when `PR_HEAD_REF` is set, otherwise use only the provided diff hunks. **`local-aligned`** uses normal workspace inspection.

**If a branch name is provided as an argument:**

Substitute the provided branch name as `<branch>`. Do **not** check out `<branch>`.

If `jj log -r @ --no-graph --template 'bookmarks'` equals `<branch>`, use the **standalone (current bookmark)** path below — same tree, explicit branch name; do not use remote-only diff.
...
2. Else resolve `<branch>` as `origin/<branch>` or `<branch>` after `jj git fetch origin <branch>` when needed.
3. Resolve default base bookmark (same logic as standalone). Compute `BASE=$(jj log <base-ref> <branch-ref>)` and `jj diff --context 10 $BASE <branch-ref>`.
4. If `<branch-ref>` cannot be resolved locally, stop: "Cannot diff branch `<branch>` without checkout. Check out that branch, pass its open PR URL/number, or review the current bookmark with `base:`."

On success for remote branch diff, set **branch-remote scope**. The working tree is **not** `<branch>`. Include `<pr-scope-mode>branch-remote</pr-scope-mode>` and `<branch-head-ref><branch-ref></branch-head-ref>` in the Stage 4 review context bundle. Reviewers and Stage 5b validators must **not** Read/Grep workspace paths for files in `FILES:`. Inspect via `jj show <branch-ref>:<path>` or diff hunks only.

Produce:

```
echo "BASE:$BASE" && echo "FILES:" && jj diff --name-only $BASE <branch-ref> && echo "DIFF:" && jj diff --context 10 $BASE <branch-ref> && echo "UNTRACKED:" && jj st
```

**If no argument (standalone on current bookmark):**

Apply the same base-detection logic as branch mode above, using the current bookmark (i.e., `gh pr view --json baseRefName,url` with no argument defaults to the current bookmark).

If no base can be resolved, **stop**. Do not fall back to `jj diff` — a standalone review without the base would only show uncommitted changes and silently miss all described work on the branch.
...
Using `jj diff $BASE` (without `..@`) diffs the merge-base against the working tree, which includes committed, staged, and unstaged changes together.
...
# would otherwise make `Jujutsu diff` fail, awk print 0, and the lite gate clear on the wrong tree.
if [ -z "${DIFF_A:-}" ] || ! jj log -r --quiet "${DIFF_A}^{commit}" >/dev/null 2>&1 \
   || { [ -n "${DIFF_B:-}" ] && ! jj log -r --quiet "${DIFF_B}^{commit}" >/dev/null 2>&1; }; then
  ...
  # globs single-quoted and inline so the shell passes them to Jujutsu literally (an unquoted or
  # variable-held glob would be expanded against the CWD before Jujutsu ever sees it).
  EXEC_LINES=$(jj diff --numstat "$DIFF_A" $DIFF_B -- '*.rb' '*.py' '*.js' '*.jsx' '*.ts' '*.tsx' '*.go' '*.rs' '*.java' '*.swift' '*.kt' '*.c' '*.cc' '*.cpp' '*.cs' '*.php' '*.ex' '*.exs' '*.scala' 2>/dev/null | awk '{s+=$1+$2} END{print s+0}')
  echo "EXEC_LINES:$EXEC_LINES"
  FILES=$(jj diff --name-only "$DIFF_A" $DIFF_B 2>/dev/null)
...
**PR/URL mode:** Use the PR title, body, and linked issues from `gh pr view` metadata. Supplement with change messages from the PR if the body is sparse.

**Branch mode:** Run `jj log --no-pager ${BASE}..<branch-ref>` using the resolved merge-base and resolved branch ref from Stage 1. Use `<branch-ref>` (the resolved `origin/<branch>` or fetched ref), not the raw `<branch>` argument — a remote-only branch has no matching local ref, so the raw name would fail or read a stale same-named local branch.

**Standalone (current bookmark):** Run:

```
echo "BRANCH:" && jj log -r @ --no-graph --template 'bookmarks' && echo "COMMITS:" && jj log --no-pager ${BASE}..@
```
...
**When intent is ambiguous:** Infer from branch name, changes, PR title/body, diff, `plan:`, and conversation. Write the best-effort intent summary and note uncertainty in Coverage — never block on a clarifying question.
...
Read-only here means **non-mutating**, not "no shell access." Reviewer sub-agents may use non-mutating inspection commands when needed to gather evidence or verify scope, including read-oriented `Jujutsu` / `gh` usage such as `Jujutsu diff`, `jj show`, `jj file annotate`, `jj log`, and `gh pr view`. In **`pr-remote`** or **`branch-remote`** scope (see Stage 1), inspect changed files via `jj show <remote-head-ref>:<path>` or diff hunks — do not Read/Grep workspace paths for files in scope. They must not edit project files, change branches, change, push, create PRs, or otherwise mutate the checkout or repository state.
...
3. **Spawn validators with bounded parallelism.** One sub-agent per finding, dispatched independently using the validator template and the same bounded scheduler from Stage 4. Each validator receives:
   ...
   - Inspection access scoped by mode: in `local-aligned`, Read/Grep/jj file annotate the cited code, callers, guards, framework defaults, and history; in `pr-remote`/`branch-remote`, inspect via `jj show <remote-head-ref>:<path>` or the provided diff hunks only — do not Read/Grep workspace paths for files in scope.
...
**Commit when the pre-review tree was clean.** Before applying, note whether the working tree already had uncommitted changes (`jj st`). The permanence gate is the **push**, not the change — a local change is private and reversible (`jj undo`).

- **Clean before the review:** after applying and verifying, change the fixes as one isolated, review-labeled fix change — `fix(review): <summary>`, or the repo's nearest convention if `review` isn't an allowed scope. Labeled and reversible, returning the tree to a known state.
- **Dirty before the review:** apply but do **not** change — the fixes interleave with the user's in-flight work and ride along with the change they were already going to make. The Applied section lists what changed.
...
```json
{
  ...
  "scope": {
    ...
    "head_id": "<jj log -r @ --no-graph --template commit_id>",
    "pr_url": "<url or null>",
    "files_changed": 0
  },
...
After Stage 6, stop. Never push, open PRs, or file tickets from this skill. In default (interactive) mode, Stage 5c has already applied and (on a clean pre-review tree) described the safe fixes; in `mode:agent` the review mutates nothing — the caller (for example `ce-work`) and the user apply fixes, file tickets, or accept residual risk using the report and artifact.
...
{
  "run_id": "<run-id>",
  "branch": "<jj log -r @ --no-graph --template 'bookmarks' at dispatch time>",
  "head_id": "<jj log -r @ --no-graph --template commit_id at dispatch time>",
  "verdict": "<Ready to merge | Ready with fixes | Not ready>",
  "completed_at": "<ISO 8601 UTC timestamp>"
}
```

Capture `branch` and `head_id` at dispatch time (no in-skill fixes will land afterward).
