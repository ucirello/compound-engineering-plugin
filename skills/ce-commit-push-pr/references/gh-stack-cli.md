...
After any `gh stack` command that changes local refs, run `jj git import` before inspecting or
mutating JJ state.
...
`add`'s exit 5 instead. Field names `currentBranch` and `branches` are the CLI's JSON schema, not
JJ bookmark names.
...
bookmark. After `jj git fetch`, create a local bookmark at the revision matching `headRefOid` only
when that commit resolves and the name does not collide.
...
alter working-copy content, so the current JJ change remains present.
