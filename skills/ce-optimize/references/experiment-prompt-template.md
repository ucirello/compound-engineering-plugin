...
5. Do NOT describe or integrate the change (the orchestrator handles winning changes)
...
- For Codex dispatch: write the filled template under `<workspace-root>/.tmp/rocketclaw/ce-optimize/` and pass it to Codex through stdin. Resolve `<workspace-root>` with `jj workspace root`, falling back to the current directory when unavailable.
