...
**Trivial-bug fast-path:** Once the problem is clear, decide whether the framework is needed at all. If the cause is immediately readable from the input (single-file typo, missing import, obvious null deref or off-by-one with a one-line fix) and verification doesn't require deep tracing, present the cause and the proposed one-line fix and run Phase 2's **Fix it now / Diagnosis only** user-choice gate before editing — the fast-path saves investigation ceremony, not the user's choice over whether to apply a fix. If the user picks fix, run Phase 3's **Workspace and bookmark check** (current-change confirmation and default-bookmark feature-bookmark prompt), apply the fix, leave a one-line note explaining the cause, and skip to Phase 4's structured summary. If diagnosis only, write the summary and stop. When in doubt, run the full framework; getting the wrong root cause costs more than the few minutes of ceremony.
...
- Current JJ workspace is on the intended bookmark/change; no unintended current-change modifications
...
- No stale build artifacts (`dist/`, `.next/`, compiled binaries from an earlier bookmark/change)
...
As you trace:
- Check recent changes in files you are reading: `jj log --oneline -10 -- [file]`
...
- The jj git remote list (a GitHub origin implies GitHub Issues + PRs; `gh` if available).
...
- Any related ticket or PR surfaced in Phase 1.4 — an open duplicate, an existing fix on another bookmark/PR, a regression's original fix, or a prior merged attempt that failed — and how it shapes the recommendation. If an open PR already fixes this, lead with that link instead of a fresh fix; if a prior merged attempt took the same approach you were about to, say so and explain what that rules out.
...
- Check the current JJ working-copy change (`jj status`). If files you need already have user edits, confirm before editing — do not overwrite in-progress changes.
- If the current bookmark is the default bookmark, ask whether to create a feature bookmark first using the platform's blocking question tool (see Phase 2 for the per-platform names). Resolve the default from `main@origin`, `master@origin`, or the GitHub default branch mapped to `<name>@origin`; default to creating one, derive a name from the bug, then run `jj new <base>; jj bookmark set <name> -r @`. On any other bookmark/change line, proceed.
- Record the pre-fix scope before editing: current JJ `@` commit ID/change ID, parent `@-` if needed, whether `jj status` is clean, and any pre-existing changed files. During Phase 3, keep a list of fix-owned files (the tests and implementation files changed for this bug). Phase 4 uses this to keep simplify/review from touching unrelated bookmark/change work.
...
**On a failed fix:** return to Phase 2 and *explicitly invalidate the current hypothesis* before forming a new one. State out loud what evidence ruled out the prior hypothesis, then form a new one with its own grounding observation and prediction. Do not retry variants of the same theory ("maybe it was the other bookmark/change", "let me also catch this case") — that is the rationalization spiral, not iteration.
...
**If Phase 3 ran**, the next move depends on whether the skill created the feature bookmark/change line in Phase 3.
...
**Simplify before review when useful.** Invoke `/ce-simplify-code` before code review when the current fix diff is non-mechanical and large enough to benefit (default: >=30 changed lines), touches multiple implementation files, introduces a new helper/abstraction, or affects shared/risky surfaces such as auth/authz, public contracts, persistence, concurrency, background jobs, or external services. Use the bookmark/change diff only when the bookmark/change line is skill-owned or clearly contains only this fix. On a pre-existing bookmark/change line, scope simplification to fix-owned files only when those files were clean before Phase 3. If a fix-owned file already had pre-existing user edits, skip `/ce-simplify-code` for that file and record `Simplify: skipped for overlapping pre-existing edits`; file-level simplification could rewrite unrelated hunks the user did not authorize. Do not let simplification widen into unrelated user work.

**Review the final fix scope.** After simplification (or after the skip decision), review every non-mechanical fix unless review tooling is unavailable. Run default `/ce-code-review` only when its diff scope is known to be this fix: the bookmark/change line was created by this skill, or the pre-fix tree was clean and you can pass `base:<pre-fix-jj-revision>`. Do not run default `/ce-code-review` on a pre-existing dirty bookmark/change line or one with unrelated committed work; standalone review uses the current bookmark/workspace diff and may apply fixes outside the bug scope. In that case, run the harness's lightweight review tool only if it accepts an explicit file scope; otherwise perform an explicit manual review of the fix-owned files and record `Code review: targeted manual due to unrelated bookmark/change work`. If `/ce-code-review` is unavailable on an otherwise fix-only scope, fall back to the harness's lightweight review tool when available; otherwise do one explicit manual diff scan and state that dedicated review was unavailable.

**Handle residual findings before shipping.** Inspect the review's Actionable Findings. Do not auto-open a PR with unresolved P0/P1 findings, or with findings whose fix needs a product/design decision. Ask the user whether to fix now, accept/defer durably, or stop. For lower-severity residuals the user accepts, preserve them before any outward handoff: if a PR will be opened, pass them as "Known Residuals" context to `/ce-commit-push-pr`; if the user chooses commit-only or stop, create `docs/residual-review-findings/<bookmark-or-jj-change-id>.md` with the accepted findings and source review context, include it in the JJ change committed with the fix, and mention the file path in the final summary. Accepted residuals must not live only in the session.
...
```
## Post-Fix Quality
**Scope**: [fix-only bookmark/change / base:<pre-fix-jj-revision> / fix-owned files only / targeted manual due to unrelated bookmark/change work]
**Simplify**: [ran/skipped + reason]
**Review**: [ran/skipped/manual + outcome]
**Residuals**: [none / accepted Known Residuals for PR / accepted residuals written to docs/residual-review-findings/<bookmark-or-jj-change-id>.md / blocked pending user decision]
**Re-verification**: [checks rerun after tail edits]
```

#### Skill-owned feature bookmark/change line (created in Phase 3): default to commit-and-PR without prompting

1. **Check for contextual overrides first.** Look at the user's original prompt, loaded memories, and the project's active instructions already in your context for preferences that conflict with auto commit-and-PR — for example, "always review before pushing", "open PRs as drafts", or "don't open PRs from skills". A signal must be an explicit instruction or a clearly applicable rule, not a vague tonal cue. If any apply, honor them — switch to the pre-existing-bookmark/change menu below, or skip the PR step entirely, whichever matches the user's stated preference.
2. **Briefly preview what will happen** — what will be committed, on what bookmark/change line, and that a PR will be opened — then proceed without waiting for confirmation. The preview exists so the user can interrupt; it is not a blocking question. Format and length are your call; keep it scannable.
...
#### Pre-existing bookmark/change line (skill did not create it): ask the user
...
When offering, use the blocking question tool described above. If the user accepts, run `/ce-compound`, then commit the resulting learning doc to the same bookmark/change line and push so the open PR picks up the new commit.
