...
Environment overrides (defaults in parentheses):
  CE_PEER_JOBS_ROOT         base dir ($(jj workspace root)/.tmp, or cwd-relative
                            .tmp when not inside a JJ repository)
...
POSIX path is behaviorally unchanged:
  ...
  jobs root defaults under the JJ workspace `.tmp` (then cwd-relative `.tmp`
            when not in a JJ repository), owner-private.
...
def _private_root_usable(path: str) -> bool:
    ...
    fails the access check, so both land on the cwd-relative `.tmp` fallback
    instead of failing at the first job.
    """
    try:
        os.mkdir(path, 0o700)
...
def jobs_root_base() -> str:
    ...
    primary = os.path.abspath(DEFAULT_ROOT)
    if _private_root_usable(primary):
        return primary
    # Same order as the skills' shell preamble: workspace `.tmp`, then
    # cwd-relative `.tmp` when the workspace root is missing or unwritable.
...
def candidate_jobs_root_bases() -> list:
    ...
    the workspace `.tmp` and the cwd-relative `.tmp` fallback (deduplicated,
    primary first).
