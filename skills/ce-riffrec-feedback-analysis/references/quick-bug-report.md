...
1. Resolve the JJ workspace root, then run the analyzer in repository-local scratch so nothing pollutes durable project paths (`SKILL_DIR` is the directory containing the `ce-riffrec-feedback-analysis` SKILL.md; set it in the same command — shell state does not persist between Bash calls):
