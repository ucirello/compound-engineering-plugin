...
| `base:<revision>` | `base:abc1234` or `base:main@origin` | JJ diff base on the **current workspace** (explicit; skips auto base detection) |
...
- **Apply locally; never push.** Never push, open PRs, or file tickets in any mode — push is the outward step the user owns. In **default (interactive)** mode the review applies safe, verified fixes and records them in an isolated described JJ change when the pre-review change was empty (Stage 5c owns the full rule). In **`mode:agent`** it never mutates the workspace — it reports and the caller applies.
...
- **Explicit mutations only.** Never run `gh pr checkout`, `jj new <target>`, `jj edit`, `jj workspace add`, or similar workspace-switch commands during scope discovery. Passing a PR number, URL, or bookmark name selects **review scope**, not permission to mutate the working copy. To review local work, enter its JJ workspace yourself (or stay in it) and pass `base:` or no target.
...
A full review spawns generic subagents for all 6 always-on local prompt assets, then adds whichever cross-cutting and stack-specific conditionals fit the diff (Stage 3c can collapse this to a lite roster for trivial, low-risk diffs). The model naturally right-sizes: a small config change triggers 0 conditionals = 6 reviewers. A Rails auth feature might trigger security + reliability + adversarial = 9 reviewers.

## Protected Artifacts

The following paths are RocketClaw pipeline artifacts and must never be flagged for deletion, removal, or `.gitignore` by any reviewer:
...
The caller already knows the diff base. Skip all base-bookmark detection and remote resolution. Resolve the provided revision directly:

```
BASE_ARG="{base_arg}"
BASE=$(jj log --no-graph -r "heads(::$BASE_ARG & ::@)" -T 'commit_id ++ "\n"' 2>/dev/null)
[ "$(printf '%s\n' "$BASE" | awk 'NF{n++} END{print n+0}')" -eq 1 ] || BASE="$BASE_ARG"
```

Then produce the same output as the other paths:

```
echo "BASE:$BASE" && echo "FILES:" && jj diff --from "$BASE" --to @ --name-only && echo "DIFF:" && jj diff --from "$BASE" --to @ --context 10 && echo "UNTRACKED:"
```

This path works with any JJ revision — a commit ID, `main@origin`, or bookmark name. Callers reviewing the current workspace should pass explicit `base:` when auto-detection is unnecessary. **Do not combine `base:` with a PR number or bookmark target.** If both are present, stop with an error: "Cannot use `base:` with a PR number or bookmark target — `base:` implies the current workspace is already correct. Pass `base:` alone, or pass the target alone and let scope detection resolve the base."

**If a PR number or GitHub URL is provided as an argument:**

Do **not** switch to the PR branch. Scope comes from GitHub read APIs plus optional local alignment when a bookmark on `@` matches the PR head branch.
...
Set `BASE:` to `pr:<number-or-url>` (logical marker, not a JJ revision). Set `UNTRACKED:` empty: JJ snapshots all non-ignored working-copy files, while ignored files remain outside review scope.
...
1. A bookmark name from `jj bookmark list -r @ -T 'name ++ "\n"'` equals `headRefName`.
2. The PR is **not** cross-repository (`isCrossRepository` is false).
3. The PR head commit is known to JJ and is an ancestor of the working-copy change: `jj log --no-graph -r '<headRefOid> & ::@' -T 'commit_id'` is non-empty. This confirms the workspace carries the PR head (allowing local fixes layered on top) rather than an unrelated same-named bookmark.
...
- **`local-aligned`:** Resolve `<resolved-base-revision>` as `baseRefName@origin`, using `jj git fetch --remote origin --branch <baseRefName>` when needed. Compute the single common ancestor with `jj log --no-graph -r 'heads(::<resolved-base-revision> & ::@)' -T 'commit_id ++ "\n"'`, then set `FILES:` from `jj diff --from "$BASE" --to @ --name-only` and `DIFF:` from `jj diff --from "$BASE" --to @ --context 10`. This includes all changes from the base through the current working-copy change. Do **not** call `gh pr diff` or append remote hunks — when local fixes exist, the JJ workspace is canonical. Note in Coverage: `scope: local-aligned (PR; local workspace diff)`.
...
When **`pr-remote`**, do not materialize PR refs in the local repo. Include `<pr-scope-mode>pr-remote</pr-scope-mode>` in the Stage 4 review context bundle and provide the `gh pr diff` hunks plus GitHub file metadata. Reviewers and validators must rely on those hunks; if additional remote content is essential, use the GitHub API through `gh`, never a Git fetch or checkout.

Reviewers and Stage 5b validators in **`pr-remote`** mode must **not** Read/Grep workspace paths for files in `FILES:`. Use only the provided diff hunks and GitHub data fetched through `gh`. **`local-aligned`** uses normal workspace inspection.

**If a bookmark/branch name is provided as an argument:**

Substitute the provided name as `<branch>`. Do **not** switch the workspace to `<branch>`.

If `jj bookmark list -r @` includes `<branch>`, use the **standalone (current bookmark)** path below — same workspace, explicit bookmark name; do not use remote-only diff.

Otherwise diff the remote/local revision **without switching the workspace**:
...
2. Else resolve `<branch>` as `<branch>@origin` or `<branch>` after `jj git fetch --remote origin --branch <branch>` when needed. `jj git` is used only for remote interoperability.
3. Resolve the default base bookmark (same logic as standalone). Compute the single common ancestor with `jj log --no-graph -r 'heads(::<base-revision> & ::<branch-revision>)' -T 'commit_id ++ "\n"'`, then use `jj diff --from "$BASE" --to <branch-revision> --context 10`.
4. If `<branch-revision>` cannot be resolved locally, stop: "Cannot diff bookmark `<branch>` without resolving it. Enter its JJ workspace, pass its open PR URL/number, or review the current workspace with `base:`."

On success for remote bookmark diff, set **branch-remote scope**. The working copy is **not** `<branch>`. Include `<pr-scope-mode>branch-remote</pr-scope-mode>` and `<branch-head-ref><branch-revision></branch-head-ref>` in the Stage 4 review context bundle. Reviewers and Stage 5b validators must **not** Read/Grep workspace paths for files in `FILES:`. Inspect via `jj file show -r <branch-revision> <path>` or diff hunks only.

Produce:

```
echo "BASE:$BASE" && echo "FILES:" && jj diff --from "$BASE" --to <branch-revision> --name-only && echo "DIFF:" && jj diff --from "$BASE" --to <branch-revision> --context 10 && echo "UNTRACKED:"
```

**If no argument (standalone in the current workspace):**

Apply the same base-detection logic as bookmark mode above, using bookmarks on `@` (and `gh pr view --json baseRefName,url` with no argument, which resolves the GitHub branch associated with the current workspace when available).

If no base can be resolved, **stop**. Do not fall back to `jj diff -r @` — a standalone review without the base would only show the current change and silently miss earlier work in the stack.
...
Using `jj diff --from "$BASE" --to @` diffs the common ancestor against the current JJ working-copy change, including the complete local stack without Git staging concepts.

**Untracked file handling:** JJ automatically tracks non-ignored new files in the working-copy snapshot, so they are included in `FILES:` and `DIFF:`. Files excluded by ignore rules are out of scope; never stop or prompt for them.
...
- **`local-aligned` / standalone / `base:`** — `DIFF_A="$BASE"` (a real JJ revision), `DIFF_B` empty (diffs base vs working copy).
- **`pr-remote`** — no local revisions exist; skip the block and emit `EXEC_LINES:UNKNOWN` + `UNCOUNTED_FILES:1` so Stage 3c forces the full roster. Do **not** model-count from GitHub hunks (it drifts per host/model).
- **`branch-remote`** — `DIFF_A="$BASE"`, `DIFF_B=<branch-head-ref>`, using the JJ revisions resolved in Stage 1.

```
# Fail closed: an unresolved/invalid endpoint must NOT become a silent EXEC_LINES:0.
# Validate both JJ revision endpoints. An unresolved remote revision must not
# become a silent EXEC_LINES:0 and clear the lite gate on the wrong tree.
if [ -z "${DIFF_A:-}" ] || ! jj log --no-graph -r "$DIFF_A" -T 'commit_id' >/dev/null 2>&1 \
   || { [ -n "${DIFF_B:-}" ] && ! jj log --no-graph -r "$DIFF_B" -T 'commit_id' >/dev/null 2>&1; }; then
  echo "EXEC_LINES:UNKNOWN"; echo "UNCOUNTED_FILES:1"; echo "SIGNALS:"
else
  # Count added/deleted executable lines from JJ's Git-format patch, excluding
  # file headers. File globs stay quoted so the shell passes them to JJ literally.
  if [ -n "${DIFF_B:-}" ]; then
    EXEC_LINES=$(jj diff --from "$DIFF_A" --to "$DIFF_B" --git -- '*.rb' '*.py' '*.js' '*.jsx' '*.ts' '*.tsx' '*.go' '*.rs' '*.java' '*.swift' '*.kt' '*.c' '*.cc' '*.cpp' '*.cs' '*.php' '*.ex' '*.exs' '*.scala' 2>/dev/null | awk '/^\+\+\+ |^--- /{next} /^\+|^-/{n++} END{print n+0}')
  else
    EXEC_LINES=$(jj diff --from "$DIFF_A" --to @ --git -- '*.rb' '*.py' '*.js' '*.jsx' '*.ts' '*.tsx' '*.go' '*.rs' '*.java' '*.swift' '*.kt' '*.c' '*.cc' '*.cpp' '*.cs' '*.php' '*.ex' '*.exs' '*.scala' 2>/dev/null | awk '/^\+\+\+ |^--- /{next} /^\+|^-/{n++} END{print n+0}')
  fi
  echo "EXEC_LINES:$EXEC_LINES"
  if [ -n "${DIFF_B:-}" ]; then FILES=$(jj diff --from "$DIFF_A" --to "$DIFF_B" --name-only 2>/dev/null)
  else FILES=$(jj diff --from "$DIFF_A" --to @ --name-only 2>/dev/null); fi
...
**Bookmark mode:** Run `jj log --no-graph -r "$BASE::<branch-revision>" -T 'commit_id.short() ++ " " ++ description.first_line() ++ "\n"'` using the resolved common ancestor and bookmark revision from Stage 1. Use `<branch-revision>` (the resolved `<branch>@origin` or local bookmark), not the raw argument.

**Standalone (current branch):** Run:

```
echo "BOOKMARKS:" && jj bookmark list -r @ && echo "CHANGES:" && jj log --no-graph -r "$BASE::@" -T 'commit_id.short() ++ " " ++ description.first_line() ++ "\n"'
```
...
**When intent is ambiguous:** Infer from bookmark name, JJ change descriptions, PR title/body, diff, `plan:`, and conversation. Write the best-effort intent summary and note uncertainty in Coverage — never block on a clarifying question.
...
Generate a unique run identifier before dispatching any agents. This ID scopes all agent artifact files and the post-review run artifact to the same workspace-local directory. Resolve the root with `jj workspace root`; outside JJ, use the current directory as the local fallback.
...
5. Review context: intent summary, file list, diff, scope mode (`local-aligned` | `pr-remote` | `branch-remote`), and `<branch-head-ref>` when set
6. Workspace-local run directory and reviewer name for the artifact file path
...
Read-only here means **non-mutating**, not "no shell access." Reviewer sub-agents may use non-mutating inspection commands when needed to gather evidence or verify scope, including `jj diff`, `jj file show`, `jj file annotate`, `jj log`, and `gh pr view`. In **`pr-remote`** scope, use GitHub metadata/diff hunks only. In **`branch-remote`** scope, inspect changed files via `jj file show -r <remote-head-revision> <path>` or diff hunks — do not Read/Grep workspace paths for files in scope. They must not edit project files, edit or create JJ changes/workspaces, push, create PRs, or otherwise mutate repository state.
...
The artifact file **must** carry the full detail-tier fields (`why_it_matters`, `evidence`); the compact *return* omits all detail-tier fields **except `first_evidence`**, but writing the compact shape to the artifact (a common reviewer slip) silently strips the detail Coverage and the keyed detail lines depend on. However review context is delivered — inlined, or file-backed for a large diff — each reviewer still receives the full subagent-template output contract; file-backed context never licenses a thinner one. `suggested_fix` is optional in both tiers -- included in compact returns when present so callers can apply fixes after review. If the file write fails, the compact return still provides everything the merge needs.

**Always-on unstructured local prompt assets** (`agent-native-reviewer`, `learnings-researcher`) are dispatched as generic subagents through the same bounded parallel scheduler as the structured personas. Read their prompt files from `references/personas/`, then give them the same review context bundle the personas receive: entry mode, any PR metadata gathered in Stage 1, intent summary, review base bookmark name when known, `BASE:` marker, file list, diff, and `UNTRACKED:` scope notes. Do not invoke them with a generic "review this" prompt. Their output is unstructured and synthesized separately in Stage 6.

**Conditional unstructured local prompt asset** `deployment-verification-agent` is dispatched as a generic subagent through the same bounded parallel scheduler when the migration-artifact gate applies. Read the prompt file from `references/personas/`, then pass the same review context bundle plus the applicability reason (for example, which migration files triggered the prompt asset). Its output is unstructured and must be preserved for Stage 6 synthesis just like the always-on unstructured prompt assets. Schema drift is handled by the `data-migration` persona as structured findings — not here.
...
3. **Spawn validators with bounded parallelism.** One sub-agent per finding, dispatched independently using the validator template and the same bounded scheduler from Stage 4. Each validator receives:
   ...
   - The full diff
   - The scope mode and remote bookmark revision, mirroring the Stage 4 reviewer bundle: inject `<pr-scope-mode>local-aligned | pr-remote | branch-remote</pr-scope-mode>` and, for `branch-remote`, `<branch-head-ref>...</branch-head-ref>`. The validator template defaults to local-aligned workspace inspection when scope is absent, so omitting it in `pr-remote`/`branch-remote` makes validators verify findings against the stale working copy — dropping valid findings or confirming false ones on the wrong revision.
   - Inspection access scoped by mode: in `local-aligned`, Read/Grep/`jj file annotate` the cited code, callers, guards, framework defaults, and history; in `pr-remote`, use GitHub data/diff hunks only; in `branch-remote`, inspect via `jj file show -r <remote-head-revision> <path>` or the provided diff hunks — do not Read/Grep workspace paths for files in scope.
...
Severity, confidence, and cross-reviewer agreement tell you what to do first and what to flag loudly — they do not gate the decision. There is no deny-list: downside is controlled after the fact (restore + visible diff + the JJ operation log), not by a precondition.
...
**Verify, then keep.** After applying, run the affected tests and lint (targeted by default; broaden when fixes span files). If they fail, restore only that fix and report it as a finding instead — an unverified fix is not finished. Never leave the workspace red.

**Review the autofix diff before finishing.** Before describing or reporting applied fixes, diff only the changes introduced during Stage 5c against the pre-apply checkpoint. Run one self-review pass over that diff:
...
**Describe when the pre-review change was empty.** Before applying, run `jj diff -r @ --name-only`; an empty result means the current change is empty. The permanence gate is the **push**, not a local JJ description; the operation log keeps local edits reversible.

- **Empty before the review:** after applying and verifying, describe the fixes as one isolated review change, then run `jj new` so the workspace returns to an empty change. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Repository-local instructions and syntax observed in `git log` take precedence; use Go guidance only when compatible, and do not force a fixed prefix or scope. Apply the result with `jj describe -m "<locally-conformant description>" && jj new`.
- **Non-empty before the review:** apply but do **not** alter the existing description or create a new change — the fixes interleave with the user's in-flight work. The Applied section lists what changed.
...
2. **Applied (default mode only).** When Stage 5c applied fixes, list them first — before the findings — in an Applied section (see review output template); each entry carries `#`, file, the fix, and reviewer (a multi-file fix is one row with one `#`), then a one-line validation outcome (e.g. "pin tests 4 -> 6; suite 94 pass, lint clean") and JJ description status (described and followed by a new empty change when the original change was empty, or left in the existing described change when it was non-empty). Flag green-but-unverifiable edits (auth/contract/concurrency) prominently. Omit this section in `mode:agent` and when nothing was applied. Applied findings appear here, not in the severity tables.
...
Emit **one raw JSON object** as the primary response — a single bare JSON value, **no markdown code fence**. A leading ```` ```json ```` fence makes the response start with backticks and breaks naive `JSON.parse` consumers, so never wrap it. Also write `review.json` under `<workspace>/.tmp/rocketclaw/ce-code-review/<run-id>/` with the same payload.
...
```json
{
  ...
  "scope": {
    ...
    "bookmarks": ["<bookmarks on @>"],
    "head_change_id": "<jj log -r @ -T change_id>",
    "head_commit_id": "<jj log -r @ -T commit_id>",
    "pr_url": "<url or null>",
    "files_changed": 0
  },
  ...
  "testing_gaps": [],
  "coverage": {},
  "artifact_path": "<workspace>/.tmp/rocketclaw/ce-code-review/<run-id>/",
  "run_id": "<run-id>"
}
```
...
5. **Protected artifacts are respected.** Discard any findings that recommend deleting files in `docs/brainstorms/`, `docs/plans/`, or `docs/solutions/`, or adding those paths to `.gitignore`.
...
After Stage 6, stop. Never push, open PRs, or file tickets from this skill. In default (interactive) mode, Stage 5c has already applied and, when the original JJ change was empty, described the fixes and opened a new empty change; in `mode:agent` the review mutates nothing — the caller (for example `ce-work`) and the user apply fixes, file tickets, or accept residual risk using the report and artifact.
...
- Include the workspace-local run-artifact path when one was written: `<workspace>/.tmp/rocketclaw/ce-code-review/<run-id>/`
...
Always write run artifacts under `<workspace>/.tmp/rocketclaw/ce-code-review/<run-id>/` (or `<cwd>/.tmp/rocketclaw/ce-code-review/<run-id>/` outside JJ):
...
{
  "run_id": "<run-id>",
  "bookmarks": ["<jj bookmark list -r @ names at dispatch time>"],
  "head_change_id": "<jj log -r @ -T change_id at dispatch time>",
  "head_commit_id": "<jj log -r @ -T commit_id at dispatch time>",
  "verdict": "<Ready to merge | Ready with fixes | Not ready>",
  "completed_at": "<ISO 8601 UTC timestamp>"
}
```

Capture `bookmarks`, `head_change_id`, and `head_commit_id` at dispatch time (no in-skill fixes will land afterward).
