...
Severity, confidence, and cross-reviewer agreement tell you what to do first and what to flag loudly — they do not gate the decision. There is no deny-list: downside is controlled after the fact through a visible diff and Jujutsu operation recovery, not by a precondition.
...
**Verify, then keep.** After applying, run the affected tests and lint (targeted by default; broaden when fixes span files). If they fail, restore that fix from the recorded pre-apply Jujutsu state and report it as a finding instead — an unverified fix is not finished. Never leave the working copy red.

**Review the autofix diff before finishing.** Before describing/finalizing or reporting applied fixes, diff only the changes introduced during Stage 5c against the pre-apply checkpoint. Run one self-review pass over that diff:
...
If this self-review changes files, rerun the affected tests or lint for those follow-up edits before describing/finalizing or reporting; the earlier validation only covers the original autofix diff.

**Finalize an isolated Jujutsu change when the pre-review working-copy revision was empty.** Before applying, record whether `jj diff` was empty and capture the current change ID. The permanence gate is `jj git push`, not a local description or new working-copy change; local operations remain recoverable through `jj op log` and `jj undo`.

- **Empty current change before the review:** after applying and verifying, describe the fix revision according to current project instructions and history read with `jj log`, then run `jj new` so the working copy returns to an empty child change. Current project instructions and `jj log` history win over generic guidance. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards.
- **Non-empty current change before the review:** apply but do **not** alter its description or split/finalize it; the fixes remain with the user's in-flight work. The Applied section lists what changed.
...
2. **Applied (explicit local apply only).** When Stage 5c applied fixes, list them first — before the findings — in an Applied section (see review output template); each entry carries `#`, file, the fix, and reviewer (a multi-file fix is one row with one `#`), then a one-line validation outcome and dynamic Jujutsu change status (described and finalized as an isolated revision when the current change was initially empty, or retained in the existing undescribed/described change when it was not). Flag green-but-unverifiable edits (auth/contract/concurrency) prominently. Omit this section when local apply was not authorized or nothing was applied. Applied findings appear here, not in the severity tables.
...
```json
{
  ...
  "scope": {
    ...
    "bookmarks": "<bookmarks attached to the working-copy revision>",
    "change_id": "<Jujutsu change ID for @>",
    "pr_url": "<url or null>",
    "files_changed": 0
  },
