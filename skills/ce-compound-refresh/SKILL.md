...
For **Keep** outcomes, list them under a reviewed-without-edits section so the result is visible without creating Jujutsu churn.
...
1. Which branch is currently checked out (main/master vs feature bookmark)
2. Whether the working tree has other uncommitted changes beyond what compound-refresh modified
3. Recent change messages to match the repo's change style
...
|---------|---------------|
| On main/master | Create a branch named for what was refreshed (e.g., `docs/refresh-auth-and-ci-learnings`), change, attempt to open a PR. If PR creation fails, report the branch name. |
...
Include only the files that compound-refresh modified — not other dirty files in the working tree.

### Interactive mode

First, run `jj log -r @ --no-graph --template 'bookmarks'` to determine the current bookmark. Then present the correct options based on the result. Include only compound-refresh files regardless of which option the user picks.

**If the current bookmark is main, master, or the repo's default branch:**

1. Create a branch, change, and open a PR (recommended) — the branch name should be specific to what was refreshed, not generic (e.g., `docs/refresh-auth-learnings` not `docs/compound-refresh`)
2. Commit directly to `{current bookmark name}`
3. Don't change — I'll handle it

**If the current bookmark is a feature bookmark, clean working tree:**

1. Commit to `{current bookmark name}` as a separate change (recommended)
2. Create a separate branch and commit
3. Don't commit

**If the current bookmark is a feature bookmark, dirty working tree (other uncommitted changes):**

1. Commit only the compound-refresh changes to `{current bookmark name}` (selective staging — other dirty files stay untouched)
2. Don't commit

### Commit message

Write a descriptive change message that:
- Summarizes what was refreshed (e.g., "update 3 stale learnings, consolidate 2 overlapping docs, delete 1 obsolete doc")
- Follows the repo's existing change conventions (check recent jj log for style)
...
6. **Amend or create a follow-up change when the check produces edits.** If step 4 or step 5 resulted in an edit to an instruction file and Phase 5 already described the refresh changes, stage the newly edited file and either amend the existing change (if still on the same branch and no push has occurred) or create a small follow-up change (e.g., `docs: add docs/solutions/ discoverability to AGENTS.md`, or `docs: add CONCEPTS.md discoverability to AGENTS.md`, or a combined message when both edits landed). If Phase 5 already pushed the branch to a remote (e.g., the branch+PR path), push the follow-up change as well so the open PR includes the discoverability change. This keeps the working tree clean and the remote in sync at the end of the run. If the user chose "Don't commit" in Phase 5, leave the instruction-file edits unstaged alongside the other uncommitted refresh changes — no separate change logic needed.
