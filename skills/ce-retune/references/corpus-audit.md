...
3. Version history. From the workspace root (`jj workspace root`), run `jj log -r "diff_lines(substring:'<substring>')"` for the change that introduced the line, then read that change description and the PR it belongs to.
...
- **`keep`**: concrete citable provenance a capable model could not infer, such as a test asserting it, a documented learning, or a change that added it to fix a named bug. The ruling must carry the citation: path, test name, or commit id.
...
| `citation` | required on `keep`: path, test name, or commit id. Empty is not a `keep` |
