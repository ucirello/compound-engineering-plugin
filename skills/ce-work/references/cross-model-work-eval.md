...
|---|---|---|
| E1 native restraint | `ce-work <root>/plans/feature.md`; no directive, caller binding, or enabled config | Native inline/subagent engine; nothing leaves the host and Work creates no workspace for an ordinary synchronous unit; Work itself still runs the finishing steps. |
...
| E18 transactional failure | Synthetic transport applies, but canonical verification fails | Restore the exact pre-fold `@` working copy under the lock before sibling, retry, resume, or fallback; preserve the external result and block if exact restoration is unprovable. |
...
| E20 linked-checkout sibling | Work is itself running in an existing linked workspace and selects external implementation for one unit | Create a new detached **sibling** with `jj workspace add DEST --name NAME -r REV`, place it under `<workspace-root>/.tmp/ce-work/<run-id>/` rather than beneath the active checkout, base it at the recorded clean canonical commit id, and keep canonical fold-in host-owned. Do not reject the route merely because the active checkout is already a workspace, and do not create a nested workspace. |
...
| E40 sandboxed worker no-commit | A Codex or Cursor unit has finished files and scoped checks in its detached workspace; the worker is about to `jj describe`/`jj commit` | Do not instruct the worker to write the working-copy change. Leave the working copy for the host snapshot and treat completion as files plus scoped checks. Host `terminalize` snapshots the tree. A Codex sandbox `EPERM` on a socket bind or peer-credential probe is not proof the host lacks the capability. |
