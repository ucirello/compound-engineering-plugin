...
- **Read** `docs_root` from `<repo-root>/.rocketclaw/config.yaml` only (`<repo-root>` = `jj workspace root`, run with cwd set to the absolute workspace root). Outside JJ, use the local project root. Do not read it from `config.local.yaml`. Unset -> `<root>` is `docs`, exactly as before.
...
Create a run directory only when an artifact or an evidence dossier needs one. Run this block with cwd set to the absolute workspace root (or the local project root outside JJ) before writing either; it rejects a symlink or a scratch root owned by another user:
