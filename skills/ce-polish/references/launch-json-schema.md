...
Polish reads `.rocketclaw/launch.json` at the workspace root to resolve the dev-server start command. The schema is a subset of VS Code's `launch.json` format, retained for editor interoperability.
...
`.rocketclaw/launch.json` is the workspace-local RocketClaw namespace for this launch metadata. The schema remains editor-compatible because:
- Claude Code, Cursor, and VS Code launch configurations use the same core fields
