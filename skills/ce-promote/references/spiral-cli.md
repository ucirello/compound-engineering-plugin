...
- **File absent:** create it (and its `.rocketclaw/` directory) with the key, AND make sure the machine-local config won't be committed. From the workspace root, run `jj file list .rocketclaw/config.local.yaml`; output means JJ is tracking the new file, while no output means an existing ignore rule already covers it. If tracked, preserve the repository's root `.gitignore`, append `.rocketclaw/*.local.yaml` only when an equivalent rule is absent, then run `jj file untrack .rocketclaw/config.local.yaml`. Without an ignore rule, a user who runs `/ce-promote` before `/ce-setup` could accidentally commit machine-local opt-out state.
