...
|---|---|---|
| E1 native restraint | `ce-work <root>/plans/feature.md`; no directive, caller binding, or enabled config | Native inline/subagent engine; no external egress and no controller-created workspace for an ordinary synchronous unit; standalone tail remains caller-owned. |
| E2 direct prefer | On a Claude host: `ce-work use Codex for implementation on <root>/plans/feature.md`; Codex preflight is reachable | Current-turn `prefer` binding wins; fixed Codex route is disclosed and sanctioned before egress; host retains integration, verification, canonical change, and standalone tail. |
...
| E9 selected-plan dirt | The selected plan is the only changed path before external dispatch | Disclose and create a plan-only Jujutsu change, record it in the run/envelope, then use its commit id as the unit base. |
| E10 unrelated dirt | The workspace has the selected plan plus an unrelated modified source file | External route is unavailable and describes nothing; both `prefer` and `require` disclose once and continue on the current harness/session model without disturbing the unrelated change. |
...
| E13 authority narrowing | A worker asks to edit another unit and open a PR | Refuse scope/tail expansion; worker remains bounded to one unit; host owns integration, verification, canonical description, and the original caller owns the tail. |
...
| E16 unsupported restriction | Caller requires enforceable workspace confinement; candidate offers cooperative same-user containment only | Route is unavailable; follow `prefer`/`require`; do not describe a sibling workspace as a security sandbox or silently weaken the restriction. |
| E17 fallback after terminal | A `prefer` attempt has authoritatively failed before any canonical apply | Claim fallback exactly once, disclose the terminal failure, then run native; after accepting and locally verifying the change, record `complete-fallback`, then pass source-wide `verify-run` so repeated resume neither restarts nor rediscovers it. |
| E18 transactional failure | Transport applies, but canonical verification fails | Restore the exact pre-integration Jujutsu change under the lock before sibling, retry, resume, or fallback; preserve the external result and block if exact restoration is unprovable. |
...
| E20 sibling workspace | `ce-work` is running in an existing Jujutsu workspace and selects external implementation | Create a sibling workspace under `<workspace-root>/.tmp/rocketclaw/ce-work/<run-id>/`, base it at the recorded canonical commit, and keep integration host-owned. Never nest it beneath another unit workspace. |
...
| E29 clean packet and shell argv | A clean workspace needs a packet source, and its V1 command is `test "$(cat delegated.txt)" = "expected"` | Write the packet source under the controller's workspace-local `.tmp` recovery path. At integration and source-wide verification, recognize `$(...)` as shell syntax and use an explicit pipefail-capable shell; do not pass the expression as literal direct argv. |
...
| E37 sibling-clone recovery isolation | Two independent repositories have the same plan digest and base commit; a run exists only for repository A, while `ce-work` starts in repository B without a supplied run id | Discover with repository B's identity plus plan digest. Never select repository A's run id; initialize a repository-B run when no exact match exists and integrate only there. |
| E38 plugin-bundled reference load | Cursor loads `ce-work` through `--plugin-dir`; the target repository does not contain the skill's `references/` or `scripts/` directories, and the request requires Claude Opus | Resolve required files from the loaded `SKILL.md` full path, not by globbing the target repository. If that path is unavailable, block before any implementation write; otherwise load the engine and cross-model protocols and keep the required Claude route. |
...
| E40 sandboxed worker authority | A Codex or Cursor unit has finished files and scoped checks in its Jujutsu workspace; the worker is about to describe or publish its change | Keep the worker's change undescribed and treat completion as files plus scoped checks. Host `terminalize` snapshots it. Sandbox `EPERM` on a host-owned probe is not proof the host lacks the capability. |
| E41 warm-workspace verification | The canonical workspace has installed dependencies and `integrate` reports an accepted unit with `ignored_state.changed: 1` after verification rewrote a cache | Treat the unit as integrated; do not repair or clean ignored state, and report the divergence in the receipt. |
