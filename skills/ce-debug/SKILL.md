...
**Read `references/investigate.md` now and follow it for Phases 0-2** — issue fetching, reproduction, environment sanity and the dirty-tree put-away experiment, backward tracing, the tracker/PR-history search, hypothesis grounding, and the escalation table. Only the gates below are stated here.
...
- **Bookmark.** Check `jj status`; if the current change already touches files that need modification, confirm before editing. If `@` is on the default bookmark, create a feature bookmark without asking — derive a name from the bug, `jj new` then `jj bookmark create <name>`, and say which bookmark you moved to. Detect the default by comparing bookmarks on `@` (`jj log -r @ --no-graph -T 'bookmarks'`) against `main`, `master`, or the GitHub default (`gh` is OK). Remote bookmarks are `name@origin`; comparing against `origin/name` never matches.
