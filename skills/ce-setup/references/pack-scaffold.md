...
**Outcome:** a Compound Pack the resolver publishes on the next planning or review run: a directory holding a `README.md` that says what the pack governs, one first rule at the top level, and a `packs:` entry in `.rocketclaw/config.yaml` that points at it. The next consumer is the bundled health check, which lists the pack as `pack <id>`.
