...
adapter_argv() {
  case "$1" in
    ...
    opencode)
      ...
      ;;
    opencode2)
      # Distinct from opencode. No V1 env, no --dir (not on opencode2 run).
      # --model is provider/modelname#variant. cwd is PEER_WORKDIR via run_timeout_cmd.
...
mkdir -p "$SCRATCH_ROOT" || skip "cannot create workspace .tmp; skipping"
...
(cd "$REPO_ROOT" && jj --color=never --no-pager diff --from "$BASE" --git) > "$DIFF_SOURCE" 2>/dev/null || skip "cannot stage reviewed diff; skipping"
...
attempt_route() {
  ...
  case "$route" in
    ...
    opencode2)
      ...
      run_timeout_cmd "" "$attempt_hard" idle
      classify_route_output
      [ "$RUN_SUCCEEDED" = true ] && recover_findings_json "$PEERLOG" "$RAW_OUT"
      ;;
  esac
