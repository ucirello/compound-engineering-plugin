...
#   <base-ref> the diff base (e.g. a common-ancestor commit ID or bookmark); the peer reviews
...
LOCAL_TMP="$REPO_ROOT/.tmp/rocketclaw/cross-model-review"
mkdir -p "$LOCAL_TMP" || skip "cannot create workspace-local scratch directory; skipping"

OUT="$RUN_DIR/adversarial-$PEER.json"
NONCE="$(date +%s)-$$-$RANDOM"
PROMPT_FILE="$LOCAL_TMP/xmodel-prompt-$NONCE"
PEERLOG="$LOCAL_TMP/xmodel-log-$NONCE"
: > "$PROMPT_FILE" || skip "cannot create prompt file; skipping"
: > "$PEERLOG" || skip "cannot create peer log; skipping"
...
if [ -s "$OUT" ]; then
  _norm="$LOCAL_TMP/xmodel-norm-$NONCE"
  : > "$_norm" || skip "cannot create normalization file; skipping"
