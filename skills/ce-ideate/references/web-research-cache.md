...
- **Repo mode:** the focus hint plus a stable repo discriminator. This keeps the cache key meaningful when focus is empty — two bare-prompt invocations in the same repo legitimately share research, but the key still differentiates repos. Cache files live under the workspace `.tmp` (or cwd-relative `.tmp` when not in a JJ repository). Resolve the discriminator with this fallback chain and hash the result (first 8 hex chars of sha256 is sufficient):
    1. `jj git remote list` — take the `origin` URL; stable across machines, correct for collaborators on the same remote.
    2. `jj workspace root` — absolute workspace path; machine-local but always available in a JJ repository.
