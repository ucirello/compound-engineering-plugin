...
**The fix:** One hypothesis, one change, one test. If the first change does not fix it, restore only that attempt's content or undo only its isolated Jujutsu operation before trying the next. Changes should be additive to understanding, not cumulative to the codebase.
