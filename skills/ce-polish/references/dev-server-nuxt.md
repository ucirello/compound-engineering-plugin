...
- **Nitro server engine:** Nitro (Nuxt's server engine) adds its own dev server behind Nuxt's; this workflow only needs the Nuxt port. Do not probe the Nitro internal port separately.
- **Port auto-increment:** Nuxt auto-increments the port if 3000 is already taken (unlike Next.js which errors). The kill-by-port step handles this by reclaiming the port before starting, so the auto-increment behavior does not cause issues in practice.
