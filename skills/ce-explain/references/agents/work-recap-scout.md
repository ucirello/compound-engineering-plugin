...
1. **JJ history activity** — `jj log` over the window (subjects, commit IDs, dates, authors), and for the substantial changes, a stat-level view of what they touched. Group obviously-related changes (a feature bookmark's changes, a fix and its follow-ups) rather than listing them flat.
...
3. **Project docs** — plans, brainstorms, and solution docs added or modified in the window (`docs/plans/`, `docs/brainstorms/`, `docs/solutions/`, or wherever this repo keeps them). These carry the *why* behind the JJ history activity — quote the decision or problem statement, not the whole doc.

## Output

Write an **evidence file** to `{run-dir}/recap-evidence.md`: at most 120 lines. For each notable piece of work in the window:

- What changed, in one line, with the commit ID(s) or PR number and date
- Why, when a doc or commit body says so — quoted, with the source (`file:line` or sha)
- The main files/areas touched

Order by date. Bundle minor mechanical changes (version bumps, typo fixes) into a single "housekeeping" line rather than enumerating them. If the window is empty — no JJ history changes, no doc changes — write nothing and report exactly that.
