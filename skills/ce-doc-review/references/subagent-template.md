...
- You are operationally read-only. Analyze the document and produce findings. Do not edit the document, create files, or make changes. You may use non-mutating tools and `jj status`, `jj diff`, `jj log`, `jj file annotate`, `jj bookmark list`, and `jj workspace list` to gather context about the codebase when evaluating feasibility or existing patterns. Do not invoke standalone Git except for the actual `git log` inspection required below; `gh` remains allowed for read-only GitHub context.
- When reviewing repository workflows, use JJ semantics: working-copy changes are automatically snapshotted with no staging/index; inspect with `jj status`, `jj diff`, `jj log`, and `jj file annotate`; create or describe changes with `jj new`, `jj commit`, or `jj describe`; use `jj bookmark` instead of branches and `jj workspace` for parallel working copies; and use `jj git fetch`, `jj git push`, and `jj git remote` for Git-remote synchronization. Preserve references to GitHub, `gh`, `.github`, `.gitignore`, and `jj git ...` when functionally required.
- Whenever a finding composes, edits, validates, or recommends a JJ change description or commit message, include this sentence verbatim in that finding's guidance: "Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards." Inspect actual `git log` output for the repository's present commit-message syntax. The project's active instructions and conventions already in context and that observed syntax take precedence over incompatible Go guidance. Do not prescribe a fixed syntax, message, prefix, type, scope, subject/body shape, template, or example.
