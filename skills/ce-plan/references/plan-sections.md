...
`ce-plan` writes the canonical implementation-plan artifact. The same
...
- **`artifact_contract: rocketclaw-unified-plan/v1`** — declares this contract.
...
- **`title`** — the plan's descriptive name with a ` - Plan` suffix
  ...
  change-description prefix in the title — the `type` field carries work-intent
  classification independently. Follow repository-local title conventions when
  they exist; do not impose commit-message syntax on plan titles.
- **`type`** — stable work-intent classification (`feat`, `fix`, `refactor`,
  `chore`, `docs`, `perf`, `test`, etc.). It supports filenames and downstream
  routing and does not prescribe a Jujutsu description prefix.
