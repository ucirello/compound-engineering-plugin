...
2. **Setup Environment**
   ...
   **Bookmark.** Determine the default bookmark from `main@origin` or `trunk@origin`. If that is unavailable, use the host's repository-metadata capability; when the available interface is GitHub CLI, run `GIT_DIR=$(jj git root) gh repo view --json defaultBranchRef`. Otherwise use `main` or `master` when one exists. Read the current bookmark from `jj log -r @ -T 'local_bookmarks' --no-graph`.

   If you are on the default bookmark, have none, or cannot tell, create a feature bookmark named from the plan or work description: `jj new <parent>` then `jj bookmark create <name>`. When a remote default exists, run `jj git fetch` first. Base the new change on the fetched `main@origin` (or `trunk@origin`) when `@` has no commits beyond it; base it on `@` when local commits must stay visible to the idempotency check and the shipping check, or when there is no remote. After the bookmark move, read `jj log -r @ -T 'local_bookmarks' --no-graph` again and treat that result as authoritative. When bookmark safety remains uncertain, use a spare bookmark and never make incremental commits on the real default.
   ...
   **Pre-work scope.** Before editing, record `jj status` (the user's in-progress files) and whether `@` carries commits not on the remote default (`jj log -r 'main@origin..@'` or `trunk@origin..@`; "unknown" without a remote). Nothing in that set is yours to commit or publish. It rides along on the bookmark move untouched: no stash, no question, no effect on naming. Enforce this without asking the user to choose:
   - Incremental commits name only work-owned files and are path-limited, so untouched WIP never enters a commit; the standalone handoff passes every pre-work file this run did not commit as `exclude:<paths>`, and ships locally via `ce-commit` when pre-existing unpushed commits are on the bookmark (`references/shipping-workflow.md`).
