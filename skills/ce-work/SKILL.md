...
2. **Setup Environment**
   ...
   First, inspect the current change, its nearest bookmarks, and the repository's trunk:
   ...
   **If already on a feature change line** (the nearest bookmark is not trunk):
   ...
   **If the current change line is based directly on trunk**, choose how to proceed:
...
4. **Choose Execution Engine, then Strategy**
   ...
   **Shared-workspace constraints** — when subagents share your working directory (no isolation): they must not run `jj describe`, `jj commit`, or the full test suite concurrently (description races + test interference); the orchestrator does all of that after the batch. A worker may run a single focused unit test only if it touches no shared state.

   **Permission mode:** Omit the `mode` parameter when dispatching subagents so the user's configured permission settings apply. Do not pass `mode: "auto"` — it overrides user-level settings like `bypassPermissions`.

   **After each serial unit:** review the diff against the unit's scope and `Files:`, run the relevant tests, fix before dispatching the next (never on a broken tree), record the unit's verification evidence from the worker's return (for the Phase 2 `verification_evidence` roll-up), update the task list (never edit the plan body — progress lives in JJ revisions), and commit. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Derive the description at runtime from the project's active instructions and conventions, then inspect recent history with the repository's preferred `git log` syntax; those sources take precedence. Where compatible, use Go guidance for imperative clarity, a concise subject, a body explaining why when useful, and the repository's wrapping convention. Do not impose a fixed message, prefix, type, scope, subject, body, template, or example. Then dispatch the next unit.
   ...
   **After a parallel batch — the orchestrator integrates; never trust the handoff summary alone:**
   ...
   2. **Inspect the actual tree, not reported paths.** Determine what each worker really changed (`jj status` and `jj diff` in its workspace or the shared directory). Reported paths are a hint; declared `Files:` are often incomplete — workers create/modify files the plan didn't anticipate.
   3. **Detect real collisions** — 2+ workers that actually modified the same file. In a shared workspace only the last writer survived: commit the non-colliding work first, then re-run the colliding units serially so each builds on the other's recorded result. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Derive each description at runtime from the project's active instructions and conventions, then inspect recent history with the repository's preferred `git log` syntax; those sources take precedence. Where compatible, use Go guidance for imperative clarity, a concise subject, a body explaining why when useful, and the repository's wrapping convention. Do not impose a fixed message, prefix, type, scope, subject, body, template, or example. With harness-native isolation the collision surfaces as a JJ conflict during integration instead (see the per-harness note).
   4. **Review, test, and commit each unit in dependency order — the orchestrator owns commits.** Select only that unit's files, preserving its Goal as semantic context for the description, then run the relevant tests and fix before the next. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Derive each description at runtime from the project's active instructions and conventions, then inspect recent history with the repository's preferred `git log` syntax; those sources take precedence. Where compatible, use Go guidance for imperative clarity, a concise subject, a body explaining why when useful, and the repository's wrapping convention. Do not impose a fixed message, prefix, type, scope, subject, body, template, or example. Commit with `jj commit <files> -m "<message composed from the standards above>"`. Capture each worker's returned verification evidence into the run's `verification_evidence` roll-up — if a worker omitted it, re-derive what the tree allows and mark the rest as unverified rather than fabricating a red-before-implementation observation the worker never reported.
   5. Update the task list (progress lives in the JJ revisions).
   6. **Release the workers** — close each worker handle so it stops holding a concurrency slot or leaving orphans (for example, Codex `close_agent`) and let the harness clean up its managed isolation. These isolated workspaces are peers invisible to any outer orchestrator (for example, Orca), so cleanup is entirely ce-work's.
   7. Dispatch the next dependency layer.

   **Per-harness integration (examples — the universal flow above is the contract):**
   - **Claude `Agent` `isolation:"worktree"`:** each worker has isolated changes. Immediately before each harness integration, record the current JJ operation ID from `jj op log -n 1 --no-graph`. Integrate through the harness in dependency order; on conflict, restore the recorded pre-integration state with `jj op restore <pre-integration-operation-id>` and re-run the unit serially against the integrated tree (hand-resolving silently discards one unit's intent).
   - **Codex `spawn_agent` worker:** integrate the worker's "uploaded changes," then `close_agent`.
   - **Cursor `Task` (shared workspace):** edits are already in your tree — review and commit per step 4; **`best-of-n-runner`:** integrate its isolated workspace.
...
2. **Incremental Commits**
   ...
   **Commit workflow:**
   ...
   # 3. Keep the active bookmark at the newest completed revision
   jj bookmark advance <bookmark-name> --to @-
   ```
