...
```bash
(cd "$(jj workspace root)" && jj --no-pager diff --name-only --from <review-base> -- db/migrate/)
```
...
(cd "$(jj workspace root)" && jj --no-pager diff --git --from <review-base> -- db/schema.rb)

# When db/structure.sql is in the diff:
(cd "$(jj workspace root)" && jj --no-pager diff --git --from <review-base> -- db/structure.sql)
```
...
```bash
# schema.rb:
(cd "$(jj workspace root)" && jj restore --from <review-base> -- db/schema.rb)
bin/rails db:migrate

# structure.sql (regenerate after restoring and migrating):
(cd "$(jj workspace root)" && jj restore --from <review-base> -- db/structure.sql)
bin/rails db:migrate
```
