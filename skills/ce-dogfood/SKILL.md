---
name: ce-dogfood
description: "Hands-off, diff-scoped browser QA of the current Jujutsu change stack: maps user flows, drives a real browser, autonomously fixes small breakages with regression tests and changes, judges experience against product personas, and writes a durable dogfood report. Manual invocation only."
...
Act as a QA engineer who dogfoods the **current Jujutsu change stack** end-to-end: understand every change, test every change in a real browser as a user would, and fix what's broken — autonomously — until the stack is genuinely ready.
...
| Recording each fix | `ce-commit` | Finish one logical JJ change and start the next. |
...
5. Fix loop     On failure: fix -> add regression test -> finish JJ change -> continue
...
Parse `$ARGUMENTS`: a PR number, a JJ bookmark/revision, or blank (use `@`). Strip `--port PORT` if present.

1. **Identify the target — keep PR identity; do not move `@` yet.**
   - **PR number:** the target *is the PR*. Carry the number through every later step and read its source and base with `GIT_DIR="$(jj git root)" gh pr view`; retain the source revision ID and provider repository/bookmark metadata because a fork PR's source bookmark can have any name. Do not reduce the target to a bare bookmark or move `@` yet.
   - **Bookmark/revision:** resolve it with `jj log -r <target>` and retain the resulting revision.
   - **Blank:** the target is the current working-copy change, `@`.
2. **Refuse to run on the trunk for revision/blank targets.** If the target resolves within `trunk()`, stop because there is no stack to dogfood. A **PR is always diffable** against its declared base, so this check never applies to a PR target.
3. **Decide isolation by what you're testing; let `ce-worktree` own JJ workspace mechanics.** Do not re-derive workspace detection or creation here. The only decision this skill makes is whether to ask for isolation:
   - **Blank / current-change target:** do **not** isolate. Dogfood in place; the fixes belong in the current stack.
   - **A PR or a different bookmark/revision:** offer isolation with the platform's blocking question tool. On **yes**, invoke `ce-worktree` for that exact target and act on its verdict. On **no**, stay in the current JJ workspace: make the target revision available with `jj git fetch --remote <remote>` when needed (for a PR, resolve `<remote>` from its `gh` provider repository/bookmark metadata, including fork metadata), then run `jj new <target-revision>` to create an empty working-copy change on top of it. Do not switch revisions with a non-JJ command; JJ preserves the previous working-copy change when moving to the new one.
...
- **The report doc** at `docs/dogfood-reports/<YYYY-MM-DD>-<target-slug>-dogfood.md` is the durable checkpoint that survives across sessions. `<target-slug>` is the PR number, bookmark, or short change ID lowercased with every run of non-alphanumeric characters collapsed to a single `-`. **Create it as soon as the matrix exists (end of Phase 2) by instantiating `references/dogfood-report-template.md`** (read that template now if you haven't) so the checkpoint carries the template-owned section shape from the start — then fill in every scenario at `Pending`, and **update it incrementally** — after each scenario is judged and after each fix change is finished — not only at the end. An interrupted run must leave a template-shaped checkpoint, not a bare matrix.
...
Resolve the comparison base once, then read the full aggregate diff from that base through the target stack. For a PR, use its declared base remote bookmark; otherwise use JJ's `trunk()` revset. Do not hard-code a default bookmark name.

```bash
# For a non-PR target. Set TARGET to @ after entering the target workspace.
BASE='trunk()'
TARGET='@'
jj diff --from "$BASE" --to "$TARGET" --name-only
jj diff --from "$BASE" --to "$TARGET"
```

For a PR, fetch its base with `jj git fetch --remote <remote> --branch <base>`, set `BASE` to the exact `<base>@<remote>` bookmark selected from the PR metadata, and keep `TARGET=@` after entering the target workspace. If `trunk()` or either endpoint is ambiguous, stop and resolve it explicitly with `jj log`; do not guess.
...
2. **Drive it** with agent-browser — navigate, snapshot for interactive refs, click, fill, submit, follow the journey to its real end state:
   ...
   ROOT=$(jj workspace root 2>/dev/null || pwd); SCRATCH="$ROOT/.tmp"; mkdir -p "$SCRATCH"; agent-browser screenshot "$SCRATCH/<scenario>.png"
   agent-browser errors      # check console/page errors
   ```

   Write transient screenshots under `$(jj workspace root)/.tmp`; if no JJ workspace root is available, use the local `$(pwd)/.tmp` fallback. Before writing, ensure the applicable `.tmp/` is excluded from deliverable content; in a JJ workspace, add it to the root `.gitignore` if the root ignore rules do not already cover it. Never use OS-global temporary storage or APIs. Only copy a screenshot into the report's location if you intend to embed it in the final report.
