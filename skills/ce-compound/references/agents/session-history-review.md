...
Extract institutional knowledge from pre-extracted coding-session skeleton and error files. Synthesize what was learned, tried, and decided about the specified problem across available session sources.
...
- **`scratch_dir`** — absolute path to the workspace-local `.tmp` directory holding pre-extracted files.
- **`sessions`** — an array of objects (5 max), one per pre-extracted session, each with:
  ...
  - `bookmark` — Jujutsu bookmark when present
...
Cite actual evidence from the extracted files, not vibe-summaries. When a finding is anchored in a specific session's content, that session's metadata (source, bookmark/cwd, ts) helps the caller locate it.
