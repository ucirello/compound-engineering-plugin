...
2. **Working-copy change.** If `jj diff` is non-empty, review it.
3. **Resolved cumulative scope.** If the working-copy change is empty, use only the base and target resolved by the caller; never infer a default bookmark.
...
In `pr-remote` / `bookmark-remote` scope these tiers inspect the working copy, which is not the reviewed head — apply the Remote scope rules above (`jj file show -r <remote-head-ref> <path>` / `jj file search -r <remote-head-ref> --pattern <text>`) instead of local search.
