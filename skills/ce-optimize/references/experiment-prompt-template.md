...
Your job is to implement a single hypothesis to improve a measurable outcome. You will modify code within a defined scope, then stop. You do NOT run the measurement harness, describe or integrate JJ changes, or evaluate results -- the orchestrator handles all of that.
...
5. Do NOT describe or integrate the JJ change (the orchestrator handles the winning diff if this experiment succeeds)
...
- For Codex dispatch: write the filled template to a temp file and pipe via stdin from the repo root (`cat /tmp/optimize-exp-XXXXX.txt | codex exec - 2>&1`).
