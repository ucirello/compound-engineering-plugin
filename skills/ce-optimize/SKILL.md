...
The files under `.context/ce-optimize/<spec-name>/` are local scratch state. They survive local resumes on the same machine but are not preserved by Jujutsu changes, bookmarks, or remote pushes unless the user exports them separately.
...
| `<workspace>/.tmp/rocketclaw/ce-optimize/result.yaml` | Per-experiment crash-recovery marker | Immediately after measurement, before CP-3 |
...
4. Delete markers already represented in the verified experiment log
...
Confirm the project root and inspect optimization bookmarks and recent changes:

```bash
jj workspace root
jj bookmark list --all-remotes "optimize/<spec-name>"
jj log -r 'optimize/<spec-name>' --limit 10
```
...
- **Fresh start**: create `optimize-archive/<spec-name>/archived-<timestamp>` at the old bookmark target with `jj bookmark create <archive-bookmark> -r "optimize/<spec-name>"`, delete the old optimization bookmark with `jj bookmark delete "optimize/<spec-name>"`, clear the experiment log, and start from the current base revision

### 0.5 Create Optimization Change, Bookmark, and Scratch Space

For a fresh run, create an empty optimization change on the selected base, name it with a bookmark, and create a working change above it:
...
For a resumed run, create only the new working change with `jj new "optimize/<spec-name>"`.

When remote state may have changed, run `jj git fetch --remote <remote>` before selecting the base. Use `jj bookmark move "optimize/<spec-name>" --to @` when advancing an existing bookmark.
...
Inspect the working change and verify it has no modifications within `scope.mutable` or `scope.immutable`:
...
Filter the changed paths against the scope paths. If any in-scope files are already modified:
- Report which files overlap
- Ask the user to move that work into a separate change with `jj new`, `jj split`, or `jj squash`, as appropriate
- Do NOT continue until the active change is clean for in-scope files

Also confirm the repository ignores `.tmp/` before creating experiment workspaces. If `.tmp/` would be tracked, stop and ask the user to add the repository-local ignore rule; do not create storage outside the project as a fallback.
...
**Workspace backend:**
1. Create an isolated experiment workspace and change from the current optimization bookmark:
   ```bash
   SKILL_DIR="<absolute path of the directory containing this SKILL.md>";
...
3. Write the filled prompt under the current Jujutsu workspace's `.tmp/rocketclaw/ce-optimize/prompts/` directory; if `jj workspace root` reports that this is not a Jujutsu repository, use `.tmp/rocketclaw/ce-optimize/prompts/` under the current project directory
4. Dispatch via Codex:
   ```bash
   ROOT=$(jj workspace root 2>/dev/null || pwd); PROMPT_DIR="$ROOT/.tmp/rocketclaw/ce-optimize/prompts"; mkdir -p "$PROMPT_DIR"; PROMPT_FILE="$PROMPT_DIR/optimize-exp-<iteration>.txt"; codex exec - < "$PROMPT_FILE"
   ```
...
1. **Snapshot and inspect the experiment change**, then run measurement in its workspace:
   ...
   jj -R "<workspace_path>" diff --name-only
...
2. **Write crash-recovery marker** — immediately after measurement, safely create `<workspace_path>/.tmp/rocketclaw/ce-optimize/` and write `result.yaml` there with the raw metrics. This ensures the measurement is recoverable even if the agent crashes before updating the main log without adding the marker to the experiment change.
...
9. **Delete the crash-recovery marker** after CP-3 verification and confirm it is absent. The verified experiment log now owns the result; retaining the marker would block safe workspace cleanup.
...
3. **If best improves on current best: KEEP**
   - Run `jj status` and `jj diff --name-only -r <experiment-bookmark>`; use `jj split -r <experiment-bookmark>` when needed so the accepted change contains only mutable-scope modifications. If no eligible diff remains, treat the experiment as non-improving and abandon it.
   - Before composing or editing the accepted change description, follow this rule exactly: Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards.
   - Repository-local instructions and the syntax observed through that history command always win; apply compatible Go quality guidance without imposing a fixed prefix, type, scope, template, or example.
   - Describe the accepted experiment with `jj describe -r <experiment-bookmark> -m "<change-description-derived-from-current-repository-standards>"`.
   - Rebase it onto the current optimization bookmark with `jj rebase -r <experiment-bookmark> -d "optimize/<spec-name>"`, then advance the optimization bookmark with `jj bookmark move "optimize/<spec-name>" --to <experiment-bookmark>`.
   - After the bookmark advances, clean up the winner's experiment workspace and experiment bookmark; the accepted change referenced by the optimization bookmark is the durable artifact.
   - This is now the new baseline for subsequent batches.

4. **Check file-disjoint runners-up** (up to `max_runner_up_combinations_per_batch`):
   ...
   - If disjoint: rebase through the runner-up's owning workspace with `jj -R "<runner-up-workspace>" rebase -r <runner-up-bookmark> -d "optimize/<spec-name>"`, then re-run the full measurement there. If the rebase was issued from another workspace, run `jj -R "<runner-up-workspace>" workspace update-stale` immediately before remeasurement.
   - If combined measurement is strictly better: advance the optimization bookmark to the runner-up change (outcome: `runner_up_kept`), then clean up that runner-up's workspace and experiment bookmark
   - Otherwise: abandon the rebased runner-up with `jj abandon <runner-up-bookmark>`, log it as "promising alone but neutral/harmful in combination" (outcome: `runner_up_reverted`), then clean up its workspace and experiment bookmark
...
6. **Revert all others**: abandon their experiment changes, clean up their workspaces and bookmarks, and log them as `reverted`
...
Total experiments: <count>
  Kept: <count> (including <runner_up_kept_count> runner-up combinations)
  Reverted: <count>
  Degenerate: <count>
  Errors: <count>
  Deferred: <count>
...
The optimization bookmark (`optimize/<spec-name>`) is preserved at the accepted stack of changes.
The experiment log and strategy digest remain in local `.context/...` scratch space for resume and audit on this machine only; they do not travel with the bookmark.
...
1. **Run `ce-code-review`** on the cumulative diff (baseline to final). Load the `ce-code-review` skill on the optimization bookmark (interactive or `mode:agent`). To land eligible fixes before the next option, apply the mechanical-apply bar below.
   ...
   **Mechanical-apply bar:** apply any finding with a concrete `suggested_fix` that is a clear, reversible improvement; push back (keep, don't apply) when the reviewer is wrong, noting why. Defer anything whose right fix needs a design or product decision (architecture direction, contract shape, behavior change needing sign-off) and any finding with no concrete fix to act on — surface what was deferred. Confirm evidence still matches at `file:line` before editing. After applying, run tests (at least targeted tests for what changed; broader suite for multi-file edits). Do not advance or push the bookmark from this step — leave the diff in the active change for the Create PR option.
2. **Run `ce-compound`** to document the winning strategy as an institutional learning.
3. **Create PR** from the optimization bookmark to the default bookmark. Move the bookmark to the final reviewed change, run `jj git push --bookmark "optimize/<spec-name>"`, then use `gh` to open the PR.
