...
This describes any remaining change, creates or moves the required bookmark, pushes with `jj git push`, and opens a pull request non-interactively. Preserve the semantic record of any newly introduced concepts in the location and syntax the repository's current conventions require. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Back-fill the PR URL into residual tickets best-effort. If `GIT_DIR="$(jj git root)" gh pr view --json number,url,state` finds an existing PR for the pushed bookmark, skip creation but still finish and push remaining revisions.
