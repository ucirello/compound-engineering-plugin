...
2. **Working copy changes.** If the working copy differs from its parents (`jj diff` is non-empty), review those.
3. **Unpushed changes vs base bookmark.** If the working copy matches its parents, review `jj diff --from <merge-base> --to @` where `<merge-base>` is `jj log -r 'heads(::@ & ::<base>)' -T commit_id --no-graph` and `<base>` is the default bookmark (main or master).
