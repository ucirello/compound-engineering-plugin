...
If no PR number was provided, detect from the current bookmark/working copy:
```bash
...
1. Check that the current change contains only files reported by fixers, then describe it with a message referencing the PR:
