...
Run this **first** when `db/schema.rb` or `db/structure.sql` appears in the diff. Use the review base ref from caller context (`<review-base>` — ancestor rev or ref). **Never assume `main`.**
...
```bash
# schema.rb:
jj restore --from <review-base> --to @ db/schema.rb
bin/rails db:migrate

# structure.sql (regenerate after restoring and migrating):
jj restore --from <review-base> --to @ db/structure.sql
bin/rails db:migrate
```
