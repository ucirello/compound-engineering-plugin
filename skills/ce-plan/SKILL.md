...
- **Publish to Proof — shareable link** — Load the `ce-proof` skill to publish the plan: create a shared Proof doc from the plan file (title = plan title), surface the share URL to the user, then return to this menu. Do not add an attribution badge, byline, generated-by line, or standalone workflow branding. One-way publish — the local plan file stays canonical, nothing syncs back. If the upload fails, see the graceful-fallback note in `references/plan-handoff.md`.
