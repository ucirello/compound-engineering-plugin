...
does not work; it prints top-level help.) Pair every `gh` call with `GIT_DIR` set to the path from
a prior `jj git root` call (fill the path; do not nest `$(...)`). After any `gh stack` command that
changes Git refs, run `jj git import` before inspecting or mutating JJ state.
...
```bash
GIT_DIR="<path from prior jj git root>" gh stack checkout "<parent-pr-number>"
```
...
| 6 | Disambiguation required | Bookmark is in several stacks — select a non-shared bookmark |
| 9 | Stacked PRs unavailable | Not enabled on this repository; tell the user and stop |

```bash
GIT_DIR="<path from prior jj git root>" gh stack view --json    # JSON on stdout: trunk, currentBranch,
...
the stack and no documented bookmark ordering, so do not derive position from this payload — use
...
bookmark. After `jj git fetch`, create a local bookmark at `headRefOid` only when that revision is
resolvable and the name does not collide.

## Building

```bash
GIT_DIR="<path from prior jj git root>" gh stack init [--base "<trunk>"] "<bookmark>"...
```
...
```bash
GIT_DIR="<path from prior jj git root>" gh stack add "<bookmark>"
```
...
touch the working-copy change, so the current JJ change remains present.
