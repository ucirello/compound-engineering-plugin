...
implementation, and any decisions made along the way land in JJ history or
...
- **`title`** — the plan's descriptive name, matching the H1 (markdown) or
  document `<h1>` (HTML) so file metadata and visible heading don't drift.
  Stable across readiness states. Follow the repository's existing plan-title
  syntax; otherwise use a concise descriptive title without inventing a prefix.
- **`type`** — the repository-derived plan classification. Infer it from the
  project's active instructions and conventions, recent history, existing plan
  metadata, and the work's semantic intent; do not impose a fixed prefix, type,
  or example.
...
- **`execution`** — execution domain for downstream routing: `code`
  ...
  skipping the JJ change/bookmark/test/CI lifecycle); absent or `code` routes
...
- **No process exhaust.** No "captured at Phase X" notes, no `## Next Steps`
  ...
  metadata belongs in JJ history and tool output, not the artifact.
