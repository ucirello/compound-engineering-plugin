...
- **Bookmark**: run `jj bookmark list -r @ -T 'name ++ "\n"'` with cwd at the workspace root. Use the bookmark name to filter session history in Phase 1. If it prints nothing (no bookmark on `@`) or exits non-zero, skip bookmark filtering.
- **Repo root**: run `jj workspace root`. Use it as the session-history repo filter in Phase 1. If it exits non-zero, fall back to the working directory.
...
#### 4. **Session History** (internal flow after launching the parallel block; automatic in Full mode, including non-interactive)
   ...
   **Extraction pipeline.** Create a per-run scratch directory under the workspace `.tmp` tree: `WORKSPACE_ROOT=$(jj workspace root 2>/dev/null || pwd); mkdir -p "$WORKSPACE_ROOT/.tmp/rocketclaw"; SCRATCH=$(mktemp -d "$WORKSPACE_ROOT/.tmp/rocketclaw/ce-compound-sessions-XXXXXX")`. For each selected session, write extracted content to scratch files:
