...
2. **Working-copy changes.** If `jj st` shows changes in the current change, review those.
3. **Current stack vs base bookmark.** If the working copy is clean, review `jj diff --git --from <base>` where `<base>` is the resolved default branch/bookmark.
...
- Prefer `jj file show -r <remote-head-revset> <path>` when a remote head revset is provided in context.
