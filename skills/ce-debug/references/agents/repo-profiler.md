...
- **Conventions & instruction files** — paths and a short digest of the root agent instructions and project docs such as `ARCHITECTURE.md`, `README.md`, `CONTRIBUTING.md`, and `STRATEGY.md`: coding standards, testing conventions, review process, and (from `STRATEGY.md`) the target problem/approach/active tracks. These are project-wide conventions, so they belong in the cached profile (unlike *subdirectory-scoped* instruction files, which stay fresh per the exclusion below).
