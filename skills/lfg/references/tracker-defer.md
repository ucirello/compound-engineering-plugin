...
For every `gh` invocation below, use the absolute JJ workspace root as cwd and set `GIT_DIR` from `jj git root` in that workspace. Use an explicit bookmark or PR URL when selecting a PR, checking the completed parent `@-` when `@` is empty rather than inferring Git HEAD.
...
- **Body:**
  ...
  - Source: a link to the PR carrying this change when one already exists at filing time; otherwise the bookmark and finalized commit ID, so the ticket points at the code even before a PR is opened. When the same run opens a PR after the ticket is filed, back-fill the PR link into the ticket (best-effort; never block shipping on the update).
