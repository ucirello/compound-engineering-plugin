...
The host resolves and sanctions one concrete route before egress; `scripts/cross-model-adversarial-review.sh` enforces that fixed route, applies read-only controls, captures schema-shaped JSON, and records identity receipts. Before dispatch it conservatively estimates diff tokens and file count. Oversized diffs are not inlined: the worker gives the peer the orchestrator's compact semantic review map and keeps the exact diff as a private, selectively readable artifact under workspace-local `.tmp`. Tool-limited routes receive that directory as an additional read root; Codex uses selective `jj diff --git --from <base> -- <path>` calls under its existing read-only sandbox. A failed route writes no artifact and never switches recipients internally.
...
2. Scope is `local-aligned` or standalone: the working copy is the reviewed head. Skip in `pr-remote` / `branch-remote`, where the local working copy is not the PR/bookmark head.
...
The nested windows are one budget with one knob, `CROSS_MODEL_HARD_SECS`. The runner derives its supervisor hard window from that ambient knob automatically (`max(1230, knob + 30)`). Clear `ROCKETCLAW_PEER_HARD_SECS` on the start prefix so a stale ambient value cannot undercut that derivation; an explicit numeric value still wins when another workflow deliberately sets one. Print the orchestrator deadline as `knob + 10` in the same shell as `start` and use that printed `peer-deadline-secs=<n>` below.
...
- `<base-ref>` = the Stage 1 `BASE` (the diff base the peer reviews via `jj diff --git --from <base-ref>`).
...
- **Dispatch-infrastructure failure** — the runner or worker itself crashed: a non-zero exit before any job starts, a preflight/detach failure, or an unresolved `$SKILL_DIR`/script path. This is distinct from the gate-not-met skips above. A **no-job-id** preflight failure is recovered entirely at **Stage 3d's no-job branch**, before the local roster is materialized. Do not re-attempt that case at fold-in. This step handles only a **job-id-returned-then-failed** crash. Re-run the same resolved fixed route by hand while holding the target, model, `jj diff --git --from <base-ref>` read scope, and adversarial persona brief fixed. Stop once a failure repeats or the deadline is spent; report the adversarial lens as degraded. Recovery may not substitute a target or provider, widen read scope, or relax the read-only boundary.
...
The peer reviews the **current working copy** (read-only) against `jj diff --git --from <base-ref>`. Reviewed code/diff content is sent to an external model provider (OpenAI, Anthropic, xAI, or Cursor, depending on the resolved peer). `CROSS_MODEL_PEERS` restricts which providers may receive content.
...
- **codex:** `-s read-only` with cwd at the workspace root (may run `jj diff` itself).
