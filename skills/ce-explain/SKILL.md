...
RUN_DIR="$ROOT/.tmp/rocketclaw/explain/$(date +%Y%m%d)-$(openssl rand -hex 3)"
mkdir -p "$RUN_DIR"
echo "$RUN_DIR"
```
...
- **Diff mode:** resolve the JJ change or revision (the `diff:` ref, or the most recent substantial change when the request points at one implicitly) and gather its evidence with `jj diff`/`jj show` — the diff itself, the files it touches, any plan or solution doc that motivated it. Keep GitHub PR lookup and any necessary `jj git` interoperability when the reference requires them. Gather silently: nothing learned here is narrated to the user until Phase 3's ordering rule is satisfied.
