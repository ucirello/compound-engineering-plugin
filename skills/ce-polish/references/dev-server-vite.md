...
Vite binds to `127.0.0.1` by default. When this workflow runs inside a devcontainer or WSL, users may need `--host 0.0.0.0` in `runtimeArgs`.
...
- **HMR websocket port:** Vite's HMR uses a separate websocket that inherits the dev-server port by default. If the project pins `server.hmr.port` in `vite.config.ts`, the reachability probe against the dev-server port still works, but the embedded browser may need additional configuration to reach HMR.
