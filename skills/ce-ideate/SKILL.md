...
This workflow produces a ranked ideation artifact — written to `docs/ideation/` when present, else workspace-local scratch (see Phase 4). It does **not** produce requirements, plans, or code.
...
**Pre-resolve the scratch directory path.** Scratch lives under the JJ workspace's `.tmp/`, falling back to local `.tmp/` when the current directory is not a JJ repo. The orchestrator owns ignore handling; do not edit `.gitignore`. Run one bash command to create the directory and capture its absolute path for downstream use.
