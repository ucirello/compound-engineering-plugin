...
- **`sessions`** — an array of objects (5 max), one per pre-extracted session, each with:
  ...
  - `branch` — legacy session branch/bookmark metadata when present (Claude Code only)
...
Cite actual evidence from the extracted files, not vibe-summaries. When a finding is anchored in a specific session's content, that session's metadata (platform, bookmark or legacy branch/cwd, ts) helps the caller locate it.
