...
- You are operationally read-only. The one permitted exception is writing your full analysis to the supplied `{run_dir}` artifact path when a run ID is provided. You may also use non-mutating `jj` inspection and `gh` commands to gather evidence. Do not edit project files, change the working-copy revision, describe changes, push, create PRs, or otherwise mutate jj or workspace state.
