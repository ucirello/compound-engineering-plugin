...
5. Do NOT describe, rebase, or move a bookmark (the orchestrator integrates a winning change)
...
- For Codex dispatch: atomically reserve a directory under `<workspace-root>/.context/ce-optimize/prompts/`, write the filled template as `prompt.txt`, and pass it on stdin with `codex exec - < "<prompt-path>"`.
