...
Run this **first** when `db/schema.rb` or `db/structure.sql` appears in the diff. Use the review base revision from caller context (`<review-base>`). **Never assume `main`.** For local and bookmark-remote scope, compare it to the reviewed Jujutsu revision. For PR-remote scope, use the supplied GitHub patch because the PR revision is intentionally not imported into the workspace.

```bash
jj diff --from <review-base> --to <reviewed-revision> --name-only db/migrate
```
...
jj diff --from <review-base> --to <reviewed-revision> --git db/schema.rb

# When db/structure.sql is in the diff:
jj diff --from <review-base> --to <reviewed-revision> --git db/structure.sql
```
