...
Resolve the workspace root with `jj workspace root --ignore-working-copy`, then read `<workspace-root>/.rocketclaw/config.local.yaml` with the native file-read capability. If the workspace root or file is unavailable, treat the config as absent.
...
Use the active harness's blocking-question capability. If no blocking capability exists or the call errors, present the same options as a numbered list in chat and wait for a reply — never silently skip.
...
- **Sign in to Spiral** (installed, unauthed) — the AI Assistant runs `spiral login --json` itself. It's non-blocking, and the **API key never touches the AI Assistant** (the token is exchanged server->CLI via a device-code flow). Parse the JSON `status`:
  ...
  - **Never have the user paste an API key into chat** — with assisted login the AI Assistant never handles the key at all.
  - **Older CLI (< 1.8.0, no assisted login):** if `spiral login --json` returns the legacy `API key required ... --token` text instead of JSON, suggest `npm i -g @every-env/spiral-cli@latest`, or have the user run `spiral login` themselves in their terminal (browser sign-in) and re-check `spiral auth status`. If they would rather not, go to Path B.
- **Install Spiral** (absent) — the pairing-code command installs and connects in one step. Direct the user to the connection settings at https://app.writewithspiral.com to copy their command, which looks like:
  ```bash
...
Resolve the workspace root with `jj workspace root --ignore-working-copy`, then add `ce_promote_spiral_optout: true` as a top-level key to `<workspace-root>/.rocketclaw/config.local.yaml`, using the native file-write/edit capability:

- **Before any write:** resolve the underlying repository metadata directory with `jj git root --ignore-working-copy` and ensure `.rocketclaw/*.local.yaml` is present in `<metadata-directory>/info/exclude`. Then use `jj file list '.rocketclaw/config.local.yaml' --ignore-working-copy` to confirm the config is not already tracked. If the metadata directory or local exclude cannot be resolved safely, or if the config is tracked, do not modify the config; continue to Path B without recording the opt-out. The local exclude keeps this machine-local setting outside Jujutsu tracking without changing a project-wide ignore file. `ce-setup` remains the canonical place for shared teammate setup.
...
- **File absent:** create the `.rocketclaw/` directory under the resolved workspace root, then create the config file with the key.

If the root can't be resolved or any write fails, proceed to Path B anyway; the opt-out is a convenience, never a blocker.

After recording, confirm it in one line so the write isn't silent and the user knows how to undo it — e.g. "Got it — I won't bring up Spiral here again (saved to `.rocketclaw/config.local.yaml`, kept outside Jujutsu tracking). Want it back later? Just ask, or remove the `ce_promote_spiral_optout` key." Keep it to a single line; don't belabor it.
