...
Every write to a Proof doc requires two identity fields:

- **Machine ID (`by` on every op, `X-Agent-Id` header):** `ai:assistant` - stable, lowercase, and machine-parseable.
- **Display name (`name` on `POST /presence`):** `AI Assistant` - human-readable and shown in Proof's presence UI.

Set the display name once per doc session by posting to presence with the `X-Agent-Id` header; Proof binds the name to that agent ID for the session. These neutral values are the defaults; a caller may pass a different neutral `identity` pair when the protocol needs distinct actors for concurrent operations.
...
`state.marks` is a union of comments, suggestions, and document-history marks. The `?kinds=comment` filter avoids treating document-history marks as review comments.
...
- Header: `X-Agent-Id: ai:assistant` (required for presence; include on ops for consistent identity)
...
`status: "accepted"` creates the suggestion mark and applies the change in one call. The mark persists in document history with a reject-to-revert affordance. Works with `kind: "insert" | "delete" | "replace"`.
...
# Keep scratch in the JJ workspace, with a project-local fallback.
WORKSPACE_ROOT=$(jj workspace root 2>/dev/null || pwd)
TMP_ROOT="$WORKSPACE_ROOT/.tmp"
mkdir -p "$TMP_ROOT"
RUN_TMP="$TMP_ROOT/proof-sync-$$"
mkdir -m 700 "$RUN_TMP"
trap 'rm -r "$RUN_TMP"' EXIT
STATE_TMP="$RUN_TMP/state.json"
curl -s "https://www.proofeditor.ai/api/agent/$SLUG/state" \
  -H "x-share-token: $TOKEN" > "$STATE_TMP"
REVISION=$(jq -r '.revision' "$STATE_TMP")

# Stream .markdown bytes through workspace-local scratch, then replace the local file.
LOCAL_TMP="$RUN_TMP/local.md"
jq -jr '.markdown' "$STATE_TMP" > "$LOCAL_TMP" && mv "$LOCAL_TMP" "$LOCAL"
```

`jq -jr` (`-j` no trailing newline, `-r` raw string) streams the markdown bytes straight to workspace-local scratch without going through a shell variable, so trailing newlines survive intact. For the usual workspace-local destination, `mv` stays on the same filesystem and replaces the file atomically.
...
- Always include `by: "ai:assistant"` on every op and `X-Agent-Id: ai:assistant` in headers for consistent protocol identity
