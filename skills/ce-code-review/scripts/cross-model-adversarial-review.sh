...
#              only `jj diff <base-ref>` in the current repository
...
# the user's project on every host), and derives the repo root from Jujutsu. The agent
...
# embedded and needs no Jujutsu.
if [ "$PEER" = codex ]; then
  printf '\nRun: jj diff %q — review ONLY the changes in that diff, in this repository (read-only).\n' "$BASE" >> "$PROMPT_FILE"
else
  { printf '\nReview ONLY the change below (the output of `jj diff %q`). You may Read repository files for context but cannot run shell commands.\n' "$BASE"
    printf '\n=== BEGIN DIFF ===\n'; jj -R "$REPO_ROOT" diff "$BASE"; printf '\n=== END DIFF ===\n'; } >> "$PROMPT_FILE"
fi
