...
3. **Check whether the issue already appears fixed on the default bookmark.** Once you know the affected surface, use `jj log` revsets and filesets plus read-only `GIT_DIR="$(jj git root)" gh` operations on that area (files, routes, components the symptom touches) to see whether a recent change or merged PR already addresses it. Use `trunk()` for the configured default bookmark and narrow history with `files(<fileset>)` when the affected paths are known. Report this as a field in your finding so the orchestrator does not re-file resolved work.
...
- **Already fixed on the default bookmark?** -- `yes` / `no` / `unclear`, with the change ID, commit ID, or PR reference you checked, or a note that you could not determine it.
...
- You are read-only except for the ONE write to your scratch artifact path. Read-oriented `jj` / `GIT_DIR="$(jj git root)" gh` and running the bundled analyzer are permitted; do not edit project files, rewrite changes, move bookmarks, push, or open PRs.
- Do not invoke other skills or assistants. Do your analysis directly and return in the format above.
