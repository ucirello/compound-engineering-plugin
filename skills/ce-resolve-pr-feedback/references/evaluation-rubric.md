...
- **A finding against text a prior round of this PR added is a representation signal.** Check `jj log` over the PR bookmark's range for earlier review-fix changes touching the same block. On the second round against the same block, stop patching: fold every finding on that block into one class item with all feedback IDs and comment text, then instruct one restatement of the goal, done condition, and safe direction that still serves every prior path. Every covered thread is replied to and resolved from that result.
...
- **Recover deliberate intent before overriding deliberate-looking code.** Inspect `jj log`, `jj file annotate`, the PR description, and surrounding code. Weigh concrete rationale against the finding rather than assuming the reviewer saw more.
