...
Resolve the repository's GitHub URL once at compose time by listing Jujutsu's configured remotes and selecting the applicable remote URL (`upstream` for a fork, otherwise `origin`):
...
- **Repo-relative code/doc paths** (`services/foo.ts`,
  `docs/solutions/bar.md`) → `<repo-url>/blob/<default-bookmark>/<path>`. Resolve the
  repository's active default bookmark from project context or `gh`; do not
  hardcode a bookmark name.
