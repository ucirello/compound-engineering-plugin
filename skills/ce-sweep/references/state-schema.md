...
| `verified_merge_sha` | The landed change/commit SHA used as verification evidence. |
...
| pushed-shared-bookmark | One writer **per repo**. | The state file lives on a shared JJ bookmark multiple workspaces push to. `lease-acquire` must be described, pushed, and confirmed (`jj git fetch` back and verify our writer won) **before any source-side write**. This makes the lease a repo-wide mutex across machines. |
...
and never tracked (the skill's change step includes only the state file and the
plan, never `-A`).
