...
# No geteuid on Windows; the current-user SID is the ownership identity. Every
# platform keeps the default under the current workspace rather than OS-global
# temporary storage.
...
def write_atomic(path: str, data: bytes) -> None:
    ...
    for _ in range(CLAIM_ATTEMPTS):
        ...
        try:
            fd = os.open(
                tmp,
                os.O_RDWR | os.O_CREAT | os.O_EXCL | O_NOFOLLOW | O_BINARY,
                0o600,
            )
            break
        except FileExistsError:
            continue
    if fd is None:
        raise OSError(f"could not reserve an atomic temporary file for {path}")
    try:
...
def _require_detach_support() -> None:
    ...
    if not hasattr(os, "fork") or not hasattr(os, "setsid"):
        raise RunnerError(
            ...
            "the native Windows detach compatibility guidance)."
        )
