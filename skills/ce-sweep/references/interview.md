...
- **Machine-local under workspace `.tmp`** (solo setups; keeps sweep bookkeeping out of versioned files, no change-description noise). Resolve the path immediately with this shell block, substituting a sanitized repository slug:
...
**Ask:** "Is this a multi-agent setup where several checkouts push the sweep state to a shared docs bookmark? Answer yes only if more than one machine or agent describes a change and `jj git push`es to the same bookmark. Default is no — a single checkout describing locally."

- **No** (default) -> `sweep_shared_branch: false`. The single-writer lease serializes overlapping sweeps within one checkout.
- **Yes** -> `sweep_shared_branch: true`. Explain: the lease becomes **push-gated** — before any source-side write, the sweep describes the lease acquisition, `jj git push`es it on the shared bookmark, and confirms its writer won, making the lease a repo-wide mutex across machines.
