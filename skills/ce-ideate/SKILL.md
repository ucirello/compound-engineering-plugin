...
This workflow produces a ranked ideation artifact — written to `docs/ideation/` when present, else a workspace-local temporary path (see Phase 4). It does **not** produce requirements, plans, or code.
...
**Pre-resolve the scratch directory path.** Scratch lives under the current JJ workspace's `.tmp/rocketclaw` directory. When no JJ workspace can be resolved, use a local `.tmp/rocketclaw` directory under the current working directory. Run one bash command to create the directory and capture its absolute path for downstream use.
