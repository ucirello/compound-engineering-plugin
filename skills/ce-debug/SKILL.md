...
As you trace:
- Check recent changes in files you are reading: `jj log -n 10 -- <file>`; use `jj file annotate <file>` when line-level provenance matters
...
- The configured JJ Git remotes from `jj git remote list` (a GitHub remote implies GitHub Issues + PRs; `gh` if available). Run `jj git fetch` first when remote state must be current.
...
- Any related ticket or PR surfaced in Phase 1.4 — an open duplicate, an existing fix under another bookmark or open PR, a regression's original fix, or a prior merged attempt that failed — and how it shapes the recommendation. If an open PR already fixes this, lead with that link instead of a fresh fix; if a prior merged attempt took the same approach you were about to, say so and explain what that rules out.
...
- Resolve the workspace with `jj workspace root` and inspect `jj status` plus `jj diff --summary`. JJ snapshots working-copy edits automatically and has no staging area. If the user has in-progress work in files that need modification, confirm before editing — do not overwrite it.
...
For every JJ change description or commit message composed in this phase: Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. The project's active runtime instructions and the commit-message style visible in actual `git log` take precedence over the Go guidance. Do not impose a fixed syntax, message, prefix, type, scope, subject/body shape, template, or example.
...
3. **Run `/ce-commit-push-pr`.** When the entry came from an issue tracker, preserve the required issue-closing linkage in the location that tracker requires, without prescribing a fixed form here. Ensure the functional handoff uses the existing JJ change and bookmark, `jj git push`, and the forge PR flow. Surface the resulting PR URL.
...
2. **Describe the local JJ change (`/ce-commit`)** — no push.
...
When offering, use the blocking question tool described above. If the user accepts, run `/ce-compound`, include the resulting learning doc in a new JJ change on the same bookmark, and push it with `jj git push` so the open PR updates. The phase-wide change-description guidance above applies to this new change.
