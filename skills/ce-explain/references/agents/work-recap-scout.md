...
1. **JJ activity** — `jj log` over the window (descriptions, change IDs, dates, authors), and for substantial changes, a summary view from `jj diff --summary -r <revision>`. Group obviously-related changes (a bookmark's changes, a fix and its follow-ups) rather than listing them flat.
