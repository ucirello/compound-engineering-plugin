...
> Diff-scoped browser QA of `<target-revision>` vs `<trunk-revision>` on <YYYY-MM-DD>.
...
<What changed between the target stack and trunk: new features, modified behavior, new/changed routes, views, components, data flows. 2-6 bullets.>
...
|---|------|--------------------|--------|-------|-----|--------------------|
| <row> | <flow> | <scenario> | <status> | <issue-or-none> | <fix-or-none> | <change-id-and-commit-id-or-none> |
...
- **<Persona>** — <paper cut> — <severity> — <fixed in `<change-id>` / deferred>
...
<Overall readiness verdict for the target stack. Ready to ship? Caveats? Outstanding blocked items? Record the result of the Phase 5 automated test suite run — a green matrix with a red suite is not "ready.">
