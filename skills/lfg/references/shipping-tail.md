...
The goal is the remaining work finalized as described JJ changes, published through a bookmark, and in an open PR whose URL you hold.

Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. The project's active repo-local instructions and the syntax visible in its history take precedence; apply Go guidance only where compatible. Do not impose a fixed prefix, type, scope, subject, body, example, or template when composing, editing, validating, or recommending a JJ description.

If the project's active instructions name a process that owns that handoff — a named skill or command, a stacking tool, or documented steps, but not JJ-description or PR-title conventions, which the default already honors, and a skill directory alone is not a directive — run it non-interactively with the same plan path and context below instead of the default. It is done only when the bookmark is pushed and you hold the URL of an open PR containing it, whether the process opened it or it already existed for that bookmark. If it cannot run headlessly, is unavailable, uses non-JJ repository mutation, or ends short of that state, stop as **blocked** naming the process — do not fall through to the default or to step 9.
...
Invoke the default with an explicit constraint that every repository read or mutation use JJ changes, revsets, filesets, bookmarks, workspaces, and `jj git` remotes. It must inspect `jj status` and `jj diff -r 'trunk()..@'`, split coherent work when needed, describe and finalize remaining changes, create or move the publication bookmark to the stack head, and run `jj git push --bookmark <bookmark> --remote <remote>` before using `gh` to open the pull request for that bookmark. Git may be used only as the backing transport through `jj git`; if this route cannot honor that boundary, stop as blocked. In a non-colocated JJ repository, run `jj git root` separately and supply the result as `GIT_DIR` to each `gh` call; this also works from Git Bash. Keep the PR free of visual identity and creator attribution; when a provider requires an actor value, use `AI Assistant` with key `ai:assistant`.

If the default prints a `New concepts:` trailer after the PR URL, record the concept name(s) for step 10. Once the PR URL is known, back-fill it into any residual tickets filed in step 6 (the `filed` list) so each ticket links to the PR carrying the finding — best-effort, and never block DONE on a failed ticket update. If a PR already exists for the bookmark, as shown by `gh pr view <bookmark> --json number,url,state`, skip PR creation but still finalize remaining JJ changes, move the bookmark, and push it.

**Per the shipping precondition, when no remote is configured, do NOT invoke `ce-commit-push-pr` or a project-defined shipping process.** Inspect `jj status` and `jj diff -r @`; if `@` contains coherent remaining work, use `jj commit` to compose its description and create a new empty working-copy change. Split first with `jj split -i --editor` when independent work needs separate descriptions. Validate each finalized revision with `jj log -r <revision>` and `jj diff -r <revision>`, then skip bookmark publication and PR creation entirely. The description policy above applies at every compose, edit, validation, and recommendation site.
