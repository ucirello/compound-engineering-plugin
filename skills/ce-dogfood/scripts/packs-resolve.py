...
`--declared-only` answers the config-only question without touching remotes or
the cache: it parses both config layers, shape-checks each entry, and prints
...
Git sources cache under `<workspace>/.tmp/<skill>/ce-packs/<sha256(url\\nref)>`
(or `.tmp/<skill>/ce-packs/` when `jj workspace root` fails) with an atomic
temp-clone-then-rename, so a keyed path's existence proves a complete clone.
All jj subprocesses run non-interactively (GIT_TERMINAL_PROMPT=0, ssh
...
skipped); path sources still resolve when `jj workspace root` succeeds.
...
# --- scratch root (workspace .tmp; local .tmp if not in a JJ workspace) ---
...
def _workspace_root() -> str | None:
    """The enclosing workspace root via `jj workspace root`. None when jj is
    missing or the working directory is not inside a workspace."""
...
def cache_base() -> str | None:
    ...
    root = os.path.join(parent, skill)
    packs = os.path.join(root, "ce-packs")
    if _private_root_usable(parent) and _private_root_usable(root) and _private_root_usable(packs):
        return packs
    return None
...
def _clone_matched_bookmark(stderr: str | None) -> bool:
    """jj git clone -b still exits 0 when no remote bookmark matches."""
    return "No matching branches found" not in (stderr or "")


def _new_ref(dest: str, ref: str) -> bool:
    """Place the clone working copy on `ref` (bookmark, tag, sha, or remote name)."""
    for rev in (ref, f"{ref}@origin"):
        proc = _run_jj(["new", "--", rev], cwd=dest)
...
def resolve_git_source(url: str, ref: str, warnings: list, label: str) -> str | None:
    ...
    try:
        try:
            proc = _run_jj(
                ["--ignore-working-copy", "--quiet", "git", "clone", "--depth", "1", "-b", ref, "--", url, tmp],
                cwd=base,
            )
            bookmark_hit = proc.returncode == 0 and _clone_matched_bookmark(proc.stderr)
            if not bookmark_hit:
                shutil.rmtree(tmp, ignore_errors=True)
                os.makedirs(tmp, mode=0o700)
                proc = _run_jj(
                    ["--ignore-working-copy", "--quiet", "git", "clone", "--", url, tmp],
                    cwd=base,
                )
                if proc.returncode != 0 or not _new_ref(tmp, ref):
...
def enumerate_packs(source_root: str, boundary: str, escaped: list, self_name: str | None = None) -> dict:
    ...
    within (the clone, the repository, or the source root itself); a
...
def resolve_entry(entry: dict, repo_root: str, roots: list, warnings: list, errors: list) -> None:
    ...
    if _is_git_url(source):
        if not isinstance(ref, str) or not ref:
            errors.append(f"{label}: git source `{source}` requires `ref:` (tag, sha, or bookmark)")
            return
