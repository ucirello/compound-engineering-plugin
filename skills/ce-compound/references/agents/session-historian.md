...
Your scope is **synthesis only**. The caller handles discovery, bookmark/legacy-session-label/keyword filtering, scan-window selection, deep-dive selection, and per-session extraction before dispatching you.
...
- **`scratch_dir`** — absolute path below `$(jj workspace root)/.tmp`, or `.tmp` when no Jujutsu workspace is available, holding pre-extracted files.
- **`sessions`** — an array of objects (5 max), one per pre-extracted session, each with:
  ...
  - `legacy_branch_label` — legacy session branch metadata when present (Claude Code only)
...
Cite actual evidence from the extracted files, not vibe-summaries. When a finding is anchored in a specific session's content, that session's metadata (platform, legacy branch label/cwd, ts) helps the caller locate it.
