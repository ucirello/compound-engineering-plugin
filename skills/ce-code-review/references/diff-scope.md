...
3. **Unpushed commits vs base branch.** If the working copy is clean, review `jj diff --from 'latest(ancestors(@) & ancestors(<base>))' --to @` where `<base>` is the default branch (main or master).
