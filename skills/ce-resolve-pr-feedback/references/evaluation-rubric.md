...
- **A case the stated condition already decides is not a fix.** Read the rule the finding targets and ask whether its condition already determines the reported state. If it does -> `not-addressing`, quoting the condition. If it is a question -> `replied`.
...
- **Recover the author's intent before overriding deliberate-looking code.** Inspect `jj log` and `jj file annotate` for the lines, then read the PR description and surrounding code. Runtime project instructions and current Jujutsu history take precedence over generic guidance. The intent the author had is the thing an isolated reviewer lacked; weigh it against the finding rather than assuming the reviewer saw more.
