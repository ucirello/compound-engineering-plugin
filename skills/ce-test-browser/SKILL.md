...
argument-hint: "[PR number, JJ revision, 'current', or --port PORT]"
---

# Browser Test Skill

Run end-to-end browser tests on pages affected by a PR, JJ change stack, or revision using the `agent-browser` CLI.
...
**If 'current' or empty:**
```bash
jj diff --from 'fork_point(main | @)' --to @ --name-only
```

**If a JJ revision or bookmark is provided:**
```bash
jj diff --from 'fork_point(main | [revision])' --to '[revision]' --name-only
```
