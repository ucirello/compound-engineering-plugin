...
Loaded by `SKILL.md` when `/ce-sweep` runs with no `feedback_sources` configured. Captures the setup that will be merged into `<workspace-root>/.rocketclaw/config.local.yaml` (the unified local config, repository-ignored and machine-local) and re-read on every subsequent run.
...
- **Tracked in the workspace** (recommended when multiple agents or machines synchronize through a shared bookmark — one source of truth everyone reads and writes). Sets `sweep_state_path` to the default `docs/feedback-sweep/state.yml`.
- **Machine-local under `<workspace-root>/.tmp/rocketclaw/`** (solo setups; keeps sweep bookkeeping out of changes). Sets `sweep_state_path` to `<workspace-root>/.tmp/rocketclaw/sweep/state.yml`; if `jj workspace root` cannot resolve, use `./.tmp/rocketclaw/sweep/state.yml`.

Let the user override the path within the workspace. A tracked path belongs outside `.tmp`; a machine-local path belongs under `<workspace-root>/.tmp/rocketclaw/`. If they pick machine-local, note that another workspace or a teammate's machine will not see this state — it is per-machine by design.
...
**Ask:** "Is this a multi-agent setup where several JJ workspaces publish sweep state through one shared bookmark? Answer yes only if more than one machine or agent publishes to that bookmark. Default is no — one workspace keeping the change local."

- **No** (default) -> omit `sweep_shared_bookmark`. The single-writer lease serializes overlapping sweeps within one workspace.
- **Yes** -> ask for the bookmark name and store it as `sweep_shared_bookmark`. Explain: the lease becomes **push-gated** — before any source-side write, the sweep describes the lease change, advances and pushes that bookmark, and confirms its writer won, making the lease a repo-wide mutex across machines. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Inspect `git log` only for actual message syntax; runtime project instructions and that observed syntax win, and no fixed message syntax applies.

**Capture:** `sweep_shared_bookmark` (bookmark name; omitted for local-change mode).
...
**Ask:** "Do you have an existing feedback state file to import, such as a prior dogfood tracker? Importing carries over its cursors and items so the first sweep skips what's already been processed. Skip if this is a clean start."
...
- `sweep_shared_bookmark` — from section 6 (omit for local-change mode; only meaningful with tracked state).
...
sweep_state_path: docs/feedback-sweep/state.yml   # shared change, or .tmp/rocketclaw/sweep/state.yml for machine-local mode
...
sweep_shared_bookmark: feedback-sweep              # omit for local-change mode; set for a push-gated shared topology
~~~
