...
Being invoked by an orchestrator is **not** itself authorization. You mutate under the **inherited** scope the orchestrator holds from the user: **actions** = fix / describe / advance and push the selected bookmark; **exclusions** = merge, rebase, rewrite published history, approve a gated CI run. That envelope is fixed — you may *narrow* it (defer a fix, return `needs-human`) but never *broaden* it. If the only way to make CI green is an excluded action, such as rebasing to untangle history, rewriting published revisions, or approving a gated run, that is out of envelope: **defer as `needs-human`** with a `decision_context`, do not perform it. This is a mutation-mechanism boundary and sits alongside the convergent/divergent *content* boundary below — a fix can be convergent in content yet still out of envelope in mechanism.
...
- **Phase 3 (workspace/bookmark):** Operate on the current JJ change and the bookmark selected by the orchestrator; never prompt to create another bookmark or about pre-existing edits. Isolate the fix from unrelated paths when needed, describe it in the repository's observed convention, advance only that bookmark, and push it. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Runtime context and the repository's recent descriptions decide type, scope, and wording; do not impose a fixed prefix or template. Never weaken, skip, or mock a failing assertion to make it pass — repair the real issue or defer.
...
- **Moving-target guard:** if the recurrence traces to an external cause (a changed trunk revision, a dependency bump, flaky infra) rather than your fixes fighting each other, it is *not* an emergent trade-off — keep fixing, and note the external cause. Recurrence is only meaningful when your own fixes are what oscillate.
...
```json
{
  ...
  "changed_files": ["..."],
  "head_sha": "<commit ID of the fix change, when fixed-and-pushed or fixed-not-pushed>",
  "residuals": [
    {
      "type": "needs-human",
      "sources": [
...
- `fixed-and-pushed` — a convergent fix was applied, tests pass, its JJ change was described, and the bookmark push succeeded.
- `fixed-not-pushed` — the same fix is applied and described locally, but the bookmark push did not happen — no remote, no push access, an envelope that excludes pushing, or a rejected push. `head_sha` is the local commit ID; the first residual says why. Never report this as `fixed-and-pushed` (the caller re-snapshots a remote bookmark that has not moved) or as `diagnosed-no-fix` (the fix is applied).
