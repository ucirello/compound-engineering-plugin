...
A free-form description in the arguments is the source of truth. Otherwise derive it from context, using what's available and blocking on no single source. Run `jj` and `gh` with cwd at the workspace root (`jj workspace root`):
...
- **The diff** — `jj diff -r main..@ --stat`, skimming notable changes so the claim is grounded in what actually changed
- **Changelog** — the top or `[Unreleased]` entry in `docs/changelog.md`, `CHANGELOG.md`, or similar
- **Recent changes** — `jj log -r ::@ --limit 15 --no-graph -T builtin_log_oneline` for the arc of the change
