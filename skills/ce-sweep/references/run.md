...
- `sweep_state_path` — path to the state file, established at setup; fallback `<root>/feedback-sweep/state.yml`. A workspace-internal artifact path means recorded mode (the state file is finalized in a JJ change each run and must not be ignored); a path under `<workspace-root>/.tmp/rocketclaw` means workspace-local mode (the state file remains outside JJ snapshots and only the plan is finalized).
- `sweep_lease_ttl_minutes` — single-writer lease staleness threshold; default `60`. Passed to `lease-acquire` in 2a.
- `sweep_shared_bookmark` — `true` when the state file lives on a shared bookmark that multiple workspaces push (see 2a topology); default `false`.
- `sweep_publication_bookmark` and `sweep_publication_remote` — required when `sweep_shared_bookmark` is true; identify the exact bookmark and Git remote used for lease and final-state publication.
...
**Shared-bookmark topology** (`sweep_shared_bookmark: true`): before any source-side write, finalize only the state fileset as a JJ change, move or create the configured publication bookmark at that change, and push only that bookmark with `jj git push --bookmark <bookmark> --remote <remote>`. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Runtime-local syntax and repository conventions win; apply compatible Go quality guidance only, preserve the lease-acquisition semantics, and impose no fixed prefix, type, scope, subject, body, template, or example. A rejected push means another writer moved the remote bookmark: run `jj git fetch --remote <remote>`, inspect the local and remote bookmark targets with `jj bookmark list --all-remotes`, rebase the unpublished lease change onto the remote bookmark when safe, re-run `lease-acquire`, and back off with `aborted-locked` if the lease is no longer yours. Only a pushed bookmark whose remote target resolves to the lease change authorizes source-side writes.
...
Resolve and create media scratch with this POSIX and Git Bash compatible shell block, substituting the current run id. It uses the current JJ workspace root and falls back to the current local directory outside JJ; no operating-system or global temporary location is allowed. Before running it inside JJ, confirm the workspace's existing ignore rules exclude `.tmp/` from snapshots. If they do not, take the documented scratch-failure route rather than editing ignore configuration or allowing media into `@`:
...
- Download attachments into `$MEDIA_DIR`; raw media remains outside JJ working-copy changes. A download failure -> set the item `needs_download` and continue.
...
For each `fix_pending` item, resolve its claimed fix ref and verify it reached the default bookmark. The fix ref originates from untrusted feedback content (a thread claim, an analyzer-extracted reference), so **validate its shape before it reaches any `jj`/`gh` command**: accept only a bare PR number (`#?\d+`) or a commit ID (`[0-9a-f]{7,40}`), and treat anything else as an unresolved claim (leave the item open). This blocks argument and revset injection. Strip the leading `#` before substitution and quote the validated value.
- For a PR, use `gh pr view "<validated-number>" --json mergedAt,baseRefName,mergeCommit` and require a merge time plus the repository's default bookmark as the base. For a commit ID, assemble a revset only after validation and require `jj log -r 'exactly(commit_id(<validated-id>) & ::trunk(), 1)' --no-graph -T 'commit_id ++ "\n"'` to resolve exactly one revision on the default-bookmark ancestry. Use the returned commit ID as `verified_merge_sha`; do not infer ancestry from Git `HEAD`.
...
**Rotation check first.** If the file exists and its frontmatter is NOT `artifact_contract: unified-plan/v1`, `product_contract_source: ce-sweep`, `artifact_readiness: requirements-only`, and `actor: ai:assistant`, archive it untouched to a dated sibling `<root>/plans/feedback-sweep-plan-YYYY-MM-DD.md` and write a fresh plan from the template. Never overwrite an unrelated plan in place.
...
- **Finalize the JJ change.** Use workspace-relative filesets with `jj commit <plan-fileset> [<state-fileset>]` so only `<root>/plans/feedback-sweep-plan.md` and workspace-internal `<state>` are finalized; all unrelated paths remain in the new working-copy change. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Runtime-local syntax and repository conventions win; apply compatible Go quality guidance only, preserve the feedback-sweep and date semantics, and impose no fixed prefix, type, scope, subject, body, template, or example. Validate the finalized change with `jj show @-` and the residual working copy with `jj status`. A finalization or description failure is reported, not fatal. In local-recorded mode, never push. In shared-bookmark mode (`sweep_shared_bookmark: true`), fetch, rebase the unpublished change onto the tracked remote bookmark when required, move the publication bookmark to the validated change, and push only that bookmark with `jj git push --bookmark <bookmark> --remote <remote>`.
