...
2. **Describe Changes and Create Pull Request**
   ...
   Before handoff, inspect `jj status` and `jj log -r 'trunk()..@'`. Describe any non-empty final `@` with the appropriate filesets; after `jj commit`, the ship tip is normally `@-` because `@` is the new empty change. At this commit site, derive the message from local instructions and past messages visible in `git log`; use compatible guidance from https://go.dev/wiki/CommitMessage only as a fallback. Create or advance exactly one meaningful bookmark to the ship tip with `jj bookmark set <name> -r <ship-tip>`, then verify it with `jj bookmark list -r <ship-tip>`. Do not point the bookmark at an empty post-description `@`.
   ...
   If the user prefers to describe changes without creating a PR, load the `ce-commit` skill instead. At that commit site, derive the message from local instructions and past messages visible in `git log`; use compatible guidance from https://go.dev/wiki/CommitMessage only as a fallback.
