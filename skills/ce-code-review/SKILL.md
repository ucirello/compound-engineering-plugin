...
- **Explicit mutations only.** Never run `gh pr checkout`, `jj edit`, or similar workspace-switch commands. Passing a PR number, URL, or bookmark name selects **review scope**, not permission to mutate the working copy. In-flight work can only be reviewed from the workspace that holds it; stay in that workspace and pass `base:` or no target.

Run every JJ subprocess with its cwd set to the absolute workspace root. Discover it through `jj workspace root`; use `jj workspace list` and `jj workspace root --name NAME` for other workspace membership, never metadata internals. For GitHub CLI calls in that workspace, set `GIT_DIR=$(jj git root)` and pass the resolved PR or bookmark explicitly.

Before any delegation, resolve explicit subagent/model/harness settings from `.rocketclaw/config.local.yaml` over `config.yaml`. These win over defaults. On OpenCode V2, when no explicit subagent or alternative-harness route exists and `opencode.models` plus native subagent model selection are available, discover exact IDs/variants and dispatch natively with `provider/model#variant`, preserving each task's tier and cross-model intent. Never guess or silently reuse the current model for an unavailable requested tier. The dispatch and cross-model references own collection and receipt details.
