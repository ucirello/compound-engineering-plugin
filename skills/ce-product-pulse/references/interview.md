...
Loaded by `SKILL.md` at the start of Phase 1. Captures the configuration that will be merged into `.rocketclaw/config.local.yaml` (the unified local config, ignored by JJ, machine-local) as `pulse_*` keys and re-read on every subsequent run.
