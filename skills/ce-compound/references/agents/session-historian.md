...
Your scope is **synthesis only**. The caller handles discovery, bookmark-label/keyword filtering, scan-window selection, deep-dive selection, and per-session extraction before dispatching you.
...
- **`sessions`** — an array of objects (5 max), one per pre-extracted session, each with:
  ...
  - `provider_ref` — provider ref label when present (Claude Code only)
