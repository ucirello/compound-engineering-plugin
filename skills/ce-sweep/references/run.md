...
**Shared-bookmark topology** (`sweep_shared_bookmark: true`): before any source-side write, finalize only the state fileset as a JJ change, move or create the configured publication bookmark at that change, and push only that bookmark with `jj git push --bookmark <bookmark> --remote <remote>`. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Runtime-local syntax and repository conventions win; preserve the lease-acquisition semantics. A rejected push means another writer moved the remote bookmark: run `jj git fetch --remote <remote>`, inspect the local and remote bookmark targets with `jj bookmark list --all-remotes`, preserve the remote state while rebasing the unpublished lease change, and re-run `lease-acquire` against that reconciled state. A conflicted bookmark or state file, or a lease now owned by another writer, stops before source-side writes with `aborted-locked`; never resolve this gate by choosing the local lease state. Only a pushed bookmark whose remote target resolves to the lease change authorizes source-side writes.
...
- For a PR, use `GIT_DIR="$(jj git root)" gh pr view "<validated-number>" --json mergedAt,baseRefName,mergeCommit` and require a merge time plus the repository's default bookmark as the base. For a commit ID, assemble a revset only after validation and require `jj log -r 'exactly(commit_id(<validated-id>) & ::trunk(), 1)' --no-graph -T 'commit_id ++ "\n"'` to resolve exactly one revision on the default-bookmark ancestry. Use the returned commit ID as `verified_merge_sha`; do not infer ancestry from backing-Git ref state.
...
**Rotation check first.** If the file exists and its frontmatter is NOT `artifact_contract: unified-plan/v1`, `product_contract_source: ce-sweep`, and `artifact_readiness: requirements-only`, archive it untouched to a dated sibling `<root>/plans/feedback-sweep-plan-YYYY-MM-DD.md` and write a fresh plan from the template. Never overwrite an unrelated plan in place.
