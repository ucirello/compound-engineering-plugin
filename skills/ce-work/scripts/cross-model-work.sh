...
# supplied detached workspace. The adapter never creates JJ workspaces, changes
...
adapter_argv() {
  case "$1" in
    ...
    opencode)
      ...
      ;;
    opencode2)
      # No --dir on opencode2 run; the invoke site cds to $WORKSPACE before exec.
...
SCRATCH="$(mktemp -d "$SCRATCH_ROOT/ce-work-adapter-XXXXXX")" || exit 2
chmod 700 "$SCRATCH"
