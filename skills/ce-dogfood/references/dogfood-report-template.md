# Dogfood Report — <change stack>

> Diff-scoped browser QA of `<change stack>` relative to `<trunk or PR base>` on <YYYY-MM-DD>.
...
<What changed between the JJ change stack and its resolved trunk or PR base: new features, modified behavior, new/changed routes, views, components, data flows. 2-6 bullets.>
...
<Overall readiness verdict for the change stack. Ready to ship? Caveats? Outstanding blocked items? Record the result of the Phase 5 automated test suite run — a green matrix with a red suite is not "ready.">
