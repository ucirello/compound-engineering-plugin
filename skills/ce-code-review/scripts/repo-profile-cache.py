...
Cache path:
    $(jj workspace root)/.tmp/rocketclaw/repo-profile/<root-sha>/<head-sha>.json
  root-sha = lexicographically-first commit ID in `roots(::@ ~ root())`
              (deterministic even for multi-root histories) — the repo identity.
  head-sha = the commit ID of `@` — the working state.

Validity (HIT) requires ALL of:
  - the cache file exists and parses as JSON,
  - stored `head_sha` == current `@`,
  - stored `profile_schema_version` == PROFILE_SCHEMA_VERSION,
  - no profile-input path is changed in `jj diff -r @ --summary`
    (the schema-derived superset in `is_profile_input`; newly-added files are
    automatically tracked by JJ unless ignored).
...
workspace `.tmp`, JJ errors) degrades to NO-CACHE/MISS and exits 0 — it never raises and
...
def changed_paths() -> "list[str] | None":
    ...
    Includes newly-added files because JJ automatically tracks non-ignored files.
    ...
    for line in result.stdout.split("\n"):
        if not line.strip():
            continue
        # Summary lines are `<status> <path>`; copy/rename output can name both
        # endpoints, so retain each side conservatively.
        rest = line[2:].strip()
        if "=>" in rest:
            # JJ may compact rename paths with braces. Any rename invalidates
            # conservatively so a profile input renamed away cannot stay cached.
...
def cache_path(root: str, head: str) -> str:
    base = cache_root()
    if base is None:
        base = os.path.join(os.getcwd(), ".tmp", "rocketclaw", "repo-profile")
    return os.path.join(base, root, f"{head}.json")
...
def do_get() -> int:
    ...
    try:
        with open(path) as f:
            # Reject a cache file not owned by us: another local user could
            # plant an entry that passes the gates below and
...
def do_put(profile_file: str) -> int:
    ...
    # edits to profile inputs, yet it would be stored under the current change
    # key and could later be stale. Persist only a profile with clean inputs.
    changed = changed_paths()
    if changed is None or any(is_profile_input(p) for p in changed):
        sys.stderr.write(
    ...
    try:
        ...
        # Atomic write in the workspace-local cache directory.
