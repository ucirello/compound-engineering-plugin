...
|---|---|---|
| E1 native restraint | `ce-work <root>/plans/feature.md`; no directive, caller binding, or enabled config | Native inline/subagent engine; no external egress and no `ce-work`-created JJ workspace for an ordinary synchronous unit; standalone tail remains `ce-work`-owned. |
...
| E16 unsupported restriction | Caller requires enforceable workspace confinement; candidate offers cooperative same-user containment only | Route is unavailable; follow `prefer`/`require`; do not describe a linked JJ workspace as a security sandbox or silently weaken the restriction. |
...
| E20 linked-checkout sibling | `ce-work` is itself running in an existing linked JJ workspace and selects external implementation for one unit | Create a new detached **sibling** with `jj workspace add --name <name> -r <base> <dest>`, place it under `$(jj workspace root)/.tmp/ce-work/<run-id>/` rather than beneath the active checkout, base it at the recorded clean canonical commit id, and keep canonical fold-in host-owned. Do not reject the route merely because the active checkout is already a workspace, and do not create a nested workspace. |
...
| E29 clean packet and shell argv | A clean linked checkout needs a packet source, and its V1 command is `test "$(cat delegated.txt)" = "expected"` | Write the packet source directly under `$(jj workspace root)/.tmp` outside the tracked tree. At integration and plan-wide verification, recognize `$(...)` as shell syntax and use an explicit pipefail-capable shell on the first attempt; do not create tracked-tree scratch or pass the expression as literal direct argv. |
...
| E40 sandboxed worker no-commit | A Codex or Cursor unit has finished files and scoped checks in its detached JJ workspace; the worker is about to `jj commit`/`jj describe` | Do not instruct the worker to write a JJ change description. Leave the working copy uncommitted and treat completion as files plus scoped checks. Host `terminalize` snapshots the tree. A Codex sandbox `EPERM` on a socket bind or peer-credential probe is not proof the host lacks the capability. |
