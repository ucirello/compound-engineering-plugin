...
(`<scratch-root>/brainstorm/<run-id>/grounding.md`) — it gives
...
skills run (plus `jj`/`gh` for the PR and CI steps, which it guards when absent). Run `gh` from the absolute workspace root with `GIT_DIR=$(jj git root)`.
