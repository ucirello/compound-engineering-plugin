...
def load_state(path):
    ...
    try:
        with open(path) as f:
            ...
            # well as an injection sink (item bodies re-read into assistant
...
def _workspace_root():
    ...
    configured = os.environ.get("ROCKETCLAW_WORKSPACE_ROOT")
    ...
    candidate = current
    while True:
        if os.path.isdir(os.path.join(candidate, ".jj")):
            return candidate
