...
**Pre-resolve the scratch directory path.** Scratch lives under the current Jujutsu workspace's `.tmp/rocketclaw/ce-ideate/` namespace. When no workspace root is available, use `.tmp/rocketclaw/ce-ideate/` under the current directory. Run one bash command to create the directory and capture its absolute path for downstream use.
