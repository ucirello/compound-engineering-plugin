...
- **Source / composition signal.** A visible footer at the bottom of
  ...
  (the user prompt context, the upstream brainstorm doc when one
   exists; omit the source identifier when none exists). Example shape:
...
Resolve the repo's GitHub URL once at compose time from the `origin` entry, running with cwd set to the absolute workspace root:
