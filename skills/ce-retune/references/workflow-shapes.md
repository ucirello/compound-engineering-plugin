...
| Cut passes | Fan out by disjoint fileset ownership, single owner for shared byte-identical assets, barrier before verification | Concurrent edits to one file lose each other; verification must see one workspace state | Overlapping ownership produces a state no agent authored and no change isolates |
...
**Corpus audit — fan out by unit, pipelined into defense.** One agent per skill directory, each reading that directory in full; that full read is what makes per-unit parallelism worth its cost, and it is why one agent cannot do all the units. Pipeline the adversarial defense stage per unit: a unit's defender needs that unit's findings plus the project's own documented learnings, tests, and JJ history — never the global finding list — so a defense can start the moment its audit returns. A global barrier here only makes every defender wait on the slowest auditor.
...
**Cut passes — disjoint fileset ownership, one owner for shared assets, then a barrier.** One problem per pass, one agent per unit, and JJ filesets that do not intersect. Byte-identical duplicated assets are the trap: if two agents each touch their own copy of the same file, you get divergence that a parity test catches at best and ships at worst. Assign every copy of a duplicated asset to exactly one owner, even when that owner's unit is not the one the copy most obviously belongs to. Close the fan-out with a barrier before verification: verification reads one workspace state, and a state with a change still landing in it is not the state you will ship. Loop mechanics are in `references/cut-passes.md`.
...
Run the mechanical suite once, as one check, not once per agent. Keep the coherence read in the fan-out and give it a human-style instruction: read the changed files end to end and name anything that no longer follows. It is the only check that finds a reference left pointing at removed text, and no search pattern will do it.
...
Resolve scratch storage once. When `jj workspace root` succeeds, use `$(jj workspace root)/.tmp/rocketclaw`; outside a JJ workspace, use the local `./.tmp/rocketclaw` fallback. Before writing there, verify the applicable local ignore rules keep `.tmp` out of the working-copy change. Put all disposable workspaces, per-run scratch, transient extracts, and tool-created temporary files below that root rather than using a system or user-global temporary location.
