...
- Write to the run's workspace-local scratch directory, or under `<root>/` when the user wants it kept; announce the deliverable's absolute path. Do **not** introduce a new mandated durable location — that store is deferred.
