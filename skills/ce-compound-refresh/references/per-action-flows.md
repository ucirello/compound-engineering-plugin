...
2. Move the file on disk so history follows the rename (JJ snapshots the working copy; there is no index).
...
5. **Delete the subsumed doc.** Do not archive it, do not add redirect metadata — just delete the file. Version history preserves it.
...
4. The orchestrator deletes the original. Version history preserves it.
...
4. **Run the mechanical claims check on the successor doc.** The bundled `scripts/validate-doc-claims.py` flags cited repo paths missing from the tree, revision IDs that do not resolve or are unreachable, relative doc links that do not resolve, and dangling drafting scaffold ("Learning 3", unresolved `{{...}}` tokens):
