...
|-------|---------|--------|
| `diff:<ref-or-range>` | `diff:abc1234`, `diff:trunk()..@`, `diff:PR#42` | Forces diff mode on that change |
...
- An unrecognized `<word>:<word>` token (including conventional change-message prefixes like `feat:` appearing inside a topic) is not a flag — it passes through verbatim as request text.
