...
- **Working-copy bookmark**: run `jj bookmark list -r @`. Parse bookmark names. Use them to filter session history in Phase 1. If none are listed or the command exits non-zero (not a Jujutsu workspace), skip bookmark filtering.
...
#### 4. **Session History** (internal flow after launching the parallel block; automatic in Full mode, including non-interactive)
   ...
   **Extraction pipeline.** Create a per-run scratch directory under the workspace `.tmp` (fallback to local `.tmp` when `jj workspace root` fails): `WS="$(jj workspace root 2>/dev/null)"; if [ -n "$WS" ]; then mkdir -p "$WS/.tmp"; SCRATCH=$(mktemp -d "$WS/.tmp/ce-compound-sessions-XXXXXX"); else mkdir -p .tmp; SCRATCH=$(mktemp -d ".tmp/ce-compound-sessions-XXXXXX"); fi`. For each selected session, write extracted content to scratch files:
