...
2. **Brainstorm one idea with `ce-brainstorm`** — turn a chosen idea into a requirements-only unified plan under `docs/plans/`; leaves ce-ideate. Asks which idea first.
...
- **Inside a jj workspace:** offer to describe a jj change containing only the ideation doc (do not create a bookmark, do not push; if the user declines, leave it as working-copy content).
