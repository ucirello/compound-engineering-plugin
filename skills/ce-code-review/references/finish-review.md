...
Severity, confidence, and cross-reviewer agreement tell you what to do first and what to flag loudly; they do not gate the decision. There is no deny-list: downside is controlled after the fact (`jj undo` plus a visible diff and change checkpoint), not by a precondition.
...
**Scope invariant.** Apply only when the working copy *is* what was reviewed: `local-aligned` or standalone. In `pr-remote` / `branch-remote` the working copy is not the reviewed head; report instead.
...
**Review the autofix diff before finishing.** Before describing the change or reporting applied fixes, diff only the changes introduced during Stage 5c against the pre-apply operation checkpoint. Run one self-review pass over that diff:
...
**Describe and advance when the pre-review change was empty.** Before applying, record whether `jj diff -r @` was empty. The permanence gate is a bookmark push, not a local change description; local operations remain reversible with `jj undo`.

- **Empty before the review:** after applying and verifying, compose the working-copy change description at runtime. Resolve the backing Git directory with `jj git root`, then inspect actual history with `git --git-dir <resolved-git-root> log -n 10 --format=full`; run these as separate shell calls and use Git only for this read. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Repository-local instructions and message syntax observed in the actual `git log` output take precedence; apply compatible Go guidance only for quality, clarity, and structure, without imposing a fixed prefix, type, scope, subject, body, layout, template, or example. Run `jj describe -m "<description-composed-from-runtime-conventions>"`, then `jj new` to return to a fresh working-copy change.
- **Non-empty before the review:** apply but do **not** describe or advance the change; the fixes remain with the user's in-flight work. The Applied section lists what changed.
...
2. **Applied (explicit local apply only).** When Stage 5c applied fixes, list them first in an Applied section; each entry carries `#`, file, the fix, and reviewer, then a one-line validation outcome and change status (described on an initially empty `@` using runtime conventions, or left in the user's existing change). Flag green-but-unverifiable edits prominently. Omit this section when local apply was not authorized or nothing was applied.
...
```json
{
  ...
  "scope": {
    ...
    "bookmarks": "<jj log -r @ --no-graph -T 'bookmarks'>",
