...
implementation, and any durable decisions made along the way land in the JJ
...
- **`title`** — the plan's descriptive name with a ` - Plan` suffix
  ...
  across readiness states (it is a plan at every stage). Do not add a fixed
  change-description prefix, type, or scope to the title.
...
whether a plan shipped is derived from `jj status`, `jj diff`, and `jj log`,
not stored in the doc. Do not add a `status` field or an
