...
3. Invoke the `ce-simplify-code` skill on the current JJ diff.
   ...
   This runs before review so the code-review in step 4 covers the simplified code. **Skip** this step when the change is docs-only (only markdown/docs paths changed) or trivial (roughly under 10 changed lines). Otherwise let `ce-simplify-code` resolve the current-diff scope itself; it preserves behavior and runs the test suite.

   Do not describe or commit in this step. `ce-simplify-code` leaves its changes in the working copy; step 4's review scopes the working copy (current-change edits included), and step 8's `ce-commit-push-pr` finalizes whatever remains. Finalizing here would sweep any still-current `ce-work` edits into a misleading `refactor` change and could stall on a working copy that never goes clean.
...
**Shipping precondition (steps 5–9).** Run `jj git remote list` once before the shipping steps. If it lists **no remote** (e.g. a sandbox/throwaway working copy with no `origin`), shipping is **local-only**: make every JJ change the steps below call for, but **skip every push, PR create/edit, and CI-watch action** — the pushes in steps 5 and 6, the push and PR creation in step 8, and step 9 in full. A missing remote is a terminal local-only state, not an error: never retry a push or hunt for a remote — make the local changes and proceed to step 10. Run steps 5–9 normally when a remote exists.

5. **Apply and persist review fixes** (REQUIRED after step 4, before residual handoff)

   Load `references/review-followup.md` and execute its apply step (mechanical apply + describe/push when changes exist). Do not proceed to the residual handoff, run browser tests, or output DONE while eligible review fixes remain only in the working copy.
...
8. Invoke the `ce-commit-push-pr` skill.
   ...
   This finalizes any remaining changes, pushes the bookmark, and opens a pull request. If step 6 already opened a PR (check with `gh pr view --json number,url,state 2>/dev/null`), skip PR creation but still describe/commit and push any current-change edits. **Per the shipping precondition, when no remote is configured, do NOT invoke `ce-commit-push-pr` if its current implementation requires a push. Instead finalize any remaining changes locally yourself (`jj status`, `jj diff`, `jj commit -m "<message>"`) and skip the push and PR creation entirely.**
...
9. **CI watch and autofix loop** (only when an open PR exists for the current bookmark)
   ...
   4. Commit only the CI-fix files you changed and push the current bookmark. If unrelated edits are present, isolate the CI-fix paths first with `jj split <changed-files>`:
