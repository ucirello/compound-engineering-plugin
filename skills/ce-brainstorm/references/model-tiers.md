...
- **Generation tier** — the claim verifier: evidence-driven mechanical verification. Use the platform's mid-tier model when the current harness exposes a known override. If model names are unknown, use the discovery or degradation rule below rather than guessing.
...
**OpenCode V2.** Apply the actual config reads and ordinary-key merge in `SKILL.md` before selecting a route. Honor effective explicit subagent or alternative-harness routing first. When none is set, the current harness is OpenCode, and `opencode.models` plus `subagent(model)` are available, discover exact IDs and variants for the required tier and invoke the native subagent with that exact model reference, preserving any requested `#variant`. Preserve any cross-model requirement; never guess an ID, silently reuse the current model, or launch a shell delegate on this native route. If discovery cannot establish a suitable model, report that limitation and perform the task inline with its existing read budget, output cap, and dossier contract. Configured routes retain their own dispatch mechanism.

**Degradation rule.** On other harnesses, when the platform's subagent primitive does not support per-agent model selection, dispatch the scout and verifier on the inherited model and keep their read budgets and output caps — cost control then comes from structure, not tiering. When the platform has no subagent primitive at all, do the topic scan inline at Phase 1.1 — still writing the grounding dossier to the scratch path, because downstream consumers (the Phase 2.6 verifier, the ce-plan handoff) receive that path — and verify claims inline before the Phase 3 write, with the same budgets.
