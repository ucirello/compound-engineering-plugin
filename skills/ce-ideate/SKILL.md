...
**Done:** a ranked ideation artifact is written to `<root>/ideation/` when that root is present, else to a `.tmp` scratch path. Every idea generated has been critiqued, and the survivors are explained. The user is left holding the next-steps menu. No requirements, plans, or code.
...
Scratch lives under `$(jj workspace root)/.tmp`, or cwd-relative `.tmp` when not in a JJ repository — never `.context/`. Generate one 8-hex `<run-id>` and reuse it for the cache and for every checkpoint. The layout is `.tmp/ce-ideate/<run-id>/`.
