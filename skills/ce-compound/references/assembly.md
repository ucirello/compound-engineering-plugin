...
> Shared domain vocabulary for this project — entities, named processes, and status concepts with project-specific meaning. Seeded with core domain vocabulary, then updated as learnings are captured; direct edits are fine. Glossary only, not a spec or catch-all.
...
1. **Mechanical claims check (every mode, including non-interactive).** Optionally run `jj git fetch --quiet` from the absolute workspace root first (best-effort; skip silently offline, because the network is never a correctness dependency). Then run the bundled validator against the written doc:
