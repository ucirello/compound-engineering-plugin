...
workspace's `.tmp/rocketclaw` tree. Every read of job state opens the file first (no-follow) and
...
environment overrides: ROCKETCLAW_PEER_IDLE_SECS, ROCKETCLAW_PEER_HARD_SECS,
ROCKETCLAW_PEER_LOG_MAX_BYTES, ROCKETCLAW_PEER_RESULT_MAX_BYTES,
ROCKETCLAW_PEER_POLL_SECS, ROCKETCLAW_PEER_GRACE_SECS (defaults in the module docstring).
"""
...
def jobs_root_base() -> str:
    try:
        ...
        result = None
    workspace_root = result.stdout.strip() if result and result.returncode == 0 else os.getcwd()
    scratch_parent = os.path.abspath(os.path.join(workspace_root, ".tmp"))
    try:
    ...
    _check_owned_dir(scratch_parent, require_private=True)
    return os.path.join(scratch_parent, "rocketclaw")
...
def write_atomic(path: str, data: bytes) -> None:
    tmp = os.path.join(os.path.dirname(path), f".atomic-{os.urandom(8).hex()}")
...
def supervise(job_dir: str, argv, result_path, conf: dict, ack_fd: int) -> None:
    ...
    try:
        ...
        try:
            worker_env = {**os.environ, "ROCKETCLAW_PEER_JOB_ID": os.path.basename(job_dir)}
...
def _require_posix_detach() -> None:
    ...
    if not hasattr(os, "fork") or not hasattr(os, "setsid"):
        ...
        raise RunnerError(
            ...
            "started. On native Windows, run the AI Assistant / this skill under WSL, "
            "or wait for a Windows-native detach path."
        )
...
def cmd_result(args) -> int:
    ...
    if getattr(args, "path", None):
        ...
        # consumer must never read a predictable workspace-local scratch path unchecked.
        try:
