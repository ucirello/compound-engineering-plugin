...
SQLITE_FILES=$(find "${SCAN_PATHS[@]}" -maxdepth 4 \( -name .git -o -name .jj \) -prune -o -type f \( -name '*.db' -o -name '*.sqlite' -o -name '*.sqlite3' \) ! -path '*/node_modules/*' ! -path '*/.claude/*' ! -path '*/.context/*' ! -path '*/.worktrees/*' ! -path '*/.tmp/*' -print 2>/dev/null | head -10 || true)
...
LOCK_FILES=$(find "${SCAN_PATHS[@]}" -maxdepth 4 \( -name .git -o -name .jj \) -prune -o -type f \( -name '*.lock' -o -name '*.pid' \) ! -path '*/node_modules/*' ! -path '*/.claude/*' ! -path '*/.context/*' ! -path '*/.worktrees/*' ! -path '*/.tmp/*' ! -name 'package-lock.json' ! -name 'yarn.lock' ! -name 'bun.lock' ! -name 'bun.lockb' ! -name 'Gemfile.lock' ! -name 'poetry.lock' ! -name 'Cargo.lock' -print 2>/dev/null | head -10 || true)
