...
`ce-code-review` is invoked here with `mode:agent`, so it is **review-only** in this context — it reports findings and writes artifacts and does not mutate the working copy, finalize JJ changes, push, or file tickets. **The caller owns apply/fix policy.** (In its own default/interactive mode the review applies safe fixes itself; that path does not apply here.)
...
- Run artifact dir: `$workspace_root/.tmp/rocketclaw/ce-code-review/<run-id>/` (`review.json`, per-reviewer JSON for `why_it_matters`). Before reading or writing artifacts, run `workspace_root=$(jj workspace root 2>/dev/null || pwd); mkdir -p "$workspace_root/.tmp/rocketclaw/ce-code-review"`.
...
- `base:` — when the diff base is already resolved with `jj log` on the current workspace; omit when reviewing a PR number/URL or standalone current bookmark.
...
Default to applying every actionable finding. Applying is a reversible working-copy edit; diffs are reviewed before finalizing the JJ change (below) and tests run after — so leaving a clear, reversible fix unapplied "to be safe" is the failure mode, not the safe choice. Bias to act:
...
There is no precondition safety checklist and no deny-list — a code-review fix is a reversible edit, so downside is controlled after the fact (diff review + tests + the JJ change-finalization checkpoint), not by gating the apply.
...
**Orchestrator owns:** parse review output → **eligibility filter on JSON fields only** → build batches → dispatch fix subagents → review diffs → tests → finalize JJ changes → Residual Work Gate.
...
3. **Parallel waves:** batches with **disjoint file sets** may run in parallel (same isolated-workspace / shared-directory rules as Phase 1 Step 4 in `ce-work` SKILL.md).
...
- Shared-directory fallback: do not run `jj describe` or `jj new` — return which `#` were applied or skipped and which files changed

**After each wave:** orchestrator reviews diffs (scope = assigned `#` only), runs tests (`requires_verification: true` on any applied finding -> at least targeted tests; multi-file -> broader suite), and finalizes a JJ change whose description identifies the applied finding IDs unless workspace-isolated subagents integrate per Phase 1. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Repository-local instructions and `git log` syntax always win; apply compatible Go quality guidance, and derive any prefix, type, scope, subject, template, and concrete wording dynamically rather than imposing fixed forms. Use `jj status` and `jj diff` to verify the change, `jj describe -m "<message derived from repository standards and applied finding IDs>"`, then `jj new`. Repeat until all batches complete.
