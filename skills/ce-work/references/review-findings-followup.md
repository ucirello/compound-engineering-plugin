...
- `base:` — when the diff base is already resolved in the current working copy; omit when reviewing a PR number/URL or standalone current bookmark/workspace.
...
Default to applying every actionable finding. Applying is a reversible edit to a tracked tree; diffs are reviewed before describing the change (below) and tests run after — so leaving a clear, reversible fix unapplied "to be safe" is the failure mode, not the safe choice. Bias to act:
...
- Shared-directory fallback: do not describe/split changes — return which `#` were applied or skipped and which files changed

**After each wave:** orchestrator reviews diffs (scope = assigned `#` only), runs tests (`requires_verification: true` on any applied finding → at least targeted tests; multi-file → broader suite), describes the change (`fix(review): apply findings #...`) unless workspace-isolated subagents integrate per Phase 1. Repeat until all batches complete.
