...
**Outcome:** a RocketClaw Pack the resolver publishes on the next planning or review run: a directory holding a `README.md` that says what the pack governs, one first rule at the top level, and a `packs:` entry in `.rocketclaw/config.yaml` that points at it. The next consumer is the bundled health check, which lists the pack as `pack <id>`.
...
- The default target is `packs/<id>/` relative to the workspace root (`jj workspace root`). Run JJ commands with that absolute root as cwd. When the user names another repo-relative directory, the config entry points there instead.
...
- A live pack entry in `config.yaml` is `- source: packs/<id>` under a top-level, uncommented `packs:` key. The bundled template ships that key as a comment, which is not a live key.
...
6. **Tell the author.** In one sentence: a rule is discovered only when it is a top-level `.md` with `title` and `applies_when`; everything else in the pack folder is storage, and `README.md` is the description. Point at the "Pack layout" documentation for the annotated tree. Report the pack under Fixed, or under Skipped when the user declined, in the Phase 3 summary.
