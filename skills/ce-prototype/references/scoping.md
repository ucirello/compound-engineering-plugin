...
For delegation, honor effective `.rocketclaw/config.yaml` and `.rocketclaw/config.local.yaml` subagent and harness settings, with local overrides taking precedence. On OpenCode V2, when no explicit subagent or alternative-harness route is configured and `opencode.models` plus native subagent dispatch with an optional model are available, discover exact model IDs and variants and dispatch natively with `provider/model#variant`. Preserve the requested tier and cross-model intent; never guess an ID or silently reuse the current model. Otherwise preserve the configured route or the inline fallback above. Treat `opencode` as a distinct harness when interpreting configuration.
