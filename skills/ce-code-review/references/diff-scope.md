...
2. **Working-copy changes.** If `jj diff` is non-empty, review those.
3. **Unpushed changes vs base bookmark.** If the working-copy change is empty, resolve the unique common ancestor with `heads(::<base> & ::@)` and review `jj diff --from <common-ancestor>`, where `<base>` is the default bookmark.

The scope step in `SKILL.md` handles discovery and passes you the resolved diff. Do not rerun version-control commands unless remote scope requires targeted inspection.

## Remote scope (`pr-remote` and `branch-remote`)

When the review context includes `<pr-scope-mode>pr-remote</pr-scope-mode>` or `<pr-scope-mode>branch-remote</pr-scope-mode>`, the working copy is **not** the reviewed head. Do **not** use Read/Grep on workspace paths for any file that belongs to the reviewed tree — the changed-file list, and any other file the review points you at, such as a criteria file. They may not match the branch or PR under review, and an unchanged file is not exempt: the workspace can hold a version the reviewed head never had.
...
In `pr-remote` / `branch-remote` scope these tiers inspect the working copy, which is not the reviewed head. Apply the Remote scope rules above instead of local search.
