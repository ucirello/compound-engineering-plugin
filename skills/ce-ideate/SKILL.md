...
**Token-parsing convention:** only literal-prefix flag tokens (`output:`, `mode:` where applicable) are consumed and stripped. Other `<word>:<word>` tokens, including change-message prefixes that may appear inside a focus hint, pass through verbatim.
...
- **Repo mode surprise-me:** the codebase-scan sub-agent samples a few representative files per top-level area (not just reads the top-level layout + AGENTS.md), surfaces recent PR/Jujutsu-change activity as signal about what's actively being worked on, and — when issue intelligence runs — passes issue themes as first-class input rather than footnote. Keep the scan bounded: representative, not exhaustive.
...
**Pre-resolve the scratch directory path.** Scratch lives under the Jujutsu workspace root at `.tmp/rocketclaw/ce-ideate/<run-id>`. When the current directory is not in a Jujutsu repo, use the current directory's local `.tmp/rocketclaw/ce-ideate/<run-id>` instead. Run one bash command to resolve and safely create the directory, then capture its absolute path for downstream use.
