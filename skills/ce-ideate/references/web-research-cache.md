...
Files live under `<scratch-dir>/web-research-cache.json`, where `<scratch-dir>` is the workspace-local `.tmp/rocketclaw/ce-ideate/<run-id>` path resolved once in SKILL.md Phase 1.
...
- **Repo mode:** the focus hint plus a stable repo discriminator. This keeps the cache key meaningful when focus is empty — two bare-prompt invocations in the same repo legitimately share research, but the key still differentiates repos. Resolve the discriminator with this fallback chain and hash the result (first 8 hex chars of sha256 is sufficient):
    1. `jj git remote list` and the `origin` URL from its output — Git interop that is stable across machines and correct for collaborators on the same remote.
    2. `jj workspace root` — absolute workspace path; machine-local but available in a Jujutsu checkout.
    3. The current working directory's absolute path — last resort when not in a Jujutsu repo.
