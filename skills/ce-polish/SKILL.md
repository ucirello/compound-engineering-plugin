...
argument-hint: "[PR number, bookmark name, or blank for current working-copy change (@)]"
---
...
1. If a PR number or bookmark name was provided, move the working copy to the target with JJ (`jj new <bookmark-or-revision>` or `jj edit <revision>` as appropriate), probing for existing workspaces first.
2. If blank, use the current working-copy change (`@`) and any bookmark currently pointing at it.
3. Verify `@` is not the trunk/default bookmark revision (`main`, `master`, or the detected default bookmark).
...
- When the user says they're done → describe the fix change and stop
