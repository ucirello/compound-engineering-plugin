...
2. Move the file with the native filesystem edit capability. JJ detects the rename from the content and path changes.
...
4. **Run the mechanical claims check on the successor doc.** The bundled `scripts/validate-doc-claims.py` flags cited repo paths missing from the working copy or relevant JJ revisions, revision IDs that do not resolve or are unreachable, relative doc links that do not resolve, and dangling drafting scaffold ("Learning 3", unresolved `{{...}}` tokens):
