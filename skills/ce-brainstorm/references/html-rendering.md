...
- **Source / composition signal.** A visible footer at the bottom of
  ...
  exists; omit the source identifier when there is no external source).
  Example shape:
...
Run from the absolute workspace root and select the `origin` row's URL from the output.
