...
4. **Save and report.** Write the deliverable to a durable, repo-tracked location, defaulting to a sensible `docs/` subpath unless the user named one, and report its absolute path. Whether to describe and seal the jj change or leave it in the working-copy change is the user's call; offer, do not force.

## Stay scoped to non-code deliverables

The carve-out is for knowledge-work output. If producing it legitimately requires a script, configuration, or data transform, route that specific sub-step through the normal code path so Test Discovery, review, and jj change hygiene apply; do not silently produce code under the carve-out. The deliverable itself stays non-code.
