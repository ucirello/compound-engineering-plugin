...
**Jujutsu bookmarks (pre-resolved):** !`jj bookmark list -r @ 2>/dev/null || true`

If the line above resolved to one or more local bookmarks, use them in Phase 1 session-history filtering so the orchestrator does not waste a turn deriving them. If it still contains a backtick command string or is empty, derive the bookmarks at runtime.

**Workspace root (pre-resolved):** !`jj workspace root 2>/dev/null || pwd`

If the line above resolved to an absolute path, use it as the session-history workspace filter in Phase 1. If it still contains a backtick command string or is empty, derive the workspace root at runtime with `jj workspace root 2>/dev/null || pwd`.
...
- `scripts/validate-doc-claims.py` — mechanical claims validator: cited paths, Jujutsu change/commit IDs, relative links, dangling drafting scaffold (run in Phase 2.45 via the `SKILL_DIR` anchor)
...
RUN_DIR="$WORKSPACE_ROOT/.tmp/rocketclaw/ce-compound/$RUN_ID"
mkdir -p "$RUN_DIR"
```
...
- On `NO-CACHE` (no Jujutsu workspace or no writable workspace-local cache): derive the orientation inline this run and skip `put`.
...
**CRITICAL — the `docs/solutions/` enumeration is NEVER cached; the Related Docs Finder must glob it FRESH every run.** `ce-compound` *writes* new learnings into `docs/solutions/`, so a cached index would miss a doc added moments ago (even a working-copy change). The cached profile supplies only the agnostic orientation above; the `docs/solutions/` search in step 3 always runs against the live tree.
...
#### 2. **Solution Extractor**
   ...
   - **Writes merge-state claims for time.** Cite PR numbers rather than bare Jujutsu change or commit IDs, which may not exist on other workspaces after history rewriting. A "fixed in X" claim requires the fix to be reachable from the current tree; otherwise phrase it as pending ("fix opened in #1608, unmerged as of this writing")
...
#### 4. **Session History** (internal flow after launching the parallel block — only if the user opted in)
   ...
   - Run session discovery, bookmark/legacy-session-label/keyword filtering, scan-window selection, deep-dive selection, and per-session extraction directly inside this skill using `scripts/session-history/`.
   ...
   **Session-history payload — keep tight.** A long, keyword-rich payload licenses widening. Use this shape:
   ...
   - **Pre-resolved context** (only if values resolved cleanly above; otherwise omit): repository name, current Jujutsu bookmarks.
   ...
   - **Filter rule (one line)**: "Only surface findings directly relevant to this specific problem. Ignore unrelated work from the same sessions or bookmark labels."
   - **Output schema**:
   ...
   **Discovery pipeline.** Infer the scan window from the problem topic, starting with 7 days. Run discovery and metadata extraction:
   ...
   SCAN_DAYS="7"
   bash "$SKILL_DIR/scripts/session-history/discover-sessions.sh" "$REPO_NAME" "$SCAN_DAYS" --cwd "$WORKSPACE_ROOT" | tr '\n' '\0' | xargs -0 python3 "$SKILL_DIR/scripts/session-history/extract-metadata.py" --cwd-filter "$WORKSPACE_ROOT"
   ```

   Pi sessions are included when present under `~/.pi/agent/sessions/`; they carry `cwd` like Codex but no legacy branch label. Compare any Claude Code legacy branch label with the current Jujutsu bookmarks. If `_meta.files_processed` is `0`, return `no relevant prior sessions`. If the first pass finds no relevant bookmark/label matches, or if processing Codex or Pi sessions, derive 2-4 keywords from the topic and re-run metadata extraction with `--keyword K1,K2,...`. Keep at most 5 sessions across Claude Code, Codex, Cursor, and Pi, ranked by bookmark/label match, keyword match count, file size over 30KB, and recency. Exclude the current session.

   **Extraction pipeline.** Create `SESSION_DIR="$RUN_DIR/sessions"` with `mkdir -p "$SESSION_DIR"`. For each selected session, write extracted content there:

   ```bash
   SKILL_DIR="<absolute path of the directory containing the SKILL.md you just read>"
   python3 "$SKILL_DIR/scripts/session-history/extract-skeleton.py" --output "$SESSION_DIR/<session-id>.skeleton.txt" < <session-file>
   ```
...
1. **Mechanical claims check (every mode, including headless).** Optionally run `jj git fetch` first (best-effort — skip silently offline; the network is never a correctness dependency). Then run the bundled validator against the written doc:
   ...
   Exit 0 means nothing flagged. Exit 1 means flags to **adjudicate, not auto-fix** — each flagged path, Jujutsu ID, link, or scaffold pattern is fixed, annotated as historical, or confirmed intentional per the reference's adjudication table. A doc may legitimately cite a path deleted by the very fix it documents; a flag is a question, not a failure. If the script cannot be resolved on this platform, apply the reference's manual checklist and say so in the output — never silently skip.

2. **Semantic grounding validator (Full and headless; lightweight skips it).** Dispatch one read-only generic subagent built from the prompt template in the reference, covering the written doc plus any `CONCEPTS.md` entries added or edited this run. It verifies code-behavior claims by quoting the defining source line, merge-state claims against remote truth (`gh` primary, Jujutsu reachability fallback), change-description claims against `jj log`, and internal completeness of countable assertions. Apply its verdicts per the reference (fix contradicted claims from the quoted evidence; soften or drop unverifiable ones; mark offline merge-state checks as degraded), then re-run the mechanical check if the body changed.
...
- Any code-heavy issue → preserve code simplification as a **read-only documentation review**. Inspect the solution draft's code examples and explanatory claims inline, or dispatch a generic subagent seeded with a local prompt only to return suggestions. Do **not** invoke `ce-simplify-code` from this phase and do not mutate product code unless the user explicitly asks for a separate code-simplification pass. Do not use the deleted `code-simplicity-reviewer`.
  Example: review the solution draft's examples for speculative abstractions, redundant wrappers, dead branches, and just-in-case parameters. Apply edits only to the documentation/examples being written by `ce-compound`; leave any unrelated working-copy changes untouched.
...
| ❌ Wrong | ✅ Correct |
|----------|-----------|
| Subagents write product files into `docs/` or edit repository-managed paths | Subagents write only scratch artifacts under `$(jj workspace root)/.tmp/rocketclaw/ce-compound/<run-id>/`, falling back to `.tmp/rocketclaw/ce-compound/<run-id>/`, and return the path; orchestrator writes the one final doc |
...
| Asserting code behavior or merge-state from conversation memory | Read the defining source line before asserting; cite PR numbers over Jujutsu IDs; soften unverifiable claims (Phase 1 extractor rules, re-checked in Phase 2.45) |
...
Subagent Results:
  ...
  ✓ Session History: 3 prior sessions matching the current bookmark, 2 failed approaches surfaced

Grounding Validation:
  ✓ Mechanical check: 14 paths, 2 Jujutsu IDs, 3 links checked — 1 flag annotated as historical
