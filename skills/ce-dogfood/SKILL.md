...
3. **Decide isolation by what you're testing; let `ce-worktree` own the worktree mechanics.** Do not re-derive worktree detection or creation here — `ce-worktree` handles existing-isolation detection, the harness-native tool, attaching to a ref, and the "already checked out" constraint, and reports its decision back. The only call this skill makes is *whether to ask for isolation at all*:
   - **Blank / current-bookmark target:** do **not** isolate — dogfood in place. You are already on the bookmark under test, the fix commits belong on it, and a bookmark should not be edited from two workspaces at once. (If you happen to already be in a worktree/workspace, that is fine — you are simply dogfooding here.)
   - **A PR or a different named bookmark:** this is an existing ref to test without disturbing your current checkout. Offer isolation (platform's blocking question tool). On **yes**, invoke `ce-worktree` to isolate **that target ref** — it attaches a worktree/workspace to the ref (or, if already isolated, checks it out in place; or reports "already checked out at `<path>` — work there" when the ref is live elsewhere). Act on `ce-worktree`'s verdict; the primary checkout is never switched. On **no**, move to the target in place (`gh pr checkout <number>` for a PR, `jj edit <bookmark>` for a bookmark), confirming first if in-progress changes would be disturbed.
...
```bash
# Resolve the trunk to a JJ revset that actually exists. Start from the detected
# default name, then fall back to common names. Prefer the remote bookmark when
# it exists because PR/CI checkouts often have only origin/main, no local main.
DEFAULT=$(gh repo view --json defaultBranchRef -q .defaultBranchRef.name 2>/dev/null)
TRUNK=""
for cand in "$DEFAULT" main master; do
  [ -n "$cand" ] || continue
  if jj log -r "$cand@origin" --no-graph -T 'commit_id ++ "\n"' >/dev/null 2>&1; then
    TRUNK="$cand@origin"; break
  elif jj log -r "$cand" --no-graph -T 'commit_id ++ "\n"' >/dev/null 2>&1; then
    TRUNK="$cand"; break
  fi
done
TRUNK=${TRUNK:-main}

jj diff --name-only --from "$TRUNK"    # what changed
jj diff --git --from "$TRUNK"          # how it changed
```
