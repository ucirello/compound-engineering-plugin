...
5. Do NOT describe, rebase, abandon, or move bookmarks (the orchestrator manages the JJ change if this experiment succeeds)
...
- For Codex dispatch: resolve the workspace root with `jj workspace root` and `pwd -P` fallback, write the filled template under `<workspace-root>/.tmp/rocketclaw/ce-optimize/prompts/`, and pass it to `codex exec --skip-git-repo-check -` via stdin. This command shape remains compatible with Git Bash.
