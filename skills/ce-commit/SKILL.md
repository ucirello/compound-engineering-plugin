...
**In Claude Code**, the five labeled sections below (JJ status, Working-copy diff, Bookmarks, Recent changes, Remote default base) contain pre-populated data. Use them directly throughout this skill -- do not re-run these commands.
...
Use the context above (JJ status, working-copy diff, bookmarks, recent changes, remote default base). All data needed for this step is already available -- do not re-run those commands.

The remote default base value returns a bookmark/base name like `main`. If it returned `__DEFAULT_BRANCH_UNRESOLVED__`, try:
...
JJ has no active branch concept. If no local bookmark points at `@`, explain that a bookmark is required only if the user wants this work attached to a named line of work for push/PR workflows. Ask whether to create a topic bookmark now. Use the platform's blocking question tool: `AskUserQuestion` in Claude Code (call `ToolSearch` with `select:AskUserQuestion` first if its schema isn't loaded), `request_user_input` in Codex, `ask_question` in Antigravity CLI (`agy`), `ask_user` in Pi (requires the `pi-ask-user` extension). Fall back to presenting options in chat only when no blocking tool exists in the harness or the call errors (e.g., Codex edit modes) — not because a schema load is required. Never silently skip the question.

- If the user chooses to create a bookmark, derive the name from the change content, create it with `jj bookmark create <bookmark-name>` or `jj bookmark set <bookmark-name>`, then treat that bookmark as the topic bookmark for the rest of the workflow.
...
If a local bookmark pointing at `@` is `main`, `master`, or the resolved default branch from Step 1, automatically create a topic bookmark before committing. Derive the bookmark name from the change content. If the current change already contains the work, create the topic bookmark on `@` with `jj bookmark create <bookmark-name>` or `jj bookmark set <bookmark-name>` so the diff stays with the commit. If the current change is empty and the work has not started yet, run `jj new <base>` where `<base>` is the default bookmark, then create the topic bookmark. Do not ask whether to create a bookmark — committing directly on the default bookmark is not an option here.
...
Run `jj status` after the commit to verify success. Report the JJ change ID(s) / commit ID(s) and subject line(s).
