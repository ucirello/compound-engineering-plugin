...
**Scope invariant.** Apply only when the working copy *is* what was reviewed — `local-aligned` or standalone. In `pr-remote` / `branch-remote` the working copy is not the reviewed head revision; do not apply — report instead.
...
**Describe and checkpoint when the pre-review working-copy change was empty.** Before applying, record whether `jj diff -r @` is empty. The permanence gate is publication, not a local change description; local jj changes remain private and reversible.

- **Empty before the review:** after applying and verifying, describe the current change and run `jj new` to leave the workspace at a fresh checkpoint. At runtime, inspect nearby change descriptions with `jj log`; project-local conventions take precedence over general guidance. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Apply compatible Go clarity and structure guidance, communicate material context or rationale without restating the diff, and do not impose fixed syntax or examples.
- **Non-empty before the review:** apply but do **not** replace or finalize the existing description; the fixes interleave with the user's in-flight change. The Applied section lists what changed.
...
```json
{
  ...
  "scope": {
    "base": "<common-base revision, pr:NNN marker, or base revision>",
    "branch": "<primary bookmark on @, or empty>",
    "head_sha": "<jj commit ID for @>",
    "pr_url": "<url or null>",
    "files_changed": 0
  },
