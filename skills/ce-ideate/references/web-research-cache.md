...
Files live under `<scratch-dir>/web-research-cache.json`, where `<scratch-dir>` is `<workspace-root>/.tmp/rocketclaw/ideate/<run-id>` (or `$PWD/.tmp/rocketclaw/ideate/<run-id>`), resolved once in SKILL.md Phase 1.
