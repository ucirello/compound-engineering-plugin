# RocketClaw Dogfood Report — <target>

> Diff-scoped browser QA of `<target>` vs `<trunk>` on <YYYY-MM-DD>.
...
<What changed between the selected Jujutsu target and trunk: new features, modified behavior, new/changed routes, views, components, data flows. 2-6 bullets.>
...
<Overall readiness verdict for the selected target. Ready to ship? Caveats? Outstanding blocked items? Record the result of the Phase 5 automated test suite run — a green matrix with a red suite is not "ready.">
