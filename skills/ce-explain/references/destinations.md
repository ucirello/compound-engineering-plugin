...
Offered for HTML output when an artifact-publishing capability is present. Give it the canonical `$RUN_DIR/explainer.html`, follow its current contract, and confirm the returned URL or reference to the user. The capability owns any adaptation needed for its artifact runtime; do not pre-process the HTML unless its contract requires body-only markup.

## Publish publicly to ht-ml.app

This is the preferred HTML publisher when an artifact surface is not selected. ht-ml.app accepts the complete standalone HTML document and works through ordinary HTTP.
...
Proof ingests markdown, so this option renders only when the run resolved `output:md`. Invoke the `ce-proof` skill via the runtime's skill-invocation capability when it is installed, passing the artifact path, a title (`Explainer: <subject>`), and the neutral actor `ai:assistant` / `AI Assistant`; surface the returned share URL. When the skill is not installed but the Proof web API is reachable, POST the markdown per that API with the same neutral actor. On failure: retry once after a short wait, then report plainly that the upload didn't succeed and why, and fall back to the local-file path. One-way publish; the run-dir file stays canonical.
