...
describe("ce-worktree SKILL.md", () => {
  ...
  test("ships no bundled script and no ${CLAUDE_SKILL_DIR} dependence", () => {
    expect(
      ...
      "ce-worktree must not bundle a scripts/ directory — it is now a portable inline VCS guardrail (issue #946). A bundled script reintroduces the cross-platform path-resolution bug class (#943/#764).",
    ).toBe(false)
    expect(
      SKILL_BODY.includes("CLAUDE_SKILL_DIR"),
      "ce-worktree/SKILL.md must not reference ${CLAUDE_SKILL_DIR} — the guardrail uses inline VCS commands only, so it resolves on every platform without a skill-dir variable.",
    ).toBe(false)
    expect(
  ...
  test("detects existing isolation before creating a workspace", () => {
    expect(
      ...
      "ce-worktree/SKILL.md must use `jj workspace list` to detect existing JJ workspace isolation (Step 0).",
    ).toBe(true)
    expect(
      SKILL_BODY.includes("jj workspace root"),
      "ce-worktree/SKILL.md must use `jj workspace root` so workspace operations anchor at the current workspace root.",
    ).toBe(true)
    expect(
  ...
  test("prefers the harness's native worktree tool before the JJ workspace fallback", () => {
    expect(
      /native worktree (primitive|tool)/i.test(SKILL_BODY),
      "ce-worktree/SKILL.md must instruct the agent to prefer the harness's native worktree tool before falling back to JJ workspaces (avoids phantom state).",
    ).toBe(true)
  })

  test("documents an inline JJ workspace fallback under .worktrees", () => {
    expect(
      SKILL_BODY.includes("jj workspace add"),
      "ce-worktree/SKILL.md must document the inline `jj workspace add` fallback.",
    ).toBe(true)
    expect(
      SKILL_BODY.includes(".worktrees/<workspace-name>"),
      "ce-worktree/SKILL.md must place fallback JJ workspaces under `.worktrees/<workspace-name>`.",
    ).toBe(true)
  })
  ...
  test("anchors the JJ fallback at the workspace root before using relative paths", () => {
    expect(
      SKILL_BODY.includes('$(jj workspace root)'),
      "ce-worktree/SKILL.md must anchor the JJ fallback at `$(jj workspace root)` so relative `.worktrees` paths resolve at the workspace root, not a subdirectory CWD.",
    ).toBe(true)
    expect(
  ...
  // PR #948 review: `jj git fetch --remote origin --branch <branch>` can fail with no `origin`
