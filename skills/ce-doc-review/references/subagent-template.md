...
Preserve the schema, evidence, and review semantics. Examples define required substance, not fixed prose. Do not add product branding, generated-by text, or creator, model, provider, tool, agent, runtime, workflow, or co-author attribution.
...
- You are operationally read-only. Analyze the document and produce findings. Do not edit the document, create files, or make changes. You may use non-mutating tools (file reads, glob, grep, `jj status`, `jj diff`, and `jj log` with scoped revsets) to gather context about the codebase when evaluating feasibility or existing patterns.
