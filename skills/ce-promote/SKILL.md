...
- **Merged/active PR** — inspect `jj bookmark list -r 'heads(::@ & bookmarks())'` and identify the unique feature bookmark relevant to the current stack, excluding the repository's default bookmark. When exactly one is relevant, run `gh pr view <bookmark> --json title,body,url 2>/dev/null`; never use argumentless/current-branch PR discovery. If no unique relevant bookmark exists, ask for the PR number and pass that explicit number to `gh pr view`. The title and body usually state the user-facing value.
- **The diff** — `jj diff --from 'trunk()' --to @ --stat` and skim notable changes to ground the claim in what actually changed.
- **Changelog** — the top/`[Unreleased]` entry in `docs/changelog.md`, `CHANGELOG.md`, or similar.
- **Recent changes** — `jj log -r 'trunk()..@' --limit 15 --no-graph -T 'commit_id.short() ++ " " ++ description.first_line() ++ "\n"'` for the feature's change arc.
