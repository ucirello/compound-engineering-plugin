...
Resolve `<root>` **only** in the no-path interactive branch, which discovers the most recent plan under `<root>/plans/`. Every other run reads the document at the path it was handed. So a non-interactive review of `./.tmp/rocketclaw/plan.md`, possibly outside any JJ workspace, never depends on a workspace root or config it does not need.
...
**Verify before any dispatch.** Every resolved path must be readable on disk. If one is not, dispatch **no** personas: reviewers read from the filesystem, so they cannot reach a path that exists only in another revision (issue #925).
