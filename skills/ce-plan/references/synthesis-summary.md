...
**Call outs:**
- Person-name filter works by JSON key (allowlist of identity keys: `printer`, `printer_name`, `owner_name`, `author`), not by name value.
- Promote scans the working-directory snapshot before the copy step, not the prepared copy.
