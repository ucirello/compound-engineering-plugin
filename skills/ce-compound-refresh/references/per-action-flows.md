...
2. Move the file with `mv` (the working copy is already the change; history follows the rename).
...
5. **Delete the subsumed doc.** Do not archive it, do not add redirect metadata — just delete the file. Change history preserves it.
...
4. The orchestrator deletes the original. Change history preserves it.
