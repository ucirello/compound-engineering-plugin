...
3. **Config** — otherwise use the per-skill keys: `plan_model` and `plan_harness` for ce-plan, `brainstorm_model` and `brainstorm_harness` for ce-brainstorm. Read them the **same way this skill's Phase 0.0 (output-mode resolution) resolves `plan_output` / `brainstorm_output`**: reuse the repo root already resolved, else run `jj workspace root` with cwd = that workspace, then apply the ordinary-key rule (`config.local.yaml` then `config.yaml`). Reuse the Phase 0.0 reads if still in hand. Ignore commented (`#`-prefixed) lines. `plan_harness: opencode2` is its own harness, not an alias of `opencode`. When the harness is `opencode2`, `plan_model` is `provider/modelname#variant` and that form is preserved. A model alias (or that provider/model form) selects it; missing / commented / invalid / no file selects none.
...
2. **Claude CLI.** Run the bundled `scripts/elevation-dispatch.sh` worker as a detached job (see Off-host dispatch). Available when `claude` is on PATH. Do not preflight authentication in the host command context: the detached worker's provider-capable call is authoritative, and an authentication failure there follows Recovery. When the resolved `plan_harness` is `opencode2`, the same worker invokes `opencode2` (not `opencode`) with surveyed `opencode2 run` flags and `plan_model` as `provider/modelname#variant`. `opencode2` is its own harness.
...
On the Claude CLI route the worker grants the elevated model read access to only that one directory (via `--add-dir` on the prompt-file's parent), so the handoff files stay readable while sibling scratch in `.tmp` is not exposed:
