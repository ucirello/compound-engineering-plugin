...
Take the `origin` URL from that listing (`name` then URL). Pair any later `gh` call that talks to the underlying Git repo with `GIT_DIR=$(jj git root)` and the workspace root as cwd.
