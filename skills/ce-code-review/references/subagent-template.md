...
**Load-bearing line provenance (conditional evidence).** When the finding's claim depends on line history — `pre_existing`, intentional/historical design, introduced-by-this-diff judgment, or a P0/P1 claim whose severity/confidence depends on authorship or age — append one concise provenance evidence item from targeted `jj file annotate` and `jj log` on the cited line. In `pr-remote` / `branch-remote` scope, query the reviewed head revision, not a mismatched workspace tree. Provenance is an **additional** evidence item; it must not replace the quote-the-line first item at anchors 75/100. Omit provenance when the finding is fully justified from the diff and surrounding code alone. Do not dump full-file annotation or attach provenance to every finding.
...
- **Code that looks wrong but is intentional.** Check comments, Jujutsu change descriptions, PR description, or surrounding code for evidence of intent before flagging. A persona-flagged "missing null check" guarded by an upstream `.present?` call is a false positive.
...
- You are operationally read-only. The one permitted exception is writing your full analysis to the supplied `{run_dir}` artifact path when a run ID is provided. You may also use non-mutating `jj` / `gh` inspection commands to gather evidence. Do not edit project files, move the working copy, describe or finalize changes, push, create PRs, or otherwise mutate workspace or repository state.
...
- **Propose a `suggested_fix` whenever any defensible code change is reachable from the diff and surrounding code.** This is the persona's commitment that "I, the reviewer with the diff and evidence in front of me, can articulate what the fix looks like." The suggested fix becomes the authoritative signal that downstream surfaces use to decide whether the agent can act on the finding. Four rules:
  ...
  - **Imperfect information is not grounds for omission.** When you don't have full context for the optimal fix, propose the most defensible default and name the assumption. Do not omit because "the right answer depends on X" — name the assumption you're making, propose the default, and let the user override.
    ...
    The "I need `<specific input>` before I can implement" framing is a soft punt. The question to ask instead is "what code change would I propose if I had to choose now?" — and propose that, with the assumption named so the user can correct it.
...
(For a large path-materialized review, `{file_list}` and `{diff}` may be **file paths** rather than inline content. When a value above is a path, Read that file to get the full list/diff before reviewing — never treat the path string itself as the content to review.)
</review-context>
```
...
| `{file_list}` | Stage 1 output | Changed-file list — inline, or a materialized file path to Read for a large review |
| `{diff}` | Stage 1 output | The diff to review — inline hunks, or a materialized file path to Read for a large review |
