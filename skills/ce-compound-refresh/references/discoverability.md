...
6. If this check edited an instruction file after Commit already ran, update the current change (`jj describe`) when that change is not yet pushed, or add a small follow-up change (`jj commit -m "<message composed from the standards above>"`) and `jj git push --bookmark NAME` if the bookmark was already pushed so an open PR includes it. Pair any `gh` call with `GIT_DIR=$(jj git root)` in the same shell. If the user chose "don't commit", leave the edits undescribed alongside the rest.
