...
Every write to a Proof doc must identify its author. Two fields carry the agent's identity:

- **Machine ID (`by` on every op, `X-Agent-Id` header):** `ai:agent` — stable, lowercase, and machine-parseable. Appears in marks, events, and the API response.
- **Display name (`name` on `POST /presence`):** `Agent` — human-readable and shown in Proof's presence UI.
...
- Header: `X-Agent-Id: ai:agent` (required for presence; include on ops for a consistent identity)
...
`status: "accepted"` creates the suggestion mark and commits the change in one call. The mark persists with a reject-to-revert affordance. Works with `kind: "insert" | "delete" | "replace"`.
...
curl -s -X POST "https://www.proofeditor.ai/api/agent/$SLUG/presence" \
  ...
  -H "x-share-token: $TOKEN" \
  -H "X-Agent-Id: ai:agent" \
  -d '{"name":"Agent","status":"reading","summary":"Uploaded doc"}'
...
- Always include `by: "ai:agent"` on every op and `X-Agent-Id: ai:agent` in headers for a consistent identity
