...
**Shared-bookmark topology** (`sweep_shared_bookmark: true`): before any source-side write, create a path-limited JJ change for the state file and push it with `jj git push`. A rejected push means another writer won the bookmark — run `jj git fetch`, rebase, re-run `lease-acquire`, and if the lease is still not yours, back off (record `aborted-locked` and stop). Only once your lease is pushed and confirmed do you touch a source.
...
- `gh pr view <validated-ref> --json mergedAt,baseRefName` (merged, base is the default bookmark), or `jj log -r '<validated-id>::<default-bookmark>'` shows the validated id in the default bookmark ancestry.
...
- **Describe the change.** Use a path-limited `jj commit` for ONLY `docs/plans/feedback-sweep-plan.md` plus `<state>` when it is repo-internal (never a broad all-files operation; machine-local state under `/tmp` is never described), with description `docs(sweep): feedback sweep <date>`. A commit failure is reported, not fatal. In local-change mode, never push. In shared-bookmark mode (`sweep_shared_bookmark: true`), run `jj git fetch`, rebase, and `jj git push` the final change.
