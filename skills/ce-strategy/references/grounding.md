...
Inspect revision history with `jj log`, running every JJ command with cwd set to the absolute workspace root obtained through `jj workspace root`. If using `gh` for PR evidence, run it from that same workspace with `GIT_DIR` set to the output of `jj git root`.
