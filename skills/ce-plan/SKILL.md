...
- If updating, revise only the still-relevant sections. Plans do not carry per-unit progress state — progress is derived from JJ/VCS history by `ce-work`, so there is no progress to preserve across edits
...
- Do not include VCS commands, change/commit messages, or exact test command recipes
