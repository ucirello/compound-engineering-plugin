...
Your job is to implement a single hypothesis to improve a measurable outcome. You will modify code within a defined scope, then stop. You do NOT run the measurement harness, describe or bookmark the change, or evaluate results -- the orchestrator handles all of that.
...
5. Do NOT describe or bookmark the change (the orchestrator finalizes a winning JJ change if this experiment succeeds)
...
- For Codex dispatch: resolve `ROOT=$(jj workspace root 2>/dev/null || pwd -P)`, safely create `"$ROOT/.tmp/rocketclaw/ce-optimize"`, create the prompt file there, and redirect it to `codex exec --skip-git-repo-check -` via stdin.
