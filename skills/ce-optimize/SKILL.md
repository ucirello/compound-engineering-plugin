...
The files under `.context/ce-optimize/<spec-name>/` are local scratch state. They survive local resumes on the same machine but are not preserved by JJ changes, bookmarks, or `jj git push` unless the user exports them separately.
...
- **Fresh start**: create `optimize-archive/<spec-name>/archived-<timestamp>` at the old bookmark target, delete the old bookmark, clear the experiment log, and start from scratch

### 0.5 Create Optimization Bookmark and Scratch Space

```bash
jj bookmark create "optimize/<spec-name>" -r @  # fresh start
# For resume instead: jj edit "optimize/<spec-name>"
```
...
- Report which files have working-copy changes
- Ask the user to preserve the current change and start a fresh change with `jj new`, or otherwise move the edits out of scope before proceeding
- Do NOT continue until the working copy has no changes to in-scope files
...
- **Working-copy status**: confirmed no in-scope changes
...
**Workspace backend (`backend: workspace`):**
...
3. Write the filled prompt under the JJ workspace's `.tmp` directory, falling back to local `.tmp` when no JJ workspace is available
4. Dispatch via Codex:
   ```bash
   TMP_ROOT="$(jj workspace root 2>/dev/null || pwd)/.tmp"
   mkdir -p "$TMP_ROOT"
   PROMPT_FILE="$(mktemp "$TMP_ROOT/optimize-exp-XXXXX.txt")"
...
3. **If best improves on current best: KEEP**
   - Inspect the experiment workspace with `jj diff`; include only mutable-scope changes in its JJ change, and treat the experiment as non-improving if no eligible diff remains
   - Compose the winning change description before integrating it. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Repository-local instructions and the repository's actual `git log` syntax always win; apply compatible Go guidance such as a concise imperative subject and explanatory body when useful. Derive any prefix, type, scope, and subject dynamically as `<repository-prefix>`, `<repository-type>`, `<repository-scope>`, and `<change-subject>` rather than imposing a fixed template or example.
   - Set the description with `jj describe -m "<dynamically-composed-message>"` in the experiment workspace, then move `optimize/<spec-name>` to that change with `jj bookmark set "optimize/<spec-name>" -r @`
   - Rebase subsequent experiment workspaces onto the updated optimization bookmark with `jj rebase`; the winner itself is integrated by bookmark movement
   - After integration succeeds, clean up the winner's experiment workspace and bookmark; the described change reachable from the optimization bookmark is the durable artifact
...
4. **Check file-disjoint runners-up** (up to `max_runner_up_merges_per_batch`):
   ...
   - If disjoint: rebase the runner-up change onto the new baseline with `jj rebase`, then re-run full measurement
   - If combined measurement is strictly better: move the optimization bookmark to the rebased runner-up (outcome: `runner_up_kept`), then clean up that runner-up's experiment workspace and bookmark
   - Otherwise: abandon the rebased runner-up with `jj abandon`, log as "promising alone but neutral/harmful in combination" (outcome: `runner_up_reverted`), then clean up its experiment workspace and bookmark
...
6. **Revert all others**: clean up workspaces, log as `reverted`
...
- Revert the experiment (clean up its workspace and abandon its change)
...
The optimization bookmark (`optimize/<spec-name>`) is preserved with all described changes from kept experiments.
