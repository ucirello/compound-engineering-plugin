...
- `unit_receipts`: route, model, detached-process, integration, verification, canonical-revision, and cleanup state for each attempted unit
- `plan_checkpoint`: the disclosed checkpoint revision when the selected plan was the only canonical change, otherwise `null`
- `blockers`
