...
- **The diff** — `jj diff --stat --from <trunk-bookmark> --to @` and skim notable changes to ground the claim in what actually changed.
- **Changelog** — the top/`[Unreleased]` entry in `docs/changelog.md`, `CHANGELOG.md`, or similar.
- **Recent commits** — `jj log -r 'latest(ancestors(@), 15)' --no-graph` for the arc of the change.
