...
# 2. Describe only files related to this logical unit (JJ has no index)
...
**Note:** Incremental commits use messages composed from the standards above, without attribution footers. The final Phase 4 handoff passes `branding:on` so `ce-commit-push-pr` can add generic branding to the PR.
