...
Every `gh` invocation that talks to the underlying git repo runs as `GIT_DIR=$(jj git root) gh …`
(pinned compound in `references/context.md`).
...
bookmark. If `jj log -r <headRefOid> -n 1 --no-graph` succeeds, create a local bookmark at that
revision (`jj bookmark create <headRefName> -r <headRefOid>` when the name is absent; if the name
exists, verify it already points at that revision). If the commit is not reachable, stop with a
residual — `jj git fetch` cannot fetch `refs/pull/<n>/head` or a raw OID. Prefer classifying by PR
number with `gh stack checkout` so this materialization is not required.

## Building

```bash
GIT_DIR=$(jj git root) gh stack init [--base "<trunk>"] "<branch>"...
```
...
```bash
GIT_DIR=$(jj git root) gh stack add "<branch>"
```
...
touch the working copy, so working-copy changes follow onto the new bookmark.
