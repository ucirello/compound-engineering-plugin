...
**Resolve the workspace root.** Pre-resolved at skill load:
!`jj workspace root 2>/dev/null || true`

If the line above is an absolute path, use it as `<workspace-root>`. If it is empty or still shows a backtick command string (a harness that did not pre-resolve), run `jj workspace root` with the shell tool. Read `<workspace-root>/.rocketclaw/config.local.yaml` with the native file-read tool.

Use JJ for repository operations: `jj status`, `jj diff`, `jj log`, and `jj file annotate <path>` for inspection; `jj bookmark` and `jj workspace` for named revisions and working copies; and `jj git fetch` / `jj git push` for remote synchronization. JJ snapshots the working copy and has no staging step; never emulate an index.
...
- `sweep_state_path` — path to the state file, established at setup; fallback `docs/feedback-sweep/state.yml`. A tracked path participates in the sweep change and must not be ignored by the repository. A path under `<workspace-root>/.tmp/rocketclaw/` is machine-local, must be ignored by the repository, and never participates in the sweep change.
- `sweep_lease_ttl_minutes` — single-writer lease staleness threshold; default `60`. Passed to `lease-acquire` in 2a.
- `sweep_shared_bookmark` — bookmark name when tracked state is synchronized across multiple JJ workspaces or machines (see 2a topology); omit for local-change mode.
...
- `<writer>` = an opaque, run-unique writer id, such as `sweep-<YYYY-MM-DD>-<random-suffix>`. Do not encode a product, model, harness, user, or host name. Use the same string for every state-engine call this run.
...
**Shared-bookmark topology** (`sweep_shared_bookmark` is set): before any source-side write, inspect `jj status` and `jj diff --summary` and require the working-copy change `@` to contain only the state file; if it contains anything else, report the isolation problem and stop rather than publishing unrelated work. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Inspect actual message syntax with `git log` and repository history with `jj log`; do not use `git log` for any other purpose. Runtime project instructions and the syntax observed in `git log` always win over compatible Go guidance. Do not impose a fixed prefix, type, scope, subject/body shape, template, or example. Set the lease change description with `jj describe`, create a new empty working-copy change with `jj new`, move the configured bookmark to the lease change with `jj bookmark set <bookmark> -r @-`, then publish it with `jj git push --bookmark <bookmark>`. A rejected push means another writer may have won the bookmark: run `jj git fetch`, inspect it with `jj bookmark list <bookmark>` and `jj log`, and read the fetched state with `jj file show -r <bookmark>@<remote> <state>`. If it contains another live lease, record `aborted-locked` and stop. Only after a missing or stale fetched lease may the dedicated sweep workspace run `jj new <bookmark>@<remote>`, acquire a fresh lease, describe that new lease change under the same message-composition rule, advance the bookmark, and retry `jj git push --bookmark <bookmark>`. Only once the bookmark push succeeds and a fetch confirms the lease do you touch a source.
...
- Download attachments into scratch `<workspace-root>/.tmp/rocketclaw/sweep/<run-id>/`; if workspace-root resolution fails, use `./.tmp/rocketclaw/sweep/<run-id>/`. Raw media never participates in a change. A download failure -> set the item `needs_download` and continue.
...
- `gh pr view <validated-ref> --json mergedAt,baseRefName` (the PR is merged and its base is the default bookmark), or `jj log -r '<validated-sha> & ::<default-bookmark>' --no-graph` (non-empty only when the corresponding change is an ancestor of the default bookmark).
...
- **Describe and finish the change.** Run `jj status` and `jj diff --summary`; require `@` to contain only `docs/plans/feedback-sweep-plan.md` plus `<state>` when it is the tracked state path. Machine-local state under `<workspace-root>/.tmp/rocketclaw/` must remain ignored and absent from the diff. If `@` contains unrelated paths, report the isolation problem and skip describing or publishing rather than including them; there is no staging step. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Inspect actual message syntax with `git log` and repository history with `jj log`; do not use `git log` for any other purpose. Runtime project instructions and the syntax observed in `git log` always win over compatible Go guidance. Do not impose a fixed prefix, type, scope, subject/body shape, template, or example. Use that composed description with `jj describe`, then `jj new`; a describe/new failure is reported, not fatal. In local-change mode, never push. When `sweep_shared_bookmark` is set, run `jj git fetch`, use `jj rebase` to place the completed sweep change onto the fetched shared bookmark when needed, move the bookmark to that change with `jj bookmark set <bookmark> -r <sweep-change>`, and publish with `jj git push --bookmark <bookmark>`.
