...
3. **Check whether the issue already appears fixed on the main branch.** Once you know the affected surface, use read-only `jj log` / `gh` on that area (files, routes, components the symptom touches) to see whether a recent change or merged PR already addresses it. Report this as a field in your finding so the orchestrator does not re-file resolved work.
...
- You are read-only except for the ONE write to your scratch artifact path. Read-oriented `Jujutsu` / `gh` and running the bundled analyzer are permitted; do not edit project files, change branches, change, push, or open PRs.
