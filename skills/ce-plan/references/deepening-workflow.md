...
if ROOT="$(jj workspace root 2>/dev/null)"; then
  ...
  mkdir -p .tmp
  SCRATCH_DIR="$(mktemp -d ".tmp/ce-plan-deepen-XXXXXX")"
fi
echo "$SCRATCH_DIR"
```
...
- Add Jujutsu commands, commit choreography, or exact test command recipes
