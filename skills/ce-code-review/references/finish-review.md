...
Severity, confidence, and cross-reviewer agreement tell you what to do first and what to flag loudly; they do not gate the decision. Downside is controlled through Jujutsu operation history, a visible diff, and the pre-apply checkpoint.
...
**Scope invariant.** Apply only when workspace files are what was reviewed: `local-aligned` or standalone. In `pr-remote` / `bookmark-remote`, report instead.
...
**Review the autofix diff before finishing.** Before describing or reporting applied fixes, diff only the Stage 5c changes against the pre-apply Jujutsu operation/change checkpoint. Run one self-review pass over that diff:
...
2. **Applied (explicit local apply only).** When Stage 5c applied fixes, list them first in an Applied section; each entry carries `#`, file, fix, and reviewer, followed by validation and revision-description status. Report whether an initially empty `@` was described using local conventions or whether changes stayed in the existing working-copy revision. Flag green-but-unverifiable edits prominently.
...
```json
{
  ...
  "scope": {
    "base": "<fork-point revision, pr:NNN marker, or explicit revision>",
    "bookmarks": "<bookmarks pointing to @>",
    "change_id": "<change ID for @>",
    "pr_url": "<url or null>",
    "files_changed": 0
  },
