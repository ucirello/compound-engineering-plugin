...
Environment overrides (defaults in parentheses):
  ROCKETCLAW_PEER_JOBS_ROOT         workspace-local jobs root
  ROCKETCLAW_WORK_RUNS_ROOT         parent work-run directory
...
the active workspace's `.tmp/rocketclaw`. Every read opens the file first (no-follow) and
...
def _workspace_root() -> str:
    try:
        proc = subprocess.run(["jj", "workspace", "root"], capture_output=True, check=False, text=True)
    except OSError:
...
ROCKETCLAW_PEER_POLL_SECS, ROCKETCLAW_PEER_GRACE_SECS.
"""
...
def write_atomic(path: str, data: bytes) -> None:
    tmp = os.path.join(os.path.dirname(path), f".atomic-{os.getpid()}-{os.urandom(8).hex()}")
...
def _require_posix_detach() -> None:
    ...
    if not hasattr(os, "fork") or not hasattr(os, "setsid"):
        ...
        raise RunnerError(
            ...
            "started. On native Windows, use a POSIX-compatible environment."
        )
...
def cmd_start(args, worker_argv) -> int:
    ...
    if not os.environ.get("ROCKETCLAW_PEER_JOBS_ROOT") and not os.environ.get("ROCKETCLAW_WORK_RUNS_ROOT"):
