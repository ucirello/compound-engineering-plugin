...
- **Explicit mutations only.** Never run `gh pr checkout`, `jj edit`, `jj new` onto another revision, or similar working-copy switch commands. Passing a PR number, URL, or branch name selects **review scope**, not permission to mutate the working tree. Uncommitted work can only be reviewed from the checkout that holds it, so to review it on a feature bookmark, stay on that bookmark (or edit it yourself) and pass `base:` or no target.
