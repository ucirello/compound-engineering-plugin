...
validates each entry, resolves path and remote URL sources, enumerates the packs each
...
availability (e.g. an unreachable remote URL source skipped per the warn-and-continue
contract).

`--declared-only` answers the config-only question without cloning or touching the
...
Entry shape (documented subset -- anything else under `packs:` is a loud error):
    ...
    packs:
      ...
      - source: https://github.com/o/r         # remote URL: ref required
        ref: v1.2.0                            # tag, sha, or bookmark
        path: packs                            # optional subfolder (URL sources only)
...
Remote URL sources cache under `<workspace>/.tmp/rocketclaw/ce-packs/<sha256(url\\nref)>`
...
hang. A missing `jj` binary degrades remote URL sources only (each warns and is
skipped); path sources still resolve when `jj workspace root` locates the repo.
...
# --- scratch root (workspace .tmp; CE_PACKS_CACHE_ROOT overrides for tests) ---
...
def _workspace_root() -> str | None:
    """The current workspace root via public `jj workspace root`, else None."""
...
def cache_base() -> str | None:
    ...
    root = os.path.join(base, ".tmp", "rocketclaw", "ce-packs")
    try:
        os.makedirs(root, exist_ok=True)
    except OSError:
        return None
    return root if os.path.isdir(root) and os.access(root, os.W_OK) else None
...
def _ref_present(cwd: str, ref: str) -> bool:
    try:
        proc = _run_jj(
...
def _clone_url(url: str, dest: str, extra: list) -> bool:
    """jj git clone into dest. extra is e.g. ['-b', ref] or ['-t', ref] or []."""
    proc = _run_jj(["--quiet", "git", "clone", "--depth", "1", *extra, url, dest])
    return proc.returncode == 0


def _position_at_ref(cwd: str, ref: str) -> bool:
    """Make the working copy the named ref (materializes tag/sha trees)."""
    try:
        proc = _run_jj(["new", ref], cwd=cwd)
    except subprocess.TimeoutExpired:
        return False
    return proc.returncode == 0
...
def resolve_git_source(url: str, ref: str, warnings: list, label: str) -> str | None:
    ...
    base = cache_base()
    if base is None:
        warnings.append(f"{label}: no writable cache root for remote sources; source skipped")
        return None
    ...
    try:
        placed = False
        try:
            # Bookmark, then tag, then default clone + jj new (sha present in
            # shallow history). jj git clone -b/-t exits 0 even on no match, so
            # presence is checked with jj log -r, not return code.
            for extra in (["-b", ref], ["-t", ref], []):
                _reset_dir(tmp)
                if not _clone_url(url, tmp, extra):
                    continue
                if not _ref_present(tmp, ref):
                    continue
                if extra != ["-b", ref] and not _position_at_ref(tmp, ref):
                    continue
                placed = True
                break
        except subprocess.TimeoutExpired:
            warnings.append(f"{label}: clone timed out after {int(GIT_TIMEOUT)}s; source skipped")
            return None
        if not placed:
...
def enumerate_packs(source_root: str, boundary: str, escaped: list, self_name: str | None = None) -> dict:
    ...
    within (the cloned checkout, the repository, or the source root itself); a
...
def nested_rules_warning(pack_id: str, pack_dir: str, boundary: str) -> str | None:
    ...
    return (
        ...
        " never reads -- move rules to the pack's top level (see Pack layout)"
    )
...
def resolve_entry(entry: dict, repo_root: str, roots: list, warnings: list, errors: list) -> None:
    ...
    if _is_git_url(source):
        if not isinstance(ref, str) or not ref:
            errors.append(f"{label}: remote URL source `{source}` requires `ref:` (tag, sha, or bookmark)")
            return
        if ref.startswith("-") or source.startswith("-"):
            errors.append(f"{label}: remote URL source/ref may not begin with `-`")
            return
        checkout = resolve_git_source(source, ref, warnings, label)
        if checkout is None:
            if tree:
                warnings.append(
                    f"{label}: if the ref contains `/`, tree-URL parsing splits it wrong -- use explicit `ref:` and `path:` fields"
                )
            return
        ...
        git_meta = None
        if ref is not None:
            errors.append(f"{label}: `ref:` is only valid on remote URL sources; path sources are read live")
            return
        if sub_path is not None:
            errors.append(f"{label}: `path:` is only valid on remote URL sources; point `source:` at the directory instead")
            return
        ...
        if os.path.isabs(expanded):
            ...
            if not _within(source_root, repo_real) or _within(source_root, os.path.join(repo_real, ".git")) or _within(source_root, os.path.join(repo_real, ".jj")):
    ...
    if git_meta:
        # Display name for a single-pack remote URL source: the path: subfolder's
...
def _repo_root() -> str | None:
    """The enclosing workspace's top level via public `jj workspace root`.
    None when the working directory is not inside a jj workspace."""
