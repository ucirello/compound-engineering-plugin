...
Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Runtime repo-local instructions and repository-local history syntax always win; apply only compatible Go guidance. Use neutral placeholders when discussing a future change description rather than prescribing a fixed syntax, example, or template.
...
- **`title`** — the artifact's descriptive name with a ` - Plan` suffix,
  ...
  prefix in the title — the `type` field carries the classification.
- **`type`** — the repository's change-description classification.
...
derived from `jj log`, not stored in the doc. Do not introduce one.
