...
POSIX path is behaviorally unchanged:
  ...
  jobs root defaults under the JJ workspace `.tmp` (cwd/.tmp when not a
            JJ workspace), owner-private.
...
# Jobs root is resolved lazily from `jj workspace root` / `.tmp` (see
# `_workspace_tmp`). DEFAULT_ROOT stays None at import so a test that
# patches cwd or CE_PEER_JOBS_ROOT after import still wins.
DEFAULT_ROOT = None
...
def _jj(*args, cwd=None):
    return subprocess.run(["jj", *args], cwd=cwd, capture_output=True, text=True, check=False)


def _workspace_tmp(cwd=None):
    """`<jj workspace root>/.tmp`, or `<cwd>/.tmp` when not a JJ workspace."""
    probe = cwd or os.getcwd()
    try:
        result = _jj("workspace", "root", cwd=probe)
    except OSError:
        return os.path.join(os.path.abspath(probe), ".tmp")
    root = result.stdout.strip() if result.returncode == 0 and result.stdout.strip() else probe
...
def ensure_owned_dirs(base: str, path: str) -> None:
    ...
    if IS_WINDOWS:
        ...
        default_root = os.path.abspath(_workspace_tmp())
...
def _require_detach_support() -> None:
    ...
    before jobs_root_base(), so an unsupported host fails with this clear
