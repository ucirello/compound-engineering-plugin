...
Security posture: the job root is an owner-private directory under the
workspace .tmp/rocketclaw tree. Every read of job state opens the file first (no-follow) and
...
POSIX path is behaviorally unchanged:
  ...
  jobs root defaults under the workspace .tmp/rocketclaw/jobs directory,
            owner-private. The workspace scratch root is the only default.
...
def workspace_jobs_root() -> str:
    """Jobs live under the workspace .tmp tree, or local .tmp outside JJ."""
    origin = os.getcwd()
    try:
        proc = subprocess.run(
    ...
    return os.path.join(root, ".tmp", "rocketclaw", "jobs")
...
def jobs_root_base() -> str:
    ...
    return os.path.abspath(workspace_jobs_root())


def candidate_jobs_root_bases() -> list:
    """The workspace jobs root this invocation resolves, or an explicit override."""
...
def skill_runs_root(skill: str) -> str:
    ...
    if skill == "ce-work" and not os.environ.get("CE_PEER_JOBS_ROOT"):
        return os.path.join(os.path.dirname(jobs_root_base()), skill)
...
def ensure_owned_dirs(base: str, path: str) -> None:
    ...
    if IS_WINDOWS:
        ...
        default_root = os.path.abspath(workspace_jobs_root())
...
def resolve_job_dir(ref: str, skill=None) -> str:
    ...
    if skill is not None:
        ...
        patterns = [os.path.join(root, "*", "*", "jobs", ref) for root in search_roots]
        # ce-work shares the controller's runs root, which is a sibling of the
        # default jobs root (or CE_WORK_RUNS_ROOT), not necessarily below it.
        work_roots = candidate_skill_runs_roots("ce-work")
        search_roots = list(dict.fromkeys([*search_roots, *work_roots]))
        patterns.extend(os.path.join(root, "*", "jobs", ref) for root in work_roots)
