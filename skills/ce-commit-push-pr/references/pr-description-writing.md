...
For current-branch mode, resolve `<base>` in priority order: caller-supplied (`base:<ref>`) → `jj log -r @ --no-graph --template 'bookmarks'` (strip `origin/`) → `gh repo view --json defaultBranchRef --jq '.defaultBranchRef.name'` → try `main`/`master`/`develop` via `jj log -r origin/<candidate>`. If none resolve, ask the user. `<head>` is `@`.
...
```bash
jj git fetch <base-remote> <base>
jj git fetch <base-remote> <head>   # PR mode only: <head> is headRefOid and may not be local
jj log  --oneline "<base-remote>/<base>..<head>"
jj log  --format=fuller "<base-remote>/<base>..<head>"   # full change messages for related-reference discovery
jj diff           "<base-remote>/<base>...<head>"
```

If the change list is empty, report "No changes to describe" and stop.

**Fallback** — use `gh pr diff <ref>` and `gh pr view <ref> --json changes` when local Jujutsu can't reach the refs (fork PR with no matching remote, shallow clone, offline, merge-base on unrelated histories). For GHES configurations that reject SHA fetch but allow `refs/pull/`:

```bash
jj git fetch <base-remote> "refs/pull/<number>/head"
PR_HEAD_SHA=$(awk '/refs\/pull\/[0-9]+\/head/ {print $1; exit}' "$(jj workspace root)/FETCH_HEAD")
```
...
- Match repo conventions visible in recent changes.
...
Before writing the body, make an explicit related-reference pass. Gather candidate work-item references from the user prompt, caller handoff, branch name, full change messages, existing PR body, PR template, plan/debug notes, and visible URLs or IDs already in context. Preserve existing related references when rewriting a PR unless the user asks to remove them.
...
| Other trackers | Use the project's documented closing keyword only when known. | Prefer a full URL or tracker ID under `Related`. | Some trackers parse change messages, PR descriptions, or both. Follow project docs or tracker integration docs when present; otherwise never guess a closing action. |
