...
1. **JJ activity** — `jj log` over the window (subjects, revision IDs, dates, authors), and for the substantial changes, a stat-level view of what they touched. Group obviously-related JJ changes (a feature bookmark change, a fix and its follow-ups) rather than listing them flat.
...
- Why, when a doc or JJ description says so — quoted, with the source (`file:line` or revision ID)
- The main files/areas touched

Order by date. Bundle minor mechanical changes (version bumps, typo fixes) into a single "housekeeping" line rather than enumerating them. If the window is empty — no JJ changes, no doc changes — write nothing and report exactly that.
