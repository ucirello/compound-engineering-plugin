...
The files under `.context/compound-engineering/ce-optimize/<spec-name>/` are local scratch state. They are ignored by the workspace rules, so they survive local resumes on the same machine but are not preserved by JJ changes, bookmarks, or pushes unless the user exports them separately.
...
```bash
   jj new
   jj bookmark create "optimize/<spec-name>" -r @  # or work from the existing bookmarked change if resuming
```
...
Verify no workspace changes to files within `scope.mutable` or `scope.immutable`:

```bash
   jj status
```

Filter the output against the scope paths. If any in-scope files have workspace changes:
- Report which files are dirty
- Ask the user to describe them into a separate JJ change or otherwise move them out of scope before proceeding
...
3. **If best improves on current best: KEEP**
   - Describe/finalize the experiment first so the winning diff exists as a real JJ change before integration
   ...
   - Integrate the finalized experiment change into the optimization bookmark
...
4. **Check file-disjoint runners-up** (up to `max_runner_up_merges_per_batch`):
   ...
   - If disjoint: rebase or duplicate the runner-up JJ change onto the new baseline (for example, `jj rebase -r <runner-up-change> -d <new-baseline>`), re-run full measurement
   ...
   - Otherwise: abandon/revert the applied runner-up change, log as "promising alone but neutral/harmful in combination" (outcome: `runner_up_reverted`), then clean up the runner-up's experiment workspace and bookmark
