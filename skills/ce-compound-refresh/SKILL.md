...
1. **Create CONCEPTS.md (build the concept map)** — seed the repo-wide concept map and submit it through the JJ change/PR flow; skip only the docs/solutions classification phases (Phases 0–4). Read `references/concepts-vocabulary.md` and follow its **Seed goal** and **Scope of a seed** (repo-wide) rules: seed the project's core domain nouns from the declared domain model (schema, core types, primary models, top-level domain docs), each meeting the qualifying bar, the codebase setting the count. Write the preamble (see Phase 4.5), cluster per the organization rules, and run the Discoverability Check so `AGENTS.md`/`CLAUDE.md` surface the new file. Then **enter Phase 5 (Submit JJ Changes)** to describe/push/PR the new `CONCEPTS.md` and any instruction-file edit through the same durable-write flow the refresh uses — do not leave the bootstrap only as an undescribed local working-copy revision.
...
## Phase 5: Submit JJ Changes

After all actions are executed and the report is generated, handle describing and submitting the JJ change. Skip this phase if no files were modified (all Keep, or all writes failed).

### Detect JJ context

Before offering options, check:
1. Which bookmark points at the current working-copy revision (`jj bookmark list -r @`), and whether it is `main`, `master`, or the repo's default bookmark vs a feature bookmark
2. Whether the working-copy revision contains changes beyond what compound-refresh modified (`jj st` and `jj diff --summary`)
3. Recent change descriptions from `jj log` to match the repo's style
...
|---------|---------------|
| On the trunk bookmark | Create a bookmark named for what was refreshed (e.g., `docs/refresh-auth-and-ci-learnings`) with `jj bookmark set <name>`, describe the change, push it with `jj git push --bookmark <name>`, and attempt to open a PR. If PR creation fails, report the bookmark name. |
| On a feature bookmark | Describe the current change; if the bookmark already contains unrelated work, isolate the refresh edits into their own JJ change before submitting. |
| JJ operations fail | Include the recommended JJ commands in the report and continue |

Include only the files that compound-refresh modified in the submitted JJ change. If unrelated edits share the working-copy revision, split or otherwise isolate the refresh edits first; do not include unrelated working-copy edits.

### Interactive mode

First, run `jj bookmark list -r @` to determine the current bookmark. Then present the correct options based on the result. Submit only compound-refresh files regardless of which option the user picks.

**If the current bookmark is main, master, or the repo's default bookmark:**

1. Create a bookmark, describe/push the JJ change, and open a PR (recommended) — the bookmark name should be specific to what was refreshed, not generic (e.g., `docs/refresh-auth-learnings` not `docs/compound-refresh`)
2. Describe the change directly on `{current bookmark name}`
3. Don't submit — I'll handle it

**If the current bookmark is a feature bookmark, clean working tree:**

1. Describe/submit the refresh as a separate JJ change on `{current bookmark name}` (recommended)
2. Create a separate bookmark and submit there
3. Don't submit

**If the current bookmark is a feature bookmark, dirty working-copy revision (other changes):**

1. Isolate and submit only the compound-refresh changes to `{current bookmark name}` (other working-copy edits stay untouched)
2. Don't submit

### Change description

Write a descriptive change description with `jj describe` that:
...
6. **Update the submitted JJ change or create a follow-up change when the check produces edits.** If step 4 or step 5 resulted in an edit to an instruction file and Phase 5 already submitted the refresh changes, include the newly edited file in the same JJ change when it has not been pushed yet, or create a small follow-up JJ change with `jj new` and `jj describe` (e.g., `docs: add docs/solutions/ discoverability to AGENTS.md`, or `docs: add CONCEPTS.md discoverability to AGENTS.md`, or a combined description when both edits landed). If Phase 5 already pushed the bookmark to a remote (e.g., the bookmark+PR path), run `jj git push --bookmark <name>` again so the open PR includes the discoverability change. This keeps the working copy clean and the remote in sync at the end of the run. If the user chose "Don't submit" in Phase 5, leave the instruction-file edits in the current working-copy revision alongside the other refresh changes — no separate submission logic needed.
