...
Start the dev server in the background and log output under `<JJ-workspace-root>/.tmp/ce-polish/`, creating that directory if needed. Probe `http://localhost:<port>` for up to 30 seconds. If it doesn't come up, show the last 20 lines of the log and ask the user what to do.
