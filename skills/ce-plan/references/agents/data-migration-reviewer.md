...
Run this **first** when the caller provides a concrete diff and `db/schema.rb` or `db/structure.sql` appears in that diff. Use the review base revision from caller context (`<review-base>` — JJ change ID, commit ID, or bookmark). **Never assume a default bookmark.**
