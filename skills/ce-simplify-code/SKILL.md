...
argument-hint: "[blank to simplify current working-copy changes, or describe what to simplify]"
---
...
2. **Otherwise, in a JJ workspace**, default to the current working-copy diff (`jj diff`) or, when a bookmark pointing at `@` has an obvious base bookmark or tracked remote bookmark, the diff from that base to `@` (`jj diff --from <base> --to @`). This covers the common case of "simplify everything I've added on this feature bookmark before opening a PR." If no useful base ref exists, use the current working-copy diff.
