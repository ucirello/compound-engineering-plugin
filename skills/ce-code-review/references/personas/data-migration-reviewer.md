...
Run this **first** when `db/schema.rb` or `db/structure.sql` appears in the diff. Use the review base ref from caller context (`<review-base>` — base JJ revision or bookmark). **Never assume `main`.**

```bash
jj diff --from <review-base> --summary -- db/migrate/
```
