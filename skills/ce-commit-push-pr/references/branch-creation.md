...
Local `<base>` may have stale commits (another session/workspace advanced it) or commits the user authored intending to base later work on. Local JJ metadata can't distinguish these — ask when unpushed commits are present.
...
If moving to the fresh remote base would conflict with current working-copy changes, stop and ask whether to carry the current change forward instead. Do not hide the conflict with a temporary-shelving workaround; JJ has already snapshotted the working copy.
