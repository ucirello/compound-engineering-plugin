...
2. **A build selector** — the harness can point a run at a specific source tree of the corpus (a `--plugin-dir`-style override, a configurable skills path, an env var), so two builds are comparable under one runner.
