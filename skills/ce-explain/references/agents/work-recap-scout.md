...
Dispatch context supplies: `{window}` (a date range, relative window, or since-revset), `{repo-root}` (the result of `jj root`), and `{run-dir}` (scratch path for your output file).

## What to gather

Work through these sources for the window, cheapest first:

1. **JJ log** — translate a date window to `mine() & (author_date(after:"<start>") | committer_date(after:"<start>")) & ~empty()` (add matching `before:` predicates for a bounded range), or a since-revset to `mine() & <since-revset>..@ & ~empty()`. Run `jj log -r '<revset>'` for descriptions, change IDs/commit IDs, dates, and authors. For each substantial revision, use `jj show --stat <change-id>` and `jj diff --summary -r <change-id>`. Group obviously-related changes (a feature bookmark's stack, a fix and its follow-ups) rather than listing them flat. Change IDs are the durable identity; include commit IDs to identify the exact revision inspected.
