...
POSIX path is behaviorally unchanged:
  ...
  jobs root defaults under the workspace .tmp (jj workspace root / .tmp,
            or ./.tmp when jj is unavailable), owner-private.
...
# Windows: no geteuid; SID ownership is below. Jobs root is workspace .tmp
# on every platform (jj workspace root / .tmp).
...
def _jj_workspace_root(cwd=None) -> str:
    try:
...
def default_jobs_root(cwd=None) -> str:
    return os.path.abspath(os.path.join(_jj_workspace_root(cwd), ".tmp"))
