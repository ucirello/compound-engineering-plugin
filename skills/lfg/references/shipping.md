...
The shipping steps are the last stretch of the run: record the change, push, PR, CI, and close-out. LFG's SKILL.md defines the shipping precondition (the `jj git remote` check) and the two invocation strings. This file defines everything else: which handoff runs, what LFG passes into it, what it does with the result, and how the run closes out.
...
This records any remaining changes, pushes the bookmark, and opens a pull request, non-interactively because of the `mode:pipeline` token. If it prints a `New concepts:` trailer after the PR URL, record the concept name(s) for step 11. If a PR already exists for the bookmark (check with `GIT_DIR=$(jj git root) gh pr view --json number,url,state 2>/dev/null`), skip PR creation but still record and push any not-yet-recorded changes. Pipeline mode leaves an existing PR body alone by default, so the description context above would be lost there; after the push, invoke `ce-commit-push-pr` again in its description-update mode on that PR with the same context, so the record reaches the reviewer.

**Per the shipping precondition, when no remote is configured, do NOT invoke `ce-commit-push-pr` or a project-defined shipping process.** The default's change step pushes unconditionally (`jj git push --bookmark <name>`), so a literal invocation would still hit the impossible push. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Repository-local syntax from project instructions and `git log` ALWAYS wins when it differs from Go guidance. Apply compatible Go guidance to quality/clarity/structure without replacing local syntax. Then record the files this run changed, by name (the work source's files, the review fixes, and any captured learning), with `jj commit -m "<message composed from the standards above>"`, and skip the push and PR creation entirely. Never snapshot the entire working copy: a file that was already dirty before the run, or one the defect route's `pre_fix_scope` lists, stays out of the change and is named in the report.
