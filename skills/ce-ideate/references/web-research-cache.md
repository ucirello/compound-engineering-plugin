...
Files live under `<scratch-dir>/web-research-cache.json`, where `<scratch-dir>` is `<workspace-root-or-current-working-directory>/.tmp/rocketclaw/ce-ideate/<run-id>`, resolved once in SKILL.md Phase 1.
...
```bash
SCRATCH_ROOT="<workspace-root-or-current-working-directory>/.tmp/rocketclaw/ce-ideate"
...
- **Repo mode:** the focus hint plus a stable repo discriminator. This keeps the cache key meaningful when focus is empty — two bare-prompt invocations in the same repo legitimately share research, while copied or relocated cache material remains distinguishable. Resolve the discriminator with this fallback chain and hash the result (first 8 hex chars of sha256 is sufficient):
    1. Parse `jj git remote list` into remote name and URL, discard blank URLs, and select one canonical URL: `upstream` first, then `origin`, then the first remaining nonempty URL in deterministic name order. Hash the selected URL only, not the command's entire output (which changes when unrelated remotes are added or reordered).
    2. If no nonempty remote URL exists, use the absolute `jj workspace root` and explicitly record that the cache key used the workspace-root fallback.
    3. If this is not a JJ workspace, use the current working directory's absolute path and explicitly record that final fallback.
