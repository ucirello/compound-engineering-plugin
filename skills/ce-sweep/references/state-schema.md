...
| --- | --- | --- |
| local-commit mode (default) | Single writer **per workspace**. | The lease serializes overlapping sweeps in the same workspace (e.g. a cron sweep and a manual one). The file is written in-tree (and may be committed locally). No cross-machine guarantee. |
| pushed-shared-branch | One writer **per repo**. | The state file lives on a shared bookmark multiple workspaces push to. `lease-acquire` must be committed, pushed, and confirmed (`jj git fetch` back and verify our writer won) **before any source-side write**. This makes the lease a repo-wide mutex across machines. |
...
and never committed (the skill's commit step includes only the state file and the
plan as filesets).
