...
Dispatch-infrastructure recovery belongs to the host: it stays on the same route only while each failure is new and plausibly recoverable and the derived shared deadline remains. Provider-failure retry belongs to the worker that owns the route. The current review workers spend that allowance only on their exact overload condition, inside the original route deadline; once a provider no-review outcome reaches the host, the host never restarts that peer. This keeps a restarted worker from acquiring a fresh allowance and turns repeated or terminal provider failures toward the skill's coverage fallback.
...
**Correct (implemented):** on a dispatch-*infrastructure* crash, re-run the *same* resolved route by hand with target/model + read-scope + independence frozen; keep going only while each failure is new and the derived shared deadline holds; stop on a repeat failure or spent deadline; then degrade to the skill's specific fallback (solo POV / in-process reviewer) and name the exact coverage lost.
