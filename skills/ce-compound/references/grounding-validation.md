...
- **Merge-state claims** ("fixed in #1608", "landed", "shipped") verify against **remote truth** — the workspace may predate a merge, so `gh pr view` (or the tracker equivalent) is primary and local Jujutsu reachability is only the fallback. The script's staleness information tells you how much to distrust the local tree for this category.
- **Change-description claims** verify against `jj log` for the referenced change or commit.
...
| path missing here, exists at upstream | Stale workspace | Verify the claim against upstream; annotate if the doc implies the file is present locally |
...
| Jujutsu change/commit ID does not resolve | Fabricated or from another repository | Replace with the PR number, or drop |
| ID reachable from `@` only | Local-only change whose commit ID may be rewritten | Replace with the PR number |
| ID reachable from the upstream bookmark only | Workspace predates the merge | Keep, with a temporal qualifier; verify the landed claim via `gh` |
| ID exists but is unreachable | Rewritten-away change | Replace with the PR number |
...
If the script cannot be resolved on this platform, apply its checks manually at the same scope — scan the body for cited paths that don't exist, Jujutsu change/commit IDs, `Learning(s) N` / `{{…}}` scaffold, and broken relative links — and note in the run output that the check was manual. Do not silently skip.
...
Grep, Glob, `jj` (non-mutating), `jj git` interoperability, and `gh` when available.
...
2. MERGE-STATE CLAIMS — assertions that a change landed ("fixed in",
   ...
   reachability from the upstream remote bookmark. Verdict: verified,
   contradicted (e.g. PR open, not merged), or unverifiable (offline / no
   gh) — mark unverifiable as "degraded", do not guess.

3. CHANGE-DESCRIPTION CLAIMS — assertions quoting or summarizing a Jujutsu
   change description. Resolve the ID and inspect its description with jj log.
   Verdict: verified, contradicted, or unverifiable.

4. INTERNAL COMPLETENESS — countable assertions ("six PRs", "three root
