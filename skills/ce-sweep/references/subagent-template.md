...
<workspace-root>
Use this absolute directory as cwd for every jj and gh call:
{workspace_root}
Supply GIT_DIR="$(jj git root)" on every gh invocation.
</workspace-root>
...
- Analyze only. You are read-only except for the single write to {scratch_artifact_path}.
  ...
  project files, change bookmarks or working-copy revisions, commit, push, or open PRs.
...
| `{workspace_root}` | Orchestrator | Absolute workspace root resolved via `jj workspace root`; cwd for JJ and gh calls. |
