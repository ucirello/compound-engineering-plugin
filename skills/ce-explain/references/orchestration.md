...
Resolve explicit subagent and harness settings from `.rocketclaw/config.yaml` with `.rocketclaw/config.local.yaml` overrides before selecting delegation. Explicit routing wins. On OpenCode V2, when no explicit subagent or alternative-harness routing applies and `opencode.models` plus native subagents with an optional model are available, discover exact model IDs and variants and dispatch natively with `provider/model#variant` at the extraction tier. Never guess an ID or silently reuse the current model instead of discovering a suitable model. Otherwise preserve configured routing and the capability fallbacks below.
...
Use public JJ commands for repository evidence, with cwd set to the absolute workspace root for every subprocess. Use `jj log` for history, `jj diff -r <revision>` for a change, and `jj diff --from <base> --to <head>` for an endpoint comparison. A JJ range such as `main..@` selects changes, not Git's endpoint syntax. For an implicit latest change or PR/bookmark lookup, check whether `@` is empty and use `@-` when it is the substantive change. Determine workspace membership with `jj workspace list` and `jj workspace root --name <name>`; do not inspect repository internals. Run `gh` from the correct workspace with `GIT_DIR` set to the result of `jj git root` there.
