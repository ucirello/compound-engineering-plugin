...
# scope; private prompt/result scratch stays under workspace `.tmp`.
#
...
adapter_argv() {
  case "$1" in
    ...
    opencode)
      ...
      ;;
    opencode2)
      # Distinct from `opencode` (V1). Flags from `opencode2 run --help`:
      # --format json, --file, --model provider/model#variant, --standalone.
      # No V1 --dir / OPENCODE_* overlay; cwd is READ_ROOT via run_timeout_cmd.
...
case "$RUN_DIR_RESOLVED/" in
  "$REPO_ROOT/.tmp/"*) ;;
  "$REPO_ROOT/"*) skip "run-dir must be under $REPO_ROOT/.tmp or outside the repository" ;;
esac
...
if [ -n "${CROSS_MODEL_SCRATCH_PARENT:-}" ]; then
  ...
  WS_ROOT="$(cd "$REPO_ROOT" && jj workspace root 2>/dev/null || printf '%s' ".")"
  SCRATCH_PARENT="$WS_ROOT/.tmp"
fi
...
case "$SCRATCH_PARENT/" in
  "$REPO_ROOT/.tmp/"*) ;;
  "$REPO_ROOT/"*) skip "private scratch parent must be under $REPO_ROOT/.tmp or outside the repository" ;;
esac
...
parse_opencode2_events() {  # <logfile> <outfile>
  # opencode2 --format json is a distinct envelope from V1. Accept a shaped
  # object, then recover a POV from the log. Never call the `opencode` binary.
  [ -s "$1" ] || return 1
  if jq -e '.' "$1" > "$2" 2>/dev/null && pov_shaped "$2"; then return 0; fi
  local text
  text="$(jq -rs '[.[] | select(.type=="text") | (.part.text // empty)] | join("")' "$1" 2>/dev/null)" || text=""
  if [ -n "$text" ] && printf '%s' "$text" | jq -e '.' > "$2" 2>/dev/null && pov_shaped "$2"; then return 0; fi
