...
If the contents have an **uncommented** top-level `rocketclaw_promote_spiral_optout: true` line, **skip Path 0** and go straight to Path B. **Ignore commented lines** — a `# rocketclaw_promote_spiral_optout: true` example is documentation, not an opt-out (a naive substring match would wrongly suppress the offer for a project that kept the default template). Otherwise, offer setup.

### Ask

Use the platform's blocking-question tool. If no blocking question capability exists or the call errors, present the same options as a numbered list in chat and wait for a reply — never silently skip.

For the **unauthed** state, run `spiral login --json` (CLI >= 1.8.0): it's non-blocking and the API key never passes through the agent. Share the returned `auth_url`; the user approves in a browser and the credential is delivered server->CLI. The blocking question is mainly the escape hatch.

Use the question stem to teach the mechanic, offer the escape hatch, AND disclose that declining is durable (so the permanent side effect isn't hidden behind a transient-sounding label): "Spiral personalizes and humanizes the copy in your voice. [It's installed but not signed in / It isn't installed yet] — sign in now, or draft directly without Spiral? (Declining drafts your copy now and won't bring up Spiral again in this project; you can set it up anytime by asking.)"
...
- **Sign in to Spiral** (installed, unauthed) — run `spiral login --json`. It's non-blocking, and the **API key never touches the agent** (the token is exchanged server->CLI via a device-code flow). Parse the JSON `status`:
  ...
  - **Never have the user paste an API key into chat** — the agent never needs to handle the key.
  - **Older CLI (< 1.8.0):** if `spiral login --json` returns the legacy `API key required ... --token` text instead of JSON, suggest `npm i -g @every-env/spiral-cli@latest`, or have the user run `spiral login` themselves in their terminal (browser sign-in) and re-check `spiral auth status`. If they would rather not, go to Path B.
...
Resolve the workspace root with `jj workspace root`, then add `rocketclaw_promote_spiral_optout: true` as a top-level key to `<root>/.rocketclaw/config.local.yaml`, using the native file-write/edit tool:

- **File already exists:** ensure an **uncommented** `rocketclaw_promote_spiral_optout: true` line is present — add one (or uncomment the example) unless an uncommented one already exists. A commented `# rocketclaw_promote_spiral_optout: true` does **not** count as present; leaving only the comment would let the comment-ignoring read path re-prompt next run.
...
After recording, confirm it in one line so the write isn't silent and the user knows how to undo it — e.g. "Got it — I won't bring up Spiral here again (saved to `.rocketclaw/config.local.yaml`, kept out of version control). Want it back later? Just ask, or remove the `rocketclaw_promote_spiral_optout` key." Keep it to a single line; don't belabor it.
