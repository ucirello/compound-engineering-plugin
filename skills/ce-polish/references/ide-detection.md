...
Codex (Claude Agent SDK, Antigravity CLI (`agy`), etc.) do not yet expose an embedded-browser handoff. For these platforms, polish falls through to the terminal path (print the URL). When a convention emerges, add a new row to the detection table above.
