...
2. `cross_model_peer:` from the two repo RocketClaw config files (`config.local.yaml` then `config.yaml`). Apply the ordinary-key rule: first active supported target wins; an invalid value continues to the next layer, then step 3. Supported targets include `opencode2` as its own alternative, distinct from `opencode`.
