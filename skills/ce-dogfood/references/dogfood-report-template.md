...
> Diff-scoped browser QA of `<bookmark>` vs the trunk. Generated by `ce-dogfood` on <YYYY-MM-DD>.
...
- **Options:** refine the rule (writable pack: through `ce-compound`; remote/URL-sourced pack: upstream change and a `ref` bump) / retire it
