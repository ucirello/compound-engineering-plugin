...
| E20 linked-checkout sibling | ce-work is itself running in an existing linked workspace and selects external implementation for one unit | Create a new **sibling** with `jj workspace add`, place it under `$(jj workspace root)/.tmp/ce-work/<run-id>/` rather than beneath the active checkout, base it at the recorded clean canonical commit id, and keep canonical fold-in host-owned. Do not reject the route merely because the active checkout is already a workspace, and do not create a nested workspace. |
...
| E40 sandboxed worker no-commit | A Codex or Cursor unit has finished files and scoped checks in its sibling workspace; the worker is about to `jj commit`/`jj describe` | Do not instruct the worker to rewrite the working-copy change. Leave the working copy undescribed and treat completion as files plus scoped checks. Host `terminalize` snapshots the tree. A Codex sandbox `EPERM` on a socket bind or peer-credential probe is not proof the host lacks the capability. |
