...
- `Procfile` or `Procfile.dev` exists at the project root
...
- **Multiple web processes:** some Procfiles split web traffic across multiple processes (API + frontend). Polish can only open one URL — users with multi-web setups should author `.workspace/launch.json` explicitly to select which process is "the dev server" for polish.
