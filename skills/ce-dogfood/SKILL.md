...
- Never switch the primary workspace out from under the user. This skill decides only whether to offer isolation — no for a blank or current-bookmark target (you are already on it), yes for a PR or another named ref — and `ce-worktree` handles the mechanics and reports the verdict. On a declined offer, check the target out in place, confirming first if the current working-copy change would be disturbed.
- Screenshots and other transient artifacts go under the workspace `.tmp` (`mkdir -p "$(jj workspace root)/.tmp"` then `mktemp -d "$(jj workspace root)/.tmp/ce-dogfood-XXXXXX"`; if not a jj workspace, `mkdir -p .tmp` then `mktemp -d ".tmp/ce-dogfood-XXXXXX"`), never the repo root; copy one in only to embed it in the report.
...
`ce-dogfood` is an orchestrator: prefer an existing RocketClaw skill over re-deriving its behavior. Isolate a PR or named-bookmark target with `ce-worktree`; take a non-obvious root cause to `ce-debug`; record each fix as a JJ change with `ce-commit`; capture a reusable lesson with `ce-compound`.
