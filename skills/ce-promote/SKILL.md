...
**This skill drafts only. It never posts, publishes, changes repository state, or opens PRs.** Posting is a human action. The output is always drafts for you to review, edit, and ship yourself.
...
- **Merged/active PR** — `gh pr view --json title,body,url 2>/dev/null` for the active GitHub PR. The title and body usually state the user-facing value.
- **The diff** — `jj diff --stat --from 'trunk()' --to @` and skim notable changes to ground the claim in what actually changed. If `trunk()` is unavailable, use the repository's configured mainline bookmark rather than assuming a fixed name.
