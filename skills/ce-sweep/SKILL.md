...
- `sweep_state_path` — path to the state file, established at setup; fallback `docs/feedback-sweep/state.yml`. A repo-internal path means change-description mode (the state file is included each run and must not be ignored); a path outside the repo (e.g. under `/tmp`) means machine-local mode (the state file is never included — only the plan is).
- `sweep_lease_ttl_minutes` — single-writer lease staleness threshold; default `60`. Passed to `lease-acquire` in 2a.
- `sweep_shared_branch` — persisted key; `true` when the state file lives behind a shared bookmark multiple workspaces push to (see 2a topology); default `false`.
...
**Shared-bookmark topology** (`sweep_shared_branch: true`, persisted key): before any source-side write, include the state file in the current JJ change, describe it, and run `jj git push`. A rejected push means another writer won the bookmark — run `jj git fetch`, reconcile with the pushed state, re-run `lease-acquire`, and if the lease is still not yours, back off (record `aborted-locked` and stop). Only once your lease is pushed and confirmed do you touch a source.
...
For each `fix_pending` item, resolve its claimed fix ref and verify it merged to the default bookmark. The fix ref originates from untrusted feedback content (a thread claim, an analyzer-extracted reference), so **validate its shape before it reaches any `jj`/`gh` command**: accept only a bare PR number (`#?\d+`) or a provider SHA (`[0-9a-f]{7,40}`), and treat anything else as an unresolved claim (leave the item open). This blocks argument/flag injection into the shell command.
- `gh pr view <validated-ref> --json mergedAt,baseRefName` (merged into the GitHub default branch), or `jj log -r 'ancestors(<validated-sha>) & ancestors(<default-bookmark>)'` to verify the provider SHA is reachable from the default bookmark.
...
- **Describe the change.** Ensure the current JJ change includes ONLY `docs/plans/feedback-sweep-plan.md` plus `<state>` when it is repo-internal (machine-local state under `/tmp` is never included), then run `jj describe -m "docs(sweep): feedback sweep <date>"`. A describe failure is reported, not fatal. In local mode, never push. In shared-bookmark mode (`sweep_shared_branch: true`, persisted key), run `jj git fetch`, reconcile if needed, and `jj git push` the final change.
