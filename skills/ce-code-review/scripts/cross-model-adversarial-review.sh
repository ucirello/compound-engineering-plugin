...
#                   `jj diff --from <base-ref> --to @` in the current repository
...
# stays private under RUN_DIR and is published only after normalization.
# NEVER emit: codex without `-s read-only`; grok `--always-approve` /
# `--permission-mode bypassPermissions`; cursor-agent `-f` / `--force` / `--yolo`.
adapter_argv() {
  case "$1" in
    codex)
      printf '%s\0' codex exec - -C "$PEER_WORKDIR" -s read-only --json \
...
[ -d "$REPO_ROOT/.jj" ] || skip "not inside a Jujutsu workspace; skipping"
...
[ ! -L "$PRIVATE_DIR" ] || skip "unsafe workspace-local private-dir symlink; skipping"
(umask 077; mkdir -p "$PRIVATE_DIR") || skip "cannot create workspace-local private dir; skipping"
[ -O "$PRIVATE_DIR" ] || skip "workspace-local private dir is not owned by the current user; skipping"
chmod 700 "$PRIVATE_DIR" || skip "cannot secure workspace-local private dir; skipping"
BASE_PROMPT="$PRIVATE_DIR/xmodel-base"
PROMPT_FILE="$PRIVATE_DIR/xmodel-prompt"
PEERLOG="$PRIVATE_DIR/xmodel-log"
...
PEERERR="$PRIVATE_DIR/xmodel-err"
RAW_DIR="$PRIVATE_DIR/raw"
[ ! -L "$RAW_DIR" ] || skip "unsafe workspace-local raw-dir symlink; skipping"
(umask 077; mkdir -p "$RAW_DIR") || skip "cannot create workspace-local raw-out dir; skipping"
[ -O "$RAW_DIR" ] || skip "workspace-local raw dir is not owned by the current user; skipping"
chmod 700 "$RAW_DIR" || skip "cannot secure workspace-local raw dir; skipping"
...
jj --repository "$REPO_ROOT" --no-pager --color=never diff --from "$BASE" --to @ > "$DIFF_SOURCE" 2>/dev/null || skip "cannot stage reviewed diff; skipping"
...
[ "$DIFF_BYTES" -gt 0 ] || skip "no changes between '$BASE' and the working-copy revision; nothing to review; skipping"
DIFF_FILES="$(jj --repository "$REPO_ROOT" --no-pager --color=never diff --from "$BASE" --to @ --name-only 2>/dev/null | wc -l | tr -d ' ')"
...
compose_prompt_codex() {
  ...
  if [ "$LARGE_DIFF_MODE" = true ]; then
    ...
    printf '\nRun `jj diff --from %q --to @` in this repository (read-only).\nReview ONLY the changes in that diff.\n' "$BASE" >> "$PROMPT_FILE"
  fi
}
...
compose_prompt_embedded() {
  ...
  printf '\nReview ONLY the change below (the output of `jj diff --from %q --to @`). You may Read repository files for context but cannot mutate the tree.\n' "$BASE" >> "$PROMPT_FILE"
...
compose_large_diff_instruction() {
  ...
  if [ "$access_mode" = codex ]; then
    printf 'Use selective `jj diff --from %s --to @ <path>` calls for exact hunks; do not load the whole diff.\n' "$BASE" >> "$PROMPT_FILE"
  else
...
run_provider() {
  ...
  rm -f "$OUT"
  if [ -s "$RAW_OUT" ]; then
    _norm="$RAW_DIR/adversarial-$provider.norm.json"
