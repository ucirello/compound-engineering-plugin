...
describe("ce-compound non-interactive depth contract", () => {
  ...
  test("routes explicit lightweight depth without prompts or subagents", () => {
    ...
    expect(skill).toMatch(/[Nn]on-interactive lightweight[^\n]+no subagents/i)
    expect(skill).toContain("Documentation complete (non-interactive lightweight mode)")
    expect(skill).toContain("In full non-interactive mode, **do not edit instruction files**")
    expect(skill).not.toContain("In full non-interactive mode, apply the edit directly")
  ...
  test("keeps full-only validation out of lightweight runs", () => {
    expect(skill).toContain("Semantic grounding validator (Full mode, including non-interactive Full; lightweight skips it)")
  ...
  test("routes non-interactive Lightweight past the interactive completion block", () => {
    ...
    expect(lightweightSection).toMatch(/In non-interactive Lightweight, do not emit this interactive block[^\n]+Non-interactive mode/i)
    expect(skill.match(/Documentation complete \(non-interactive lightweight mode\)/g)).toHaveLength(1)
  })
