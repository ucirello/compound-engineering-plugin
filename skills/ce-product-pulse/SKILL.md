...
Default to the current harness's blocking question tool. Fall back to numbered options on the host's user-visible chat surface only when no blocking tool exists or the call errors, not merely because its schema must be loaded. Never silently skip the question.
...
**Resolve ordinary configuration keys from the two repo files.**
...
**Read config.** Resolve `<repo-root>` with `jj root`, then apply the ordinary-key rule above. If the root cannot be resolved or `pulse_product_name` is unset in both layers, treat this as a first run. Otherwise extract values for the `pulse_*` keys listed under "Config keys" below.
