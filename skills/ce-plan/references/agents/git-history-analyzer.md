...
**Tool Selection:** Use native file-search/glob (e.g., `Glob`), content-search (e.g., `Grep`), and file-read (e.g., `Read`) tools for all non-JJ exploration. Use shell only for JJ commands, one command per call. Use `jj git` only when remote interoperability is required.
...
2. **Code Origin Tracing**: Run `jj file annotate <file>` to trace the origins of specific code sections through JJ changes.
...
- Look for patterns in both code changes and JJ change descriptions or commit messages.
