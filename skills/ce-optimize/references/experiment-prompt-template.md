...
5. Do NOT describe or bookmark the JJ change (the orchestrator handles accepted changes)
6. Do NOT modify files outside the mutable scope
7. When done, run `jj status` and `jj diff --stat` so the orchestrator can see your changes
...
- For Codex dispatch: write the filled template under `$(jj workspace root)/.tmp/rocketclaw/ce-optimize/` and pipe it via stdin; if root discovery fails, use `.tmp/rocketclaw/ce-optimize/` under the current directory.
