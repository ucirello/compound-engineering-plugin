...
Scratch lives under `$(jj workspace root)/.tmp` (or `./.tmp` when `jj workspace root` is nonzero) — never `.context/`. Generate one 8-hex `<run-id>` and reuse it for the cache and for every checkpoint.
