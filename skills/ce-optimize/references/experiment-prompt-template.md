...
- For Codex dispatch: resolve `<workspace-root>` with `jj workspace root` (fall back to the current directory when JJ is unavailable), atomically write the filled template under `<workspace-root>/.tmp/rocketclaw/ce-optimize/<spec-name>/prompts/` after creating its parent directories, and redirect it to `codex exec -` via stdin.
