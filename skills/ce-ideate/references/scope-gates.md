...
| Phase | Delta |
|---|---|
| 0.3 mode | **Deterministic — skip Decision 1/2.** CWD inside a JJ workspace (`jj workspace root` succeeds) → repo-grounded; otherwise elsewhere-software. Never elsewhere-non-software: with no subject there is no naming/narrative/personal intent to infer. No ambiguity-confirmation question. |
