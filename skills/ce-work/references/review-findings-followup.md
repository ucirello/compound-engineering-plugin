...
`ce-code-review` is invoked here with `mode:agent`, so it is **review-only** in this context — it reports findings and writes artifacts and does not mutate the working copy, describe changes, push, or file tickets. **The caller owns apply/fix policy.** (In its own default/interactive mode the review applies safe fixes itself; that path does not apply here.)
...
```
ce-code-review mode:agent plan:<plan-path> base:<base-revision-or-revset>
```
...
- `base:` — when the diff base is already resolved on the current working copy; omit when reviewing a PR number/URL or standalone current bookmark.
...
**After each wave:** orchestrator reviews diffs (scope = assigned `#` only), runs tests (`requires_verification: true` on any applied finding → at least targeted tests; multi-file → broader suite), commits (`fix(review): apply findings #…`) unless workspace-isolated subagents integrate per Phase 1. Repeat until all batches complete.
