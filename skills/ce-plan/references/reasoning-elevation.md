...
3. **Config** — otherwise use the per-skill keys: `plan_model` for ce-plan, `brainstorm_model` for ce-brainstorm, and the matching optional harness key `plan_harness` / `brainstorm_harness`. Read them the **same way this skill's Phase 0.0 (output-mode resolution) resolves `plan_output` / `brainstorm_output`**: reuse the repo root already resolved, else run `jj workspace root`, then apply the ordinary-key rule (`config.local.yaml` then `config.yaml`). Reuse the Phase 0.0 reads if still in hand. Ignore commented (`#`-prefixed) lines. A model alias selects it; missing / commented / invalid / no file selects none.

   **`plan_harness` / `brainstorm_harness`:** the only supported value is `opencode2`. It is a distinct harness from `opencode` and is not backward compatible — `opencode` here is invalid, not an alias. When the harness is `opencode2`, the matching model key is `provider/modelname#variant` (variant is `#variant` on the model token, not a separate flag). Any other harness value selects none.
...
When elevation is active, resolve an adapter in this fixed order and use the first that serves the requested model. **`opencode2` is its own branch**, never folded into `opencode` or the Claude CLI path.

When `plan_harness` / `brainstorm_harness` is `opencode2`:

1. **opencode2 CLI.** Run the bundled `scripts/elevation-dispatch.sh` worker as a detached job with harness `opencode2` (see Off-host dispatch). Available when `opencode2` is on PATH. The worker runs `opencode2 run --standalone --auto --model provider/model#variant --format json --file PROMPT` with cwd = `jj workspace root` (no `--dir`). Do not preflight authentication in the host command context: the detached worker's provider-capable call is authoritative, and an authentication failure there follows Recovery.
2. **Inline on the session model.** The always-available fallback. Do not fall through to the Claude CLI or to v1 `opencode` — those are different harnesses.

When no harness key selected `opencode2`:
...
Hand over the working context as **file paths the subagent reads itself**, never a re-narrated prose brief. Create **one private per-run handoff directory** under workspace `.tmp` and write the prompt-file and every evidence file into *that* directory:

```bash
workspace_root=$(jj workspace root) || workspace_root="."
mkdir -p "$workspace_root/.tmp"
HANDOFF_DIR="$(mktemp -d "$workspace_root/.tmp/ce-elevation-XXXXXX")"
```

On the Claude CLI route the worker grants the elevated model read access to only that one directory (via `--add-dir` on the prompt-file's parent), so the handoff files stay readable while the rest of the workspace `.tmp` tree — other same-user scratch and credentials — is not exposed:
