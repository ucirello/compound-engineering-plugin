...
**Scope:** common ancestor with `<base-bookmark>` -> working-copy revision (14 files, 342 lines)
...
Jujutsu state: described as `<repository-conforming-review-fix-description>` and isolated with `jj new` because the working-copy revision was initially empty.
...
Detail lines for Pre-existing and history-dependent P0/P1 findings may include the same short change-id provenance string the artifact `evidence` carries when history was load-bearing; do not dump full-file annotation output into the report.
...
- **Applied section (explicit local apply only)** -- when Stage 5c was authorized and applied fixes, list them first, before the severity tables, as `# | File | Fix | Reviewer`, followed by a one-line validation outcome and the **Jujutsu state**: described with `<repository-conforming-review-fix-description>` and isolated with `jj new` when the working-copy revision was initially empty, or left in the existing change when it was not. A fix spanning multiple files is one row with one stable `#`. Applied findings appear only here, not in severity tables. Omit when local apply was not authorized or nothing was applied.

Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards.

Repository-local instructions and observed history take precedence. Where compatible, use imperative mood, capitalize the initial word, omit a terminal period in the subject, keep the subject concise, and reserve the body for motivation or important consequences.
...
- **Deployment Notes section** -- key checklist items from the `deployment-verification-assistant` local prompt asset. Omit if the prompt did not run. Schema drift surfaces as `data-migration` findings — no separate section.
