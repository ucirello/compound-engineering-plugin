...
1. Workspace root (resolve via `jj workspace root`, with `pwd -P` fallback).
2. `docs/DESIGN.md`.
...
Resolve the workspace's GitHub URL once at compose time. This is remote interoperability, so `jj git` is appropriate:
...
- **Workspace-relative code/doc paths** (`services/foo.ts`,
  `<root>/solutions/bar.md`) → `<repo-url>/blob/<default-bookmark>/<path>`. Resolve
  the default bookmark from the project's active instructions or remote state;
  never assume a fixed bookmark name.
...
Bitbucket, internal host) and the equivalent bookmark-tree URL pattern
