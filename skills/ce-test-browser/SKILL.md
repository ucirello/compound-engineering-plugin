...
description: Run browser tests for pages affected by the current bookmark/change or PR.
argument-hint: "[PR number, bookmark name, 'current', or --port PORT]"
---

# Browser Test Skill

Run end-to-end browser tests on pages affected by a PR or bookmark/change using the `agent-browser` CLI.
...
```bash
jj diff --from main --to <bookmark-or-rev> --name-only
```
...
```bash
# Test current bookmark/change (auto-detects port)
/ce-test-browser
