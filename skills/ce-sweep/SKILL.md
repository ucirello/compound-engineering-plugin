...
- **Read** `docs_root` from `<repo-root>/.rocketclaw/config.yaml` only (`<repo-root>` = `jj workspace root`, with the current directory as fallback when no Jujutsu workspace is available). Do not read it from `config.local.yaml`. Unset -> `<root>` is `docs`, exactly as before.
...
- **Read** `<repo-root>/.rocketclaw/config.local.yaml`, then `config.yaml` (`<repo-root>` = `jj workspace root`, with the current directory as fallback when no Jujutsu workspace is available). Missing files are skipped. Ignore rules do not change resolution.
- **Win** with the first active (non-commented) value. For scalars, empty is unset; an invalid value continues to the next layer, then the skill default. For lists and maps, a present key — including an empty list or map — replaces the whole key.
- **Do not** use this rule for `docs_root` — that key is `config.yaml` only.
<!-- ce-config-layers:end -->

**Resolve the repo root.** Run `jj workspace root` with the shell tool to resolve `<repo-root>`; if it fails because no Jujutsu workspace is available, use the current directory. Then apply the ordinary-key rule above and read both files when they exist.
...
- `sweep_state_path` — path to the state file, established at setup; fallback `<root>/feedback-sweep/state.yml`. A path under `<workspace-root>/.tmp/` means machine-local mode and is never committed; another repo-internal path means committed mode and must not be ignored.
...
Read `references/interview.md` and follow it. Setup is interactive-only: if the run is non-interactive, report `first run requires interactive setup` and stop. The interview writes `feedback_sources` and the `sweep_*` keys into `<repo-root>/.rocketclaw/config.local.yaml` and offers a scheduling handoff. When it completes, continue into Phase 2.
...
**Shared-bookmark topology** (`sweep_shared_branch: true`): before any source-side write, identify the shared local bookmark and its tracked Git remote bookmark from the current Jujutsu state and project conventions; stop safely if that identity is ambiguous. Commit only the state fileset with `jj commit <state> -m <dynamic-description>`, move the shared bookmark to the resulting revision, and publish it with `jj git push --bookmark <shared-bookmark>`. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Runtime project instructions and the history visible through `jj log` take precedence; apply compatible Go quality guidance and do not use a fixed message or syntax template. A rejected push means another writer won the bookmark: use `jj git fetch`, reconcile the local change onto the tracked remote bookmark with the current `jj rebase` semantics, re-run `lease-acquire`, and if the lease is still not yours, back off (record `aborted-locked` and stop). Only once your lease is pushed and confirmed do you touch a source.
...
- `gh pr view <validated-ref> --json mergedAt,baseRefName` (merged, base is the default branch), or query `jj log` for the intersection of the validated commit and the ancestors of the default bookmark; a matching revision verifies ancestry. Preserve `gh` for GitHub metadata and use commit SHAs through Jujutsu's Git-backed revision resolution.
...
- **Commit.** Commit only `<root>/plans/feedback-sweep-plan.md` plus `<state>` when it is in committed mode by passing those filesets to `jj commit`; never include the workspace `.tmp` state. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Runtime project instructions and the history visible through `jj log` take precedence; apply compatible Go quality guidance and pass a neutral, dynamically composed description with `-m <dynamic-description>` rather than using a fixed message or syntax template. A commit failure is reported, not fatal. In local-commit mode, never push. In shared-bookmark mode (`sweep_shared_branch: true`), use `jj git fetch`, reconcile onto the tracked remote bookmark with `jj rebase`, move the shared bookmark to the committed revision, and publish it with `jj git push --bookmark <shared-bookmark>`.
