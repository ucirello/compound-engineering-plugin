...
1. **Resolve the target directory and extension.**
   ...
   - **Otherwise (no repo, or elsewhere with no `docs/ideation/`):** write into the run's workspace-local `<scratch-dir>` resolved in Phase 1 (`.tmp/rocketclaw/ce-ideate/<run-id>/` beneath the Jujutsu workspace root, or beneath the current directory when no Jujutsu repo exists). Do **not** create a `docs/ideation/` tree for a subject unrelated to the repo. Announce the absolute path and note that `.tmp` is temporary and the file should be moved to keep it.
...
2. **Build a focused seed** from the idea's substance already in the orchestrator's context. Do **not** pass the whole file — wasteful and noisy (the other survivors, grounding, and rejection table are irrelevant to defining this one idea, and an HTML file carries CSS/SVG chrome). Do **not** pass only a file pointer — that forces `ce-brainstorm` to re-open and re-extract the idea the orchestrator already holds. The seed is feature-description-shaped:
   ...
   The basis/evidence directly feeds `ce-brainstorm`'s product-pressure-test, so it won't re-derive what we already know. Append a one-line source pointer: `(Source: <path>, idea "<title>")` — it records origin and lets brainstorm pull adjacent detail if it wants, without being forced to read anything.
...
- **Inside a Jujutsu repo:** offer to describe only the ideation-doc change with `jj describe` (do not create a bookmark, do not push; if the user declines, leave its change description untouched). Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Repository-local instructions and syntax observed in `git log` take precedence; use Go guidance only when compatible. Do not use `git` for the operation.
...
- the deliverable was written automatically in both modes (Phase 4) — to `docs/ideation/` when present, else the workspace-local `.tmp/rocketclaw` area
