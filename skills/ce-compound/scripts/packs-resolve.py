...
`--declared-only` answers the config-only question without touching clone or the
...
clone. Clone uses `jj git clone` (branch via `-b`, then tag via `-t`); subprocesses
run non-interactively (GIT_TERMINAL_PROMPT=0, ssh BatchMode, bounded timeout):
missing credentials degrade to a warning, never a hang. A missing `jj` binary
degrades git-URL sources only (each warns and is skipped); path sources still
resolve when `jj workspace root` locates the workspace. Environment overrides:
PACKS_CACHE_ROOT (cache base for tests), PACKS_CLONE_TIMEOUT (seconds,
default 60). Public jj cannot fetch an arbitrary commit SHA as a pack ref.
...
GIT_TIMEOUT = float(os.environ.get("PACKS_CLONE_TIMEOUT") or 60)
...
# --- scratch root (workspace .tmp/rocketclaw/packs; cwd/.tmp if not a jj workspace) ---
...
def _workspace_root_or_cwd() -> str:
    proc = subprocess.run(
...
def cache_base() -> str | None:
    configured = os.environ.get("PACKS_CACHE_ROOT")
    if configured:
        root = os.path.abspath(configured)
        os.makedirs(root, exist_ok=True)
        return root
    root = os.path.join(_workspace_root_or_cwd(), ".tmp", "rocketclaw", "packs")
    try:
...
def _run_jj(args: list, cwd: str | None = None):
    return subprocess.run(
        ["jj", "--no-pager", *args], cwd=cwd, env=_clone_env(), timeout=GIT_TIMEOUT,
...
def resolve_git_source(url: str, ref: str, warnings: list, label: str) -> str | None:
    ...
    if os.path.lexists(dest):
        ...
        if os.path.lexists(dest):
            ...
            return None
    tmp = os.path.join(base, f"{key}.part-{os.getpid()}-{os.urandom(4).hex()}")
    try:
        try:
            proc = _run_jj(["--quiet", "git", "clone", "--depth", "1", "-b", ref, url, tmp], cwd=base)
            if proc.returncode != 0:
                if os.path.isdir(tmp):
                    shutil.rmtree(tmp, ignore_errors=True)
                proc = _run_jj(["--quiet", "git", "clone", "--depth", "1", "-t", ref, url, tmp], cwd=base)
        except subprocess.TimeoutExpired:
            warnings.append(f"{label}: jj git clone timed out after {int(GIT_TIMEOUT)}s; source skipped")
            return None
        if proc.returncode != 0:
            warnings.append(
                f"{label}: cannot clone `{ref}` from {url} as a branch or tag; "
                "source skipped (public jj git clone has no SHA-pin fetch)"
            )
            return None
...
def enumerate_packs(source_root: str, boundary: str, escaped: list, self_name: str | None = None) -> dict:
    ...
    within (the clone checkout, the repository, or the source root itself); a
...
def nested_rules_warning(pack_id: str, pack_dir: str, boundary: str) -> str | None:
    ...
    return (
        ...
        " never reads -- move rules to the pack's top level"
    )
...
def resolve_entry(entry: dict, repo_root: str, roots: list, warnings: list, errors: list) -> None:
    ...
    if _is_git_url(source):
        if not isinstance(ref, str) or not ref:
            errors.append(f"{label}: git-URL source `{source}` requires `ref:` (tag, sha, or branch)")
            return
...
def _repo_root() -> str | None:
    """The enclosing workspace root via public jj. None when cwd is not a jj workspace."""
    proc = subprocess.run(
        ["jj", "--no-pager", "workspace", "root"],
        capture_output=True, text=True, check=False,
    )
