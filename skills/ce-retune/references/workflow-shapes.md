...
**Corpus audit — fan out by unit, pipelined into defense.** One agent per skill directory, each reading that directory in full; that full read is what makes per-unit parallelism worth its cost, and it is why one agent cannot do all the units. Pipeline the adversarial defense stage per unit: a unit's defender needs that unit's findings plus the project's own documented learnings, tests, and history from `jj log` — never the global finding list — so a defense can start the moment its audit returns. A global barrier here only makes every defender wait on the slowest auditor.
...
**Every artifact of this method needs a path decided before the phase that writes it.** The registration and the scored run table (`references/noise-floor.md`), the phase-marker map and the extractor (`references/baseline-mining.md`), the ruled finding set (`references/corpus-audit.md`), the ownership manifest and the rewrite contract (`references/cut-passes.md`). Resolve the workspace root with `jj workspace root`, falling back to the current directory, and keep temporary or per-run artifacts together under `<workspace-root>/.tmp/rocketclaw/ce-retune/<run-id>/` with a dynamically chosen run id; never use an OS-global temporary directory or `TMPDIR`. The registration must be re-readable after results exist, the extractor and map must be re-runnable when the taxonomy changes, and a contract or manifest passed only through a dispatch prompt cannot be checked afterward against what the agents actually did. Durable artifacts — registration, final scored table, write-up — belong in the project's normal tracked documentation location; per-run scratch does not.
