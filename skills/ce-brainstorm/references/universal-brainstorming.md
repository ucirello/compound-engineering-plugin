...
Do not write `artifact_contract: rocketclaw-unified-plan/v1`,
...
- **Publish to a shareable editor** → load the `ce-proof` skill to publish the doc and get a shareable link to read, comment on, or share with others (one-way; the local summary stays canonical)
