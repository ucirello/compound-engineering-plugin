...
- You are operationally read-only. The one permitted exception is writing full analysis to `{run_dir}`. Allowed repository commands are `jj diff`, `jj log`, `jj status`, `jj file show`, `jj file annotate`, and read-only `gh`. Do not run mutating JJ, Git, or GitHub commands.
