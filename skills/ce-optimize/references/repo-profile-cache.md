...
- The `docs/solutions/` enumeration (a new learning, even in the current working-copy change, must be visible — re-globbing it is ~free and the match reads files fresh anyway).
...
- `<root-id>` = lexicographically-first JJ commit ID in `roots(::@)` — a deterministic history identity.
- `<working-copy-id>` = the JJ commit ID of `@` — the working state.

Each JJ workspace keeps its cache under its own root. Lookup uses JJ metadata only; on a hit, only this one file is read.
