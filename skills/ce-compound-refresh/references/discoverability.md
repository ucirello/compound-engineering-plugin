...
6. If this check edited an instruction file after Commit already ran, fold only that edit into the refresh revision (same bookmark, not yet pushed) or add a small follow-up revision under the message standards in `references/commit.md`, and publish with `jj git push` if the bookmark was already pushed so an open PR includes it. If the user chose "don't commit", leave the edits in the working copy alongside the rest.
