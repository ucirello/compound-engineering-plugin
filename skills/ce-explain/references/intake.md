...
|-------|---------|--------|
| `diff:<ref-or-range>` | `diff:<change-id>`, `diff:<from>..<to>`, `diff:PR#<number>` | Forces diff mode on that change |
...
- An unrecognized `<word>:<word>` token is not a flag — it passes through verbatim as request text.
...
- **Diff** — the request names a resolvable JJ revision, change ID, commit ID, bookmark, PR, "the last change", "what you just did", or "this change".
