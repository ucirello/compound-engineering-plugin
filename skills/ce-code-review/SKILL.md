...
- **Explicit mutations only.** Never run `gh pr checkout`, `jj edit`, `jj new`, bookmark moves, or similar workspace-switch commands. Passing a PR number, URL, or bookmark name selects **review scope**, not permission to mutate the working tree. To review local work on a feature bookmark, move to that workspace yourself (or stay on it) and pass `base:` or no target.
...
```
echo "BASE:$BASE" && echo "FILES:" && jj diff --summary --from "$BASE" --to @ && echo "DIFF:" && jj diff --from "$BASE" --to @ && echo "UNTRACKED:" && jj status
```

This path works with any JJ revset — a commit ID, `main@origin`, or a bookmark name. Callers reviewing the current workspace should pass explicit `base:` when auto-detection is unnecessary. **Do not combine `base:` with a PR number or bookmark target.** If both are present, stop with an error: "Cannot use `base:` with a PR number or bookmark target — `base:` implies the current workspace is already the correct target. Pass `base:` alone, or pass the target alone and let scope detection resolve the base."
...
3. The PR head commit is present in the local workspace stack: `jj log -r '<headRefOid>..@'` succeeds. This confirms the working tree actually carries the PR head (allowing unpublished local fixes layered on top) rather than an unrelated same-named bookmark.
...
- **`local-aligned`:** Resolve `<resolved-base-ref>` from `baseRefName` (fetch if needed). Set `BASE=<resolved-base-ref>@origin` when available, then set `FILES:` from `jj diff --summary --from "$BASE" --to @` and `DIFF:` from `jj diff --from "$BASE" --to @` (includes committed and working-copy changes on the PR bookmark). Do **not** call `gh pr diff` or append remote hunks — when unpublished fixes exist, the local tree is canonical. Note in Coverage: `scope: local-aligned (PR; local tree diff)`.
...
1. Best-effort fetch remote bookmarks without checkout: `jj git fetch --remote origin`.
2. When the PR head is locally addressable, set `PR_HEAD_REF=<headRefName>@origin` or the fetched head commit ID for reviewers and validators. When fetch fails, omit `PR_HEAD_REF` and note in Coverage — reviewers must rely on diff hunks only.
3. Best-effort resolve the PR base as `<baseRefName>@origin`. When it succeeds, set `PR_BASE_REF` to that JJ revision — a **real JJ base ref** reviewers and validators use for file-level diffs (e.g. `data-migration-reviewer` runs `jj diff --from <PR_BASE_REF> --to <PR_HEAD_REF> -- db/schema.rb`/`structure.sql`). The `pr:<number-or-url>` logical marker in `BASE:` stays the scope marker. When the fetch fails, omit `PR_BASE_REF` and note in Coverage — schema-drift and other JJ-diff checks fall back to diff hunks only and must **not** assume `main`.
...
3. Resolve default base bookmark (same logic as standalone). Compute `jj diff --from <base-ref> --to <bookmark-ref>`.
...
```
echo "BASE:$BASE" && echo "FILES:" && jj diff --summary --from "$BASE" --to <bookmark-ref> && echo "DIFF:" && jj diff --from "$BASE" --to <bookmark-ref> && echo "UNTRACKED:" && jj status
```

**If no argument (standalone on current bookmark/change):**

Apply the same base-detection logic as bookmark mode above, using the current bookmark/change (i.e., `gh pr view --json baseRefName,url` with no argument defaults to the current PR when available).

If no base can be resolved, **stop**. Do not fall back to `jj diff` — a standalone review without the base would only show current-change changes and silently miss stacked work on the bookmark.
...
Using `jj diff --from "$BASE" --to @` diffs the base against the current working-copy commit, which includes committed and working-copy changes together.
...
- **`pr-remote` / `bookmark-remote`** — `DIFF_A=<PR_BASE_REF>`, `DIFF_B=<PR_HEAD_REF>` (or `<branch-head-ref>`) — the **fetched** refs from Stage 1. Do **not** model-count from hunks (it drifts per host/model). If either ref was not fetched, skip the block and emit `EXEC_LINES:UNKNOWN` + `UNCOUNTED_FILES:1` so Stage 3c forces the full roster.
...
if [ -z "${DIFF_A:-}" ] || ! jj log -r "${DIFF_A}" --no-graph >/dev/null 2>&1 \
  ...
  # JJ's diff --stat output is less stable across versions, so count patch lines conservatively.
  EXEC_LINES=$(jj diff --from "$DIFF_A" ${DIFF_B:+--to "$DIFF_B"} -- '*.rb' '*.py' '*.js' '*.jsx' '*.ts' '*.tsx' '*.go' '*.rs' '*.java' '*.swift' '*.kt' '*.c' '*.cc' '*.cpp' '*.cs' '*.php' '*.ex' '*.exs' '*.scala' 2>/dev/null | awk '/^[+-][^+-]/ {n++} END{print n+0}')
...
**Bookmark mode:** Run `jj log -r '${BASE}..<bookmark-ref>' --no-graph` using the resolved base and resolved bookmark ref from Stage 1. Use `<bookmark-ref>` (the resolved `<bookmark>@origin` or fetched ref), not the raw `<bookmark>` argument — a remote-only bookmark may have no matching local bookmark, so the raw name would fail or read a stale same-named local bookmark.
...
**Only resolve the cache when the working tree is the reviewed tree** — `local-aligned`, standalone, or `base:` scope (Stage 1). In `pr-remote` or `bookmark-remote` scope, **skip Stage 2c entirely**: the helper keys and derives from the local parent commit, which is *not* the reviewed ref, so its profile would describe the wrong tree (e.g. `main`'s stack while reviewing a PR that changes manifests); reviewers work from the fetched refs/diff as those modes already require.
...
**Commit when the pre-review tree was clean.** Before applying, note whether the working copy already had changes (`jj status`). The permanence gate is the **push**, not the commit — a local JJ change is private and reversible with `jj undo` or by abandoning/restoring the change.
...
```json
{
  ...
  "head_sha": "<jj log -r @ commit_id at dispatch time>",
  "verdict": "<Ready to merge | Ready with fixes | Not ready>",
