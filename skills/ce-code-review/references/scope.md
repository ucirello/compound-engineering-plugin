...
Compute the diff range, file list, and diff. Run commands from the absolute workspace root obtained through `jj workspace root`. Set `GIT_DIR=$(jj git root)` for every `gh` call. JJ has no checked-out branch or staging area: identify a unique task bookmark from `jj bookmark list` and `jj log`, considering `@-` when `@` is empty. Do not guess when multiple bookmarks could identify the task.
...
```
echo "BASE:$BASE" && echo "FILES:" && jj diff --from "$BASE" --to @ --name-only && echo "DIFF:" && jj diff --from "$BASE" --to @ --git --context 10 && echo "UNTRACKED:" && jj status
```

This path works with any single revision — a commit ID, `main@origin`, or a bookmark. Callers reviewing the current workspace should pass explicit `base:` when auto-detection is unnecessary. **Do not combine `base:` with a PR number or bookmark target.** If both are present, stop with an error: "Cannot use `base:` with a PR number or bookmark target — `base:` implies the current workspace is already the correct tree. Pass `base:` alone, or pass the target alone and let scope detection resolve the base."

**If a PR number or GitHub URL is provided as an argument:**

Do **not** switch the workspace to the PR. Scope comes from GitHub read APIs plus optional local alignment when the task bookmark and ancestry establish that `@` contains the PR head.
...
Set `BASE:` to `pr:<number-or-url>` (logical marker, not a revision). Set `UNTRACKED:` to any paths `jj status` reports as not tracked in the **current** workspace; automatically tracked new files are included in `@` already.
...
1. The resolved task bookmark equals `headRefName`; use bookmarks on `@-` too when `@` is empty.
2. The PR is **not** cross-repository (`isCrossRepository` is false).
3. `jj log --no-graph -r '<headRefOid> & ancestors(@)' -T commit_id` returns exactly `headRefOid`. This confirms the workspace carries the PR head (allowing unpushed local fixes layered on top) rather than an unrelated same-named bookmark.
...
- **`local-aligned`:** Resolve `<resolved-base-ref>` from `baseRefName` (fetch if needed). Resolve exactly one merge base with `jj log --no-graph -r 'heads(ancestors(@) & ancestors(<resolved-base-ref>))' -T 'commit_id ++ "\n"'`; fail closed if unavailable or ambiguous. Set `BASE` to that ID, `FILES:` from `jj diff --from "$BASE" --to @ --name-only`, and `DIFF:` from `jj diff --from "$BASE" --to @ --git --context 10` (includes committed and in-flight changes). Do **not** call `gh pr diff` or append remote hunks — when unpushed fixes exist, the local tree is canonical. Note in Coverage: `scope: local-aligned (PR; local tree diff)`.
...
1. Best-effort fetch PR head without switching the workspace: `jj git fetch --remote origin --branch <headRefName>`.
2. Set `PR_HEAD_REF` to the immutable commit ID only when `jj log --no-graph -r '<headRefName>@origin' -T commit_id` equals metadata `headRefOid`. A successful fetch does not prove identity: a fork PR can share an unrelated origin bookmark name, and the PR can move during the fetch. If fetching or identity verification fails, omit `PR_HEAD_REF`, record the reason in Coverage, and use diff hunks only.
3. Best-effort fetch the PR base with `jj git fetch --remote origin --branch <baseRefName>`. Resolve `<baseRefName>@origin` with `jj log --no-graph -r '<baseRefName>@origin' -T commit_id` and set `PR_BASE_REF` to that commit ID. It is the concrete base reviewers use for file-level `jj diff --from <PR_BASE_REF> --to <PR_HEAD_REF> -- <path>` checks. Keep the logical `pr:<number-or-url>` marker in `BASE:`. On failure omit `PR_BASE_REF`, record the gap, and use supplied hunks only; never assume `main` or compare to the unrelated local `@`.
...
If the resolved task bookmark equals `<branch>`, use the **standalone (current workspace)** path below — same tree, explicit bookmark name; do not use remote-only diff.
...
3. Resolve the default base bookmark (same logic as standalone). Resolve exactly one `heads(ancestors(<base-ref>) & ancestors(<branch-ref>))` commit using `jj log`; stop on ambiguity. Set `BASE` to that ID and compare with `jj diff --from "$BASE" --to <branch-ref> --git --context 10`.
...
Apply the same base-detection logic as bookmark mode above, passing the resolved task bookmark explicitly to `gh pr view <bookmark> --json baseRefName,url`. If no PR exists, use the repository's known default branch (GitHub repository metadata or active project conventions), resolve its remote bookmark, and require one merge base with `@`.
...
Using `jj diff --from "$BASE" --to @` compares the merge-base to the current working-copy revision, including both prior commits and in-flight edits.

**Untracked file handling:** Always inspect `UNTRACKED:`. JJ automatically tracks eligible new files. Files explicitly untracked, ignored, or rejected by tracking limits remain out of scope; list those exclusions in Coverage and continue on the recorded tree — never stop or prompt merely for an excluded path.
