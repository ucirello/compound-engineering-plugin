...
Environment overrides (defaults in parentheses):
  CE_PEER_JOBS_ROOT         optional base dir under <workspace>/.tmp
...
POSIX path is behaviorally unchanged:
  ...
  jobs root defaults under the current workspace's `.tmp/rocketclaw/peer-jobs`,
            with the same owner and ACL checks as POSIX.
...
def jobs_root_base() -> str:
    ...
    if configured:
        ...
        try:
            contained = os.path.commonpath((configured, allowed)) == allowed
        except ValueError:
            contained = False
        if not contained:
            raise RunnerError("CE_PEER_JOBS_ROOT must stay under the current workspace's .tmp directory")
        return configured
    return os.path.abspath(DEFAULT_ROOT)


def candidate_jobs_root_bases() -> list:
    """The workspace-local root where an existing job may live."""
...
def write_atomic(path: str, data: bytes) -> None:
    ...
    for attempt in range(CLAIM_ATTEMPTS):
        tmp = os.path.join(
            os.path.dirname(path), f".tmp-{os.getpid()}-{time.time_ns()}-{attempt}"
        )
        try:
            ...
            continue
    if fd < 0:
        raise RunnerError(f"could not reserve same-directory atomic temp file for {path}")
    try:
