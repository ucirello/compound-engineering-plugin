...
- **Composition timestamp.** A visible footer at the bottom of the doc names
  only the composition timestamp and, when applicable, the human- or
  research-provided source document. Do not add authoring identity or generator
  ...
  Under exclusive output mode this signal lets readers assess staleness without
  adding author branding.
...
Resolve `<default-branch>` from GitHub metadata or active project conventions;
do not assume a fixed name.
...
whole pill as a soft tint), never by an accent on one edge. A
...
- **Composition timestamp** is present as a visible footer at the bottom of
  the doc, with a human/research source document when applicable and no
  authoring identity or generator metadata.
