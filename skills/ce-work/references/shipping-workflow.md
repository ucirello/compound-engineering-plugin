...
3. **Code Review**
   ...
   **3b. Apply fixes (caller-owned).** Load `references/review-findings-followup.md`: filter on JSON, batch by file, dispatch fix subagents. Orchestrator integrates, tests, and describes the JJ change. Then proceed to the Residual Work Gate.
...
4. **Residual Work Gate** (REQUIRED when `ce-code-review` ran and left actionable residuals)
   ...
   **Interactive sessions:** Ask the user using the platform's blocking question tool (`AskUserQuestion` in Claude Code with `ToolSearch select:AskUserQuestion` pre-loaded if needed, `request_user_input` in Codex, `ask_question` in Antigravity CLI (`agy`), `ask_user` in Pi (requires the `pi-ask-user` extension)). Fall back to numbered options in chat only when the harness genuinely lacks a blocking tool. Never silently skip the gate.
   ...
   - `Apply/fix now` — load `references/review-findings-followup.md`, dispatch batched fix subagents for remaining eligible findings, run tests, and describe the change if needed; optionally re-run `ce-code-review` only after the diff changed materially.
...
2. **Record and Create Pull Request**
   ...
   **Do not publish what the user did not offer.** `ce-commit-push-pr` publishes the selected bookmark and its PR spans that bookmark's revision range. Check the pre-work scope from Phase 1 Step 2: if the bookmark carries pre-existing unpublished changes that are not already in an open PR for it, load `ce-commit` instead, record only this run's work locally, and say what stayed local and why. Otherwise:

   Load the `ce-commit-push-pr` skill to handle change descriptions, bookmark publication through `jj git` remotes, and PR creation. Pass `exclude:<paths>` naming every pre-work path this run left untouched or unshipped. If the session already stated shipping topology, including a PR stack and any parent PR or bookmark, pass it on that invocation. The handoff must not request or add branding or output attribution.
   ...
   If the user prefers to record the JJ change without creating a PR, load the `ce-commit` skill instead — only after the same ship-handoff gate passes.
...
- [ ] JJ change descriptions follow the project's active instructions and current `jj log` conventions
...
- [ ] `ce-commit-push-pr` received the bookmark topology and attribution prohibition
