...
2. **Otherwise, in a JJ repository**, default to the current change stack relative to the trunk (`jj diff --from 'trunk()' --to '@'`). This covers the common case of "simplify everything I've added on this bookmarked change stack before opening a PR." If no trunk revision is configured, fall back to the working-copy change (`jj diff`).
