...
2. **Brainstorm one idea with `ce-brainstorm`** — go deeper on one chosen idea (asks which). In universal mode this is **not** the first step of an implementation chain — there is no `ce-plan` -> `ce-work` after; `ce-brainstorm` develops the idea further and ends there. Seed it with the idea's substance and source context per §5.2, not the whole file.
