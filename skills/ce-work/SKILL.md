...
4. **Choose Execution Engine, then Strategy**
   ...
   **After a parallel batch — the orchestrator integrates; never trust the handoff summary alone:**
   ...
   4. **Review, test, and commit each unit in dependency order — the orchestrator owns commits.** Select only that unit's files, preserve its Goal as semantic context for the description, run the relevant tests, and fix before the next. Record only those selected files as one logical unit; JJ creates a new working-copy child, so move the selected bookmark to the completed revision at `@-`, never to the new empty `@`. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Repository-local instructions and the message syntax observed in actual `git log` output take precedence over compatible Go guidance. Apply compatible Go guidance only for quality, clarity, and structure. Do not impose any fixed prefix, type, scope, subject, body, layout, template, or example, and use neutral placeholders where an interface requires fields. Capture each worker's returned verification evidence into the run's `verification_evidence` roll-up — if a worker omitted it, re-derive what the tree allows and mark the rest as unverified rather than fabricating a red-before-implementation observation the worker never reported.
   5. Update the task list (progress lives in JJ changes/commits).
   6. **Release the workers** — close each worker handle after its revision is safely integrated, then let the provider perform its managed-worktree cleanup. Do not perform raw Git worktree or branch cleanup, infer workspace paths, or remove provider-managed paths; the provider owns that lifecycle.
   7. Dispatch the next dependency layer.

   **Per-harness integration (examples — the universal flow above is the contract):**
   - **Claude `Agent` `isolation:"worktree"`:** each worker uses a provider-managed Git worktree. After the provider exposes the worker revision, if JJ cannot see it, run `jj git import`; then integrate the revision in dependency order through JJ and retain it until recovery is complete. On conflict, abandon only the failed integration revision and re-run that unit serially against the integrated JJ change; do not silently discard either unit's intent. The provider owns worktree cleanup.
...
2. **Apply fixes** — Load `references/review-findings-followup.md`. Filter eligibility on JSON only, **batch applicable findings by file**, dispatch fix subagents (parallel when file sets are disjoint). The orchestrator merges diffs, runs tests, and commits — it does not pre-investigate findings. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Repository-local instructions and the message syntax observed in actual `git log` output take precedence over compatible Go guidance. Apply compatible Go guidance only for quality, clarity, and structure. Do not impose any fixed prefix, type, scope, subject, body, layout, template, or example, and use neutral placeholders where an interface requires fields.
