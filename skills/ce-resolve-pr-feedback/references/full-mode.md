...
```bash
GIT_DIR=$(jj git root) gh pr view --json number -q .number
```
...
**GitHub Enterprise host.** The bundled `gh api graphql` scripts hit `gh`'s default host unless told otherwise, so on a GHE PR they would wrongly target `github.com`. Pair every `gh` invocation that talks to the underlying git repo with `GIT_DIR=$(jj git root)` (and when a workspace cwd is in play, also run `gh` with that workspace root as cwd). Derive the host: if the caller passed a full PR **URL**, take its host; otherwise read it from `GIT_DIR=$(jj git root) gh repo view --json url -q .url`. Then — because shell state does **not** persist between separate Bash calls — pass the host as a `GH_HOST=<host>` **env prefix inline on every bundled-script call** (`gh api` honors `GH_HOST` as the request host). A single `export` in one block does **not** carry to the reply/resolve/verify blocks that run as later Bash calls, which is why each call below shows the prefix. On `github.com`, drop the `GH_HOST=<host> ` prefix entirely. Keep `GIT_DIR=$(jj git root)` on every `gh` call.
...
1. Record only files reported by fixers as a JJ change (no index — omit add; limit via filesets). Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Repo-local syntax from project instructions and `git log` ALWAYS wins when it differs from Go guidance. Apply compatible Go guidance to quality/clarity/structure without replacing repo-local syntax. Semantic requirements: the change records addressing PR review feedback for this PR, and the body lists the changes from the fixer summaries. Do not use a fixed `type:` / `type(scope):` / `fix:` / `feat:` template unless that syntax is what the project instructions or recent `git log` already use.

```bash
jj commit [filesets from fixer summaries] -m "<message composed from the standards above>"
```

2. Push the bookmark to remote. Resolve the current bookmark with `jj log -r @ -T 'bookmarks.join("\n")' --no-graph`, then:
