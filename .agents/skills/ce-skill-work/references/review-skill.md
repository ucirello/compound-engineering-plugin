...
- Description is a context pointer for a model-invoked skill: sentence 1 names the distinctive mechanism with the leading prompt word first, then one trigger per genuinely distinct branch in "Use when..." or "Use for..." form as an observable work-state. Neighbor routing prefers "Use <sibling> for <that job>". An identity-boilerplate opener, a category opener that omits the mechanism, a site/synonym/capability catalog for one branch, or a quoted-utterance catalog on a model-invoked skill, is a Change; workflow, flags, phase lists, or body-owned detail in the description is also a Change. Distinctive how may stay. Use the contrast pairs in `references/new-skill.md` when the shape needs a labeled example.
