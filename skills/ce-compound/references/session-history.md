...
- **JJ bookmark**: from the absolute workspace root inspect `jj log --no-graph -r '@ | @-' -T 'commit_id ++ " " ++ empty ++ " " ++ local_bookmarks ++ "\n"'`. Use the bookmarks on `@`, or on `@-` when `@` is empty. Use an unambiguous bookmark name to match the historical session branch field in Phase 1. If no unique bookmark resolves or the command fails, skip branch filtering.
- **Repo root**: run `jj workspace root` from the absolute workspace root. Use it as the session-history repo filter in Phase 1. If it exits non-zero (not a JJ workspace), fall back to the working directory.
...
#### 4. **Session History** (internal flow after launching the parallel block; automatic in Full mode, including non-interactive)
   ...
   **Keep the session-history payload tight.** A long, keyword-rich payload invites the flow to widen its search. Use this shape:
   ...
   - **Session context** (only if the values resolved cleanly above; otherwise omit): repo name, current JJ bookmark matched against recorded branch metadata.
   ...
   **Discovery pipeline.** Infer the scan window from the problem topic, starting with 7 days. Run discovery and metadata extraction:
   ...
   if [ -f "$SKILL_DIR/scripts/session-history/discover-sessions.sh" ] && [ -f "$SKILL_DIR/scripts/session-history/extract-metadata.py" ]; then
     ...
     REPO_ROOT=$(jj workspace root 2>/dev/null || pwd -P); REPO_NAME=$(basename "$REPO_ROOT"); SCAN_DAYS="7"; bash "$SKILL_DIR/scripts/session-history/discover-sessions.sh" "$REPO_NAME" "$SCAN_DAYS" --cwd "$REPO_ROOT" | tr '\n' '\0' | xargs -0 "$PY" "$SKILL_DIR/scripts/session-history/extract-metadata.py" --cwd-filter "$REPO_ROOT";
   ...
   **Extraction pipeline.** Create `SCRATCH=$(mktemp -d "<absolute run_dir>/sessions-XXXXXX")` using the workspace-local run directory from research. For each selected session, write extracted content to scratch files:
