...
POSIX path is behaviorally unchanged:
  ...
  jobs root defaults under the JJ workspace `.tmp` (then cwd/.tmp), owner-private.
...
def _workspace_tmp() -> str:
    ...
    if result.returncode == 0 and result.stdout.strip():
        return os.path.join(result.stdout.strip(), ".tmp")
    return os.path.join(os.getcwd(), ".tmp")


DEFAULT_ROOT = None  # unused; jobs root is `_workspace_tmp()` (workspace `.tmp`)
...
def _private_root_usable(path: str) -> bool:
    ...
    Creation is the probe: a sandbox that denies writes under the workspace `.tmp` refuses the
...
def candidate_jobs_root_bases() -> list:
    ...
    the workspace `.tmp` and the cwd `.tmp` fallback (deduplicated, primary first).
    ...
    if configured:
        return [os.path.abspath(configured)]
    bases = [os.path.abspath(_workspace_tmp())]
...
def cmd_result(args) -> int:
    ...
    if getattr(args, "path", None):
        ...
        # consumer must never read a predictable path unchecked.
        try:
