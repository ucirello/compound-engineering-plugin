...
- **`scratch_dir`** — absolute path to a per-run directory under the workspace `.tmp/rocketclaw/ce-compound/`, or the local `./.tmp/rocketclaw/ce-compound/` fallback, holding pre-extracted files.
- **`sessions`** — an array of objects (5 max), one per pre-extracted session, each with:
  ...
  - `bookmarks` — current JJ bookmarks when present
  - `legacy_branch` — source-session branch metadata when present; use only to match a current bookmark
...
Cite actual evidence from the extracted files, not vibe-summaries. When a finding is anchored in a specific session's content, that session's metadata (platform, bookmarks/cwd, ts) helps the caller locate it.
