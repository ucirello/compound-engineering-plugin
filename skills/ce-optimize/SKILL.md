...
**Outcome:** confirmed improvements to the named target live on an `optimize/<spec-name>` JJ bookmark, with a disk log. The user takes over at wrap-up.
...
Resolve the absolute workspace root once through `jj workspace root`, then set each JJ subprocess's cwd to that root (or the experiment's root for its commands). Never substitute `-R` for cwd. Scratch lives under `<repo-root>/.tmp/optimize/`; outside JJ, local `.tmp/` is the scratch fallback, but experiment isolation requires JJ. Keep scratch ignored and outside measured source snapshots. Discover workspace membership with `jj workspace list` and roots with `jj workspace root --name NAME`, never metadata-directory inspection. Before `gh` commands, set `GIT_DIR` to the result of `jj git root` in the correct workspace.

**Delegation configuration:** load `.rocketclaw/config.yaml` and overlay ordinary keys from `.rocketclaw/config.local.yaml`. Explicit subagent and harness routing wins, including `cross_model_peer: opencode`, `work_engine_preferences: [{harness: opencode, model: provider/modelname#variant}]`, and `plan_harness`/`brainstorm_harness: opencode` paired with their model settings. OpenCode means V2. When the current harness is OpenCode, no explicit subagent or alternative-harness routing applies, and `opencode.models` plus native subagent dispatch with an optional model are available, discover exact model IDs/variants and dispatch natively with `provider/model#variant`. Preserve the intended tier and cross-model independence; never guess an ID or silently reuse the current model. Otherwise preserve configured routing. Apply this policy to research, experiments, and judges; Phase 3 provides the external OpenCode branch.
...
- **Clean-tree gate.** Do not continue while the current JJ working-copy change contains unrelated changes in `scope.mutable` or `scope.immutable`. Preserve those changes in a separate revision before starting experiments. The reference defines the check and what to ask for.
