...
1. **JJ activity** — `jj log` over the window (subjects, shas, dates, authors), cwd = `{repo-root}`, and for the substantial commits, a stat-level view of what they touched. Group obviously-related commits (a feature bookmark's commits, a fix and its follow-ups) rather than listing them flat.
...
3. **Project docs** — plans, brainstorms, and solution docs added or modified in the window (`<root>/plans/`, `docs/brainstorms/`, `<root>/solutions/`, or wherever this repo keeps them). These carry the *why* behind that activity — quote the decision or problem statement, not the whole doc.
