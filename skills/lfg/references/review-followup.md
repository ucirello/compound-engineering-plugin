...
`ce-code-review` is review-only. LFG applies eligible fixes itself, then describes a JJ change.
...
4. If `jj st` shows changes, commit only review-driven files with `jj commit <review-driven-files> -m "fix(review): apply review findings"`, and push with `jj git push` before step 6 **when a remote is configured** (per LFG's shipping precondition). If there is no remote at all, do not push — the local change suffices. If no eligible fixes were applied, note explicitly and skip the `jj commit`.
