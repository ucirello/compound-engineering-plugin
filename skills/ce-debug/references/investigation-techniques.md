...
When a bug is a regression ("it worked before"), use binary search to find the breaking change. Prefer automated bisection when you can express the bug as a command:

```bash
jj bisect run --good <known-good-rev> --bad @ -- <test-command>
```

For manual bisection, use JJ's `bisect(...)` revset to pick candidate revisions, test them, and narrow the good/bad bounds. Keep the current bookmark/ref intact; do not switch branches just to bisect.

The test command should exit 0 for good, non-zero for bad.

Inspect the resulting candidate with:

```bash
jj show <candidate-rev>
```
