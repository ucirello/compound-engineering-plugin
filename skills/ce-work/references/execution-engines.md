...
<!-- work-config-layers:start -->
**Resolve ordinary work-execution YAML keys from the two repository files.**
...
Follow the dispatch strategy in `references/execution-strategy.md` (inline, serial subagents, or parallel subagents) and the Phase 2 execution loop. `ce-work` owns task creation, unit sequencing, dispatch, verification, and canonical JJ changes.
...
Tail ownership decides who opens the final PR; it does not forbid progress signals during a long run. For long goals, meaningful described JJ changes and an optional progress artifact below `$(jj workspace root)/.tmp/rocketclaw` keep the trajectory observable. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Runtime conventions win; apply compatible Go guidance to message quality, clarity, and structure, and impose no fixed message shape. Only final PR creation is gated: a standalone top-level goal may open a draft PR only when it explicitly owns that channel; in Return-to-Caller Mode `ce-work` must not open a PR, but may finalize local changes and return receipts. Never write progress into the plan body; JJ changes and the envelope carry it.
