...
| `mode:report-only` | `mode:report-only` | **Deprecated — ignored.** Former no-artifacts mode; default behavior is review-only without moving the working-copy revision |
...
| `base:<commit-id-or-ref>` | `base:abc1234` or `base:main@origin` | Diff base for the **current working copy** (explicit; skips auto base detection) |
...
**Mode alias:** `mode:headless` normalizes to `mode:agent`. `mode:agent` + `mode:headless` is not a conflict. `mode:non-interactive` is **not** an alias for `mode:agent` — that token means “suppress prompts” in other plugin skills; if it appears here, treat it as an unrecognized/conflicting `mode:` token and stop (fail closed).
