...
2. **Otherwise, in a JJ workspace**, use the current change stack: `@` plus its mutable ancestors through the nearest immutable ancestor. The project's active instructions and current `jj log` topology take precedence when resolving that stack; do not assume a fixed revset, bookmark, or integration line. Diff the explicit range with `jj diff --from <resolved-stack-base> --to @`; if there is no usable stack base, use `jj diff -r @`. A workspace selects the working-copy change, local bookmarks name lines of work, and working-copy edits are recorded directly in `@`. Use `jj git` only when remote Git interoperability is required, not for local scope discovery.
...
An interface or data shape that existed only in an earlier iteration of the current unshipped scope is not protected behavior once you verify it has no deployed, persisted, public, external, dependent JJ change or bookmark, or in-repo caller outside the resolved scope. Remove that compatibility path only when every required caller update fits the existing mutation boundary; otherwise preserve it.
...
Use the project's active local instructions and observed command syntax over generic defaults. Run project-wide typecheck and lint, then tests matched to blast radius: scoped tests for local changes, broader tests for shared or wide-reach changes, and the full suite when the runner cannot scope tests. For Go code, preserve `gofmt` formatting and run the project's configured Go quality checks rather than substituting an unconfigured fixed tool list.
