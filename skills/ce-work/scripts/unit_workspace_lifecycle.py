...
def discover_resume_run(repo: str, plan_digest: str) -> tuple[str, list[dict]]:
    ...
    # Read-only: repairing a root this session cannot write would abort
    # discovery before the writable one.
    entries = []
            ...
            if snap == unit["integration"]["pre_fold"]:
                ...
                with locked_manifest(run_id, write=True) as current:
                    ...
                    current_unit["integration"]["applied"] = {
                        "at": now_iso(),
                        "post_commit": snap["head"],
                        "status_sha256": snap["status_sha256"],
                        "reconciled": True,
                    }
                    event(current, "transport-apply-reconciled", uid, {"post_commit": snap["head"]})
...
def cmd_complete_fallback(args) -> tuple[str, dict]:
    ...
    if not re.fullmatch(r"[0-9a-f]{40}|[0-9a-f]{64}", args.accepted_head):
        raise Operational("REFUSED", "native fallback accepted head must be a JJ commit id")
    ...
    with locked_manifest(args.run_id, write=True) as doc:
        ...
        if wave.get("id"):
            ...
            changed_paths = name_only_diff(repo, claim_head, args.accepted_head)
...
def cmd_cleanup(args) -> tuple[str, dict]:
    ...
    with admin_lock(identity):
        present = [r for r in workspace_rows(repo) if os.path.realpath(str(r.get("path", ""))) == os.path.realpath(workspace)]
        forget_name = workspace_name or (present[0]["name"] if present else None)
        if forget_name:
            jj(repo, "workspace", "forget", forget_name)
            test_fault("cleanup-after-worktree-remove")
        if any(os.path.realpath(str(r.get("path", ""))) == os.path.realpath(workspace) for r in workspace_rows(repo)):
    ...
    if ref and commit:
        ...
        if current:
            jj(repo, "bookmark", "forget", f"exact:{ref}")
    with locked_manifest(args.run_id, write=True) as doc:
        unit = doc["units"][args.unit_id]
