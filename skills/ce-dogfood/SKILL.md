...
1. **Inspect before changing the working copy.** Run `jj status`, `jj log -r '@ | @-'`, `jj bookmark list --all-remotes`, and `jj workspace list`. JJ has no active bookmark: determine the target from the argument and revision graph, not from an assumed checked-out branch.
