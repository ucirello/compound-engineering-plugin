...
**IMPORTANT: All file references in the plan document must use repo-relative paths (e.g., `src/models/user.rb`), never absolute paths (e.g., `/Users/name/Code/project/src/models/user.rb`). This applies everywhere — implementation unit file lists, pattern references, origin document links, and prose mentions. Absolute paths break portability across machines, Jujutsu workspaces, and teammates.**
...
**Read config.** The Jujutsu workspace root is pre-resolved at skill load:
!`jj workspace root 2>/dev/null || true`

If the line above is an absolute path, use it as `<workspace-root>`. If it is empty or still shows a backtick command string (a non-Claude harness that did not run the pre-resolution), resolve `<workspace-root>` at runtime by running `jj workspace root` with the shell tool. Then read `<workspace-root>/.rocketclaw/config.local.yaml` with the native file-read tool. If the workspace root cannot be resolved or the file does not exist, fall through to the defaults below.
...
**Token-parsing convention:** only literal-prefix flag tokens (`output:`, `mode:`, the exact `confirm:auto`/`confirm:ask` forms, `delegate:` where applicable) are consumed and stripped. Other `<word>:<word>` tokens — including repository-specific change-title prefixes — and any unrecognized `confirm:<value>` (e.g., a `confirm: delete-account modal` feature description) pass through verbatim.
...
- If updating, revise only the still-relevant sections. Plans do not carry per-unit progress state — progress is derived from the Jujutsu working-copy change and its ancestors by `ce-work`, so there is no progress to preserve across edits
...
**Resolve the project profile from the shared cache first.** The agnostic profile (stack, deps, conventions, structure) is identical for the current Jujutsu working-copy change, so reuse it instead of having `repo-research-analyst` re-derive `technology`/`architecture`/`conventions` every run. Before the helper can write under `.tmp/`, inspect the repository-root `.gitignore` and add `.tmp/` if needed, preserving existing entries. Set `SKILL_DIR` to this skill's directory and run the helper (protocol in `references/repo-profile-cache.md`):
...
- Draft a clear, searchable title that states the intended outcome. Follow repository-local title conventions when they exist; do not impose commit-message syntax on plan titles.
- Determine the plan type from the repository's work-intent classification when one exists; otherwise use a neutral semantic category.
- Build the filename following the repository convention, or the neutral shape below when none exists.
  ...
  - Shape: `<date>-<sequence>-<descriptive-slug>-plan.<format>`; derive the slug from the neutral title rather than a fixed message prefix
...
- **All file paths must be repo-relative** — never use absolute paths like `/Users/name/Code/project/src/file.ts`. Use `src/file.ts` instead. Absolute paths make plans non-portable across machines, Jujutsu workspaces, and teammates. When a plan targets a different repo than the document's home, state the target repo once at the top of the plan (e.g., `**Target repo:** my-other-project`) and use repo-relative paths throughout
...
- Do not include `jj` commands, change descriptions, or exact test command recipes.
...
```text
docs/plans/YYYY-MM-DD-NNN-<descriptive-name>-plan.<md|html>
```
...
- Set `artifact_contract: rocketclaw-unified-plan/v1`, `artifact_readiness: implementation-ready`, and `execution: code` for software implementation plans.
- Do not set `artifact_contract: rocketclaw-unified-plan/v1` on universal-planning outputs, answer-seeking outputs, or approach-plans unless they include the full software implementation contract.
