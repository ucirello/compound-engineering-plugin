...
Elevation sends the one reasoning-heaviest step to a **user-chosen model**, so a user on a cheaper session model still gets a high-reasoning result without switching their whole session. It runs on **any harness**: the host serves the chosen model natively where routing permits, otherwise the selected OpenCode V2 or Claude CLI is invoked, otherwise the step runs inline on the session model with the fallback disclosed. The elevated call is read-only and verifies its own brief.
...
Resolve the per-skill **model choice immediately before adapter selection**, so the decision reflects the current conversation rather than an intake snapshot. The value is a model alias or an exact OpenCode `provider/model#variant` reference, not a boolean. Apply `SKILL.md`'s actual config-read gate before dispatch; an unreadable config is not evidence that routing is unset.
...
2. **Caller carrier** — a structured `<per-skill-key>:<model-alias>` token that an automatic orchestrator may pass in the invocation (LFG passes `plan_model:<alias>` to ce-plan; the analogous `brainstorm_model:<alias>` to ce-brainstorm). Use it when live user intent does not decide the choice. Strip it from the request text and never reconstruct it from product prose. It is honored in pipeline / `disable-model-invocation` runs. Accept the existing alias form `^[A-Za-z0-9._-]{1,64}$`, or OpenCode's discovered `provider/model#variant` reference (variant optional); a malformed carrier counts as absent, not guessed. Carry the paired `plan_harness` / `brainstorm_harness` routing token too when supplied.
3. **Config** — otherwise use the per-skill key: `plan_model` for ce-plan, `brainstorm_model` for ce-brainstorm, together with `plan_harness` / `brainstorm_harness`. Read them the **same way this skill's Phase 0.0 (output-mode resolution) resolves `plan_output` / `brainstorm_output`**: reuse the repo root already resolved, else run `jj workspace root`, then apply the ordinary-key rule (`config.local.yaml` then `config.yaml`). Reuse the Phase 0.0 reads if still in hand. Ignore commented (`#`-prefixed) lines. A model alias or OpenCode `provider/modelname#variant` selects it; missing / commented / invalid / no file selects none. `opencode` explicitly selects OpenCode V2, distinct from `claude`.
...
If the session model already **is** the resolved model, including any explicitly requested OpenCode variant, and no explicit routing requires another harness or subagent, there is nothing to elevate: skip dispatch (see Transparency for whether a line is still printed).

## Adapter selection

When elevation is active, honor effective explicit subagent and alternative-harness routing first. An explicit harness confines adapter selection to that harness before the disclosed inline fallback. Otherwise use this order:

For OpenCode, model identity means the exact discovered provider and model ID plus any explicitly requested variant. An omitted variant allows the server default; a supplied variant must match the receipt. Apply this identity rule to both native and CLI dispatch rather than accepting only a family match.

1. **Native in-harness dispatch.** On OpenCode V2, when `opencode.models` and `subagent(model)` are available and no explicit routing selects another mechanism, discover exact model IDs and variants and invoke the native subagent with `model: provider/model#variant`. Preserve the requested tier and any cross-model intent; never guess, silently reuse the current model, or launch a shell delegate on this route. If model discovery or native dispatch cannot serve that request, disclose the limitation and run inline. On other hosts, attempt the platform subagent primitive with a per-agent model override (e.g. `model: "fable"` on Claude Code). Capability is proven by attempt, not self-assessment. **Receipt rule (R6):** a "receipt" is the serving side's report of which model actually ran. A native run whose receipt violates the requested identity is discarded: OpenCode requires the exact provider, model, and requested variant; other hosts compare model families. Other hosts may fall through to the next permitted adapter. A run with no receipt proceeds and is recorded as unverified.
2. **Selected CLI.** Run the bundled `scripts/elevation-dispatch.sh` worker as a detached job (see Off-host dispatch). For `brainstorm_harness: opencode` or `plan_harness: opencode`, use `ELEVATION_HARNESS=opencode`; the worker uses only OpenCode V2's `/api/session` interface through `opencode api`. Discover/confirm the exact provider/model and variant before dispatch. Claude remains the default CLI when no harness is selected. Available when the selected executable is on PATH. Do not preflight authentication in the host command context: the detached worker's provider-capable call is authoritative, and an authentication failure there follows Recovery.
...
- On the **OpenCode V2 CLI** route the new session receives a deny-all permission ruleset with only read, glob, grep, webfetch, and websearch allowed. The worker reads V2 assistant messages, validates the model receipt and completion, and interrupts its own session on completion or failure. This does not disable installed server plugins or project instructions; pass the read-only boundary and evidence trust rules in the prompt too.

Hand over the working context as **file paths the subagent reads itself**, never a re-narrated prose brief. Resolve the absolute workspace root through `jj workspace root` (or the current directory outside JJ), create its private, owner-checked `.tmp` directory, and create **one private per-run handoff directory** with `mktemp -d "$WORKSPACE_ROOT/.tmp/elevation-XXXXXX"`. Write the prompt-file and every evidence file into *that* directory. On the Claude CLI route the worker grants the elevated model read access to only that one directory (via `--add-dir` on the prompt-file's parent), so the handoff files stay readable while unrelated scratch is not exposed:
...
## Off-host dispatch (selected CLI route)
...
```json
{
  ...
  "justification": "Allow the disclosed read-only reasoning-elevation request to reach the selected provider."
}
```
...
```
   ...
   ELEVATION_HARNESS="<resolved harness: claude or opencode>" CE_PEER_GRACE_SECS=40 CE_PEER_HARD_SECS=5400 CE_ELEVATION_HARD_SECS=5400 CE_PEER_LOG_MAX_BYTES=52428800 \
...
The Claude worker streams `--output-format stream-json --verbose`; the OpenCode worker reports only changed assistant messages as progress. Progress resets the idle window; a genuinely stalled model stops growing the log and is reaped while a productive long run continues. The 40-second runner grace lets the OpenCode worker interrupt its server-side session before being killed.
...
A successful run has JSON `status: ok`. Treat any result whose `receipt` is `mismatch` as a failure even when `status` is `ok`: discard the output and degrade to the session model. OpenCode compares the exact provider, model, and requested variant; other hosts compare the requested family. Native routing follows the adapter-selection rule above, including its OpenCode inline fallback.
