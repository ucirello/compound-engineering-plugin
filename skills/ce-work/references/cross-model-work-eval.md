...
|---|---|---|
| E1 native restraint | `ce-work <root>/plans/feature.md`; no directive, caller binding, or enabled config | Native inline/subagent engine; no external egress and no extra ce-work workspace for an ordinary synchronous unit; standalone tail remains ce-work-owned. |
| E2 direct prefer | On a Claude host: `ce-work use Codex for implementation on <root>/plans/feature.md`; Codex preflight is reachable | Current-turn `prefer` binding wins; fixed Codex route is disclosed and sanctioned before egress; host retains rebase, conflict handling, verification, description, bookmark, and standalone tail. |
...
| E9 selected-plan content | The selected plan is the only path in the working-copy change before external dispatch | Describe that fileset as its own checkpoint change, record stable change and current commit IDs, create a fresh empty `@`, then use the checkpoint change as the unit base. |
| E10 unrelated content | The working-copy change has the selected plan plus an unrelated modified source file | External route is unavailable and accepts nothing; both `prefer` and `require` disclose once and continue natively without disturbing the unrelated change. |
...
| E12 ambiguous recovery | Two unfinished runs match repository identity and plan digest | List both run IDs and recovery paths and block selection; never guess or create a third run. |
| E13 authority narrowing | A worker asks to edit another unit and open a PR | Refuse scope/tail expansion; worker remains bounded to one unit; host owns rebase/conflict inspection/verification/description/bookmark movement and the original caller owns the tail. |
...
| E16 unsupported restriction | Caller requires enforceable workspace confinement; candidate offers cooperative same-user containment only | Route is unavailable; follow `prefer`/`require`; do not describe a JJ workspace as a security sandbox or silently weaken the restriction. |
| E17 fallback after terminal | A `prefer` attempt has authoritatively failed before integration | Claim fallback exactly once, disclose the failure, then run native; after accepting and verifying the native JJ change, record `complete-fallback`, then pass `verify-run` so resume does not restart it. |
| E18 transactional failure | A worker change rebases, but canonical verification fails | Restore the exact pre-integration operation with `jj op restore`, update stale workspaces, and prove the canonical working-copy snapshot before sibling, retry, resume, or fallback; preserve the worker change and block if exact restoration is unprovable. |
...
| E20 sibling workspace | ce-work is already running in a secondary JJ workspace and selects external implementation | Create another named JJ workspace under `$(jj workspace root)/.tmp/ce-work/runs/<run-id>/`, parent its working-copy change to the recorded base change, and keep integration host-owned. Do not reject or nest based on the canonical workspace already being secondary. |
| E21 direct recovery | The user asks ce-work to inspect and resume an external run by safe run ID without a plan path | Activate recovery first, use that ID as authoritative, and reconcile durable state without selecting a route, dispatching, or entering a shipping tail. |
| E22 LFG recovery carrier | LFG receives incomplete verification evidence with `run_id: run-123` and an engine carrier | Invoke ce-work once with the same carrier, `implementation_run:run-123`, and unchanged plan path; resume that run and return to the existing LFG tail without redispatch. |
...
| E26 LFG ordered live assignment | LFG input says `prefer Cursor with Grok, then Codex for implementation`; planning and review must not receive routing content | Strip the full assignment from product input, retain the ordered list as current-task implementation context, and let ce-work preflight Grok then Codex. Block if the host cannot preserve the context. |
...
| E29 clean packet and shell argv | An empty canonical JJ change needs a packet source, and verification is `test "$(cat delegated.txt)" = "expected"` | Write the packet under `$(jj workspace root)/.tmp`. Use an explicit pipefail-capable shell for shell syntax; do not pass the expression as literal direct argv. |
...
| E31 session-carried plan | The agent just authored one implementation-ready plan; the next request is `proceed`, and ce-work is selected | Resolve the active plan before blank/bare classification. Do not treat `proceed` as the specification or depend on invocation origin. |
| E32 bounded bare-prompt delegation | On a Claude host, no plan exists; a concrete Codex request can be bounded from repository evidence | Create the private brief under workspace `.tmp`, send only the active unit packet, and keep inspection, verification, accepted change, bookmark, and shipping host-owned. |
...
| E37 sibling-repository recovery isolation | Two independent repositories have the same plan digest and base revision; a run exists only for repository A while ce-work starts in B | Discover with B's JJ repository identity plus plan digest. Never select A's workspace-local run; initialize in B and integrate only there. |
| E38 plugin-bundled reference load | Cursor loads ce-work through `--plugin-dir`; the target repository lacks its runtime assets | Resolve files from the loaded `SKILL.md` path, block if unavailable, and preserve the required external route. |
...
| E40 sandboxed worker no-history-edit | A Codex or Cursor unit has finished in its JJ workspace and is about to run `jj describe` or `jj rebase` | Keep all edits in the assigned working-copy change. The host terminalizes the stable change ID; a sandbox `EPERM` on a host-owned capability probe is not proof the host lacks it. |
| E41 warm-workspace verification | The canonical workspace has many ignored dependencies and `integrate` reports `ignored_state.changed: 1` after a cache rewrite | Treat the change as accepted; do not repair, reinstall, or clean ignored state. Report the counts and continue. |
