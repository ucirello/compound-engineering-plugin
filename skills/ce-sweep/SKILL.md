...
**Resolve the workspace root at runtime.** Run `jj workspace root` as one shell-tool call. If it succeeds with an absolute path, use that as `<workspace-root>`; if JJ is unavailable or the command fails, use the current working directory as the local fallback. Read `<workspace-root>/.rocketclaw/config.local.yaml` with the native file-read tool.
...
- `sweep_state_path` — path to the state file, established at setup; fallback `docs/feedback-sweep/state.yml`. A path outside `.tmp/rocketclaw/` means tracked mode (the state file is included in each run's JJ change and must not be ignored); a path under `<workspace-root>/.tmp/rocketclaw/` means workspace-local mode (the state file is never included in the change — only the plan is).
- `sweep_lease_ttl_minutes` — single-writer lease staleness threshold; default `60`. Passed to `lease-acquire` in 2a.
- `sweep_shared_branch` — compatibility key set to `true` when the state file is published through a shared bookmark that multiple workspaces update (see 2a topology); default `false`.
...
- `<scratch-root>` = `<workspace-root>/.tmp/rocketclaw/ce-sweep/<run-id>/`. Every temporary, cache, download, extraction, and analysis artifact for the run stays under this path. When there is no JJ workspace, `<workspace-root>` is the current-working-directory fallback resolved in Phase 0; never redirect scratch to an OS temp or cache directory.
...
**Shared-bookmark topology** (`sweep_shared_branch: true`): resolve and retain the configured shared bookmark as `$SHARED_BOOKMARK` and its intended remote as `$REMOTE`. Before any source-side write, use JJ filesets to isolate only the state file in a dedicated change without altering unrelated working-copy content, describe that change, set the shared bookmark to it, and publish only it with `jj git push --remote "$REMOTE" --bookmark "exact:$SHARED_BOOKMARK"`. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Inspect actual message syntax with `git log`; do not use Git for any other purpose. The project's active instructions and conventions and the syntax observed in `git log` always take precedence over compatible Go guidance. Retain the repository's semantic requirements, but do not impose fixed syntax, prefixes, types, scopes, subjects, bodies, examples, or templates. A rejected push means another writer won the bookmark: run `jj git fetch --remote "$REMOTE"`, rebase the lease change onto the updated remote bookmark with `jj rebase`, re-run `lease-acquire`, and if the lease is still not yours, back off (record `aborted-locked` and stop). Only once the lease change is published and confirmed do you touch a source.
...
For each `fix_pending` item, resolve its claimed fix ref and verify it reached the default branch's remote bookmark. The fix ref originates from untrusted feedback content (a thread claim, an analyzer-extracted reference), so **validate its shape before it reaches any `jj`/`gh` command**: accept only a bare PR number (`#?\d+`) or an exported Git SHA (`[0-9a-f]{7,40}`), and treat anything else as an unresolved claim (leave the item open). This blocks argument/flag injection into the shell command.
- Resolve the explicit relevant JJ Git remote as `<remote>`, the default bookmark name as `<default-bookmark>`, and the explicit remote bookmark as `<default-bookmark>@<remote>`. If any cannot be resolved, leave the claim unverified. Run `jj git fetch --remote <remote>` before checking ancestry; never verify against only a local bookmark.
- For a PR, run `gh pr view <validated-ref> --json mergedAt,baseRefName,mergeCommit`; require `mergedAt`, require `baseRefName` to equal `<default-bookmark>`, and validate `mergeCommit.oid` as an exported Git SHA. That merge commit is `<landing-sha>`. For a SHA ref, the validated SHA is `<landing-sha>`.
- Run `jj log -r '<landing-sha> & ::<default-bookmark>@<remote>' --no-graph`; a non-empty result verifies that exact landing revision against the freshly fetched default remote bookmark.
...
- Verified -> perform the source's configured close-out action (same write -> read-back -> confirm discipline as 2d), then `upsert-item` with `status: closed` carrying all three evidence fields: `fix_ref`, `verified_merge_sha` set to `<landing-sha>`, and `verified_at`. Close-out is terminal.
...
**Rotation check first.** If the file exists and its frontmatter is NOT both `product_contract_source: feedback-sweep` and `artifact_readiness: requirements-only`, archive it untouched to a dated sibling `docs/plans/feedback-sweep-plan-YYYY-MM-DD.md` and write a fresh plan from the template. Never overwrite an unrelated plan in place.
...
- **Describe the change.** Use JJ filesets to isolate ONLY `docs/plans/feedback-sweep-plan.md` plus `<state>` when it is tracked in the sweep's JJ change, preserving all unrelated working-copy content; never include `.tmp/rocketclaw/` state or unrelated paths. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Inspect actual message syntax with `git log`; do not use Git for any other purpose. The project's active instructions and conventions and the syntax observed in `git log` always take precedence over compatible Go guidance. Retain the repository's semantic requirements, but do not impose fixed syntax, prefixes, types, scopes, subjects, bodies, examples, or templates. Apply the resulting description with `jj describe`; a description failure is reported, not fatal. In local-change mode, never push. In shared-bookmark mode (`sweep_shared_branch: true`), run `jj git fetch --remote "$REMOTE"`, rebase with `jj rebase`, update `$SHARED_BOOKMARK`, and publish only it with `jj git push --remote "$REMOTE" --bookmark "exact:$SHARED_BOOKMARK"`.
