...
1. Include only files reported by fixers in the current change. If unrelated edits are present, use `jj split` or a path-limited `jj commit` to keep this change scoped to the fixer files. Describe the change with a message referencing the PR:
