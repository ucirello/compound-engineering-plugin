...
2. **Caller carrier** — a structured `<per-skill-key>:<model-alias>` token that an automatic orchestrator may pass in the invocation (LFG passes `plan_model:<alias>` to ce-plan; the analogous `brainstorm_model:<alias>` to ce-brainstorm). Use it when live user intent does not decide the choice. Strip it from the request text and never reconstruct it from product prose. It is honored in pipeline / `disable-model-invocation` runs. Aliases match `^[A-Za-z0-9._-]{1,64}$`; OpenCode model selectors use discovered `provider/modelname#variant` IDs instead. Carry `plan_harness:opencode` or `brainstorm_harness:opencode` with the corresponding selector. A malformed carrier counts as absent, not guessed.
3. **Config** — otherwise use the per-skill key: `plan_model` with `plan_harness` for ce-plan, `brainstorm_model` with `brainstorm_harness` for ce-brainstorm. Read them the **same way this skill's Phase 0.0 (output-mode resolution) resolves `plan_output` / `brainstorm_output`**: reuse the workspace root already resolved, else run `jj workspace root` with an absolute cwd, then apply the ordinary-key rule (`.rocketclaw/config.local.yaml` then `.rocketclaw/config.yaml`). Reuse the Phase 0.0 reads if still in hand. Ignore commented (`#`-prefixed) lines. A model alias or valid OpenCode selector selects it; missing / commented / invalid / no file selects none. An explicit effective subagent or alternative-harness route wins over a native default.
...
If the session model already **is** the resolved model, including any requested OpenCode variant, there is nothing to elevate: skip dispatch (see Transparency for whether a line is still printed). An unknown session variant does not establish a match.

## Adapter selection

When elevation is active, resolve the governing route below, then use the first permitted adapter that serves the requested model. The native-default OpenCode route permits only native dispatch or disclosed inline fallback; the numbered CLI fallback applies to other routes.

**OpenCode V2 branch.** Before selecting a native default, actually read both target-workspace `.rocketclaw` config layers for explicit subagent and harness routes, even when live intent chose the model or output resolution skipped config. Reuse only current file contents already read, not an assumed empty config. Merge ordinary keys by the rule above and preserve the complete model selector, including `#variant`.

For `plan_harness: opencode` (or `brainstorm_harness: opencode`), use OpenCode rather than interpreting its provider/model selector as a Claude alias. On an OpenCode host with `opencode.models` and native `subagent(model)`, discover the exact ID and variant and delegate natively as `provider/model#variant`, preserving the high-reasoning tier and receipt rule below. Use the same native route by default only when no explicit subagent or alternative-harness route governs. Never guess a model or silently reuse the current one. Native-default discovery or dispatch failure uses the disclosed inline fallback, never shell delegation. The bundled OpenCode V2 adapter is available for an explicitly selected OpenCode harness when native dispatch cannot serve it; do not send its selector to Claude. An explicit alternative route keeps its own dispatch mechanism.

1. **Native in-harness dispatch.** Attempt the platform subagent primitive with a per-agent model override (e.g. `model: "fable"` on the Claude Code `Agent`/`Task` tool). Capability is proven by attempt, not self-assessment — a harness that can serve the model natively does; one that cannot fails the attempt and falls through. **Receipt rule (R6):** a "receipt" is the serving side's report of which model actually ran. An OpenCode receipt must match the requested provider, model, and any requested variant; other native routes compare model family. A mismatching receipt falls through to the next adapter. A run with no receipt proceeds as unverified (it does not fall through).
2. **Selected CLI.** When the governing route permits CLI dispatch, run the bundled `scripts/elevation-dispatch.sh` worker as a detached job (see Off-host dispatch). Carry the effective harness into `ELEVATION_HARNESS`. With `ELEVATION_HARNESS=opencode`, the worker calls its bundled `scripts/opencode-elevation.py` through OpenCode V2's `/api/session` interface using `opencode api`. Confirm the exact provider/model and variant before dispatch. Claude is the default CLI only when no harness was selected and the OpenCode native-default condition does not apply. Available when the selected executable is on PATH. Do not preflight authentication in the host command context: the detached worker's provider-capable call is authoritative, and an authentication failure there follows Recovery.
...
- On the **OpenCode V2 CLI** route the new session receives a deny-all permission ruleset with only read, glob, grep, webfetch, and websearch allowed. The worker validates assistant completion and the exact model/variant receipt, then interrupts its own session on completion or failure. Session permissions do not disable installed server plugins or project instructions; include the read-only boundary and evidence trust rules in the prompt too.

Hand over the working context as **file paths the subagent reads itself**, never a re-narrated prose brief. Resolve the workspace root with `jj workspace root` (outside JJ, use the absolute current directory). Create its `.tmp` directory if absent, reject a symlink scratch root, and create **one private per-run handoff directory** with `mktemp -d "$WORKSPACE_ROOT/.tmp/elevation-XXXXXX"`. Write the prompt-file and every evidence file into *that* directory. On the Claude CLI route the worker grants the elevated model read access to only that one directory (via `--add-dir` on the prompt-file's parent), so the handoff files stay readable while the rest of the scratch root — other same-user scratch and credentials — is not exposed:
