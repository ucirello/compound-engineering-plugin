...
If this workflow composes, edits, validates, or recommends a Jujutsu change description or commit message, determine syntax from the project's active instructions and actual `git log`; those sources always win over compatible Go guidance. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Do not impose a fixed prefix, type, scope, message, subject/body shape, template, or example.
