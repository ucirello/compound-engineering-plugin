...
**The fix:** One hypothesis, one change, one test. If the first change does not fix it, remove the attempted hunks with `jj restore -i` or isolate them with `jj split` before trying the next. Changes should be additive to understanding, not cumulative to the codebase.
