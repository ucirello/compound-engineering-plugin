...
- **Read** `docs_root` from the repository's shared workflow configuration only (`<repo-root>` = `jj workspace root`). Do not read a checkout-local override. Unset -> `<root>` is `docs`.
- **Validate** a set value: a repo-relative directory whose real, symlink-resolved path stays inside the workspace and is neither the workspace root nor under JJ or Git repository metadata. Otherwise stop with an error naming `docs_root` and the value -- never fall back to `docs`.
...
Read the plan metadata before continuing past step 1's gate. A unified plan proceeds only when it is `artifact_readiness: implementation-ready` with `execution: code`. Every other value stops the pipeline: `artifact_readiness: requirements-only`, any unrecognized readiness value, an invalid progress-like readiness value, and `execution: knowledge-work`. An output that is not an implementation plan at all — an approach plan, an answer-seeking or universal output — stops it too, whether or not it carries a contract marker.
