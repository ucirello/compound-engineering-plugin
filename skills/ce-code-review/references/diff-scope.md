...
2. **Current stack vs discovered base.** Review the resolved Stage 1 diff from
   its JJ fork point through `@`, including the full unpublished stack and the
   working-copy change. A non-empty `jj diff -r @` does not narrow scope to `@`
   alone.
