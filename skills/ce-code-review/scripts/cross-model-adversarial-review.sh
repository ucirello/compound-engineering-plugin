...
#   <base-ref> the diff base (e.g. a JJ revision or bookmark); the peer reviews
...
REPO_ROOT="$(jj root 2>/dev/null)" || skip "not inside a JJ workspace; skipping"
...
# embedded and needs no JJ command.
if [ "$PEER" = codex ]; then
