...
4. If `jj st` (cwd = workspace root) shows working-copy changes, commit only the review-driven files — there is no staging area; name those paths on `jj commit` so unrelated working-copy paths stay out of the change. Never snapshot the entire working copy.
