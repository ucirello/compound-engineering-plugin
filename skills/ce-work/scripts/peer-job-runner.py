...
Environment overrides (defaults in parentheses):
  CE_PEER_JOBS_ROOT         base dir ($(jj workspace root)/.tmp, or local .tmp
                            when this is not a JJ workspace)
...
POSIX path is behaviorally unchanged:
  ...
  jobs root defaults under $(jj workspace root)/.tmp (then local .tmp),
            owner-private.
...
def _jj_workspace_tmp() -> str:
    ...
    path = os.path.join(os.path.abspath(root), ".tmp")
...
def jobs_root_base() -> str:
    ...
    if DEFAULT_ROOT is None:
        raise RunnerError("cannot derive the jobs root from the JJ workspace")
