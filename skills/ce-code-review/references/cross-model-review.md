...
2. Scope is `local-aligned` or standalone — the working copy IS the reviewed head. Skip in `pr-remote` / `remote-bookmark`: the peer reviews the local working copy, which is not the PR/bookmark head.
...
bash "$SKILL_DIR/scripts/cross-model-adversarial-review.sh" "<peer>" "<base-revision>" "<run-dir>"
```

- `<peer>` = `XPEER` from Step 1 (`codex` or `claude`).
- `<base-revision>` = the Stage 1 `BASE` (the diff base the peer reviews via `jj diff --from <base-revision> --to @`).
...
`scripts/cross-model-adversarial-review.sh <peer> <base-revision> <run-dir>`:
