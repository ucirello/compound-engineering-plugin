...
**Pack discovery (every tier).** Compound Packs declared in repo config constrain the Product Contract on every repo-backed software path that reaches synthesis — Lightweight, Standard, Deep, and the Phase 0.2 route that skips the scan below for already-clear requirements — so resolve them before the tier split, by running this skill's resolver:
