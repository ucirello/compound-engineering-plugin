...
2. **Working-copy revision.** If `jj diff -r @` is non-empty, review those changes. JJ snapshots the working copy automatically.
3. **Unpushed revisions vs base bookmark.** If `@` is empty, review `jj diff --from 'heads(::@ & ::<base>)' --to @`, where `<base>` is the resolved default bookmark.
...
When the review context includes `<pr-scope-mode>pr-remote</pr-scope-mode>` or `<pr-scope-mode>bookmark-remote</pr-scope-mode>`, the workspace is **not** the reviewed head. Do **not** use Read/Grep on workspace paths for files in the changed-file list — they may not match the bookmark or PR under review.

Instead:

- For `pr-remote`, prefer `jj file show -r <pr-head-ref> <path>` when the fetched ref is provided. Otherwise read the authoritative remote file with `gh api repos/<pr-head-repository>/contents/<path>?ref=<pr-head-oid> -H 'Accept: application/vnd.github.raw+json'`.
- For `bookmark-remote`, use `jj file show -r <bookmark-head-ref> <path>` when provided; otherwise rely on diff hunks.
