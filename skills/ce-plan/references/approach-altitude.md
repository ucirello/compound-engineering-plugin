...
- **Scoping synthesis** (Phase 0.7 / 5.1.5): a *scope* checkpoint for a deliverable already chosen — it confirms what the implementation plan will target. Approach altitude is an *altitude* checkpoint that decides whether to proceed with the deliverable at all; it sits above the implementation plan, not inside producing one.
