...
**(c) No instruction-file port grep.** The script does not grep agent-instruction files for port references. They carry natural language that may mention ports in contexts unrelated to the dev server (documentation, examples, troubleshooting), producing false positives that are hard to debug. Framework config files and `.env` are the reliable sources of truth. The agent may still honor a dev-server port from the project's active instructions already in context, but the script does not shell-grep instruction files.
