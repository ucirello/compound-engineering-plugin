...
If invoked specifically to create or bootstrap `CONCEPTS.md` (e.g., "create a CONCEPTS.md", "build the concept map", "set up shared vocabulary"), the intent is ambiguous between two jobs — building the vocabulary file and running a docs/solutions refresh — so disambiguate before proceeding. Use the harness's blocking question capability. Fall back to numbered options in chat only when no blocking question capability exists or its call errors. Never silently skip the question. Two options:

1. **Create CONCEPTS.md (build the concept map)** — seed the repo-wide concept map and record it as a described change; skip only the docs/solutions classification phases (Phases 0–4). Read `references/concepts-vocabulary.md` and follow its **Seed goal** and **Scope of a seed** (repo-wide) rules: seed the project's core domain nouns from the declared domain model (schema, core types, primary models, top-level domain docs), each meeting the qualifying bar, the codebase setting the count. Write the preamble (see Phase 4.5), cluster per the organization rules, and run the Discoverability Check so the project's active root instruction source surfaces the new file. Then **enter Phase 5 (Record Changes)** to describe and, when selected, publish the new `CONCEPTS.md` and any instruction-source edit through the same durable-write flow the refresh uses — do not leave the bootstrap undescribed.
...
- Ask questions **one at a time** using the harness's blocking question capability. Fall back to numbered options in plain text only when no blocking question capability exists or its call errors. Never silently skip the question
...
| **Delete** | No longer useful, applicable, or distinct | Delete the file — Jujutsu history preserves it if anyone needs to recover it later |
...
10. **Delete, don't archive.** There is no `_archived/` directory. When a doc is no longer useful, delete it. Jujutsu history preserves every deleted file — that is the archive. A dedicated archive directory creates problems: archived docs accumulate, pollute search results, and nobody reads them. If someone needs a deleted doc, use `jj log --summary -- docs/solutions/` to locate the revision that removed it, then inspect that revision with `jj show`.
...
- **Auto memory** — if the harness injected an auto-memory block into the system prompt, does it contain entries in the same problem domain? Scan that block directly. If the block is absent, skip this dimension. A memory note describing a different approach than what the learning recommends is a supplementary drift signal.
...
- Add context to the evidence report, tagged with `(auto memory)`, when memory notes suggest the approach may have changed since the learning was written
...
>
> Also scan any auto-memory block injected into your system prompt. Check for notes related to the learning's problem domain. Report any memory-sourced drift signals separately from codebase-sourced evidence, tagged with `(auto memory)` in the evidence section. If the block is not present in your context, skip this check.
...
The Consolidate action is: merge unique content from the subsumed doc into the canonical doc, then delete the subsumed doc. Not archive — delete. Jujutsu history preserves it.
...
Action: delete the file. No archival directory, no metadata — just delete it. Jujutsu history preserves every deleted file if recovery is ever needed.
...
- Prefer the platform's native content-search capability over shell. Drop to shell when materially better for the case.
...
- **Decorative** — principle stated inline, citation is a "see also" pointer or bare attribution. Delete is fine; clean up citations in the same described revision.
...
Always present choices using the harness's blocking question capability. Fall back to numbered options in plain text only when no blocking question capability exists or its call errors. Never silently skip the question.
...
Note: if this run **creates** `CONCEPTS.md` from scratch, the Discoverability Check below also surfaces it so future agents can discover it — by editing the project's substantive root instruction source in interactive mode (with consent), or, in headless mode, by emitting a "Discoverability recommendation" line in the report rather than editing instruction sources (per the headless boundary in step 4c — headless does doc maintenance, not project config). Either way the created file is surfaced or flagged for surfacing; subsequent runs skip this because the instruction source is already current or the recommendation was already reported.
...
For **Keep** outcomes, list them under a reviewed-without-edits section so the result is visible without creating revision churn.
...
## Phase 5: Record Changes

After all actions are executed and the report is generated, record the modifications as a Jujutsu change. Skip this phase if no files were modified (all Keep, or all writes failed).

### Detect Jujutsu context

Before offering options, run `jj status`, inspect `jj log -r 'trunk()..@'`, and list bookmarks in the current stack with `jj bookmark list -r '::@'`. Determine:

1. Whether the working-copy change is directly based on `trunk()` or already belongs to a feature stack
2. Whether `@` contains modifications beyond the files RocketClaw refresh changed
3. Whether an existing local bookmark names the feature stack

Jujutsu has no active bookmark and no staging area. Never infer a current line of work from workspace position alone. If unrelated modifications share `@`, isolate only the refresh paths with `jj split -m <description> <refresh-paths>`; the selected paths become the described `@-` revision and unrelated modifications remain in `@`. If no unrelated modifications exist, run `jj describe -m <description>` and then `jj new`; the recorded refresh revision is then `@-`.

For a new line of work, create a bookmark at the recorded revision with `jj bookmark create <bookmark> -r <refresh-revision>`. For an existing line of work, advance its bookmark with `jj bookmark set <bookmark> -r <refresh-revision>`. Publish only that bookmark with `jj git push --bookmark <bookmark>`.
...
|---------|---------------|
| Directly based on `trunk()` with no feature bookmark | Isolate the refresh revision, create a specific bookmark at that revision, publish it with `jj git push --bookmark <bookmark>`, and attempt to open a review request through an available repository-host interface. If review-request creation fails, report the bookmark. |
| Existing feature stack | Isolate the refresh as a separate described revision, move the stack's bookmark to it when needed, and publish that bookmark if the stack is already published. |
| Jujutsu operation fails | Include the recommended `jj` or `jj git` commands in the report and continue. |

Only the files RocketClaw refresh modified belong in its described revision. Leave unrelated working-copy modifications in their existing change.

### Interactive mode

Present only options supported by the detected Jujutsu state.

**If directly based on `trunk()` with no feature bookmark:**

1. Create a specific bookmark for the refresh revision, publish it, and open a review request (recommended)
2. Record the revision without publishing it
3. Leave the changes undescribed

**If already on a feature stack and there are no unrelated modifications in `@`:**

1. Record the refresh as a separate revision in the current stack (recommended)
2. Record it on a separate bookmark
3. Leave the changes undescribed

**If unrelated modifications share `@`:**

1. Split only the RocketClaw refresh paths into a separate described revision and leave all other modifications in `@` (recommended)
2. Leave the changes undescribed

### Change description

Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards.

The project's active instructions and syntax observed at runtime via `jj log` always win; apply only compatible Go guidance. Do not impose a fixed prefix, type, scope, template, or example. Describe the refresh outcome and affected knowledge area succinctly; the changed files carry the detail. Apply the description with `jj describe` to the isolated refresh revision.
...
After the refresh report is generated, check whether the project's active root instructions would lead an agent to discover and search `docs/solutions/` before starting work in a documented area. This runs every time — the knowledge store only grows in value when agents can find it. If this check produces edits, they are recorded as part of (or immediately after) the Phase 5 change flow — see step 6 below.

1. Identify the project's active root instruction source and any shim that includes it. The substantive source is the assessment and edit target; ignore shims. If no root instruction source exists, skip this check entirely.
...
4. If not:
   ...
   b. Draft the smallest addition that communicates the three things. Match the file's existing style and density. The addition should describe the knowledge store itself, not the plugin.
      ...
      ```
   c. In interactive mode, explain to the user why this matters — agents working in this repo (including fresh sessions, other tools, or collaborators without RocketClaw) won't know to check `docs/solutions/` unless the instruction source surfaces it. Show the proposed change and where it would go, then use the harness's blocking question capability to get consent before making the edit. Fall back to presenting the proposal in chat only when no blocking question capability exists or its call errors. Never silently skip the question. In headless mode, include it as a "Discoverability recommendation" line in the report — do not attempt to edit instruction files (headless scope is doc maintenance, not project config).
...
6. **Fold in or create a follow-up change when the check produces edits.** If step 4 or step 5 edited an instruction source after Phase 5 recorded the refresh, use `jj squash --into <refresh-revision> <instruction-paths>` when that revision has not been published. If it has been published, describe the instruction edits as a follow-up revision, move the same bookmark to the new tip, and publish it with `jj git push --bookmark <bookmark>` so the open review request includes the update. If the user chose to leave the refresh undescribed, leave these edits in the same working-copy change; no separate revision logic is needed.
