...
2. Scope is `local-aligned` or standalone — the working copy IS the reviewed head. Skip in `pr-remote` / `bookmark-remote`: the peer reviews the local tree, which is not the PR/bookmark head.
...
- Self-locates the persona + schema via `BASH_SOURCE` (works from any CWD); derives the repo root from `jj root`.
