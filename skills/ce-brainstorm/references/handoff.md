...
skills run (plus `jj`/`gh` for the PR and CI steps, which it guards when absent; pair every `gh` call that talks to the underlying git repo with `GIT_DIR=$(jj git root)`).
