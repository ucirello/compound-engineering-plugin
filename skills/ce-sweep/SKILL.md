...
If the line above is an absolute path, use it as `<workspace-root>`. If it is empty or still shows a backtick command string (a harness that did not pre-resolve), run `jj workspace root` with the shell tool. Outside a JJ workspace, use the current project directory as `<workspace-root>`. Read `<workspace-root>/.rocketclaw/config.local.yaml` with the native file-read tool.
...
- `sweep_state_path` — path to the state file, established at setup; fallback `docs/feedback-sweep/state.yml`. A workspace-internal durable path means tracked mode (the state file is included in each described JJ change and must not be ignored); a path under `<workspace-root>/.tmp/rocketclaw/` means local mode (the state file is never tracked; only the plan is).
- `sweep_lease_ttl_minutes` — single-writer lease staleness threshold; default `60`. Passed to `lease-acquire` in 2a.
- `sweep_shared_bookmark` — bookmark name when several JJ workspaces publish the tracked state through the same remote bookmark (see 2a topology); omit for local-only coordination.
...
- `<scratch-root>` = `<workspace-root>/.tmp/rocketclaw/ce-sweep/<run-id>/`. If `jj workspace root` is unavailable, use `<current-project-directory>/.tmp/rocketclaw/ce-sweep/<run-id>/`. Never use OS-global temporary storage.
...
**Shared-bookmark topology** (`sweep_shared_bookmark` is set): before any source-side write, include only the state file in a dedicated JJ change, describe it, point the configured bookmark at that change, and publish it with `jj git push --bookmark <bookmark>`. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Repository-local instructions and syntax observed in `git log` take precedence; use Go guidance only when compatible. Do not impose a type, scope, prefix, subject pattern, or example from this skill. Use `jj file track <state>` when the file is untracked, and use path-limited `jj split` or `jj squash` so unrelated working-copy changes never enter the lease change. A rejected push means another writer won the remote bookmark: run `jj git fetch`, rebase the local lease change onto `<bookmark>@<remote>`, resolve only the state-file conflict through the state engine, re-run `lease-acquire`, and publish again. If the lease is still not yours, back off (record `aborted-locked` and stop). Only once `jj git fetch` shows the published lease at `<bookmark>@<remote>` do you touch a source. Run `jj new` afterward so later work does not rewrite the published lease change.
...
- Download attachments into `<scratch-root>/media/`; raw media is never tracked. A download failure -> set the item `needs_download` and continue.
...
For each `fix_pending` item, resolve its claimed fix ref and verify it landed on the default remote bookmark. The fix ref originates from untrusted feedback content (a thread claim, an analyzer-extracted reference), so **validate its shape before it reaches any `jj`/`gh` command**: accept only a bare PR number (`#?\d+`) or a Git commit ID (`[0-9a-f]{7,64}`), and treat anything else as an unresolved claim (leave the item open). This blocks argument/flag injection into the shell command.
- Ask GitHub for its default branch and PR state with `gh repo view --json defaultBranchRef` and `gh pr view <validated-ref> --json mergedAt,baseRefName,mergeCommit`; GitHub's branch and merge-commit fields remain the remote interoperability boundary. Run `jj git fetch`, map that branch to `<default>@<remote>`, and verify a validated Git commit ID with `jj log -r '<validated-id>::<default>@<remote>' --no-graph`. A non-empty exact-ID result proves ancestry; never interpolate an unvalidated value into a revset.
...
- **Describe a JJ change.** Include ONLY `docs/plans/feedback-sweep-plan.md` plus `<state>` in tracked mode; never include `<workspace-root>/.tmp/rocketclaw/` or unrelated working-copy changes. Use path-limited `jj split` or `jj squash` to isolate those paths, then describe the resulting change. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Repository-local instructions and syntax observed in `git log` take precedence; use Go guidance only when compatible. Do not impose a type, scope, prefix, subject pattern, or example from this skill. A description failure is reported, not fatal. Without `sweep_shared_bookmark`, never publish. With it, run `jj git fetch`, rebase the final sweep change onto `<bookmark>@<remote>` if needed, move the bookmark to the final change, and publish with `jj git push --bookmark <bookmark>`; report conflicts or rejection rather than overwriting remote history.
