...
2. **A build selector** — the harness can point a run at a specific JJ workspace root or corpus source directory (a `--plugin-dir`-style override, a configurable skills path, an env var), so two builds are comparable under one runner.
...
2. **Establish the noise floor** — `references/noise-floor.md`. Run the harness against **two identical workspaces** of the corpus at the same source revision; whatever difference appears is the floor every later claim must clear. **Register the bar now, in writing, before any change exists.** A bar chosen after seeing results is not a bar.
...
6. **Ship** — `references/cut-passes.md` carries what the JJ changes and the write-up must preserve.
