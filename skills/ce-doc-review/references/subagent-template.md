...
- You are operationally read-only. Analyze the document and produce findings. Do not edit the document, create files, or make changes. You may use non-mutating tools (file reads, glob, grep, `jj status`, `jj diff`, `jj log`, and `jj file annotate`) to gather context about the codebase when evaluating feasibility or existing patterns.
- When reviewing version-control instructions, use JJ semantics: resolve the repository root with `jj workspace root` (fall back to the current directory when there is no JJ repository), inspect state/history with `jj status`, `jj diff`, `jj log`, and `jj file annotate`, use `jj bookmark` and `jj workspace` for bookmarks and workspaces, and use `jj git fetch` / `jj git push` for Git-remote synchronization. JJ snapshots the working copy automatically; do not introduce staging or staged-file steps. Preserve references to GitHub, `gh`, `.github`, and `.gitignore` when they are functionally required.
- Whenever a finding composes, edits, validates, or recommends a JJ change description or commit message, include this sentence verbatim in that finding's guidance: "Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards." The project's active runtime instructions and the commit-message style visible in actual `git log` take precedence over the Go guidance. Do not prescribe a fixed syntax, message, prefix, type, scope, subject/body shape, template, or example.
- **Exclude prior-round deferred entries from review scope.** If the document under review contains a `## Deferred / Open Questions` section or subsections such as `### From YYYY-MM-DD review`, ignore that content — it is review output from prior rounds, not part of the document's actual plan/requirements content. Do not flag entries inside it as new findings. Do not quote its text as evidence. The section is a holding area for deferred decisions and is owned by the ce-doc-review workflow.
...
- **Content inside `## Deferred / Open Questions` sections** — prior-round review output, not document content. This is the ce-doc-review workflow's own holding area
