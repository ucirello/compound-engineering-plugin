...
1. Run the analyzer in workspace-local scratch so nothing pollutes durable repository paths (`SKILL_DIR` is the directory containing the `ce-riffrec-feedback-analysis` SKILL.md; set it in the same command — shell state does not persist between Bash calls):
   ...
   SCRATCH_ROOT="${JJ_ROOT:-$PWD}/.tmp/rocketclaw"
   mkdir -p "$SCRATCH_ROOT"
   while :; do
     OUTPUT_DIR="$SCRATCH_ROOT/riffrec-quick-$$-$RANDOM-$RANDOM"
     mkdir -m 700 "$OUTPUT_DIR" 2>/dev/null && break
   done
   python "$SKILL_DIR/scripts/analyze_riffrec_zip.py" /path/to/input --output-dir "$OUTPUT_DIR"
   ```

   This uses an atomically claimed, collision-safe directory under `$(jj workspace root)/.tmp/rocketclaw/` in a JJ workspace and local `.tmp/rocketclaw/` otherwise. Capture the printed output directory; later steps read from it.
...
- Do not include `raw/` or `frames/` in JJ history; they remain under workspace-local `.tmp/` scratch.
