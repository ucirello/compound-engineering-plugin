...
**(c) No project-instruction port grep.** Project instructions carry natural language that may mention ports in contexts unrelated to the dev server, producing false positives that are hard to debug. Framework config files and `.env` are the reliable machine-readable sources. The AI Assistant may still honor a dev-server port from the project's active instructions already in context, but the script does not shell-grep instruction prose.
