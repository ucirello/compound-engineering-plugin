...
- **File absent:** create it (and its `.rocketclaw/` directory) with the key. Keep that machine-local file out of any change without editing a tracked `.gitignore` (this is a drafts-only action) and without reading or writing inside `.jj/` or `.git/`. `ce-setup` is the canonical place that adds the shared `.gitignore` entry for teammates. Without any ignore, a later change could include machine-local opt-out state.
...
After recording, confirm it in one line so the write isn't silent and the user knows how to undo it — e.g. "Got it — I won't bring up Spiral here again (saved to `.rocketclaw/config.local.yaml`, kept out of the working copy). Want it back later? Just ask, or remove the `ce_promote_spiral_optout` key." Keep it to a single line; don't belabor it.
