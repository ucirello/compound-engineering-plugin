...
2. **Setup Environment**
   ...
   **Bookmark.** Determine the default bookmark from `main@origin` (or the remote default). If that is unavailable, use the host's repository-metadata capability; when the available interface is GitHub CLI, pair it with `GIT_DIR=$(jj git root)` and run `gh repo view --json defaultBranchRef`. Otherwise use `main` or `master` when one exists. Read the current bookmark with `jj bookmark list -r @`.

   If you are on the default bookmark or cannot tell, create a feature bookmark named from the plan or work description (`jj new` then `jj bookmark set <name>`). When a remote default exists, run `jj git fetch` first. Base the new change on the fetched `<default>@origin` when `@` has no commits beyond it; base it on `@` when local commits must remain visible to idempotency and the shipping gate, or when there is no remote. After the bookmark move, run `jj bookmark list -r @` again and treat that result as authoritative. When bookmark safety remains uncertain, use a spare bookmark and never make incremental commits on the real default.
   ...
   **Pre-work scope.** Before editing, record `jj diff --name-only` (the user's in-progress files) and whether `@` carries commits not on the remote default (`jj log '<default>@origin..@'`; "unknown" without a remote). Nothing in that set is yours to commit or publish, and it rides along on the bookmark move untouched — no stash, no question, no effect on naming. It is enforced without a menu:
