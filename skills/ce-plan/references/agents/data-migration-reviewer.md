...
Run this **first** when the caller provides a concrete diff and `db/schema.rb` or `db/structure.sql` appears in that diff. Use the review base from caller context (`<review-base>` — a JJ revision/revset or commit id accepted by `jj diff --from`). **Never assume `main`.**
