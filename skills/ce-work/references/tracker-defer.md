...
- **Length cap:** when the composed body would exceed a tracker's body length limit, truncate with a neutral continuation pointer to `.tmp/rocketclaw/ce-code-review/<run-id>/` and include the finding_id in both the truncated body and the metadata block so the workspace-local artifact is discoverable.
