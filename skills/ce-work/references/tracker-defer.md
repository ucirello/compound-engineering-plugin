...
Used by autonomous callers such as `lfg` that must not prompt. All blocking questions are skipped; the fallback chain is executed silently in order. Behavior:
...
2. **GitHub Issues via `gh`** — when `gh auth status` succeeds and the explicitly targeted repository has issues enabled (the repository-targeted `gh repo view --json hasIssuesEnabled` returns `true`)
...
| GitHub Issues | `gh issue create` | Pass `-R <github-repo>` or use the inline `GIT_DIR` form for a non-colocated Jujutsu workspace. Use `--label` for severity when labels exist; omit it if the repository has no label fixture. Fall back to a label-less issue on first failure. | Markdown | `--label P0` / `--label P1` / etc. when labels exist |
