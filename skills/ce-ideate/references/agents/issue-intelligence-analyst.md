...
Prefer the tracker implied by the focus hint or the repository remotes reported by `jj git remote list`. For a GitHub fork with both `upstream` and `origin`, resolve issues against **`upstream`** — issues live on the upstream repository, not the fork. In a non-colocated JJ workspace, point `gh` at the backing repository reported by `jj git root` through `GIT_DIR` for each `gh` call; do not replace `gh` with Git commands. A missing binary, unset env var, or unloaded MCP server is **not** proof the tracker is unavailable — probe what is actually reachable before concluding; note that a GitHub MCP aliased under a non-`github` prefix is reachable but will not match `mcp__github__*` until that server's prefix is added to the dispatch allowlist. The fetch mechanism differs per tracker; everything else in this prompt is tracker-agnostic.
