...
jj log -r "$BASE..@" --no-graph -T 'commit_id.short() ++ " " ++ description.first_line() ++ "\n"'
```

The first command reports the closest local bookmark target(s) in the working-copy commit's ancestry; JJ has no attached current bookmark. Treat the second command's output as `COMMITS:`.
...
In `pr-remote` / `bookmark-remote`, current source and any targeted probe must use `jj file show -r <reviewed-head-revision> <path>` against the supplied reviewed head revision, or the supplied diff hunks when no head revision is available; never inspect workspace paths.
