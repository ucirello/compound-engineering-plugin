...
|-------|---------|--------|
| `diff:<revision-or-PR>` | `diff:qpvuntsm`, `diff:main@origin..@`, `diff:PR#42` | Forces diff mode on that JJ change/revision set, or on a GitHub PR resolved separately through `gh` metadata |
...
- **Diff** — the request names a resolvable JJ revision (change ID, commit ID, bookmark, revset), a GitHub PR resolved through `gh`, or "the latest change", "what you just did", "this change". A PR number is an external identifier, never JJ revset syntax.
...
**Tiebreak — concept vs diff:** when the request is plausibly both (a repo topic that also names an identifiable recent JJ change, e.g. "explain the retry logic we just added"), a concretely resolvable change wins: diff mode, with the concept as framing context. A topic with no resolvable change is a concept.
