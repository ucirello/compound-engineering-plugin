...
- **`title`** — the plan's descriptive name with a ` - Plan` suffix
  ...
  fixed change-description prefix in the title — the `type` field carries that classification.
- **`type`** — the project-defined intent classification. Resolve it from the
  ...
  the intent the eventual JJ change description or commit message should reflect.
...
- **`execution`** — execution domain for downstream routing: `code`
  ...
  skipping the bookmark/change/test/CI lifecycle); absent or `code` routes
...
- **No process exhaust.** No "captured at Phase X" notes, no `## Next Steps`
  ...
  metadata belongs in JJ change descriptions, commit messages, and tool output, not the artifact.
