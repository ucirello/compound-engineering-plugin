...
Build under `.context/ce-prototype/<date>-<slug>/`, so the prototype is still there when the implementation that follows reads it alongside the decisions capsule. That path has to be ignored first: inspect the workspace's active ignore rules for `.context/`, anchored at the root reported by `jj workspace root`, and when it is not covered, offer to append that one line to the root ignore file, appending only if they agree and leaving the rest of the file alone. Fall back to `<workspace-root>/.tmp/rocketclaw/ce-prototype/<date>-<slug>/` when they decline, when they ask that this run not be kept under `.context`, when the current directory is not inside a Jujutsu workspace, or when the durable path fails the safety checks; if `jj workspace root` is unavailable, use the current directory as `<workspace-root>`. Calling the prototype throwaway is not that request — throwaway describes the code, and a kept prototype is still never deleted. Give each question in a multi-question run its own child directory. Never delete a kept prototype — the directory is theirs to prune.
...
Scale into the existing app only as a throwaway overlay when the user asks or the question is density or chrome on an existing page — an isolated page will hide that. That overlay is not the shipped feature. Do not leave prototype code in the product's Jujutsu change or bookmarks. Undo those edits with `jj restore` when the try ends — restore only the files you changed, never work you did not make. An overlay run therefore leaves no artifact behind; nothing survives it. If you cannot undo them cleanly, name the files you left modified rather than handing off a modified working copy.
