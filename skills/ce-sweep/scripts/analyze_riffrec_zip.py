...
def write_source_materials(
    ...
    lines = [
        ...
        "- Commit policy: raw media, audio chunks, zip contents, session dumps, and extracted screenshots are local-only by default; change generated Markdown/JSON/manifests when useful for brainstorm/planning traceability.",
    ...
    lines.append("Extracted screenshots are retained locally for agent inspection and should not be described by default.")
    lines.append("")
    if moments:
    ...
    lines.append("Raw files are intentionally local-only by default. Do not change these unless the user explicitly asks and privacy/security is acceptable.")
    lines.append("")
