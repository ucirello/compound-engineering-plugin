...
- **Merge-state claims** ("fixed in #1608", "landed", "shipped") verify against **remote truth** — the workspace may predate a merge, so `gh pr view` (or the tracker equivalent) is primary and jj reachability from `trunk()` is only the fallback.
...
| Change reachable from `@` only | Local-only change whose commit ID may change | Replace with the PR number |
| Change reachable from `trunk()` only | Workspace predates the merge | Keep, with a temporal qualifier; verify the landed claim via `gh` |
| Change exists but is unreachable | Abandoned or rewritten change | Replace with the PR number |
...
If the script cannot be resolved on this platform, apply its checks manually at the same scope — scan the body for cited paths that do not exist, revision IDs, `Learning(s) N` / `{{…}}` scaffold, and broken relative links — and note in the run output that the check was manual. Do not silently skip.
