...
Resolve `<workspace-root>` with `jj workspace root`; if that fails or returns empty, use `pwd -P`. State lives at `<workspace-root>/.tmp/pr-watch/<host>-<owner>-<repo>-<pr>/state.json`, a stable cross-invocation path. The `<host>` segment is load-bearing for GitHub Enterprise: without it, two PRs sharing `owner/repo#N` on different hosts would reuse one state file and cross-contaminate dispositions. The helper owns all reads and writes under a file lock, and its atomic temporary files are created as siblings inside this workspace-local state directory before replacement.
...
A `check_key` is `"<workflow>/<name>"` (or `"<name>"` when there is no workflow) — stable across polls for the same head, which is all the dedup needs (see below). Each `snapshot` emits `changed_this_tick`, `quiet_seconds`, `invocation_id`, `invocation_started_at`, `invocation_elapsed_seconds`, `invocation_budget_seconds`, `invocation_remaining_seconds`, `persisted_state_created_at`, `persisted_state_age_seconds`, `pr_chain`, `stack_blocker`, review-signal lifecycle fields, blocked-external review-drain facts, and derived `trajectory` facts. The blocked-external clock is head-scoped and narrower than `quiet_seconds`: external review movement or a new head resets it; check, base, stack, and disposition-only movement does not. The first snapshot starts one fixed invocation; later calls must match its token, anchor, and budget. Persisted-state age never contributes to that cap. The chain probe uses host-qualified GitHub GraphQL reads and derives freshness from remote base/head OIDs. Only a stack-field schema-unavailable response with a successful default-branch read degrades to `absent`; other failures stay `probe-error`. Ordinary open-PR base/head relationships classify manual dependencies only when no manager is confirmed.
...
- **Non-thread feedback candidates** (top-level PR comments + review-submission bodies). Surfaced as `actionable.comments` for content that has **no inline thread**. The field name supports claim→act→confirm; it does not mean the detector has semantically proven that the body requires work. The fetch excludes only empty bodies and messages known to be from the PR author; the resolver judges semantics. Unlike a thread there is no remote resolve, so `mark --comment <id> --disposition dispatched|needs-human` is the only way to silence it. A dispatched item does not reactivate on edits because status services may rewrite comments on every push; a `needs-human` item does reactivate when edited past its acted baseline. A new comment remains actionable, and edits still reset review quiet time without reopening dispatched work.
...
- **Branch currency.** `branch_currency` is the third attention stream. Its identity binds host-qualified live base repository/ref/OID, head SHA, merge status, and route. "Mergeable against the current base" and "head contains the latest base" are separate: only `BEHIND`, `DIRTY`, a branch-protection requirement, or an explicitly selected always-current policy requires maintenance. Ordinary historical base movement with GitHub reporting `CLEAN` does not. `UNKNOWN` mergeability or any non-null `base_ref_blocker` emits no item. Managed stacks and manager/relationship `probe-error` are excluded; a `normal-base` target may be independent or an eligible target-local manual dependency, and a root with open child dependents remains allowed. Never mutate those dependents.
  ...
  A current open item with carried `parked_semantic_fingerprints` must be previewed first. Compute the current fingerprint from sorted conflicted paths plus Jujutsu-materialized conflict-side identities, excluding the base OID, and mark `--currency-inspected-fingerprint <fingerprint>`. Unchanged evidence remains parked; changed evidence retires the old park and reopens attention. Before either a host update or Jujutsu repair, atomically mark the exact item with `--currency-key <currency_key> --currency-disposition claimed` plus the invocation tuple. Stale invocation fencing and `max-runtime` take precedence.
...
- **Interrupted Jujutsu repair.** On re-entry, inspect `jj status`, `jj log`, the known repair change, and remote bookmarks. Reconcile that change to the validated result or abandon only its known attempted state; never layer another repair over unknown local state.

## Managed-stack continuation

Sequential watching is available only while a fresh API probe positively reports `manager_status == "confirmed"` and posture is `stack-ready` or `stack-land` (or `target` after acceptance). It uses one active PR and watcher. On transition, stop the old watcher, require the snapshot's next ordered entry to be immediate, open, and in draft scope, fetch its remote bookmark with `jj git fetch`, track it with `jj bookmark track`, and move the clean workspace with `jj edit`. Initialize the new state directory with `--continue-invocation`, the original invocation tuple, carried dead time, and the same posture. Recheck downstack settledness only at transitions, before mutation, and at readiness. Loss of manager confirmation ends continuation.

Under `stack-land`, after settle the caller-owned land step merges the bottom-most open settled PR, then runs `jj git fetch` before continuing; recipes live in `references/stack-commands.md`. A just-landed MERGED outcome is a transition, not a Terminal exit for the run.
...
- **Managed stack:** `target_needs_rebase` true/unknown becomes `stack-blocked`; do not use the ordinary base route. Before a delegate may publish the target, record API-derived ordered entries and remote OIDs, fetch their bookmarks with `jj git fetch`, and require a clean Jujutsu workspace plus confirmed membership. After target publication, preserve open dependents bottom-up with `jj rebase` and bookmark-scoped `jj git push`, re-fetching after each attempt. Treat partial publications as observed progress and name the first rejected, conflicted, or divergent layer. Never assume multi-bookmark atomicity or claim stack readiness until manager order, ancestry, review, and CI are re-proven on every current head.
...
- **Normal-base `BEHIND`:** require `host_branch_update_capability == true`; denied/`false` or `unknown` becomes `needs-human`, and that capability is never treated as publication authority. Claim the exact item and revalidate its observed head and base OIDs. Invoke GitHub's update-branch endpoint once with `expected_head_sha`; treat HTTP 422 as a stale claim. Confirm only after fresh API ancestry evidence proves the resulting head contains the observed base and the currency gate clears.
- **Normal-base `DIRTY`:** prove bookmark-scoped publication authority, require a clean Jujutsu workspace at the observed head, and fetch exact head/base bookmarks with `jj git fetch`. Preview in a uniquely named temporary Jujutsu workspace under workspace `.tmp`, fingerprint sorted conflicted paths plus materialized conflict-side identities, then abandon the preview change and forget/remove only that workspace. Record `--currency-inspected-fingerprint` before claiming a previously parked item.

  A resolution is mechanical only when positive intent evidence leaves no reasonable alternative behavior. Otherwise park with the fingerprint, options, tradeoffs, and a lean. For a mechanical case, claim and revalidate, create a Jujutsu merge change, mark mutation observed, resolve only the previewed conflict, and validate. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Inspect active project instructions and `jj log`, preserve the repair semantics, derive syntax dynamically, and run `jj describe -r <repair-change> -m "<description-composed-from-runtime-conventions>"` without fixed syntax or attribution. Move only the head bookmark and publish it with bookmark-scoped `jj git push`. Confirm only after `jj git fetch` plus GitHub evidence proves the validated repair is remote and a fresh snapshot clears the gate.
...
- **External head change or rewritten remote history:** the head OID moved under the loop. The snapshot clears OID-scoped CI state automatically; re-snapshot and never clobber unrelated published work.
...
- **No publication access / fork PR:** a delegated publication will fail. Detect that before mutation or from the delegated result, report it, and stop.
