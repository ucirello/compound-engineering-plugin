...
Run this **first** when `db/schema.rb` or `db/structure.sql` appears in the diff. Use the review base ref from caller context (`<review-base>` — JJ revset or ref). **Never assume `main`.**

```bash
jj diff --name-only --from <review-base> -- db/migrate/
```
...
jj diff --git --from <review-base> -- db/schema.rb

# When db/structure.sql is in the diff:
jj diff --git --from <review-base> -- db/structure.sql
```
