...
When this workflow needs run artifacts, resolve `workspace_root=$(jj workspace root 2>/dev/null)` and store them under `"$workspace_root/.tmp/rocketclaw/doc-review/<run-id>/"`. If `jj workspace root` is unavailable, set `workspace_root=$PWD` and use `"$workspace_root/.tmp/rocketclaw/doc-review/<run-id>/"`. Do not use OS-global temporary storage or temporary-file generators.
