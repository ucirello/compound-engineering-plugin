...
| `verified_merge_sha` | The merge change/commit id the fix landed on. The key name is legacy. |
...
| --- | --- | --- |
| local-change mode (default) | Single writer **per workspace**. | The lease serializes overlapping sweeps in the same working copy (e.g. a cron sweep and a manual one). The file is written in-tree (and may be described in a local JJ change). No cross-machine guarantee. |
| pushed-shared-bookmark | One writer **per repo**. | The state file lives on a shared bookmark multiple workspaces push to. `lease-acquire` must be described, pushed, and confirmed (`jj git fetch` back and verify our writer won) **before any source-side write**. This makes the lease a repo-wide mutex across machines. |
...
and never described in a JJ change (the skill's path-limited commit step includes only the state file and the plan, never a broad all-files operation).
