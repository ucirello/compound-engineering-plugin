...
- **Code-behavior claims** verify against the **local JJ working copy** because they describe what this session produced and verified here.
- **Merge-state claims** verify against **remote truth**. The workspace may predate a merge, so `gh pr view` or the tracker equivalent is primary and JJ reachability is only the fallback. The script's staleness information tells you how much to distrust the local workspace for this category.

Before running the script, optionally run `jj git fetch` as a best-effort Git-interop refresh. Skip silently on failure or offline; the network is never a correctness dependency. When remote state cannot be checked at all, keep the claim, add an as-of qualifier, and record degraded verification in the run report.
...
| path missing here, exists at `trunk()` | Stale workspace | Verify the claim against trunk or remote truth; annotate if the doc implies the file is present locally |
...
| ID reachable from `@` only | Local-only commit; ID may change on rewrite or merge | Replace with the PR number |
| ID reachable from `trunk()` only | Workspace predates the merge | Keep, with a temporal qualifier; verify the landed claim via `gh` |
| ID exists but is unreachable | Rewritten-away commit | Replace with the PR number |
...
2. MERGE-STATE CLAIMS — assertions that a change landed. Primary check: query
   the pull request's state, merge time, and base through gh (remote truth).
   Fallback: JJ reachability from trunk(). Verdict: verified,
