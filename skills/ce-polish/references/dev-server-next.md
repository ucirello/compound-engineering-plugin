...
- **Monorepo roots:** in a pnpm/Turborepo workspace, `npm run dev` at the root typically fans out to multiple packages. Users should set `cwd` in `.rocketclaw/launch.json` to the specific Next app (`cwd: "apps/web"`).
- **Env loading:** `.env.local` is loaded automatically by Next; the workflow does not need to export it.
