# Dogfood Report — <bookmark-or-change>

> Diff-scoped browser QA of `<bookmark-or-change>` vs the trunk. <YYYY-MM-DD>.
...
<What changed between the target and main: new features, modified behavior, new/changed routes, views, components, data flows. 2-6 bullets.>

## Personas

<The primary personas the flows were judged against, and what each cares about. Note the source: a pack rule cited as `(pack: <id>, <path within the pack>)`, STRATEGY.md or PRODUCT.md "Users" ("Who it's for" in older strategy files), VISION.md, a persona doc, or "inferred" if none existed. Resolver warnings or errors from pack discovery, and "packs unresolved" when the resolver could not run, are noted here once.>
...
- **What the target does instead, and why:** <the intended behavior and the evidence it is intentional — plan, PR description, change history>
- **Options:** refine the rule (writable pack: through `ce-compound`; git-URL pack: upstream change and a `ref` bump) / retire it
