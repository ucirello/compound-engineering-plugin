...
6. If this check edited an instruction file after the refresh change was already described, fold the edit into that change when it is still the working-copy change; otherwise describe a follow-up change per `references/commit.md` (the instruction file now points agents at the solutions store) and `jj git push --bookmark <name>` if that bookmark was already pushed so an open PR includes it. If the user chose "don't describe", leave the edits in the working copy alongside the rest.
