...
- If this session has no writable workspace, but the user named a repository and the harness exposes a remote repository-work surface with a writable Jujutsu workspace, run Phase 2 on that surface instead and report the remote workspace-local fixes in Phase 3.
