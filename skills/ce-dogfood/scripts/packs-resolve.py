...
`--declared-only` answers the config-only question without cloning or touching
...
Git-URL sources cache under `<workspace>/.tmp/rocketclaw/ce-packs/<sha256(url\\nref)>`
...
hang. A missing `jj` binary degrades git-URL sources only (each warns and is
skipped); path sources still resolve, with the workspace located by
`jj workspace root` (cwd at that workspace; never inferred from `.jj/` or `.git/`
on disk). Environment overrides: CE_PACKS_CACHE_ROOT (cache base for tests),
...
# --- scratch root (workspace .tmp/rocketclaw/ce-packs; local .tmp if no workspace) ---
...
def cache_base() -> str | None:
    ...
    parent = workspace if workspace else os.path.abspath(os.getcwd())
    root = os.path.join(parent, ".tmp", "rocketclaw", "ce-packs")
...
def _run_jj(args: list, cwd: str):
    return subprocess.run(
        ["jj", *args], cwd=cwd, env=_jj_git_env(), timeout=GIT_TIMEOUT,
        capture_output=True, text=True,
    )


def _unique_part_dir(base: str, key: str) -> str:
    """A dest path under `base` that does not yet exist, so `jj git clone` can create it."""
    while True:
        candidate = os.path.join(base, f"{key}.part-{os.getpid()}-{os.urandom(4).hex()}")
...
def resolve_git_source(url: str, ref: str, warnings: list, label: str) -> str | None:
    ...
    if os.path.lexists(dest):
        ...
        if os.path.lexists(dest):
            ...
            return None
    tmp = _unique_part_dir(base, key)
    try:
        cloned = False
        for extra in (["-b", ref], ["-t", ref]):
            try:
                proc = _run_jj(
                    ["--quiet", "git", "clone", "--depth", "1", "--no-colocate", *extra, url, tmp],
                    cwd=base,
                )
...
def _repo_root() -> str | None:
    """The current workspace root from `jj workspace root`. None when jj is
    missing or the working directory is not inside a workspace. Never infers
    identity from `.jj/` or `.git/` on disk."""
    if shutil.which("jj") is None:
        return None
    proc = subprocess.run(
        ["jj", "workspace", "root"],
...
def _main(argv: list) -> int:
    ...
    if repo_root is None:
        warnings.append("not inside a jj workspace; no config to read")
