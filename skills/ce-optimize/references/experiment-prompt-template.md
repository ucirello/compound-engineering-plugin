...
5. Do NOT run `jj describe`, move bookmarks, or push (the orchestrator handles the winning change)
...
- For Codex dispatch: resolve `TMP_ROOT="$(jj workspace root 2>/dev/null || pwd)/.tmp"`, create it with `mkdir -p "$TMP_ROOT"`, write the filled template to `$(mktemp "$TMP_ROOT/optimize-exp-XXXXX.txt")`, and pipe that file to `codex exec --skip-git-repo-check -`.
