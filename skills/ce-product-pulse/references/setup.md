...
Write the captured config to `<repo-root>/.rocketclaw/config.local.yaml` as flat `pulse_*` keys, using the schema in `references/interview.md` under "Config file shape". Resolve the absolute workspace root with `jj workspace root`, using that root as cwd for subsequent JJ subprocesses. To write: (1) if the file or directory does not exist, create `.rocketclaw/` and write the YAML file; (2) if the file exists, merge new keys into the existing YAML, preserving any non-pulse keys (e.g., `plan_*`) untouched. If `.rocketclaw/config.local.yaml` is not already covered by the repo's `.gitignore` (which JJ honors), offer to add the entry before writing. Show the resulting pulse block to the user in chat and offer one round of edits.
