...
- The `docs/solutions/` enumeration (a new learning, even when present only in the working-copy change, must be visible — re-globbing it is ~free and the match reads files fresh anyway).
...
- `<root-id>` = the lexicographically first real root commit from `jj log -r 'roots(::@ & ~root())' --no-graph -T 'commit_id ++ "\n"'` — the repo identity.
- `<base-id>` = the commit ID from `jj log -r '@-' --no-graph -T 'commit_id ++ "\n"'` — the stable state.

Each JJ workspace keeps its own entry under `.tmp/rocketclaw/repo-profile/` at the workspace root. Lookup is JJ metadata only; on a hit, only this one file is read. Outside a JJ workspace, scratch storage falls back to `.tmp/rocketclaw/repo-profile/` under the current project root, but this revision-keyed cache returns `NO-CACHE` because no valid key exists.
...
- `MISS` then a write-path on the next line → dispatch the `repo-profiler` persona to derive the profile, write its JSON output to a file, then persist it. This `put` runs after the profiler, so it is a **separate Bash-tool call** from the `get` above — shell variables do not persist between calls, so **re-set `SKILL_DIR` in the same command**:
  ...
  ```
- `NO-CACHE` → no JJ workspace or no writable workspace-local cache. Derive the profile fresh for this run and **skip** `put` (nothing to persist). Any run-local profile file belongs under `$(jj workspace root)/.tmp/rocketclaw/repo-profile/`; use `.tmp/rocketclaw/repo-profile/` under the current project root when no JJ workspace exists.
