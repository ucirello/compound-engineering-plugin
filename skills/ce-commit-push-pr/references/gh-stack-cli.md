...
Pair every `gh` invocation that talks to git with `GIT_DIR` set from `jj git root`.
...
|---|---|---|
| 0 | Success | Parent is in a stack, and `@` has moved to it |
...
bookmark. Create a local bookmark at `headRefOid` (`jj bookmark create <name> -r <headRefOid>`), fetching so that commit is
reachable — reachability leaves the commit with no bookmark to name.
...
Processes bookmarks bottom to top and checks out the **last** one. **Existing bookmarks are adopted;
...
- **`gh stack link`** — GitHub-only by design, creates no local tracking, so a later
  ...
  bookmarks managed by external tools (jj, Sapling, git-town).
