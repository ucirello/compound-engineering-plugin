...
- No isolated-workspace setup (Phase 1 Step 2).
...
4. **Save and report.** Write the deliverable to a durable tracked location, defaulting to a sensible `docs/` subpath unless the user named another path, and report its absolute path. Whether to finish it as a described Jujutsu change is the user's call; offer, do not force.
