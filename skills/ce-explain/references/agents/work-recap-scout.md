...
1. **JJ activity** — `jj log` over the window (descriptions, change IDs, commit IDs, dates, authors), and for substantial changes use `jj log --stat` or `jj diff --stat -r <revision>`. Group obviously related changes rather than listing them flat.
2. **Merged and open PRs** — only when a PR interface is reachable (a `GIT_DIR="$(jj git root)" gh` CLI that responds, a connector/MCP tool). This portion is capability-gated: when no interface is reachable, note "PR evidence unavailable" in one line and move on — never treat the missing interface as an error and never guess PR state from bookmark names.
