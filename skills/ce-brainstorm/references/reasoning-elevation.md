...
Resolve the per-skill **model choice immediately before adapter selection**, so the decision reflects the current conversation rather than an intake snapshot. The value is a model alias (e.g. `fable`, `opus`) or a `provider/modelname#variant` string, not a boolean. Pair it with `brainstorm_harness` / `plan_harness` when the caller names a harness.
...
2. **Caller carrier** — a structured `<per-skill-key>:<model>` token that an automatic orchestrator may pass in the invocation (LFG passes `plan_model:<alias-or-provider/modelname#variant>` to ce-plan; the analogous `brainstorm_model:` to ce-brainstorm; and `plan_harness:` / `brainstorm_harness:` for the harness). Use it when live user intent does not decide the choice. Strip it from the request text and never reconstruct it from product prose. It is honored in pipeline / `disable-model-invocation` runs. A model alias must match `^[A-Za-z0-9._-]{1,64}$`; a `provider/modelname#variant` string is also valid and is preserved as-is. A harness value of `opencode2` is its own branch. A malformed carrier counts as absent, not guessed.
3. **Config** — otherwise use the per-skill keys: `plan_model` / `plan_harness` for ce-plan, `brainstorm_model` / `brainstorm_harness` for ce-brainstorm. Read them the **same way this skill's Phase 0.0 (output-mode resolution) resolves `plan_output` / `brainstorm_output`**: reuse the repo root already resolved, else run `jj workspace root` (cwd = that workspace), then apply the ordinary-key rule (`config.local.yaml` then `config.yaml`). Reuse the Phase 0.0 reads if still in hand. Ignore commented (`#`-prefixed) lines. A model alias **or** a `provider/modelname#variant` string selects the model; `brainstorm_harness: opencode2` (or `plan_harness: opencode2`) selects the opencode2 adapter, which is its own branch — never fold it into `opencode`. Missing / commented / invalid / no file selects none.
...
1. **Native in-harness dispatch.** Attempt the platform subagent primitive with a per-agent model override (e.g. `model: "fable"` on the Claude Code `Agent`/`Task` tool). Capability is proven by attempt, not self-assessment — a harness that can serve the model natively does; one that cannot fails the attempt and falls through. **Receipt rule (R6):** a "receipt" is the serving side's report of which model actually ran. A native run whose receipt names a *different* model family than requested falls through to the next adapter; a run with *no* receipt proceeds and is recorded as unverified (it does NOT fall through). When `brainstorm_harness` / `plan_harness` is `opencode2`, native dispatch counts only if this host **is** opencode2; do not treat `opencode` as that host.
2. **Claude CLI.** Run the bundled `scripts/elevation-dispatch.sh` worker as a detached job (see Off-host dispatch) with the default harness. Available when `claude` is on PATH and the resolved harness is not `opencode2`. Do not preflight authentication in the host command context: the detached worker's provider-capable call is authoritative, and an authentication failure there follows Recovery.
3. **opencode2 CLI.** When the resolved harness is `opencode2`, run the same bundled worker with `ELEVATION_HARNESS=opencode2` so it invokes `opencode2`, never `opencode`. Available when `opencode2` is on PATH. Pass `brainstorm_model` / `plan_model` through as `--model provider/modelname#variant` (surveyed `opencode2 run --help` on v2.0.8: `--model`, `--format json`, `--auto`, `--file`; no `--dir`). This branch is distinct from Claude CLI and from any `opencode` adapter.
...
Hand over the working context as **file paths the subagent reads itself**, never a re-narrated prose brief. Create **one private per-run handoff directory** under the workspace `.tmp/rocketclaw` tree (`WORKSPACE_ROOT="$(jj workspace root 2>/dev/null || printf '.')"; mkdir -p "$WORKSPACE_ROOT/.tmp/rocketclaw"; mktemp -d "$WORKSPACE_ROOT/.tmp/rocketclaw/ce-elevation-XXXXXX"`) and write the prompt-file and every evidence file into *that* directory. On the Claude CLI route the worker grants the elevated model read access to only that one directory (via `--add-dir` on the prompt-file's parent), so the handoff files stay readable while the rest of the workspace `.tmp` tree — other same-user scratch and credentials — is not exposed:
...
```
   ...
   ELEVATION_HARNESS="<claude | opencode2; use opencode2 when brainstorm_harness/plan_harness is opencode2>" \
