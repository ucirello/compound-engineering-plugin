...
`ce-code-review` is invoked here with `mode:agent`, so it is **review-only** in this context — it reports findings and writes artifacts and does not mutate the working copy, create or describe changes, publish bookmarks, or file tickets. **The caller owns apply/fix policy.** (In its own default/interactive mode the review applies safe fixes itself; that path does not apply here.)
...
- Run artifact dir: `$(jj workspace root)/.tmp/rocketclaw/ce-code-review/<run-id>/` (`review.json`, per-reviewer JSON for `why_it_matters`); if `jj workspace root` is unavailable, use the current project's local `.tmp/rocketclaw/ce-code-review/<run-id>/`
...
```
ce-code-review mode:agent plan:<plan-path> base:<diff-base-or-ref>
```
...
- `base:` — when the diff base is already resolved for the current working copy; omit when reviewing a PR number/URL or a standalone current change.
...
Default to applying every actionable finding. Applying is a reversible edit to a tracked tree; diffs are reviewed before the change is finalized (below) and tests run after — so leaving a clear, reversible fix unapplied "to be safe" is the failure mode, not the safe choice. Bias to act:
...
There is no precondition safety checklist and no deny-list — a code-review fix is a reversible edit, so downside is controlled after the fact (`jj diff` review + tests + the change checkpoint), not by gating the apply.
...
**Orchestrator owns:** parse review output → **eligibility filter on JSON fields only** → build batches → dispatch fix subagents → review `jj diff` output → tests → isolate/describe changes → Residual Work Gate.
...
3. **Parallel waves:** batches with **disjoint filesets** may run in parallel (same workspace/shared-directory rules as Phase 1 Step 4 in `ce-work` SKILL.md).
4. **Same file, many findings:** keep one subagent per file. If the prompt would exceed a comfortable size (~8 findings), split into **serial** subagent passes on that file (first batch highest severity, then next batch after integration or after the prior agent returns).
...
- Shared-directory fallback: do not run Jujutsu history operations — return which `#` were applied or skipped and which files changed

**After each wave:** the orchestrator reviews `jj status` and `jj diff` (scope = assigned `#` only), runs tests (`requires_verification: true` on any applied finding → at least targeted tests; multi-file → broader suite), isolates the findings with `jj split <fileset>` when needed, describes the coherent change, and opens the next change with `jj new` unless workspace-isolated subagents return separate changes for integration per Phase 1. Do not impose a fixed type, scope, or template. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Repository-local instructions and syntax observed in `git log` take precedence; use Go guidance only when compatible. Repeat until all batches complete.
