...
def write_source_materials(
    ...
    lines = [
        ...
        "- VCS policy: raw media, audio chunks, zip contents, session dumps, and extracted screenshots are local-only by default; include generated Markdown/JSON/manifests in JJ changes when useful for brainstorm/planning traceability.",
    ...
    if chunk_files:
        ...
        lines.append(f"  - retained locally in `{link(raw_dir / 'transcription_chunks')}`; not safe to include in JJ changes by default.")

    lines.extend(["", "## Local-Only Frames", ""])
    lines.append("Extracted screenshots are retained locally for agent inspection and should not be included in JJ changes by default.")
    lines.append("")
    if moments:
    ...
    lines.append("Raw files are intentionally local-only by default. Do not include these in JJ changes unless the user explicitly asks and privacy/security is acceptable.")
    lines.append("")
