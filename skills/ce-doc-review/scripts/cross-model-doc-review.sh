...
# Workspace-local scratch. Never OS /tmp or $TMPDIR. If JJ cannot name a
# workspace, a cwd-relative .tmp is enough.
workspace_tmp() {
  local root
...
adapter_argv() {
  case "$1" in
    ...
    opencode)
      ...
      ;;
    opencode2)
      # Distinct from opencode: invoke opencode2 only. Flags from
      # `opencode2 run --help` (v2.0.8): --format json, --file, --model
      # provider/modelname#variant. No --dir (cwd is PEER_WORKDIR) and no
      # --variant (variant is the #segment of --model).
...
parse_opencode_events() {  # <logfile> <outfile>
  ...
  tmp="$(mktemp "$(ensure_workspace_tmp)/xmodel-doc-opencode-text-XXXXXX")" || return 1
  printf '%s' "$text" > "$tmp"
...
parse_opencode2_events() {  # <logfile> <outfile>
  ...
  [ -n "$text" ] || return 1
  printf '%s' "$text" | jq -e 'select((.findings|type)=="array")' > "$2" 2>/dev/null && return 0
  tmp="$(mktemp "$(ensure_workspace_tmp)/xmodel-doc-opencode2-text-XXXXXX")" || return 1
  printf '%s' "$text" > "$tmp"
  recover_findings_json "$tmp" "$2"
  local st=$?
  rm -f "$tmp"
...
run_provider() {   # <provider>
  ...
  PEER_WORKDIR="$(mktemp -d "$(ensure_workspace_tmp)/xmodel-doc-peer-XXXXXX")" || PEER_WORKDIR="$RUN_DIR"
  ...
  rm -f "$OUT"
  if [ -s "$RAW_OUT" ]; then
    _norm="$(mktemp "$(ensure_workspace_tmp)/xmodel-doc-norm-XXXXXX")"
