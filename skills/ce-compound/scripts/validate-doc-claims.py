...
citations against the repository:
    ...
    2. Cited commit SHAs (7-64 hex chars with at least one digit and one
...
def jj(*args: str, cwd: str | None = None) -> tuple[int, str]:
    try:
...
def is_path_candidate(token: str, *, known_path: bool = False) -> bool:
    ...
    if token.startswith(("origin/", "upstream/", "refs/")):
        return False  # bookmark/remote refs, not repo paths
...
def main(argv: list[str]) -> int:
    ...
    # --- Repo context -----------------------------------------------------
    code, repo_root = jj("workspace", "root", cwd=doc_dir)
    in_jj = code == 0 and bool(repo_root)
    upstream: str | None = None
    if in_jj:
        code, _ = jj("log", "-r", "trunk()", "-n", "1", "--no-graph", cwd=repo_root)
        if code == 0:
            upstream = "trunk()"
        else:
            for candidate in ("main", "master"):
                code, _ = jj(
                    "log", "-r", candidate, "-n", "1", "--no-graph", cwd=repo_root
                )
                if code == 0:
                    upstream = candidate
                    break
        if upstream:
            code, behind = jj(
                "log", "-r", f"::{upstream} ~ ::@", "--count", cwd=repo_root
            )
    ...
    def rev_has_path(rev: str, path: str) -> bool:
        code, out = jj("file", "list", "-r", rev, "--", path, cwd=repo_root)
