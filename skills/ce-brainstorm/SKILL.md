...
- **Read** `docs_root` from `<repo-root>/.rocketclaw/config.yaml` only (`<repo-root>` = `jj workspace root`, cwd = that workspace). Do not read it from `config.local.yaml`. Unset -> `<root>` is `docs`, exactly as before.
...
- **Read** `<repo-root>/.rocketclaw/config.local.yaml`, then `config.yaml` (`<repo-root>` = `jj workspace root`, cwd = that workspace). Missing files are skipped. Gitignore does not change resolution.
