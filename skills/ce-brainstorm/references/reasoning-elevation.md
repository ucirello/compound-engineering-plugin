...
3. **Config** — otherwise use the per-skill key: `plan_model` for ce-plan, `brainstorm_model` for ce-brainstorm. Read it the **same way this skill's Phase 0.0 (output-mode resolution) resolves `plan_output` / `brainstorm_output`**: reuse the workspace root already resolved, else run `jj workspace root`, then apply the ordinary-key rule (`config.local.yaml` then `config.yaml`). Reuse the Phase 0.0 reads if still in hand. Ignore commented (`#`-prefixed) lines. A model alias selects it; missing / commented / invalid / no file selects none. Read `plan_harness` / `brainstorm_harness` the same way. `opencode2` is not `opencode`. When the harness is `opencode2`, the matching model key is `provider/modelname#variant` (variant is `#variant` on the model id, not a `--variant` flag).
...
2. **opencode2 CLI.** Only when the resolved harness is `opencode2`. `opencode2` is not `opencode` — do not use the `opencode` binary, `opencode run`, or opencode flags as a fallback. Available when `opencode2` is on PATH. Dispatch is `opencode2 run --standalone --auto --model provider/model#variant` (no `--dir`, no `--variant` flag). Run the bundled `scripts/elevation-dispatch.sh --harness opencode2` worker as a detached job (see Off-host dispatch).
...
Hand over the working context as **file paths the subagent reads itself**, never a re-narrated prose brief. Create **one private per-run handoff directory** under the workspace `.tmp` (`WS_ROOT="$(jj --no-pager workspace root 2>/dev/null || echo .)"; mkdir -p "$WS_ROOT/.tmp" && mktemp -d "$WS_ROOT/.tmp/ce-elevation-XXXXXX"`) and write the prompt-file and every evidence file into *that* directory. On the Claude CLI route the worker grants the elevated model read access to only that one directory (via `--add-dir` on the prompt-file's parent), so the handoff files stay readable while the rest of the scratch root — other same-user scratch and credentials — is not exposed:
...
```
   ...
   CE_PEER_HARD_SECS=5400 CE_ELEVATION_HARD_SECS=5400 CE_PEER_LOG_MAX_BYTES=52428800 \
     ...
     -- bash "$SKILL_DIR/scripts/elevation-dispatch.sh" <optional: --harness opencode2> "<model>" "<prompt-file>" "<result-path>"
   ```

When `brainstorm_harness` (or `plan_harness`) is `opencode2`, pass `--harness opencode2` and a `provider/modelname#variant` model. That route is its own branch — never rewrite it to `opencode`.
