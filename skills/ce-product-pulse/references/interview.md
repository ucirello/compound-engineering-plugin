...
Loaded by `SKILL.md` at the start of Phase 1. Captures the configuration that will be merged into `.rocketclaw/config.local.yaml` (the unified RocketClaw local config, machine-local) as `pulse_*` keys and re-read on every subsequent run.
...
If the file does not yet exist, create the directory and file. Do not edit `.gitignore`; the orchestrator owns ignore configuration.
