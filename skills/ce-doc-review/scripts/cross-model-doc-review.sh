...
adapter_argv() {
  case "$1" in
    ...
    opencode)
      ...
      ;;
    opencode2)
      # Distinct from `opencode`: V2 `run` has no --dir/--variant. Isolation is
      # cwd=$PEER_WORKDIR (run_timeout_cmd) plus --standalone. Model is
      # --model provider/model#variant; no V1 env overlay or opencode fallback.
...
XMODEL_TMP="$(jj workspace root 2>/dev/null || printf '%s' ".")/.tmp"
mkdir -p "$XMODEL_TMP" || skip "cannot create workspace .tmp at $XMODEL_TMP; skipping"
PROMPT_FILE="$(mktemp "$XMODEL_TMP/xmodel-doc-prompt-XXXXXX")"
PEERLOG="$(mktemp "$XMODEL_TMP/xmodel-doc-log-XXXXXX")"
...
PEERERR="$(mktemp "$XMODEL_TMP/xmodel-doc-err-XXXXXX")"
PEER_WORKDIR=""
RAW_OUT=""
RUN_SUCCEEDED=false
PROVIDER_OUTCOME="ok"
...
parse_opencode_events() {  # <logfile> <outfile>
  ...
  tmp="$(mktemp "${XMODEL_TMP:-.}/ce-opencode-text-XXXXXX")" || return 1
  printf '%s' "$text" > "$tmp"
...
run_provider() {   # <provider>
  ...
  PEER_WORKDIR="$(mktemp -d "${XMODEL_TMP:-.}/xmodel-doc-peer-XXXXXX")" || PEER_WORKDIR="$RUN_DIR"
  ...
  rm -f "$OUT"
  if [ -s "$RAW_OUT" ]; then
    _norm="$(mktemp "${XMODEL_TMP:-.}/xmodel-doc-norm-XXXXXX")"
