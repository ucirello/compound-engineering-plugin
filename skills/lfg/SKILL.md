...
3. Invoke the `ce-simplify-code` skill on the bookmark diff.
   ...
   Do not describe a new change in this step. `ce-simplify-code` leaves its changes in the working tree; step 4's review scopes the working tree (undescribed changes included), and step 8's `ce-commit-push-pr` describes whatever remains. Creating a change description here would sweep any still-undescribed `ce-work` edits into a misleading `refactor` change and could stall on a tree that never goes clean.
...
**Shipping precondition (steps 5–9).** Run `jj git remote list` once before the shipping steps. If it lists **no remote** (e.g. a sandbox/throwaway checkout with no `origin`), shipping is **local-only**: create every change description the steps below call for, but **skip every push, PR create/edit, and CI-watch action** — the pushes in steps 5 and 6, the push and PR creation in step 8, and step 9 in full. A missing remote is a terminal local-only state, not an error: never retry a push or hunt for a remote — make the local changes and proceed to step 10. Run steps 5–9 normally when a remote exists.

5. **Apply and persist review fixes** (REQUIRED after step 4, before residual handoff)

   Load `references/review-followup.md` and execute its apply step (mechanical apply + path-limited commit/push when changes exist). Do not proceed to the residual handoff, run browser tests, or output DONE while eligible review fixes remain only in the working tree undescribed.
...
6. **Autonomous residual handoff** (only when step 4 reported one or more actionable `downstream-resolver` findings not applied in step 5; skip when it reported `Actionable findings: none.`)
   ...
   6. If no open PR exists, create a tracked fallback file at `docs/residual-review-findings/<bookmark-or-head-change>.md` containing the composed section and the source PR-review run context. Commit only that file with `jj commit docs/residual-review-findings/<bookmark-or-head-change>.md -m "docs(review): record residual review findings"`, and push the current bookmark with `jj git push` **when a remote is configured** (per the shipping precondition). If there is no remote at all, do not push — the local change is the durable sink. This is the durable no-PR sink. Do not output DONE until the residual findings are durable: either the existing PR body has been updated, or this fallback file change has been made (pushed when a remote exists, local when none). A push that fails when a remote exists is a stop-and-report; never retry a push, or block DONE, when no remote exists.
