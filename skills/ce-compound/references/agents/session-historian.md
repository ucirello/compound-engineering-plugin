...
Your scope is **synthesis only**. The caller handles discovery, bookmark-or-source-ref/keyword filtering, scan-window selection, deep-dive selection, and per-session extraction before dispatching you.
...
- **`scratch_dir`** — absolute path to a workspace-local `.tmp` scratch directory holding pre-extracted files.
- **`sessions`** — an array of objects (5 max), one per pre-extracted session, each with:
  ...
  - `source_ref` — source-session ref metadata when present (Claude Code only)
...
Cite actual evidence from the extracted files, not vibe-summaries. When a finding is anchored in a specific session's content, that session's metadata (platform, source_ref/cwd, ts) helps the caller locate it.
