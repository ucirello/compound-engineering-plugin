...
Severity, confidence, and cross-reviewer agreement tell you what to do first and what to flag loudly — they do not gate the decision. There is no deny-list: downside is controlled after the fact (restore + visible diff + the change checkpoint), not by a precondition.
...
**Scope invariant.** Apply only when the working-copy commit *is* what was reviewed — `local-aligned` or standalone. In `pr-remote` / `bookmark-remote` the workspace is not the reviewed revision; do not apply — report instead.
...
**Review the autofix diff before finishing.** Before describing, advancing, or reporting applied fixes, diff only the changes introduced during Stage 5c against the pre-apply checkpoint. Run one self-review pass over that diff:
...
If this self-review changes files, rerun the affected tests or lint for those follow-up edits before describing, advancing, or reporting; the earlier validation only covers the original autofix diff.

**Describe and advance when the pre-review change was empty.** Before applying, snapshot with `jj status` and record whether `jj diff --summary -r @` was empty. The permanence gate is the **push**, not a local change description; Jujutsu operations are recoverable through the operation log.

Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Compose the change description from the project's active instructions and conventions already in your context plus current descriptions visible in `jj log`. Active instructions take precedence over repository history, and the sentence's `git log` wording is non-operational. Use the Go guidance only for message quality, clarity, and structure, never as a source of repository format. Do not impose or supply any fixed prefix, type, scope, subject, body, layout, template, or example.

- **Empty `@` before the review:** after applying and verifying, describe the fixes as one isolated change with `jj commit`, using the project's local convention, which leaves a new empty working-copy change on top.
- **Non-empty `@` before the review:** apply but do not create a new change or alter its description. The fixes remain in the user's in-flight working-copy change, and the Applied section lists what changed.
...
2. **Applied (explicit local apply only).** When Stage 5c applied fixes, list them first — before the findings — in an Applied section (see review output template); each entry carries `#`, file, the fix, and reviewer (a multi-file fix is one row with one `#`), then a one-line validation outcome and change status (described and advanced from an initially empty `@`, or retained in the user's existing working-copy change). Flag green-but-unverifiable edits (auth/contract/concurrency) prominently. Omit this section when local apply was not authorized or nothing was applied. Applied findings appear here, not in the severity tables.
...
```json
{
  ...
  "scope": {
    ...
    "bookmarks": ["<local bookmark names on the reviewed stack>"],
    "change_id": "<jj change ID for the reviewed revision>",
    "commit_id": "<jj commit ID for the reviewed revision>",
    "pr_url": "<url or null>",
    "files_changed": 0
  },
...
After Stage 6, stop. When local apply was explicitly authorized, Stage 5c may already have applied and, from an initially empty `@`, described and advanced the verified fixes. Otherwise the caller or user decides what to apply from the report and artifacts.
...
```json
{
  ...
  "commit_id": "<jj log -r @ commit ID at dispatch time>",
  "verdict": "<Ready to merge | Ready with fixes | Not ready>",
