...
| 3. Environment guard | Refuse dangerous operations in contexts where they make no sense | The operation can be catastrophic if run in the wrong environment | In tests (`NODE_ENV === 'test'`), refuse `jj git init` outside `<jj-workspace-root>/.tmp`, with local `.tmp` only when no workspace root exists |
