...
Build under workspace-root `.tmp/prototype/<date>-<slug>/` (local `.tmp` outside JJ). Keep the prototype with its decisions capsule for later implementation; scratch survival is best-effort.

`references/preview.md` owns run-directory resolution. Do not create a second run directory; that splits the screens from the capsule.
