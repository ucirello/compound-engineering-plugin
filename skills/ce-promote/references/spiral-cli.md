...
- **File absent:** create it (and its `.rocketclaw/` directory) with the key, AND make sure the machine-local config won't be committed. Check repo-root ignore files (`.gitignore`; do not read `.git/` or `.jj/` exclude files) for whether the root-relative path `<root>/.rocketclaw/config.local.yaml` would be ignored. There is no public `jj` equivalent of `git check-ignore`; do not call `git`. If it isn't already ignored, offer to append `.rocketclaw/*.local.yaml` to `.gitignore` before writing — never resolve a git-path exclude file. `ce-setup` is the canonical place that adds the shared `.gitignore` entry for teammates. Without any ignore, a user who runs `/ce-promote` before `/ce-setup` could accidentally commit machine-local opt-out state.
