...
2. **Working-copy revision.** If `jj diff -r @` is non-empty, review those changes. JJ snapshots the working copy automatically; there is no staging area.
3. **Unpushed revisions vs base bookmark.** If `@` is empty, review `jj diff --from 'heads(::@ & ::<base>)' --to @`, where `<base>` is the default bookmark (usually main or master).
