...
JJ change described: `<message derived from repository-local standards and history>` (working copy was clean before review).
...
- **Applied section (default mode only)** -- when the review applied fixes (Stage 5c), list them first, before the severity tables, as `# | File | Fix | Reviewer` followed by a one-line validation outcome (e.g. "suite 214 pass, lint clean") and the **JJ change-description status** — described as an isolated change when the working copy was clean before the review, or left in the existing change when it was already dirty. A fix spanning multiple files is **one row with one `#`** (e.g. `controller.rb:88 (+test)`) -- never duplicate the number across rows. Flag green-but-unverifiable edits (auth/contract/concurrency) inline in the `Fix` cell, e.g. `(security-posture — verify in diff)`. Applied findings keep their stable `#` and appear only here, not in the severity tables. Omit in `mode:agent` and when nothing was applied
...
When `mode:agent` is active, **do not** emit the markdown table report above. Emit **one parseable JSON object** as the primary response and write the same payload to `review.json` under `<workspace-root>/.tmp/rocketclaw/ce-code-review/<run-id>/`, falling back to `$PWD/.tmp/rocketclaw/ce-code-review/<run-id>/` outside a JJ repo. Create the directory first.

Whenever this template renders, recommends, or validates a JJ change description, apply this instruction:
Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards.
Repository-local instructions and `git log` syntax always win. Apply compatible Go quality guidance, but derive all prefixes, types, scopes, subjects, and templates dynamically; preserve the semantic requirement to explain the applied fixes and verification.
