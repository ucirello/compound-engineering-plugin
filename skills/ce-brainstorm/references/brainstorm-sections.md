...
- **`title`** — the artifact's descriptive name with a ` - Plan` suffix
  ...
  stays stable when `ce-plan` enriches it. Do not put a repository-local
  change-description prefix in the title — the `type` field carries the
  dynamically inferred classification.
- **`type`** — `<repository-appropriate-type>`, inferred from the project's
  active local standards rather than a fixed list.
