...
The files under `.context/optimize/<spec-name>/` are local scratch state. They are ignored by JJ, so they survive local resumes on the same machine but are not preserved by changes, bookmarks, or pushes unless the user exports them separately.
...
**If the bookmark exists**, check for an existing experiment log at `.context/optimize/<spec-name>/experiment-log.yaml`.
...
- **Fresh start**: create `optimize-archive/<spec-name>/archived-<timestamp>` at the old optimization bookmark, forget the old local optimization bookmark, clear the experiment log, and start from the current revision

### 0.5 Create Optimization Bookmark and Scratch Space

```bash
jj bookmark create "optimize/<spec-name>" -r @  # fresh start
# For resume, use: jj edit "optimize/<spec-name>"
```
...
Verify the working-copy change has no edits to files within `scope.mutable` or `scope.immutable`:
...
- Ask the user to finish or move those edits to another JJ change before proceeding
- Do NOT continue until the working-copy change is clean for in-scope files
...
**Worktree backend (implemented with JJ workspaces; retain the `worktree` config value for spec compatibility):**
1. Create an experiment workspace:
   ```bash
   SKILL_DIR="<absolute path of the directory containing this SKILL.md>"
   WORKSPACE_PATH=$(bash "$SKILL_DIR/scripts/experiment-worktree.sh" create "<spec_name>" <exp_index> "optimize/<spec_name>" <shared_files...>)  # creates bookmark optimize-exp/<spec_name>/exp-<NNN>
   ```
...
1. Check environment guard -- do NOT delegate if already inside a Codex sandbox:
   ...
   test -n "${CODEX_SANDBOX:-}" || test -n "${CODEX_SESSION_ID:-}" || test ! -w "$(jj workspace root 2>/dev/null || pwd)"
   ```
2. Fill the experiment prompt template
3. Write the filled prompt under `<workspace-root>/.tmp/rocketclaw/optimize/`; if `jj workspace root` is unavailable or that root is not writable, use local `.tmp/rocketclaw/optimize/` instead. Create the selected directory first and use a collision-resistant filename under it.
4. Dispatch via Codex:
   ```bash
   ROOT="$(jj workspace root 2>/dev/null || true)"; if [[ -n "$ROOT" && -w "$ROOT" ]]; then TMP_ROOT="$ROOT/.tmp/rocketclaw/optimize"; else TMP_ROOT=".tmp/rocketclaw/optimize"; fi; mkdir -p "$TMP_ROOT"; cat "$TMP_ROOT/optimize-exp-<unique>.txt" | codex exec --skip-git-repo-check - 2>&1
   ```
...
7. **IMMEDIATELY append to experiment log on disk (CP-3)** — do not defer this to batch evaluation. Write the experiment entry (iteration, hypothesis, outcome, metrics, learnings) to `.context/optimize/<spec-name>/experiment-log.yaml` right now. Use the transitional outcome `measured` once the experiment has valid metrics but has not yet been compared to the current best. Update the outcome to `kept`, `reverted`, or another terminal state in the evaluation step, but the raw metrics are on disk and safe from context compaction.
...
3. **If best improves on current best: KEEP**
   - Finalize the experiment's JJ change description before advancing the optimization bookmark
   - Include only mutable-scope changes in that change; if no eligible diff remains, treat the experiment as non-improving and abandon it
   ...
   - Apply the resulting description with `jj describe` to the explicit winning revision in its experiment workspace, move `optimize/<spec-name>` to that same revision, then clean up the winner's experiment workspace and bookmark; the revision at the optimization bookmark is the durable artifact
...
4. **Check file-disjoint runners-up** (up to `max_runner_up_merges_per_batch`):
   ...
   - If disjoint: run `jj duplicate` for the runner-up onto `optimize/<spec-name>` and capture the newly created change ID reported by that command as the duplicated revision. Repoint the runner-up's temporary experiment workspace to that exact revision with `jj edit`, then run the full measurement in that workspace; never measure the original runner-up workspace tree or an implicit `@`.
   - Before retaining a duplicated runner-up, derive and apply its JJ description from the same runtime sources. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. The project's active instructions and conventions already in context take precedence, followed by recent history inspected with the repository's preferred `git log` syntax. Apply Go guidance only where compatible, using its quality principles for clear imperative phrasing, concision, rationale when useful, and readable wrapping. Derive all wording and structure at runtime; do not impose a fixed message, prefix, type, scope, subject, body, template, or example.
   - If combined measurement is strictly better: move the optimization bookmark to the captured duplicated revision, not the workspace's implicit `@` or the original runner-up revision (outcome: `runner_up_kept`), then clean up that runner-up's experiment workspace and bookmark
   - Otherwise: abandon the captured duplicated revision explicitly, log as "promising alone but neutral/harmful in combination" (outcome: `runner_up_reverted`), then clean up the runner-up's experiment workspace and bookmark
...
6. **Discard all others**: clean up experiment workspaces and bookmarks, log as `reverted`
...
- Discard the experiment change by cleaning up its workspace and bookmark
...
1. **Run `/ce-code-review`** on the cumulative diff (baseline to final). Load the `ce-code-review` skill on the optimization bookmark (interactive or `mode:agent`). To land eligible fixes before the next option, apply the mechanical-apply bar below.
   ...
   **Mechanical-apply bar:** apply any finding with a concrete `suggested_fix` that is a clear, reversible improvement; push back (keep, don't apply) when the reviewer is wrong, noting why. Defer anything whose right fix needs a design or product decision (architecture direction, contract shape, behavior change needing sign-off) and any finding with no concrete fix to act on — surface what was deferred. Confirm evidence still matches at `file:line` before editing. After applying, run tests (at least targeted tests for what changed; broader suite for multi-file edits). Do not describe, push, or advance the bookmark from this step — leave the diff in the optimization change for the Create PR option.
2. **Run `/ce-compound`** to document the winning strategy as an institutional learning.
3. **Create PR** from the optimization bookmark to the default bookmark, retaining `gh`/GitHub interoperability and using `jj git push --bookmark "optimize/<spec-name>"` before `gh pr create` when the bookmark is not already published.
