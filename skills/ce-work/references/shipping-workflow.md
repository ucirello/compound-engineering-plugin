...
3. **Code Review**
   ...
   **3b. Apply fixes (caller-owned).** Load `references/review-findings-followup.md`: filter on JSON, batch by file, dispatch fix subagents. The orchestrator integrates JJ changes, tests, and commits under the message rules in that reference. Then proceed to the Residual Work Gate.
...
4. **Residual Work Gate** (REQUIRED when `ce-code-review` ran and left actionable residuals)
   ...
   **Interactive sessions:** Ask the user using the platform's blocking question tool (`AskUserQuestion` in Claude Code with `ToolSearch select:AskUserQuestion` pre-loaded if needed, `request_user_input` in Codex, `ask_question` in Antigravity CLI (`agy`), `ask_user` in Pi (requires the `pi-ask-user` extension)). Fall back to numbered options in chat only when the harness genuinely lacks a blocking tool. Never silently skip the gate.
   ...
   - `Accept and proceed` — record the residual findings verbatim in a durable "Known Residuals" sink before shipping. If a PR will be created or updated in Phase 4, include them in the PR description's "Known Residuals" section (the agent owns this when calling `ce-commit-push-pr`). Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. For that change-description edit and any commit, runtime project instructions take precedence, followed by syntax and style observed in `jj log`; apply only compatible Go commit-message quality guidance. Do not impose fixed syntax, examples, or templates, and use neutral placeholders where an interface requires fields. If the user later chooses the no-PR `ce-commit` path, create `docs/residual-review-findings/<bookmark-or-change-id>.md`, include the accepted findings and source review-run context, include it in the implementation's JJ change/commit, and mention the file path in the final summary. The user has acknowledged the risk, but the findings must not live only in the transient session.
...
6. **Prepare Operational Validation Plan** (REQUIRED)
   - Add a `## Post-Deploy Monitoring & Validation` section to the PR description for every change. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. For this change-description composition, runtime project instructions take precedence, followed by syntax and style observed in `jj log`; apply only compatible Go commit-message quality guidance. Do not impose fixed syntax, examples, or templates, and use neutral placeholders where an interface requires fields.
   - Include concrete:
...
1. **Prepare Validation Context**
   ...
   Note whether the completed work has observable behavior (UI rendering, CLI output, API/library behavior with a runnable example, generated artifacts, or workflow output), and summarize any manual validation performed. If the user supplied evidence (URL, markdown embed, local artifact path), pass it to `ce-commit-push-pr` as PR-description context. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. For this change-description context, runtime project instructions take precedence, followed by syntax and style observed in `jj log`; apply only compatible Go commit-message quality guidance. Do not impose fixed syntax, examples, or templates, and use neutral placeholders where an interface requires fields.

2. **Commit and Create Pull Request**

   Load the `ce-commit-push-pr` skill to handle committing, pushing, and PR creation. The skill handles convention detection, bookmark safety, logical commit splitting, and adaptive PR descriptions through JJ with `gh` interoperability. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Runtime project instructions take precedence, followed by syntax and style observed in `jj log`; apply only compatible Go commit-message quality guidance. Do not impose fixed syntax, examples, or templates, and use neutral placeholders where an interface requires fields.

   When providing context for the PR description, include the items below. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. For both commit messages and change descriptions, runtime project instructions take precedence, followed by syntax and style observed in `jj log`; apply only compatible Go commit-message quality guidance. Do not impose fixed syntax, examples, or templates, and use neutral placeholders where an interface requires fields.
   ...
   If the user prefers to commit without creating a PR, load the `ce-commit` skill instead. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Runtime project instructions take precedence, followed by syntax and style observed in `jj log`; apply only compatible Go commit-message quality guidance. Do not impose fixed syntax, examples, or templates, and use neutral placeholders where an interface requires fields.
...
- [ ] Validation/evidence context passed to `ce-commit-push-pr` when the change has observable behavior. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. For validation of this change-description input, runtime project instructions take precedence, followed by syntax and style observed in `jj log`; only compatible Go commit-message quality guidance applies. No fixed syntax, examples, or templates were imposed, and any required placeholders are neutral.
- [ ] Commit messages were composed and validated under this rule: Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Runtime project instructions take precedence, followed by syntax and style observed in `jj log`; only compatible Go commit-message quality guidance applies. No fixed syntax, examples, or templates were imposed, and any required placeholders are neutral.
- [ ] PR description includes Post-Deploy Monitoring & Validation section (or explicit no-impact rationale), validated under this rule: Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Runtime project instructions take precedence, followed by syntax and style observed in `jj log`; only compatible Go commit-message quality guidance applies to the change description. No fixed syntax, examples, or templates were imposed, and any required placeholders are neutral.
...
- [ ] PR description includes summary, testing notes, and evidence when captured. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. For validation of this change description, runtime project instructions take precedence, followed by syntax and style observed in `jj log`; only compatible Go commit-message quality guidance applies. No fixed syntax, examples, or templates were imposed, and any required placeholders are neutral.
