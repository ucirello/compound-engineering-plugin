...
- **Rotation check (before any write).** If `<root>/plans/feedback-sweep-plan.md` exists and its frontmatter is NOT `artifact_contract: unified-plan/v1`, `product_contract_source: ce-sweep`, `artifact_readiness: requirements-only`, and `actor: ai:assistant`, it belongs to something else: move it untouched to `<root>/plans/feedback-sweep-plan-YYYY-MM-DD.md` and write a fresh plan from this template. Never overwrite an unrelated plan in place.
