...
- **Read** `docs_root` from `<repo-root>/.rocketclaw/config.yaml` only (`<repo-root>` = `jj workspace root` with cwd = that workspace). Do not read it from `config.local.yaml`. Unset -> `<root>` is `docs`, exactly as before.
...
5. **Pipeline mode and model elevation.** Pipeline mode skips interactive questions but does **not** disable model elevation — `plan_model` / `plan_harness` config (and a `plan_model:<alias>` token passed by the caller) is still honored (see the model-elevation sub-step below and `references/reasoning-elevation.md`). `plan_harness: opencode2` is its own harness, not an alias of `opencode`; when it is set, `plan_model` is `provider/modelname#variant` and that form is preserved. It does not change the output format.

**Token-parsing convention:** only literal-prefix flag tokens (`output:`, `mode:`, the exact `confirm:auto`/`confirm:ask` forms, `plan_model:<alias>`, `plan_harness:<harness>`, `delegate:` where applicable) are consumed and stripped. Other `<word>:<word>` tokens — including unrecognized `confirm:<value>` — pass through verbatim into the feature description. A stripped `plan_model:<alias>` or `plan_harness:<harness>` token is kept for the Phase 5.2 model-elevation step.
...
- **Read** `<repo-root>/.rocketclaw/config.local.yaml`, then `config.yaml` (`<repo-root>` = `jj workspace root` with cwd = that workspace). Missing files are skipped. Ignore files do not change resolution.
...
**Model-elevation visibility.** Treat a stripped `plan_model:<alias>` / `plan_harness:<harness>` token, or a `plan_model` / `plan_harness` config value you have found, as a pending Phase 5.2 input, not a resolved choice. Phase 5.2 resolves the choice from the current conversation, that token, and config immediately before authoring, so later user intent cannot be lost.
