...
Record `headRefName` as `PR_BOOKMARK` and `headRefOid` as `PR_HEAD_OID`. Resolve `PUSH_REMOTE` exactly as Full Mode step 1 does: require exactly one configured remote whose normalized URL matches the PR head repository. Do not assume `origin`.
