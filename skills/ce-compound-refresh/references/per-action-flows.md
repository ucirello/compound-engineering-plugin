...
2. Move the file with the available file-move capability, then inspect `jj diff --summary` to verify JJ records the intended removal and addition. Do not invoke another VCS for a local workspace operation.
...
4. **Run the mechanical claims check on the successor doc.** The bundled `scripts/validate-doc-claims.py` flags cited workspace paths missing from the tree, revision identifiers that do not resolve or are unreachable, relative doc links that do not resolve, and dangling drafting scaffold ("Learning 3", unresolved `{{...}}` tokens):
...
Before unlinking the file, run a final inbound-link check across the workspace's markdown content to catch any references missed during Phase 1 investigation. Prefer the harness's native content-search capability for efficiency; use ranged or context-line reads around matches rather than loading whole files.
