...
`ce-code-review` is invoked here with `mode:agent`, so it is **review-only** in this context — it reports findings and writes artifacts and does not mutate the JJ working-copy change, describe changes, move bookmarks, run `jj git push`, or file tickets. **The caller owns apply/fix policy.** (In its own default/interactive mode the review applies safe fixes itself; that path does not apply here.)
...
- Run artifact dir: `$(jj workspace root)/.tmp/code-review/<run-id>/` (`review.json`, per-reviewer JSON for `why_it_matters`); if `jj workspace root` is unavailable, use `.tmp/code-review/<run-id>/` under the current project directory
...
- `base:` — when the comparison revision is already resolved from a JJ revset; omit when reviewing a PR number/URL or the standalone current change.
...
- Stable finding `#` — reuse in JJ change descriptions, residual sinks, and subagent prompts. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. At that site, inspect the project's active instructions and run `git log`; the project's active instructions take precedence over `git log`, and both take precedence over compatible Go guidance. `jj log` may provide supplemental context but never replaces `git log`. Derive the message syntax dynamically; do not impose a fixed syntax, prefix, type, scope, subject, body structure, template, or example.

## What to apply

Default to applying every actionable finding. Applying is a reversible edit to a JJ working-copy change; `jj diff --git` is reviewed before the change is described and tests run after, so leaving a clear, reversible fix unapplied "to be safe" is the failure mode, not the safe choice. Bias to act:
...
- **Flag, don't block, green-but-unverifiable edits** — when an applied fix touches auth/authz, a public or cross-service contract/schema, or concurrency, a passing test does not prove safety; apply it when there is a clear `suggested_fix` and confidence, and call it out prominently in the `jj diff --git` review.

There is no precondition safety checklist and no deny-list — a code-review fix is a reversible edit, so downside is controlled after the fact (`jj diff --git` review + tests + the JJ change checkpoint), not by gating the apply.
...
**Orchestrator owns:** parse review output → **eligibility filter on JSON fields only** → build batches → dispatch fix subagents → review `jj diff --git` → tests → split/describe JJ changes → Residual Work Gate. At that composition site, apply this guidance: Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. At that site, inspect the project's active instructions and run `git log`; the project's active instructions take precedence over `git log`, and both take precedence over compatible Go guidance. `jj log` may provide supplemental context but never replaces `git log`. Derive the message syntax dynamically; do not impose a fixed syntax, prefix, type, scope, subject, body structure, template, or example.
...
3. **Parallel waves:** batches with **disjoint filesets** may run in parallel (same isolated-JJ-workspace/shared-directory rules as Phase 1 Step 4 in `ce-work` SKILL.md).
...
- Shared-directory fallback: do not mutate JJ history — return which `#` were applied or skipped and which files changed

**After each wave:** the orchestrator reviews `jj diff --git` (scope = assigned `#` only), runs tests (`requires_verification: true` on any applied finding -> at least targeted tests; multi-file -> broader suite), isolates the wave with a fileset when needed, and describes the JJ change unless workspace-isolated subagents are integrated per Phase 1. At that composition site, apply this guidance: Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. At that site, inspect the project's active instructions and run `git log`; the project's active instructions take precedence over `git log`, and both take precedence over compatible Go guidance. `jj log` may provide supplemental context but never replaces `git log`. Derive the message syntax dynamically; do not impose a fixed syntax, prefix, type, scope, subject, body structure, template, or example. Repeat until all batches complete.
...
Any actionable finding not applied in this pass is **residual work** — proceed to the Residual Work Gate with an updated count. Do not re-invoke `ce-code-review` solely to re-apply the same findings unless `jj diff --git` changed materially after fixes.
