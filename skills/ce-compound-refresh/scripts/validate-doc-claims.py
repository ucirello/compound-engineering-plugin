#!/usr/bin/env python3
"""Validate cited claims in a solution doc against JJ history.
...
citations against the repository:
    ...
    1. Cited repo-relative paths (backticked, containing at least one '/')
       ...
       at the parent revision or `trunk()` still count as real paths and
       are classified (deleted in the working copy vs stale workspace). Tokens
...
whatever revisions exist locally. Run a best-effort `jj git fetch --all-remotes` first
...
def main(argv: list[str]) -> int:
    ...
    if in_jj:
        if revset_has_match("trunk()", repo_root):
            upstream = "trunk()"
        if upstream:
            code, behind_output = jj(
                ["log", "-r", "@..trunk()", "--no-graph", "-T", 'commit_id ++ "\\n"'],
                repo_root,
            )
            behind = len(behind_output.splitlines()) if code == 0 else 0
            if behind > 0:
                infos.append(
