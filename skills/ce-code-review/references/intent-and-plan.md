...
**Bookmark mode:** Run `jj log -r '<BASE>..<branch-ref>'` from the absolute workspace root, using the resolved merge-base and head ref from Stage 1. Use `<branch-ref>` (the resolved `<branch>@origin` or fetched commit ID), not the raw `<branch>` argument — a remote-only bookmark has no matching local ref, so the raw name could fail or read a stale same-named local bookmark.

**Standalone (current branch):** Run:

```
echo "BOOKMARKS:" && jj log -r '@ | @-' --no-graph -T 'bookmarks ++ "\n"' && echo "COMMITS:" && jj log -r "${BASE}..@"
```
