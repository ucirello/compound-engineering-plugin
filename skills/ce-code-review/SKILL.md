...
argument-hint: "[mode:agent] [blank to review current workspace, or provide PR link]"
---
...
| `base:<revision>` | `base:abc1234` or `base:main@upstream` | Diff base in the **current workspace** (explicit; skips auto base detection) |
...
- **Apply locally; never push.** Never push, open PRs, or file tickets in any mode — push is the outward step the user owns. In **default (interactive)** mode the review applies safe, verified fixes and commits them when the pre-review working-copy change was empty (Stage 5c owns the full rule). In **`mode:agent`** it never mutates the working copy — it reports and the caller applies.
...
- **Explicit mutations only.** Never switch or replace the workspace while resolving review scope. Passing a PR number, URL, or bookmark name selects **review scope**, not permission to mutate the working copy. To review local work on another bookmark, move to that change yourself (or stay on it) and pass `base:` or no target.
- **Smart defaults.** Ignored files: exclude them and list any explicitly identified paths in Coverage. Plan: use `plan:` when passed; otherwise discover conservatively from PR body or bookmark/change-description keywords. Weak advisory P2/P3 from testing/maintainability alone: demote to `testing_gaps` / `residual_risks` per Stage 5.
...
**Always-on (full review):** local prompt assets `correctness-reviewer`, `testing-reviewer`, `maintainability-reviewer`, `project-standards-reviewer`, plus `agent-native-reviewer` and `learnings-researcher`. (Stage 3c may reduce this set to a lite roster for trivial, low-risk diffs.)
...
The following paths are RocketClaw pipeline artifacts and must never be flagged for deletion, removal, or ignore rules by any reviewer:
...
```
BASE_ARG="{base_arg}"
if [[ "$BASE_ARG" =~ ^[0-9a-fA-F]{7,64}$ ]]; then
  BASE="commit_id($BASE_ARG)"
else
  BASE="$BASE_ARG"
fi
```

The hexadecimal check distinguishes raw abbreviated/full Git SHAs from normal JJ revisions. Validate `BASE` with `exactly($BASE, 1)` before diffing; an invalid or ambiguous raw SHA is an error, not a reason to reinterpret it as other revset syntax.

Then produce the same output as the other paths:

```
echo "BASE:$BASE" && echo "FILES:" && jj diff --from "$BASE" --to @ --name-only && echo "DIFF:" && jj diff --from "$BASE" --to @ --git --context 10 && echo "UNTRACKED:"
```

This path works with any JJ revision — a change ID, commit ID, local bookmark, or remote bookmark such as `main@upstream`. Callers reviewing the current workspace should pass explicit `base:` when auto-detection is unnecessary. **Do not combine `base:` with a PR number or bookmark target.** If both are present, stop with an error: "Cannot use `base:` with a PR number or bookmark target — `base:` implies the current workspace already contains the reviewed change. Pass `base:` alone, or pass the target alone and let scope detection resolve the base."

**If a PR number or GitHub URL is provided as an argument:**

Do **not** edit the PR change. Scope comes from GitHub read APIs plus optional local alignment when `@` contains the PR head.
...
```
gh pr view <number-or-url> --json title,body,baseRefName,headRefName,headRefOid,isCrossRepository,headRepository,headRepositoryOwner,url,files,reviews,comments --jq '{title, body, baseRefName, headRefName, headRefOid, isCrossRepository, headRepository, headRepositoryOwner, url, files: [.files[].path], hasPriorComments: ((.reviews | map(select(.state != "APPROVED" or .body != "")) | length) > 0 or (.comments | length) > 0)}'
```

Set `BASE:` to `pr:<number-or-url>` (logical marker, not a JJ revision). Set `UNTRACKED:` empty: JJ automatically tracks non-ignored files and has no staging-area distinction.

Resolve repository remotes before using any PR refs. Resolve `headRefName`, `baseRefName`, and `headRefOid` as non-empty scalar strings before interpolation. Validate each ref name inline: allow only ASCII letters, digits, `.`, `_`, `-`, and `/`; split on `/` and reject an empty component, a component with a leading or trailing dot, or a component ending in `.lock`; also reject `..`, `@{`, a leading dash, a trailing slash, and control characters. Validate the OID as one full hexadecimal Git object ID. Derive the PR base repository from the PR URL and the head repository from `headRepository`/`headRepositoryOwner` (the base repository when `isCrossRepository` is false). Normalize GitHub HTTPS and SSH URLs from `jj git remote list`, then match each repository to exactly one configured remote: `PR_BASE_REMOTE` for the base and `PR_HEAD_REMOTE` for the head. Never prefer a remote because it is named `origin`. An absent or ambiguous match is unavailable, not permission to guess.
...
1. `jj bookmark list "exact:<headRefName>" -r '::@'` reports that local bookmark in the ancestry of `@`.
2. The PR is **not** cross-repository (`isCrossRepository` is false).
3. `jj log -r 'commit_id(<validated-headRefOid>) & ::@' --no-graph -T commit_id` resolves exactly one commit. This confirms the working copy carries the GitHub head commit (allowing unpushed local changes layered on top) rather than an unrelated same-named bookmark. Never interpolate a raw SHA directly as a revset.
...
- **`local-aligned`:** Require `PR_BASE_REMOTE`. Resolve `<resolved-base-ref>` as `<validated-baseRefName>@<PR_BASE_REMOTE>` (run `jj git fetch --remote <PR_BASE_REMOTE> --branch exact:<validated-baseRefName>` if needed). Compute `BASE` with `jj log -r 'exactly(fork_point(<resolved-base-ref> | @), 1)' --no-graph -T 'commit_id ++ "\n"'`; then set `FILES:` from `jj diff --from "$BASE" --to @ --name-only` and `DIFF:` from `jj diff --from "$BASE" --to @ --git --context 10`. If the remote or single fork point is unavailable, use `pr-remote`; do not pick one of multiple candidates. This includes all changes through the working-copy change; JJ has no index. Do **not** call `gh pr diff` or append remote hunks — when unpushed fixes exist, the local workspace is canonical. Note in Coverage: `scope: local-aligned (PR; local workspace diff)`.
- **`pr-remote`:** Set `FILES:` from the PR `files` array. Set `DIFF:` from `gh pr diff <number-or-url> --color=never`. If `gh pr diff` fails, stop with an actionable error — do not mutate the workspace as a fallback.

When **`pr-remote`**, before Stage 4:

1. When `PR_HEAD_REMOTE` is available, best-effort fetch the PR head without editing the workspace: `jj git fetch --remote <PR_HEAD_REMOTE> --branch exact:<validated-headRefName>`. Accept it only when `exactly(<validated-headRefName>@<PR_HEAD_REMOTE> & commit_id(<validated-headRefOid>), 1)` resolves; this rejects force-push races and stale metadata.
2. When validation succeeds, set `PR_HEAD_REF=commit_id(<validated-headRefOid>)` for reviewers and validators. When the remote, fetch, or OID/ref match is unavailable, omit `PR_HEAD_REF` and note it in Coverage — reviewers must rely on diff hunks only. Raw GitHub OIDs are always wrapped in `commit_id()`.
3. When `PR_BASE_REMOTE` is available, best-effort fetch the PR base with `jj git fetch --remote <PR_BASE_REMOTE> --branch exact:<validated-baseRefName>`. When it succeeds, set `PR_BASE_REF=<validated-baseRefName>@<PR_BASE_REMOTE>`, a real JJ revision reviewers and validators use for file-level diffs. The `pr:<number-or-url>` logical marker in `BASE:` stays the scope marker. When the remote or fetch is unavailable, omit `PR_BASE_REF` and note it in Coverage — schema-drift and other JJ-diff checks fall back to diff hunks only and must **not** assume `main`.
...
If `jj bookmark list "exact:<bookmark>" -r '::@'` reports the local bookmark, use the **standalone (current workspace)** path below — same history, explicit bookmark name; do not use remote-only diff.

Otherwise diff the remote/local bookmark **without editing the workspace**:
...
2. Else match the repository identified by GitHub to exactly one configured JJ remote by normalized URL, then resolve `<bookmark>` as `<bookmark>@<matched-remote>` or `<bookmark>` after `jj git fetch --remote <matched-remote> --branch exact:<bookmark>` when needed. If no unique remote matches, try only the local bookmark; never assume `origin`.
3. Resolve the default base bookmark at runtime (same logic as standalone). Compute `BASE` with `jj log -r 'exactly(fork_point(<base-ref> | <bookmark-ref>), 1)' --no-graph -T 'commit_id ++ "\n"'`, then use `jj diff --from "$BASE" --to <bookmark-ref> --git --context 10`. Stop if the fork point is not exactly one commit.
4. If `<bookmark-ref>` cannot be resolved locally, stop: "Cannot diff bookmark `<bookmark>` without editing the workspace. Move to that change, pass its open PR URL/number, or review the current workspace with `base:`."
...
```
echo "BASE:$BASE" && echo "FILES:" && jj diff --from "$BASE" --to <bookmark-ref> --name-only && echo "DIFF:" && jj diff --from "$BASE" --to <bookmark-ref> --git --context 10 && echo "UNTRACKED:"
```

**If no argument (standalone in the current workspace):**

Apply the same base-detection logic as bookmark mode above, using local bookmarks in `::@` (and `gh pr view --json baseRefName,url` where GitHub can infer the associated PR).
...
Using `jj diff --from "$BASE" --to @` diffs the common ancestor against the working-copy change, including the whole JJ stack through `@`; JJ has no staged/unstaged split.

**Ignored-file handling:** `UNTRACKED:` is empty under JJ because non-ignored files are automatically tracked. Ignored files remain out of scope; if the caller explicitly identifies one, list it in Coverage and continue — never stop or prompt.
...
- **`local-aligned` / standalone / `base:`** — `DIFF_A="$BASE"` (a real JJ revision), `DIFF_B=@`.
...
if [ -z "${DIFF_A:-}" ] || [ -z "${DIFF_B:-}" ] \
   || ! jj log -r "$DIFF_A" --no-graph -T commit_id >/dev/null 2>&1 \
   || ! jj log -r "$DIFF_B" --no-graph -T commit_id >/dev/null 2>&1; then
  echo "EXEC_LINES:UNKNOWN"; echo "UNCOUNTED_FILES:1"; echo "SIGNALS:"
else
  # Count added/deleted executable lines from JJ's Git-format diff while excluding
  # file headers. Filesets are passed directly to JJ, not expanded by the shell.
  EXEC_LINES=$(jj diff --from "$DIFF_A" --to "$DIFF_B" --git -- 'glob:**/*.rb' 'glob:**/*.py' 'glob:**/*.js' 'glob:**/*.jsx' 'glob:**/*.ts' 'glob:**/*.tsx' 'glob:**/*.go' 'glob:**/*.rs' 'glob:**/*.java' 'glob:**/*.swift' 'glob:**/*.kt' 'glob:**/*.c' 'glob:**/*.cc' 'glob:**/*.cpp' 'glob:**/*.cs' 'glob:**/*.php' 'glob:**/*.ex' 'glob:**/*.exs' 'glob:**/*.scala' 2>/dev/null | awk '/^\+\+\+|^---/{next} /^\+|^-/{n++} END{print n+0}')
...
**Bookmark mode:** Run `jj log -r '${BASE}..<bookmark-ref>'` using the resolved common ancestor and bookmark revision from Stage 1. Use `<bookmark-ref>` (the resolved `<bookmark>@<matched-remote>` or local bookmark), not the raw argument — a remote-only bookmark may have no matching local bookmark.

**Standalone (current workspace):** Run:

```
echo "BOOKMARKS:" && jj bookmark list -r '::@' && echo "CHANGES:" && jj log -r "${BASE}..@"
```
...
3. **Auto-discover.** Extract 2-3 keywords from the relevant bookmark or change description (e.g., `topic/onboarding-skill` -> `onboarding`, `skill`). Glob `docs/plans/*` and filter filenames containing those keywords. If exactly one match, use it. If multiple matches or the match looks ambiguous (e.g., generic keywords like `review`, `repair`, `update` that could hit many plans), **skip auto-discovery** — a wrong plan is worse than no plan. If zero matches, skip.
...
Pass `{workspace_root}` and `{run_id}` to every persona sub-agent so they can write their full analysis to `{workspace_root}/.tmp/rocketclaw/code-review/{run_id}/{reviewer_name}.json`.
...
8. **For `data-migration` only:** the resolved review base and head revisions from Stage 1, wrapped in `<review-base>` and `<review-head>` inside the review context so schema drift checks never assume `main` or the local `@`. Use `@` as the head for local-aligned/standalone scope and the fetched PR/bookmark head for remote scope.
...
Read-only here means **non-mutating**, not "no shell access." Reviewer sub-agents may use non-mutating inspection commands when needed to gather evidence or verify scope, including read-oriented `jj` / `gh` usage such as `jj diff`, `jj show`, `jj file show`, `jj file annotate`, `jj log`, and `gh pr view`. In **`pr-remote`** or **`branch-remote`** scope (see Stage 1), inspect changed files via `jj file show -r <remote-head-ref> <path>` or diff hunks — do not Read/Grep workspace paths for files in scope. They must not edit project files, move bookmarks, describe or commit changes, push, create PRs, or otherwise mutate the workspace or repository state.

Each persona sub-agent writes full JSON (all schema fields) to `<workspace-root>/.tmp/rocketclaw/code-review/{run_id}/{reviewer_name}.json` and returns compact JSON with merge-tier fields only:
...
3. **Spawn validators with bounded parallelism.** One sub-agent per finding, dispatched independently using the validator template and the same bounded scheduler from Stage 4. Each validator receives:
   ...
   - `why_it_matters` when available — loaded from the per-agent artifact file at `<workspace-root>/.tmp/rocketclaw/code-review/{run_id}/{reviewer_name}.json`; omit when the file is absent or the artifact write failed. The validator proceeds without it, using the diff and cited code directly.
   - The full diff
   - The scope mode and remote head ref, mirroring the Stage 4 reviewer bundle: inject `<pr-scope-mode>local-aligned | pr-remote | branch-remote</pr-scope-mode>` and, when set, `<pr-head-ref>...</pr-head-ref>` or `<branch-head-ref>...</branch-head-ref>`. The validator template defaults to local-aligned workspace inspection when these are absent, so omitting them in `pr-remote`/`branch-remote` makes validators verify findings against the stale working copy — dropping valid findings or confirming false ones on the wrong revision.
   - Inspection access scoped by mode: in `local-aligned`, Read/Grep/`jj file annotate` the cited code, callers, guards, framework defaults, and history; in `pr-remote`/`branch-remote`, inspect via `jj file show -r <remote-head-ref> <path>` or the provided diff hunks only — do not Read/Grep workspace paths for files in scope.
...
Severity, confidence, and cross-reviewer agreement tell you what to do first and what to flag loudly — they do not gate the decision. There is no deny-list: downside is controlled after the fact (`jj undo` + visible diff + the isolated-change checkpoint), not by a precondition.

**Scope invariant.** Apply only when the working copy *is* what was reviewed — `local-aligned` or standalone. In `pr-remote` / `branch-remote` the working copy is not the reviewed head; do not apply — report instead.
...
**Describe and commit when the pre-review working-copy change was empty.** Before applying, note whether `jj diff -r @ --name-only` already reported changes. The permanence gate is the **push**, not the local change; JJ operations are reversible with `jj undo`.

- **Empty before the review:** after applying and verifying, describe and commit the fixes as one isolated JJ change. Before composing or validating its description, inspect the project's active instructions and conventions already in context and recent messages using the repository's own `git log` syntax. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. At runtime, the project's active instructions and conventions and the syntax actually present in its history are authoritative. Where they leave room, apply compatible Go guidance: an imperative, concise subject; a body that explains why when needed; and readable wrapping. Do not impose fixed prefixes, types, scopes, subjects, messages, templates, examples, or any other fixed message categories. Apply the description with `jj describe`, then use `jj commit` to finish the change and create a new working-copy change.
- **Non-empty before the review:** apply but do **not** describe or commit — the fixes interleave with the user's in-flight work and remain in the change they were already building. The Applied section lists what changed.
...
2. **Applied (default mode only).** When Stage 5c applied fixes, list them first — before the findings — in an Applied section (see review output template); each entry carries `#`, file, the fix, and reviewer (a multi-file fix is one row with one `#`), then a one-line validation outcome (e.g. "pin tests 4 -> 6; suite 94 pass, lint clean") and change status (described and committed as an isolated, repository-conformant JJ change when `@` was empty before review, or left in the user's existing change when it was non-empty). Flag green-but-unverifiable edits (auth/contract/concurrency) prominently. Omit this section in `mode:agent` and when nothing was applied. Applied findings appear here, not in the severity tables.
...
```json
{
  ...
  "scope": {
    "base": "<common-ancestor commit ID, pr:NNN marker, or base: ref>",
...
After Stage 6, stop. Never push, open PRs, or file tickets from this skill. In default (interactive) mode, Stage 5c has already applied and (when the pre-review working-copy change was empty) committed the safe fixes; in `mode:agent` the review mutates nothing — the caller (for example `ce-work`) and the user apply fixes, file tickets, or accept residual risk using the report and artifact.
...
Always write run artifacts under `<workspace-root>/.tmp/rocketclaw/code-review/<run-id>/`, where `<workspace-root>` is `jj workspace root` or the current directory when no JJ repository exists:
...
{
  "run_id": "<run-id>",
  "bookmark": "<nearest local bookmark in ::@ at dispatch time, or null>",
