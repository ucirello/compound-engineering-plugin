...
**Read `references/investigate.md` now and follow it for Phases 0-2** — issue fetching, reproduction, environment sanity and the dirty-tree sibling-working-copy experiment, backward tracing, the tracker/PR-history search, hypothesis grounding, and the escalation table. Only the gates below are stated here.
...
- **Bookmark.** Check `jj status`; if the user has uncommitted work in files that need modification, confirm before editing. If a bookmark at the working copy (`jj bookmark list -r @`) is the default bookmark, create a feature bookmark without asking — derive a name from the bug, `jj new` then `jj bookmark create <name> -r @`, and say which bookmark you moved to. Detect the default by comparing against `main`, `master`, or `trunk()` — `trunk()` is the default bookmark of the default remote.
- **Record the pre-fix scope:** current `@`, whether `jj status` is clean, and any pre-existing changed files. Then keep a list of **fix-owned files** (the tests and implementation changed for this bug) as you work. Phase 4 answers both of its questions from this record and cannot reconstruct it afterwards.
...
- **Ships** when all three hold: the pre-fix tree was clean, nothing on the bookmark is work the user has not already offered, and `origin` is **PR-capable**: somewhere `gh` can actually open a PR. Establish those however fits the repo in front of you. Two facts make it less obvious than it looks.
...
- **Not a JJ repo** (`jj workspace root` fails): nothing commits. Stop after the summary and the quality block.
...
**After a PR is open** — apply the reference's learning-capture criteria. Only when that gate qualifies the fix, offer capture; if the user accepts, invoke the `ce-compound` skill, then `jj commit` only the artifacts it actually wrote or updated and `jj git push` so the open PR picks them up. If it writes nothing, end without a documentation change.
