...
Resolve `<root>` the first time you compose a `<root>/` path; a read of `<root>/solutions/` counts as composing one. Pass the resolved path to scouts, never the config. A project that is not a Jujutsu repository has no `<root>`, so its prior-decision scan uses local ADRs and design docs instead.
