...
2. **opencode2.** Distinct from the Claude CLI adapter. When the resolved harness is `opencode2` and `opencode2` is on PATH, dispatch `opencode2 run --standalone --format json --file "$PROMPT_FILE" --model "$MODEL"` with cwd = the handoff directory, without `--auto` (cooperative read-only). If the binary is missing, fall through to the remaining adapters.
...
`ROCKETCLAW_PEER_HARD_SECS` (the outer runner cap) and `ROCKETCLAW_ELEVATION_HARD_SECS` (the worker's own inner cap) are set to the **same** raised backstop well above any legitimate run (R11) — keep them equal so the inner cap never reaps a healthy run before the outer one. `ROCKETCLAW_PEER_LOG_MAX_BYTES` is raised for the streaming route so a healthy high-volume run is not reaped as a failure (R22). `start` returns a job id in under ~2s.
