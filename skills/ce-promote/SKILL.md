...
- **The diff** — `jj diff --summary --from main` and skim notable changes to ground the claim in what actually changed.
- **Changelog** — the top/`[Unreleased]` entry in `docs/changelog.md`, `CHANGELOG.md`, or similar.
- **Recent commits** — `jj log -r 'latest(::@, 15)'` for the arc of the change.
