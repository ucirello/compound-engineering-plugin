...
3. **Code Review**
   ...
   Review the diff with **`ce-code-review`** as the single path. It self-right-sizes (a lite roster for small, low-risk, code-only diffs; the full roster otherwise), so there is no "escalate to a heavier reviewer" decision and **no harness-specific review detection**. The size and sensitive-surface judgment lives inside `ce-code-review`'s reviewer selection and small-diff gate.
   ...
   **3b. Apply fixes (caller-owned).** Load `references/review-findings-followup.md`: filter on JSON, batch by file, dispatch fix subagents. The orchestrator integrates, tests, and commits. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Runtime repository-local instructions and actual `git log` take precedence over compatible Go guidance. Do not impose a fixed syntax, message, prefix, type, scope, subject/body shape, template, or example. Then proceed to the Residual Work Gate.
...
2. **Commit and Create Pull Request**
   ...
   Load the `ce-commit-push-pr` skill to handle committing, `jj git push`, and PR creation. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Runtime repository-local instructions and actual `git log` take precedence over compatible Go guidance. Do not prescribe a fixed message, prefix, type, scope, subject/body shape, template, or example. The skill handles convention detection, bookmark safety, logical change splitting, and adaptive PR descriptions.
...
- [ ] Commit descriptions follow project conventions. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Runtime repository-local instructions and actual `git log` take precedence over compatible Go guidance. Do not impose a fixed syntax, message, prefix, type, scope, subject/body shape, template, or example.
