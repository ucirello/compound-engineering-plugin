...
- If updating, revise only the still-relevant sections. Plans do not carry per-unit progress state — progress is derived from Jujutsu changes by `ce-work`, so there is no progress to preserve across edits
