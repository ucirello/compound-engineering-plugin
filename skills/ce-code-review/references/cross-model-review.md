...
- `<run-dir>` = the Stage 4 workspace-local run dir (`$(jj workspace root)/.tmp/rocketclaw/ce-code-review/<run-id>/`, or `./.tmp/rocketclaw/ce-code-review/<run-id>/` outside a JJ repo). The script writes `adversarial-external.json` there.
