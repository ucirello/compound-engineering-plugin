...
Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Derive every change description at runtime from the project's active instructions and conventions, then inspect recent history with the repository's preferred `git log` syntax; those sources take precedence. Apply Go guidance only where compatible, and do not impose a fixed message, prefix, type, scope, subject, body, template, or example.
