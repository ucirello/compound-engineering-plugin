...
Run this **first** when `db/schema.rb` or `db/structure.sql` appears in the diff. Use the review base revision from caller context (`<review-base>` — common-ancestor revision or bookmark). **Never assume `main`.**
