...
Preserve required evidence, schemas, machine-readable values, and operational benchmark, model, and harness references. Fixed examples and report wording specify substance, not mandatory syntax unless an exact token is machine-readable. Do not add product branding, generated-by text, or attribution.
...
| Cross-cutting rewrite | One agent authors the contract, then fan out application | The replacement text must be identical everywhere to stay greppable and safely abandonable | No contract: N phrasings of one rewrite. No fan-out: context exhaustion, silent skips |
| Cut passes | Fan out by disjoint file ownership, single owner for shared byte-identical assets, barrier before verification | Concurrent edits to one file lose each other; verification must see one Jujutsu change | Overlapping ownership produces a change nobody authored and whose history cannot localize the regression |
...
- **Skip the contract** and N agents produce N phrasings of one rewrite. The change stops being greppable, so you cannot confirm coverage; it stops being safely abandonable as a unit; and when the next measurement fails you cannot tell whether the cause is the change or one agent's variant of it.
...
**Every artifact of this method needs a path decided before the phase that writes it.** The registration and the scored run table (`references/noise-floor.md`), the phase-marker map and the extractor (`references/baseline-mining.md`), the ruled finding set (`references/corpus-audit.md`), the ownership manifest and the rewrite contract (`references/cut-passes.md`). Pick one directory for the whole effort and keep them together: the registration must be re-readable by a skeptic after results exist, the extractor and map must be re-runnable when the taxonomy changes, and a contract or manifest passed only through a dispatch prompt cannot be checked afterward against what the agents actually did. Durable artifacts belong in the workspace's normal documentation location. Keep every temporary artifact under `<workspace-root>/.tmp/retune/<run-id>/`, resolving the root with `jj workspace root`; never use an operating-system temporary directory or API.
