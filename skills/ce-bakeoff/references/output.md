...
- **Read** `docs_root` from `<repo-root>/.rocketclaw/config.yaml` only (`<repo-root>` = the absolute root returned by public `jj workspace root`, or the absolute current directory outside JJ). Run subsequent JJ subprocesses with cwd set to that absolute workspace root. Do not read it from `config.local.yaml`. Unset -> `<root>` is `docs`, exactly as before.
