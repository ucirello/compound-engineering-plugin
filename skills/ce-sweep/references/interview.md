...
**Ask:** "Should item content from `{{source id}}` be withheld from described state and from plan text? Say yes when the source can carry screen recordings, PII, customer data, or anything you don't want written to a file that may be described or shared. When yes, the sweep drops item body and quote before writing state — only titles, urls, ids, and status persist. Default is no."
...
- **Committed to the repo** (recommended when multiple agents or machines share branches — one source of truth everyone reads and writes). Sets `sweep_state_path` to the described default `docs/feedback-sweep/state.yml`.
...
## 6. Shared branch (only if described state)

**Skip this section entirely if the user chose machine-local state in section 4** — the shared-branch topology only applies to described state.

**Ask:** "Is this a multi-agent setup where several checkouts push the sweep state to a shared docs branch? Answer yes only if more than one machine or agent changes and pushes to the same branch. Default is no — a single checkout committing locally."

- **No** (default) -> `sweep_shared_branch: false`. The single-writer lease serializes overlapping sweeps within one checkout.
- **Yes** -> `sweep_shared_branch: true`. Explain: the lease becomes **push-gated** — before any source-side write, the sweep changes and pushes the lease acquisition on the shared branch and confirms its writer won, making the lease a repo-wide mutex across machines.
...
- `sweep_shared_branch` — from section 6 (default `false`; only meaningful with described state).
...
sweep_state_path: docs/feedback-sweep/state.yml   # described (multi-agent) or /tmp path (solo)
...
- Each `feedback_sources` entry carries: `type` (`slack` | `github-issues` | `email`), `id` (short handle), `target` (channel ID / `owner/repo` / mailbox hint), `ack_action` and `closeout_action` (emoji/label names; omit both for email), `sensitive` (`true` withholds body/quote from described state and plan text), and `approved` (standing approval for source-side writes; `false` keeps the source read-only with `ack_deferred` items).
