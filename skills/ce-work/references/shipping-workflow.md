...
2. **Describe Changes and Create Pull Request**
   ...
   Before handoff, inspect `jj status` and `jj log -r 'trunk()..@'`. Describe any non-empty final `@` with the appropriate filesets; after `jj commit`, the ship tip is normally `@-` because `@` is the new empty change. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Local instructions and history syntax win; use Go guidance only when compatible. Create or advance exactly one meaningful bookmark to the ship tip with `jj bookmark set <name> -r <ship-tip>`, then verify it with `jj bookmark list -r <ship-tip>`. Do not point the bookmark at an empty post-description `@`.
   ...
   If the user prefers to describe changes without creating a PR, load the `ce-commit` skill instead. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Local instructions and history syntax win; use Go guidance only when compatible.
...
- [ ] Change descriptions follow repository standards. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Local instructions and history syntax win; use Go guidance only when compatible.
