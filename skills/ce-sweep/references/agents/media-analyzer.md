...
3. **Check whether the issue already appears fixed on the main branch.** Once you know the affected product area, use read-only `jj log` / `GIT_DIR="$(jj git root)" gh` on that area (files, routes, components the symptom touches) to see whether a recent commit or merged PR already addresses it. Report this as a field in your finding so the orchestrator does not re-file resolved work.
...
- You are read-only except for the ONE write to your scratch artifact path. Read-oriented `jj` / `gh` (pair `gh` that talks to the local repo with `GIT_DIR="$(jj git root)"`) and running the bundled analyzer are permitted. Do not edit project files, change bookmarks, commit, push, or open PRs.
