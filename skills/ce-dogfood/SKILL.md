...
2. **Drive it** with agent-browser — navigate, snapshot for interactive refs, click, fill, submit, follow the journey to its real end state:
   ...
   mkdir -p "$WORKSPACE_ROOT/.tmp/rocketclaw/ce-dogfood/screenshots"
   agent-browser screenshot "$WORKSPACE_ROOT/.tmp/rocketclaw/ce-dogfood/screenshots/<scenario>.png"
   agent-browser errors      # check console/page errors
   ```

   Write transient screenshots under `$(jj workspace root)/.tmp/rocketclaw/ce-dogfood/`; when no JJ repository is available, use `./.tmp/rocketclaw/ce-dogfood/`. Ensure `.tmp/` is ignored in `.gitignore` before creating files there; because JJ snapshots working-copy files automatically and has no staging area, use `jj file untrack` if `.tmp` was already tracked. Only copy a screenshot into the report's location if you intend to embed it in the final report.
