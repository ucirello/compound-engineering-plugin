...
3. **Unpushed change stack vs base bookmark.** If the working-copy change is empty, review `jj diff --from <base> --to @`, where `<base>` is the default bookmark (usually `main@origin` or `master@origin`).
