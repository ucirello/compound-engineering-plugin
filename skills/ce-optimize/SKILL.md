...
- **Fresh start**: archive the old bookmark to `optimize-archive/<spec-name>/archived-<timestamp>` with `jj bookmark create <archive-name> -r optimize/<spec-name>` and move/recreate `optimize/<spec-name>` for the new run, clear the experiment log, start from scratch

### 0.5 Create Optimization Change/Bookmark and Scratch Space

```bash
jj new main@origin                         # or the project default bookmark
jj bookmark set "optimize/<spec-name>" -r @  # or jj edit optimize/<spec-name> if resuming
```
...
- Ask the user to describe/commit them, move them to another change, or otherwise clear them before proceeding
...
3. **If best improves on current best: KEEP**
   - Describe the experiment change first (`jj describe -m "optimize(<spec-name>): <hypothesis description>"`) so the winning diff has a durable description before integration
   ...
   - Rebase the experiment change onto the optimization bookmark as needed, then move the optimization bookmark to the winning change with `jj bookmark move optimize/<spec-name> --to @`
...
4. **Check file-disjoint runners-up** (up to `max_runner_up_merges_per_batch`):
   ...
   - Otherwise: abandon the rebased trial, log as "promising alone but neutral/harmful in combination" (outcome: `runner_up_reverted`), then clean up the runner-up's experiment workspace and bookmark
...
6. **Abandon all others**: cleanup workspaces, log as `reverted`
...
- Abandon the experiment change (cleanup workspace)
...
1. **Run `/ce-code-review`** on the cumulative diff (baseline to final). Load the `ce-code-review` skill on the optimization bookmark/change (interactive or `mode:agent`). To land eligible fixes before the next option, apply the mechanical-apply bar below.
...
5. **Done** -- leave the optimization bookmark/change for manual review.
