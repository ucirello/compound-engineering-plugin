...
1. **Schema drift (when `schema.rb` / `structure.sql` is in the diff)** — unrelated dump changes from other lines of work
...
Run this **first** when `db/schema.rb` or `db/structure.sql` appears in the diff. Use the review base and head revisions from caller context. Resolve current syntax from local `jj help` before running it. **Never assume `main`.**
