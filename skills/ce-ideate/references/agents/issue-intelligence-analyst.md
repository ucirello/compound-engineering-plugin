...
1. **Jujutsu repository** — confirm the current directory is a Jujutsu repo using `jj workspace root`
2. **GitHub remote** — detect the repository. Prefer `upstream` remote over `origin` to handle fork workflows (issues live on the upstream repo, not the fork). Use `jj git remote list` only when remote inspection is needed, then use `gh repo view --json nameWithOwner` to confirm the resolved repo.
