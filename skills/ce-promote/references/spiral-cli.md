...
Resolve the workspace root, then add `ce_promote_spiral_optout: true` as a top-level key to `<root>/.compound-engineering/config.local.yaml`, using the native file-write/edit tool:
...
- **File absent:** create it (and its `.compound-engineering/` directory) with the key, AND make sure the machine-local config won't enter version control. Check whether the root-relative path `<root>/.compound-engineering/config.local.yaml` is ignored by the project's VCS ignore rules. Prefer the repo's local exclude mechanism when available; otherwise add `.compound-engineering/*.local.yaml` to `.gitignore` only when the user explicitly wants a shared rule. `ce-setup` is the canonical place that adds the shared `.gitignore` entry for teammates. Without any ignore, a user who runs `/ce-promote` before `/ce-setup` could accidentally include machine-local opt-out state in a change.
