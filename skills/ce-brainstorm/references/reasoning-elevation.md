...
1. **Harness-directed `opencode2`.** When the resolved `brainstorm_harness` is `opencode2` and `command -v opencode2` succeeds, dispatch via the bundled `scripts/elevation-dispatch.sh` worker with `ELEVATION_HARNESS=opencode2` (see Off-host dispatch). Use only `opencode2` flags from `opencode2 run --help`: `run --format json --file <prompt-file> --auto` and `--model provider/modelname#variant` when a model is set. Do not pass OpenCode (`opencode`) flags such as `--dir` or `--variant`. When `brainstorm_harness` is `opencode`, that is a different CLI (`command -v opencode`) — do not run `opencode2` for it. If `opencode2` is missing from PATH, fall through.
...
Hand over the working context as **file paths the subagent reads itself**, never a re-narrated prose brief. Create **one private per-run handoff directory** (`WS_ROOT="$(jj workspace root 2>/dev/null)" || WS_ROOT=""; TMP_BASE="${WS_ROOT:-.}/.tmp"; mkdir -p "$TMP_BASE"; mktemp -d "$TMP_BASE/ce-elevation-XXXXXX"` — if `jj workspace root` fails, `TMP_BASE=".tmp"`) and write the prompt-file and every evidence file into *that* directory. On the Claude CLI route the worker grants the elevated model read access to only that one directory (via `--add-dir` on the prompt-file's parent), so the handoff files stay readable while the rest of the scratch root — other same-user scratch and credentials — is not exposed:
...
```
   ...
   CE_PEER_HARD_SECS=5400 CE_ELEVATION_HARD_SECS=5400 CE_PEER_LOG_MAX_BYTES=52428800 \
     ELEVATION_HARNESS="<opencode2 when brainstorm_harness is opencode2; otherwise unset>" \
