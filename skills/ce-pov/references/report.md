...
- Write disposable output under `$(jj workspace root)/.tmp/rocketclaw/ce-pov/`; outside a JJ workspace, use `.tmp/rocketclaw/ce-pov/` relative to the current directory. Use `docs/` when the user wants it kept, and announce the absolute path. Do **not** introduce a new mandated `docs/` location — that store is deferred.
