...
Skip if nothing changed. Otherwise commit **only** the files this refresh modified (named fileset; there is no staging area), and describe the change in the repo's convention. **Read `references/commit.md`** for the per-mode bookmark decision and the jj-failure fallback.
