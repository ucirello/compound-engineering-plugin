...
- **Phase 3 (workspace/bookmark):** Operate on the current bookmark; the orchestrator owns workspace context, so never prompt to create a bookmark or about working-copy changes. Finalize the fix with a repository-conforming description and push. Local repository conventions and visible history take precedence; apply compatible Go guidance only where they do not decide the wording. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Never weaken, skip, or mock a failing assertion to make it pass; repair the real issue or defer.
...
- **Moving-target guard:** if the recurrence traces to an external cause (a base-bookmark update, a dependency bump, flaky infrastructure) rather than your fixes fighting each other, it is *not* an emergent trade-off — keep fixing, and note the external cause. Recurrence is only meaningful when your own fixes are what oscillate.
...
- `fixed-and-pushed` — a convergent fix was applied, tests pass, finalized and pushed.
