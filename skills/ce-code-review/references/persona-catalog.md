...
Use `deployment-verification-assistant` when the migration-artifact gate applies **and** the change is risky (destructive DDL, backfills, NOT NULL without default, column renames/drops). Schema drift and migration safety live in the `data-migration` persona, not a separately registered actor.

| Prompt asset | Focus |
|-------|-------|
| `deployment-verification-assistant` | Go/No-Go deployment checklist with SQL verification queries and rollback procedures |
...
6. **For RocketClaw conditional prompt assets**, use `deployment-verification-assistant` when the migration-artifact gate applies and the change is risky (see above).
