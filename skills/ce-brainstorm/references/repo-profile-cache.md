...
Each JJ workspace keeps its own entry under `.tmp/rocketclaw/repo-profile/`. Lookup is JJ metadata only; on a hit, only this one file is read. Outside a JJ repo, scratch storage falls back to `.tmp/rocketclaw/repo-profile/` under the current project directory, but this revision-keyed cache returns `NO-CACHE` because no valid key exists.
...
- `MISS` then a write-path on the next line → dispatch the `repo-profiler` persona to derive the profile, write its JSON output to a file, then persist it. This `put` runs after the profiler, so it is a **separate Bash-tool call** from the `get` above — shell variables do not persist between calls, so **re-set `SKILL_DIR` in the same command**:
  ...
  ```
- `NO-CACHE` → no JJ repo or no writable workspace-local cache. Derive the profile fresh for this run and **skip** `put` (nothing to persist). Any run-local profile file belongs under `$(jj workspace root)/.tmp/rocketclaw/repo-profile/`; use `.tmp/rocketclaw/repo-profile/` under the current project directory when no JJ repo exists.
