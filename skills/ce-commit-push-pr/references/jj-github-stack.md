...
Resolve a parent PR with `gh pr view <ref> --json baseRefName,headRefName,headRefOid,author,url,state`. Fetch its head bookmark from the correct remote with `jj git fetch --remote <remote> --branch <headRefName>`, then verify that `<headRefName>@<remote>` resolves to `headRefOid`. A name match without an object match is not proof of parent identity.
...
Create each PR with `gh pr create`, setting its base to the immediate parent layer's bookmark and its head to the layer bookmark. The bottom layer targets the resolved repository base or named parent. Use explicit PR URLs for later `gh pr view` and `gh pr edit` calls.
