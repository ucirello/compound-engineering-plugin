...
1. Materialize two Jujutsu workspaces from the same immutable source revision. Record that source change ID and commit ID for each arm.
...
| `source_change_id`, `source_commit_id` | identifies the logical source and proves the arms used the same immutable revision |
