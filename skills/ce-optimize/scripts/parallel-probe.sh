...
SQLITE_FILES=$(find "${SCAN_PATHS[@]}" -maxdepth 4 \( -name .jj -o -name .git -o -name node_modules -o -name .claude -o -name .context -o -name .worktrees -o -name .tmp \) -prune -o -type f \( -name '*.db' -o -name '*.sqlite' -o -name '*.sqlite3' \) -print 2>/dev/null | head -10 || true)
...
LOCK_FILES=$(find "${SCAN_PATHS[@]}" -maxdepth 4 \( -name .jj -o -name .git -o -name node_modules -o -name .claude -o -name .context -o -name .worktrees -o -name .tmp \) -prune -o -type f \( -name '*.lock' -o -name '*.pid' \) ! -name 'package-lock.json' ! -name 'yarn.lock' ! -name 'bun.lock' ! -name 'bun.lockb' ! -name 'Gemfile.lock' ! -name 'poetry.lock' ! -name 'Cargo.lock' -print 2>/dev/null | head -10 || true)
