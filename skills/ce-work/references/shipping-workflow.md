...
1. **Run Core Quality Checks**
   ...
   Refresh the configured JJ remote and rebase the current JJ stack onto the latest trunk before final tests:

   ```bash
   jj git fetch
   jj rebase -r 'trunk()..@' -o 'trunk()'
   jj status
   jj log -r 'trunk()..@ & conflicts()'
   ```

   `-r` selects only the revisions in the current ancestry stack; do not use `-b @`, whose selection can include descendants outside that stack. JJ records rebase conflicts in changes and does not require a continuation command. If the conflict revset is non-empty, resolve the affected files or use `jj resolve`, inspect `jj diff`, and rerun the status/conflict check. Then always run before submitting:
   ...
   # Use linting-agent before jj git push
   ```
...
3. **Code Review**
   ...
   **3a. Review (read-only).** Resolve the range with `jj log -r 'trunk()..@'` and `jj diff --from 'trunk()' --to @`, then invoke `ce-code-review` with `mode:agent` (add `plan:<path>` when known; normally `base:trunk()`, or another explicit base revset). Pass **`depth:full`** when the plan, the task, or the user explicitly asked for a full / deep / thorough review — that is the one escalation signal `ce-code-review` cannot infer from the diff alone. Do not pass `mode:autofix`. Parse the JSON.

   **3b. Apply fixes (caller-owned).** Load `references/review-findings-followup.md`: filter on JSON, batch by file, dispatch fix subagents. The orchestrator integrates JJ changes, tests, and describes logical units. Then proceed to the Residual Work Gate.
...
4. **Residual Work Gate** (REQUIRED when `ce-code-review` ran and left actionable residuals)
   ...
   **Interactive sessions:** Ask the user using the platform's blocking question tool (`AskUserQuestion` in Claude Code with `ToolSearch select:AskUserQuestion` pre-loaded if needed, `request_user_input` in Codex, `ask_question` in Antigravity CLI (`agy`), `ask_user` in Pi (requires the `pi-ask-user` extension)). Fall back to numbered options in chat only when the harness genuinely lacks a blocking tool. Never silently skip the gate.
   ...
   - `Apply/fix now` — load `references/review-findings-followup.md`, dispatch batched fix subagents for remaining eligible findings, run tests, describe a change if needed; optionally re-run `ce-code-review` only after the diff changed materially.
   ...
   - `Accept and proceed` — record the residual findings verbatim in a durable "Known Residuals" sink before shipping. If a PR will be created or updated in Phase 4, include them in the PR description's "Known Residuals" section. If the user later chooses the no-PR `ce-commit` path, create `docs/residual-review-findings/<bookmark-or-change-id>.md`, include the accepted findings and source review-run context, include that path in the appropriate `jj commit` fileset, and mention it in the final summary. At that commit site: Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Repository-local instructions and syntax visible in `git log` always win; apply Go guidance only when compatible. Derive the message syntax dynamically; do not impose fixed prefixes, types, scopes, subjects, templates, or examples. Preserve the accepted residuals' repository-relevant semantic content. The user has acknowledged the risk, but the findings must not live only in the transient session.
...
1. **Prepare Validation Context**
   ...
   Note whether the completed work has observable behavior (UI rendering, CLI output, API/library behavior with a runnable example, generated artifacts, or workflow output), and summarize any manual validation performed. If the user supplied evidence (URL, markdown embed, local artifact path), retain it as PR-description context.

2. **Describe Changes and Create Pull Request**

   Before handoff, inspect `jj status` and `jj log -r 'trunk()..@'`. Describe any non-empty final `@` with the appropriate filesets; after `jj commit`, the ship tip is normally `@-` because `@` is the new empty change. At this commit site: Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Repository-local instructions and syntax visible in `git log` always win; apply Go guidance only when compatible. Derive the message syntax dynamically; do not impose fixed prefixes, types, scopes, subjects, templates, or examples. Preserve the final change's repository-relevant semantic content. Create or advance exactly one meaningful bookmark to the ship tip with `jj bookmark set <name> -r <ship-tip>`, then verify it with `jj bookmark list -r <ship-tip>`. Do not point the bookmark at an empty post-description `@`.

   Resolve `heads(trunk()..@ & ~empty())` and require exactly one publishable tip. Obtain the target GitHub repository URL from explicit PR metadata or `GIT_DIR="$(jj git root)" gh repo view --json url`, normalize it against `jj git remote list`, and require exactly one matching configured remote; stop if repository identity is ambiguous. Fetch that remote, advance the bookmark to the publishable tip, and push only that bookmark with `jj git push --remote "$REMOTE" --bookmark "exact:$BOOKMARK"`. Resolve `REPOSITORY` from that remote. Before writing the PR body, resolve the workspace root with `jj workspace root`; when JJ is unavailable, use the current directory as the local fallback. Preserve existing `.gitignore` entries while ensuring `.tmp/` is ignored, refuse `.tmp` when it is a symlink or non-directory, and create `<resolved-root>/.tmp/rocketclaw/ce-work/`. Create or update the PR with explicit bookmark and repository context: `gh pr view "$BOOKMARK" --repo "$REPOSITORY" --json number,url,state`; atomically create a unique PR-body scratch file under `<resolved-root>/.tmp/rocketclaw/ce-work/`, write the body there, then use `gh pr create` or `gh pr edit` with explicit repository, bookmark, base, title, and body-file values. Never reuse a fixed PR-body filename or rely on implicit repository or bookmark discovery.
   ...
   If the user prefers to describe changes without creating a PR, load the `ce-commit` skill instead. At that delegated commit site: Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Repository-local instructions and syntax visible in `git log` always win; apply Go guidance only when compatible. Derive the message syntax dynamically; do not impose fixed prefixes, types, scopes, subjects, templates, or examples. Preserve the final change's repository-relevant semantic content.
...
- [ ] Validation/evidence context included in the PR description when the change has observable behavior
- [ ] Change descriptions follow repository standards
