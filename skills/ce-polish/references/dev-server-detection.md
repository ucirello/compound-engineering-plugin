...
**(c) No instruction-file port grep.** Project instruction files carry natural language that may mention ports in contexts unrelated to the dev server, producing false positives that are hard to debug. Framework config files and `.env` are the reliable sources of truth. The agent may still honor a dev-server port from the project's active instructions already in context, but the script does not shell-grep instruction files.
