...
Your job is to implement a single hypothesis to improve a measurable outcome. You will modify code within a defined scope, then stop. You do NOT run the measurement harness, describe or finalize the JJ change, or evaluate results -- the orchestrator handles all of that.
...
5. Do NOT describe or finalize the JJ change (the orchestrator handles a winning change)
...
- For Codex dispatch: use `<workspace-root>/.tmp/rocketclaw/optimize/` when `jj workspace root` resolves to a writable root, otherwise use local `.tmp/rocketclaw/optimize/`. Create the selected directory, write the filled template to a collision-resistant file there, and pipe it to `codex exec --skip-git-repo-check -` via stdin.
