...
2. `cross_model_peer:` from the two repo config files (`config.local.yaml` then `config.yaml`). Apply the ordinary-key rule: first active supported target wins; an invalid value continues to the next layer, then step 3. Honor `cross_model_peer: opencode2` as its own target — not compatible with `opencode`, and never a V1 opencode fallback.
...
- `<target>` is exactly one of `codex`, `claude`, `grok`, `cursor`, `composer`, `opencode`, or `opencode2`; `<fixed-route>` is its already-sanctioned concrete route token from the Step 1 table (`codex`, `claude`, `grok-cli`, `grok-cursor`, `cursor`, `composer`, `opencode`, or `opencode2`).
