...
|-------|---------|--------|
| `diff:<revision-or-revset>` | `diff:<change-id>`, `diff:<bookmark>..@`, `diff:PR#<number>` | Forces diff mode on that change |
...
- An unrecognized `<word>:<word>` token (including a project-specific description prefix appearing inside a topic) is not a flag — it passes through verbatim as request text. The same holds for a *recognized* token that fails the reads-as-a-flag test above.
...
- **Diff** — the request names a resolvable change: a change or commit ID, bookmark, PR, "the previous change", "what you just did", "this change".
