...
**Colocated Git dir.** `gh` needs the colocated Git backend. Resolve the path once with `jj git root` as its own call, then set `GIT_DIR=<that path>` as an env prefix on every `gh` invocation and every bundled-script call. Do not nest `$(jj git root)` in the same command. Combine with `GH_HOST` when both apply.

**Step 1** -- Get comment details and GraphQL node ID via REST (cheap, single comment):
```bash
GIT_DIR=<path-from-jj-git-root> GH_HOST=<host> gh api repos/OWNER/REPO/pulls/comments/COMMENT_ID \
...
GIT_DIR=<path-from-jj-git-root> GH_HOST=<host> bash "$SKILL_DIR/scripts/get-thread-for-comment" PR_NUMBER COMMENT_NODE_ID [OWNER/REPO]
```
...
```bash
GIT_DIR=<path-from-jj-git-root> GH_HOST=<host> gh api --paginate repos/OWNER/REPO/pulls/PR_NUMBER/reviews --jq '.[] | select(.state == "PENDING") | .id'
```
