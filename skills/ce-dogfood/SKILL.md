...
| When | Skill | Why |
|------|-------|-----|
| Phase 0 isolation | `ce-worktree` | Run the dogfood in an isolated JJ workspace so the main checkout stays clean. |
...
5. Fix loop     On failure: fix -> add regression test -> describe/split change -> continue
...
1. **Identify the target — keep PR identity; do not switch the working tree yet.**
   ...
   - **Blank:** the target is the current bookmark or working-copy change.
...
3. **Decide isolation by what you're testing; let `ce-worktree` own the JJ workspace mechanics.** Do not re-derive workspace detection or creation here — `ce-worktree` handles existing-isolation detection, the harness-native tool, attaching to a ref, and reports its decision back. The only call this skill makes is *whether to ask for isolation at all*:
   - **Blank / current-bookmark target:** do **not** isolate — dogfood in place. You are already on the bookmark or working-copy change under test, and the fix changes belong on it. (If you happen to already be in a workspace, that is fine — you are simply dogfooding here.)
   - **A PR or a different named bookmark:** this is an existing ref to test without disturbing your current checkout. Offer isolation (platform's blocking question tool). On **yes**, invoke `ce-worktree` to isolate **that target ref** — it attaches a JJ workspace to the ref (or, if already isolated, checks it out in place; or reports where to work when the ref is live elsewhere). Act on `ce-worktree`'s verdict; the primary checkout is never switched. On **no**, check the target out in place (`gh pr checkout <number>` for a PR, `jj new <bookmark>` for a bookmark), confirming first if undescribed working-copy changes would be disturbed.
...
Derive the trunk ref once, then pull the full diff against it and read it. Do not hard-code `main` — a repo whose default bookmark is `master` or anything else would produce the wrong comparison.

```bash
# Resolve the trunk to a bookmark that actually exists. Start from gh's detected
# default name, then fall back to common names.
DEFAULT=$(gh repo view --json defaultBranchRef -q .defaultBranchRef.name 2>/dev/null)
TRUNK=""
for cand in "$DEFAULT" main master; do
  [ -n "$cand" ] || continue
  if jj bookmark list "$cand" >/dev/null 2>&1; then
    TRUNK=$cand; break
  fi
done
TRUNK=${TRUNK:-main}
...
4. Describe the fix with `ce-commit`. Keep one logical fix per JJ change; use `jj split` or a path-limited `jj commit` if the working copy contains unrelated edits.
