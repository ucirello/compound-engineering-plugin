...
**Jujutsu bookmark (pre-resolved):** !`jj bookmark list -r @ -T 'name ++ "\n"' 2>/dev/null | head -n 1 || true`

If the line above resolved to a plain bookmark name, use it in Phase 1 session-history filtering so the orchestrator does not waste a turn deriving it. If it still contains a backtick command string or is empty, derive the current bookmark at runtime. Session records may call this value a branch because that is the source harness's metadata field.

**Workspace root (pre-resolved):** !`jj workspace root 2>/dev/null || pwd`

If the line above resolved to an absolute path, use it as the session-history repository filter in Phase 1. If it still contains a backtick command string or is empty, derive the workspace root at runtime with `jj workspace root 2>/dev/null || pwd`.
...
- `scripts/session-history/` — session discovery and extraction scripts copied into this skill so session-history support does not depend on a standalone public skill
...
If the user says yes, run the internal session-history step in Phase 1 (see step 4). If no, skip it. Do not ask this in lightweight mode or headless mode. There is no standalone session-history product surface; this support exists only inside the compounding workflow.
...
Phase 1 subagents write their full structured output to a per-run scratch artifact under `$(jj workspace root)/.tmp/rocketclaw/compound/<run-id>/` and return only a compact confirmation containing the artifact path. When `jj workspace root` is unavailable, use `<current-directory>/.tmp/rocketclaw/compound/<run-id>/` as the local fallback. The orchestrator Reads those artifacts back in Phase 2 assembly. This scratch space does not make the files additional deliverables. **Only the orchestrator writes product files** — the final solution doc and the maintenance side effects below. Subagents must not touch `docs/`, project instruction files, or any tracked path other than the workspace-local `.tmp/` scratch tree. Beyond the Phase 2 solution doc, the orchestrator's other writes are maintenance side effects — not additional deliverables, and creating one when absent is expected, not a violation of this rule:
...
- On `NO-CACHE` (no Jujutsu workspace or no writable workspace-local reuse directory): derive the orientation inline this run and skip `put`.
...
**CRITICAL — the `docs/solutions/` enumeration is NEVER cached; the Related Docs Finder must glob it FRESH every run.** `ce-compound` *writes* new learnings into `docs/solutions/`, so a cached index would miss a doc added moments ago (even one only in the working-copy commit). The cached profile supplies only the agnostic orientation above; the `docs/solutions/` search in step 3 always runs against the live tree.
...
#### 2. **Solution Extractor**
   ...
   - **Writes merge-state claims for time.** Cite PR numbers rather than bare commit IDs — IDs may be rewritten by Jujutsu history edits and squash merges and may not exist in other workspaces. A "fixed in X" claim requires the fix to be reachable from the current tree; otherwise phrase it as pending ("fix opened in #1608, unmerged as of this writing")
...
#### 4. **Session History** (internal flow after launching the parallel block — only if the user opted in)
   ...
   - Run session discovery, bookmark/recorded-branch filtering, scan-window selection, deep-dive selection, and per-session extraction directly inside this skill using `scripts/session-history/`.
   ...
   **Session-history payload — keep tight.** A long, keyword-rich payload licenses widening. Use this shape:
   ...
   - **Pre-resolved context** (only if values resolved cleanly above; otherwise omit): repository name, current Jujutsu bookmark.
   ...
   - **Filter rule (one line)**: "Only surface findings directly relevant to this specific problem. Ignore unrelated work from the same sessions or bookmarks/recorded branches."
   - **Output schema**:
   ...
   **Discovery pipeline.** Infer the scan window from the problem topic, starting with 7 days. Run discovery and metadata extraction:
   ...
   Pi sessions are included when present under `~/.pi/agent/sessions/`; they carry `cwd` like Codex but no branch metadata. If `_meta.files_processed` is `0`, return `no relevant prior sessions`. If the first pass finds no relevant bookmark/recorded-branch matches, or if processing Codex or Pi sessions, derive 2-4 keywords from the topic and re-run metadata extraction with `--keyword K1,K2,...`. Keep at most 5 sessions across Claude Code, Codex, Cursor, and Pi, ranked by bookmark/recorded-branch match, keyword match count, file size over 30KB, and recency. Exclude the current session.
...
8. **Validate parser-safety of the written frontmatter** to catch silent-corruption issues the prose rules miss: malformed `---` delimiter lines, unquoted ` #` in scalar values (silent comment truncation), and unquoted `: ` in scalar values (silent mapping confusion). The bundled validator ships **inside the skill bundle**; resolve it from this skill's directory because the runtime shell's working directory is the user's project:
   ...
   Nested values, array items, and already-quoted values are out of scope here (array-item quoting is handled by the schema/YAML-safety step above). Then state in the completion output that the bundled script validator was unavailable and the checks were applied manually.
...
> Shared domain vocabulary for this project — entities, named processes, and status concepts with project-specific meaning. Seeded with core domain vocabulary, then accretes as learning and refresh workflows process learnings; direct edits are fine. Glossary only, not a spec or catch-all.
...
2. **Semantic grounding validator (Full and headless; lightweight skips it).** Dispatch one read-only generic subagent built from the prompt template in the reference, covering the written doc plus any `CONCEPTS.md` entries added or edited this run. It verifies code-behavior claims by quoting the defining source line, merge-state claims against remote truth (`gh` primary, Jujutsu revset reachability fallback), and internal completeness of countable assertions. Apply its verdicts per the reference (fix contradicted claims from the quoted evidence; soften or drop unverifiable ones; mark offline merge-state checks as degraded), then re-run the mechanical check if the body changed.
...
- Any code-heavy issue → preserve code simplification as a **read-only documentation review**. Inspect the solution draft's code examples and explanatory claims inline, or dispatch a generic subagent seeded with a local prompt only to return suggestions. Do **not** invoke `ce-simplify-code` from this phase and do not mutate product code unless the user explicitly asks for a separate code-simplification pass. Do not use the deleted `code-simplicity-reviewer`.
  Example: review the solution draft's examples for speculative abstractions, redundant wrappers, dead branches, and just-in-case parameters. Apply edits only to the documentation/examples being written by `ce-compound`; leave other working-copy changes untouched.
...
| Wrong | Correct |
|----------|-----------|
| Subagents write product files into `docs/` or edit tracked paths | Subagents write only scratch artifacts under `$(jj workspace root)/.tmp/rocketclaw/compound/<run-id>/` (or the local `.tmp/rocketclaw/compound/` fallback) and return the path; orchestrator writes the one final doc |
...
Subagent Results:
  ...
  Session History: 3 prior sessions on the same bookmark/recorded branch, 2 failed approaches surfaced
