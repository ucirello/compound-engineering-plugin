...
1. **Resolve the target directory and extension.**
   ...
   - **Otherwise (no repo, or elsewhere with no `<root>/ideation/`):** write into the run's workspace-local scratch area — the `<scratch-dir>` resolved in Phase 1 (`<scratch-root>/ideate/<run-id>/`). Do **not** write loose files into the user's current working directory, and do **not** create a `<root>/ideation/` tree for a subject unrelated to the repo. Announce the absolute path and note it is scratch storage (move it to a durable location to keep it).
...
- **Inside a JJ workspace:** offer to record only the ideation doc as a described change, preserving unrelated working-copy changes. If the user declines, leave the document in the working copy. On acceptance, invoke `ce-commit` with that file-only scope and explicit constraints not to create a bookmark or push. If the callee cannot honor those constraints, report the blocker and leave the document uncommitted. Pass this message policy: "Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards." Here `git log` means history inspected using `jj log` in this JJ workflow; runtime project instructions and history syntax override the Go guidance.
...
- the deliverable was written automatically in both modes (Phase 4) — to `<root>/ideation/` when present, else the workspace-local scratch area, never loose in the user's CWD
