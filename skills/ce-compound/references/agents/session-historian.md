...
- **`scratch_dir`** — absolute path to a scratch directory under the workspace `.tmp` holding pre-extracted files.
- **`sessions`** — an array of objects (5 max), one per pre-extracted session, each with:
  ...
  - `branch` — bookmark when present (Claude Code only; recorded as `gitBranch`)
