...
#                     repo root via `jj root`, with a colocated-repo fallback
#                     for test fixtures that are not JJ workspaces.
#
...
#                          no positional arg and no repo root could be resolved
...
if [ -n "$TARGET_PATH" ]; then
  ...
  if [ -z "$TARGET_PATH" ] && [ -e ".git" ]; then
    TARGET_PATH=$(pwd)
  fi
  if [ -z "$TARGET_PATH" ]; then
    echo "ERROR: could not resolve repo root and no path argument provided" >&2
    exit 1
  fi
fi
