...
**Scope:** common ancestor with the review base bookmark -> JJ working copy (14 files, 342 lines)
...
JJ change described and followed by a new empty change (the original change was empty).
...
- **Applied section (default mode only)** -- when the review applied fixes (Stage 5c), list them first, before the severity tables, as `# | File | Fix | Reviewer` followed by a one-line validation outcome (e.g. "suite 214 pass, lint clean") and the **JJ description status** — described as an isolated change and followed by `jj new` when the original change was empty, or left in the existing described change when it was non-empty. A fix spanning multiple files is **one row with one `#`** (e.g. `controller.rb:88 (+test)`) -- never duplicate the number across rows. Flag green-but-unverifiable edits (auth/contract/concurrency) inline in the `Fix` cell, e.g. `(security-posture — verify in diff)`. Applied findings keep their stable `#` and appear only here, not in the severity tables. Omit in `mode:agent` and when nothing was applied
...
When `mode:agent` is active, **do not** emit the markdown table report above. Emit **one parseable JSON object** as the primary response and write the same payload to `review.json` under `<workspace>/.tmp/rocketclaw/ce-code-review/<run-id>/`.
