...
2. **Otherwise, in a JJ repository**, default to the diff between the working copy and its base bookmark (e.g., `jj diff --from main@origin --to @` or against the configured upstream bookmark). This covers the common case of "simplify everything I've added on this feature bookmark before opening a PR." If no upstream/base bookmark can be resolved, fall back to the working-copy change (`jj diff -r @`).
