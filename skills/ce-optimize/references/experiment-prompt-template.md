...
5. Do NOT `jj commit` or `jj describe` (the orchestrator will record the winning diff before merge if this experiment succeeds)
...
- For Codex dispatch: write the filled template to a temp file under the workspace `.tmp` and pipe via stdin (`cat "$SCRATCH_ROOT/optimize-exp-XXXXX.txt" | codex exec --skip-git-repo-check - 2>&1`, with `SCRATCH_ROOT="$(jj workspace root 2>/dev/null || printf '%s' ".")/.tmp"`).
