...
```
echo "BASE:$BASE" && echo "FILES:" && jj diff --from "$BASE" --summary && echo "DIFF:" && jj diff --from "$BASE" && echo "UNTRACKED:" && jj st
```

This path works with any JJ revset/ref — a change ID, `main@origin`, or a bookmark name. Callers reviewing the current checkout should pass explicit `base:` when auto-detection is unnecessary. **Do not combine `base:` with a PR number or bookmark target.** If both are present, stop with an error: "Cannot use `base:` with a PR number or bookmark target — `base:` implies the current checkout is already the correct bookmark/workspace. Pass `base:` alone, or pass the target alone and let scope detection resolve the base."
...
Set `BASE:` to `pr:<number-or-url>` (logical marker — not a JJ change ID). Set `UNTRACKED:` from `jj st` on the **current** checkout (usually empty during PR-remote review).
...
1. The active JJ bookmark equals `headRefName`.
2. The PR is **not** cross-repository (`isCrossRepository` is false).
3. The PR head change is contained in the local checkout: `jj log -r '<headRefOid>..@' --no-graph --limit 1` succeeds. This confirms the working copy actually carries the PR head (allowing unpublished local fixes layered on top) rather than an unrelated same-named bookmark.
...
- **`local-aligned`:** Resolve `<resolved-base-ref>` from `baseRefName` (fetch if needed). Set `BASE=<resolved-base-ref>`, then set `FILES:` from `jj diff --from "$BASE" --summary` and `DIFF:` from `jj diff --from "$BASE"` (includes described and working-copy changes on the PR bookmark). Do **not** call `gh pr diff` or append remote hunks — when unpublished fixes exist, the local tree is canonical. Note in Coverage: `scope: local-aligned (PR; local tree diff)`.
...
1. Best-effort fetch PR head without checkout: `jj git fetch --remote origin --branch <headRefName>` (substitute the PR head ref from metadata).
...
3. Best-effort fetch the PR base without checkout: `jj git fetch --remote origin --branch <baseRefName>`. When it succeeds, set `PR_BASE_REF=<baseRefName>@origin` — a **real JJ base ref** reviewers and validators use for file-level JJ diffs (e.g. `data-migration-reviewer` runs `jj diff --from <PR_BASE_REF> -- db/schema.rb`/`structure.sql`). The `pr:<number-or-url>` logical marker in `BASE:` stays the scope marker; `PR_BASE_REF` is the diffable base. When the fetch fails, omit `PR_BASE_REF` and note in Coverage — schema-drift and other JJ-diff checks fall back to diff hunks only and must **not** assume `main`.
...
If the active JJ bookmark equals `<bookmark>`, use the **standalone (current bookmark)** path below — same tree, explicit bookmark name; do not use remote-only diff.
...
3. Resolve default base bookmark (same logic as standalone). Set `BASE=<base-ref>` and run `jj diff --from "$BASE" --to <bookmark-ref>`.
4. If `<bookmark-ref>` cannot be resolved locally, stop: "Cannot diff bookmark `<bookmark>` without switching. Switch to that workspace/bookmark, pass its open PR URL/number, or review the current bookmark with `base:`."
...
```
echo "BASE:$BASE" && echo "FILES:" && jj diff --from "$BASE" --to <bookmark-ref> --summary && echo "DIFF:" && jj diff --from "$BASE" --to <bookmark-ref> && echo "UNTRACKED:" && jj st
```

**If no argument (standalone on current bookmark):**

Apply the same base-detection logic as bookmark mode above, using the current bookmark (i.e., `gh pr view --json baseRefName,url` with no argument defaults to the current GitHub PR association when available).

If no base can be resolved, **stop**. Do not fall back to `jj diff` alone — a standalone review without the base would only show working-copy changes and silently miss all described work on the bookmark.
...
Using `jj diff --from "$BASE"` diffs the base against the working copy, which includes described changes and current working-copy changes together. JJ has no staging area.

**Untracked file handling:** Always inspect `UNTRACKED:`. New paths that are not part of the reviewed JJ diff are out of scope. When non-empty, list excluded files in Coverage and continue on tracked changes only — never stop or prompt.
...
if [ -z "${DIFF_A:-}" ] || ! jj log -r "$DIFF_A" --no-graph --limit 1 >/dev/null 2>&1 \
   || { [ -n "${DIFF_B:-}" ] && ! jj log -r "$DIFF_B" --no-graph --limit 1 >/dev/null 2>&1; }; then
  ...
  EXEC_LINES=$(jj diff --stat --from "$DIFF_A" ${DIFF_B:+--to "$DIFF_B"} -- '*.rb' '*.py' '*.js' '*.jsx' '*.ts' '*.tsx' '*.go' '*.rs' '*.java' '*.swift' '*.kt' '*.c' '*.cc' '*.cpp' '*.cs' '*.php' '*.ex' '*.exs' '*.scala' 2>/dev/null | awk '{s+=$1+$2} END{print s+0}')
  echo "EXEC_LINES:$EXEC_LINES"
  FILES=$(jj diff --summary --from "$DIFF_A" ${DIFF_B:+--to "$DIFF_B"} 2>/dev/null)
...
**Bookmark mode:** Run `jj log -r "${BASE}..<bookmark-ref>" --no-graph -T 'change_id.short() ++ " " ++ description.first_line() ++ "\n"'` using the resolved base and resolved bookmark ref from Stage 1. Use `<bookmark-ref>` (the resolved `<bookmark>@origin` or fetched ref), not the raw `<bookmark>` argument — a remote-only bookmark may have no matching local ref, so the raw name would fail or read a stale same-named local bookmark.

**Standalone (current bookmark):** Run:

```
echo "BOOKMARK:" && jj bookmark list --tracked 2>/dev/null | awk '/\*/ {print $1; exit}' && echo "CHANGES:" && jj log -r "${BASE}..@" --no-graph -T 'change_id.short() ++ " " ++ description.first_line() ++ "\n"'
```
...
**Only resolve the cache when the working copy is the reviewed tree** — `local-aligned`, standalone, or `base:` scope (Stage 1). In `pr-remote` or `bookmark-remote` scope, **skip Stage 2c entirely**: the helper keys and derives from local `@`, which is *not* the reviewed ref, so its profile would describe the wrong tree (e.g. `main`'s stack while reviewing a PR that changes manifests); reviewers work from the fetched refs/diff as those modes already require.
...
Severity, confidence, and cross-reviewer agreement tell you what to do first and what to flag loudly — they do not gate the decision. There is no deny-list: downside is controlled after the fact (abandon/split + visible diff + the change checkpoint), not by a precondition.
...
**Describe an isolated change when the pre-review tree was clean.** Before applying, note whether the working copy already had changes (`jj st`). The permanence gate is the **push**, not the local JJ change — a local change is private and reversible (`jj abandon` or `jj split`/`jj squash`).
...
- **Dirty before the review:** apply but do **not** create or describe a separate change — the fixes interleave with the user's in-flight work and ride along with the change they were already going to make. The Applied section lists what changed.
...
```json
{
  ...
  "scope": {
    "base": "<JJ base ref, pr:NNN marker, or base: ref>",
    "bookmark": "<current bookmark name>",
    "head_change": "<jj log -r @ change ID>",
    "pr_url": "<url or null>",
    "files_changed": 0
  },
...
After Stage 6, stop. Never push, open PRs, or file tickets from this skill. In default (interactive) mode, Stage 5c has already applied and (on a clean pre-review tree) described the safe fixes in an isolated JJ change; in `mode:agent` the review mutates nothing — the caller (for example `ce-work`) and the user apply fixes, file tickets, or accept residual risk using the report and artifact.
...
{
  "run_id": "<run-id>",
  "bookmark": "<active JJ bookmark at dispatch time>",
  "head_change": "<jj log -r @ change ID at dispatch time>",
  "verdict": "<Ready to merge | Ready with fixes | Not ready>",
