...
2. **Caller carrier** — a structured `<per-skill-key>:<model-alias>` token that an automatic orchestrator may pass in the invocation (LFG passes `plan_model:<alias>` to ce-plan; the analogous `brainstorm_model:<alias>` to ce-brainstorm). Use it when live user intent does not decide the choice. Strip it from the request text and never reconstruct it from product prose. It is honored in pipeline / `disable-model-invocation` runs. The alias must match `^[A-Za-z0-9._-]{1,64}$`; when the resolved harness is `opencode2`, `/` and `#` are also allowed so `provider/modelname#variant` matches. A malformed carrier counts as absent, not guessed.
3. **Config** — otherwise use the per-skill keys: `plan_model` / `plan_harness` for ce-plan, `brainstorm_model` / `brainstorm_harness` for ce-brainstorm. Read them the **same way this skill's Phase 0.0 (output-mode resolution) resolves `plan_output` / `brainstorm_output`**: reuse the workspace root already resolved, else run `jj workspace root`, then apply the ordinary-key rule (`config.local.yaml` then `config.yaml`). Reuse the Phase 0.0 reads if still in hand. Ignore commented (`#`-prefixed) lines. A model alias selects the model; missing / commented / invalid / no file selects none. Valid harness values are `opencode` and `opencode2` as **distinct** branches (never fold `opencode2` into `opencode`); any other value is invalid and continues to the next layer, then none. When the harness is `opencode2`, the model is `provider/modelname#variant` — alias validation allows `/` and `#`.
...
2. **opencode2.** Distinct from `opencode` and from the Claude CLI adapter. When the resolved harness is `opencode2` and `opencode2` is on PATH, run the bundled `scripts/elevation-dispatch.sh` worker as a detached job with harness `opencode2` (see Off-host dispatch). The worker invokes `opencode2 run --standalone --format json --file "$PROMPT_FILE" --model "$MODEL"` with cwd = the handoff directory, without `--auto` (cooperative read-only). Preserve `provider/modelname#variant` as one `--model` token. If the binary is missing, fall through to the remaining adapters.
3. **opencode.** Distinct from `opencode2`. When the resolved harness is `opencode` and `opencode` is on PATH, run the same worker with harness `opencode` — never the `opencode2` binary or flags. If the binary is missing, fall through.
4. **Claude CLI.** Run the bundled `scripts/elevation-dispatch.sh` worker as a detached job with harness `claude` (see Off-host dispatch). Available when `claude` is on PATH. Do not preflight authentication in the host command context: the detached worker's provider-capable call is authoritative, and an authentication failure there follows Recovery.
5. **Inline on the session model.** The always-available fallback.
...
Hand over the working context as **file paths the subagent reads itself**, never a re-narrated prose brief. Resolve the workspace root with `jj workspace root`; when Jujutsu is unavailable, use the current local directory. Create **one private unique per-run handoff directory** under `<resolved-root>/.tmp/rocketclaw/ce-elevation/` and write the prompt-file and every evidence file into *that* directory. On the Claude CLI route the worker grants the elevated model read access to only that one directory (via `--add-dir` on the prompt-file's parent), so the handoff files stay readable while unrelated workspace scratch is not exposed:
...
```
   ...
   PLAN_HARNESS="<claude|opencode|opencode2 — the resolved harness; opencode2 is distinct from opencode>";
   ROCKETCLAW_PEER_HARD_SECS=5400 ROCKETCLAW_ELEVATION_HARD_SECS=5400 ROCKETCLAW_PEER_LOG_MAX_BYTES=52428800 \
     "$PY" "$SKILL_DIR/scripts/peer-job-runner.py" start \
     --skill "$SKILL_NAME" --run-id "<run-id>" --label elevation \
     --result-path "<result-path>" \
     -- bash "$SKILL_DIR/scripts/elevation-dispatch.sh" "<model>" "<prompt-file>" "<result-path>" "$PLAN_HARNESS"
   ```

`ROCKETCLAW_PEER_HARD_SECS` (the outer runner cap) and `ROCKETCLAW_ELEVATION_HARD_SECS` (the worker's own inner cap) are set to the **same** raised backstop well above any legitimate run (R11) — keep them equal so the inner cap never reaps a healthy run before the outer one. `ROCKETCLAW_PEER_LOG_MAX_BYTES` is raised for the streaming route so a healthy high-volume run is not reaped as a failure (R22). Pass the resolved harness as the worker's fourth argument so `plan_harness: opencode2` cannot be folded into `opencode`. `start` returns a job id in under ~2s.
