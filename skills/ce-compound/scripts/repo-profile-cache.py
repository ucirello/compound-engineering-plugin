...
Cache path:
    $(jj workspace root)/.tmp/rocketclaw/repo-profile/<root-id>/<base-id>.json
  root-id = lexicographically-first real root commit reachable from `@`
            (deterministic even for multi-root histories) — the repo identity.
  base-id = the commit ID selected by `jj log -r '@-'` — the stable state.

Validity (HIT) requires ALL of:
  - the cache file exists and parses as JSON,
  - stored `base_id` == the current working-copy parent,
  - stored `profile_schema_version` == PROFILE_SCHEMA_VERSION,
  - no profile-input path is changed in `jj diff -r @ --name-only`,
  - `jj diff --summary -r @` reports no rename or copy
    (the schema-derived superset in `is_profile_input`; JJ snapshots new
    files automatically, so a newly-added manifest or AGENTS.md invalidates).
...
# Any change to one of these — including a newly snapshotted file — must invalidate
...
# an omitted manifest means a changed dependency at an unchanged base serves a stale
...
def revision_ids(revset: str, cwd: str) -> "list[str] | None":
    out = jj(
...
def changed_paths(cwd: str) -> "list[str] | None":
    ...
    Jujutsu snapshots new files automatically, so newly-added inputs are seen.
    ...
    """
    out = jj("diff", "-r", "@", "--name-only", cwd=cwd)
    if out is None:
        return None
    return [line for line in out.splitlines() if line]


def diff_has_rename_or_copy(cwd: str) -> "bool | None":
    """Whether JJ reports any rename/copy, or None when it cannot be proven.

    A rename can remove a profile-input source path while `--name-only` exposes
    only the destination. Conservatively invalidate on every rename/copy.
    """
    out = jj("diff", "--summary", "-r", "@", cwd=cwd)
    if out is None:
        return None
    return any(line.lstrip().startswith(("R ", "C ")) for line in out.splitlines())


def cache_root(workspace: "str | None") -> str:
    # Keep scratch under the JJ workspace root, with the current project root
    # as the local fallback when no workspace can be resolved.
    base = workspace if workspace else os.path.abspath(".")
    return os.path.join(base, ".tmp", "rocketclaw", "repo-profile")


def cache_path(workspace: str, root_id: str, base_id: str) -> str:
    return os.path.join(
        cache_root(workspace), root_id, f"{base_id}.json"
    )


def resolve_keys() -> "tuple[str, str, str] | None":
    """The workspace and revision IDs, or None outside a usable JJ workspace."""
    workspace = workspace_root()
    if not workspace:
        return None
    roots = revision_ids("roots(::@ & ~root())", workspace)
    bases = revision_ids("@-", workspace)
    if not roots or not bases or len(bases) != 1:
        return None
    root_id = sorted(roots)[0]
    base_id = bases[0]
    return workspace, root_id, base_id
...
def do_get() -> int:
    ...
    if keys is None:
        print("NO-CACHE")
        return 0
    workspace, root_id, base_id = keys
    path = cache_path(workspace, root_id, base_id)
    ...
    has_rename_or_copy = diff_has_rename_or_copy(workspace)
    # Could not determine cleanliness, or a profile input changed/was added.
    if (
        changed is None
        or has_rename_or_copy is None
        or has_rename_or_copy
        or any(is_profile_input(p) for p in changed)
    ):
        return miss()
...
def do_put(profile_file: str) -> int:
    ...
    # Do not cache a profile derived from changed profile inputs: it would be
    # stored under the working-copy parent's key and could be served after those
    # edits are discarded, when the same parent no longer has those inputs.
    changed = changed_paths(workspace)
    has_rename_or_copy = diff_has_rename_or_copy(workspace)
    if (
        changed is None
        ...
        sys.stderr.write(
            "repo-profile-cache: profile inputs are changed; not caching\n"
        )
        print("NO-CACHE")
        return 0
    ...
    try:
        ...
        # Write beside the destination, then replace atomically so a concurrent
