...
Classify the request into exactly one input shape — concept, diff, idea, or work-recap window — before any grounding runs. Parse by reasoning over the user's prompt; do not depend on argument-token substitution mechanics, which vary by runtime.
...
- An unrecognized `<word>:<word>` token (for example, a repository-local change-description prefix appearing inside a topic) is not a flag — it passes through verbatim as request text.
...
- **Diff** — the request names a resolvable Jujutsu change, commit, bookmark, revset, or hosted PR: an ID, `@`, "the last change", "what you just did", "this change".
