...
- Write transient output beneath `<workspace-root>/.tmp/rocketclaw/`, falling back to `<current-directory>/.tmp/rocketclaw/` outside a Jujutsu workspace, or under `docs/` when the user wants it kept; announce the absolute path. Do **not** introduce a new mandated `docs/` location — that store is deferred.
