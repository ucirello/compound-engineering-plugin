...
Environment overrides (defaults in parentheses):
  CE_PEER_JOBS_ROOT         base dir (<jj workspace root>/.tmp/peer-jobs,
                            or <cwd>/.tmp/peer-jobs when not a jj repo)
...
POSIX path is behaviorally unchanged:
  ...
  jobs root defaults under the workspace `.tmp/peer-jobs` (cwd/.tmp when
            jj workspace root is unavailable), owner-private.
...
def _private_root_usable(path: str) -> bool:
    ...
    Creation is the probe: a denied write under the workspace `.tmp` refuses the
    mkdir, and a pre-existing root that is not writable fails the access check.
    """
    try:
        os.mkdir(path, 0o700)
...
def _workspace_root() -> str:
    """Public `jj workspace root`, or cwd when jj is unavailable or not a repo."""
    try:
        proc = subprocess.run(
            ["jj", "workspace", "root"],
            cwd=os.getcwd(),
...
def _derived_jobs_root() -> str:
    return os.path.join(_workspace_root(), ".tmp", "peer-jobs")
...
def jobs_root_base() -> str:
    ...
    try:
        ...
        raise RunnerError(f"cannot create workspace .tmp at {ws_tmp}: {exc}")
    root = os.path.abspath(os.path.join(ws_tmp, "peer-jobs"))
    if not _private_root_usable(root):
        raise RunnerError(f"cannot create a writable private jobs root at {root}")
    return root


def candidate_jobs_root_bases() -> list:
    """Every root an existing job may live under: the configured root, or the
    workspace `.tmp/peer-jobs` derived from `jj workspace root` (else cwd).
    ...
    if configured:
        return [os.path.abspath(configured)]
    return [os.path.abspath(_derived_jobs_root())]
...
def ensure_owned_dirs(base: str, path: str) -> None:
    ...
    if IS_WINDOWS:
        ...
        default_root = os.path.abspath(_derived_jobs_root())
