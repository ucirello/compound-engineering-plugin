...
4. PR metadata: title, body, URL, number, and head object ID when reviewing a PR (empty string otherwise). Passed in a `<pr-context>` block so reviewers can verify code against stated intent
5. Review context: intent summary, file list, diff, scope mode (`local-aligned` | `pr-remote` | `bookmark-remote`), and the reviewed PR object ID or bookmark revision when set
...
Read-only here means **non-mutating**, not "no shell access." Reviewer sub-agents may use non-mutating `jj` inspection and `gh` provider operations. When a claim depends on line history, attach one concise provenance line from targeted `jj file annotate` / `jj log`, additional to the quote-the-line gate and omitted when the diff alone is sufficient. In **`pr-remote`**, inspect changed files through `gh api` at the reviewed object ID or supplied hunks. In **`bookmark-remote`**, use `jj file show`, `jj file annotate`, and `jj log` at the reviewed revision. Never inspect unrelated workspace paths or mutate revisions, descriptions, bookmarks, remotes, or provider state.
...
**Generic conditional local prompt assets** (`agent-native-reviewer`, `learnings-researcher`) are dispatched only when selected by Stage 3, through the same deterministic foreground batch dispatch as the structured personas. Read their prompt files from `references/personas/`, then give them the same review context bundle the personas receive: entry mode, PR metadata, intent summary, review base revision, `BASE:` marker, file list, diff, and tracked-file exclusions. Do not invoke them with a generic "review this" prompt. Their output is unstructured and synthesized separately in Stage 6.
