...
- Any durable decisions made during the dialogue can flow naturally to
  downstream artifacts (`ce-plan`, a Jujutsu change description,
  `docs/solutions/`)
...
decision flows to `ce-plan` (or directly to a Jujutsu change description, or to
...
- **`title`** — the artifact's descriptive name with a ` - Plan` suffix,
  ...
  it.
- **`type`** — the artifact's classification.
...
progress. No unified plan artifact carries mutable progress state; whether
work shipped is derived from Jujutsu history, not stored in the doc. Do not
introduce one.
...
- **No process exhaust.** No "captured at Phase X" notes, no `## Next Steps`
  pointing to ce-plan, no italic provenance lines. Delivery-process metadata
  belongs in Jujutsu change descriptions and tool output, not the artifact.
