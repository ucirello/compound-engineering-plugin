...
After all actions are executed and the report is generated, handle describing the JJ changes. Skip this phase if no files were modified (all Keep, or all writes failed).

### Detect JJ context

Before offering options, check:
1. Whether the current change is on the default/trunk bookmark vs a feature bookmark/change
2. Whether the working copy has other edits beyond what compound-refresh modified
...
|---------|---------------|
| On trunk/default bookmark | Create a bookmark named for what was refreshed (e.g., `docs/refresh-auth-and-ci-learnings`), describe the change, attempt to open a PR. If PR creation fails, report the bookmark name. |
| On a feature bookmark/change | Describe as a separate JJ change on the current line of work |
| JJ operations fail | Include the recommended `jj` commands in the report and continue |

Keep only the files that compound-refresh modified in the described change — not other dirty files in the working copy.

### Interactive mode

First, run `jj log -r @ --no-graph --template 'bookmarks.join(", ") ++ if(bookmarks.is_empty(), change_id.short(), "")'` and compare `@` to `trunk()` to determine the current bookmark/change context. Then present the correct options based on the result. Keep only compound-refresh files in the described change regardless of which option the user picks.

**If the current change is on trunk/default:**
...
2. Describe directly on `{current bookmark/change}`
3. Don't describe — I'll handle it

**If the current change is on a feature bookmark/change, clean working copy:**

1. Describe `{current bookmark/change}` as a separate JJ change (recommended)
2. Create a separate bookmark and describe there
3. Don't describe

**If the current change is on a feature bookmark/change, dirty working copy (other edits):**

1. Describe only the compound-refresh changes on `{current bookmark/change}` (separate unrelated edits first so other files stay untouched)
2. Don't describe
...
- Follows the repo's existing message conventions (check recent `jj log` output for style)
