...
Use `jj git fetch` and `jj git push` for Git remote synchronization. Keep every repository read and mutation in JJ. In a non-colocated repository, resolve the backing Git directory with `jj git root` for `gh` when required and use `jj git export` before a provider consumer needs updated refs. On Windows, use the active shell's native environment-assignment syntax; Git Bash is supported, but do not emit Git Bash syntax into another shell.
