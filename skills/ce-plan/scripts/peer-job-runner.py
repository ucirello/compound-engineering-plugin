...
the JJ workspace `.tmp`. Every read of job state opens the file first (no-follow) and
...
POSIX path is behaviorally unchanged:
  ...
  jobs root defaults under the JJ workspace `.tmp` (then cwd `.tmp` when
            not a JJ repo), owner-private.
...
# Jobs root is derived lazily from `jj workspace root` / cwd `.tmp`.
# Ownership still uses the current-user SID on Windows (see below).
...
def _default_jobs_root() -> str:
    """JJ workspace `.tmp`, or cwd `.tmp` when not a JJ repo."""
    ws = _jj_workspace_root()
    if ws:
        return os.path.join(ws, ".tmp")
    return os.path.abspath(os.path.join(".", ".tmp"))
...
def candidate_jobs_root_bases() -> list:
    """Every root an existing job may live under (the configured override, or
    the single workspace `.tmp` default).
    """
