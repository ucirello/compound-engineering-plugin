...
1. **Create CONCEPTS.md (build the concept map)** — seed the repo-wide concept map and finalize it as a described JJ change; skip only the docs/solutions classification phases (Phases 0–4). Read `references/concepts-vocabulary.md` and follow its **Seed goal** and **Scope of a seed** (repo-wide) rules: seed the project's core domain nouns from the declared domain model (schema, core types, primary models, top-level domain docs), each meeting the qualifying bar, the codebase setting the count. Write the preamble (see Phase 4.5), cluster per the organization rules, and run the Discoverability Check so the root agent-instructions file surfaces the new file. Then **enter Phase 5 (Describe and Finalize Changes)** for the new `CONCEPTS.md`, any instruction-file edit, and any bookmark/PR flow — do not leave the bootstrap undescribed.
...
- Ask questions **one at a time** using the platform's blocking question tool. Fall back to numbered options in plain text only when no blocking tool exists in the harness or the call errors. Never silently skip the question
...
10. **Delete, don't archive.** There is no `_archived/` directory. When a doc is no longer useful, delete it. JJ history preserves every deleted file — that is the archive. A dedicated archive directory creates problems: archived docs accumulate, pollute search results, and nobody reads them. If someone needs a deleted doc, `jj log -r :: -- docs/solutions/` locates changes touching that tree; inspect candidates with `jj show <revision>`.
...
- **Loaded memory** — does any user-memory block in your current context contain entries in the same problem domain? Scan that block directly. If no such block is present, skip this dimension. A memory note describing a different approach than what the learning recommends is a supplementary drift signal.
...
- Add context to the evidence report ("(loaded memory) notes suggest approach X may have changed since this learning was written")
...
>
> Also scan any user-memory block in your current context. Check for notes related to the learning's problem domain. Report any memory-sourced drift signals separately from codebase-sourced evidence, tagged with "(loaded memory)" in the evidence section. If no such block is present, skip this check.
...
- **Decorative** — principle stated inline, citation is a "see also" pointer. Delete is fine; clean up citations in the same change.
...
- What evidence was found -- tag any memory-sourced findings with "(loaded memory)" to distinguish them from codebase-sourced evidence
...
## Phase 5: Describe and Finalize Changes

After all actions are executed and the report is generated, handle describing and finalizing the JJ change. Skip this phase if no files were modified (all Keep, or all writes failed).

### Detect JJ context

Before offering options, check:
1. Which nearest local bookmarks identify the current stack (`jj bookmark list -r 'heads(::@ & bookmarks())'`) and which bookmark the project's active instructions and conventions identify as the default
2. Whether `jj status` shows other working-copy changes beyond what this refresh modified
3. The project's active instructions and conventions plus recent messages using the repository's own `git log` syntax

If unrelated changes share `@`, first run `jj commit <refresh-filesets>...`; the finalized refresh revision is then `@-`, and unrelated changes remain in the new `@`. Otherwise, the refresh revision is `@` until `jj new` starts the next change. Use `<refresh-revision>` below for that resolved revision.
...
|---------|---------------|
| The nearest bookmark is the default bookmark | Describe `<refresh-revision>`, create a specific new bookmark there, run `jj new` if still editing that revision, push with `jj git push --bookmark <bookmark>`, and attempt to open a PR with `gh`. If PR creation fails, report the bookmark name. |
| The nearest bookmark is non-default | Describe `<refresh-revision>`, run `jj new` if still editing that revision, and move or create the intended bookmark there only when required by the repository's workflow |
| No nearest bookmark | Derive the intended base and bookmark behavior from the project's active instructions and conventions; do not guess a bookmark model |
| JJ operations fail | Include the recommended JJ commands in the report and continue |

Finalize only files this refresh modified: use `jj commit <filesets>...` when unrelated changes share `@`, which leaves unselected changes in the new working-copy change. Never absorb unrelated changes.

### Interactive mode

First, run `jj bookmark list -r 'heads(::@ & bookmarks())'` and identify the default bookmark from the project's active instructions and conventions. Then present the correct options based on the result. Finalize only this refresh's files regardless of which option the user picks.

**If the nearest bookmark is the repo's default bookmark:**

1. Describe `<refresh-revision>`, create a specific bookmark there, start a new change if needed, push with `jj git push --bookmark <bookmark>`, and open a PR with `gh` (recommended)
2. Describe the change directly on `{default bookmark}`, then start a new change
3. Leave the change undescribed — I'll handle it

**If the nearest bookmark is non-default and `@` has no unrelated changes:**

1. Describe `<refresh-revision>`, start a new change if needed, and advance `{current bookmark}` to the described revision if the repository workflow requires it (recommended)
2. Create a separate bookmark for the described change
3. Leave the change undescribed

**If `@` contains unrelated changes:**

1. Run `jj commit <refresh-filesets>...` so only the refresh changes are finalized and unrelated changes remain in the new `@`
2. Leave the change undescribed

### Change description

Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards.

The project's active instructions and conventions plus the repository's preferred `git log` syntax always win. Use only compatible Go guidance: imperative clarity, a concise subject, a body that explains why when useful, and repository-preferred wrapping. Treat the refresh's semantic result as composition context, not fixed subject or body requirements. Do not impose fixed messages, prefixes, types, scopes, subjects, bodies, templates, or examples.
...
After the refresh report is generated, check whether the project's instruction files would lead an agent to discover and search `docs/solutions/` before starting work in a documented area. This runs every time — the knowledge store only compounds value when agents can find it. If this check produces edits, include them in the Phase 5 JJ change or a follow-up change — see step 6 below.

1. Identify the project's root-level agent-instruction files. Read them and determine which holds the substantive content — one may just be a shim that includes another. The substantive file is the assessment and edit target; ignore shims. If none exists, skip this check entirely.
...
6. **Update the described change or create a follow-up change when the check produces edits.** Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. The project's active instructions and conventions plus the repository's preferred `git log` syntax always win. Use only compatible Go guidance: imperative clarity, a concise subject, a body that explains why when useful, and repository-preferred wrapping. Treat the discoverability edits and their rationale as composition context, not fixed subject or body requirements. Do not impose fixed messages, prefixes, types, scopes, subjects, bodies, templates, or examples. If step 4 or step 5 edited an instruction file and the refresh revision is still mutable and has not been pushed, move the new file changes into it with `jj squash <instruction-fileset> --into <refresh-revision>` and update its description with `jj describe <refresh-revision>` only when the existing description no longer fits. Otherwise, describe the current follow-up change with runtime-derived wording, run `jj new`, advance the relevant bookmark to the follow-up revision, and push it with `jj git push --bookmark <bookmark>` so the open PR includes the edit. If the user chose to leave changes undescribed in Phase 5, leave these edits in the working-copy change too.
