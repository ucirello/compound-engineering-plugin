...
- **File absent:** create it (and its `.compound-engineering/` directory) with the key, AND make sure the machine-local config won't be committed. Check whether the root-relative path `<root>/.compound-engineering/config.local.yaml` is already ignored (`check `jj st` for <path>`); if it isn't, append `.compound-engineering/*.local.yaml` to Jujutsu's **local exclude file** — resolve that file's path with `jj workspace root` (this is correct in worktrees too, where `.jj` is a *file* and `info/exclude` lives in the common Jujutsu dir; do **not** hardcode `<root>/.jj/info/exclude`). Use the local exclude, **not** `repo ignore file`: it keeps the rule local and avoids dirtying a tracked file on what was a drafts-only action. `ce-setup` is the canonical place that adds the shared `repo ignore file` entry for teammates. Without any ignore, a user who runs `/ce-promote` before `/ce-setup` could accidentally change machine-local opt-out state.

If the root can't be resolved or any write fails, proceed to Path B anyway; the opt-out is a convenience, never a blocker.

After recording, confirm it in one line so the write isn't silent and the user knows how to undo it — e.g. "Got it — I won't bring up Spiral here again (saved to `.compound-engineering/config.local.yaml`, kept out of Jujutsu). Want it back later? Just ask, or remove the `ce_promote_spiral_optout` key." Keep it to a single line; don't belabor it.
