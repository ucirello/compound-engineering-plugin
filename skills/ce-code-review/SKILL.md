...
Do **not** move to the PR bookmark. Scope comes from GitHub read APIs plus optional local alignment when @ already matches the PR head bookmark.
...
Set `BASE:` to `pr:<number-or-url>` (logical marker — not a JJ commit ID). Set `UNTRACKED:` from `jj file list` on the **current** working copy (usually empty during PR-remote review).
...
1. Best-effort fetch PR head without moving the working copy: `jj git fetch --remote origin <headRefName>:refs/review/pr-<number>-head` (substitute PR number from metadata).
...
3. Best-effort fetch the PR base without moving the working copy: `jj git fetch --remote origin <baseRefName>`. When it succeeds, resolve a concrete ref with `jj log -r FETCH_HEAD --no-graph -T commit_id` and set `PR_BASE_REF` to that SHA — a **real base ref** reviewers and validators use for file-level JJ diffs (e.g. `data-migration-reviewer` runs `jj diff --from <PR_BASE_REF> --to <PR_HEAD_REF> -- db/schema.rb`/`structure.sql`). The `pr:<number-or-url>` logical marker in `BASE:` stays the scope marker; `PR_BASE_REF` is the diffable base. When the fetch fails, omit `PR_BASE_REF` and note in Coverage — schema-drift and other JJ diff checks fall back to diff hunks only and must **not** assume `main`.
...
Substitute the provided bookmark name as `<bookmark>`. Do **not** `jj edit` or otherwise move to `<bookmark>`.

If `jj log -r @ --no-graph -T 'bookmarks.join(" ")'` equals `<bookmark>`, use the **standalone (current bookmark)** path below — same tree, explicit bookmark name; do not use remote-only diff.

Otherwise diff the remote/local ref **without moving the working copy**:
...
2. Else resolve `<bookmark>` as `origin/<bookmark>` or `<bookmark>` after `jj git fetch --remote origin <bookmark>` when needed.
...
```
echo "BASE:$BASE" && echo "FILES:" && jj diff --from "$BASE" --to <bookmark-ref> --name-only && echo "DIFF:" && jj diff --from "$BASE" --to <bookmark-ref> --context 10 && echo "UNTRACKED:" && jj file list
```

**If no argument (standalone on current bookmark):**

Apply the same base-detection logic as bookmark mode above, using the current bookmark (i.e., `gh pr view --json baseRefName,url` with no argument defaults to the current PR context).

If no base can be resolved, **stop**. Do not fall back to `jj diff` — a standalone review without the base would only show working-copy changes and silently miss all committed work on the bookmark.
...
Using `jj diff --from "$BASE" --to @` diffs the base against the working-copy change.

**Untracked file handling:** Always inspect `UNTRACKED:`. Untracked paths are out of scope. When non-empty, list excluded files in Coverage and continue on tracked changes only — never stop or prompt.
...
if [ -z "${DIFF_A:-}" ] || ! jj log -r "${DIFF_A}^{commit}" >/dev/null 2>&1 \
  ...
  # globs single-quoted and inline so JJ receives them literally (an unquoted or
...
5. **Use mid-tier model for validators.** Same platform model class the mid-tier persona reviewers use; omit the override if the model name is unknown. Validators are read-only — same constraints as persona reviewers. They may use non-mutating inspection commands (Read, Grep, Glob, JJ annotation, gh).
