...
1. Run the analyzer in workspace-local scratch so nothing pollutes the product tree (`SKILL_DIR` is the directory containing the `ce-riffrec-feedback-analysis` SKILL.md; set it in the same command because shell state does not persist between Bash calls). Resolve the Jujutsu workspace with `jj workspace root`; if that fails, use `.tmp/rocketclaw/` under the current project directory. Never use `/tmp`, `TMPDIR`, `mktemp`, `tempfile`, or another global temporary location.
