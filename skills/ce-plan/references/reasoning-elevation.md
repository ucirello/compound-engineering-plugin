...
Resolve the per-skill **model choice immediately before adapter selection**, so the decision reflects the current conversation rather than an intake snapshot. The value is a model alias (e.g. `fable`, `opus`) **or** a `provider/modelname#variant` id, not a boolean. Parse `provider/modelname#variant` and preserve provider, model, and variant; do not drop the variant or rewrite the provider.

Also resolve **`plan_harness`** (ce-plan) / **`brainstorm_harness`** (ce-brainstorm) from the same config layers. `plan_harness: opencode2` is its **own branch**, not compatible with `opencode`. Do not reuse V1 opencode flags (`--dir`) or the `opencode` binary from an opencode2 selection.
...
2. **Caller carrier** — when live user intent does not decide the choice, an automatic orchestrator may pass a structured `<per-skill-key>:<model>` carrier (LFG passes `plan_model:<alias-or-id>` to ce-plan; the analogous `brainstorm_model:<alias-or-id>` to ce-brainstorm). Strip it from the request text and never reconstruct it from product prose. It is honored in pipeline / `disable-model-invocation` runs. Accept either a model alias matching `^[A-Za-z0-9._-]{1,64}$` or a `provider/modelname` / `provider/modelname#variant` id; a malformed carrier is absent, not guessed.
3. **Config** — otherwise use the per-skill keys: `plan_model` and `plan_harness` for ce-plan, `brainstorm_model` and `brainstorm_harness` for ce-brainstorm. Read them the **same way this skill's Phase 0.0 resolves `plan_output` / `brainstorm_output`**: reuse the repo root already resolved, else run `jj workspace root`, then apply the ordinary-key rule (`config.local.yaml` then `config.yaml`). Reuse the Phase 0.0 reads if still in hand. Ignore commented (`#`-prefixed) lines. A model alias or `provider/modelname#variant` selects the model; `opencode2` selects the opencode2 harness branch; missing / commented / invalid / no file selects none.
...
2. **opencode2 CLI** — only when `plan_harness` (or `brainstorm_harness`) is `opencode2`. Run the bundled `scripts/elevation-dispatch.sh` worker as a detached job with harness `opencode2` (see Off-host dispatch). Pass `--model` as the full `provider/modelname#variant` (or alias) unchanged. Available when `opencode2` is on PATH. This branch is **not** V1 `opencode`: do not invoke the `opencode` binary, do not pass `--dir`, and do not fall back to V1 opencode from an opencode2 selection. If this route is unavailable, skip the Claude CLI adapter and degrade to inline.
3. **Claude CLI** — when the resolved harness is not `opencode2`. Run the bundled `scripts/elevation-dispatch.sh` worker as a detached job (see Off-host dispatch). Available when `claude` is on PATH. Do not preflight authentication in the host command context: the detached worker's provider-capable call is authoritative, and an authentication failure there follows Recovery.
...
Hand over the working context as **file paths the subagent reads itself**, never a re-narrated prose brief. Create **one private per-run handoff directory** under the workspace `.tmp` (`WS="$(jj workspace root 2>/dev/null || pwd)"; mkdir -p "$WS/.tmp/ce-plan"; mktemp -d "$WS/.tmp/ce-plan/ce-elevation-XXXXXX"`) and write the prompt-file and every evidence file into *that* directory. On the Claude CLI route the worker grants the elevated model read access to only that one directory (via `--add-dir` on the prompt-file's parent), so the handoff files stay readable while the rest of the workspace `.tmp` tree — other same-run scratch — is not exposed:
...
```
   ...
   CE_PEER_HARD_SECS=5400 CE_ELEVATION_HARD_SECS=5400 CE_PEER_LOG_MAX_BYTES=52428800 \
     ...
     -- bash "$SKILL_DIR/scripts/elevation-dispatch.sh" "<model>" "<prompt-file>" "<result-path>" "<harness-or-empty>"
   ```

Pass `<harness-or-empty>` as `opencode2` when `plan_harness` (or `brainstorm_harness`) resolved to `opencode2`; otherwise omit it (Claude CLI). Do not pass `opencode`.
