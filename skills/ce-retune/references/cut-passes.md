...
- An agent needs to run the suite or the harness to check its own edit. Concurrent runs in one tree race on scratch and working-copy state.
- Agents create or rewrite changes or move bookmarks. A shared JJ working-copy change cannot isolate parallel agents' edits; JJ has no staging index.
...
Here `git log` means history inspected using `jj log` for this JJ workflow; never execute Git to inspect history or compose messages. Read the linked guidance before composing a message; the runtime project's instructions and visible history take precedence over Go-specific syntax. Use JJ change descriptions and commits, preserving each pass's hypothesis, affected files, and relevant issue or PR references.
