...
!`GIT_DIR="$(jj git root)" git log -n 10 --format=full`
...
Run the block in one shell invocation; its exported `GIT_DIR` makes the allowed runtime `git log` and `gh` calls use JJ's backing Git repository.
...
After resolving the feature bookmark name, detect its PR explicitly; never infer it from `@` or another implicit revision:
...
For JJ change descriptions, determine repository-local syntax at runtime from the project's active runtime-local instructions and actual `git log`; those sources always win. Do not impose a fixed prefix, type, scope, subject/body shape, template, or example. Keep `jj log` for revision and change workflow only. If ownership history is needed to understand the convention around a changed file, use `jj file annotate <path>`. Match PR titles to repository conventions independently.

## Step 3: Describe changes and push

If the default bookmark is the closest bookmark, feature-bookmark creation needs to handle a stale local `<base>`, unpublished changes on local `<base>`, and working-copy changes that conflict with the fresh remote base. Read `references/bookmark-creation.md` and follow its decision flow before continuing, passing the configured remote that matches the GitHub base repository as `<base-remote>`; do not assume `origin`.

Scan changed files for naturally distinct concerns. If they clearly group into separate logical changes, create separate JJ changes (2-3 max). Group with JJ filesets at file level only. When ambiguous, one change is fine. Operate directly on JJ's automatically snapshotted working-copy change.

Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards.

The project's active runtime-local instructions and actual `git log` syntax win. Do not impose a fixed prefix, type, scope, subject/body shape, template, or example. Compose a description for each group, then finish each selected fileset directly while leaving other paths in the working-copy change:

```bash
DESCRIPTION='<composed message>'
jj commit <fileset> -m "$DESCRIPTION"
```
...
**Concept teaching gate** before composition. Use the pre-resolved workspace root from Context (if it is empty or shows a literal command string, resolve it at runtime with `jj workspace root`) and read `<workspace-root>/.rocketclaw/config.local.yaml` with the native file-read tool. Only an **active (non-commented)** `pr_teaching_section:` key counts — lines starting with `#` are YAML comments, and commented examples do not activate a key. The gate is off only when the active value is exactly `false`; a missing file, missing key, or any other value means the default: **on**. The same read resolves `pr_teaching_archive:` — on only when the active value is exactly `true`, otherwise **off** — and a per-run `archive:on|off` token overrides the archive key for this invocation.
...
1. Check the candidate fileset against JJ's repository ignore rules. If it is ignored, print a one-line warning and skip archival entirely, writing nothing; do not override ignore rules.
2. Write the file (create the directory if needed) with YAML frontmatter `title`, `date`, `input_shape: concept`, `subject`, and the teaching content. If the file already exists from a prior run, overwrite it. Confirm the path appears in `jj file list <fileset>`; if it does not, remove the new file, warn, and skip archival.
...
4. The project's active runtime-local instructions and actual `git log` syntax win. Do not impose a fixed prefix, type, scope, subject/body shape, template, or example. Populate `DESCRIPTION` with the message composed under step 3, run `jj commit <fileset> -m "$DESCRIPTION"`, move the feature bookmark to `@-`, and run `jj git push --bookmark <bookmark> --remote <head-remote>`. If there is no change to describe, the doc is already present from a prior run — keep the link and continue.
...
The body **must** be written under `$(jj workspace root)/.tmp/rocketclaw/commit-push-pr/` and passed via `--body-file <path>`. If there is no JJ workspace, use `$PWD/.tmp/rocketclaw/commit-push-pr/`. Never use OS-global scratch storage, `--body-file -`, stdin pipes, heredoc-to-stdin, or `--body "$(cat ...)"` — wrappers and stdin handling can silently produce an empty PR body while `gh` still exits 0 and returns a URL.

```bash
if WORKSPACE_ROOT="$(jj workspace root 2>/dev/null)"; then BODY_DIR="$WORKSPACE_ROOT/.tmp/rocketclaw/commit-push-pr"; else BODY_DIR="$PWD/.tmp/rocketclaw/commit-push-pr"; fi && mkdir -p "$BODY_DIR" && BODY_FILE="$BODY_DIR/pr-body-$$.md" && cat > "$BODY_FILE" <<'__PR_BODY_END__'
