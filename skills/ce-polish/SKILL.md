...
Start the dev server in the background and log output under `$(jj workspace root)/.tmp/rocketclaw/ce-polish/`, creating that directory if needed. Outside a JJ workspace, use the current directory's `.tmp/rocketclaw/ce-polish/` instead. Probe `http://localhost:<port>` for up to 30 seconds. If it doesn't come up, show the last 20 lines of the log and ask the user what to do.
