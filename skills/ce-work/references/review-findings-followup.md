...
`ce-code-review` is invoked here with `mode:agent`, so it is **review-only** in this context — it reports findings and writes artifacts and does not mutate the workspace, describe a change, move a bookmark, push, or file tickets. **The caller owns apply/fix policy.** Standalone review is also report-only unless local apply was explicitly authorized.
...
- `base:` — when the diff base is already resolved in the current workspace; omit when reviewing a PR number/URL or a standalone current change.
...
Default to applying every actionable finding. Applying is a reversible edit to a tracked tree; diffs are reviewed before the JJ change is described and tests run after — so leaving a clear, reversible fix unapplied "to be safe" is the failure mode, not the safe choice. Bias to act:
...
There is no precondition safety checklist and no deny-list — a code-review fix is a reversible edit, so downside is controlled after the fact (diff review + tests + the described-change checkpoint), not by gating the apply.
...
**Orchestrator owns:** parse review output → **eligibility filter on JSON fields only** → build batches → dispatch fix subagents → review diffs → tests → describe the JJ change → Residual Work Gate.
...
- Shared-directory fallback: do not describe changes or move bookmarks — return which `#` were applied or skipped and which files changed

**After each wave:** orchestrator reviews diffs (scope = assigned `#` only), runs tests (`requires_verification: true` on any applied finding → at least targeted tests; multi-file → broader suite), then describes the JJ change unless workspace-isolated subagents return transport changes per Phase 1. Repeat until all batches complete.

Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards.
Runtime project instructions and observed history syntax take precedence. Apply only compatible Go message-quality guidance; never impose a fixed prefix, type, scope, template, or example message.
