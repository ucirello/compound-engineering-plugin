...
argument-hint: "[blank to simplify current change-stack changes, or describe what to simplify]"
---
...
2. **Otherwise, in a Jujutsu repository**, default to the total diff for the current change stack since its base bookmark (e.g., `jj diff -r 'trunk()..@'`). This covers the common case of "simplify everything I've added in this change stack before opening a PR." If no base bookmark resolves, fall back to the current working-copy change (`jj diff`).
3. **Outside a Jujutsu repository or when no diff is available**, review the most recently modified files mentioned by the user or edited earlier in this conversation.
