...
- **opencode2:** distinct from V1 `opencode`. Cwd at the repo root (parent CLI `[<directory>]`; no `--dir`). `--standalone` private server; `--format json`; `--file` for the prompt; `--model provider/model#variant` when a model is bound. Do not reuse V1 flags or `OPENCODE_DISABLE_PROJECT_CONFIG`.
