...
**Read config.** Resolve the workspace root at runtime with `jj workspace root`. If it succeeds, read `$(jj workspace root)/.tmp/rocketclaw/config.local.yaml` with the native file-read tool; otherwise use the current directory's local `.tmp/rocketclaw/config.local.yaml`. If the file does not exist, fall through to the defaults below.
...
**Token-parsing convention:** only literal-prefix flag tokens (`output:`, `mode:`, `delegate:` where applicable) are consumed and stripped. Other colon-delimited tokens in a feature description pass through verbatim.
...
Write to `docs/plans/YYYY-MM-DD-NNN-<type>-<topic>-plan.<md|html>` — extension follows `OUTPUT_FORMAT`. Include `artifact_contract: unified-plan/v1`, `artifact_readiness: requirements-only`, and `product_contract_source: brainstorm`. Title is `<Name> - Plan` (matching the H1; no change-description prefix). Keep the doc light and standalone-readable: a Goal Capsule (objective, product authority, open blockers) and the Product Contract. Do **not** emit a Goal Launch Block or Reader Index. See `references/brainstorm-sections.md` — which owns the artifact content rules, including repo-relative file paths inside the doc. When confirming in chat, report the written artifact with its absolute path so the reference is clickable.
