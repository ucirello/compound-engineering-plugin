...
**OpenCode V2 (`opencode`):** use these effective layers and invoke `ce-noslop` through the native skill mechanism. Preserve explicit subagent or alternative-harness settings for any delegation the callee owns. Other harnesses use their own callable skill mechanism; this composition step does not select a worker model.
