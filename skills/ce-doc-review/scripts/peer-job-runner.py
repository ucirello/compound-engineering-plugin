...
Environment overrides (defaults in parentheses):
  ...
  ROCKETCLAW_PEER_HARD_SECS         hard cap on worker wall clock (630)
  ROCKETCLAW_PEER_LOG_MAX_BYTES     out.log byte cap (10485760)
  ROCKETCLAW_PEER_RESULT_MAX_BYTES  result byte cap (5242880)
...
the current JJ workspace's `.tmp/rocketclaw` tree, with the current directory as
the fallback workspace. Every read of job state opens the file first (no-follow) and
...
ROCKETCLAW_PEER_IDLE_SECS, ROCKETCLAW_PEER_HARD_SECS,
ROCKETCLAW_PEER_LOG_MAX_BYTES, ROCKETCLAW_PEER_RESULT_MAX_BYTES,
ROCKETCLAW_PEER_POLL_SECS, ROCKETCLAW_PEER_GRACE_SECS (defaults above).
"""
...
def workspace_local_tmp(path: str, label: str) -> str:
    scratch = os.path.join(workspace_root(), ".tmp")
    candidate = os.path.abspath(path)
    try:
        inside = os.path.commonpath((scratch, candidate)) == scratch
    except ValueError:
        inside = False
    if not inside:
        raise RunnerError(f"{label} must be under {scratch}")
    return candidate


def jobs_root_base() -> str:
    configured = os.environ.get("ROCKETCLAW_PEER_JOBS_ROOT")
    root = configured or os.path.join(workspace_root(), ".tmp", "rocketclaw")
    return workspace_local_tmp(root, "ROCKETCLAW_PEER_JOBS_ROOT")


def skill_runs_root(skill: str) -> str:
    if skill == "ce-work" and os.environ.get("ROCKETCLAW_WORK_RUNS_ROOT"):
        return workspace_local_tmp(
            os.environ["ROCKETCLAW_WORK_RUNS_ROOT"], "ROCKETCLAW_WORK_RUNS_ROOT"
        )
...
def cfg(skill=None) -> dict:
    return {
        ...
        "hard": _env_num("ROCKETCLAW_PEER_HARD_SECS", 630.0, float),
...
def write_atomic(path: str, data: bytes) -> None:
    ...
    for _ in range(CLAIM_ATTEMPTS):
        candidate = os.path.join(
            os.path.dirname(path), f".atomic-{os.getpid()}-{os.urandom(4).hex()}"
        )
        try:
            fd = os.open(
                candidate,
            ...
            continue
    if fd is None or staging_path is None:
        raise RunnerError("could not reserve an atomic publish path")
    try:
...
def _require_posix_detach() -> None:
    ...
    workspace-root discovery, so native Windows fails with this clear message
    instead of an unrelated error or an AttributeError mid-detach."""
    if not hasattr(os, "fork") or not hasattr(os, "setsid"):
        ...
        raise RunnerError(
            ...
            "started. On native Windows, run the active agent / this skill under "
            "WSL or use a Windows-native detached-job runner."
        )
...
def cmd_start(args, worker_argv) -> int:
    ...
    base_anchor = os.path.dirname(base)
    if not os.path.exists(base_anchor):
        base_anchor = os.path.dirname(base_anchor)
    ensure_owned_dirs(base_anchor, jobs_root)
    _check_owned_dir(base)
    os.chmod(base, 0o700)
    _check_owned_dir(base, require_private=True)
