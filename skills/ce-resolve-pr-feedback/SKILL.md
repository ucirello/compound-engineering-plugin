...
allowed-tools: Bash(gh *), Bash(GIT_DIR=* gh *), Bash(jj *), Bash(git log *), Read
---
...
If scratch space is needed, use `$(jj workspace root)/.tmp`; when no JJ workspace root resolves, use `.tmp/` under the current project directory. Before writing, inspect the repository's root ignore rules and add `.tmp/` to the root `.gitignore` when needed, preserving existing entries. Do not use OS temporary directories. Create only the needed namespaced child path and remove it when finished.

## Success Criteria

- All unresolved review threads evaluated
- Valid fixes recorded in a JJ change and pushed with `jj git push`
