...
- **Merge-state claims** ("fixed in #1608", "landed", "shipped") verify against **remote truth** — the working copy may predate a merge, so `gh pr view` (or the tracker equivalent) is primary and local JJ reachability is only the fallback. The script's `INFO: working copy is N revisions behind …` line tells you how much to distrust the local tree for this category.
...
| SHA reachable from `@` only | Local-only revision; commit ID may change when the change is rewritten or squash-merged | Replace with the PR number |
| SHA reachable from upstream only | Working copy predates the merge | Keep, with a temporal qualifier; verify the landed claim via `gh` |
