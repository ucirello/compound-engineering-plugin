...
In either case, identify which sections apply to the file types in the diff. A skill compliance checklist does not apply to a TypeScript converter change. Match rules to the files they govern. For any commit-description validation or recommendation: Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. The repository's runtime conventions and visible JJ description history override compatible Go guidance.
...
- **Rules that don't apply to the changed file type.** Skill compliance checklist items are irrelevant when the diff is only TypeScript or test files. Match rules to what they govern; apply commit-description conventions only where a description is actually composed, edited, validated, or recommended.
