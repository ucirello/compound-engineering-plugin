...
3. **Check whether the issue already appears fixed on the default remote bookmark.** Once you know the affected surface, use read-only `jj log` and `gh` on that area (files, routes, components the symptom touches) to see whether a recent JJ change or merged PR already addresses it. Use `jj git fetch` only when fresh remote interoperability data is required; do not publish. Report this as a field in your finding so the orchestrator does not re-file resolved work.
...
- **Already fixed on the default bookmark?** -- `yes` / `no` / `unclear`, with the JJ change, Git commit ID, or PR reference you checked, or a note that you could not determine it.
...
- You are read-only except for the ONE write to your scratch artifact path. Read-oriented `jj` / `gh`, `jj git fetch`, and running the bundled analyzer are permitted; do not edit project files, create or describe changes, move bookmarks, run `jj git push`, or open PRs.
