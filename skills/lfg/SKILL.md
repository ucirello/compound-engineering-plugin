...
This pipeline requires a JJ workspace backed by Git. Use `jj` for every local repository mutation and inspection; do not substitute index, branch, or worktree operations. Use `gh` only for GitHub operations, setting `GIT_DIR` to the path from `jj git root` when the workspace is not colocated. Put transient artifacts under `<workspace>/.tmp/local/lfg/`; if that location cannot be used, keep them in memory rather than using OS temporary storage, and never include transient artifacts in a shipped change.
...
3. **Read `references/review-followup.md` now**, then invoke the `ce-simplify-code` skill on the delivery change set (`trunk()..@`) — **skip** only the invocation, never the read, when the change is docs-only (only markdown/docs paths changed) or trivial (roughly under 10 changed lines). That file governs steps 3 through 6, which have no usable form without it: only it carries this step's scope and structure pins, the review read-back, which findings step 5 applies and how they become a described change, and the residual record step 6 makes durable.
...
**Shipping precondition (steps 5–9).** Run `jj git remote list` once before the shipping steps. No remote means shipping is local-only: preserve every described change the steps below call for, but **skip every bookmark push, PR create/edit, and CI-watch action**, including step 9 in full. That is terminal, not an error.

5. **Apply and persist review fixes** (REQUIRED after step 4, before residual handoff)

   Execute the apply step of `references/review-followup.md`. Do not proceed to the residual handoff, run browser tests, or output DONE while eligible review fixes remain mixed into an undescribed working-copy change.
...
8. Ship: the goal is the remaining work represented by described JJ changes, published through one explicit bookmark, and in an open PR whose URL you hold. **Read `references/shipping-tail.md` first** — it governs steps 8 through 10 — then follow its project-process or native-JJ route. It preserves functional `ce-commit-push-pr` routing only when that process supports JJ change/bookmark semantics. Only that file carries what LFG threads into the handoff, the `New concepts:` trailer and ticket back-fill, the local-only substitution, and step 9's stack handoff.

9. **Watch the PR to CI-decided via `ce-babysit-pr`** (only when an open PR exists for the shipping bookmark)

   Detect the PR by querying `gh pr list --head <shipping-bookmark> --state open --json number,url,state`; if none exists or `gh` is unavailable, skip to step 10. When step 8 already handed off a stack babysit, `references/shipping-tail.md` decides this step — never start a second bare pipeline babysit on the shipping-bookmark URL. Otherwise invoke **`ce-babysit-pr mode:pipeline <pr-url>`**, and follow that same file for the returned `{ status, fixes_applied, residuals }`. Do not reimplement CI-watching here.
