...
- `base:` — when the diff base is already resolved on the current JJ workspace/bookmark; omit when reviewing a PR number/URL or standalone current bookmark.
...
3. **Parallel waves:** batches with **disjoint file sets** may run in parallel (same JJ workspace / shared-directory rules as Phase 1 Step 4 in `ce-work` SKILL.md).
...
- Shared-directory fallback: do not split/squash/commit or otherwise mutate JJ change structure — return which `#` were applied or skipped and which files changed
