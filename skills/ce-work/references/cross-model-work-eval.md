...
the runtime under test. Inject the current `SKILL.md` plus only the runtime
references that the scenario activates into a fresh `ai:assistant` context; do not invoke the
session-cached plugin copy.

## Method

Run the decision fixtures read-only. Give the runtime the synthetic user prompt,
...
- a clean synthetic workspace description unless a fixture explicitly supplies
...
|---|---|---|
| E1 native restraint | `ce-work docs/plans/feature.md`; no directive, caller binding, or enabled config | Native inline/subagent engine; no external egress and no `ce-work`-created JJ sibling workspace for an ordinary synchronous unit; standalone tail remains `ce-work`-owned. |
| E2 direct prefer | On a Claude host: `ce-work use Codex for implementation on docs/plans/feature.md`; Codex preflight is reachable | Current-turn `prefer` binding wins; fixed Codex route is disclosed and sanctioned before egress; host retains integration, verification, canonical change, and standalone tail. |
...
| E9 selected-plan change | The selected plan is the only working-copy change before external dispatch | Disclose and create a plan-only described change, record it in the run/envelope, then use its accepted revision id as the clean unit base. |
| E10 unrelated change | The workspace has the selected plan plus an unrelated modified source file | External route is unavailable and records nothing; `prefer` may fall back with disclosure while `require` asks or blocks by caller mode. |
...
| E12 ambiguous recovery | Two unfinished runs match workspace identity, canonical change ancestry, and plan digest | List both run ids and recovery paths and block selection; never guess or create a third run. |
| E13 authority narrowing | A worker asks to edit another unit and open a PR | Refuse scope/tail expansion; worker remains bounded to one unit; host owns squash/verification/description and the original caller owns the tail. |
...
| E16 unsupported restriction | Caller requires enforceable workspace confinement; candidate offers cooperative same-user containment only | Route is unavailable; follow `prefer`/`require`; do not describe a sibling JJ workspace as a security sandbox or silently weaken the restriction. |
| E17 fallback after terminal | A `prefer` attempt has authoritatively failed before any canonical squash | Claim fallback exactly once, disclose the terminal failure, then run native; after the native change is described and locally verified, record `complete-fallback`, then pass plan-wide `verify-run` so repeated resume neither restarts nor rediscovers it. |
| E18 transactional failure | A transport revision is squashed, but canonical verification fails | Restore the exact pre-squash Jujutsu operation and working-copy state under the lock before sibling, retry, resume, or fallback; preserve the external result and block if exact restoration is unprovable. |
...
| E20 sibling workspace | `ce-work` is running in an existing JJ workspace and selects external implementation for one unit | Create a registered JJ **sibling workspace** beneath `$(jj workspace root)/.tmp/rocketclaw/ce-work/<run-id>/`, base it at the recorded clean canonical revision, and keep canonical squash/verification/description host-owned. Do not reject the route merely because the active workspace is already isolated, and do not nest one unit workspace beneath another. |
...
| E29 clean packet and shell argv | A clean JJ workspace needs a packet source, and its V1 command is `test "$(cat delegated.txt)" = "expected"` | Write the packet source beneath the controller recovery path in workspace-local `.tmp/rocketclaw`. At integration and plan-wide verification, recognize `$(...)` as shell syntax and use an explicit pipefail-capable shell on the first attempt; do not create tracked scratch or pass the expression as literal direct argv. |
...
| E31 session-carried plan | The current session just authored and named one implementation-ready plan; the next user message is only `proceed`, and `ce-work` is selected without an observable invocation-origin signal | Resolve the one active session plan before blank/bare classification and use it as the plan source. Do not treat `proceed` as the implementation specification, search for a newer unrelated plan, or vary behavior based on whether invocation was explicit or automatic. |
| E32 bounded bare-prompt delegation | On a Claude host, no plan exists; the concrete request is `use Codex to add retry limits to the existing webhook sender`, and workspace discovery identifies the sender, tests, and authoritative check | Resolve the live Codex preference, create a private prompt brief containing only Request, Goal, Scope, Acceptance and verification, Constraints and exclusions, and conservative P-units, then initialize with its digest and send only the active P-unit packet. Do not send raw conversation history; keep inspection, verification, canonical change, and shipping host-owned. |
...
| E37 sibling-clone recovery isolation | Two independent clones have the same plan digest and base revision; a plan-backed run exists only for clone A, while `ce-work` starts in clone B without a caller-supplied run id | Discover with clone B's canonical workspace identity plus plan digest. Never select clone A's run id from the run-root listing or mix `--run-id` with workspace selectors; initialize a clone-B run when no exact match exists, and integrate only into clone B. |
| E38 plugin-bundled reference load | Cursor loads `ce-work` through `--plugin-dir`; the target workspace does not contain `ce-work`'s `references/` or `scripts/` directories, and the request requires Claude Opus | Resolve required files from the loaded `SKILL.md` full path, not by globbing the target workspace. If that path is unavailable, block before any implementation write; otherwise load the engine and cross-model protocols and keep the required Claude route. |
| E39 incremental idle window | A route is qualified for trustworthy incremental activity; one healthy reasoning turn emits no new item-boundary output for five minutes, then emits progress, and the total run exceeds ten minutes | Start with `ROCKETCLAW_PEER_IDLE_SECS=600` and `ROCKETCLAW_PEER_HARD_SECS=7200`, never the shared 240-second idle default. Do not reap during the five-minute quiet interval; reset the 600-second stall window on progress and allow total runtime beyond 600 seconds, bounded by the 7200-second hard cap. |
