...
Resolve scratch storage once. When `jj workspace root` succeeds, use `$(jj workspace root)/.tmp`; outside a JJ workspace, use the local `./.tmp` fallback. Before writing there, verify the applicable local ignore rules keep `.tmp` out of the working-copy change. Put all disposable workspaces, per-run scratch, transient extracts, and tool-created temporary files below that root rather than using a system or user-global temporary location.
