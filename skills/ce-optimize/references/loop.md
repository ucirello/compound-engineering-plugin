...
**Worktree backend** (jj workspaces; spec key remains `execution.backend: worktree`):
...
3. Write the filled prompt to a temp file under the workspace `.tmp`:
   ```bash
...
4. Dispatch via Codex:
   ```bash
   cat "$SCRATCH_ROOT/optimize-exp-XXXXX.txt" | codex exec --skip-git-repo-check - 2>&1
   ```
...
3. **If `decide.mjs` returns `keep` for that winner: KEEP**
   - Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Repo-local syntax from project instructions and `git log` ALWAYS wins when it differs from Go guidance. Apply compatible Go guidance to quality/clarity/structure without replacing repo-local syntax. The composed message should identify this optimization experiment and the hypothesis; do not use a fixed type/scope/subject template.
   - In the experiment workspace, record the winning diff as a real change before any merge or duplicate: `(cd "$WORKTREE_PATH" && jj commit -m "<message composed from the standards above>" <mutable-scope filesets>)`. Include only mutable-scope changes in that change; if no eligible diff remains, treat the experiment as non-improving and restore it (`jj restore` / `jj abandon`).
   - From the optimization workspace, merge the experiment bookmark: `(cd "$optimization_workspace_root" && jj new @ "<experiment-bookmark>")`.
   - After the merge succeeds, clean up the winner's experiment workspace (`jj workspace forget NAME` then delete the directory) and forget its bookmark; the integrated change on the optimization bookmark is the durable artifact
...
4. **Check file-disjoint runners-up** (up to `max_runner_up_merges_per_batch`):
   ...
   - Otherwise: abandon the duplicated change (`jj abandon`), log as "promising alone but neutral/harmful in combination" (outcome: `runner_up_reverted`), then clean up the runner-up's experiment workspace and bookmark
