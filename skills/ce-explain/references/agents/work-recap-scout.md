...
Dispatch context supplies: `{window}` (a date range, relative window, or since-reference), `{workspace-root}`, and `{run-dir}` (scratch path for your output file).

## What to gather

Work through these sources for the window, cheapest first:

1. **JJ activity** — `jj log` over the window (descriptions, change and commit IDs, dates, authors), and for the substantial changes, a stat-level view of what they touched. Use the repository's local revsets and templates when available. Group obviously related changes rather than listing them flat.
2. **Merged and open PRs** — only when a GitHub interface is reachable (`gh` or a connector/MCP tool). This portion is capability-gated: when no interface is reachable, note "PR evidence unavailable" in one line and move on — never treat the missing interface as an error and never guess PR state from bookmark names.
...
- What changed, in one line, with the change or commit ID(s), PR number, and date
- Why, when a doc or change description says so — quoted, with the source (`file:line` or ID)
