...
Run this **first** when the caller provides a concrete diff and `db/schema.rb` or `db/structure.sql` appears in that diff. Use the review base revision from caller context (`<review-base>`). **Never assume `main`.** Run JJ commands with cwd set to the absolute workspace root. The caller's review target controls the diff; when it names a bookmark or PR and `@` is empty, inspect `@-` rather than treating the empty change as the reviewed work.
