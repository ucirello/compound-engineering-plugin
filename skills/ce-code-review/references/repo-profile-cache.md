...
- The `docs/solutions/` enumeration (a new learning in the working-copy change must be visible — re-globbing it is ~free and the match reads files fresh anyway).
...
- `<root-sha>` = lexicographically-first commit ID selected by `roots(::@ ~ root())` — the repository identity.
- `<head-sha>` = the commit ID of `@` — the working state.

Workspaces rooted at the same path and working-copy commit share the same entry. Lookup is JJ metadata only; on a hit, only this one file is read.
