...
**Shared-bookmark topology** (`sweep_shared_bookmark: true`): before any source-side write, finalize only the state fileset as a JJ change, move or create the configured publication bookmark at that change, and push only that bookmark with `jj git push --bookmark <bookmark> --remote <remote>`. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Use the repository's current local syntax; do not impose a fixed type, scope, prefix, footer, or body template. Preserve the lease-acquisition semantics. A rejected push means another writer moved the remote bookmark: run `jj git fetch --remote <remote>`, inspect the local and remote bookmark targets with `jj bookmark list --all-remotes`, preserve the remote state while rebasing the unpublished lease change, and re-run `lease-acquire` against that reconciled state. A conflicted bookmark or state file, or a lease now owned by another writer, stops before source-side writes with `aborted-locked`; never resolve this gate by choosing the local lease state. Only a pushed bookmark whose remote target resolves to the lease change authorizes source-side writes.
