...
- **Unified artifact guard.** Universal-planning outputs are not software implementation plans. Do not label them `artifact_contract: rocketclaw-unified-plan/v1` and do not produce a `/goal` launch block unless the deliverable itself is a complete software implementation plan with Product Contract, Planning Contract, Implementation Units, Verification Contract, and Definition of Done.
...
1. **Save to disk** — Write the plan as a markdown file. Ask where:
   ...
   - `$(jj workspace root)/.tmp/universal-plans/`, with the physical current project directory's `.tmp/universal-plans/` as the local fallback when workspace-root resolution fails; before writing there, inspect the repository-root `.gitignore` and add `.tmp/` if needed, preserving existing entries
   - A custom path
