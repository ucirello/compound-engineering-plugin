...
| pushed-shared-bookmark | One writer **per repo**. | The state file lives on a shared bookmark multiple workspaces push to. `lease-acquire` must be committed, pushed, and confirmed (fetch back and verify our writer won) **before any source-side write**. This makes the lease a repo-wide mutex across machines. |
...
`run-record` is intentionally **lease-agnostic**. A run that aborted precisely because the lease was `LOCKED` (`outcome: aborted-locked`) must still be able to record that fact, but that write happens while the lease holder is mid-sweep. To keep it from clobbering the holder's concurrent upserts, every mutating subcommand holds an **OS advisory lock** across its whole load-modify-write. The lock lives at `<workspace-root>/.tmp/sweep/<sha256-of-absolute-state-path>.lock`, with the state path resolved through symlinks. Calls from the same workspace targeting the same state serialize regardless of lease ownership. The lease decides *which writer is running the sweep*. The file lock decides *which process is writing the file right now*. The lock file and atomic-write staging files are never committed. Outside JJ, the current local project directory supplies the scratch root. Scratch or lock creation failure stops the write; a state destination on a different filesystem from scratch cannot be atomically replaced and returns `ERROR` without replacing the old state. The skill's JJ commit step selects only committed-mode state and the plan by explicit paths; machine-local state under workspace `.tmp` is excluded.
