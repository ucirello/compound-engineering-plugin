...
# 2. Describe the working-copy change for only files related to this logical unit
...
**Note:** Incremental change descriptions carry no attribution footers. The final Phase 4 handoff passes `branding:on` so `ce-commit-push-pr` can apply project PR attribution.
