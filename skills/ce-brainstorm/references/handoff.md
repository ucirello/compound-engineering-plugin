...
(`$(jj workspace root)/.tmp/rocketclaw/brainstorm/<run-id>/grounding.md`, or the local `.tmp/rocketclaw/brainstorm/<run-id>/grounding.md` fallback outside JJ) — it gives
...
independent code review and applied fixes, describe the JJ change, push its bookmark, open a PR, and watch CI to
...
skills run (plus `jj`/`gh` for the change/PR/CI tail, which it guards when absent).
