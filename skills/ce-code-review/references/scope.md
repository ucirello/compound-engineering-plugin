...
```
BASE_ARG="{base_arg}"
BASE=$(jj log -r "heads(::@ & ::$BASE_ARG)" --no-graph -T 'commit_id' -n 1 2>/dev/null) || BASE="$BASE_ARG"
```

Then produce the same output as the other paths (cwd = `jj workspace root` so paths stay repo-relative):

```
echo "BASE:$BASE" && echo "FILES:" && jj diff --name-only --from $BASE && echo "DIFF:" && jj diff --git --from $BASE --context 10 && echo "UNTRACKED:"
```

JJ snapshots non-ignored working-copy files into `@`, so they appear in `jj diff --from $BASE`; `UNTRACKED:` is empty. This path works with any ref — a commit id, `main@origin`, a bookmark name. Callers reviewing the current checkout should pass explicit `base:` when auto-detection is unnecessary. **Do not combine `base:` with a PR number or branch target.** If both are present, stop with an error: "Cannot use `base:` with a PR number or branch target — `base:` implies the current checkout is already the correct branch. Pass `base:` alone, or pass the target alone and let scope detection resolve the base."
...
**Skip-condition pre-check.** Before scope detection, run a PR-state probe (`GIT_DIR=$(jj git root)` so `gh`'s underlying git calls succeed):
...
Set `BASE:` to `pr:<number-or-url>` (logical marker — not a commit id). Set `UNTRACKED:` empty on the **current** checkout (JJ snapshots non-ignored files into `@`; usually empty during PR-remote review).
...
3. The PR head commit is contained in the local checkout: `jj log -r '<headRefOid> & ::@' --no-graph -T 'commit_id' -n 1` prints the head oid. This confirms the working tree actually carries the PR head (allowing unpushed local fixes layered on top) rather than an unrelated same-named bookmark.
...
- **`local-aligned`:** Resolve `<resolved-base-ref>` from `baseRefName` (fetch if needed). Compute `BASE=$(jj log -r "heads(::@ & ::<resolved-base-ref>)" --no-graph -T 'commit_id' -n 1)`, then set `FILES:` from `jj diff --name-only --from $BASE` and `DIFF:` from `jj diff --git --from $BASE --context 10` (includes the working-copy change on the PR bookmark). Do **not** call `gh pr diff` or append remote hunks — when unpushed fixes exist, the local tree is canonical. Note in Coverage: `scope: local-aligned (PR; local tree diff)`.
- **`pr-remote`:** Set `FILES:` from the PR `files` array. Set `DIFF:` from `GIT_DIR=$(jj git root) gh pr diff <number-or-url> --color=never`. If `gh pr diff` fails, stop with an actionable error — do not fall back to checkout.

When **`pr-remote`**, before Stage 4:

1. Best-effort fetch PR head without checkout: `jj git fetch --remote origin --branch <headRefName>` (substitute PR number from metadata for Coverage notes).
...
3. Best-effort fetch the PR base without checkout: `jj git fetch --remote origin --branch <baseRefName>`. When it succeeds, resolve a concrete ref with `jj log -r '<baseRefName>@origin' --no-graph -T 'commit_id' -n 1` and set `PR_BASE_REF` to that commit id — a **real JJ base ref** reviewers and validators use for file-level diffs (e.g. `data-migration-reviewer` runs `jj diff --from <PR_BASE_REF> -- db/schema.rb`/`structure.sql`). The `pr:<number-or-url>` logical marker in `BASE:` stays the scope marker; `PR_BASE_REF` is the diffable base. When the fetch fails, omit `PR_BASE_REF` and note in Coverage — schema-drift and other jj-diff checks fall back to diff hunks only and must **not** assume `main`.
...
If `jj bookmark list -r @ -T 'name ++ "\n"'` includes `<branch>`, use the **standalone (current bookmark)** path below — same tree, explicit bookmark name; do not use remote-only diff.
...
3. Resolve default base bookmark (same logic as standalone). Compute `BASE=$(jj log -r "heads(::<base-ref> & ::<branch-ref>)" --no-graph -T 'commit_id' -n 1)` and `jj diff --git --from $BASE --to <branch-ref> --context 10`.
...
```
echo "BASE:$BASE" && echo "FILES:" && jj diff --name-only --from $BASE --to <branch-ref> && echo "DIFF:" && jj diff --git --from $BASE --to <branch-ref> --context 10 && echo "UNTRACKED:"
```

**If no argument (standalone on current branch):**

Apply the same base-detection logic as branch mode above, using the current bookmark (i.e., `GIT_DIR=$(jj git root) gh pr view --json baseRefName,url` with no argument defaults to the current bookmark).

If no base can be resolved, **stop**. Do not fall back to `jj diff` (working-copy vs parent) — a standalone review without the base would only show the current change and silently miss all other work on the bookmark.
...
Using `jj diff --from $BASE` (without `--to`) diffs the merge-base against the working-copy commit `@`, which includes the full current change.

**Untracked file handling:** Always inspect `UNTRACKED:`. JJ snapshots non-ignored files into `@`, so they appear in the diff and `UNTRACKED:` is typically empty. When non-empty, list excluded files in Coverage and continue on tracked changes only — never stop or prompt.
