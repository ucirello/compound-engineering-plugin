...
5. **Pipeline mode and model elevation.** Pipeline mode skips interactive questions but does **not** disable model elevation — `plan_model` / `plan_harness` config (and `plan_model:<alias>` / `plan_harness:<value>` tokens passed by the caller) is still honored (see the model-elevation sub-step below and `references/reasoning-elevation.md`). It does not change the output format.

**Token-parsing convention:** only literal-prefix flag tokens (`output:`, `mode:`, the exact `confirm:auto`/`confirm:ask` forms, `plan_model:<alias>`, `plan_harness:<value>`, `delegate:` where applicable) are consumed and stripped. Other `<word>:<word>` tokens — including commit prefixes like `feat:` and any unrecognized `confirm:<value>` — pass through verbatim into the feature description. A stripped `plan_model:<alias>` or `plan_harness:<value>` token is kept for the Phase 5.2 model-elevation step.
...
**Model-elevation visibility.** Treat a stripped `plan_model:<alias>` or `plan_harness:<value>` token, or a `plan_model` / `plan_harness` config value you have found, as a pending Phase 5.2 input, not a resolved choice. Phase 5.2 resolves the choice from the current conversation, those tokens, and config immediately before authoring, so later user intent cannot be lost. `plan_harness: opencode2` is its own branch, not an alias of `opencode`.
