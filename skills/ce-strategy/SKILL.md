...
Resolve `<workspace-root>` at runtime with `jj workspace root`. If the current project is not a Jujutsu workspace or JJ is unavailable, use the physical current project directory. `STRATEGY.md`, `README.md`, `CONCEPTS.md`, and sibling product documents are rooted there.

Before reading RocketClaw plans, brainstorms, solutions, or other context artifacts, resolve `<root>` from `<workspace-root>/.rocketclaw/config.yaml`:

- An unset `docs_root` means `<root>` is `<workspace-root>/docs`.
- A set value is workspace-relative and must resolve inside the workspace, outside `.jj/`, and somewhere other than the workspace root. Stop and report an invalid value.
- Read RocketClaw context only from `<root>`; do not also read `docs/` when the configured root differs.
...
2. **Grounded, not blank-slate.** Most workspaces already say what the product is - in the README, RocketClaw context, and the shape of the code. Read that first so the interview opens from a working model instead of an empty page. The user's answer still decides every section; evidence earns the right to ask a sharper question, never to fill in a section on its own.
