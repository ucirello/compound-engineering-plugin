...
- You may edit and test in this Jujutsu workspace. Do not describe, rebase, squash, abandon, bookmark, or advance its working-copy change. Report `completed` when files and scoped checks are done; the host pins the revision. Do not push, open a PR, ship, or integrate into another workspace.
...
- Before returning `completed`, inspect the complete `jj diff -r @` against the packet's expected scope. Remove only disposable artifacts created by your own checks. If an unexplained or non-disposable path remains, return `blocked` or `scope_expansion`; otherwise list every remaining changed path in `changed_files`.
- Your changed-file list and prose are evidence only. The host independently derives the complete Jujutsu revision and alone decides whether to integrate it.
