...
| ❌ Wrong | ✅ Correct |
|----------|-----------|
| Subagents write product files into `docs/` or edit tracked paths | Subagents write only scratch artifacts under `$(jj workspace root)/.tmp/rocketclaw/ce-compound/<run-id>/` (or local `./.tmp/rocketclaw/ce-compound/<run-id>/` outside JJ) and return the path; orchestrator writes the one final doc |
