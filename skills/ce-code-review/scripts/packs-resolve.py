...
complete clone. Clones use `jj git clone` non-interactively (GIT_TERMINAL_PROMPT=0,
ssh BatchMode, bounded timeout): missing credentials degrade to a warning, never
a hang. A missing `jj` binary degrades URL sources only (each warns and is
...
def resolve_git_source(url: str, ref: str, warnings: list, label: str) -> str | None:
    ...
    try:
        ...
        if proc.returncode != 0:
            # Named branch/tag clone failed. jj git fetch only accepts bookmarks
            # and tags, so an arbitrary sha cannot be fetched via public jj.
            warnings.append(
                f"{label}: cannot clone `{ref}` from {url} (jj git clone --branch); source skipped"
            )
            return None
...
def _repo_root() -> str | None:
    """The enclosing workspace root via public jj. None when not a jj workspace."""
