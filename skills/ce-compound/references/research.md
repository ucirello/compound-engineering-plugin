...
### Phase 0.5: Supplementary Memory Scan

Before launching Phase 1 subagents, check any supplementary memory already injected into context for notes relevant to the problem being documented.

1. Use only a provider-supplied memory block already present in context; do not search for or create one.
2. If the block is absent or empty, proceed to Phase 1 unchanged.
...
5. Pass this block as additional context to the Context Analyzer and Solution Extractor. If memory evidence reaches the final documentation, mark it only as supplementary memory evidence.
...
if [ -L "$SCRATCH_ROOT" ]; then printf '%s\n' "unsafe scratch root symlink: $SCRATCH_ROOT" >&2; exit 1; fi;
(umask 077; mkdir -p "$SCRATCH_ROOT") || exit 1;
RUN_ID=$(date +%Y%m%d-%H%M%S)-$$;
RUN_DIR="$SCRATCH_ROOT/document-learning/$RUN_ID";
...
#### 1. **Context Analyzer**
   ...
   - Incorporates supplementary memory excerpts (if provided by the orchestrator) as secondary evidence
...
#### 2. **Solution Extractor**
   ...
   - Incorporates supplementary memory excerpts (if provided by the orchestrator) as secondary evidence; conversation history and the verified fix take priority, and contradictions are recorded as cautionary context
   ...
   **Bug track output sections:**
   ...
   - **Prevention**: Strategies to avoid recurrence, best practices, and test cases. Include project-native evidence where it materially clarifies the guardrail
   ...
   **Knowledge track output sections:**
   ...
   - **Examples**: Neutral placeholders or project-native evidence showing the practice in action
...
#### 3. **Related Docs Finder**
   ...
   **Search strategy (grep-first filtering for efficiency):**
   ...
   3. Use the native content-search capability to pre-filter candidate files before reading content. Search relevant frontmatter fields case-insensitively with neutral topic placeholders.
   ...
   **GitHub issue search:**
   ...
   Prefer the `gh` CLI for searching related issues with the topic, all states, and a small result limit. If `gh` is not installed, fall back to an available GitHub provider interface. If neither is available, skip GitHub issue search and note it in the output.
