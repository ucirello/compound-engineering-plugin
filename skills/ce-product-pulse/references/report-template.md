...
Preserve the required sections, metrics, source facts, safety constraints, output path, and human or research-source attribution. The template and examples define content and order rather than fixed change-description syntax. Do not add project or workflow branding, generated-by text, visual badges, or creator, model, provider, tool, agent, harness, runtime, workflow, or co-author attribution.
...
Use the block below as the report's required content shape. Replace every `{{placeholder}}` with query output and adapt prose to the product's runtime conventions. Delete lines whose data isn't available for this run.
