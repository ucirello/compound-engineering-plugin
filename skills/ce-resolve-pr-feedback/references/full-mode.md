...
```bash
GIT_DIR=<path-from-jj-git-root> gh pr view --json number -q .number
```
...
**GitHub Enterprise host.** The bundled `gh api graphql` scripts hit `gh`'s default host unless told otherwise, so on a GHE PR they would wrongly target `github.com`. Derive the host: if the caller passed a full PR **URL**, take its host; otherwise read it from `GIT_DIR=<path-from-jj-git-root> gh repo view --json url -q .url`. Then — because shell state does **not** persist between separate Bash calls — pass the host as a `GH_HOST=<host>` **env prefix inline on every bundled-script call** (`gh api` honors `GH_HOST` as the request host). A single `export` in one block does **not** carry to the reply/resolve/verify blocks that run as later Bash calls, which is why each call below shows the prefix. On `github.com`, drop the `GH_HOST=<host> ` prefix entirely.

**Colocated Git dir.** `gh` needs the colocated Git backend. Resolve the path once with `jj git root` as its own call, then set `GIT_DIR=<that path>` as an env prefix on every `gh` invocation and every bundled-script call (those scripts invoke `gh`). Do not nest `$(jj git root)` in the same command. Combine with `GH_HOST` when both apply.

```bash
PR_HOST=$(printf '%s' "<pr-url-if-one-was-passed>" | sed -n 's#^https\?://\([^/]*\)/.*#\1#p');
[ -z "$PR_HOST" ] && PR_HOST=$(GIT_DIR=<path-from-jj-git-root> gh repo view --json url -q .url 2>/dev/null | sed -n 's#^https\?://\([^/]*\)/.*#\1#p');
...
GIT_DIR=<path-from-jj-git-root> GH_HOST=<derived-host> bash "$SKILL_DIR/scripts/get-pr-comments" PR_NUMBER OWNER/REPO   # omit GH_HOST=<derived-host> on github.com
```
...
```bash
GIT_DIR=<path-from-jj-git-root> gh pr view PR_NUMBER --json reviews,comments
GIT_DIR=<path-from-jj-git-root> gh api repos/{owner}/{repo}/pulls/PR_NUMBER/comments
```
...
Repository-local syntax from project instructions and `git log` ALWAYS wins. Do not choose Conventional Commits vs Go style here; determine the syntax at runtime from those sources.

Keep these semantic constraints: the message references the PR (`#PR_NUMBER`) and lists the changes from fixer summaries. When step 5 noted a pre-existing failure, keep that as a footer: `Note: pre-existing failure in <test> not addressed by this PR.`

There is no staging area — the working copy is the change. Name only the files reported by fixers on the commit so unrelated working-copy paths stay out.

```bash
jj commit -m "<message composed from the standards above>" [files from fixer summaries]
```

Push the bookmark that tracks this PR:
...
```bash
SKILL_DIR="<absolute path of the directory containing the ce-resolve-pr-feedback SKILL.md>";
GIT_DIR=<path-from-jj-git-root> GH_HOST=<derived-host> GH_REPO=OWNER/REPO gh api repos/{owner}/{repo}/pulls/comments/COMMENT_ID --jq .node_id
GIT_DIR=<path-from-jj-git-root> GH_HOST=<derived-host> bash "$SKILL_DIR/scripts/get-thread-for-comment" PR_NUMBER COMMENT_NODE_ID OWNER/REPO
```
...
```bash
SKILL_DIR="<absolute path of the directory containing the ce-resolve-pr-feedback SKILL.md>";
GIT_DIR=<path-from-jj-git-root> GH_HOST=<derived-host> bash "$SKILL_DIR/scripts/reply-to-pr-thread" PR_NUMBER ROOT_COMMENT_ID OWNER/REPO <<'EOF'
...
```bash
GIT_DIR=<path-from-jj-git-root> GH_HOST=<derived-host> GH_REPO=OWNER/REPO gh api repos/{owner}/{repo}/pulls/comments/REPLY_COMMENT_ID --jq .body
GIT_DIR=<path-from-jj-git-root> GH_HOST=<derived-host> GH_REPO=OWNER/REPO gh api repos/{owner}/{repo}/pulls/comments/REPLY_COMMENT_ID --jq '.pull_request_review_id // empty'
```
...
```bash
GIT_DIR=<path-from-jj-git-root> GH_HOST=<derived-host> GH_REPO=OWNER/REPO gh api --method PATCH repos/{owner}/{repo}/pulls/comments/REPLY_COMMENT_ID -f body="$(cat <<'EOF'
> the specific sentence being addressed from the reviewer's comment
...
```bash
GIT_DIR=<path-from-jj-git-root> GH_HOST=<derived-host> GH_REPO=OWNER/REPO gh api repos/{owner}/{repo}/pulls/PR_NUMBER/reviews/REVIEW_ID --jq .state
```
...
```bash
SKILL_DIR="<absolute path of the directory containing the ce-resolve-pr-feedback SKILL.md>";
GIT_DIR=<path-from-jj-git-root> GH_HOST=<derived-host> bash "$SKILL_DIR/scripts/get-pr-comments" PR_NUMBER OWNER/REPO | jq -r '.pending_review // empty'
```
...
```bash
SKILL_DIR="<absolute path of the directory containing the ce-resolve-pr-feedback SKILL.md>";
GIT_DIR=<path-from-jj-git-root> GH_HOST=<derived-host> bash "$SKILL_DIR/scripts/resolve-pr-thread" THREAD_ID
```
...
```bash
GIT_DIR=<path-from-jj-git-root> GH_HOST=<derived-host> gh pr comment PR_NUMBER -R OWNER/REPO --body "$(cat <<'EOF'
> the specific sentence being addressed from the reviewer's comment
