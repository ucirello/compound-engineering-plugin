...
- GitHub (`#123`, `org/repo#123`, github.com URL): Parse the issue reference from `<bug_description>` and fetch with `GIT_DIR="$(jj git root)" gh issue view <number> --json title,body,comments,labels`. For URLs, pass the URL directly to the same bridged `gh` command.
