...
- **Explicit mutations only.** Never run `gh pr checkout`, `jj edit`, `jj new <bookmark>`, or similar working-copy switch commands. Passing a PR number, URL, or bookmark name selects **review scope**, not permission to mutate the working copy. Unpublished work can only be reviewed from the workspace that holds it, so to review it on a feature bookmark, stay on that change (or edit it yourself) and pass `base:` or no target.
