...
**Branch mode:** Run `jj log -r "${BASE}..<branch-ref>" --no-graph -T 'commit_id.short() ++ " " ++ description.first_line()'` using the resolved merge-base and resolved branch ref from Stage 1. Use `<branch-ref>` (the resolved `<branch>@origin` or fetched rev), not the raw `<branch>` argument — a remote-only bookmark has no matching local bookmark, so the raw name would fail or read a stale same-named local bookmark.

**Standalone (current branch):** Run:

```
echo "BRANCH:" && jj log -r @ -T 'bookmarks.join("\n")' --no-graph --no-pager && echo "COMMITS:" && jj log -r "${BASE}..@" --no-graph -T 'commit_id.short() ++ " " ++ description.first_line()' --no-pager
```
...
In `pr-remote` / `branch-remote`, current source and any targeted probe must use `jj file show -r <reviewed-head-ref>` against the supplied reviewed head ref, or the supplied diff hunks when no head ref is available; never inspect workspace paths.
