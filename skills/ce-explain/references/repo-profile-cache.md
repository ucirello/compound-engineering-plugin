...
- `<workspace-root>` = `jj workspace root`; when JJ is unavailable, storage falls back to local `.tmp` and the helper returns `NO-CACHE` because no JJ key can be proven.
- `<root-commit-id>` = the lexicographically first real root commit ID in `::@` (excluding JJ's synthetic root).
- `<commit-id>` = the current working-copy commit ID rendered by `jj log -r @`.

Each workspace keeps its own entry. Lookup uses JJ metadata only; on a hit, only this one file is read.
...
- `MISS` then a write-path on the next line → dispatch the `repo-profiler` persona to derive the profile, write its JSON output to a file, then persist it. This `put` runs after the profiler, so it is a **separate Bash-tool call** from the `get` above — shell variables do not persist between calls, so **re-set `SKILL_DIR` in the same command**:
  ...
  ```
- `NO-CACHE` → no usable JJ state or no writable workspace-local cache. Derive the profile fresh for this run and **skip** `put` (nothing to persist).
...
A cached entry is a `HIT` only when its `profile_schema_version` and root/current JJ commit IDs match and **no profile-input path** differs in the working-copy commit. Freshness uses `jj diff --name-only -r @ <profile-input-fileset>`; JJ snapshots non-ignored working-copy files into `@`, so newly added inputs are included. The fileset is generated from the same conservative **superset** used by the classifier — dependency manifests + lockfiles (any depth), license, root instruction/doc files, `CONCEPTS.md`/`STRATEGY.md`, topology sources (`Dockerfile`, `.github/workflows/`, `.cursor/rules`). A changed source file, `docs/plans/*`, or other non-input path does **not** invalidate. Root/current IDs are resolved again after freshness checks and before atomic replacement, so a snapshot or rewritten ancestor fails closed instead of publishing or serving under stale keys. Completeness of the input set is the cardinal-rule safety requirement: over-invalidating costs a re-derive; under-invalidating would serve a stale profile.

## Degradation

The cache is an optimization, never a correctness dependency. Outside a JJ workspace, with no writable `<workspace-root>/.tmp`, or on an unreadable/malformed entry, the helper returns `NO-CACHE`/`MISS` (exit 0) and the skill derives fresh. When JJ is unavailable, it resolves storage relative to the current directory's `.tmp` but does not persist without a trustworthy JJ key. It never blocks and never serves a profile it cannot prove fresh. If the helper invocation itself fails — a non-zero exit, empty output, or an unresolved `SKILL_DIR` so the script isn't found — treat it exactly like `NO-CACHE`: derive the profile fresh this run and proceed. Never stall waiting on the cache.
