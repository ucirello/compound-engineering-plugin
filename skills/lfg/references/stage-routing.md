...
- **Planning** routes to `ce-plan` as a `plan_model:<alias>` carrier — the plan-authoring **model** (model elevation). Example aliases: `fable`, `opus`. A `provider/modelname#variant` pin is also a valid `plan_model` value: preserve provider, model, and variant. Planning also accepts `plan_harness: opencode2` when the assignment scopes the **opencode2** harness to planning ("plan with opencode2"); pass `plan_harness:opencode2` alongside `plan_model:<alias>` when a model pin is also present, or alone when only the harness is assigned. No V1/opencode fallback when opencode2 is selected. An assignment that scopes any *other* harness to planning ("plan with codex", "plan on cursor") is **not supported** — surface it as a routing-carrier blocker rather than encoding a harness name as `plan_model:<harness>`, which `ce-plan` cannot serve and would silently fall back to the session model. Only opencode2 and the implementation stage route to a different harness.
