...
> Diff-scoped browser QA of `<target>` from its fork point with `<trunk-or-pr-base>` on <YYYY-MM-DD>.
...
- **Recommendation:** <suggested direction for the human to confirm>
...
<Overall readiness verdict for the target. Ready to publish? Caveats? Outstanding blocked items? Record the Phase 5 automated-suite result, whether the local target bookmark contains every fix, and that no push was performed — a green matrix with a red suite is not "ready.">
