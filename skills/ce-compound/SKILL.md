...
- **Read** `docs_root` from `<workspace-root>/.rocketclaw/config.local.yaml`, then `<workspace-root>/.rocketclaw/config.yaml`; the first non-empty value wins (`<workspace-root>` = `jj workspace root`; outside a JJ workspace, use the current directory as the local fallback). Unset -> `<root>` is `docs`, exactly as before.
