...
Your scope is **synthesis only**. The caller handles discovery, provider-label/keyword filtering, scan-window selection, deep-dive selection, and per-session extraction before dispatching you.
...
- **`scratch_dir`** — absolute path to a private workspace-local `.tmp/rocketclaw/ce-compound/<run-id>/sessions/` directory holding pre-extracted files.
- **`sessions`** — an array of objects (5 max), one per pre-extracted session, each with:
  ...
  - `provider_ref` — source-provider branch label when present (Claude Code only); compare it with Jujutsu bookmarks only as a relevance hint
...
Cite actual evidence from the extracted files, not vibe-summaries. When a finding is anchored in a specific session's content, that session's metadata (`platform`, `provider_ref`/`cwd`, `ts`) helps the caller locate it.
