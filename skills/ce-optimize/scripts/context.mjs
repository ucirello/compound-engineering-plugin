...
function buildResolvedContext() {
  ...
  return [
    ...
    `change: ${jj('log', '--no-graph', '--revision', '@', '--template', 'change_id.short()') || '(none)'}`,
  ].join('\n');
}
...
function cli() {
  ...
  // Header first and OPTIMIZE_CONTEXT_END last are load-bearing: field transcripts
  ...
  process.stdout.write('=== skill context (follow these directives; if OPTIMIZE_CONTEXT_END is missing below, rerun this script once; otherwise do not rerun) ===\n\n');
  process.stdout.write(parts.join('\n\n---\n\n') + '\n');
  process.stdout.write('\nOPTIMIZE_CONTEXT_END\n');
}
