...
| `mode:report-only` | `mode:report-only` | **Deprecated — ignored.** Former no-artifacts mode; default behavior is review-only without workspace switching |
| `base:<sha-or-ref>` | `base:abc1234` or `base:main@origin` | Diff base on the **current workspace** (explicit; skips auto base detection) |
...
- **Apply locally; never push.** Never push, open PRs, or file tickets in any mode — push is the outward step the user owns. In **default (interactive)** mode the review applies safe, verified fixes and commits them when the pre-review workspace was clean (Stage 5c owns the full rule). In **`mode:agent`** it never mutates the workspace — it reports and the caller applies.
...
- **Explicit mutations only.** Never run `gh pr checkout`, `jj new`, `jj edit`, `jj bookmark set`, or similar workspace/bookmark-switch commands. Passing a PR number, URL, or bookmark name selects **review scope**, not permission to mutate the workspace. To review local working-copy work on a feature bookmark, move to that bookmark yourself (or stay on it) and pass `base:` or no target.
- **Smart defaults.** Newly-added files: review files included in the JJ diff and list excluded paths in Coverage. Plan: use `plan:` when passed; otherwise discover conservatively from PR body or bookmark keywords. Weak advisory P2/P3 from testing/maintainability alone: demote to `testing_gaps` / `residual_risks` per Stage 5.
...
- `docs/plans/*.{md,html}` -- unified plan artifacts created by ce-brainstorm or ce-plan (decision artifacts; execution progress is derived from JJ/GitHub state, not stored in plan bodies)
...
```
BASE_ARG="{base_arg}"
BASE=$(jj log -r "heads(::@ & ::$BASE_ARG)" --no-graph -T 'commit_id ++ "\n"' 2>/dev/null | head -n1) || BASE="$BASE_ARG"
```

Then produce the same output as the other paths:

```
echo "BASE:$BASE" && echo "FILES:" && jj diff --from "$BASE" --name-only && echo "DIFF:" && jj diff --from "$BASE" --context 10 && echo "UNTRACKED:" && jj diff -r @ --name-only
```

This path works with any JJ revision — a SHA, `main@origin`, or a bookmark name. Callers reviewing the current workspace should pass explicit `base:` when auto-detection is unnecessary. **Do not combine `base:` with a PR number or bookmark target.** If both are present, stop with an error: "Cannot use `base:` with a PR number or bookmark target — `base:` implies the current workspace is already the correct bookmark. Pass `base:` alone, or pass the target alone and let scope detection resolve the base."

**If a PR number or GitHub URL is provided as an argument:**

Do **not** switch to the PR bookmark. Scope comes from GitHub read APIs plus optional local alignment when `@` already matches the PR head bookmark.
...
If no skip rule fires, fetch PR metadata **without switching workspace**:
...
Set `BASE:` to `pr:<number-or-url>` (logical marker — not a JJ revision). Set `UNTRACKED:` from `jj diff -r @ --name-only` on the **current** workspace (usually empty during PR-remote review).
...
1. `jj log -r @ -T 'bookmarks.join(" ") ++ "\n"' --no-graph` contains `headRefName`.
2. The PR is **not** cross-repository (`isCrossRepository` is false).
3. The PR head commit is contained in the local workspace ancestry: `jj log -r '<headRefOid>::@' --no-graph` returns the PR head. This confirms the working-copy commit actually carries the PR head (allowing unpublished local fixes layered on top) rather than an unrelated same-named bookmark.
...
- **`pr-remote`** — any check fails. The workspace is **not** the PR head; workspace file contents for changed paths may be stale or unrelated.

**Diff by scope mode** (do not mix remote and local diffs — contradictory hunks cause false positives):

- **`local-aligned`:** Resolve `<resolved-base-ref>` from `baseRefName` (fetch if needed). Compute `BASE=$(jj log -r "heads(::@ & ::<resolved-base-ref>)" --no-graph -T 'commit_id ++ "\n"' | head -n1)`, then set `FILES:` from `jj diff --from "$BASE" --name-only` and `DIFF:` from `jj diff --from "$BASE" --context 10` (includes described changes and working-copy changes on the PR bookmark). Do **not** call `gh pr diff` or append remote hunks — when unpublished fixes exist, the local workspace is canonical. Note in Coverage: `scope: local-aligned (PR; local workspace diff)`.
...
1. Best-effort fetch PR head without switching workspace: `jj git fetch --remote origin --branch <headRefName>` (substitute the PR head bookmark from metadata).
...
3. Best-effort fetch the PR base without switching workspace: `jj git fetch --remote origin --branch <baseRefName>`. When it succeeds, set `PR_BASE_REF=<baseRefName>@origin` — a **real JJ base ref** reviewers and validators use for file-level diffs (e.g. `data-migration-reviewer` runs `jj diff --from <PR_BASE_REF> -- db/schema.rb`/`structure.sql`). The `pr:<number-or-url>` logical marker in `BASE:` stays the scope marker; `PR_BASE_REF` is the diffable base. When the fetch fails, omit `PR_BASE_REF` and note in Coverage — schema-drift and other JJ diff checks fall back to diff hunks only and must **not** assume `main`.
...
Substitute the provided bookmark name as `<bookmark>`. Do **not** switch to `<bookmark>`.

If `jj log -r @ -T 'bookmarks.join(" ") ++ "\n"' --no-graph` contains `<bookmark>`, use the **standalone (current bookmark)** path below — same workspace, explicit bookmark name; do not use remote-only diff.

Otherwise diff the remote/local ref **without switching workspace**:
...
2. Else resolve `<bookmark>` as `<bookmark>@origin` or `<bookmark>` after `jj git fetch --remote origin --branch <bookmark>` when needed.
3. Resolve default base bookmark (same logic as standalone). Compute `BASE=$(jj log -r "heads(::<base-ref> & ::<bookmark-ref>)" --no-graph -T 'commit_id ++ "\n"' | head -n1)` and `jj diff --from "$BASE" --to "<bookmark-ref>" --context 10`.
4. If `<bookmark-ref>` cannot be resolved locally, stop: "Cannot diff bookmark `<bookmark>` without switching workspace. Move to that bookmark, pass its open PR URL/number, or review the current bookmark with `base:`."

On success for remote bookmark diff, set **branch-remote scope** (the existing scope label is retained for compatibility). The workspace is **not** `<bookmark>`. Include `<pr-scope-mode>branch-remote</pr-scope-mode>` and `<branch-head-ref><bookmark-ref></branch-head-ref>` in the Stage 4 review context bundle. Reviewers and Stage 5b validators must **not** Read/Grep workspace paths for files in `FILES:`. Inspect via `jj file show -r <bookmark-ref> <path>` or diff hunks only.

Produce:

```
echo "BASE:$BASE" && echo "FILES:" && jj diff --from "$BASE" --to "<bookmark-ref>" --name-only && echo "DIFF:" && jj diff --from "$BASE" --to "<bookmark-ref>" --context 10 && echo "UNTRACKED:" && jj diff -r @ --name-only
```

**If no argument (standalone on current bookmark):**

Apply the same base-detection logic as bookmark mode above, using the current bookmark (i.e., `gh pr view --json baseRefName,url` with no argument defaults to the current GitHub branch/bookmark when available).

If no base can be resolved, **stop**. Do not fall back to `jj diff` alone — a standalone review without the base would only show working-copy commit changes and silently miss earlier changes on the bookmark.
...
Using `jj diff --from "$BASE"` diffs the merge base against the working-copy commit, which includes described changes and current working-copy changes together.

**New file handling:** Always inspect `UNTRACKED:`. Newly-added paths that appear only in the working-copy commit are in scope when they are part of the JJ diff; unrelated untracked files outside JJ's snapshot are out of scope. When excluding paths, list them in Coverage and continue — never stop or prompt.
...
- **`local-aligned` / standalone / `base:`** — `DIFF_A="$BASE"` (a real revision/ref), `DIFF_B` empty (diffs base vs working-copy commit).
...
if [ -z "${DIFF_A:-}" ] || ! jj log -r "$DIFF_A" --no-graph >/dev/null 2>&1 \
  ...
  EXEC_LINES=$(jj diff --from "$DIFF_A" ${DIFF_B:+--to "$DIFF_B"} --stat -- '*.rb' '*.py' '*.js' '*.jsx' '*.ts' '*.tsx' '*.go' '*.rs' '*.java' '*.swift' '*.kt' '*.c' '*.cc' '*.cpp' '*.cs' '*.php' '*.ex' '*.exs' '*.scala' 2>/dev/null | awk '/\|/ {added+=gsub(/\+/,"+"); removed+=gsub(/-/,"-")} END{print added+removed+0}')
...
**Bookmark mode:** Run `jj log --no-graph -T 'commit_id.short() ++ " " ++ description.first_line() ++ "\n"' -r '${BASE}..<bookmark-ref>'` using the resolved merge-base and resolved bookmark ref from Stage 1. Use `<bookmark-ref>` (the resolved `<bookmark>@origin` or fetched ref), not the raw `<bookmark>` argument — a remote-only bookmark may have no matching local bookmark, so the raw name could fail or read a stale same-named local bookmark.

**Standalone (current bookmark):** Run:

```
echo "BOOKMARK:" && jj log -r @ -T 'bookmarks.join(" ") ++ "\n"' --no-graph && echo "CHANGES:" && jj log --no-graph -T 'commit_id.short() ++ " " ++ description.first_line() ++ "\n"' -r "${BASE}..@"
```
...
**Only resolve the cache when the workspace is the reviewed tree** — `local-aligned`, standalone, or `base:` scope (Stage 1). In `pr-remote` or `branch-remote` scope, **skip Stage 2c entirely**: the helper keys and derives from the local `@`, which is *not* the reviewed ref, so its profile would describe the wrong tree (e.g. `main`'s stack while reviewing a PR that changes manifests); reviewers work from the fetched refs/diff as those modes already require.
...
3. **Spawn validators with bounded parallelism.** One sub-agent per finding, dispatched independently using the validator template and the same bounded scheduler from Stage 4. Each validator receives:
   ...
   - The full diff
   - The scope mode and remote head ref, mirroring the Stage 4 reviewer bundle: inject `<pr-scope-mode>local-aligned | pr-remote | branch-remote</pr-scope-mode>` and, when set, `<pr-head-ref>...</pr-head-ref>` or `<branch-head-ref>...</branch-head-ref>`. The validator template defaults to local-aligned workspace inspection when these are absent, so omitting them in `pr-remote`/`branch-remote` makes validators verify findings against stale working-copy state — dropping valid findings or confirming false ones on the wrong revision.
...
**Act policy (bias to act).** Default to applying every finding that is a clear improvement and a reversible edit, regardless of severity. The work is a visible JJ diff that can be restored — so leaving a clean fix unapplied "to be safe" is the failure mode, not the safe choice. Decide by judgment, not a safety checklist:
...
Severity, confidence, and cross-reviewer agreement tell you what to do first and what to flag loudly — they do not gate the decision. There is no deny-list: downside is controlled after the fact (`jj restore` / `jj abandon` + visible diff + the JJ change checkpoint), not by a precondition.

**Scope invariant.** Apply only when the workspace *is* what was reviewed — `local-aligned` or standalone. In `pr-remote` / `branch-remote` the workspace is not the reviewed head; do not apply — report instead.
...
**Commit when the pre-review workspace was clean.** Before applying, note whether the workspace already had working-copy changes (`jj st`). The permanence gate is the **push**, not the commit — a local JJ change is private and reversible (`jj abandon` for an unwanted new change, or `jj restore` for file-level rollback).

- **Clean before the review:** after applying and verifying, commit the fixes as one isolated, review-labeled JJ change — `fix(review): <summary>`, or the repo's nearest convention if `review` isn't an allowed scope. Labeled and reversible, returning the workspace to a known state.
- **Dirty before the review:** apply but do **not** commit — the fixes interleave with the user's in-flight work and ride along with the change they were already going to make. The Applied section lists what changed.
...
```json
{
  ...
  "scope": {
    "base": "<merge-base revision, pr:NNN marker, or base: ref>",
    "bookmark": "<current bookmark name>",
    "head_revision": "<jj @ commit id>",
    "pr_url": "<url or null>",
    "files_changed": 0
  },
...
After Stage 6, stop. Never push, open PRs, or file tickets from this skill. In default (interactive) mode, Stage 5c has already applied and (on a clean pre-review workspace) committed the safe fixes; in `mode:agent` the review mutates nothing — the caller (for example `ce-work`) and the user apply fixes, file tickets, or accept residual risk using the report and artifact.
...
{
  "run_id": "<run-id>",
  "bookmark": "<jj bookmark(s) at dispatch time>",
  "head_revision": "<jj @ commit id at dispatch time>",
  "verdict": "<Ready to merge | Ready with fixes | Not ready>",
  "completed_at": "<ISO 8601 UTC timestamp>"
}
```

Capture `bookmark` and `head_revision` at dispatch time (no in-skill fixes will land afterward).
