...
The JJ-only policy permits one read-only exception: `ce-work`'s `unit_workspace_ignored.py` may inventory ignored files with exactly `git ls-files --others --ignored --exclude-standard -z --`, with cwd set to the absolute workspace root, `GIT_DIR` obtained from `jj git root` there, and `GIT_WORK_TREE` set to that root. Judge that helper against this scoped allowance and preservation of before/after ignored metadata and reporting; do not demand removal merely because it invokes Git. The allowance does not authorize Git execution in this resolver or other helpers.
