...
- If updating, revise only the still-relevant sections. Plans do not carry per-unit progress state — progress is derived from the working-copy change by `ce-work`, so there is no progress to preserve across edits
