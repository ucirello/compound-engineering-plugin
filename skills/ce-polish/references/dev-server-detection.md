...
**(c) No instruction-file port grep.** The script does not shell-search named instruction files for port references. Project instructions carry natural language that may mention ports in unrelated contexts, producing false positives that are hard to debug. Framework config files and `.env` are the reliable machine-readable sources; the AI Assistant still honors a dev-server port from the active project instructions already in context.
