...
Resolve the Git-backed workspace's GitHub URL once at compose time:

```bash
jj git remote list
```

Select the configured GitHub remote from that output, preferring `origin` when present. This `jj git` interoperability is necessary because the links target GitHub.

Apply linking to three reference shapes:

- **Repo-relative code/doc paths** (`services/foo.ts`,
  `<root>/solutions/bar.md`) → `<repo-url>/blob/<remote-default-or-commit-id>/<path>`,
  using the remote's established default bookmark or a resolved commit ID rather
  than hardcoding a reference name.
