...
- **Yes** -> ask for an existing dedicated bookmark (for example `feedback-sweep`) and its JJ remote (default `origin`). Require bookmark shape `^[A-Za-z0-9][A-Za-z0-9._/-]*$` and remote shape `^[A-Za-z0-9][A-Za-z0-9._-]*$`; reject `@`, quotes, whitespace, and revset operators. Explain that the lease is **push-gated**: before any source-side write, the sweep publishes and fetch-confirms its lease change on that exact tracked bookmark. The workspace must be dedicated and clean because the sweep creates `@` from the remote bookmark before acquiring the lease. Run `jj bookmark track <bookmark>@<remote>` once so `jj git fetch --remote <remote> --tracked` propagates remote updates.
