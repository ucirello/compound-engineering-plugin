...
Creation and resume are deliberately open at their edges. The managed store and `rocketclaw-handoff/v1` metadata are defaults that make RocketClaw handoffs easy to find; they do not restrict where a handoff may be created or what a user may resume from. A resume source may come from any person, AI Assistant, or system and may use any readable format.
...
4. Redact secrets, credentials, and unrelated personal information. Preserve operational paths only when the next AI Assistant needs them.
...
```bash
if jj workspace root >/dev/null 2>&1; then TMP_ROOT="$(jj workspace root)/.tmp"; else TMP_ROOT="$(pwd -P)/.tmp"; fi;
...
Choose whatever sections and document organization best communicate this particular session to the next AI Assistant. The headings below are examples of useful coverage, not a required or closed template: add new sections or combine, rename, reorder, and omit the examples when that makes the handoff clearer.

Include only what a fresh AI Assistant cannot safely infer, drawing from:
...
Treat a supplied local file, URL or page, pasted document, or other specific artifact as the user's selection. Read that source with an appropriate available capability, then follow **Orient from the selected source**. Do not require this skill as its source or require `rocketclaw-handoff/v1`; provenance, ownership, location, and format are not eligibility gates. Do not search for an alternative automatically. If the source cannot be read, explain the access problem and ask the user for a reachable source or different direction.
