...
(`$(jj workspace root)/.tmp/rocketclaw/brainstorm/<run-id>/grounding.md`, or local `.tmp/rocketclaw/brainstorm/<run-id>/grounding.md` when `jj workspace root` fails) — it gives
...
independent code review and applied fixes, describe the JJ changes, push the
bookmark with `jj git`, open the PR, and watch CI to
...
Share workflow) and returns the share URL. Surface
