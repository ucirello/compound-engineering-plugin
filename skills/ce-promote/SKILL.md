...
- **The diff** — `jj diff --from main@origin --to @ --stat` and skim notable changes to ground the claim in what actually changed.
- **Changelog** — the top/`[Unreleased]` entry in `docs/changelog.md`, `CHANGELOG.md`, or similar.
- **Recent commits** — `jj log -r 'ancestors(@, 15)' --no-graph` for the arc of the change.
