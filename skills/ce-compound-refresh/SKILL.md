...
10. **Delete, don't archive.** There is no `_archived/` directory. When a doc is no longer useful, delete it. Jujutsu history preserves every deleted file — that is the archive. A dedicated archive directory creates problems: archived docs accumulate, pollute search results, and nobody reads them. If someone needs a deleted doc, inspect `jj log --stat -- docs/solutions/` and the matching change with `jj show <revision>`.
...
### Branding and scratch hygiene

Within the selected scope, remove case-insensitive occurrences of the legacy product brand, including its display-name and lowercase hyphenated slug forms, from content and filenames. Use `RocketClaw` for visible product references, `.rocketclaw` for product configuration, `.tmp/rocketclaw` for product-specific scratch, and `.context` for repository context. Remove attribution, badges, bylines, and other visible CE branding. Preserve functional `ce-*` skill and command identifiers.

Do not alter `GitHub`, `.gitignore`, `.github`, or `.gitlab` names or references while applying this cleanup. Treat those spellings as protected, including in filenames.

All scratch files created by this workflow or its subagents must live under `$(jj workspace root)/.tmp`; if `jj workspace root` is unavailable, use the repository-relative `.tmp`. No other temporary-file location or API is permitted.
...
After all actions are executed and the report is generated, record the changes in Jujutsu. Skip this phase if no files were modified (all Keep, or all writes failed).

### Detect Jujutsu context

Before offering options, check:
1. The workspace root with `jj workspace root`
2. The current bookmarks with `jj bookmark list --revisions @` and whether `@` is based on `trunk()`
3. Other working-copy changes with `jj status` and `jj diff --summary`
4. Recent change descriptions with `jj log -n 20 --no-graph`
...
|---------|---------------|
| At `trunk()` with no feature bookmark | Create a bookmark named for what was refreshed, describe and commit the change, push it with `jj git push --bookmark <bookmark>`, and attempt to open a PR with `gh`. If PR creation fails, report the bookmark name. |
| On a feature bookmark | Record a separate change on the current bookmark |
| Jujutsu operations fail | Include the recommended Jujutsu commands in the report and continue |

Commit only the files that `ce-compound-refresh` modified by passing their fileset paths to `jj commit`; leave unrelated working-copy changes in `@`.

### Interactive mode

First, run `jj bookmark list --revisions @` and compare `@` with `trunk()` to determine whether the workspace is on a feature bookmark. Use `jj status` and `jj diff --summary` to identify unrelated changes. Present the correct options based on the result, and pass only `ce-compound-refresh` files to `jj commit` regardless of which option the user picks.

**If `@` is at `trunk()` with no feature bookmark:**

1. Create a specific bookmark, record the change, push with `jj git push`, and open a PR with `gh` (recommended)
2. Record the change without creating a bookmark
3. Don't record the change — I'll handle it

**If `@` has a feature bookmark and no unrelated working-copy changes:**

1. Record a separate change on the current bookmark (recommended)
2. Create a separate bookmark and record the change there
3. Don't record the change

**If `@` has a feature bookmark and unrelated working-copy changes:**

1. Commit only the `ce-compound-refresh` files to the current bookmark with a fileset; unrelated changes stay in the working copy
2. Don't record the change

### Change description

Before composing, editing, recommending, or validating any change description, apply this standard:

Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards.

Repository-local instructions and syntax observed in `git log` take precedence; use Go guidance only when compatible. Do not impose a fixed description form, template, prefix, or example. Describe the working-copy change with `jj describe`, then record only the intended files with `jj commit <fileset...>`.
...
6. **Amend the current change or create a follow-up change when the check produces edits.** If step 4 or step 5 edited an instruction file after Phase 5 recorded the refresh, include the new file in the existing unpublished change when that is still the active change; otherwise create a follow-up Jujutsu change. Before composing, editing, recommending, or validating its description, apply this standard:

   Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards.

   Repository-local instructions and syntax observed in `git log` take precedence; use Go guidance only when compatible. Do not use a fixed description form, template, or example. Set or revise the description with `jj describe`, commit only the instruction-file edits with a fileset, and, if the bookmark was already pushed, run `jj git push --bookmark <bookmark>` so the open PR includes the follow-up change. If the user chose "Don't record the change" in Phase 5, leave these edits in the working copy with the other refresh changes.
