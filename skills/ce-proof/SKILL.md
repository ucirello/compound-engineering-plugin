...
Every write to a Proof doc must carry an agent identity. Two fields provide it:

- **Machine ID (`by` on every op, `X-Agent-Id` header):** `ai:assistant` — stable, lowercase, and machine-parseable. Appears in marks, events, and the API response.
- **Display name (`name` on `POST /presence`):** `AI Assistant` — human-readable and shown in Proof's presence chips and comment-author labels.
...
`status: "accepted"` creates the suggestion mark and commits the change in one call. The mark persists as an audit trail with per-edit authorship and a reject-to-revert affordance. Works with `kind: "insert" | "delete" | "replace"`.
...
# Keep scratch in the workspace-local .tmp directory, with local .tmp as the fallback outside JJ.
ROOT="$(jj workspace root 2>/dev/null || pwd -P)"
TMP_DIR="$ROOT/.tmp"
mkdir -p "$TMP_DIR"
STATE_INDEX=0
while :; do
  STATE_TMP="$TMP_DIR/ce-proof-state.$$-$STATE_INDEX"
  (umask 077; set -C; : > "$STATE_TMP") 2>/dev/null && break
...
- Always include `by: "ai:assistant"` on every op and `X-Agent-Id: ai:assistant` in headers for consistent identity
