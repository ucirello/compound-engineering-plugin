...
- Self-locates the persona + schema via `BASH_SOURCE` (works from any CWD); derives the repo root from `Jujutsu`.
- Composes the peer prompt from the canonical persona brief + a JSON-only contract. Codex fetches its own diff with read-only `Jujutsu` inside its sandbox; Claude (which has no sandbox) is hard-denied `Bash`, so it gets the diff embedded and needs no shell. After capture, the script forces `reviewer = adversarial-<peer>` (the persona's example name `adversarial` would otherwise collide with the in-process reviewer and erase the cross-model agreement signal).
