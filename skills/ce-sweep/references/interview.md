...
Loaded by `SKILL.md` when `/ce-sweep` runs with no `feedback_sources` configured. Captures the setup that will be merged into `<workspace-root>/.rocketclaw/config.local.yaml` (the RocketClaw local config, ignored and machine-local) and re-read on every subsequent run.
...
- **Tracked in the workspace** (recommended when multiple agents or machines share a bookmark: one source of truth everyone reads and writes). Sets `sweep_state_path` to `docs/feedback-sweep/state.yml`.
- **Workspace-local scratch state** (solo setups; keeps sweep bookkeeping out of JJ history). Sets `sweep_state_path` to `<workspace-root>/.tmp/rocketclaw/ce-sweep/state.yml`. Outside a JJ workspace, use `<current-project-directory>/.tmp/rocketclaw/ce-sweep/state.yml`.

Let the user override the path if they want a different location. If durable user-curated state must be workspace-bound but untracked, place it under `.context/rocketclaw/ce-sweep/`; do not put disposable downloads or run artifacts there. If they pick local scratch state, note that another JJ workspace or a teammate's machine will not see it.
...
**Skip this section entirely if the user chose local scratch state in section 4** — shared-bookmark coordination only applies to tracked state.

**Ask:** "Is this a multi-agent setup where several JJ workspaces publish the sweep state through one shared remote bookmark? Answer yes only if more than one machine or agent writes and publishes the same state. Default is no: one workspace keeps its changes local."

- **No** (default) -> omit `sweep_shared_bookmark`. The single-writer lease serializes overlapping sweeps within one JJ workspace.
- **Yes** -> ask for the existing bookmark name and store it as `sweep_shared_bookmark`. Explain: the lease becomes **publish-gated**; before any source-side write, the sweep describes and publishes the lease acquisition through that bookmark with `jj git push`, then fetches and confirms its writer won. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Repository-local instructions and syntax observed in `git log` take precedence; use Go guidance only when compatible. This interview prescribes no type, scope, prefix, subject pattern, or example.

**Capture:** `sweep_shared_bookmark` (bookmark string when enabled; otherwise omit).
...
Merge the captured settings into `<workspace-root>/.rocketclaw/config.local.yaml`. Resolve the root with `jj workspace root`; outside JJ, use the current project directory.
...
- If `.rocketclaw/config.local.yaml` is not already covered by the repository's `.gitignore`, offer to add the entry before writing.
...
- `sweep_shared_bookmark` — from section 6; omit when coordination is local.
...
sweep_shared_bookmark: sweep-state                # omit for local-only coordination
~~~
