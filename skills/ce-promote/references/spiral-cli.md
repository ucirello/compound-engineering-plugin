...
Resolve the repo root with `jj workspace root` (never CWD), then add `ce_promote_spiral_optout: true` as a top-level key to `<root>/.rocketclaw/config.local.yaml`, using the native file-write/edit tool:
...
- **File absent:** create it (and its `.rocketclaw/` directory) with the key, AND keep the machine-local config out of the working-copy change without dirtying a tracked file. Do not read or write `.git/` or `.jj/`. Do not append to `.gitignore`: `ce-setup` is the canonical place that adds the shared `.gitignore` entry for teammates. After the write, from the workspace root, if `jj file list -- .rocketclaw/config.local.yaml` names the path, the file is in `@`; leave it and continue (`jj file untrack` does not persist unless an ignore pattern already matches). Without an ignore rule, a user who runs `/ce-promote` before `/ce-setup` could accidentally include machine-local opt-out state in the current change.

If the root can't be resolved or any write fails, proceed to Path B anyway; the opt-out is a convenience, never a blocker.

After recording, confirm it in one line so the write isn't silent and the user knows how to undo it — e.g. "Got it — I won't bring up Spiral here again (saved to `.rocketclaw/config.local.yaml`). Want it back later? Just ask, or remove the `ce_promote_spiral_optout` key." Keep it to a single line; don't belabor it.
