...
|-------|---------|--------|
| `diff:<revision-or-range>` | `diff:<revision>`, `diff:<revision-range>`, `diff:PR#<number>` | Forces diff mode on that change |
...
- An unrecognized `<word>:<word>` token, including one that resembles a commit convention, is not a flag — it passes through verbatim as request text.
...
- **Diff** — the request names a resolvable change: a commit ID, change ID, bookmark, PR, "the last commit", "what you just did", "this change".
