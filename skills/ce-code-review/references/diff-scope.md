...
In `pr-remote` / `bookmark-remote` scope these tiers inspect the working copy, which is not the reviewed head — apply the Remote scope rules above (`jj file show` / `jj file search -r <remote-head-ref>`) instead of local search.
