...
- **File absent:** create it (and its `.compound-engineering/` directory) with the key, AND make sure the machine-local config will not be included in a JJ change. Check whether the repo's ignore file (usually `<root>/.gitignore` in colocated JJ repos) already contains `.compound-engineering/*.local.yaml`; if it does not, append that entry. `ce-setup` is the canonical place that adds the shared ignore entry for teammates. Without any ignore, a user who runs `/ce-promote` before `/ce-setup` could accidentally include machine-local opt-out state in a change.
