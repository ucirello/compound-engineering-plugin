...
Job directory (durable state, the source of truth):
  <root>/<skill-storage-name>/<run-id>/jobs/<job-id>/
    Skill routing IDs retain `ce-`; storage names omit that prefix.
...
Environment overrides (defaults in parentheses):
  CE_PEER_JOBS_ROOT         base dir (<workspace-root>/.tmp, or <cwd>/.tmp
...
POSIX path is behaviorally unchanged:
  ...
  jobs root defaults to the workspace-local .tmp directory, owner-private.
...
def _workspace_scratch_root() -> str:
    ...
    try:
        proc = subprocess.run(
            ["jj", "--ignore-working-copy", "workspace", "root"], cwd=cwd,
            capture_output=True, text=True, timeout=10,
        )
        root = proc.stdout.strip() if proc.returncode == 0 else ""
    except (OSError, subprocess.TimeoutExpired):
        root = ""
    return os.path.join(root or cwd, ".tmp")
...
def _private_root_usable(path: str) -> bool:
    ...
    Creation is the probe; an inaccessible workspace scratch root is not
    replaced with global storage.
    """
    try:
        os.mkdir(path, 0o700)
...
def candidate_jobs_root_bases() -> list:
    """Lookup uses the same workspace-local or explicitly configured root."""
...
def skill_runs_root(skill: str) -> str:
    ...
    return os.path.join(jobs_root_base(), skill_storage_name(skill))


def skill_storage_name(skill: str) -> str:
    """Keep routing IDs intact while omitting their brand prefix in storage."""
    return skill[3:] if skill.startswith("ce-") and _is_safe_token(skill[3:]) else skill


def candidate_skill_runs_roots(skill: str) -> list:
    if skill == "ce-work" and os.environ.get("CE_WORK_RUNS_ROOT"):
        return [os.path.abspath(os.environ["CE_WORK_RUNS_ROOT"])]
    return [os.path.join(base, skill_storage_name(skill)) for base in candidate_jobs_root_bases()]
...
def cmd_start(args, worker_argv) -> int:
    ...
    ensure_owned_dirs(skill_dir if skill_dir != os.path.join(base, skill_storage_name(args.skill)) else base, jobs_root)
