...
2. **Otherwise, in a JJ workspace**, default to the diff for the current change (`jj diff -r @`) or, when a bookmark pointing at `@` has an obvious base bookmark or tracked remote bookmark, the diff from that base to `@`. This covers the common case of "simplify everything I've added on this feature bookmark before opening a PR." If no useful base ref exists, fall back to the current working-copy diff (`jj diff`).
