...
`ce-code-review` is invoked here with `mode:agent`, so it is review-only in this context: it reports findings and writes artifacts but does not mutate the workspace, jj history, bookmarks, GitHub state, or tracker state. The caller owns apply/fix policy.
...
```
ce-code-review mode:agent plan:<plan-path> base:<base-revset-or-ref>
```
...
- `base:` — when the diff base revset is already resolved in the current workspace; omit when reviewing a PR number/URL or standalone bookmark.
...
Default to applying every actionable finding. Applying is a reversible edit to a jj change; diffs are reviewed before finalization and tests run after, so leaving a clear fix unapplied "to be safe" is the failure mode. Bias to act:
...
There is no precondition safety checklist and no deny-list; downside is controlled by jj operation recovery, diff review, tests, and an accepted-change checkpoint.
...
3. **Parallel waves:** batches with disjoint file sets may run in parallel under the workspace/shared-directory rules in Phase 1 Step 4.
...
- Shared-directory fallback: do not describe, split, squash, rebase, or bookmark changes; return which `#` were applied or skipped and which files changed

**After each wave:** the orchestrator reviews `jj diff`, runs the required tests, and finalizes only the assigned finding changes. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Local instructions and observed history win; apply compatible Go clarity and structure. Do not impose fixed syntax, examples, or product-origin metadata. Repeat until all batches complete.
