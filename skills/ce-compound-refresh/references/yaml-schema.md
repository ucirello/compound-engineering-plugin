...
`schema.yaml` in this directory is the canonical contract for `.context/solutions/` frontmatter.
...
Default layout for a repo with no existing learnings. When `.context/solutions/` already has an established directory taxonomy, place the doc in the existing directory that covers this area (the corpus-first rule above) rather than creating a new directory from this table that nothing else uses.
...
fields like `description:` have their own quoting rules in the project's
active instructions.
