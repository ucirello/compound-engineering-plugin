...
skills run (plus `jj`/`gh` for the PR/CI tail, which it guards when absent). Pair every `gh` invocation that talks to the underlying git repo with `GIT_DIR=$(jj git root)` (and when a workspace cwd is in play, also run `gh` with that workspace root as cwd).
