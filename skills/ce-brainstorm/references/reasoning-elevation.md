...
Hand over the working context as **file paths the subagent reads itself**, never a re-narrated prose brief. Resolve the workspace root with `jj workspace root`, falling back to the current directory, and create **one private per-run handoff directory** under `<workspace-root>/.tmp/rocketclaw/` with `mktemp -d "<workspace-root>/.tmp/rocketclaw/elevation-XXXXXX"`; create the parent first. Write the prompt-file and every evidence file into *that* directory. On the Claude CLI route the worker grants the elevated model read access to only that one directory (via `--add-dir` on the prompt-file's parent), so the handoff files stay readable while other workspace scratch is not exposed:
