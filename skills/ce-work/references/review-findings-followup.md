...
`ce-code-review` is invoked here with `mode:agent`, so it is **review-only** in this context — it reports findings and writes artifacts and does not mutate the current JJ workspace, describe JJ changes, run `jj git push`, or file tickets. **The caller owns apply/fix policy.** (In its own default/interactive mode the review applies safe fixes itself; that path does not apply here.)
...
- `base:` — when the diff base is already resolved in the current JJ workspace; omit when reviewing a PR number/URL or standalone current working-copy change.
...
Default to applying every actionable finding. Applying is a reversible edit to the JJ working copy; diffs are reviewed before describing the JJ change (below) and tests run after — so leaving a clear, reversible fix unapplied "to be safe" is the failure mode, not the safe choice. Bias to act:
...
- Shared-directory fallback: do not describe or advance JJ changes — return which `#` were applied or skipped and which files changed

**After each wave:** orchestrator reviews diffs (scope = assigned `#` only), runs tests (`requires_verification: true` on any applied finding → at least targeted tests; multi-file → broader suite), describes the current JJ change (`fix(review): apply findings #…`) unless workspace-isolated subagents integrate per Phase 1. Repeat until all batches complete.
