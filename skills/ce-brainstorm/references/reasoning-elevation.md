...
3. **Config** — otherwise the per-skill key: `plan_model` for ce-plan, `brainstorm_model` for ce-brainstorm. Read it the **same way this skill's Phase 0.0 resolves `plan_output` / `brainstorm_output`**: reuse the workspace root already resolved, else run `jj workspace root` and use `pwd -P` if it fails, then apply the ordinary-key rule (`config.local.yaml` then `config.yaml`). Reuse the Phase 0.0 reads if still in hand. Ignore commented (`#`-prefixed) lines. A model alias → elevate to it; missing / commented / invalid / no file → off.
...
Hand over the working context as **file paths the subagent reads itself**, never a re-narrated prose brief. Resolve `WORKSPACE_ROOT="$(jj workspace root 2>/dev/null || pwd -P)"`, safely create the owner-private `$WORKSPACE_ROOT/.tmp`, then create one exclusive `$WORKSPACE_ROOT/.tmp/rocketclaw/ce-elevation-<run-id>` handoff directory with mode `0700`. Write the prompt-file and every evidence file into that directory. On the Claude CLI route the worker grants read access only to that directory, so other run artifacts are not exposed:
...
1. **Write the prompt-file into the private handoff directory.** Put the prompt-file and every evidence scratch file in the one workspace-local handoff directory from "Read-only posture and brief handoff" above. The worker grants read access to the prompt-file's parent, so co-location makes the evidence readable without exposing sibling runs. Build the prompt-file as the elevated model's brief: the instruction to interpret findings and author the plan (or generate approaches), plus the absolute paths of those co-located files. Reference those files inside the prompt-file, not as extra worker arguments.
