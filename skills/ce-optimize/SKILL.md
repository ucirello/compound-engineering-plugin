...
This skill reads prior learnings from the project's documented solutions location. Resolve that location from the project's active conventions when first needed; otherwise use `docs/solutions/` under `jj workspace root`. Pass the resolved path, not a configuration file, to any subagent.
...
The files under `<workspace-root>/.context/ce-optimize/<spec-name>/` are local persistent resume state, where `<workspace-root>` is `jj workspace root`; use `./.context/ce-optimize/<spec-name>/` only when no jj workspace root resolves. They survive local resumes on the same machine but are not part of bookmark history or remote operations unless the user exports them separately.
...
### File Locations (all under `<workspace-root>/.context/ce-optimize/<spec-name>/`)
...
6. Write the spec to `<workspace-root>/.context/ce-optimize/<spec-name>/spec.yaml`
...
Check whether the run bookmark exists:

```bash
jj bookmark list "optimize/<spec-name>"
```

**If the bookmark exists**, check for an existing experiment log at `<workspace-root>/.context/ce-optimize/<spec-name>/experiment-log.yaml`.
...
- **Fresh start**: rename the old bookmark to `optimize-archive/<spec-name>/archived-<timestamp>`, clear the experiment log, and create a new run bookmark from the requested starting revision

### 0.5 Create Optimization Bookmark and Persistent State

Create `optimize/<spec-name>` at the baseline revision for a fresh run, or start a new working-copy change from the existing bookmark when resuming:

```bash
jj bookmark create "optimize/<spec-name>" -r "<baseline-revision>"
jj new "optimize/<spec-name>"
```

Create the persistent run-state directory:
```bash
WORKSPACE_ROOT="$(jj workspace root 2>/dev/null || pwd)";
mkdir -p "$WORKSPACE_ROOT/.context/ce-optimize/<spec-name>/"
```
...
- Ask the user to move, describe, or abandon those changes before proceeding
- If a description is composed, edited, validated, or recommended, inspect local description history with `jj log`. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Local syntax and history win; apply only compatible Go guidance without imposing fixed syntax or examples.
- Do NOT continue until the workspace is clean for in-scope files
...
1. Create the experiment log file at `<workspace-root>/.context/ce-optimize/<spec-name>/experiment-log.yaml`
...
**Workspace backend:**
1. Create an experiment workspace from the run bookmark:
   ```bash
   SKILL_DIR="<absolute path of the directory containing this SKILL.md>";
   WORKSPACE_PATH=$(bash "$SKILL_DIR/scripts/experiment-worktree.sh" create "<spec_name>" <exp_index> "optimize/<spec_name>" <shared_files...>)
   ```
2. Record the workspace change ID for later description, rebase, bookmark, and abandon operations:
   ```bash
   CHANGE_ID=$(jj -R "$WORKSPACE_PATH" log -r '@' --no-graph -T 'change_id')
   ```
3. Apply port parameterization if configured (set env vars for the measurement script)
...
5. Dispatch a subagent with the filled prompt, working in the experiment workspace

**Codex backend:**
1. Check the environment guard; do not delegate outside a jj workspace:
   ```bash
   jj workspace root
   ```
2. Fill the experiment prompt template
3. Atomically reserve a run-local prompt directory under the run's `.context/ce-optimize` namespace and write the filled prompt there
4. Dispatch via Codex:
   ```bash
   WORKSPACE_ROOT="$(jj workspace root 2>/dev/null || pwd)";
   PROMPT_ROOT="$WORKSPACE_ROOT/.context/ce-optimize/prompts";
   mkdir -p "$PROMPT_ROOT";
   PROMPT_DIR="$PROMPT_ROOT/exp-<iteration>-$$";
   while ! (umask 077 && mkdir "$PROMPT_DIR") 2>/dev/null; do PROMPT_DIR="$PROMPT_ROOT/exp-<iteration>-$$-$RANDOM"; done;
   codex exec - < "$PROMPT_DIR/prompt.txt" 2>&1
   ```
...
5. **If gates pass AND primary type is `judge`**:
   - **Independence gate — check before dispatching.** A judge must not have authored the hypothesis or run the experiment it is scoring, and must not see other judges' results; that independence is what makes these scores usable as a keep/reject gate. If the host exposes no way to dispatch judges as separate agents, do **not** score inline: mark the experiment's outcome `error` with the reason (judges undispatchable), skip judge evaluation exactly as a failed degenerate gate does, and continue to the log-and-append step so the entry is still written to disk. An experiment stopped here never carries judge metrics, so it is not eligible to become `best` and does not enter the keep/reject comparison — it is unmeasured, not poor-scoring. Report the blocker to the user at the batch summary.
...
3. **If best improves on current best: KEEP**
   - Confirm `jj diff --summary` in the experiment workspace contains only mutable-scope changes; if no eligible diff remains, treat the experiment as non-improving and abandon its change
   - Inspect the relevant local description history with `jj log`. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards.
   - Local syntax and history win; apply compatible Go guidance without imposing a fixed prefix or format. Set the experiment change description with `jj describe`, then read it back with `jj log -r @ --no-graph -T description` and correct it before integration if it violates those standards.
   - Move `optimize/<spec-name>` to the winning change with `jj bookmark set`, then clean up the winner's experiment workspace. The bookmarked change is the durable artifact and the baseline for subsequent batches.
...
4. **Check file-disjoint runners-up** (up to `max_runner_up_integrations_per_batch`):
   ...
   - If disjoint: rebase the runner-up change onto the new run bookmark with `jj rebase`, then re-run full measurement in its workspace
   - Inspect the relevant local description history with `jj log`. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards.
   - Local syntax and history win; apply compatible Go guidance without imposing a fixed prefix or format. Set or edit the rebased change description with `jj describe`, then read it back with `jj log -r <runner-up-change> --no-graph -T description` and correct it before recommending or moving the bookmark.
   - If combined measurement is strictly better: move the run bookmark to the runner-up change (outcome: `runner_up_kept`), then clean up that runner-up's experiment workspace
   - Otherwise: abandon the rebased runner-up change, log it as "promising alone but neutral/harmful in combination" (outcome: `runner_up_abandoned`), then clean up its experiment workspace
...
6. **Abandon all others**: clean up experiment workspaces, abandon their changes, and log them as `abandoned`
...
4. **Write strategy digest** to `<workspace-root>/.context/ce-optimize/<spec-name>/strategy-digest.md`:
...
- Abandon the experiment change and clean up its workspace
...
The optimization bookmark (`optimize/<spec-name>`) is preserved with all kept changes in its history.
The experiment log and strategy digest remain in local `<workspace-root>/.context/ce-optimize/...` persistent state for resume and audit on this machine only; they do not travel with the bookmark.

Present post-completion options via the platform question tool:

1. **Run `ce-code-review`** on the cumulative `jj diff` from baseline to the run bookmark. Load the `ce-code-review` skill in a workspace based on the optimization bookmark (interactive or `mode:agent`). To land eligible fixes before the next option, apply the mechanical-apply bar below.

   **Mechanical-apply bar:** apply any finding with a concrete `suggested_fix` that is a clear, reversible improvement; push back (keep, don't apply) when the reviewer is wrong, noting why. Defer anything whose right fix needs a design or product decision (architecture direction, contract shape, behavior change needing sign-off) and any finding with no concrete fix to act on — surface what was deferred. Confirm evidence still matches at `file:line` before editing. After applying, run tests (at least targeted tests for what changed; broader suite for multi-file edits). Leave the resulting jj change local for the Create PR option.
   Inspect the relevant local description history with `jj log`. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards.
   Before recommending the change for the PR, use local syntax and history plus compatible Go guidance to compose or edit its `jj describe` text, then validate the stored description; do not impose a fixed prefix or example.
2. **Run `ce-compound`** to document the winning strategy as an institutional learning.
3. **Create PR** from the optimization bookmark to the default bookmark; `gh` and GitHub remain the PR interface.
...
# Remove transient batch artifacts
WORKSPACE_ROOT="$(jj workspace root 2>/dev/null || pwd)";
rm -f "$WORKSPACE_ROOT/.context/ce-optimize/<spec-name>/strategy-digest.md"
```
