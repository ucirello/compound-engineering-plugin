...
`ce-code-review` is invoked here with `mode:agent`, so it is **review-only** in this context — it reports findings and writes artifacts and does not mutate the workspace, describe or finalize changes, move bookmarks, publish, or file tickets. **The caller owns apply/fix policy.** Standalone review is also report-only unless local apply was explicitly authorized.
...
- `base:` — when the diff base is already resolved in the current workspace; omit when reviewing a PR number/URL or standalone current bookmark.
...
- Stable finding `#` — reuse in residual sinks and subagent prompts; include it in Jujutsu descriptions only when active project conventions require that traceability

## What to apply

Default to applying every actionable finding. Applying is a reversible edit to a tracked tree; diffs are reviewed before change finalization and tests run after, so leaving a clear, reversible fix unapplied "to be safe" is the failure mode, not the safe choice. Bias to act:
...
There is no precondition safety checklist and no deny-list — a code-review fix is a reversible edit, so downside is controlled after the fact (diff review, tests, and the Jujutsu change checkpoint), not by gating the apply.
...
**Orchestrator owns:** parse review output → **eligibility filter on JSON fields only** → build batches → dispatch fix subagents → review diffs → tests → finalize Jujutsu changes → Residual Work Gate.
...
3. **Parallel waves:** batches with **disjoint file sets** may run in parallel (same linked-workspace/shared-directory rules as Phase 1 Step 4 in `ce-work` SKILL.md).
...
- Shared-directory fallback: do not describe or finalize changes — return which `#` were applied or skipped and which files changed

**After each wave:** the orchestrator reviews diffs (scope = assigned `#` only), runs tests (`requires_verification: true` on any applied finding -> at least targeted tests; multi-file -> broader suite), and finalizes the Jujutsu change unless workspace-isolated subagents integrate per Phase 1. Runtime project instructions and history read via `jj log` win. Apply compatible Go quality guidance. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Use a neutral dynamic description derived from the applied findings; do not use a fixed message template. Repeat until all batches complete.
