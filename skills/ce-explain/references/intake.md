...
|-------|---------|--------|
| `diff:<revision-or-range>` | `diff:<change-id>`, `diff:<bookmark>..@`, `diff:PR#<number>` | Forces diff mode on that change |
...
- **Diff** — the request names a resolvable JJ change: a change ID, bookmark, PR, "the previous change", "what you just did", "this change".
