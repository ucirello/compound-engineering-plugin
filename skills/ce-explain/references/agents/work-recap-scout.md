...
1. **JJ activity** — `jj log` over the window (descriptions, change IDs, commit IDs, dates, authors), and for substantial changes, a stat-level view from `jj diff --stat -r <revision>`. Group obviously related changes (a bookmark's changes, a fix and its follow-ups) rather than listing them flat.
...
- What changed, in one line, with the change/commit IDs or PR number and date
