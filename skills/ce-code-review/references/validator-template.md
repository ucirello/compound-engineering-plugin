...
When remote scope is in context, do not inspect the workspace copy of {finding_file}. Use `jj file show -r <remote-head-ref> {finding_file}` when a remote head revision is set; otherwise use diff hunks only.

When scope is local-aligned, use read and search tools plus `jj file annotate` to inspect the cited code and related behavior.
</scope-context>
...
2. **Is the issue introduced by THIS diff?** Use `jj file annotate`, `jj log`, or diff inspection against the reviewed revision. If the cited line predates this PR's changes and the diff does not interact with it, the finding is pre-existing.
...
- Do not edit files, finalize JJ changes, move bookmarks, or push. You are operationally read-only.
