...
Environment overrides (defaults in parentheses):
  CE_PEER_JOBS_ROOT         base dir (<workspace-root>/.tmp, or local .tmp
                             outside a JJ workspace)
  CE_WORK_RUNS_ROOT         parent RocketClaw Work dir containing all <run-id>/ dirs
...
POSIX path is behaviorally unchanged:
  ...
  jobs root defaults under the workspace's .tmp on Windows too, owner-private.
...
def _workspace_root() -> str:
    ...
    try:
        result = subprocess.run(["jj", "workspace", "root"], cwd=current,
                                capture_output=True, text=True, timeout=10)
    except (OSError, subprocess.TimeoutExpired):
        return current
    return os.path.abspath(result.stdout.strip()) if result.returncode == 0 and result.stdout.strip() else current
...
def _private_root_usable(path: str) -> bool:
    ...
    Creation and ownership checks probe the workspace-local scratch directory.
    """
    try:
        os.mkdir(path, 0o700)
...
def candidate_jobs_root_bases() -> list:
    """Lookup uses the same workspace-local or explicit root as creation."""
    configured = os.environ.get("CE_PEER_JOBS_ROOT")
    if configured:
        return [os.path.abspath(configured)]
    return [os.path.abspath(DEFAULT_ROOT)]
