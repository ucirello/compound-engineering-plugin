...
All other persona subagents and local prompt assets use the platform's configured balanced tier when the dispatch primitive supports an explicit override. See the Spawning subsection below.
...
**Tier override at dispatch time — this is a correctness guarantee, not attribution.** Omitting the override on a high-cost parent silently runs every reviewer at that tier. The tier is a deterministic function of the persona, so record each reviewer's tier in an internal working list during Stage 3 and honor it without rendering tier or harness labels to the user:
...
- **Balanced tier** — every other persona and local prompt asset: use the platform's balanced tier only when the active dispatch primitive exposes an explicit supported selector. Otherwise omit the override and inherit the parent; task wording alone does not select a tier.
...
Read-only here means **non-mutating**, not "no shell access." Reviewer sub-agents may use non-mutating inspection commands when needed to gather evidence or verify scope, including `jj diff`, `jj file show`, `jj file annotate`, `jj log`, and `gh pr view`. When a finding's claim depends on line history (`pre_existing`, intent, introduced-by-this-diff, or P0/P1 confidence that depends on authorship/age), reviewers are expected to attach one concise provenance evidence line from targeted annotation/log evidence on the cited line — additional to the quote-the-line gate, never a full-file dump, and omitted when the finding is justified from the diff alone. In **`pr-remote`** or **`bookmark-remote`** scope, inspect changed files via `jj file show -r <remote-head-ref> <path>` or diff hunks; do not Read/Grep workspace paths for files in scope, and gather history against that reviewed head ref. They must not edit project files, change the working-copy revision, describe or commit changes, push, create PRs, or otherwise mutate repository state.
...
The peer return enters Stage 5 as reviewer `adversarial-external`, like any persona artifact. A pass that never started is recorded as not run (or as the in-process fallback when selected); a started peer that fails, times out, dies, or is reaped is named with its terminal state rather than vanishing silently.
