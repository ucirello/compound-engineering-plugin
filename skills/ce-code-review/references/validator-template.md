...
When scope is local-aligned (default), use read tools (Read, Grep, Glob, JJ history inspection) to inspect the cited code and its callers, guards, middleware, or framework defaults that might handle the concern elsewhere.
</scope-context>
...
2. **Is the issue introduced by THIS diff?** Use JJ history or diff inspection. If the cited line predates this PR's commits and the diff does not interact with it (does not call into it, does not change its callers in a way that newly exposes the issue), the finding is pre-existing — not validated for externalization regardless of whether it is a real issue.
