...
**OpenCode V2 routing.** Resolve explicit subagent model and harness settings from `.rocketclaw/config.local.yaml` over `config.yaml`; those settings win. With no explicit subagent or alternative-harness route, when the current host is OpenCode and `opencode.models` plus native subagent model selection are available, discover exact model IDs and variants and dispatch natively using `provider/model#variant`. Preserve the session-capability tier for the three highest-stakes personas and choose a discovered mid-tier model for the others. Do not guess IDs or silently substitute the current model when a requested tier is unavailable; report degraded coverage. This rule also governs the trivial-PR classifier, validator, and finish leaves. Other hosts retain their supported dispatch behavior.
...
Local review stays in the harness's in-turn batch and uses its native waiting and result delivery. Do not turn it into a detached background bash/CLI delegate with foreground polling or scheduled wakeups (the retired ce-work-beta failure mode). The cross-model peer is the only delegate allowed to overlap this batch: its shell route stays detached, and its OpenCode native route uses the host's lifecycle. Never insert shell no-ops (`echo waiting`, `noop`, `yield turn`, `end turn`, `true`, or sleeps), detached status polls, scheduled wakeups, or user-facing "still waiting" turns to await reviewers.
...
8. **For `data-migration` only:** the concrete resolved review base from Stage 1, wrapped in `<review-base>`, and reviewed endpoint in `<review-head>` (`@` for local scope; the verified remote head otherwise). For remote scope use `PR_BASE_REF`, never the logical `pr:` marker; an unavailable endpoint means supplied hunks only. Schema drift checks never assume `main`.
...
**Exception: tree-mutating reviewers.** A persona whose method mutates the tree (`testing`, when it runs mutation testing) operates only on a faithful snapshot of the reviewed tree, never the shared workspace. Pass the exact reviewed commit ID and compare the isolated workspace's `jj log -r @ --no-graph -T commit_id` before mutating. Discover workspace membership through `jj workspace list` and `jj workspace root --name NAME`, not metadata paths. A harness-created workspace may start at an unrelated default revision; on any mismatch, or if isolation is unavailable, use a scratch copy under the original workspace's `.tmp` that preserves the reviewed tree and its in-flight changes. Read-only siblings stay on the shared workspace.

Read-only here means **non-mutating**, not "no shell access." Reviewer subagents may inspect with `jj diff`, `jj file show`, `jj file annotate`, `jj log`, and `gh pr view`. Run JJ commands with cwd at the supplied absolute workspace root; set `GIT_DIR=$(jj git root)` there for GitHub CLI calls. When a finding's claim depends on line history (`pre_existing`, intent, introduced-by-this-diff, or P0/P1 confidence that depends on authorship/age), attach one concise provenance evidence line from targeted annotation/history on the cited line. It supplements the motivating-line quote, is never a full-file dump, and is omitted for diff-local findings. In **`pr-remote`** or **`branch-remote`** scope, inspect changed files via `jj file show -r <remote-head-ref> <path>` or supplied hunks; gather provenance against that reviewed ref, never workspace contents. Do not edit project files, switch revisions, commit, push, create PRs, or otherwise mutate repository state.
...
**RocketClaw generic conditional local prompt assets** (`agent-native-reviewer`, `learnings-researcher`) are dispatched only when selected by Stage 3, through the same deterministic foreground batch dispatch as the structured personas. Read their prompt files from `references/personas/`, then give them the same review context bundle the personas receive: entry mode, any PR metadata gathered in Stage 1, intent summary, review base bookmark name when known, `BASE:` marker, file list, diff, and `UNTRACKED:` scope notes.
