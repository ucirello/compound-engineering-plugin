...
The required protocol identity is `ai:assistant`; render its human-readable name as `AI Assistant` in artifacts and source metadata.
...
- `sweep_state_path` — path to the state file, established at setup; fallback `docs/feedback-sweep/state.yml`. The committed path participates in the sweep change and must not be gitignored. A path under `<workspace-root>/.tmp/rocketclaw/` is machine-local, must be gitignored, and never participates in the sweep change.
- `sweep_lease_ttl_minutes` — single-writer lease staleness threshold; default `60`. Passed to `lease-acquire` in 2a.
- `sweep_shared_bookmark` — bookmark name when committed state is synchronized across multiple JJ workspaces or machines (see 2a topology); omit for local-change mode.
...
- `gh pr view <validated-ref> --json mergedAt,baseRefName` (merged, base is the default bookmark), or `jj log -r '<validated-sha> & ::<default-bookmark>' --no-graph` (non-empty only when the change is an ancestor of the default bookmark).
...
- **Describe and finish the change.** Run `jj status` and `jj diff --summary`; require `@` to contain only `docs/plans/feedback-sweep-plan.md` plus `<state>` when it is the committed state path. Machine-local state under `<workspace-root>/.tmp/rocketclaw/` must remain ignored and absent from the diff. If `@` contains unrelated paths, report the isolation problem and skip describing or publishing rather than including them; there is no staging step. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Determine syntax from active project instructions and actual `git log`; those sources always win over compatible Go guidance. Do not impose a fixed prefix, type, scope, subject/body shape, template, or example. Use that composed description with `jj describe`, then `jj new`; a describe/new failure is reported, not fatal. In local-change mode, never push. When `sweep_shared_bookmark` is set, run `jj git fetch`, rebase the completed sweep change onto the fetched shared bookmark when needed, move the bookmark to that change with `jj bookmark set <bookmark> -r <sweep-change>`, and publish with `jj git push --bookmark <bookmark>`.
