...
3. Invoke the `ce-simplify-code` skill on the bookmark/change diff.
   ...
   Do not create or describe a JJ change in this step. `ce-simplify-code` leaves its changes in the working tree; step 4's review scopes the working tree (current-change edits included), and step 8's `ce-commit-push-pr` describes whatever remains. Describing here would sweep any still-undescribed `ce-work` edits into a misleading `refactor` change and could stall on a tree that never goes clean.
...
5. **Apply and persist review fixes** (REQUIRED after step 4, before residual handoff)
   ...
   Load `references/review-followup.md` and execute its apply step (mechanical apply + describe/push bookmark when changes exist). Do not proceed to the residual handoff, run browser tests, or output DONE while eligible review fixes remain only in the working tree undescribed.
...
6. **Autonomous residual handoff** (only when step 4 reported one or more actionable `downstream-resolver` findings not applied in step 5; skip when it reported `Actionable findings: none.`)
   ...
   6. If no open PR exists, create a tracked fallback file at `docs/residual-review-findings/<bookmark-or-change-id>.md` containing the composed section and the source PR-review run context. Commit only that file with `jj commit docs/residual-review-findings/<bookmark-or-change-id>.md -m "docs(review): record residual review findings"`, and push the current bookmark **when a remote is configured** (per the shipping precondition). Resolve a writable remote dynamically: prefer `origin` when present in `jj git remote list`, otherwise choose the first configured remote. If the current change has no bookmark, create one with `jj bookmark create <bookmark> -r @`. Then run `jj git push --remote <remote> --bookmark <bookmark>`. If there is no remote at all, do not push — the local JJ change is the durable sink. This is the durable no-PR sink. Do not output DONE until the residual findings are durable: either the existing PR body has been updated, or this fallback file change has been made (pushed when a remote exists, local when none). A push that fails when a remote exists is a stop-and-report; never retry a push, or block DONE, when no remote exists.
