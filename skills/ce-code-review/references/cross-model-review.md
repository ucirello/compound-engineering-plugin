...
- `<run-dir>` = the Stage 4 workspace-local run dir (`<workspace-root>/.tmp/rocketclaw/ce-code-review/<run-id>/`). The script writes `adversarial-<peer>.json` there.
...
- Self-locates the persona + schema via `BASH_SOURCE` (works from any CWD); derives the repo root from `jj workspace root`, with a local-directory fallback.
