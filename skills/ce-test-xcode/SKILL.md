...
Ask the user with the harness's blocking question capability. If its schema is not loaded, discover or load the capability first. Fall back to numbered options in chat only when no blocking question capability exists or the call errors. Never silently skip the question:
