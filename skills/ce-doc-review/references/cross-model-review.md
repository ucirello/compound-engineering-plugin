...
- **opencode2:** Distinct from `opencode`; not backward compatible. Binary `opencode2`; `opencode2 run --format json --file PATH` plus `--model provider/modelname#variant` when a model is set. No `--dir`, no `--variant`, no v1 `OPENCODE_CONFIG_CONTENT` denylist. CWD is the empty peer workdir. This review pass does not pass `--auto` (cooperative read-only).
