...
1. **JJ workspace** — confirm the current directory is in the repo with `jj root`
2. **GitHub repository** — detect the repository. Prefer the upstream repository over a fork when issue workflows do that. Use `gh repo view --json nameWithOwner` to confirm the resolved repo.
