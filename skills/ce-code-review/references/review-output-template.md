...
**Scope:** fork point with `trunk()` -> working-copy commit (14 files, 342 lines)
...
Change status: described and advanced because `@` was empty before review.
...
Detail lines for Pre-existing and history-dependent P0/P1 findings may include the same short change-id provenance string the artifact `evidence` carries when that history was load-bearing. Do not dump full-file annotations into the report.
