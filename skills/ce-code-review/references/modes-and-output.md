...
| `mode:report-only` | `mode:report-only` | **Deprecated -- ignored.** Former no-artifacts mode; default behavior is review-only without changing the working-copy revision |
...
| `base:<revision>` | `base:<change-or-bookmark>` | Diff base in the **current workspace** (explicit; skips automatic base detection) |
...
- Multiple incompatible scope selectors appear together, such as `base:` and a PR number/bookmark target; `base:` means review the current workspace against this base
...
| **`mode:agent`** | One JSON object (see ### JSON output format below) + the same `<workspace>/.tmp/rocketclaw/code-review/<run-id>/` artifacts |
