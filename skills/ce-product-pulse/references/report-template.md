...
Preserve the required sections, metrics, source facts, safety constraints, and output path. The template and examples define content and order rather than fixed message syntax. Do not add branding, generated-by text, or creator, model, provider, tool, agent, runtime, workflow, co-author, or other attribution.
