...
```json
{
  ...
  "head_sha": "<commit_id of the fix change, when fixed>",
  "branch": "<bookmark the fix was committed on, when fixed>",
  "pre_fix_scope": {
    "head": "<commit_id of @ before the fix change>",
    "dirty_files": ["<files already modified or untracked before Phase 3 that are not fix-owned; left untouched>"],
    "commits_beyond_base": ["<changes the bookmark carried beyond the trunk bookmark before the fix, oldest first; empty on a fresh bookmark>"],
