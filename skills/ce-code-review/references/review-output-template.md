...
When `mode:agent` is active, **do not** emit the markdown table report above. Emit **one parseable JSON object** as the primary response and write the same payload to `review.json` under `.tmp/rocketclaw/ce-code-review/<run-id>/`.
