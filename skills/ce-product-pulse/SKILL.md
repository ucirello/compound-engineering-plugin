...
Preserve each site's required facts, routing semantics, metrics, and safety constraints. Headings, prompts, templates, and examples define required content and interaction rather than fixed message syntax. Do not add branding, generated-by text, or creator, model, provider, tool, agent, runtime, workflow, co-author, or other attribution.

## Interaction Method

Default to the platform's blocking question capability. If its schema is not loaded, discover it through the harness's tool-discovery capability first. Fall back to numbered options in chat only when no blocking question capability exists or the call errors, not merely because discovery is required. Never silently skip the question.
...
This skill writes pulse reports under `<root>/pulse-reports/`. Resolve `<root>` when you first compose a `<root>/` path (per the block below), never before you need it. A write to `<root>/...` and a read of `<root>/solutions/` both count as composing a `<root>/` path, so either one triggers resolution; only a run that touches no `<root>/` path at all -- a scratch-only or no-workspace flow -- skips it. If temporary artifacts are needed, place them under `<workspace-root>/.tmp/rocketclaw/ce-product-pulse/<run-id>/`, falling back to the current local directory's `.tmp/rocketclaw/ce-product-pulse/<run-id>/` when no JJ workspace is available; never use OS-global temporary storage.

<!-- rocketclaw-docs-root:start -->
**Resolve the artifact root `<root>` before composing any artifact path.**

- **Read** `docs_root` from `<workspace-root>/.rocketclaw/config.yaml` only (`<workspace-root>` = `jj workspace root`, falling back to the current local directory when no JJ workspace is available). Do not read it from `config.local.yaml`. Unset -> `<root>` is `docs`, exactly as before.
...
**Resolve ordinary CE yaml keys from the two workspace files.**

- **Read** `<workspace-root>/.rocketclaw/config.local.yaml`, then `config.yaml` (`<workspace-root>` = `jj workspace root`, falling back to the current local directory when no JJ workspace is available). Missing files are skipped. Ignore rules do not change resolution.
- **Win** with the first active (non-commented) value. For scalars, empty is unset; an invalid value continues to the next layer, then the skill default. For lists and maps, a present key — including an empty list or map — replaces the whole key.
- **Do not** use this rule for `docs_root` — that key is `config.yaml` only.
<!-- ce-config-layers:end -->

**Read config.** Resolve `<workspace-root>` at runtime with `jj workspace root`, falling back to the current local directory only when JJ is unavailable or the directory is not a JJ workspace, then apply the ordinary-key rule above. If `pulse_product_name` is unset in both layers, treat this as a first run. Otherwise extract values for the `pulse_*` keys listed under "Config keys" below.
...
Write the captured config to `<workspace-root>/.rocketclaw/config.local.yaml` as flat `pulse_*` keys, using the schema in `references/interview.md` under "Config file shape". Resolve the workspace root with `jj workspace root`; if that fails, use the current local directory. To write: (1) if the file or directory does not exist, create `.rocketclaw/` and write the YAML file; (2) if the file exists, merge new keys into the existing YAML, preserving any non-pulse keys (e.g., `plan_*`) untouched. If `jj file list .rocketclaw/config.local.yaml` reports the file, offer to add it to the workspace's ignore rules and run `jj file untrack .rocketclaw/config.local.yaml` after approval. Show the resulting pulse block to the user in chat and offer one round of edits.
...
- Does not report "what shipped." Shipped work lives in the issue tracker and change history, not here. Pulse is strictly about user experience and system performance.
