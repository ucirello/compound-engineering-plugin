...
5. Do NOT describe, bookmark, or integrate the JJ change (the orchestrator handles a winning change)
...
- For Codex dispatch: write the filled template under `$(jj workspace root)/.tmp/optimize/<spec-name>/`, using local `.tmp` if no JJ workspace root is available, and redirect it to `codex exec -` via stdin.
