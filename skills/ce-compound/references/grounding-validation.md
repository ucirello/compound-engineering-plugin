...
- **Code-behavior claims** (enum values, status semantics, limits, defaults) verify against the **local workspace at `@`**; they describe what this session produced and verified here.
- **Merge-state claims** ("fixed in <pr-number>", "landed", "shipped") verify against **remote truth**. The workspace may predate a merge, so `gh pr view` (or the tracker equivalent) is primary and Jujutsu revset reachability is the fallback. The script's `INFO: working-copy revision is N revisions behind …` line tells you how much to distrust local state for this category.

Before running the script, optionally run `jj git fetch --remote <remote>` (best-effort; skip silently on failure or offline because the network is never a correctness dependency). When remote state cannot be checked, keep the claim, add an as-of qualifier, and record degraded verification in the run report.
...
| path missing here, exists at remote bookmark | Stale workspace | Verify the claim against that bookmark; annotate if the doc implies the file is present locally |
...
| commit ID reachable from `@` only | Local-only revision; ID may change after `jj rebase` or `jj squash` | Replace with the PR number |
| commit ID reachable from remote bookmark only | Workspace predates the merge | Keep, with a temporal qualifier; verify the landed claim via `gh` |
| commit ID exists but is unreachable | Rewritten revision | Replace with the PR number |
...
If the script cannot be resolved on this platform, apply its checks manually at the same scope: scan the body for cited paths that do not exist, hexadecimal commit IDs, `Learning(s) N` / `{{…}}` scaffold, and broken relative links. Note in the run output that the check was manual; do not silently skip.
...
Dispatch **one generic read-only AI Assistant** covering the written solution doc plus any `CONCEPTS.md` entries added or edited this run (Phase 2.4's entries are claims too). Build its prompt from this template:

```
Actor: ai:assistant

Validate documentation about to enter a permanent
knowledge store. You are read-only: never edit files. Inspect with Read,
Grep, Glob, `jj workspace root`, `jj status`, `jj diff`, `jj log`, and gh when available.
...
2. MERGE-STATE CLAIMS — assertions that a change landed ("fixed in",
   "merged", "shipped in", "resolved by <pr-number>"). Primary check: gh pr view
   <pr-number> --json state,mergedAt,baseRefName (remote truth). Fallback:
   `jj log` reachability from the applicable remote bookmark. Verdict: verified,
