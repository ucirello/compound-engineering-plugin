...
If you use mutation testing (edit a production file, run the suite, revert), do it only in an isolated jj workspace or a scratch copy that is a faithful snapshot of the reviewed tree — verify before mutating: your copy's `@` commit id must equal the reviewed commit (`jj log -r @ -T 'commit_id' --no-graph` with cwd at that workspace root from `jj workspace list` then `jj workspace root --name`; never read `.jj/` or `.git/`). A harness-created workspace may be cut from the primary checkout or default bookmark instead, and `local-aligned` scope needs the working-copy changes a committed-parent workspace lacks. On any mismatch, fall back to a scratch copy of the reviewed tree. Never mutate the shared checkout the rest of the reviewer batch is reading.
