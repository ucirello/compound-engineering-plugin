...
**Trivial-bug fast-path:** Once the problem is clear, decide whether the framework is needed at all. If the cause is immediately readable from the input (single-file typo, missing import, obvious null deref or off-by-one with a one-line fix) and verification doesn't require deep tracing, present the cause and the proposed one-line fix and run Phase 2's **Fix it now / Diagnosis only** user-choice gate before editing — the fast-path saves investigation ceremony, not the user's choice over whether to apply a fix. If the user picks fix, run Phase 3's **Workspace and bookmark check** (existing-work confirmation and default-bookmark bookmark-creation prompt), apply the fix, leave a one-line note explaining the cause, and skip to Phase 4's structured summary. If diagnosis only, write the summary and stop. When in doubt, run the full framework; getting the wrong root cause costs more than the few minutes of ceremony.
...
- Correct bookmark/workspace active; no unintended existing changes
...
- If the bug looks like a regression ("it worked before"), use a JJ-compatible bisection workflow (see `references/investigation-techniques.md`)
...
- Check for workspace changes (`jj status`). If the user has existing work in files that need modification, confirm before editing — do not overwrite in-progress changes.
- If the current bookmark/change is the default bookmark, ask whether to create a feature bookmark first using the platform's blocking question tool (see Phase 2 for the per-platform names). To detect the default bookmark, compare against `main`, `master`, or the project default from `gh repo view --json defaultBranchRef --jq '.defaultBranchRef.name'`. Default to creating one; derive a name from the bug, run `jj new`, then `jj bookmark create <name> -r @`. On any other bookmark/change, proceed.
- Record the pre-fix scope before editing: current change ID (`jj log -r @ --no-graph --template 'change_id.short()'`), whether `jj status` is clean, and any pre-existing changed files. During Phase 3, keep a list of fix-owned files (the tests and implementation files changed for this bug). Phase 4 uses this to keep simplify/review from touching unrelated bookmark work.
...
#### Post-fix polish/review tail (before JJ describe/PR)
...
#### Pre-existing bookmark/change (skill did not create it): ask the user
...
When offering, use the blocking question tool described above. If the user accepts, run `/ce-compound`, then describe the resulting learning doc on the same bookmark/change and push so the open PR picks up the new change.
