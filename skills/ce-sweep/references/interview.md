...
- **Machine-local under workspace `.tmp`** (solo setups; keeps sweep bookkeeping uncommitted, with no commit noise). Resolve the path immediately with this shell block, substituting a sanitized repository slug. Run it with cwd set to the absolute workspace root; outside JJ, use the absolute local project directory:
  ...
  SWEEP_STATE_PATH="$SCRATCH_ROOT/sweep/<repo-slug>/state.yml";
...
**Ask:** "Is this a multi-agent setup where several workspaces push the sweep state to a shared docs bookmark? Answer yes only if more than one machine or agent commits and pushes to the same remote bookmark. Default is no, meaning a single workspace committing locally."
...
- **Yes** -> `sweep_shared_branch: true`. Explain that the lease becomes **push-gated**: before any source-side write, the sweep commits and pushes the lease acquisition on the shared bookmark and confirms its writer won. This makes the lease a repo-wide mutex across machines. Establish the shared bookmark and remote from project conventions or ask when ambiguous; the run must not guess the publication target.
...
Merge the captured settings into `<repo-root>/.rocketclaw/config.local.yaml`. Resolve the repo root with `jj workspace root`, then run JJ commands with cwd set to that absolute root.
...
- If `.rocketclaw/config.local.yaml` is not already covered by the repo's `.gitignore`, offer to add the entry before writing. Machine-local workspace `.tmp` must also be ignored so JJ does not automatically track scratch or state.
...
sweep_state_path: <resolved-root>/feedback-sweep/state.yml   # concrete path (<root> resolved before persisting); committed (multi-agent) or workspace .tmp path (solo)
