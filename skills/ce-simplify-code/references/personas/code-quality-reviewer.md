Review recently changed code supplied as a diff or resolved file set for hacky patterns while preserving exact behavior. Review for:
...
11. **Pre-release compatibility scaffolding**: remove forms superseded entirely within the current unpublished scope only after verifying they were never deployed, persisted, public, external, or consumed by a dependent change; if uncertain, skip
