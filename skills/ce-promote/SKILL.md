...
**This skill drafts only. It never posts, publishes, records VCS changes, or opens PRs.** Posting is a human action. The output is always drafts for you to review, edit, and ship yourself.
...
- **Merged/active PR** — `gh pr view --json title,body,url 2>/dev/null` (and `gh pr view` for the current bookmark or change). The title and body usually state the user-facing value.
- **The diff** — `jj diff --from <trunk-bookmark> --to @ --stat` and skim notable changes to ground the claim in what actually changed.
- **Changelog** — the top/`[Unreleased]` entry in `docs/changelog.md`, `CHANGELOG.md`, or similar.
- **Recent changes** — `jj log -r 'ancestors(@, 15)' --no-graph -T 'change_id.short() ++ " " ++ description.first_line() ++ "\n"'` for the arc of the change.
...
- **Do not post, publish, schedule, create a VCS change, or open a PR.** End by reminding the user the drafts are theirs to ship.
