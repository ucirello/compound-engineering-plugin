...
- **Conventions & instruction files** — paths + digests of the root agent instructions and project docs such as `ARCHITECTURE.md`, `README.md`, `CONTRIBUTING.md`, and `STRATEGY.md`.
...
- The `docs/solutions/` enumeration (a new learning, even in the current undescribed change, must be visible — re-globbing it is ~free and the match reads files fresh anyway).
...
- Resolve the prefix with `jj workspace root`; when no JJ repository is available, use the local `./.tmp/rocketclaw/repo-profile/` path instead.
- `<root-id>` = the commit ID selected by `jj log -r 'root()'` — the repository identity.
- `<current-id>` = the commit ID selected by `jj log -r @` — the exact working-copy snapshot.

Each JJ workspace keeps its cache under its own `.tmp`; on a hit, only this one file is read.
...
- `MISS` then a write-path on the next line → dispatch the `repo-profiler` persona to derive the profile, write its JSON output to a file, then persist it. This `put` runs after the profiler, so it is a **separate Bash-tool call** from the `get` above — shell variables do not persist between calls, so **re-set `SKILL_DIR` in the same command**:
  ...
  ```
- `NO-CACHE` → no usable JJ repo or no writable workspace-local cache. Derive the profile fresh for this run and **skip** `put` (nothing to persist).
