...
3. **Config** — otherwise use the per-skill keys: `plan_model` plus optional `plan_harness` for ce-plan, `brainstorm_model` plus optional `brainstorm_harness` for ce-brainstorm. Read them the **same way this skill's Phase 0.0 (output-mode resolution) resolves `plan_output` / `brainstorm_output`**: reuse the repo root already resolved, else run `jj workspace root`, then apply the ordinary-key rule (`config.local.yaml` then `config.yaml`). Reuse the Phase 0.0 reads if still in hand. Ignore commented (`#`-prefixed) lines. A model alias selects the model; missing / commented / invalid / no file selects none. `plan_harness` / `brainstorm_harness` may be `opencode2`. That value is a distinct harness from `opencode` and is not interchangeable with it. When the harness is `opencode2`, the matching model key is `provider/modelname#variant` (variant is the `#variant` suffix on `--model`, not a separate flag).
...
1. **Native in-harness dispatch.** Attempt the platform subagent primitive with a per-agent model override (e.g. `model: "fable"` on the Claude Code `Agent`/`Task` tool). Capability is proven by attempt, not self-assessment — a harness that can serve the model natively does; one that cannot fails the attempt and falls through. **Receipt rule (R6):** a "receipt" is the serving side's report of which model actually ran. A native run whose receipt names a *different* model family than requested falls through to the next adapter; a run with *no* receipt proceeds and is recorded as unverified (it does NOT fall through). Skip this step when `plan_harness` / `brainstorm_harness` is `opencode2` — that config selects the opencode2 CLI, not the host's native primitive and not `opencode`.
2. **opencode2 CLI.** When `plan_harness` / `brainstorm_harness` is `opencode2`, run the bundled `scripts/elevation-dispatch.sh` worker as a detached job with harness `opencode2` (see Off-host dispatch). Available when `opencode2` is on PATH. This is a separate branch from `opencode` (v1): different binary, different argv, not a fallback to or from `opencode`. Model token is `provider/modelname#variant`. Do not pass `--dir` or `--variant`. The worker's cwd is the workspace root.
3. **Claude CLI.** Run the bundled `scripts/elevation-dispatch.sh` worker as a detached job (see Off-host dispatch). Available when `claude` is on PATH. Do not preflight authentication in the host command context: the detached worker's provider-capable call is authoritative, and an authentication failure there follows Recovery. Do not use this adapter when the resolved harness is `opencode2`.
...
Hand over the working context as **file paths the subagent reads itself**, never a re-narrated prose brief. Create **one private per-run handoff directory** under the workspace `.tmp` (`workspace_root=$(jj workspace root) || workspace_root="."; mkdir -p "$workspace_root/.tmp"; mktemp -d "$workspace_root/.tmp/ce-elevation-XXXXXX"`) and write the prompt-file and every evidence file into *that* directory. On the Claude CLI route the worker grants the elevated model read access to only that one directory (via `--add-dir` on the prompt-file's parent), so the handoff files stay readable while the rest of workspace `.tmp` — other same-user scratch and credentials — is not exposed:
...
When `plan_harness` / `brainstorm_harness` is `opencode2`, pass `opencode2` as the fourth argument (not `opencode`). When the harness is unset, pass an empty fourth argument or omit it so the worker uses the Claude CLI adapter. The opencode2 argv the worker execs is `opencode2 run --standalone --auto --model provider/model#variant --format json --file PROMPT` with cwd = workspace root: no `--dir`, no `--variant` flag.
...
- **Requested but unavailable before provider-capable dispatch** (no native support, `claude` absent, `opencode2` absent when that harness was selected, or the required launch permission unavailable) → run the step inline on the session model, name **which routing precondition was unmet**, and state what would make the requested model reachable. An `opencode2` miss is not satisfied by `opencode` being on PATH. Once provider-capable dispatch is established, an authentication failure is instead a route-level Recovery outcome: name the observed authentication failure and the login or credential-refresh remediation.
