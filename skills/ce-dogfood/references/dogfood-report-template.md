...
> Diff-scoped browser QA of `<target>` vs the trunk. Generated by `/ce-dogfood` on <YYYY-MM-DD>.
...
<What changed between the target and trunk: new features, modified behavior, new/changed routes, views, components, data flows. 2-6 bullets.>
...
<One Mermaid flowchart per distinct user journey the diff touches. Include happy path and decision points (validation error, empty, permission-denied), side effects (emails, jobs, notifications), and the true end state — including click-through destinations.>
...
<Overall readiness verdict for the change. Ready to ship? Caveats? Outstanding blocked items? Record the result of the Phase 5 automated test suite run — a green matrix with a red suite is not "ready.">
