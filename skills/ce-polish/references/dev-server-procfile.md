...
- `Procfile` or `Procfile.dev` exists at the Jujutsu workspace root
...
- **Socket files:** `overmind` writes a socket to `.overmind.sock` by default. RocketClaw polish's kill-by-port logic reclaims the port but does not clean up the socket. Set `OVERMIND_SOCKET` to `$RUN_DIR/overmind.sock`, where `RUN_DIR` is the safely created workspace-local `.tmp/rocketclaw-polish` directory from `SKILL.md`; never redirect it to a global temporary directory.
- **Procfile vs Procfile.dev:** production and development Procfiles often differ. Always prefer `Procfile.dev` for RocketClaw polish.
- **Multiple web processes:** some Procfiles split web traffic across multiple processes (API + frontend). RocketClaw polish can only open one URL; users with multi-web setups should author `.rocketclaw/launch.json` explicitly to select which process is the dev server.
