...
Read this RocketClaw workflow after Phase 2 ideation agents return and the orchestrator has merged and deduped their outputs into a master candidate list. Do not load before Phase 2 completes.
...
1. **Resolve the target directory and extension.**
   ...
   - **Otherwise (no workspace, or elsewhere with no `docs/ideation/`):** write into the run's workspace-local scratch area — the `<scratch-dir>` resolved in Phase 1 (`<workspace-root>/.tmp/ce-ideate/<run-id>/`, with local `.tmp` fallback outside a workspace). Do **not** write directly into the user's current working directory, and do **not** create a `docs/ideation/` tree for a subject unrelated to the repo. Announce the absolute path and note that `.tmp` is scratch space, so move the artifact to a durable location to keep it.
...
- **Inside a Jujutsu workspace:** inspect `jj status` and `jj diff` first. Offer to preserve only the ideation doc as its own change. If unrelated work is present in `@`, isolate the document with `jj split <ideation-path>` and retain the isolated revision reported by the command; otherwise keep the current change intact. Do not run `jj bookmark`, `jj rebase`, `jj squash`, `jj git fetch`, or `jj git push`. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Let repo-local instructions and observed history control syntax; apply compatible Go commit-message quality without imposing a fixed prefix, type, scope, or example. Set the isolated revision's description with `jj describe -r <isolated-revision> -m "<change-description>"`; for an unsplit current change, use `jj describe -m "<change-description>"` and then `jj new` only after the user accepts the operation. If the user declines, leave the change undescribed and do not mutate it.
- **Workspace-local scratch or non-workspace file:** skip the change-description offer.
...
Do not delete the run's workspace-local scratch directory (`<scratch-dir>`) on completion — it holds the V15 web-research cache reused across run-ids by later ideation invocations in the same session (see `references/web-research-cache.md`), the Checkpoint A/B files, the evidence dossiers, and (in the no-workspace case) the deliverable itself. The user or project cleanup policy owns eventual cleanup.
...
- the deliverable was written automatically in both modes (Phase 4) — to `docs/ideation/` when present, else workspace-local `.tmp`, never directly in the user's CWD
