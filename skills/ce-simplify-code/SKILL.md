...
2. **Otherwise, in a JJ repository**, default to the current change diff (`jj diff`). This covers the common case of "simplify everything I've added on this feature bookmark before opening a PR." If no current-change diff is available, compare against the bookmark or base ref used for the PR.
