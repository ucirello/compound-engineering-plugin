...
Cache path:
  ...
  root-id = stable hash of `jj workspace root` — the local workspace identity.
...
Validity (HIT) requires ALL of:
  ...
  - stored `head_id` == current working-copy change id,
  - stored `profile_schema_version` == PROFILE_SCHEMA_VERSION,
  - no profile-input path is dirty or newly-added per `jj diff --summary`
...
def changed_paths() -> "list[str] | None":
    ...
    Includes newly-added entries so a newly-added profile input is seen.
...
def resolve_keys() -> "tuple[str, str] | None":
    ...
    root = root_id()
    head = jj("log", "-r", "@", "--no-graph", "--template", "commit_id")
...
def do_put(profile_file: str) -> int:
    ...
    # edits to profile inputs, yet it would be stored under the clean @ key
    # and served as a HIT after those edits are reverted (same @, clean tree)
    # — stale. Only persist a profile that matches the described @.
    changed = changed_paths()
    if changed is None or any(is_profile_input(p) for p in changed):
        sys.stderr.write(
