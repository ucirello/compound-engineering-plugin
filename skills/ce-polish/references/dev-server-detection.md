...
**(c) No instruction-file port grep.** The script does not grep agent-instruction files for port references. Those files carry natural language that may mention ports in unrelated contexts, producing false positives that are hard to debug. Framework config files and `.env` are the reliable machine-readable sources. The agent may still honor a dev-server port from the project's active instructions already in context.
