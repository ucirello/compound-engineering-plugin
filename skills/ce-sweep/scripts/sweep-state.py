...
def load_state(path):
    ...
    try:
        with open(path, encoding="utf-8") as f:
            ...
            # and it is a correctness dependency (lease, cursors, closed status)
            # as well as an injection sink (item bodies re-read into agent
            # context). Reject a file not owned by us so another user cannot
