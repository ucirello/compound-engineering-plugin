...
3. **Unpushed changes vs the base bookmark.** If the current change is empty, review `jj diff --from <base> --to @` where `<base>` is the repository's default bookmark.
...
When the review context includes remote scope, the working copy is **not** the reviewed revision. Do **not** use Read/Grep on workspace paths for files in the changed-file list; they may not match the bookmark or PR under review.
...
In remote scope these tiers inspect the working copy, which is not the reviewed revision; apply the Remote scope rules above instead of local search.
...
Issues in unchanged code that the diff did not touch and does not interact with. Mark these as `"pre_existing": true` in your output. They are reported separately and do not count toward the review verdict. When history makes the pre-existing call, attach concise JJ provenance as defined in `subagent-template.md`.
