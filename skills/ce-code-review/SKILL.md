...
- **Explicit mutations only.** Never run `gh pr checkout`, `jj edit`, `jj new`, `jj bookmark set/create`, or similar working-copy/bookmark-switch commands. Passing a PR number, URL, or bookmark name selects **review scope**, not permission to mutate the working copy. To review local unpublished work on a feature bookmark, switch to that bookmark yourself (or stay on it) and pass `base:` or no target.
...
- `docs/plans/*.{md,html}` -- unified plan artifacts created by ce-brainstorm or ce-plan (decision artifacts; execution progress is derived from JJ/VCS history, not stored in plan bodies)
...
```
echo "BASE:$BASE" && echo "FILES:" && jj diff --name-only --from "$BASE" --to @ && echo "DIFF:" && jj diff --context 10 --from "$BASE" --to @ && echo "STATUS:" && jj status
```

This path works with any JJ revset, commit ID, or bookmark. Callers reviewing the current workspace should pass explicit `base:` when auto-detection is unnecessary. **Do not combine `base:` with a PR number or bookmark target.** If both are present, stop with an error: "Cannot use `base:` with a PR number or bookmark target — `base:` implies the current workspace is already the correct target. Pass `base:` alone, or pass the target alone and let scope detection resolve the base."

**If a PR number or GitHub URL is provided as an argument:**

Do **not** edit or switch to the PR bookmark. Scope comes from GitHub read APIs plus optional local alignment when `@` already contains the PR head.
...
Set `BASE:` to `pr:<number-or-url>` (logical marker — not a commit ID). Set `STATUS:` from `jj status` on the **current** workspace (usually irrelevant during PR-remote review).
...
1. The current bookmark from `jj log -r @ --no-graph -T 'bookmarks.join(" ")'` contains `headRefName`.
2. The PR is **not** cross-repository (`isCrossRepository` is false).
3. The PR head commit is contained in the local workspace: `jj log -r '<headRefOid> & ::@'` resolves. This confirms the workspace actually carries the PR head (allowing unpublished local fixes layered on top) rather than an unrelated same-named bookmark.
...
1. Best-effort fetch PR head/base metadata without checkout with `jj git fetch --remote origin --branch <headRefName>` and `jj git fetch --remote origin --branch <baseRefName>`.
2. When fetch succeeds, set `PR_HEAD_REF=<headRefName>@origin` and `PR_BASE_REF=<baseRefName>@origin` for reviewers and validators. The `pr:<number-or-url>` logical marker in `BASE:` stays the scope marker; `PR_BASE_REF` is the diffable base. When fetch fails, omit these refs and note in Coverage — schema-drift and other JJ-diff checks fall back to diff hunks only and must **not** assume `main`.
...
Substitute the provided bookmark name as `<bookmark>`. Do **not** edit or switch to `<bookmark>`.

If the current bookmark equals `<bookmark>`, use the **standalone (current bookmark)** path below — same tree, explicit bookmark name; do not use remote-only diff.
...
3. Resolve default base bookmark (same logic as standalone). Compute `BASE=$(jj log -r "heads(::<base-ref> & ::<bookmark-ref>)" --no-graph -T 'commit_id ++ "\n"')` and `jj diff --context 10 --from "$BASE" --to <bookmark-ref>`.
4. If `<bookmark-ref>` cannot be resolved locally, stop: "Cannot diff bookmark `<bookmark>` without switching. Switch to that bookmark, pass its open PR URL/number, or review the current bookmark with `base:`."

On success for remote bookmark diff, set **bookmark-remote scope**. The working tree is **not** `<bookmark>`. Include `<pr-scope-mode>bookmark-remote</pr-scope-mode>` and `<branch-head-ref><bookmark-ref></branch-head-ref>` in the Stage 4 review context bundle. Reviewers and Stage 5b validators must **not** Read/Grep workspace paths for files in `FILES:`. Inspect via `jj file show -r <bookmark-ref> <path>` or diff hunks only.

Produce:

```
echo "BASE:$BASE" && echo "FILES:" && jj diff --name-only --from "$BASE" --to <bookmark-ref> && echo "DIFF:" && jj diff --context 10 --from "$BASE" --to <bookmark-ref> && echo "STATUS:" && jj status
```

**If no argument (standalone on current branch):**

Apply the same base-detection logic as bookmark mode above, using the current bookmark (i.e., `gh pr view --json baseRefName,url` with no argument defaults to the current PR when one exists).

If no base can be resolved, **stop**. Do not fall back to `jj diff -r @` — a standalone review without the base would only show the current working-copy change and silently miss earlier work on the bookmark.
...
Using `jj diff --from "$BASE" --to @` diffs the base against the working copy, which includes committed and current working-copy changes together.

**New-file handling:** Always inspect `STATUS:`. New files in the JJ working-copy change are in scope unless ignored by the repo. List ignored/excluded files in Coverage when relevant — never stop or prompt solely because new files exist.
...
- **`local-aligned` / standalone / `base:`** — `DIFF_A="$BASE"`, `DIFF_B="@"` (diffs base vs working copy).
- **`pr-remote` / `bookmark-remote`** — `DIFF_A=<PR_BASE_REF>`, `DIFF_B=<PR_HEAD_REF>` (or `<branch-head-ref>`) — the fetched/visible refs from Stage 1. Do **not** model-count from hunks (it drifts per host/model). If either ref was not fetched, skip the block and emit `EXEC_LINES:UNKNOWN` + `UNCOUNTED_FILES:1` so Stage 3c forces the full roster.

```
# Fail closed: an unresolved/invalid endpoint must NOT become a silent EXEC_LINES:0.
# Validate BOTH endpoints — a set-but-unresolved DIFF_B (pr-remote/bookmark-remote head ref)
# would otherwise make `jj diff` fail and the lite gate clear on the wrong tree.
if [ -z "${DIFF_A:-}" ] || [ -z "${DIFF_B:-}" ] \
  ...
  # EXEC_LINES from JJ stat output. If parsing ever fails, leave UNKNOWN rather than clearing the lite gate on the wrong tree.
  EXEC_LINES=$(jj diff --stat --from "$DIFF_A" --to "$DIFF_B" -- '*.rb' '*.py' '*.js' '*.jsx' '*.ts' '*.tsx' '*.go' '*.rs' '*.java' '*.swift' '*.kt' '*.c' '*.cc' '*.cpp' '*.cs' '*.php' '*.ex' '*.exs' '*.scala' 2>/dev/null | awk '/^[[:space:]]*[0-9]+ files? changed/ {next} {for (i=1; i<=NF; i++) if ($i ~ /^[0-9]+$/) s+=$i} END{print s+0}')
  echo "EXEC_LINES:$EXEC_LINES"
  FILES=$(jj diff --name-only --from "$DIFF_A" --to "$DIFF_B" 2>/dev/null)
...
**Bookmark mode:** Run `jj log -r '${BASE}..<bookmark-ref>' --no-graph -T 'commit_id.short() ++ " " ++ description.first_line() ++ "\n"'` using the resolved base and bookmark ref from Stage 1. Use `<bookmark-ref>` (the resolved `<bookmark>@origin` or local bookmark), not the raw `<bookmark>` argument when a remote-only bookmark could be stale or absent locally.

**Standalone (current branch):** Run:

```
echo "BOOKMARK:" && jj log -r @ --no-graph -T 'bookmarks.join(" ") ++ "\n"' && echo "COMMITS:" && jj log -r "${BASE}..@" --no-graph -T 'commit_id.short() ++ " " ++ description.first_line() ++ "\n"'
```
...
**Only resolve the cache when the working tree is the reviewed tree** — `local-aligned`, standalone, or `base:` scope (Stage 1). In `pr-remote` or `bookmark-remote` scope, **skip Stage 2c entirely**: the helper keys and derives from local `@`, which is *not* the reviewed ref, so its profile would describe the wrong tree (e.g. `main`'s stack while reviewing a PR that changes manifests); reviewers work from the fetched refs/diff as those modes already require.
...
5. Review context: intent summary, file list, diff, scope mode (`local-aligned` | `pr-remote` | `bookmark-remote`), and remote head ref (`PR_HEAD_REF` or `<branch-head-ref>`) when set
...
3. **Spawn validators with bounded parallelism.** One sub-agent per finding, dispatched independently using the validator template and the same bounded scheduler from Stage 4. Each validator receives:
   ...
   - The full diff
   - The scope mode and remote head ref, mirroring the Stage 4 reviewer bundle: inject `<pr-scope-mode>local-aligned | pr-remote | bookmark-remote</pr-scope-mode>` and, when set, `<pr-head-ref>...</pr-head-ref>` or `<branch-head-ref>...</branch-head-ref>`. The validator template defaults to local-aligned workspace inspection when these are absent, so omitting them in `pr-remote`/`bookmark-remote` makes validators verify findings against the stale working tree — dropping valid findings or confirming false ones on the wrong tree.
   - Inspection access scoped by mode: in `local-aligned`, Read/Grep/JJ-annotate the cited code, callers, guards, framework defaults, and history; in `pr-remote`/`bookmark-remote`, inspect via `jj file show -r <remote-head-ref> <path>` or the provided diff hunks only — do not Read/Grep workspace paths for files in scope.
...
**Commit when the pre-review tree was clean.** Before applying, note whether the working copy already had changes (`jj status`). The permanence gate is the **push**, not the commit — a local JJ commit is private and reversible with `jj undo` or a follow-up change.
...
{
  "run_id": "<run-id>",
  "bookmark": "<jj log -r @ --no-graph -T 'bookmarks.join(\" \")' at dispatch time>",
