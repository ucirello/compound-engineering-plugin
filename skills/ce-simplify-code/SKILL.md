...
2. **Otherwise, in a JJ repository**, default to the diff between the current change and its trunk baseline (e.g., `jj diff --from trunk() --to @`). This covers the common case of "simplify everything I've added before opening a PR." If no base rev is available, fall back to `jj diff` for the current change.
