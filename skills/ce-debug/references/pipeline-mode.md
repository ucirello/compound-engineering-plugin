...
- **Phase 3 (workspace/bookmark):** Operate on the current bookmark — the orchestrator owns bookmark context; never prompt to create a bookmark, never prompt about uncommitted work. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Repo-local syntax from project instructions and `git log` ALWAYS wins when it differs from Go guidance. Apply compatible Go guidance to quality/clarity/structure without replacing repo-local syntax. Semantic requirement: the change records the convergent fix. Then `jj commit -m "<message composed from the standards above>"` the fix-owned filesets and `jj git push`. Never weaken, skip, or mock a failing assertion to make it pass — repair the real issue or defer.
...
- `fixed-not-pushed` — the same fix is applied and committed locally, but the push did not happen — no remote, no push access, an envelope that excludes pushing, a rejected push. `head_sha` is the local `commit_id`; the first residual says why. Never report this as `fixed-and-pushed` (the caller re-snapshots a remote head that has not moved) or as `diagnosed-no-fix` (the fix is applied).
