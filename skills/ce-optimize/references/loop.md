...
**Worktree backend** (JJ workspaces; the spec token stays `worktree`):
...
3. Write the filled prompt to a temp file under the workspace `.tmp` (or local `.tmp` if not in a JJ repository):
   ```bash
   ROOT=$(jj workspace root 2>/dev/null || true)
   if [ -n "$ROOT" ]; then TMP="$ROOT/.tmp/ce-optimize"; else TMP=".tmp/ce-optimize"; fi
   mkdir -p "$TMP"
   PROMPT_FILE=$(mktemp "$TMP/optimize-exp-XXXXXX")
   ```
...
3. **If `decide.mjs` returns `keep` for that winner: KEEP**
   ...
   - Repository-local commit-message syntax from project instructions and `git log` ALWAYS wins when it differs from Go guidance. Apply compatible Go guidance to quality/clarity/structure without replacing repository-local syntax.
   - Describe the experiment change in its workspace first so the winning diff exists as a described change before any integrate or duplicate. The message must describe this hypothesis and the optimization spec; do not use a fixed prefix, type, or scope template.
     ```bash
     (cd "<experiment-workspace>" && jj describe -m "<message composed from the standards above>")
     ```
   - Include only mutable-scope changes in that change; if no eligible diff remains, treat the experiment as non-improving and restore it
   - Integrate the described experiment change into the optimization bookmark from the optimization workspace (`jj squash --from <experiment-change> --into @`, or `jj new @ <experiment-change>` then squash)
   - After the integrate succeeds, clean up the winner's experiment workspace and bookmark; the integrated change on the optimization bookmark is the durable artifact
...
4. **Check file-disjoint runners-up** (up to `max_runner_up_merges_per_batch`):
   ...
   - If disjoint: duplicate the runner-up onto the new baseline (`jj duplicate <runner-up-change> -o @`) and run the same decide loop as step 3.3 against a fresh sample set for that combined snapshot — do not reuse the standalone experiment's accumulated samples, whose meaning is against the previous baseline. Collect further measurement whenever `next_measurement` is not `none`. Keep the original standalone log entry for audit.
   - Keep the duplicated change only when that result is eligible and `next_measurement` is `none` (outcome: `runner_up_kept`); then clean up that runner-up's experiment workspace and bookmark
   - Otherwise: abandon the duplicate (`jj abandon` / `jj undo`), log as "promising alone but neutral/harmful in combination" (outcome: `runner_up_reverted`), then clean up the runner-up's experiment workspace and bookmark
