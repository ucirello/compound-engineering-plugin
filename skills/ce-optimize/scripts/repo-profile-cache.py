...
Cache path:
    <workspace-root>/.tmp/rocketclaw/repo-profile/<root-id>/<working-copy-id>.json
  root-id = lexicographically-first root commit ID in `roots(::@)`
            (deterministic even for multi-root histories).
  working-copy-id = commit ID of `@` — the working state.

Validity (HIT) requires ALL of:
  - the cache file exists and parses as JSON,
  - stored `working_copy_id` == the current `@` commit ID,
  - stored `profile_schema_version` == PROFILE_SCHEMA_VERSION,
  - no profile-input path is changed in `jj diff --summary`
    (the schema-derived superset in `is_profile_input`; JJ snapshots newly
    added, non-ignored files, so a new manifest or AGENTS.md invalidates).
...
workspace-local cache, JJ errors) degrades to NO-CACHE/MISS and exits 0 — it never raises and
...
# an omitted manifest means a changed dependency input could serve a stale
...
def root_id() -> "str | None":
    out = jj(
        "log", "-r", "roots(::@)", "--no-graph", "-T", 'commit_id ++ "\\n"'
    )
    if not out:
        return None
...
def changed_paths() -> "list[str] | None":
    ...
    Includes newly added, non-ignored files snapshotted by JJ.
...
def cache_path(workspace: str, root: str, working_copy: str) -> str:
    return os.path.join(
        workspace, ".tmp", "rocketclaw", "repo-profile", root,
        f"{working_copy}.json"
    )


def resolve_keys() -> "tuple[str, str, str] | None":
    """The workspace and revision cache keys, or None outside a usable JJ repo."""
    workspace = workspace_root()
    root = root_id()
    working_copy = jj("log", "-r", "@", "--no-graph", "-T", "commit_id")
    if not workspace or not root or not working_copy:
        return None
    return workspace, root, working_copy
...
def do_get() -> int:
    ...
    if keys is None:
        print("NO-CACHE")
        return 0
    workspace, root, working_copy = keys
    path = cache_path(workspace, root, working_copy)
    ...
    try:
        with open(path) as f:
            # Reject cache files not owned by us to avoid loading planted text.
    ...
    if (
        ...
        or doc.get("working_copy_id") != working_copy
...
def do_put(profile_file: str) -> int:
    ...
    # Do not cache a profile derived from changed profile inputs; it could later
    # be stale under the same key.
    # Only persist a profile from a clean profile-input snapshot.
    changed = changed_paths()
    if changed is None or any(is_profile_input(p) for p in changed):
        sys.stderr.write(
    ...
    try:
        ...
        # Atomic write: a unique sibling in the workspace-local cache plus
        # os.replace, so a concurrent reader never sees torn JSON.
        tmp = os.path.join(
            os.path.dirname(path), f".tmp-{os.getpid()}-{time.time_ns()}.json"
        )
