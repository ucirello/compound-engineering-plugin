...
- `unit_receipts`: route, model, detached-process, workspace-change, integration-operation, verification, accepted-change, and cleanup state for each attempted unit
- `plan_checkpoint`: the disclosed checkpoint change when the selected plan was the only pre-controller working-copy content, otherwise `null`
- `blockers`
