...
**Durability:** the in-session watch dies with the session; re-invoking resumes from workspace-local `.tmp/rocketclaw` state. For an unattended multi-day watch, escalate to a durable scheduler (Grok `scheduler_create --durable`, or cron running a dynamically rendered harness invocation) — a fresh headless run is context-blind, so persist consequential decisions to disk. **Shell env vars do not persist between separate tool calls** on any harness — re-set `SKILL_DIR`/`STATE_DIR` inline in every command.
...
A loop can churn without finishing: CI **ping-pong** (fix A surfaces B, fix B brings A back — often an emergent trade-off), a review-bot **treadmill** (each pushed change spawns fresh nits), or **wrong-approach whack-a-mole** (each nit is valid but the approach, e.g. a regex, is the problem). A raw attempt counter can't tell these from *legitimate progress* (four independent failures each fixed once) — so the decision is **agent reasoning over the trajectory**, and the split is strict:
...
- **Branch currency.** `branch_currency` is the third attention stream. Its identity binds host-qualified live base repository/ref/OID, head SHA, merge status, and route. "Mergeable against the current base" and "head contains the latest base" are separate: only `BEHIND`, `DIRTY`, a branch-protection requirement, or an explicitly selected always-current policy requires maintenance. Ordinary historical base movement with GitHub reporting `CLEAN` does not. `UNKNOWN` mergeability or any non-null `base_ref_blocker` emits no item. Managed stacks and manager/relationship `probe-error` are excluded; a `normal-base` target may be independent or an eligible target-local manual dependency, and a root with open child dependents remains allowed. Never mutate those dependents.
  ...
  A current open item with carried `parked_semantic_fingerprints` must be previewed first. Compute the current fingerprint from sorted conflicted paths plus stable identities of their materialized conflict contents, excluding the base OID, and mark `--currency-inspected-fingerprint <fingerprint>`. Unchanged evidence remains parked; changed evidence retires the old park and reopens attention. Before either a host update or Jujutsu merge change, atomically mark the exact item with `--currency-key <currency_key> --currency-disposition claimed`, plus `--invocation-id "$RUN_INVOCATION_ID" --session-started-at "$RUN_STARTED_AT" --invocation-budget-seconds "$RUN_BUDGET_SECONDS"`. Stale invocation fencing and `max-runtime` take precedence over a new claim or mutation.
...
- **Managed stack:** `target_needs_rebase` true/unknown becomes `stack-blocked`; do not use the ordinary GitHub update operation or a local base merge. Before a delegate may push the target, record the manager-ordered open branches at or above it and each remote bookmark's Git commit ID; require an empty Jujutsu working-copy change and confirmed membership. After an authorized target push, retain the pushed ID, re-confirm local provider membership, fetch through `jj git fetch`, and verify the target's local and remote bookmark targets. Run provider-owned upstack rebase/push operations only from the first open dependent, then import or fetch their resulting state into Jujutsu and compare every open dependent with the baseline and expected tips. Treat partial updates as observed progress, never infer atomicity, and surface the first rejected or divergent layer. Abort provider conflicts and never bypass Jujutsu push safety after target movement or lease rejection. Never claim stack readiness until manager order, ancestry, review, and CI are re-proven on every current head.
...
- **Normal-base `BEHIND`:** require `host_branch_update_capability == true`; denied/`false` or `unknown` becomes `needs-human`, and that capability is never treated as Jujutsu bookmark or push authority. Claim the exact item and revalidate its observed head and base OIDs. Invoke the host update operation once through GitHub's `PUT /repos/{owner}/{repo}/pulls/{number}/update-branch` endpoint with `expected_head_sha` set to the claimed observation's head SHA; never use an update helper that cannot transmit that precondition. Treat an HTTP 422 head mismatch as a stale claim: re-snapshot and reconcile without resubmitting. Host acceptance or a `mutation-observed` outcome is not completion. Confirm only after a fresh snapshot plus `jj log` ancestry evidence proves the resulting head contains the observed base OID and the currency gate is clear. Remote head movement alone is not proof.
- **Normal-base `DIRTY`:** `host_branch_update_capability` is irrelevant and does not apply. Separately prove `jj git push` authority to the exact head bookmark without mutating; unknown or denied proof parks. Require the verified PR-head Jujutsu workspace state, fetch the exact observed base with `jj git fetch`, and create a disposable merge change for the preview without moving or pushing the PR bookmark. Fingerprint the sorted conflicted paths and stable identities of their materialized conflict contents, explicitly excluding the base OID. If `parked_semantic_fingerprints` is present, record `--currency-inspected-fingerprint` before any claim: an unchanged fingerprint remains parked, while changed evidence retires the old park and reopens the item.

  A resolution is mechanical only when positive intent evidence leaves no reasonable alternative behavior. Two plausible resolutions, a material intent change, stale/unauthorized/incomplete evidence, or unbounded work requires abandoning the disposable change and `--currency-disposition needs-human --semantic-conflict-fingerprint <fingerprint>`, with concise competing options, tradeoffs, and a lean. For a mechanical case, claim and revalidate, create the merge change from the exact observed parents, and mark `--currency-outcome mutation-observed` when that change is created. Resolve only the previewed conflict, validate proportionally, describe it using the project's active instructions and history read with `jj log`, move the PR bookmark, and use `jj git push` for that bookmark and remote. Apply compatible Go quality guidance without a fixed syntax or template. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Confirm only after the remote head equals or contains the validated change's Git commit ID and a fresh snapshot clears the gate; remote head movement alone is not confirmation. Re-entry reconciles or abandons the existing merge change safely.
...
- **External head change / rewritten remote bookmark:** the head SHA moved under the loop. The snapshot clears SHA-scoped CI state automatically; just re-snapshot. Never clobber unrelated pushed work.
