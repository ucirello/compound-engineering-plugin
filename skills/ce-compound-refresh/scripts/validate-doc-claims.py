...
citations against the repository:
    ...
    1. Cited repo-relative paths (backticked, containing at least one '/')
       ...
       (deleted/unrecorded vs stale working copy). Tokens missing everywhere
       are flagged only when path-shaped; slash-delimited identifiers
       (bookmark names, remote refs, provider/model IDs) and slash-prefixed
       URL routes are skipped.
    2. Cited revision IDs (7-64 hex chars with at least one digit and one
...
SHA_RE = re.compile(r"\b[0-9a-f]{7,64}\b")
...
def main(argv: list[str]) -> int:
    ...
    upstream_display: str | None = None
    if in_jj:
        code, trunk_id = jj(
            ["log", "-r", "trunk()", "-n", "1", "--no-graph", "-T", "commit_id"],
            repo_root,
        )
        _, root_id = jj(
            ["log", "-r", "root()", "-n", "1", "--no-graph", "-T", "commit_id"],
            repo_root,
        )
        if code == 0 and trunk_id and trunk_id != root_id:
            upstream = "trunk()"
            _, names = jj(
                [
                    "log",
            ...
            first = names.split("\n")[0].strip() if names else ""
            # Strip remote suffix (main@origin -> main) for display.
            if "@" in first:
                first = first.split("@", 1)[0]
            upstream_display = first or "trunk()"
        if upstream:
            code, behind = jj(
            ...
            if code == 0 and behind.isdigit() and int(behind) > 0:
                infos.append(
                    f"INFO: working copy is {behind} revisions behind "
                    f"{upstream_display} — "
                    "verify merge-state claims against remote truth "
                    "(GIT_DIR=$(jj git root) gh pr view from the workspace "
                    "root), not this working copy"
                )
        else:
            infos.append(
    ...
    def is_ancestor(sha: str, tip: str) -> bool:
        code, out = jj(
            [
                "log",
                "-r",
    ...
    for raw in BACKTICK_RE.findall(body):
        ...
        if tracked_head:
            flags.append(
                ...
                "the working copy: deleted or unrecorded removal? Annotate as "
            ...
            flags.append(
                ...
                f"{upstream_display}: stale working copy? Annotate or verify against upstream."
            )
        else:
            where = (
                f"working copy or {upstream_display}"
                if upstream_display
                else "working copy"
            )
            flags.append(
    ...
    if in_jj:
        for m in SHA_RE.finditer(body):
            ...
            loc = loc_suffix(sha)
            code, _ = jj(
                ["log", "-r", sha, "-n", "1", "--no-graph", "-T", "commit_id"],
                repo_root,
            )
            if code != 0:
                flags.append(
            ...
            if in_head and not in_up:
                flags.append(
                    f"FLAG sha {sha}{loc} — reachable from @ but not {upstream_display}: "
                ...
                flags.append(
                    ...
                    f"from {upstream_display}: this working copy predates the merge. Add a "
                    "temporal qualifier or verify the claim via GIT_DIR=$(jj git root) gh."
                )
            else:
                flags.append(
                    f"FLAG sha {sha}{loc} — exists but unreachable from @"
                    + (f" or {upstream_display}" if upstream_display else "")
