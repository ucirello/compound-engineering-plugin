...
2. **Caller carrier** — a structured `<per-skill-key>:<model-alias>` token that an automatic orchestrator may pass in the invocation (LFG passes `plan_model:<alias>` to ce-plan; the analogous `brainstorm_model:<alias>` to ce-brainstorm). Use it when live user intent does not decide the choice. Strip it from the request text and never reconstruct it from product prose. It is honored in pipeline / `disable-model-invocation` runs. The alias must match `^[A-Za-z0-9._-]{1,64}$`; a malformed carrier counts as absent, not guessed. A parallel `plan_harness:<value>` / `brainstorm_harness:<value>` carrier selects the off-host CLI; `opencode2` is its own value, not an alias of `opencode`. When the harness is `opencode2`, the model id is `provider/modelname#variant` and must be preserved unchanged (including `#variant`); that form is valid for the model carrier on this branch even though it does not match the alias regex above.
3. **Config** — otherwise use the per-skill keys: `plan_model` / `plan_harness` for ce-plan, `brainstorm_model` / `brainstorm_harness` for ce-brainstorm. Read them the **same way this skill's Phase 0.0 (output-mode resolution) resolves `plan_output` / `brainstorm_output`**: reuse the repo root already resolved, else run `jj workspace root`, then apply the ordinary-key rule (`config.local.yaml` then `config.yaml`). Reuse the Phase 0.0 reads if still in hand. Ignore commented (`#`-prefixed) lines. A model alias (or, when harness is `opencode2`, a `provider/modelname#variant` id) selects the model; missing / commented / invalid / no file selects none. `plan_harness: opencode2` (and the brainstorm analogue) is its own branch.
...
1. **Native in-harness dispatch.** Attempt the platform subagent primitive with a per-agent model override (e.g. `model: "fable"` on the Claude Code `Agent`/`Task` tool). Capability is proven by attempt, not self-assessment — a harness that can serve the model natively does; one that cannot fails the attempt and falls through. **Receipt rule (R6):** a "receipt" is the serving side's report of which model actually ran. A native run whose receipt names a *different* model family than requested falls through to the next adapter; a run with *no* receipt proceeds and is recorded as unverified (it does NOT fall through). When `plan_harness` / `brainstorm_harness` is explicitly `opencode2`, skip this step and use the opencode2 CLI branch below.
2. **opencode2 CLI** (only when the resolved harness is `opencode2`). Distinct from `opencode`. Available when `command -v opencode2` succeeds. Run the bundled `scripts/elevation-dispatch.sh` worker as a detached job with `CE_ELEVATION_HARNESS=opencode2` (see Off-host dispatch). The worker invokes `opencode2 run --format json --model <id> --file <prompt-file> --auto` — flags from `opencode2 run --help` only; do not pass `opencode` flags such as `--dir`. Pass `--model` the `provider/modelname#variant` id unchanged. No receipt is defined for this CLI; record served as unverified. If `opencode2` is absent, do not fall through to the Claude CLI or to `opencode`; degrade to inline.
3. **Claude CLI.** Run the bundled `scripts/elevation-dispatch.sh` worker as a detached job (see Off-host dispatch). Used when harness is unset or not `opencode2`. Available when `claude` is on PATH. Do not preflight authentication in the host command context: the detached worker's provider-capable call is authoritative, and an authentication failure there follows Recovery.
4. **Inline on the session model.** The always-available fallback.
...
- On the **opencode2** route, `opencode2 run --help` has no tool-restriction or `--add-dir` flag. The worker uses `--format json --model <id> --file <prompt-file> --auto` only. Write/shell denial is an instruction, not a hard guarantee. Do not pass `opencode` flags such as `--dir`.

Hand over the working context as **file paths the subagent reads itself**, never a re-narrated prose brief. Create **one private per-run handoff directory** under the workspace `.tmp` (fallback: local `.tmp`) and write the prompt-file and every evidence file into *that* directory:

```bash
if ROOT="$(jj workspace root 2>/dev/null)"; then
  mkdir -p "$ROOT/.tmp"
  HANDOFF_DIR="$(mktemp -d "$ROOT/.tmp/ce-elevation-XXXXXX")"
else
  mkdir -p .tmp
  HANDOFF_DIR="$(mktemp -d ".tmp/ce-elevation-XXXXXX")"
fi
```

On the Claude CLI route the worker grants the elevated model read access to only that one directory (via `--add-dir` on the prompt-file's parent), so the handoff files stay readable while the rest of the workspace `.tmp` — other same-user scratch and credentials — is not exposed:
...
```
   ...
   export CE_ELEVATION_HARNESS="<resolved plan_harness / brainstorm_harness, or omit>";
