...
**Boundaries:** the user drives what to inspect and change; do not invent an autonomous checklist or expand into general QA. Never work on the repository's default bookmark. This workflow may edit and locally commit the requested polish, but it never pushes or opens a PR.
...
4. **Close locally.** When the user says they are done, invoke `ce-commit` for the polish changes, then report the change(s), the still-running server URL, and any residual blocker.
