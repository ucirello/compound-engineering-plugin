...
When composing or revising the change description, apply this policy: Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Runtime project instructions and description conventions observed through the project's working `jj log` invocation take precedence; determine that invocation at runtime rather than prescribing fixed `jj log` syntax. Treat `git log` here as backing-Git history inspection through JJ/Git interoperability, not as a replacement for the JJ workflow. Apply compatible Go guidance only to quality, clarity, and structure. Do not impose any fixed prefix, type, scope, subject, body, layout, syntax, template, or example; use neutral placeholders when discussing variable content.
