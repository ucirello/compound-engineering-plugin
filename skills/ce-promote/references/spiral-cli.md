...
Read the RocketClaw project config. Resolve the workspace root with Jujutsu and use the current directory only when outside a Jujutsu workspace:

```bash
root="$(jj workspace root 2>/dev/null || pwd)"; cat "$root/.rocketclaw/config.local.yaml" 2>/dev/null || printf '%s\n' '__NO_CONFIG__'
```
...
Resolve the workspace root with `jj workspace root`, falling back to the current directory outside a Jujutsu workspace. Then add `ce_promote_spiral_optout: true` as a top-level key to `<root>/.rocketclaw/config.local.yaml`, using the native file-write/edit tool:
...
- **File absent:** create it (and its `.rocketclaw/` directory) with the key, AND make sure the machine-local config won't be tracked. Check the repository's existing ignore rules. If `<root>/.rocketclaw/config.local.yaml` is not ignored, append `.rocketclaw/*.local.yaml` to `<root>/.gitignore`, unless an equivalent rule is already present. Do not replace or reorder existing `.gitignore` content. `ce-setup` uses the same shared rule for teammates.
