...
Every `gh` invocation in this file runs in the same shell as `GIT_DIR=$(jj git root) gh ...` so gh
sees the colocated Git store.
...
bookmark. Create a local bookmark at `headRefOid` (`jj new <headRefOid>` then `jj bookmark create <name>` when that commit is
reachable). If the commit is not in the repository, `jj git fetch --remote <remote>` and retry; if it is still unreachable, treat that as a residual — public jj cannot fetch an arbitrary `refs/pull/<n>/head`.
