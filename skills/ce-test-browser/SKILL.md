...
2. **Determine test scope** from the argument: a PR number → `GIT_DIR=$(jj git root) gh pr view [number] --json files -q '.files[].path'`; `current` or empty → `jj diff --name-only --from main --to @`; a bookmark name → `jj diff --name-only --from main --to [bookmark]`.
