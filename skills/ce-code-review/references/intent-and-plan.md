...
**Branch mode:** Run `jj log -r "${BASE}..<branch-ref>" --no-graph -T 'builtin_log_oneline'` using the resolved merge-base and resolved branch ref from Stage 1. Use `<branch-ref>` (the resolved `<branch>@origin` or fetched ref), not the raw `<branch>` argument — a remote-only bookmark has no matching local bookmark, so the raw name would fail or read a stale same-named local bookmark.

**Standalone (current branch):** Run:

```
echo "BRANCH:" && jj bookmark list -r @ -T 'name ++ "\n"' && echo "COMMITS:" && jj log -r "${BASE}..@" --no-graph -T 'builtin_log_oneline'
```
