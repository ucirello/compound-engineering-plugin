...
1. **Jujutsu activity** — `jj log` over the window (descriptions, change IDs, dates, authors), and for substantial changes, `jj diff --stat` at the relevant revisions. Group obviously related changes (a bookmark's change stack, a fix and its follow-ups) rather than listing them flat. Determine the date and revset syntax from the installed JJ runtime rather than substituting another VCS's history commands.
...
Order by date. Bundle minor mechanical changes (version bumps, typo fixes) into a single "housekeeping" line rather than enumerating them. If the window is empty — no Jujutsu changes and no doc changes — write nothing and report exactly that.
