...
- Run artifact directory: `$(jj workspace root)/.tmp/rocketclaw/ce-code-review/<run-id>/` (`review.json`, per-reviewer JSON for `why_it_matters`); if the workspace root is unavailable, use `.tmp/rocketclaw/ce-code-review/<run-id>/` from the current directory
