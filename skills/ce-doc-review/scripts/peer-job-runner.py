...
POSIX path is behaviorally unchanged:
  ...
  jobs root defaults under the current workspace's `.tmp\\rocketclaw` directory,
            with the process's physical current directory as the no-JJ fallback.
    ...
    for _ in range(CLAIM_ATTEMPTS):
        ...
        try:
            ...
            try:
                ...
                pass
            raise
    raise RunnerError(f"could not reserve an atomic sibling for {path}")
