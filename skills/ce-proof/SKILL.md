...
Every write is attributed with both fields, and the defaults do not vary. The machine ID is `ai:assistant`, sent as `by` on every op and as the `X-Agent-Id` header. The display name is `AI Assistant`, sent as `name` on `POST /presence`, set once per doc session so Proof binds it to that actor ID. Preserve a caller-supplied identity pair when a distinct actor, provider, model, or human should own the doc; never replace human attribution with the defaults or improvise a variant.

## Credentials and boundaries

- `accessToken` is the everyday bearer for read, edit, presence, and events. `ownerSecret` carries owner authority only — delete and other owner-level ops — and is never the everyday bearer. Capture both at create time, and persist `ownerSecret` for the session separately from `accessToken`, in shell vars or equivalent; it is required for owner delete while the doc is unclaimed. Neither belongs in tracked files, JJ changes, or durable logs, and `ownerSecret` never appears in user-facing copy.
...
- Never put secrets, credentials, API keys, private tokens, or sensitive personal data into a Proof doc unless the user explicitly approves, and never silently replace a tracked project doc with a Proof link.
...
Pulling a doc down to a local file overwrites that file. When the pull is a side effect of some other action rather than something the user asked for, confirm the path first. Keep transient pull data under `.tmp/rocketclaw` at the root returned by `jj workspace root`; outside a JJ workspace, use `.tmp/rocketclaw` under the current local directory. Do not fall back to an OS-global temporary location.
