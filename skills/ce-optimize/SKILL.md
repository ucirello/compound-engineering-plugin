...
The files under `.context/ce-optimize/<spec-name>/` are local scratch state. They are ignored by the VCS, so they survive local resumes on the same machine but are not preserved by changes, bookmarks, or pushes unless the user exports them separately.
...
- **Resume**: read ALL state from the experiment log on disk (do not rely on any in-memory context from a prior session). Recover any measured-but-unlogged experiments by scanning JJ workspace directories for `result.yaml` markers. Continue from the last iteration number in the log.
- **Fresh start**: run `jj bookmark rename "optimize/<spec-name>" "optimize-archive/<spec-name>/archived-<timestamp>"`, clear the experiment log, and start from scratch

### 0.5 Create Optimization Bookmark and Scratch Space

For a fresh run (after archiving an existing bookmark when applicable):
```bash
jj bookmark create "optimize/<spec-name>" -r @
jj new "optimize/<spec-name>"
```

For a resume, leave the bookmark in place and start a working-copy change from it:
```bash
jj new "optimize/<spec-name>"
```
...
Verify there are no unrecorded working-copy changes to files within `scope.mutable` or `scope.immutable`:
...
- Ask the user to describe and finalize the current change, or move the work into another JJ workspace, before proceeding
...
**Workspace backend** (`execution.backend: worktree` in existing specs):
1. Create an experiment JJ workspace:
   ```bash
   SKILL_DIR="<absolute path of the directory containing this SKILL.md>"
...
3. Create the filled prompt under the workspace-local temporary directory:
   ```bash
    ROOT=$(jj workspace root 2>/dev/null || pwd -P)
    PROMPT_DIR="$ROOT/.tmp/rocketclaw/ce-optimize"
    mkdir -p "$PROMPT_DIR"
    PROMPT_INDEX=0
    while :; do
      PROMPT_FILE="$PROMPT_DIR/optimize-exp-$$-$PROMPT_INDEX.txt"
      (umask 077; set -C; : > "$PROMPT_FILE") 2>/dev/null && break
      PROMPT_INDEX=$((PROMPT_INDEX + 1))
    done
    ```
4. Dispatch via Codex:
   ```bash
   codex exec --skip-git-repo-check - < "$PROMPT_FILE" 2>&1
   ```
...
3. **If best improves on current best: KEEP**
   - Include only mutable-scope changes in the experiment's JJ change; if no eligible diff remains, treat the experiment as non-improving and discard it
   - Read the project's active local instructions and conventions, then inspect relevant history before composing the change description. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards.
   - Local instructions and the syntax observed in `git log` take precedence over any other guidance; use Go guidance only when compatible. Do not impose a fixed type, scope, prefix, or template; preserve only the semantic requirement that the description identify the experiment and hypothesis.
   - Run `jj describe -r <experiment-revision> -m "<message composed from local standards>"`, then advance the optimization bookmark with `jj bookmark set "optimize/<spec-name>" -r <experiment-revision>`
   - After the bookmark advances, clean up the winner's experiment workspace; the bookmarked JJ change is the durable artifact
...
4. **Check file-disjoint runners-up** (up to `max_runner_up_merges_per_batch`):
   ...
   - If disjoint: rebase the runner-up change onto the new baseline with `jj rebase -s <runner-up-revision> -d "optimize/<spec-name>"`, then re-run full measurement
   - If combined measurement is strictly better: advance the optimization bookmark to the rebased runner-up (outcome: `runner_up_kept`), then clean up that runner-up's experiment workspace
   - Otherwise: leave the optimization bookmark at the prior baseline, log as "promising alone but neutral/harmful in combination" (outcome: `runner_up_reverted`), then clean up the runner-up's experiment workspace
...
6. **Discard all others**: clean up workspaces, log as `reverted`
...
- Discard the experiment change and clean up its workspace
...
The optimization bookmark (`optimize/<spec-name>`) is preserved at the accumulated kept changes.
...
1. **Run `/ce-code-review`** on the cumulative diff (baseline to final). Load the `ce-code-review` skill at the optimization bookmark (interactive or `mode:agent`). To land eligible fixes before the next option, apply the mechanical-apply bar below.

   **Mechanical-apply bar:** apply any finding with a concrete `suggested_fix` that is a clear, reversible improvement; push back (keep, don't apply) when the reviewer is wrong, noting why. Defer anything whose right fix needs a design or product decision (architecture direction, contract shape, behavior change needing sign-off) and any finding with no concrete fix to act on — surface what was deferred. Confirm evidence still matches at `file:line` before editing. After applying, run tests (at least targeted tests for what changed; broader suite for multi-file edits). Do not describe, bookmark, or push from this step — leave the working-copy change at the optimization bookmark for the Create PR option.
