...
When invoking any skill referenced below, resolve its name against the available-skills list the host platform provides and use that exact entry. Some platforms list skills under a plugin namespace; others list the bare name. Invoking a short-form guess that isn't in the list will fail — always match a listed entry verbatim before calling the Skill/Task tool.
...
Put every temporary file or directory this pipeline creates under `<workspace-root>/.tmp/rocketclaw/lfg/<run-id>/`. If `jj workspace root` is unavailable, use `.tmp/rocketclaw/lfg/<run-id>/` relative to the current directory. Do not place temporary data outside that workspace-local hierarchy or use a runtime-default location. Repo-bound context artifacts remain under `<workspace-root>/.context/`.
...
3. Invoke the `ce-simplify-code` skill on the current Jujutsu stack diff.

   This runs before review so the code-review in step 4 covers the simplified code. **Skip** this step when the change is docs-only (only markdown/docs paths changed) or trivial (roughly under 10 changed lines). Otherwise let `ce-simplify-code` resolve the current-stack scope itself; it preserves behavior and runs the test suite. Pass the plan path from step 1 as structure-pin context, not as the simplification scope (the current Jujutsu stack remains the scope), with a one-line constraint: `session-settled:`-labeled KTDs are structure pins the simplification must preserve (deliberate duplication stays duplicated).
...
**Shipping precondition (steps 5–9).** Run `jj git remote list` once before the shipping steps. If it lists **no remote** (e.g. a sandbox/throwaway Jujutsu workspace initialized without a remote), shipping is **local-only**: finish every change the steps below call for, but **skip every push, PR create/edit, and CI-watch action** — the pushes in steps 5 and 6, the push and PR creation in step 8, and step 9 in full. A missing remote is a terminal local-only state, not an error: never retry a push or hunt for a remote — finish the local changes and proceed to step 10. Run steps 5–9 normally when a remote exists.

5. **Apply and persist review fixes** (REQUIRED after step 4, before residual handoff)

   Load `references/review-followup.md` and execute its apply step (mechanical apply + describe/finish/push when changes exist). Do not proceed to the residual handoff, run browser tests, or output DONE while eligible review fixes remain only in the unfinished working-copy change.
...
8. Ship: the goal is the remaining work described and finished, its bookmark pushed, and an open PR whose URL you hold. If the project's active instructions name a process that owns that handoff (a named skill or command, a stacking tool, or documented steps — not change-description or PR-title conventions, which the default honors; a skill directory alone is not a directive), run it non-interactively with the same plan path and context below. It is done only when the bookmark is pushed and you hold the URL of an open PR containing it — one the process opened, or the existing PR from step 6; if it cannot run headlessly, is unavailable, or ends short of that state, stop as **blocked** naming the process — do not fall through to the default or to step 9. Otherwise: invoke the `ce-commit-push-pr` skill with `mode:pipeline`. Thread the recorded plan path from step 1 into the invocation, along with any proceeded-and-flagged `settled_decision_conflicts` entries from step 2, so the PR body's settled-decisions provenance line and its proceed-under-flag clause can fire.

   Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards.

   The project's active instructions and the description syntax observed at runtime in `jj log` win. Apply compatible Go guidance only to quality, clarity, and structure; it does not prescribe imperative mood, casing, punctuation, line wrapping, subject/body shape, or any fixed syntax. Every Jujutsu example must use a dynamic placeholder such as `jj commit -m "<change-description>"`; never supply a fixed message for the agent to reuse.

   This describes and finishes any remaining working-copy change, moves or creates a dynamically named bookmark at the finished change, pushes that bookmark, and opens a pull request — non-interactively, per the mode token. If it prints a `New concepts:` trailer after the PR URL, record the concept name(s) for step 10. Once the PR URL is known, back-fill it into any residual tickets filed in step 6 (the `filed` list) so each ticket links to the PR carrying the finding — best-effort, and never block DONE on a failed ticket update. If step 6 already opened a PR (check with `gh pr view --json number,url,state`), skip PR creation but still finish any non-empty working-copy change, set the existing PR bookmark to the finished revision with `jj bookmark set <bookmark> -r @-`, and push it with `jj git push --bookmark <bookmark> --remote <remote>`. **Per the shipping precondition, when no remote is configured, do NOT invoke `ce-commit-push-pr` or a project-defined shipping process whose contract requires a remote. Instead, if `jj diff --summary` reports changes, compose a dynamic description under the rule above and run `jj commit -m "<change-description>"`; then skip bookmark push and PR creation entirely.**

9. **Watch the PR to CI-decided via `ce-babysit-pr`** (only when an open PR exists for the current pushed bookmark)
