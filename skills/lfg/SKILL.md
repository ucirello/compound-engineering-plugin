...
3. Invoke the `ce-simplify-code` skill on the current JJ change-stack diff.
   ...
   Do not describe or finalize the JJ change in this step. `ce-simplify-code` leaves its edits in the working copy; step 4 reviews that working-copy change, and step 8's `ce-commit-push-pr` describes and ships whatever remains. Describing it here could combine still-undescribed `ce-work` edits under an inaccurate message.
...
**Shipping precondition (steps 5–9).** Run `jj git remote list` once before the shipping steps. If it lists **no remote**, shipping is **local-only**: describe and finalize every JJ change the steps below call for, but **skip every push, PR create/edit, and CI-watch action**. A missing remote is a terminal local-only state, not an error: never retry a push or hunt for a remote. Run steps 5–9 normally when a remote exists.

5. **Apply and persist review fixes** (REQUIRED after step 4, before residual handoff)

   Load `references/review-followup.md` and execute its apply step (mechanical apply plus JJ describe/bookmark push when changes exist). Do not proceed to the residual handoff, run browser tests, or output DONE while eligible review fixes remain only in an undescribed working-copy change.
...
6. **Autonomous residual handoff** (only when step 4 reported one or more actionable `downstream-resolver` findings not applied in step 5; skip when it reported `Actionable findings: none.`)
   ...
   5. If an open PR exists, update it directly with `gh`; do not load any confirmation-driven PR update skill. Append or replace the `## Residual Review Findings` section in the current PR body. Set `WORKSPACE_ROOT="$(jj workspace root 2>/dev/null || pwd)"`, run `mkdir -p "$WORKSPACE_ROOT/.tmp"`, write the new body beneath `$WORKSPACE_ROOT/.tmp/`, then run:

      ```bash
      gh pr edit PR_NUMBER --body-file "$WORKSPACE_ROOT/.tmp/<pr-body-file>"
      ```

   6. If no open PR exists, create a tracked fallback file at `docs/residual-review-findings/<bookmark-or-change-id>.md` containing the composed section and the source PR-review run context. Describe and finalize a JJ change containing only that file, then push its bookmark when a remote is configured. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Repository-local instructions and `git log` syntax always win; apply compatible Go quality guidance. Use `<message-derived-from-repository-standards-and-the-residual-finding-change>` rather than a fixed prefix, type, scope, subject, template, or example. Inspect with `jj diff`, set the description with `jj describe -m <message>`, create the next change with `jj new`, ensure the shipped change has a local bookmark using `jj bookmark list` and `jj bookmark set <bookmark> -r <shipped-change>`, then run `jj git push --bookmark <bookmark> --remote <remote>`. Resolve `<remote>` from `jj git remote list`, preferring `origin` when present. If there is no remote, the locally described JJ change is the durable sink. A push failure with a configured remote is a stop-and-report.
...
8. Invoke the `ce-commit-push-pr` skill with `mode:pipeline`.
   ...
   This describes any remaining JJ changes, pushes the bookmark with `jj git push`, and opens a pull request non-interactively. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Repository-local instructions and `git log` syntax always win; apply compatible Go quality guidance. Require a dynamic `<message-derived-from-repository-standards-and-the-actual-change>` and reject fixed prefixes, types, scopes, subjects, templates, or examples. If it prints a `New concepts:` trailer after the PR URL, record the concept name(s) for step 10. If step 6 already opened a PR, skip PR creation but still describe and push remaining changes. **When no remote is configured, do not invoke `ce-commit-push-pr`; use `jj status`, `jj diff`, `jj describe -m <message>`, and `jj new`, then skip bookmark push and PR creation.**
...
9. **CI watch and autofix loop** (only when an open PR exists for the current bookmark)
   ...
   4. Keep only the files changed for this fix in the current JJ change, describe it, and push its bookmark. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Repository-local instructions and `git log` syntax always win; apply compatible Go quality guidance. Use `<message-derived-from-repository-standards-and-the-CI-fix>` rather than a fixed prefix, type, scope, subject, template, or example:
   ...
   - Append or replace this section in the PR body. Set `WORKSPACE_ROOT="$(jj workspace root 2>/dev/null || pwd)"`, run `mkdir -p "$WORKSPACE_ROOT/.tmp"`, write the new body beneath `$WORKSPACE_ROOT/.tmp/`, then run:
