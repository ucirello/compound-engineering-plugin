...
3. **Current change stack vs base bookmark.** If the working copy has no standalone edits, review `jj diff --from <base> --to @` where `<base>` is the default bookmark.
...
- Rely on diff hunks in the provided `<diff>` only.
