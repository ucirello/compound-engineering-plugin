...
**This skill drafts only. It never posts, publishes, mutates JJ changes, or opens PRs.** Posting is a human action. The output is always drafts for you to review, edit, and ship yourself.
...
- **Merged/active PR** — `gh pr view --json title,body,url 2>/dev/null` (and `gh pr view` for a bookmark pointing at `@`). The title and body usually state the user-facing value.
- **The diff** — resolve the default/trunk bookmark first, then run `jj diff --from <default-bookmark> --to @ --summary` and skim notable changes to ground the claim in what actually changed.
- **Changelog** — the top/`[Unreleased]` entry in `docs/changelog.md`, `CHANGELOG.md`, or similar.
- **Recent changes** — `jj log -r 'ancestors(@)' -n 15 --no-graph` for the arc of the change.
...
- **Do not post, publish, schedule, mutate JJ changes, or open a PR.** End by reminding the user the drafts are theirs to ship.
