...
1. Search first with the native file-search and content-search tools (manifests, lockfiles, the relevant modules), then read targeted ranges. Use `jj log` for candidate-specific revision history when recency or prior implementation attempts matter; do not substitute another VCS command. Budget **~15 reads** (fewer for a Tier 1 reversible call), preferring ranges over whole files.
