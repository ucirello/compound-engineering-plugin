...
- **Merge-state claims** ("fixed in #1608", "landed", "shipped") verify against **remote truth** — the checkout may predate a merge, so `gh pr view` (or the tracker equivalent) is primary and JJ reachability through `trunk()` is only the fallback. If `trunk()` is unavailable, the script reports that classification is limited to `@`; treat merge-state verification as degraded rather than guessing.
...
| path missing here, exists at `trunk()` | Current change differs from trunk | Verify the intended state; annotate if the doc implies the file is present locally |
...
| JJ change/commit ID does not resolve | Fabricated, stale, or from another repo | Replace with a durable review reference, or drop |
| revision ID reachable from `@` only | Current-change identity may be rewritten by publication | Prefer a durable review reference |
| revision ID reachable from `trunk()` only | Workspace does not include the trunk revision | Keep with a temporal qualifier; verify the landed claim via `gh` |
| revision ID exists but is unreachable | Abandoned or rewritten history | Prefer a durable review reference |
...
If the script cannot be resolved on this platform, apply its checks manually at the same scope — scan the body for cited paths that don't exist, JJ change/commit IDs, `Learning(s) N` / `{{…}}` scaffold, and broken relative links — and note in the run output that the check was manual. Do not silently skip.
...
knowledge store. You are read-only: never edit files. Inspect with native
read/search tools, read-only jj commands, and gh when available.
...
2. MERGE-STATE CLAIMS — assertions that a change landed ("fixed in",
   ...
   reachability through trunk(). Verdict: verified,
