...
Do not finalize a change in this step. `ce-simplify-code` leaves its edits in the working-copy change; step 4's review includes that change, and step 9's `ce-commit-push-pr` finalizes whatever remains. Finalizing here would sweep unfinished `ce-work` edits into a misleading simplification-only commit and could stall on a working copy that still carries unfinished work.
...
4. If eligible fixes were applied, inspect `jj diff` and finalize only review-driven changes with `jj commit -m "<message>" <review-driven-paths>`. If a path also contains pre-existing edits, separate the owned hunks before finalizing; never include unrelated work merely because it shares a filename. JJ leaves the remainder in the new working-copy change. Before composing the message, read https://go.dev/wiki/CommitMessage. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Here `git log` means history inspected using `jj log`; never execute Git. Runtime project instructions and history syntax override the Go guidance. Then push before step 6 **when a remote is configured** (per LFG's shipping precondition) and, on the defect route, when everything the push would publish is work the user offered (`references/debug-return.md`). Ensure the run's bookmark points at the finalized revision, not unfinished `@`; select its configured writable remote, otherwise prefer `origin` or the first writable remote from `jj git remote list`. Publish only that bookmark with `jj git push --remote <remote> --bookmark <bookmark>`; this also publishes and tracks a new bookmark. If there is no remote, the local commit suffices. If no eligible fixes were applied, say so explicitly and skip the commit.
...
Invoke `ce-compound` with `mode:non-interactive` when the run produced durable reasoning that the final code, tests, and plan or diagnosis do not carry, and losing it would plausibly cause recurrence or substantial rediscovery. The plan or `root_cause`, the review residuals, and the fixes applied in step 5 are the evidence for that judgment. `ce-compound` writes into the repository's tracked learnings store in the working-copy change and asks nothing; `Documentation skipped` is a successful outcome, not a stop. Step 9 commits and pushes whatever it wrote with the rest of the change, so the learning is in the PR at the moment it opens and CI runs against a head that contains it. Do not run it after the babysit result: a push after "CI decided" leaves an unwatched head.
