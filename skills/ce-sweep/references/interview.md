...
Loaded by `SKILL.md` when `/ce-sweep` runs with no `feedback_sources` configured. Captures the setup that will be merged into `<repo-root>/.rocketclaw/config.local.yaml` (the ignored, workspace-local config) and re-read on every subsequent run.
...
- **Workspace-local under `.tmp/rocketclaw`** (solo setups; keeps sweep bookkeeping out of changes). Sets `sweep_state_path` to `$(jj workspace root)/.tmp/rocketclaw/sweep/state.yml`; if workspace-root resolution fails, use `.tmp/rocketclaw/sweep/state.yml` relative to the current directory.

Let the user override the path if they want a different location. If they pick workspace-local, note that a fresh workspace or a teammate's machine will not see this state — it is per-machine by design.
...
**Skip this section entirely if the user chose workspace-local state in section 4** — the shared-bookmark topology only applies to tracked state.

**Ask:** "Is this a multi-agent setup where several workspaces publish the sweep state through a shared docs bookmark? Answer yes only if more than one machine or agent finishes changes and pushes the same bookmark. Default is no — a single workspace recording changes locally."

- **No** (default) -> `sweep_shared_branch: false`. The single-writer lease serializes overlapping sweeps within one workspace.
- **Yes** -> `sweep_shared_branch: true`. Ask for the shared bookmark name and remote. Inspect `jj bookmark list` for name collisions and validate the remote with `jj git remote list`; when exactly one remote exists, offer it as the default, and when several exist, prefer `origin` but let the user choose. Explain: the lease becomes **push-gated** — before any source-side write, the sweep finishes and pushes the lease-acquisition change on the shared bookmark and confirms its writer won, making the lease a workspace-wide mutex across machines. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. The project's active instructions and conventions already in context take precedence, followed by recent history inspected at runtime with the repository's preferred `git log` syntax. Apply Go guidance only where compatible: imperative clarity, a concise subject, a body that explains why when useful, and repository-preferred wrapping. Treat the lease acquisition's repository-relevant semantic facts as composition context, not fixed subject or body requirements. Derive all wording and structure at runtime; do not impose fixed syntax, messages, prefixes, types, scopes, subjects, bodies, templates, or examples.

**Capture:** `sweep_shared_branch` (`true` | `false`), plus `sweep_shared_bookmark` and `sweep_shared_remote` when true.
...
Merge the captured settings into `<repo-root>/.rocketclaw/config.local.yaml`. Resolve the workspace root with `jj workspace root`.
...
- If `.rocketclaw/config.local.yaml` is not already ignored by the workspace's active ignore rules, offer to add the entry before writing.
...
- `sweep_shared_branch` — from section 6 (default `false`; only meaningful with tracked state).
- `sweep_shared_bookmark` and `sweep_shared_remote` — from section 6 when shared-bookmark mode is enabled; omit otherwise.
...
sweep_state_path: docs/feedback-sweep/state.yml   # tracked (multi-agent) or .tmp/rocketclaw path (solo)
...
sweep_shared_branch: false                        # true: push-gated lease for shared-bookmark topology
# sweep_shared_bookmark: feedback-sweep           # required when sweep_shared_branch is true
# sweep_shared_remote: origin                     # required when sweep_shared_branch is true
~~~
