...
- Write ephemeral output under `$(jj workspace root)/.tmp/rocketclaw/ce-pov/`; if there is no Jujutsu workspace, use `.tmp/rocketclaw/ce-pov/` under the current project folder. Use `docs/` when the user wants it kept, and announce the absolute path. Do **not** introduce a new mandated `docs/` location — that store is deferred.
...
- Omit creator-credit metadata, visual credit marks, and standalone product branding. If a required schema has an actor field, use `ai:assistant`; if it requires a display name, use `AI Assistant`.

## Sharing

Publish via whatever the user has — best available, never required:

- `ce-proof` (Proof) — markdown-only, so if the report is HTML, render an ephemeral markdown copy under the same workspace-local `.tmp` tree as the Proof source.
