...
**Pre-resolve the scratch directory.** Generate a `<run-id>` once (8 hex chars) and reuse it for the V15 cache and the Phase 2/4 checkpoints so they share one per-run directory. Scratch lives under the workspace `.tmp` directory — never `.context/`. Run this to resolve the workspace root (or cwd if not a JJ repo), create the run directory, and capture its absolute path:

```bash
ROOT="$(jj workspace root 2>/dev/null || pwd)"
mkdir -p "$ROOT/.tmp"
SCRATCH_DIR="$ROOT/.tmp/ce-ideate/<run-id>"
mkdir -p "$SCRATCH_DIR"
echo "$SCRATCH_DIR"
```
