...
argument-hint: "[mode:agent] [blank to review current bookmark/working copy, or provide PR link]"
---
...
Parse `$ARGUMENTS` for optional tokens. Strip each recognized token before interpreting the remainder as a PR number, GitHub URL, bookmark name, or JJ revision.
...
| `mode:report-only` | `mode:report-only` | **Deprecated — ignored.** Former no-artifacts mode; default behavior is review-only without moving the working copy |
| `base:<revision-or-bookmark>` | `base:abc1234` or `base:main@origin` | Diff base on the **current working copy** (explicit; skips auto base detection) |
...
- Multiple incompatible scope selectors appear together (e.g. `base:` **and** a PR number/bookmark-or-revision target — `base:` means "review the current working copy against this base")
...
- **Apply locally; never push.** Never push, open PRs, or file tickets in any mode — push is the outward step the user owns. In **default (interactive)** mode the review applies safe, verified fixes and describes them when the pre-review working copy was clean (Stage 5c owns the full rule). In **`mode:agent`** it never mutates the tree — it reports and the caller applies.
...
- **Explicit mutations only.** Never run `gh pr checkout`, `jj new`, `jj edit`, `jj bookmark set`, or similar working-copy/bookmark/workspace-switch commands. Do **not** edit or switch the working copy/bookmark/workspace to satisfy review scope. Passing a PR number, URL, bookmark name, or JJ revision selects **review scope**, not permission to mutate the working copy. To review local working-copy changes on a topic bookmark, move to that bookmark/workspace yourself (or stay on it) and pass `base:` or no target.
- **Smart defaults.** Untracked files: review tracked changes only and list excluded paths in Coverage. Plan: use `plan:` when passed; otherwise discover conservatively from PR body or bookmark/revision keywords. Weak advisory P2/P3 from testing/maintainability alone: demote to `testing_gaps` / `residual_risks` per Stage 5.
- **Report outcomes, not machinery.** What you show the user is about the review: what's being examined (the PR/bookmark/revision), which coverage is included and the one-line reason for each conditional lens, the independent cross-model pass and which model runs it, and the findings. Keep the skill's internals out of user-facing text — model-tier assignments, raw scope-mode codenames (`local-aligned`/`pr-remote`), writing the diff to disk, loading persona files, parallel-dispatch bookkeeping, and step-by-step narration of your own setup. Name what the user would recognize (a PR number, a reviewer's concern, a peer model), not the plumbing. This governs *what* you surface and suppress; it does not script the wording — use your own voice.
...
- `docs/plans/*.{md,html}` -- unified plan artifacts created by ce-brainstorm or ce-plan (decision artifacts; execution progress is derived from JJ state, not stored in plan bodies)
...
The caller already knows the diff base. Skip all base bookmark/revision detection and remote resolution. Use the provided value directly:

```
BASE_ARG="{base_arg}"
BASE="latest(ancestors(@) & ancestors($BASE_ARG))"
```

Then produce the same output as the other paths:

```
echo "BASE:$BASE" && echo "FILES:" && jj diff --from "$BASE" --to @ --name-only && echo "DIFF:" && jj diff --from "$BASE" --to @ && echo "WORKING_COPY_STATUS:" && jj status
```

This path works with any JJ revision or bookmark. Callers reviewing the current working copy should pass explicit `base:` when auto-detection is unnecessary. **Do not combine `base:` with a PR number or bookmark/revision target.** If both are present, stop with an error: "Cannot use `base:` with a PR number or bookmark/revision target — `base:` implies the current working copy is already the correct target. Pass `base:` alone, or pass the target alone and let scope detection resolve the base."

**If a PR number or GitHub URL is provided as an argument:**

Review PRs without moving the workspace: do **not** move the working copy to the PR head. Scope comes from GitHub read APIs plus optional local alignment when `@` already matches the PR head revision.
...
- **Trivial-PR judgment**: spawn a lightweight sub-agent on the platform's cheapest capable model when a known override exists; otherwise omit the model override and inherit. Give it the PR title, body, and changed file paths. The agent's task: "Is this an automated or trivial PR that does not warrant a code review? Consider: dependency lock-file or manifest-only bumps, automated release changes, chore version increments with no substantive code changes. When in doubt, answer no — false negatives (skipped reviews that should have run) are more costly than false positives (unnecessary reviews)." If the judgment returns yes: stop with reason `PR appears to be a trivial automated PR; not reviewing. Run without a PR argument to review the current bookmark/working copy, or pass base:<revision> if review is intended.`

When any skip rule fires, stop without dispatching reviewers. **Default mode:** emit the reason as plain text. **`mode:agent`:** emit JSON only — `{"status":"skipped","reason":"<same message>"}` — so programmatic callers can parse the outcome. **Standalone**, **`base:`**, and **remote-bookmark** paths are unaffected. **Draft PRs are reviewed normally.**

If no skip rule fires, fetch PR metadata **without moving the working copy**:
...
Set `BASE:` to `pr:<number-or-url>` (logical marker — not a JJ revision). Set `WORKING_COPY_STATUS:` from `jj status` on the **current** working copy (usually empty during PR-remote review).
...
1. `jj bookmark list --revisions @` includes `headRefName`.
2. The PR is **not** cross-repository (`isCrossRepository` is false).
3. The PR head commit is contained in the local working copy ancestry: `jj log -r '<headRefOid> & ::@' --no-graph -T commit_id` returns a commit ID. This confirms the working copy actually carries the PR head (allowing unpushed local fixes layered on top) rather than an unrelated same-named bookmark.
...
- **`local-aligned`:** Resolve `<resolved-base-revision>` from `baseRefName` (fetch if needed). Set `BASE="latest(ancestors(@) & ancestors(<resolved-base-revision>))"`, then set `FILES:` from `jj diff --from "$BASE" --to @ --name-only` and `DIFF:` from `jj diff --from "$BASE" --to @` (includes described and working-copy changes on the PR head/bookmark). Do **not** call `gh pr diff` or append remote hunks — when unpushed fixes exist, the local working copy is canonical. Note in Coverage: `scope: local-aligned (PR; local tree diff)`.
- **`pr-remote`:** Set `FILES:` from the PR `files` array. Set `DIFF:` from `gh pr diff <number-or-url> --color=never`. If `gh pr diff` fails, stop with an actionable error — do not fall back to moving the working copy.

When **`pr-remote`**, before Stage 4:

1. Best-effort fetch PR head without moving the working copy: `jj git fetch --remote origin --branch <headRefName>`.
2. When fetch succeeds, set `PR_HEAD_REV=<headRefName>@origin` for reviewers and validators. When fetch fails, omit `PR_HEAD_REV` and note in Coverage — reviewers must rely on diff hunks only.
3. Best-effort fetch the PR base without moving the working copy: `jj git fetch --remote origin --branch <baseRefName>`. When it succeeds, set `PR_BASE_REV=<baseRefName>@origin` — a **real JJ base revision/bookmark** reviewers and validators use for file-level JJ diffs (e.g. `data-migration-reviewer` runs `jj diff --from <PR_BASE_REV> --to <PR_HEAD_REV> -- db/schema.rb`/`structure.sql`). The `pr:<number-or-url>` logical marker in `BASE:` stays the scope marker; `PR_BASE_REV` is the diffable base revision. When the fetch fails, omit `PR_BASE_REV` and note in Coverage — schema-drift and other JJ-diff checks fall back to diff hunks only and must **not** assume `main`.
4. Include `<pr-scope-mode>pr-remote</pr-scope-mode>` and, when set, `<pr-head-revision>...</pr-head-revision>` and `<pr-base-revision>...</pr-base-revision>` in the Stage 4 review context bundle.
...
**If a bookmark name or JJ revision is provided as an argument:**

Substitute the provided bookmark or revision as `<target>`. Do **not** move the working copy to `<target>`.

If `jj bookmark list --revisions @` includes `<target>`, use the **standalone (current bookmark)** path below — same working copy, explicit bookmark name; do not use remote-only diff.

Otherwise diff the remote bookmark or local revision **without moving the working copy**:

1. Try `gh pr view <target> --json baseRefName,url,headRefName` — if a PR exists, prefer the **PR number/URL path** above (same remote diff rules).
2. Else resolve `<target>` as `<target>@origin` or `<target>` after `jj git fetch --remote origin --branch <target>` when needed.
3. Resolve the default base bookmark/revision (same logic as standalone). Set `BASE="latest(ancestors(<target-revision>) & ancestors(<base-revision>))"` and run `jj diff --from "$BASE" --to <target-revision>`.
4. If `<target-revision>` cannot be resolved locally, stop: "Cannot diff bookmark/revision `<target>` without moving the working copy. Move to that bookmark/workspace, pass its open PR URL/number, or review the current bookmark with `base:`."

On success for remote bookmark diff, set **remote-bookmark scope**. The working copy is **not** `<target>`. Include `<pr-scope-mode>remote-bookmark</pr-scope-mode>` and `<remote-head-revision><target-revision></remote-head-revision>` in the Stage 4 review context bundle. Reviewers and Stage 5b validators must **not** Read/Grep workspace paths for files in `FILES:`. Inspect via `jj file show -r <target-revision> <path>` or diff hunks only.

Produce:

```
echo "BASE:$BASE" && echo "FILES:" && jj diff --from "$BASE" --to <target-revision> --name-only && echo "DIFF:" && jj diff --from "$BASE" --to <target-revision> && echo "WORKING_COPY_STATUS:" && jj status
```

**If no argument (standalone on current bookmark/working copy):**

Apply the same base-detection logic as bookmark/revision mode above, using the current bookmark (i.e., `gh pr view --json baseRefName,url` with no argument defaults to the current bookmark when `gh` can resolve it).

If no base can be resolved, **stop**. Do not fall back to `jj diff -r @` or `jj diff --from @- --to @` — a standalone review without the base would only show the working-copy change and silently miss earlier work on the bookmark/change stack.
...
Using `jj diff --from "$BASE" --to @` diffs the JJ common ancestor against the working-copy commit, which includes described and working-copy changes together.

**New-file handling:** Always inspect `WORKING_COPY_STATUS:`. New paths included in the JJ change are in scope; ignored files outside JJ's tracked change are out of scope. When out-of-scope paths are present, list excluded files in Coverage and continue on tracked changes only — never stop or prompt.
...
- **`local-aligned` / standalone / `base:`** — `DIFF_A="$BASE"` (a real JJ revision/bookmark), `DIFF_B` empty (diffs base vs working copy).
- **`pr-remote` / `remote-bookmark`** — `DIFF_A=<PR_BASE_REV>`, `DIFF_B=<PR_HEAD_REV>` (or `<remote-head-revision>`) — the **fetched** revisions from Stage 1. Do **not** model-count from hunks (it drifts per host/model). If either revision was not fetched, skip the block and emit `EXEC_LINES:UNKNOWN` + `UNCOUNTED_FILES:1` so Stage 3c forces the full roster.

```
# Fail closed: an unresolved/invalid endpoint must NOT become a silent EXEC_LINES:0.
# Validate BOTH endpoints — a set-but-unfetched DIFF_B (pr-remote/remote-bookmark head revision)
...
if [ -z "${DIFF_A:-}" ] || ! jj log -r "$DIFF_A" --no-graph -T commit_id >/dev/null 2>&1 \
  ...
  DIFF_TO_ARGS=()
  [ -n "${DIFF_B:-}" ] && DIFF_TO_ARGS=(--to "$DIFF_B")
  # EXEC_LINES via --stat (per-file changed counts) summed in awk — robust across
  ...
  # globs single-quoted and inline so the shell passes them to JJ literally (an unquoted or
  # variable-held glob would be expanded against the CWD before JJ ever sees it).
  EXEC_LINES=$(jj diff --from "$DIFF_A" "${DIFF_TO_ARGS[@]}" --stat -- '*.rb' '*.py' '*.js' '*.jsx' '*.ts' '*.tsx' '*.go' '*.rs' '*.java' '*.swift' '*.kt' '*.c' '*.cc' '*.cpp' '*.cs' '*.php' '*.ex' '*.exs' '*.scala' 2>/dev/null | awk '/\|/ {for (i=1; i<=NF; i++) if ($i ~ /^[0-9]+$/) s+=$i} END{print s+0}')
  echo "EXEC_LINES:$EXEC_LINES"
  FILES=$(jj diff --from "$DIFF_A" "${DIFF_TO_ARGS[@]}" --name-only 2>/dev/null)
...
**Bookmark/revision mode:** Run `jj log -r "${BASE}..<target-revision>" --no-graph -T 'commit_id.short() ++ " " ++ description.first_line() ++ "\n"'` using the resolved JJ common ancestor and resolved revision from Stage 1. Use `<target-revision>` (the resolved `<bookmark>@origin` or fetched revision), not the raw `<target>` argument — a remote-only bookmark may have no matching local bookmark, so the raw name would fail or read a stale same-named local bookmark.

**Standalone (current bookmark):** Run:

```
echo "BOOKMARK:" && jj bookmark list --revisions @ && echo "CHANGES:" && jj log -r "${BASE}..@" --no-graph -T 'commit_id.short() ++ " " ++ description.first_line() ++ "\n"'
```
...
**When intent is ambiguous:** Infer from bookmark/revision name, change descriptions, PR title/body, diff, `plan:`, and conversation. Write the best-effort intent summary and note uncertainty in Coverage — never block on a clarifying question.
...
**Only resolve the cache when the working copy is the reviewed tree** — `local-aligned`, standalone, or `base:` scope (Stage 1). In `pr-remote` or `remote-bookmark` scope, **skip Stage 2c entirely**: the helper keys and derives from the local `@`, which is *not* the reviewed revision, so its profile would describe the wrong tree (e.g. `main`'s stack while reviewing a PR that changes manifests); reviewers work from the fetched revisions/diff as those modes already require.
...
5. Review context: intent summary, file list, diff, scope mode (`local-aligned` | `pr-remote` | `remote-bookmark`), and remote head revision (`PR_HEAD_REV` or `<remote-head-revision>`) when set
...
Read-only here means **non-mutating**, not "no shell access." Reviewer sub-agents may use non-mutating inspection commands when needed to gather evidence or verify scope, including read-oriented `jj` / `gh` usage such as `jj diff --from/--to`, `jj file show`, `jj file annotate`, `jj log`, and `gh pr view`. In **`pr-remote`** or **`remote-bookmark`** scope (see Stage 1), inspect changed files via `jj file show -r <remote-head-revision> <path>` or diff hunks — do not Read/Grep workspace paths for files in scope. They must not edit project files, change bookmarks/workspaces, describe changes, push, create PRs, or otherwise mutate the working copy or repository state.
...
That return enters Stage 5 as reviewer `adversarial-<peer>`, like any persona artifact. The pass is **non-blocking** — skip silently when no peer is identified, the peer CLI is missing/unauthed, or it errors/times out. Skip it entirely in `pr-remote` / `remote-bookmark` scope (the peer would review the local working copy, not the reviewed head). Announce per that reference's announce rules — interactive hosts (Claude or Cursor) in default mode only; silent under Codex and in `mode:agent`.
...
3. **Spawn validators with bounded parallelism.** One sub-agent per finding, dispatched independently using the validator template and the same bounded scheduler from Stage 4. Each validator receives:
   ...
   - The full diff
   - The scope mode and remote head revision, mirroring the Stage 4 reviewer bundle: inject `<pr-scope-mode>local-aligned | pr-remote | remote-bookmark</pr-scope-mode>` and, when set, `<pr-head-revision>...</pr-head-revision>` or `<remote-head-revision>...</remote-head-revision>`. The validator template defaults to local-aligned workspace inspection when these are absent, so omitting them in `pr-remote`/`remote-bookmark` makes validators verify findings against the stale working copy — dropping valid findings or confirming false ones on the wrong tree.
   - Inspection access scoped by mode: in `local-aligned`, Read/Grep/`jj file annotate` the cited code, callers, guards, framework defaults, and history; in `pr-remote`/`remote-bookmark`, inspect via `jj file show -r <remote-head-revision> <path>` or the provided diff hunks only — do not Read/Grep workspace paths for files in scope.
...
**Scope invariant.** Apply only when the working tree *is* what was reviewed — the JJ working copy is `local-aligned` or standalone. In `pr-remote` / `remote-bookmark` the working copy is not the reviewed head; do not apply — report instead.
...
**Describe when the pre-review working copy was clean.** Before applying, note whether the current JJ change already had changes (`jj diff --summary`). The permanence gate is the **push**, not the change description — a local JJ change is private and reversible (`jj restore` for file-level rollback, or `jj undo` for the last operation).

- **Clean before the review:** Commit when the pre-review tree was clean: after applying and verifying, commit the fixes as one isolated, review-labeled fix commit — `fix(review): <summary>`, or the repo's nearest convention if `review` isn't an allowed scope. Labeled and reversible, returning the tree to a known state.
...
2. **Applied (default mode only).** When Stage 5c applied fixes, list them first — before the findings — in an Applied section (see review output template); each entry carries `#`, file, the fix, and reviewer (a multi-file fix is one row with one `#`), then a one-line validation outcome (e.g. "pin tests 4 -> 6; suite 94 pass, lint clean") and JJ change status (described on a clean working copy as `fix(review): …` or the repo's nearest convention, or left in the current JJ change for the user when the working copy already had changes). Flag green-but-unverifiable edits (auth/contract/concurrency) prominently. Omit this section in `mode:agent` and when nothing was applied. Applied findings appear here, not in the severity tables.
...
```json
{
  ...
  "scope": {
    "base": "<JJ common-ancestor revision, pr:NNN marker, or base:<revision>>",
    "bookmark": "<current JJ bookmark(s)>",
...
Do not offer push/PR/create-branch next steps from this skill. In JJ terms, this also means no create-bookmark next steps.
...
{
  "run_id": "<run-id>",
  "bookmark": "<jj bookmark list --revisions @ at dispatch time>",
