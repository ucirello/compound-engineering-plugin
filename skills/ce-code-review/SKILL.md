...
- **No blocking prompts.** Never use `AskUserQuestion`, `request_user_input`, `ask_user`, or other blocking question tools. Infer intent, plan, and scope from explicit tokens, VCS state, PR metadata, and conversation. Note uncertainty in Coverage or the verdict — do not stop to ask.
- **Explicit mutations only.** Never run `gh pr checkout`, raw Git branch-switch commands, or similar checkout/switch commands. Passing a PR number, URL, or branch name selects **review scope**, not permission to mutate the working tree. To review local uncommitted work on a feature bookmark, move to that work yourself (or stay on it) and pass `base:` or no target.
...
- `docs/plans/*.{md,html}` -- unified plan artifacts created by ce-brainstorm or ce-plan (decision artifacts; execution progress is derived from version-control history, not stored in plan bodies)
...
```
echo "BASE:$BASE" && echo "FILES:" && jj diff --name-only --from "$BASE" && echo "DIFF:" && jj diff --git --context 10 --from "$BASE" && echo "STATUS:" && jj st
```

This path works with any JJ revset or ref — a SHA, `<bookmark>@origin`, or a bookmark name. Callers reviewing the current checkout should pass explicit `base:` when auto-detection is unnecessary. **Do not combine `base:` with a PR number or branch target.** If both are present, stop with an error: "Cannot use `base:` with a PR number or branch target — `base:` implies the current checkout is already the correct work. Pass `base:` alone, or pass the target alone and let scope detection resolve the base."
...
Set `BASE:` to `pr:<number-or-url>` (logical marker — not a commit ID). Set `STATUS:` from `jj st` on the **current** checkout (usually irrelevant during PR-remote review).
...
1. The current bookmark from `jj log -r 'heads(::@ & bookmarks())' --no-graph -T 'bookmarks.join(" ") ++ "\n"'` equals `headRefName`.
2. The PR is **not** cross-repository (`isCrossRepository` is false).
3. The PR head commit is contained in the local checkout: `jj log -r '<headRefOid>::@' --no-graph` exits 0 and prints a path. This confirms the working tree actually carries the PR head (allowing unpushed local fixes layered on top) rather than an unrelated same-named branch.
...
- **`local-aligned`:** Resolve `<resolved-base-ref>` as `<baseRefName>@origin` when available, else `<baseRefName>`. Fetch if needed with `jj git fetch --remote origin --branch <baseRefName>`. Set `BASE:` to the resolved ref, `FILES:` from `jj diff --name-only --from "$BASE"`, and `DIFF:` from `jj diff --git --context 10 --from "$BASE"` (includes committed and current working-copy changes on the PR bookmark). Do **not** call `gh pr diff` or append remote hunks — when unpushed fixes exist, the local tree is canonical. Note in Coverage: `scope: local-aligned (PR; local tree diff)`.
...
1. Best-effort fetch PR head metadata without checkout via `gh pr view`; do not create local raw Git refs. If the head is not available as a JJ revset, reviewers must rely on diff hunks only.
2. Best-effort fetch the PR base with `jj git fetch --remote origin --branch <baseRefName>`. When it succeeds, set `PR_BASE_REF=<baseRefName>@origin` — a real JJ base revset reviewers and validators use for file-level diffs (e.g. `jj diff --git --from <PR_BASE_REF> -- db/schema.rb`). The `pr:<number-or-url>` logical marker in `BASE:` stays the scope marker. When the fetch fails, omit `PR_BASE_REF` and note in Coverage — schema-drift and other JJ diff checks fall back to diff hunks only and must **not** assume `main`.
3. Include `<pr-scope-mode>pr-remote</pr-scope-mode>` and, when set, `<pr-base-ref>...</pr-base-ref>` in the Stage 4 review context bundle.

Reviewers and Stage 5b validators in **`pr-remote`** mode must **not** Read/Grep workspace paths for files in `FILES:`. Use only the provided diff hunks unless a JJ revset for the remote head is explicitly provided, in which case inspect via `jj file show -r <remote-head-revset> <path>`. **`local-aligned`** uses normal workspace inspection.

**If a branch name is provided as an argument:**

Substitute the provided branch name as `<branch>`. Do **not** check out `<branch>`.

If the current bookmark from `jj log -r 'heads(::@ & bookmarks())' --no-graph -T 'bookmarks.join(" ") ++ "\n"'` equals `<branch>`, use the **standalone (current branch/bookmark)** path below — same tree, explicit branch name; do not use remote-only diff.
...
3. Resolve default base branch (same logic as standalone). Set `BASE=<base-ref>` and diff with `jj diff --git --context 10 --from "$BASE" --to <branch-ref>`.
4. If `<branch-ref>` cannot be resolved locally, stop: "Cannot diff branch `<branch>` without checkout. Switch to that work, pass its open PR URL/number, or review the current branch with `base:`."
...
```
echo "BASE:$BASE" && echo "FILES:" && jj diff --name-only --from "$BASE" --to <branch-ref> && echo "DIFF:" && jj diff --git --context 10 --from "$BASE" --to <branch-ref> && echo "STATUS:" && jj st
```
...
If no base can be resolved, **stop**. Do not fall back to `jj diff --git` alone — a standalone review without the base would only show current-change work and silently miss all committed work on the bookmark.
...
Using `jj diff --git --from "$BASE"` diffs the base revset against the working copy in Git format, which includes committed changes in the current stack plus current working-copy changes.

**Untracked file handling:** Always inspect `STATUS:`. Unknown paths are out of scope unless they are part of the reviewed JJ change. When present but out of scope, list excluded files in Coverage and continue on tracked changes only — never stop or prompt.
...
if [ -z "${DIFF_A:-}" ] || ! jj log -r "$DIFF_A" --no-graph -T 'commit_id ++ "\n"' >/dev/null 2>&1 \
  ...
  # EXEC_LINES via JJ --stat summed for counted code files — robust across shells and immune
  ...
  EXEC_FILES=$(jj diff --name-only --from "$DIFF_A" $DIFF_TO_ARGS 2>/dev/null | awk '/\.(rb|py|js|jsx|ts|tsx|go|rs|java|swift|kt|c|cc|cpp|cs|php|ex|exs|scala)$/')
  if [ -n "$EXEC_FILES" ]; then
    EXEC_LINES=$(printf '%s\n' "$EXEC_FILES" | xargs jj diff --stat --from "$DIFF_A" $DIFF_TO_ARGS -- 2>/dev/null | awk -F'|' '/\|/ {gsub(/ /, "", $2); s+=$2} END{print s+0}')
  else
    EXEC_LINES=0
  fi
  echo "EXEC_LINES:$EXEC_LINES"
  FILES=$(jj diff --name-only --from "$DIFF_A" $DIFF_TO_ARGS 2>/dev/null)
...
**Branch mode:** Run `jj log -r "${BASE}::<branch-ref>" --no-graph` using the resolved base and branch ref from Stage 1. Use `<branch-ref>` (the resolved `<branch>@origin` or local bookmark), not the raw `<branch>` argument — a remote-only branch may have no matching local bookmark, so the raw name would fail or read stale work.

**Standalone (current branch):** Run:

```
echo "BOOKMARK:" && jj log -r 'heads(::@ & bookmarks())' --no-graph -T 'bookmarks.join(" ") ++ "\n"' && echo "COMMITS:" && jj log -r "${BASE}::@" --no-graph
```
...
Read-only here means **non-mutating**, not "no shell access." Reviewer sub-agents may use non-mutating inspection commands when needed to gather evidence or verify scope, including read-oriented `jj` / `gh` usage such as `jj diff --git`, `jj diff --name-only`, `jj file show`, `jj file annotate`, `jj log`, and `gh pr view`. In **`pr-remote`** or **`branch-remote`** scope (see Stage 1), inspect changed files via `jj file show -r <remote-head-revset> <path>` when a remote head revset is set, or diff hunks only — do not Read/Grep workspace paths for files in scope. They must not edit project files, change branches, commit, push, create PRs, or otherwise mutate the checkout or repository state.
...
3. **Spawn validators with bounded parallelism.** One sub-agent per finding, dispatched independently using the validator template and the same bounded scheduler from Stage 4. Each validator receives:
   ...
   - Inspection access scoped by mode: in `local-aligned`, Read/Grep/`jj file annotate` the cited code, callers, guards, framework defaults, and history; in `pr-remote`/`branch-remote`, inspect via `jj file show -r <remote-head-revset> <path>` or the provided diff hunks only — do not Read/Grep workspace paths for files in scope.
...
{
  "run_id": "<run-id>",
  "branch": "<current branch/bookmark at dispatch time>",
