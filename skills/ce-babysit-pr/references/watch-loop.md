...
| OpenCode V2 | Native background shell tool and its completion notification; keep the session active for the watcher output | No (session-bound) |
...
**Durability:** the in-session watch dies with the session. Re-invoking the skill resumes from disk, because workspace-root `.tmp` persists across ticks. For an unattended multi-day watch, use the durable scheduler and verified harness-specific headless interface in `references/setup.md` from the absolute workspace root. A fresh headless run has no memory of this conversation, so persist consequential decisions to disk. **Shell env vars do not persist between separate tool calls** on any harness — re-set `SKILL_DIR`/`STATE_DIR` inline in every command.
...
- **Managed stack:** trunk movement alone is not a reason to restack. The manager's `needsRebase` flag turns on for plain trunk drift, and GitHub already prices that into the target's mergeability against its parent base, so `stack_blocker` is null and the target may be ready whenever GitHub reports a certain `MERGEABLE`/`CLEAN`. `stack_blocker` is `target-needs-rebase` / `managed-freshness-unknown` only when the target is stale or of unknown freshness **and** GitHub does not clear it; that becomes `stack-blocked`. Never use `gh pr update-branch` or a local base merge on a managed target.
  ...
  After an authorized target push: retain the pushed SHA, re-confirm remote membership/order, require no unrelated workspace changes, fetch the exact target bookmark and verify its local/remote heads remain at that SHA. Start at the first open dependent, or skip when none exists. Apply the bounded JJ slice-rebase and push protocol in `references/stack-commands.md`; never rebase the target or refresh it from trunk. Verify the target remains unchanged before publishing.

  After push success or rejection, re-probe the target against its pushed SHA and every **open dependent** against recorded and expected OIDs. The target's intentional pushed change is not divergence. Retain partial dependent updates as progress and name the first failed layer in a recoverable residual; prefer atomicity when available, never assume it. This continuation is implicit in babysitting a managed layer. Abort a conflicting owned unpublished JJ propagation using its recorded operation only when unrelated work is preserved, then park a complete decision. Target movement, changed membership or rejected remote-state checks require a residual, never an unchecked retry or inferred stack. Re-prove order, ancestry, review and CI before readiness.
