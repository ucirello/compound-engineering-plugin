...
argument-hint: "[blank to simplify current change, or describe what to simplify]"
---
...
2. **Otherwise, in a JJ workspace**, default to the current change's diff (`jj diff`). If the user asks for a range against a base revision or bookmark, use `jj diff --from <base> --to @`. This covers the common case of "simplify everything I've added in this change before opening a PR."
