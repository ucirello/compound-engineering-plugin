...
RUN_DIR=""; for n in 0 1 2 3 4 5 6 7; do candidate="$SCRATCH_ROOT/ce-explain-$(date +%Y%m%d)-$$-$n"; if (umask 077; mkdir "$candidate") 2>/dev/null; then RUN_DIR="$candidate"; break; fi; done;
[ -n "$RUN_DIR" ] || exit 1; chmod 700 "$RUN_DIR" || exit 1;
echo "$RUN_DIR";
```
