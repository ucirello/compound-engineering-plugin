...
2. **Setup Environment**
   ...
   First, resolve the workspace root and inspect the current change, its nearest bookmarks, and trunk. Treat `trunk()` as valid only when `trunk() & ~root()` resolves to exactly one commit; if it is empty or ambiguous, resolve the tracked default bookmark or stop and ask instead of treating `root()` as trunk:
   ...
   Use `$workspace_root/.tmp/rocketclaw/ce-work` for repository-local temporary files. If `jj workspace root` is unavailable, use local `.tmp/rocketclaw/ce-work` relative to the current directory.

   **If already on a feature change line** (the nearest bookmark is not the validated trunk):

   First, check whether the bookmark name is **meaningful** and tells future readers what the work is about. Auto-generated workspace names or other opaque names do not.
   ...
   **If the current change line is based directly on the validated trunk**, choose how to proceed:
   ...
   **Option C: Continue in the current unbookmarked change**
   - Create a meaningful bookmark before `jj git push`
...
4. **Choose Execution Engine, then Strategy**
   ...
   **Dispatch** uses your harness's subagent/worker mechanism. Give each worker:
   ...
   - **Do not describe or commit changes.** Workers implement and may run their *own unit's* focused tests in isolation as a self-check, but the **orchestrator owns change selection, descriptions, commits, and the authoritative test runs**. (Capability note: a harness that *reaps* the isolated workspace on worker completion — none of our current targets do — would instead require the worker to return a durable revision; confirm before assuming it.)
   ...
   **After each serial unit:** review the diff against the unit's scope and `Files:`, run the relevant tests, fix before dispatching the next (never on a broken tree), record the unit's verification evidence from the worker's return (for the Phase 2 `verification_evidence` roll-up), update the task list (never edit the plan body — progress lives in JJ revisions), and commit. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Derive the description at runtime from the project's active instructions and actual `git log`; those sources always win over compatible Go guidance. Do not impose a fixed message, prefix, type, scope, subject/body shape, template, or example. Then dispatch the next unit.
   ...
   **After a parallel batch — the orchestrator integrates; never trust the handoff summary alone:**
   ...
   3. **Detect real collisions** — 2+ workers that actually modified the same file. In a shared workspace only the last writer survived: commit the non-colliding work first, then re-run the colliding units serially so each builds on the other's recorded result. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Derive each description at runtime from the project's active instructions and actual `git log`; those sources always win over compatible Go guidance. Do not impose a fixed message, prefix, type, scope, subject/body shape, template, or example. With harness-native isolation the collision surfaces as a JJ conflict during integration instead (see the per-harness note).
   4. **Review, test, and commit each unit in dependency order — the orchestrator owns commits.** Select only that unit's files, preserving its Goal as semantic context for the description, then run the relevant tests and fix before the next. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Derive each description at runtime from the project's active instructions and actual `git log`; those sources always win over compatible Go guidance. Do not impose a fixed message, prefix, type, scope, subject/body shape, template, or example. Commit the selected files with `jj commit <files>`. Capture each worker's returned verification evidence into the run's `verification_evidence` roll-up — if a worker omitted it, re-derive what the tree allows and mark the rest as unverified rather than fabricating a red-before-implementation observation the worker never reported.
   ...
   **Provider integration (examples — the universal flow above is the contract):**
...
2. **Incremental Commits**
   ...
   After completing each task, evaluate whether to create an incremental commit. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Derive the description at runtime from the project's active instructions and actual `git log`; those sources always win over compatible Go guidance. Do not impose a fixed message, prefix, type, scope, subject/body shape, template, or example.
   ...
   **Heuristic:** commit only when the description can state a complete, valuable change; otherwise wait.

   If the plan has Implementation Units, use them as a starting guide for commit boundaries — but adapt based on what you find during implementation. A unit might need multiple commits if it's larger than expected, or small related units might land together. Preserve each unit's Goal as semantic context when composing the runtime-derived description.
   ...
   **Commit workflow:**
   ...
   # 2. Commit only files related to this logical unit; JJ has no staging area
   jj commit <files-related-to-this-logical-unit>

   # 3. Keep the selected work bookmark at the newest completed revision
   jj bookmark set <bookmark-name> -r @-
   ```

   **Handling conflicts:** If conflicts arise during rebasing or integration, resolve them immediately. Incremental commits make conflict resolution easier since each commit is small and focused.

   **Parallel subagent mode:** Commit ownership is split by isolation mode (see Phase 1 Step 4):
   - **Workspace-isolated:** subagents may describe and commit their revisions inside their isolated workspace; the orchestrator integrates those revisions in dependency order after the batch. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Derive each description at runtime from the project's active instructions and actual `git log`; those sources always win over compatible Go guidance. Do not impose a fixed message, prefix, type, scope, subject/body shape, template, or example.
   - **Shared-directory fallback:** subagents do not commit; the orchestrator selects and commits each unit after the entire parallel batch completes. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Derive each description at runtime from the project's active instructions and actual `git log`; those sources always win over compatible Go guidance. Do not impose a fixed message, prefix, type, scope, subject/body shape, template, or example.
