...
1. **Resolve the target directory and extension.**
   ...
   - **Otherwise (no repo, or elsewhere with no `docs/ideation/`):** write into the run's workspace-local scratch — the `<scratch-dir>` resolved in Phase 1 (`<workspace-root>/.tmp/ce-ideate/<run-id>/`). Do **not** write into the user's current working directory, and do **not** create a `docs/ideation/` tree for a subject unrelated to the repo. Announce the absolute path and note that it is scratch storage.
...
- **Inside a JJ repo:** offer to describe only the ideation-doc change (do not create a bookmark and do not run `jj git push`; if the user declines, leave the change undescribed). Before composing or editing the description, inspect `jj status`, `jj diff`, and `jj log` and isolate the document in its own change when unrelated files are present. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Repository-local instructions and the repository's `git log` syntax always win; apply compatible Go quality guidance. Derive any type, scope, subject, and body structure dynamically from those standards rather than using fixed prefixes or templates. Preserve the semantic requirement that the description explains the ideation document's purpose and substance. Apply it with `jj describe -m <dynamically-composed-message>`.
- **Scratch-area or non-repo file:** skip the change-description offer.
...
- the deliverable was written automatically in both modes (Phase 4) — to `docs/ideation/` when present, else workspace-local scratch, never the user's CWD
