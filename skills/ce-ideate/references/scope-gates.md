...
| Phase | Delta |
|---|---|
| 0.3 mode | **Deterministic — skip Decision 1/2.** `jj workspace root` succeeding → repo-grounded; otherwise elsewhere-software. Never elsewhere-non-software: with no subject there is no naming/narrative/personal intent to infer. No ambiguity-confirmation question. |
...
Classify the **subject of ideation** (settled in 0.2) into one of three modes for dispatch routing. A user inside any workspace can ideate about something unrelated to that workspace; a user with no workspace can ideate about code they hold in their head.
