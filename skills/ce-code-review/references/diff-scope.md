...
2. **Working copy changes.** If the working-copy commit is not empty (`jj diff` is non-empty), review those.
3. **Unpushed changes vs base bookmark.** If the working copy is empty, review `jj diff --from $(jj log -r "heads(::@ & ::<base>)" --no-graph -T 'commit_id' -n 1) --to @` where `<base>` is the default bookmark (main or master).
