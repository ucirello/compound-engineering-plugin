...
1. **JJ activity** — `jj log` over the window (descriptions, change IDs, commit IDs, dates, authors), and for the substantial changes, a stat-level view of what they touched. Group obviously-related changes (a bookmark's stack, a fix and its follow-ups) rather than listing them flat.
...
- What changed, in one line, with the change ID, commit ID, PR number, and date when available
