...
- `target`: exactly one of `codex`, `claude`, `grok`, `cursor`, `composer`, `opencode`, or `opencode2` — a **harness** name, never a model name. `opencode2` is a distinct harness, not backward compatible with `opencode`; never fold an `opencode2` assignment into `opencode`.
