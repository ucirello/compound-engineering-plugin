...
**This skill drafts only. It never posts, publishes, describes/commits VCS changes, or opens PRs.** Posting is a human action. The output is always drafts for you to review, edit, and ship yourself.
...
- **The diff** — `jj diff --stat` and skim notable changes to ground the claim in what actually changed.
- **Changelog** — the top/`[Unreleased]` entry in `docs/changelog.md`, `CHANGELOG.md`, or similar.
- **Recent changes** — `jj log -r '::@' --no-graph -T 'commit_id.short() ++ " " ++ description.first_line() ++ "\n"'` for the arc of the change.
...
- **Do not post, publish, schedule, describe/commit VCS changes, or open a PR.** End by reminding the user the drafts are theirs to ship.
