...
**Read `references/commit-and-push.md`.** It owns feature-bookmark creation, file-granularity grouping, exact filesets, JJ descriptions, bookmark movement, and push. Honor `exclude:<paths>` exactly.

## Step 4: Compose The PR Title And Body

**Read `references/pr-description-writing.md` in full**, then `references/compose.md`. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. The project's active instructions and the description syntax observed at runtime in `jj log` win for change descriptions. Apply compatible Go guidance only to quality, clarity, and structure; it does not prescribe any fixed syntax or example. The references own title/body quality, evidence, teaching, project metadata, preservation, and pre-apply audit. Pass an existing PR URL when rewriting it.

## Step 5: Apply And Report

**Read `references/apply-and-handoff.md`.** Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. The project's active instructions and the description syntax observed at runtime in `jj log` win for any change description created while applying the PR. Apply compatible Go guidance only to quality, clarity, and structure; it does not prescribe any fixed syntax or example. The reference owns routes, preview, archival, workspace-local body files, and babysit handoff.
