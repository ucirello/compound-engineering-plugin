...
2. `cross_model_peer:` from the two repo RocketClaw config files (`config.local.yaml` then `config.yaml`). Supported targets include `codex`, `claude`, `grok`, `cursor`, `composer`, `opencode`, and `opencode2` (`opencode2` is its own branch, not an `opencode` alias). Apply the ordinary-key rule: first active supported target wins; an invalid value continues to the next layer, then step 3.
...
- **opencode2:** distinct from `opencode`; no V1/`opencode` fallback. `opencode2 run --format json --file` with cwd at the workspace root (no `--dir`; that flag is not on `opencode2 run`). `--model` is `provider/modelname#variant` when a model is pinned. `opencode2 run --help` documents no permission-deny overlay equivalent to V1 `OPENCODE_CONFIG_CONTENT`; do not pass `--auto` (that auto-approves). Read-only isolation is cwd + documented flags only.
