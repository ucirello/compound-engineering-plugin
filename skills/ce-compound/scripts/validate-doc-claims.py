...
citations against the repository:
    ...
    1. Cited repo-relative paths (backticked, containing at least one '/')
       ...
       at the working-copy change or the remote default bookmark still count as real paths and
       are classified (deleted in the working copy vs workspace behind upstream). Tokens
       missing everywhere are flagged only when path-shaped; slash-delimited
       identifiers (bookmark names, JJ revisions, provider/model IDs) are skipped.
    2. Cited commit SHAs (7-40 hex chars with at least one digit and one
       ...
       the working-copy change and the remote default bookmark.
...
def is_path_candidate(token: str) -> bool:
    ...
    if token.startswith(("origin/", "upstream/", "refs/")):
        return False  # JJ revisions, not repo paths
...
def main(argv: list[str]) -> int:
    ...
    if in_jj:
        for candidate in ("main@origin", "master@origin"):
            ...
            if code == 0:
                ...
                break
        if upstream:
            code, behind = jj(["log", "-r", f"@..{upstream}", "--no-graph", "-T", 'commit_id ++ "\\n"'], repo_root)
            behind_count = len(behind.splitlines()) if code == 0 and behind else 0
            if behind_count > 0:
                infos.append(
                    f"INFO: working copy is {behind_count} changes behind {upstream} — "
                    "verify merge-state claims against remote truth (gh pr view), "
                    "not this workspace"
                )
        else:
            infos.append(
                "INFO: no remote default bookmark found — "
                "path/SHA classification limited to the working-copy change"
            )
    else:
        infos.append(
    ...
    def working_copy_has_path(path: str) -> bool:
        if not in_jj:
            return False
    ...
    for raw in BACKTICK_RE.findall(body):
        ...
        if os.path.exists(os.path.join(base, check)):
            checked_paths += 1
            continue
        tracked_working_copy = working_copy_has_path(check)
        tracked_upstream = upstream_has_path(check)
        if not (tracked_working_copy or tracked_upstream) and not is_path_shaped(
            check, base
        ):
            continue  # bookmark name / provider ID, not a path citation
        checked_paths += 1
        loc = loc_suffix(raw)
        if tracked_working_copy:
            flags.append(
                f"FLAG path `{token}`{loc} — tracked at @ but missing from "
                "the working copy: deleted in the working copy? Annotate as "
            ...
            flags.append(
                ...
                f"{upstream}: workspace behind upstream? Annotate or verify against upstream."
            )
        else:
            where = (
    ...
    if in_jj:
        for m in SHA_RE.finditer(body):
            ...
            if code != 0:
                ...
                continue
            in_working_copy_code, in_working_copy_out = jj(
                ["log", "-r", f"{sha}::@", "--no-graph", "-T", "commit_id"], repo_root
            )
            in_working_copy = in_working_copy_code == 0 and bool(in_working_copy_out)
            in_up = False
            if upstream is not None:
                in_up_code, in_up_out = jj(
                    ["log", "-r", f"{sha}::{upstream}", "--no-graph", "-T", "commit_id"], repo_root
                )
                in_up = in_up_code == 0 and bool(in_up_out)
            if in_working_copy and (in_up or upstream is None):
                continue
            if in_working_copy and not in_up:
                flags.append(
