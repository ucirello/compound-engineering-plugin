...
| --- | --- | --- |
| local-commit mode (default) | Single writer **per workspace**. | The lease serializes overlapping sweeps in the same workspace (e.g. a cron sweep and a manual one). The file is written in-workspace (and may be committed locally). No cross-machine guarantee. |
| pushed-shared-bookmark | One writer **per repository**. | The state file lives on a shared bookmark multiple workspaces push to. `lease-acquire` must be described, pushed, and confirmed (fetch back and verify our writer won) **before any source-side write**. This makes the lease a repository-wide mutex across machines. |
