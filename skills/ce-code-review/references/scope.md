...
Compute the diff range, file list, and diff. Minimize permission prompts by combining into as few commands as possible. Set `WORKSPACE_ROOT=$(jj workspace root)` once and run every jj command with cwd at that root so file lists are repo-relative. Never read `.jj/` or `.git/`. Workspace membership is `jj workspace list` then `jj workspace root --name`. Pair every `gh` call that talks to the underlying git repo with `GIT_DIR=$(cd "$WORKSPACE_ROOT" && jj git root)`.
...
BASE_ARG="{base_arg}"
BASE=$(cd "$WORKSPACE_ROOT" && jj --no-pager log -r "heads(ancestors(@) & ancestors($BASE_ARG))" -T 'commit_id' --no-graph) || BASE="$BASE_ARG"
```

Then produce the same output as the other paths. Run every jj command with cwd at `$WORKSPACE_ROOT` so file lists are repo-relative. Jujutsu has no index: working-copy files are already in `@`, so there is no separate untracked list.

```
echo "BASE:$BASE" && echo "FILES:" && (cd "$WORKSPACE_ROOT" && jj --no-pager diff --name-only --from $BASE) && echo "DIFF:" && (cd "$WORKSPACE_ROOT" && jj --no-pager diff --git --from $BASE --context 10) && echo "UNTRACKED:"
```

This path works with any revset — a commit id, `main@origin`, a bookmark name. Callers reviewing the current workspace should pass explicit `base:` when auto-detection is unnecessary. **Do not combine `base:` with a PR number or branch target.** If both are present, stop with an error: "Cannot use `base:` with a PR number or branch target — `base:` implies the current checkout is already the correct bookmark. Pass `base:` alone, or pass the target alone and let scope detection resolve the base."

**If a PR number or GitHub URL is provided as an argument:**

Do **not** check out the PR bookmark. Scope comes from GitHub read APIs plus optional local alignment when `@` already matches the PR head.

**Skip-condition pre-check.** Before scope detection, run a PR-state probe:

```
GIT_DIR=$(cd "$(jj workspace root)" && jj git root) gh pr view <number-or-url> --json state,title,body,files
```
...
```
GIT_DIR=$(cd "$(jj workspace root)" && jj git root) gh pr view <number-or-url> --json title,body,baseRefName,headRefName,headRefOid,isCrossRepository,url,files,reviews,comments --jq '{title, body, baseRefName, headRefName, headRefOid, isCrossRepository, url, files: [.files[].path], hasPriorComments: ((.reviews | map(select(.state != "APPROVED" or .body != "")) | length) > 0 or (.comments | length) > 0)}'
```

Set `BASE:` to `pr:<number-or-url>` (logical marker — not a commit id). Set `UNTRACKED:` empty: Jujutsu has no index, and working-copy files are already in `@` (usually empty of extra paths during PR-remote review).
...
1. A local bookmark on `@` equals `headRefName` (`(cd "$WORKSPACE_ROOT" && jj --no-pager log -r @ -T 'local_bookmarks.map(|b| b.name()).join("\n")' --no-graph)`).
2. The PR is **not** cross-repository (`isCrossRepository` is false).
3. The PR head commit is contained in the local workspace: `(cd "$WORKSPACE_ROOT" && jj --no-pager log -r '<headRefOid> & ancestors(@)' -T 'commit_id' --no-graph -n 1)` is non-empty. This confirms the working copy actually carries the PR head (allowing unpushed local fixes layered on top) rather than an unrelated same-named bookmark.
...
- **`local-aligned`:** Resolve `<resolved-base-ref>` from `baseRefName` (fetch if needed; `main` maps to `main@origin`). Compute `BASE` as `(cd "$WORKSPACE_ROOT" && jj --no-pager log -r "heads(ancestors(@) & ancestors(<resolved-base-ref>))" -T 'commit_id' --no-graph)`, then set `FILES:` from `(cd "$WORKSPACE_ROOT" && jj --no-pager diff --name-only --from $BASE)` and `DIFF:` from `(cd "$WORKSPACE_ROOT" && jj --no-pager diff --git --from $BASE --context 10)` (includes all working-copy changes on the PR bookmark; JJ has no index). Do **not** call `gh pr diff` or append remote hunks — when unpushed fixes exist, the local tree is canonical. Note in Coverage: `scope: local-aligned (PR; local tree diff)`.
- **`pr-remote`:** Set `FILES:` from the PR `files` array. Set `DIFF:` from `GIT_DIR=$(cd "$WORKSPACE_ROOT" && jj git root) gh pr diff <number-or-url> --color=never`. If `gh pr diff` fails, stop with an actionable error — do not fall back to checkout.

When **`pr-remote`**, before Stage 4:

1. Best-effort fetch PR head without checkout: `(cd "$WORKSPACE_ROOT" && jj git fetch --branch <headRefName>)` (substitute PR number from metadata when naming coverage).
...
3. Best-effort fetch the PR base without checkout: `(cd "$WORKSPACE_ROOT" && jj git fetch --branch <baseRefName>)`. When it succeeds, set `PR_BASE_REF` to `<baseRefName>@origin` — a **real diffable base** reviewers and validators use for file-level diffs (e.g. `data-migration-reviewer` runs `jj diff --from <PR_BASE_REF> -- db/schema.rb`/`structure.sql` with cwd at the workspace root). The `pr:<number-or-url>` logical marker in `BASE:` stays the scope marker; `PR_BASE_REF` is the diffable base. When the fetch fails, omit `PR_BASE_REF` and note in Coverage — schema-drift and other diff checks fall back to diff hunks only and must **not** assume `main`.
...
Reviewers and Stage 5b validators in **`pr-remote`** mode must **not** Read/Grep workspace paths for files in `FILES:`. Inspect via `(cd "$WORKSPACE_ROOT" && jj --no-pager file show -r <PR_HEAD_REF> <path>)` when `PR_HEAD_REF` is set, otherwise use only the provided diff hunks. **`local-aligned`** uses normal workspace inspection.

**If a branch name is provided as an argument:**

Substitute the provided branch name as `<branch>`. Do **not** check out `<branch>`.

If a local bookmark on `@` equals `<branch>`, use the **standalone (current bookmark)** path below — same tree, explicit bookmark name; do not use remote-only diff.

Otherwise diff the remote/local ref **without checkout**:

1. Try `GIT_DIR=$(cd "$WORKSPACE_ROOT" && jj git root) gh pr view <branch> --json baseRefName,url,headRefName` — if a PR exists, prefer the **PR number/URL path** above (same remote diff rules).
2. Else resolve `<branch>` as `<branch>@origin` or `<branch>` after `(cd "$WORKSPACE_ROOT" && jj git fetch --branch <branch>)` when needed.
3. Resolve default base bookmark (same logic as standalone). Compute `BASE` as `(cd "$WORKSPACE_ROOT" && jj --no-pager log -r "heads(ancestors(<base-ref>) & ancestors(<branch-ref>))" -T 'commit_id' --no-graph)` and `(cd "$WORKSPACE_ROOT" && jj --no-pager diff --git --from $BASE --to <branch-ref> --context 10)`.
4. If `<branch-ref>` cannot be resolved locally, stop: "Cannot diff bookmark `<branch>` without checkout. Edit that bookmark, pass its open PR URL/number, or review the current bookmark with `base:`."

On success for remote bookmark diff, set **branch-remote scope**. The working tree is **not** `<branch>`. Include `<pr-scope-mode>branch-remote</pr-scope-mode>` and `<branch-head-ref><branch-ref></branch-head-ref>` in the Stage 4 review context bundle. Reviewers and Stage 5b validators must **not** Read/Grep workspace paths for files in `FILES:`. Inspect via `(cd "$WORKSPACE_ROOT" && jj --no-pager file show -r <branch-ref> <path>)` or diff hunks only.

Produce:

```
echo "BASE:$BASE" && echo "FILES:" && (cd "$WORKSPACE_ROOT" && jj --no-pager diff --name-only --from $BASE --to <branch-ref>) && echo "DIFF:" && (cd "$WORKSPACE_ROOT" && jj --no-pager diff --git --from $BASE --to <branch-ref> --context 10) && echo "UNTRACKED:"
```
...
If no base can be resolved, **stop**. Do not fall back to `jj diff` against `@-` alone — a standalone review without the base would only show the working-copy change and silently miss all committed work on the bookmark.
...
Using `jj diff --from $BASE` (no `--to`) diffs the merge-base against the working-copy commit `@`, which includes every change in the working copy. Jujutsu has no index.

**Untracked file handling:** Jujutsu has no staging area; working-copy files are already in `@`. `UNTRACKED:` is empty. Continue on the working-copy diff — never stop or prompt.
...
if [ -n "$WORKSPACE_ROOT" ]; then SCRATCH_ROOT="$WORKSPACE_ROOT/.tmp/rocketclaw"; else SCRATCH_ROOT=".tmp/rocketclaw"; fi;
if [ -L "$SCRATCH_ROOT" ]; then echo "unsafe scratch root symlink: $SCRATCH_ROOT" >&2; exit 1; fi;
(umask 077; mkdir -p "$SCRATCH_ROOT") || exit 1;
if [ -L "$SCRATCH_ROOT" ]; then echo "scratch root must not be a symlink: $SCRATCH_ROOT" >&2; exit 1; fi;
