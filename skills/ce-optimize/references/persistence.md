...
5. **Per-experiment result markers for crash recovery.** Each experiment writes a `.tmp/optimize/result.yaml` marker in its workspace immediately after measurement. On resume, scan for these markers to recover experiments that were measured but not yet logged.
...
### File Locations (all under `<workspace-root>/.tmp/optimize/<spec-name>/`)

The scratch space under `.tmp/` must be ignored. It survives a local resume but does not travel with the bookmark, so anything needed durably must be exported to a tracked path.
...
| `<workspace>/.tmp/optimize/result.yaml` | Per-experiment crash-recovery marker | Immediately after measurement, before CP-3 |

### On Resume

When Phase 0.4 detects an existing run:
1. Read the experiment log from disk. It is the ground truth
2. Resolve registered experiment roots with the public workspace APIs, then scan their `.tmp/optimize/result.yaml` markers not yet in the log
