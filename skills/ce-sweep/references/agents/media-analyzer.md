...
1. **Run the bundled analyzer on each media path.** The orchestrator gives you the absolute ce-sweep skill directory in the prompt's `<skill-dir>` block; set it inline in the same command (shell state does not persist between calls):
   ...
   Add `--no-transcribe` unless the transcription key, endpoint, and model are configured (`TRANSCRIPTION_API_KEY`, `TRANSCRIPTION_API_URL`, and `RIFFREC_TRANSCRIBE_MODEL`) -- otherwise the analyzer wastes a round-trip discovering the configuration is absent. **Always add `--no-transcribe` when `Sensitive` is true**, regardless of configuration: transcription uploads the media to a third-party service, which would leak the sensitive content the sweep is contracted to withhold. The analyzer extracts the transcript (when configured and not suppressed), selects high-signal moments, and writes frames plus `analysis.md` / `problem-analysis.md` under the output directory it reports.
...
- Do not invoke RocketClaw skills or delegated workers. Do your analysis directly and return in the format above.
