...
GIT_DIR="$GIT_DIR" GH_HOST=<host> gh api repos/OWNER/REPO/pulls/comments/COMMENT_ID \
...
workspace_root=$(jj workspace root) || { echo "not a jj workspace" >&2; exit 1; }
GIT_DIR=$(cd "$workspace_root" && jj git root)
GIT_DIR="$GIT_DIR" GH_HOST=<host> gh api --paginate repos/OWNER/REPO/pulls/PR_NUMBER/reviews --jq '.[] | select(.state == "PENDING") | .id'
```
