...
If you use mutation testing (edit a production file, run the suite, revert), do it only in an isolated Jujutsu workspace or a scratch copy that is a faithful snapshot of the reviewed tree — verify before mutating: your copy's `@` must equal the reviewed commit (a harness-created workspace may be cut from the primary checkout or default bookmark instead), and `local-aligned` scope needs the uncommitted changes a committed-`@` workspace lacks. On any mismatch, fall back to a scratch copy of the reviewed tree. Never mutate the shared checkout the rest of the reviewer batch is reading.
