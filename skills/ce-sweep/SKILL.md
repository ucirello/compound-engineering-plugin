---
name: ce-sweep
description: "Sweep configured feedback sources (Slack, GitHub Issues; email experimental) for new items: acknowledge at source, analyze recordings, verify fixes merged to `main@origin`, and emit an `lfg`-ready plan. First run sets up sources; supports mode:non-interactive for scheduled runs."
...
Preserve required human and research-source provenance. Do not add RocketClaw branding, generated-by text, visual badges, or creator, model, provider, tool, agent, harness, runtime, workflow, co-author, or similar attribution to plans, state, source actions, JJ descriptions, or summaries.
...
**Resolve the workspace root.** Run `jj workspace root` with the shell tool to resolve `<workspace-root>`, then apply the ordinary-key rule above. Read both files when they exist.
...
- `sweep_state_path` — path to the state file, established at setup; fallback `<root>/feedback-sweep/state.yml`. A workspace-internal path outside `.tmp/` means recorded mode (the state file is recorded each run and must not be ignored); a path under `<workspace-root>/.tmp/` means workspace-local mode (the state file is never recorded — only the plan is).
- `sweep_lease_ttl_minutes` — single-writer lease staleness threshold; default `60`. Passed to `lease-acquire` in 2a.
- `sweep_shared_bookmark` — `true` when the state file lives on a shared bookmark multiple workspaces publish to (see 2a topology); default `false`.
...
**Shared-bookmark topology** (`sweep_shared_bookmark: true`): resolve the shared bookmark and its writable remote from the project's active instructions and current JJ state. Before any source-side write, record only the state file in a JJ change, move that bookmark to the recorded change, and publish only that bookmark with `jj git push`. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. The required sentence's `git log` wording is guidance only: inspect descriptions with `jj log`; the repository's runtime conventions and visible JJ description history win. Preserve the lease-acquisition meaning while using `<description-composed-from-runtime-conventions>`; do not impose fixed syntax or a fixed message. Fetch the resolved remote and confirm the remote bookmark's state names this writer before touching a source. A rejected publication or different remote lease winner means another writer won: rebase the local change onto that remote bookmark with JJ, re-run `lease-acquire`, and back off with `aborted-locked` if the lease is no longer yours.
...
Resolve and create workspace-local media scratch with this shell block, substituting the current run id. If there is no JJ workspace, use `.tmp` beneath the physical current directory:
...
- Download attachments into `$MEDIA_DIR`; raw media is never recorded. A download failure -> set the item `needs_download` and continue.
...
For each `fix_pending` item, resolve its claimed fix ref and verify it merged to `main@origin`. The fix ref originates from untrusted feedback content (a thread claim, an analyzer-extracted reference), so **validate its shape before it reaches any `jj`/`gh` command**: accept only a bare PR number (`#?\d+`) or a commit SHA (`[0-9a-f]{7,40}`), and treat anything else as an unresolved claim (leave the item open). This blocks argument/flag injection into the shell command.
- `gh pr view <validated-ref> --json mergedAt,baseRefName` (merged, base is `main`), or a read-only `jj log` revset intersecting `<validated-sha>` with `ancestors(main@origin)`.
...
- **Record.** Record ONLY `<root>/plans/feedback-sweep-plan.md` plus `<state>` when it is outside `.tmp/`; never include the whole working-copy change, and never record workspace-local state under `.tmp/`. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. The required sentence's `git log` wording is guidance only: inspect descriptions with `jj log`; the repository's runtime conventions and visible JJ description history win. Preserve the feedback-sweep/date meaning while using `<description-composed-from-runtime-conventions>`; do not impose fixed syntax or a fixed message. A JJ recording failure is reported, not fatal. In local-record mode, never publish. In shared-bookmark mode (`sweep_shared_bookmark: true`), fetch the resolved remote, rebase with JJ when needed, move the shared bookmark to the final recorded change, and publish only that bookmark.
