...
1. **Resolve the target directory and extension.**
   ...
   - **Otherwise (outside a JJ workspace, or elsewhere with no `<root>/ideation/`):** write into the run's local scratch area — the `<scratch-dir>` resolved in Phase 1 (`<scratch-root>/ce-ideate/<run-id>/`). Do **not** create a `<root>/ideation/` tree for a subject unrelated to the workspace. Announce the absolute path and note that `.tmp` is scratch storage, so the user should move the file to a durable location if they want to keep it.
...
- **Inside a JJ workspace:** offer to finalize only the ideation document in the current change; do not create or move a bookmark and do not push. Before composing, editing, recommending, or validating its change description, follow local runtime syntax and repository conventions, applying Go guidance only where compatible. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Use `jj commit <workspace-relative-document-path>` so that path stays in the finalized change while other working-copy changes move to the new change, then verify the finalized change with `jj log -r @-` and the remaining work with `jj status`. If the user declines, leave the working-copy change as-is.
- **Outside a JJ workspace:** skip the finalization offer.
...
- the deliverable was written automatically in both modes (Phase 4) — to `<root>/ideation/` when present, else the run's `.tmp/rocketclaw/` scratch area
