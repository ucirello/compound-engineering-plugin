...
Each JJ workspace keeps its own entry under `.tmp/rocketclaw/repo-profile/`. Lookup is JJ metadata only; on a hit, only this one file is read. Outside a JJ repo, scratch storage falls back to `$PWD/.tmp/rocketclaw/repo-profile/`, but this revision-keyed cache returns `NO-CACHE` because no valid key exists.
