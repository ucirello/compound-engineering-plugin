...
**Read config.** Resolve `<repo-root>` at runtime by running `jj workspace root` with the shell tool. Then read `<repo-root>/.rocketclaw/config.local.yaml` with the native file-read tool. If the root cannot be resolved (not a Jujutsu repository) or the file does not exist, fall through to the defaults below.
...
- Draft a clear, searchable title in the repository's established plan-title style
- Determine the plan category from the repository's existing plan metadata and vocabulary; do not impose a fixed category set
- Build the filename following the repository convention: `docs/plans/YYYY-MM-DD-NNN-<descriptive-name>-plan.md`
  ...
  - Example shape: `2026-01-15-001-user-authentication-flow-plan.md`
...
Interactive mode exists because on-demand deepening is a different user posture — the user already has a plan they are invested in and wants to be surgical about what changes. This applies whether the plan was produced by this skill, written by hand, or produced by another tool.
