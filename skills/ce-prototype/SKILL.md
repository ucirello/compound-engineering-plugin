...
Build a kept run under `<jj workspace root>/.context/prototype/<date>-<slug>/`, so the prototype is still there when the implementation that follows reads it alongside the decisions capsule. Jujutsu automatically snapshots new files and uses `.gitignore` files for ignore rules, so confirm that `.context/` is ignored before writing; when it is not, offer to append that one line to the workspace-root `.gitignore`, appending only if they agree and leaving the rest of the file alone. Use `<jj workspace root>/.tmp/rocketclaw/prototype/<date>-<slug>/` when they ask that this run not be kept or the durable path fails its safety checks; without a jj workspace, use `<current working directory>/.tmp/rocketclaw/prototype/<date>-<slug>/`. In a jj workspace, confirm that `.tmp/` is ignored before using the fallback and offer the same narrow `.gitignore` update when needed; if the required local root cannot be made safe without a declined write, stop rather than moving the run to OS-global temp. Calling the prototype throwaway is not a request to discard it — throwaway describes the code, and a kept prototype is still never deleted. Give each question in a multi-question run its own child directory. Never delete a kept prototype — the directory is theirs to prune.
