...
1. **JJ activity** — `jj log` over the window (subjects, commit IDs, dates, authors), and for the substantial commits, `jj diff --stat -r <revision>` to see what they touched. Group obviously-related commits (a bookmark's changes, a fix and its follow-ups) rather than listing them flat. Run every JJ subprocess with cwd set to the absolute `{repo-root}` workspace root. Use only public commands, never repository internals.
2. **Merged and open PRs** — only when a PR interface is reachable (a `gh` CLI that responds, a connector/MCP tool). Run `gh` in the correct workspace with `GIT_DIR` set to the output of `jj git root` there. Use `jj workspace list` and `jj workspace root --name <name>` if workspace membership needs resolving. For current PR/bookmark lookup, check empty `@` and its substantive parent `@-`. This portion is capability-gated: when no interface is reachable, note "PR evidence unavailable" in one line and move on — never treat the missing interface as an error and never guess PR state from bookmark names.
