...
1. **Create CONCEPTS.md (build the concept map)** — seed the repo-wide concept map and save it as a JJ change; skip only the docs/solutions classification phases (Phases 0–4). Read `references/concepts-vocabulary.md` and follow its **Seed goal** and **Scope of a seed** (repo-wide) rules: seed the project's core domain nouns from the declared domain model (schema, core types, primary models, top-level domain docs), each meeting the qualifying bar, the codebase setting the count. Write the preamble (see Phase 4.5), cluster per the organization rules, and run the Discoverability Check so `AGENTS.md`/`CLAUDE.md` surface the new file. Then **enter Phase 5 (Save Changes)** to describe/push/PR the new `CONCEPTS.md` and any instruction-file edit through the same durable-write flow the refresh uses — do not leave the bootstrap unsaved.
...
After all actions are executed and the report is generated, handle saving the changes as JJ changes. Skip this phase if no files were modified (all Keep, or all writes failed).

### Detect JJ context

Before offering options, check:
1. Which bookmark/ref the working copy is based on (main/master vs feature bookmark)
2. Whether the working copy has other changes beyond what compound-refresh modified (`jj st`)
...
|---------|---------------|
| On main/master | Create a bookmark named for what was refreshed (e.g., `docs/refresh-auth-and-ci-learnings`), describe the change, attempt to open a PR. If PR creation fails, report the bookmark name. |
| On a feature bookmark | Describe a separate JJ change on the current bookmark |
...
First, run `jj st` and `jj log -r 'bookmarks() & @::'` to determine the current bookmark/ref context. Then present the correct options based on the result. Include only compound-refresh files regardless of which option the user picks.

**If the current bookmark/ref is main, master, or the repo's default bookmark/ref:**
...
2. Save directly on `{current bookmark/ref name}`
3. Don't save — I'll handle it

**If the current bookmark/ref is a feature bookmark, clean working copy:**

1. Save to `{current bookmark/ref name}` as a separate JJ change (recommended)
2. Create a separate bookmark and save there
3. Don't save

**If the current bookmark/ref is a feature bookmark, dirty working copy (other changes):**

1. Save only the compound-refresh changes to `{current bookmark/ref name}` (other dirty files stay untouched)
2. Don't save
