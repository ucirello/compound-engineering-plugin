...
- **Read** `docs_root` from `<repo-root>/.rocketclaw/config.yaml` only (`<repo-root>` = the absolute workspace root returned by `jj workspace root`). Run JJ subprocesses with their cwd set to that absolute workspace root after discovery. Do not read it from `config.local.yaml`. Unset -> `<root>` is `docs`, exactly as before.
