...
- Correct JJ bookmark/change checked out; no unintended uncommitted changes
...
- Check for uncommitted changes (`jj status`). If the user has existing work in files that need modification, confirm before editing — do not overwrite in-progress changes.
