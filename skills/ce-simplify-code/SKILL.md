...
argument-hint: "[blank to simplify current JJ change, or describe what to simplify]"
---
...
2. **Otherwise, in a JJ repository**, default to the current change diff (`jj diff`) or, when a bookmark/base is configured, the diff between the current bookmark and its base. This covers the common case of "simplify everything I've added on this topic bookmark before opening a PR." If no base is available, fall back to the current JJ change.
