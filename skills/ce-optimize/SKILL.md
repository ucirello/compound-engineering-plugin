...
Run this once at the start of this invocation, before any subagent dispatch, and follow the directives it prints — except where one conflicts with this skill's own rules on asking the user questions, whether those rules are scoped to a non-interactive mode or apply in every mode, in which case this skill's rules win and no blocking question is asked. Run the fence exactly as written, as its own command: do not pipe or filter it (no `head`, `tail`, or `grep`), do not truncate its output, and do not bundle it into a batch with other commands. Its output opens with a `=== skill context` header and ends with `OPTIMIZE_CONTEXT_END`; if you received one of those lines without the other, the output was truncated — rerun the fence verbatim once. That recovery is the only rerun: otherwise do not rerun it within the same invocation; a later invocation of this or any other skill runs its own. If no Node runtime is available the skill proceeds unchanged.
...
This skill reads learnings under `<root>/solutions/`. Resolve `<root>` when you first compose a `<root>/` path (per the block below), never before you need it. A write to `<root>/...` and a read of `<root>/solutions/` both count as composing a `<root>/` path, so either one triggers resolution; only a run that touches no `<root>/` path at all -- a scratch-only or no-repo flow -- skips it; pass the resolved path to any subagent, not the config. Resolve `<workspace-root>` with `jj workspace root`; if that fails because no JJ workspace is available, use `pwd -P`. Keep all scratch under `<workspace-root>/.tmp/`; this makes the no-workspace fallback the current directory's local `.tmp/`, never an OS-temp or external path.

<!-- docs-root:start -->
**Resolve the artifact root `<root>` before composing any artifact path.**

- **Read** `docs_root` from `<workspace-root>/.rocketclaw/config.yaml` only. Do not read it from `config.local.yaml`. Unset -> `<root>` is `docs`, exactly as before.
...
The files under `<workspace-root>/.tmp/rocketclaw/ce-optimize/<spec-name>/` are local scratch state. They survive local resumes in that workspace but are not preserved by JJ changes, bookmarks, or remote publication unless the user exports them separately.
...
### File Locations (all under `<workspace-root>/.tmp/rocketclaw/ce-optimize/<spec-name>/`)
...
| `<experiment-workspace>/result.yaml` | Per-experiment crash-recovery marker | Immediately after measurement, before CP-3 |
...
6. Write the spec to `<workspace-root>/.tmp/rocketclaw/ce-optimize/<spec-name>/spec.yaml`
...
Check whether the `optimize/<spec-name>` bookmark already targets a change:

```bash
jj log --revision 'bookmarks(exact:"optimize/<spec-name>")' --no-graph --template 'change_id ++ "\n"'
```

**If the bookmark exists**, check for an existing experiment log at `<workspace-root>/.tmp/rocketclaw/ce-optimize/<spec-name>/experiment-log.yaml`.
...
- **Fresh start**: create `optimize-archive/<spec-name>/archived-<timestamp>` at the old bookmark target, delete the old optimization bookmark, clear the experiment log, and start from the caller-approved base revision.

### 0.5 Create Optimization Change, Bookmark, and Scratch Space

```bash
jj new <approved-base-revision>
jj bookmark set "optimize/<spec-name>" --revision @
```

Create scratch directory:
```bash
WORKSPACE_ROOT="$(jj workspace root 2>/dev/null || pwd -P)";
mkdir -p "$WORKSPACE_ROOT/.tmp/rocketclaw/ce-optimize/<spec-name>/"
```
...
### 1.1 Clean-Workspace Gate

Verify the current JJ change has no unrecorded in-scope edits that should not become the optimization base:

```bash
jj status
```

Inspect the output against the scope paths. If any in-scope files contain edits:
- Report which files are dirty
- Ask the user to preserve them in an appropriate JJ change, or move the optimization to a separate JJ workspace
- Do NOT continue until the current workspace has no unintended in-scope edits
...
1. Create the experiment log file at `<workspace-root>/.tmp/rocketclaw/ce-optimize/<spec-name>/experiment-log.yaml`
...
- **Clean-workspace status**: confirmed clean
...
**Workspace backend:**
1. Create an experiment workspace and JJ change from the current optimization bookmark:
   ```bash
   SKILL_DIR="<absolute path of the directory containing this SKILL.md>";
 EXPERIMENT_WORKSPACE=$(bash "$SKILL_DIR/scripts/experiment-worktree.sh" create "<spec_name>" <exp_index> "optimize/<spec_name>" <shared_files...>)  # creates bookmark optimize-exp/<spec_name>/exp-<NNN>
   ```
...
3. Resolve `<workspace-root>` with `jj workspace root` and `pwd -P` fallback, create `<workspace-root>/.tmp/rocketclaw/ce-optimize/prompts/`, and write the filled prompt there
4. Dispatch via Codex:
   ```bash
    WORKSPACE_ROOT="$(jj workspace root 2>/dev/null || pwd -P)";
    PROMPT_FILE="$WORKSPACE_ROOT/.tmp/rocketclaw/ce-optimize/prompts/<experiment-id>.txt";
...
1. **Run measurement** in the experiment's workspace:
   ```bash
   SKILL_DIR="<absolute path of the directory containing this SKILL.md>";
    bash "$SKILL_DIR/scripts/measure.sh" "<measurement.command>" <timeout_seconds> "<experiment-workspace>/<measurement.working_directory or .>" <env_vars...>
   ```
...
7. **IMMEDIATELY append to experiment log on disk (CP-3)** — do not defer this to batch evaluation. Write the experiment entry (iteration, hypothesis, outcome, metrics, learnings) to `<workspace-root>/.tmp/rocketclaw/ce-optimize/<spec-name>/experiment-log.yaml` right now. Use the transitional outcome `measured` once the experiment has valid metrics but has not yet been compared to the current best. Update the outcome to `kept`, `rejected`, or another terminal state in the evaluation step, but the raw metrics are on disk and safe from context compaction.
...
3. **If best improves on current best: KEEP**
    - Confirm the experiment change contains only mutable-scope edits; if no eligible diff remains, treat the experiment as non-improving and abandon it
    - Describe the winning JJ change before advancing the optimization bookmark. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. The mandated sentence's `git log` wording is not an operational instruction; the project's active instructions and change-description syntax inferred at runtime from `jj log` always win. Preserve the winning experiment's semantic outcome while adapting syntax dynamically. Apply compatible Go guidance only for quality, clarity, and structure. Do not impose a fixed prefix, type, scope, subject, body, layout, template, or example, and do not add product branding, generated-by text, or creator, model, provider, tool, agent, runtime, workflow, or co-author attribution.
    - Set `optimize/<spec-name>` to the winning change with `jj bookmark set "optimize/<spec-name>" --revision <winning-change-id>`
    - Remove the winner's experiment workspace and experiment bookmark without abandoning the accepted change; the accepted JJ change targeted by the optimization bookmark is the durable artifact:
      ```bash
      SKILL_DIR="<absolute path of the directory containing this SKILL.md>";
      bash "$SKILL_DIR/scripts/experiment-worktree.sh" forget "<spec-name>" <exp-index>
      ```
...
4. **Check file-disjoint runners-up** (up to `max_runner_up_integrations_per_batch`):
   ...
   - **File-level disjointness**: two experiments are disjoint if they modified completely different files. Same file = overlapping, even if different lines.
    - If disjoint: rebase the runner-up change onto the new optimization bookmark with `jj rebase --revision <runner-up-change-id> --destination "optimize/<spec-name>"`, then re-run full measurement
    - If combined measurement is strictly better: describe the rebased runner-up change before setting the optimization bookmark to it. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. The mandated sentence's `git log` wording is not an operational instruction; the project's active instructions and change-description syntax inferred at runtime from `jj log` always win. Preserve the runner-up integration's semantic outcome while adapting syntax dynamically. Apply compatible Go guidance only for quality, clarity, and structure. Do not impose a fixed prefix, type, scope, subject, body, layout, template, or example, and do not add product branding, generated-by text, or creator, model, provider, tool, agent, runtime, workflow, or co-author attribution. Record outcome `runner_up_integrated`, then forget that runner-up's experiment workspace and delete only its experiment bookmark.
    - Otherwise: abandon the rebased runner-up change, log it as "promising alone but neutral/harmful in combination" (outcome: `runner_up_rejected`), then clean up its experiment workspace and bookmark
...
6. **Reject all others**: abandon their JJ changes, clean up their experiment workspaces and bookmarks, and log them as `rejected`
...
2. **Finalize outcomes** — update experiment entries from step 3.4 evaluation (mark `kept`, `rejected`, `runner_up_integrated`, etc.). Write these outcome updates to disk immediately.

3. **Update the `best` section** in the experiment log if a new best was found. Write to disk.

4. **Write strategy digest** to `<workspace-root>/.tmp/rocketclaw/ce-optimize/<spec-name>/strategy-digest.md`:
...
- Abandon the experiment change and clean up its workspace and bookmark
...
Total experiments: <count>
  Kept: <count> (including <runner_up_integrated_count> runner-up integrations)
  Rejected: <count>
  Degenerate: <count>
  Errors: <count>
  Deferred: <count>
...
The optimization bookmark (`optimize/<spec-name>`) is preserved at the accepted JJ change stack.
The experiment log and strategy digest remain under `<workspace-root>/.tmp/rocketclaw/...` for resume and audit in this workspace only; they do not travel with JJ changes or bookmarks.

Present post-completion options via the platform question tool:

1. **Run `ce-code-review`** on the cumulative diff (baseline to final). Load the `ce-code-review` skill in the optimization workspace (interactive or `mode:agent`). To include eligible fixes before the next option, apply the mechanical-apply bar below.

   **Mechanical-apply bar:** apply any finding with a concrete `suggested_fix` that is a clear, reversible improvement; push back (keep, don't apply) when the reviewer is wrong, noting why. Defer anything whose right fix needs a design or product decision (architecture direction, contract shape, behavior change needing sign-off) and any finding with no concrete fix to act on — surface what was deferred. Confirm evidence still matches at `file:line` before editing. After applying, run tests (at least targeted tests for what changed; broader suite for multi-file edits). Do not describe, publish, or push from this step — leave the edits in the current JJ change for the Create PR option.
2. **Run `ce-compound`** to document the winning strategy as an institutional learning.
3. **Create PR**: describe the final JJ change stack, set or move the optimization bookmark, use `jj git push --bookmark "optimize/<spec-name>"` for remote interop, then use `gh` to create the PR against the default bookmark's corresponding remote branch. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. The mandated sentence's `git log` wording is not an operational instruction; the project's active instructions and change-description syntax inferred at runtime from `jj log` always win. Preserve the final accepted optimization stack's semantic outcome while adapting syntax dynamically. Apply compatible Go guidance only for quality, clarity, and structure. Do not impose a fixed prefix, type, scope, subject, body, layout, template, or example, and do not add product branding, generated-by text, or creator, model, provider, tool, agent, runtime, workflow, or co-author attribution.
4. **Continue** with more experiments: re-enter Phase 3 with the current state. State re-read first.
5. **Done** -- leave the optimization bookmark and change stack for manual review.

### 4.4 Cleanup

Clean up scratch space:
```bash
# Keep the experiment log for local resume/audit in this workspace
# Remove temporary batch artifacts
WORKSPACE_ROOT="$(jj workspace root 2>/dev/null || pwd -P)";
rm -f "$WORKSPACE_ROOT/.tmp/rocketclaw/ce-optimize/<spec-name>/strategy-digest.md"
```
...
Do NOT forget experiment workspaces that are still being referenced.
