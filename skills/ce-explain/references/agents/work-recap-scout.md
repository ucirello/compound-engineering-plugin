...
1. **Jujutsu activity** — use `jj log` with date revsets for the window to gather descriptions, change/commit IDs, and dates, then inspect substantial changes with `jj show --stat` or `jj diff --stat -r <revset>`. Group obviously related changes (a bookmark's changes, a fix and its follow-ups) rather than listing them flat.
...
- What changed, in one line, with the Jujutsu change/commit ID(s) or PR number and date
