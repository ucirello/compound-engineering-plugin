...
argument-hint: "[PR number, bookmark/change, 'current', or --port PORT]"
---

# Browser Test Skill

Run end-to-end browser tests on pages affected by a PR, bookmark, or change changes using the `agent-browser` CLI.
...
**If 'current' or empty:**
```bash
jj diff --from <trunk-bookmark> --to @ --name-only
```

**If bookmark name provided:**
```bash
jj diff --from <trunk-bookmark> --to <bookmark-or-rev> --name-only
```
...
# Test specific bookmark or change or change
