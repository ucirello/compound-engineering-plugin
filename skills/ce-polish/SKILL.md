...
4. **Close locally.** When the user says they are done, describe the current Jujutsu change and stop. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. The project's active instructions and the description syntax observed at runtime in `jj log` win. Apply compatible Go guidance only to quality, clarity, and structure; it does not prescribe imperative mood, casing, punctuation, line wrapping, subject/body shape, or any fixed syntax. Validate the description against those standards, then apply it with `jj describe -m "<description-composed-from-runtime-conventions>"`. Report the current change, the still-running server URL, and any residual blocker.
