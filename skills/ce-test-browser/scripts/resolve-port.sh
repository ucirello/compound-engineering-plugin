...
#   --check        exit 0 when the resolved port has a listener, 1 otherwise
...
port="$explicit"
if [ -z "$port" ]; then
  port=$(grep -Eo -- '--port[= ]+[0-9]{2,5}' "$workspace_root/package.json" 2>/dev/null | grep -Eo '[0-9]{2,5}' | head -1)
fi
if [ -z "$port" ]; then
  # take the first digit run after PORT=, so quotes and a trailing "# comment" drop out
  port=$(grep -h '^PORT=' "$workspace_root/.env" "$workspace_root/.env.local" "$workspace_root/.env.development" 2>/dev/null | tail -1 | grep -Eo '[0-9]+' | head -1)
fi
port="${port:-3000}"
