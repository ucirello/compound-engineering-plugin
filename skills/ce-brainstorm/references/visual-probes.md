...
Use the workspace-root `.tmp` by default (local `.tmp` outside JJ) because visual probes are disposable scratch:
...
Keep disposable probes under the workspace-root `.tmp` (local `.tmp` outside JJ). If the user asks to preserve a sketch, promote the selected artifact to the project's durable documentation location; the Phase 3 requirements-only unified plan still lives under `<root>/plans/`.
