...
**JJ workspace.** Run every JJ subprocess with its cwd set to the absolute workspace root, discovered through `jj workspace root`; use `jj workspace list` and `jj workspace root --name <name>` for workspace membership. Do not inspect VCS internals. For `gh`, set `GIT_DIR` from `jj git root` in that same workspace and select the PR by explicit bookmark or URL. JJ snapshots working-copy changes automatically; unfinished work stays in `@`, outside the finalized changes selected for publication. Resolve the shipping bookmark from the actual completed change, checking `@-` when `@` is empty. Runtime scratch belongs under the workspace root's `.tmp/` (local `.tmp/` outside JJ).

`ce-work` owns one scoped exception: its ignored-file inventory helper may use the read-only Git inventory command with workspace-root cwd and explicit Git environment. Preserve its before/after ignored metadata and reporting; this does not authorize Git execution by LFG.
...
3. **Read `references/review-followup.md` now**; it governs steps 3 through 7. Invoke the `ce-simplify-code` skill on the change diff; skip only the invocation for a docs-only or roughly sub-10-line change.
...
**Shipping precondition (every push from step 5 on).** Run `jj git remote list` once. No remote means local-only: make every commit the steps call for, but skip every push, PR create/edit, and CI-watch action, including step 10 in full. That is terminal, not an error.

5. **Apply and persist review fixes** as that file defines. Do not proceed to the residual handoff, run browser tests, or output DONE while eligible review fixes remain only in the unfinished working-copy change.
