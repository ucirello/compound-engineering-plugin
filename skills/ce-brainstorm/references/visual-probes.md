...
Store disposable visual probes inside the current JJ workspace, falling back to the current directory when no JJ workspace exists:

```text
<workspace-root-or-current-directory>/.tmp/rocketclaw/ce-brainstorm-visual/<run-id>/
  screens/
    001-<decision>.html
  state/
    display-info.json
...
When the user explicitly wants to inspect, preserve, or curate the sketches after the session, keep them in the same `.tmp/rocketclaw/ce-brainstorm-visual/<run-id>/` namespace and report that path. The probe is otherwise disposable scratch; the durable artifact is the Phase 3 requirements-only unified plan under `docs/plans/`.
