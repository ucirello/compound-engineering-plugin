...
Files live under `<scratch-dir>/web-research-cache.json`, where `<scratch-dir>` is `<workspace-root-or-current-working-directory>/.tmp/ideate/<run-id>`, resolved once in SKILL.md Phase 1.
...
```bash
SCRATCH_ROOT="<workspace-root-or-current-working-directory>/.tmp/ideate"
