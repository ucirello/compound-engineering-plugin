...
- **Socket files:** `overmind` writes a socket to `.overmind.sock` by default. Polish's kill-by-port logic reclaims the port but does not clean up the socket. If overmind is already running and polish restarts it, the new process may fail with "connection refused" until the stale socket is removed. If isolation is needed, create `$(jj workspace root)/.tmp`, falling back to `$(pwd -P)/.tmp` when no JJ workspace root is available, and point `OVERMIND_SOCKET` at a collision-resistant per-run path there. Do not use OS-global temporary storage.
