...
Supported sources: RocketClaw zip, standalone video, standalone audio, and
meeting notes text/markdown. The script extracts transcript, high-signal
video frames when available, and RocketClaw markdown artifacts.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("source_path", type=Path, help="Path to a RocketClaw zip, video, audio, or meeting notes file")
    parser.add_argument(
        "--output-dir",
        type=Path,
        help="Directory for extracted evidence/kickoff artifacts below <workspace-root>/.tmp. Defaults to .tmp/rocketclaw-feedback/<source-stem>.",
    )
    parser.add_argument("--topic", help="Kebab-case topic for requirements-kickoff frontmatter")
    parser.add_argument(
        "--model",
        default=os.environ.get("ROCKETCLAW_TRANSCRIBE_MODEL", "gpt-4o-mini-transcribe"),
...
def slugify(value: str) -> str:
    ...
    return re.sub(r"-{2,}", "-", slug) or "rocketclaw-feedback"
...
def resolve_output_dir(requested: Path | None, source_path: Path, workspace_root: Path) -> Path:
    scratch_root = (workspace_root / ".tmp").resolve()
    output_dir = (
        requested.expanduser().resolve()
        if requested
        else scratch_root / "rocketclaw-feedback" / slugify(source_path.stem)
    )
    if not output_dir.is_relative_to(scratch_root) or output_dir == scratch_root:
        raise ValueError(f"Output directory must be below {scratch_root}")
    return output_dir


def classify_source(source_path: Path) -> str:
    if zipfile.is_zipfile(source_path):
        return "rocketclaw_zip"
...
def write_source_materials(
    ...
    lines = [
        ...
        "- JJ tracking policy: raw media, audio chunks, zip contents, session dumps, and extracted screenshots remain untracked by default; track generated Markdown/JSON/manifests when useful for brainstorm/planning traceability.",
    ...
    if chunk_files:
        ...
        lines.append(f"  - retained locally in `{link(raw_dir / 'transcription_chunks')}`; do not track in a JJ change by default.")
    ...
    lines.append("Raw files are intentionally local-only by default. Do not track them in a JJ change unless the user explicitly asks and privacy/security is acceptable.")
    lines.append("")
...
def main() -> int:
    ...
    try:
        output_dir = resolve_output_dir(args.output_dir, source_path, workspace_root)
