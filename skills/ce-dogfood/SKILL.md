...
- A numeric target stays a PR identity through isolation and workspace switch — never collapse it to its head ref, whose name may itself be `main`.
- Never switch the primary workspace out from under the user. This skill decides only whether to offer isolation — no for a blank or current-bookmark target (you are already on it), yes for a PR or another named ref — and `ce-worktree` owns the mechanics and the verdict. On a declined offer, switch this workspace to the target, confirming first if working-copy changes would be disturbed.
- Screenshots and other transient artifacts go to workspace `.tmp` (`SCRATCH_ROOT="$(jj workspace root 2>/dev/null || printf '%s' ".")/.tmp"; mkdir -p "$SCRATCH_ROOT"; mktemp -d "$SCRATCH_ROOT/ce-dogfood-XXXXXX"`), never the repo root; copy one in only to embed it in the report.
