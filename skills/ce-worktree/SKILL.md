...
Order of operations: **detect existing isolation -> prefer a native workspace/worktree tool -> fall back to plain JJ.** Never create a workspace the harness cannot see.
...
If `jj workspace list` shows only the current workspace, treat this as the primary checkout and continue to Step 1.

If multiple workspaces exist and the current `jj workspace root` is not the primary checkout path (or is already under the harness/workspace directory), you are **already in an isolated workspace**. Report the workspace root and current bookmark/revision:
...
## Step 1: Prefer the harness's native workspace/worktree tool
