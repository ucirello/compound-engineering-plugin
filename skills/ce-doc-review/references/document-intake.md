...
**Missing-document gate — verify before any dispatch.** Persona reviewers read from the filesystem and several run without Bash, so they cannot read another Jujutsu revision directly. Confirm every resolved path is readable in a workspace before Phase 2. Location does not matter: an absolute path outside the current workspace or a document in another workspace reviews fine. If any path is unreadable, dispatch **no** personas:

- **Interactive:** stop and name the missing path(s): "Document(s) not found on disk: <paths>. Open the revision in a Jujutsu workspace or provide corrected readable paths before retrying the review."
- **Non-interactive:** output "Review failed: document(s) not found on disk: <paths>. Expected input: paths to readable files on disk; open the revision in a Jujutsu workspace or provide corrected paths." and return without dispatching reviewers.
...
First check the unified artifact contract (`artifact_contract: unified-plan/v1`). When reading persisted plans, normalize the historical contract alias formed by prefixing `ce-` to `unified-plan/v1`, and historical source aliases formed by prefixing `ce-` to `brainstorm` or `plan-bootstrap`, to their neutral values. Never write or pass the historical aliases downstream.
