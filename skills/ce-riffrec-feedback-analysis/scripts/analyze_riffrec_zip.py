...
def write_source_materials(
    ...
    lines = [
        ...
        "- Jujutsu change policy: raw media, audio chunks, zip contents, session dumps, and extracted screenshots are local-only by default; generated Markdown/JSON/manifests may remain in a shareable change when useful for brainstorm/planning traceability.",
    ...
    if chunk_files:
        ...
        lines.append(f"  - retained locally in `{link(raw_dir / 'transcription_chunks')}`; not safe to include in a shared change by default.")

    lines.extend(["", "## Local-Only Frames", ""])
    lines.append("Extracted screenshots are retained locally for agent inspection and should not be included in a shared change by default.")
    lines.append("")
    if moments:
    ...
    lines.append("Raw files are intentionally local-only by default. Do not include them in a shared change unless the user explicitly asks and privacy/security is acceptable.")
    lines.append("")
...
def main() -> int:
    ...
    write_requirements_kickoff(kickoff_md, topic, session, findings, moments, workspace)
    ...
    print(f"Brainstorm handoff: ce-brainstorm {display_path(kickoff_md, workspace)}")
