...
- **Source / composition signal.** A visible footer at the bottom of the doc
  preserves the composition timestamp and input/source identifier: the user
  prompt context, an upstream artifact path when one exists, or `direct input`
  when there is no external source. Example shape:
  `<footer class="composition-signal">Composed 2026-05-17T14:23Z from <code>docs/plans/...-plan.md</code></footer>`.
  Do not name the executing agent, model, product, or composing skill. Under
  exclusive output mode this footer is the artifact's provenance; omitting it
  leaves readers unable to assess freshness or trace the input.
...
want workspace-specific HTML defaults can add DESIGN.md to that workspace.
...
- **Source / composition signal** is present as a visible footer at the bottom
  of the doc (composition timestamp + input/source identifier, with no creator
  identity).
