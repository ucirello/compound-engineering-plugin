...
The kernel's write gate remains active: every implementation change is path-limited to that unit's owned files; a bare `jj commit` without filesets can absorb the user's pre-existing working-copy edits and is forbidden.
