...
1. **Open in browser** *(html)* / **Publish a shareable document** *(md)* — open the HTML deliverable, or publish the markdown through `ce-proof` for a shareable link (per §5.1). On publishing failure the auto-written local file stays intact.
2. **Brainstorm one idea with `ce-brainstorm`** — go deeper on one chosen idea (asks which). In universal mode this is **not** the first step of an implementation chain — there is no `ce-plan` → `ce-work` after; `ce-brainstorm` develops the idea further (a name into a brand brief, a plot into an outline, a decision into a weighed framework) and ends there. Seed it with the idea's substance and neutral source context (per §5.2), not the whole file.
