...
- Analyze only. You are read-only except for the single write to {scratch_artifact_path}.
  Running the bundled analyzer and read-oriented jj / gh are permitted
  (pair every `gh` with `GIT_DIR=$(jj git root)` in the same shell); do not edit
  project files, change bookmarks, describe/commit, push, or open PRs.
