...
This finalizes any remaining run-owned changes, pushes the bookmark, and opens a pull request, non-interactively because of the `mode:pipeline` token. If it prints a `New concepts:` trailer after the PR URL, record the concept name(s) for step 11. `ce-commit-push-pr` owns PR detection: when an open PR already exists for the shipping bookmark, its result must identify that PR and include the finalized, pushed run-owned changes without creating a duplicate. The shipping bookmark must identify the completed change, including `@-` when `@` is empty, rather than Git HEAD. Pipeline mode leaves an existing PR body alone by default, so the description context above would be lost there; after the push, invoke `ce-commit-push-pr` again in its description-update mode on that PR with the same context, so the record reaches the reviewer.

**Per the shipping precondition, when no remote is configured, do NOT invoke `ce-commit-push-pr` or a project-defined shipping process.** Instead finalize only this run's changes (the work source's files, the review fixes, and any captured learning), and skip the push and PR creation entirely. Before composing the message, read https://go.dev/wiki/CommitMessage. Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards. Here `git log` means history inspected using `jj log`; never execute Git. Runtime project instructions and history syntax override the Go guidance. Use `jj commit -m "<message>" <run-owned-paths>` only after verifying those paths contain solely run-owned changes; separate mixed hunks first or stop with their ownership unresolved. Pre-existing work, including the defect route's `pre_fix_scope`, stays in the working-copy change outside the finalized result and is named in the report.
