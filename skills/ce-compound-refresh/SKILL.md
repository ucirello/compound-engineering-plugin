...
Skip if nothing changed. Otherwise `jj commit` **only** the filesets this refresh modified. **Read `references/commit.md`** for the per-mode bookmark decision, change-description standards, and the jj-failure fallback.
