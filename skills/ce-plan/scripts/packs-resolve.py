...
validates each entry, resolves path and remote sources, enumerates the packs each
...
availability (e.g. an unreachable remote source skipped per the warn-and-continue
contract).
...
Entry shape (documented subset -- anything else under `packs:` is a loud error):
    ...
    packs:
      ...
      - source: https://github.com/o/r         # remote URL: ref required
        ...
        path: packs                            # optional subfolder (remote only)
...
Remote sources cache under `<workspace>/.tmp/rocketclaw/ce-packs/<sha256(url\\nref)>`
with an atomic temp-clone-then-rename, so a keyed path's existence proves a
complete clone. All `jj git clone` subprocesses run with cwd at the clone dest
(or the cache parent), non-interactively (GIT_TERMINAL_PROMPT=0 for the
underlying transport, ssh BatchMode, bounded timeout): missing credentials
degrade to a warning, never a hang. A missing `jj` binary degrades remote
sources only (each warns and is skipped); path sources still resolve, with the
repository located via `jj workspace root`. Environment overrides:
...
# --- scratch root (workspace `.tmp`/rocketclaw, or local `.tmp` off-workspace) ---
...
def cache_base() -> str | None:
    ...
    if configured:
        ...
        return root
    ws = workspace_root()
    parent = os.path.join(ws, ".tmp") if ws else os.path.abspath(".tmp")
    root = os.path.join(parent, "rocketclaw", "ce-packs")
    try:
...
# --- remote clone via jj git clone -------------------------------------------

@functools.lru_cache(maxsize=None)
def _jj_env() -> dict:
    env = dict(os.environ)
    # Underlying git transport for `jj git clone`; not a VCS identity check.
...
def _run_jj(args: list, cwd: str | None = None):
    ...
    return subprocess.run(
        ["jj", "--no-pager", *args], cwd=root, env=_jj_env(), timeout=GIT_TIMEOUT,
...
def resolve_git_source(url: str, ref: str, warnings: list, label: str) -> str | None:
    ...
    if os.path.lexists(dest):
        ...
        if os.path.lexists(dest):
            ...
            return None
    # jj git clone creates DESTINATION; do not pre-mkdir it.
    tmp = os.path.join(base, f"{key}.part-{os.getpid()}-{os.urandom(3).hex()}")
    ...
    try:
        try:
            proc = _run_jj(["git", "clone", "--depth", "1", "--branch", ref, url, tmp], cwd=base)
            if proc.returncode != 0:
                _wipe_tmp()
                proc = _run_jj(["git", "clone", "--depth", "1", "--tag", ref, url, tmp], cwd=base)
            if proc.returncode != 0:
                _wipe_tmp()
                # Default clone, then try the ref as a revision already fetched.
                proc = _run_jj(["git", "clone", "--depth", "1", url, tmp], cwd=base)
                if proc.returncode == 0:
                    edited = _run_jj(["new", ref], cwd=tmp)
                    if edited.returncode != 0:
                        warnings.append(
                            f"{label}: cannot select `{ref}` from {url}; jj git clone "
                            "has --branch/--tag but no public SHA-fetch API; source skipped"
                        )
                        return None
                else:
                    warnings.append(f"{label}: cannot clone `{ref}` from {url}; source skipped")
                    return None
...
def enumerate_packs(source_root: str, boundary: str, escaped: list, self_name: str | None = None) -> dict:
    ...
    within (the remote checkout, the repository, or the source root itself); a
...
def resolve_entry(entry: dict, repo_root: str, roots: list, warnings: list, errors: list) -> None:
    ...
    if _is_git_url(source):
        if not isinstance(ref, str) or not ref:
            errors.append(f"{label}: remote source `{source}` requires `ref:` (tag, sha, or bookmark)")
            return
        if ref.startswith("-") or source.startswith("-"):
            errors.append(f"{label}: remote source/ref may not begin with `-`")
            return
        ...
        git_meta = None
        if ref is not None:
            errors.append(f"{label}: `ref:` is only valid on remote sources; path sources are read live")
            return
        if sub_path is not None:
            errors.append(f"{label}: `path:` is only valid on remote sources; point `source:` at the directory instead")
            return
    ...
    if git_meta:
        # Display name for a single-pack remote source: the path: subfolder's
...
def _repo_root() -> str | None:
    """The enclosing workspace root via public `jj workspace root`. None when
    the working directory is not inside a jj workspace. Never walks `.jj`/`.git`."""
