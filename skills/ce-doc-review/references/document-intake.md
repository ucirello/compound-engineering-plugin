...
**Missing-document check — verify before any dispatch.** Persona reviewers read from the filesystem and several run without Bash, so they cannot read Jujutsu revisions or bookmarks. A path that exists only on a change that is not in the working copy wastes the entire persona team discovering they cannot proceed (issue #925). Confirm every resolved path is readable on disk before Phase 2 (dispatch). Location does not matter — an absolute path outside the checkout or a doc in another checkout reviews fine. If any path is unreadable, dispatch **no** personas:

- **Interactive:** stop and name the missing path(s): "Document(s) not found on disk: <paths>. Create a working copy on the bookmark containing them (`jj new <bookmark>`), use a JJ workspace, or provide corrected readable paths before retrying the review."
- **Non-interactive:** output "Review failed: document(s) not found on disk: <paths>. Expected input: paths to readable files on disk; create a working copy on the bookmark containing them or provide corrected paths." and return without dispatching reviewers.
