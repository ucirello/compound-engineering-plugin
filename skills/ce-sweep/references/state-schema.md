...
| --- | --- | --- |
| local-commit mode (default) | Single writer **per checkout**. | The lease serializes overlapping sweeps in the same working copy (e.g. a cron sweep and a manual one). The file is written in-tree (and may be included in a local change). No cross-machine guarantee. |
| pushed-shared-branch | One writer **per repo**. | The state file lives on a shared bookmark multiple checkouts push to. `lease-acquire` must be described, `jj git push`ed, and confirmed (`jj git fetch` back and verify our writer won) **before any source-side write**. This makes the lease a repo-wide mutex across machines. |
...
and never included in a change (the skill's commit step includes only the state
file and the plan, never the whole working copy).
