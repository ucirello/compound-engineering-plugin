...
3. **Config** — otherwise the per-skill key: `plan_model` for ce-plan, `brainstorm_model` for ce-brainstorm. Read it the **same way this skill's Phase 0.0 resolves `plan_output` / `brainstorm_output`**: reuse the repo root already resolved, else run `jj workspace root`, then read `<repo-root>/.rocketclaw/config.local.yaml` with the native file-read tool, reusing the Phase 0.0 read if still in hand. Ignore commented (`#`-prefixed) lines. A model alias → elevate to it; missing / commented / invalid / no file → off.
...
Hand over the working context as **file paths the subagent reads itself**, never a re-narrated prose brief. Create **one private per-run handoff directory** under `<workspace-root>/.tmp/rocketclaw/<skill-name>/`, falling back to `<current-directory>/.tmp/rocketclaw/<skill-name>/` when `jj workspace root` fails, and write the prompt-file and every evidence file into *that* directory. Create the parent safely with `mkdir -p` and claim the per-run child with one atomic `mkdir`. On the Claude CLI route the worker grants the elevated model read access to only that one directory (via `--add-dir` on the prompt-file's parent), so the handoff files stay readable while the rest of the workspace-local scratch tree — other same-user scratch and credentials — is not exposed:
