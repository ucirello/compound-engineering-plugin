...
Store disposable visual probes inside the current JJ workspace, falling back to `$PWD/.tmp` when no JJ workspace exists:
...
When the user explicitly wants to inspect, preserve, or curate the sketches after the session, keep them in the same `.tmp/rocketclaw/brainstorm-visual/<run-id>/` namespace and report that path. The probe is otherwise disposable scratch; the durable artifact is the Phase 3 requirements-only unified plan under `docs/plans/`.
