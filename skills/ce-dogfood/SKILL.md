...
- A JJ workspace for the repo under test (`jj root` must succeed). Use JJ for all repository reads and mutations.
...
2. **Drive it** with agent-browser — navigate, snapshot for interactive refs, click, fill, submit, follow the journey to its real end state:
   ...
   agent-browser screenshot "$(jj workspace root)/.tmp/rocketclaw/ce-dogfood/screenshots/<scenario>.png"
   agent-browser errors      # check console/page errors
   ```

   Write transient screenshots to `$(jj workspace root)/.tmp/rocketclaw/ce-dogfood/screenshots/`, with the current directory's `.tmp/rocketclaw/ce-dogfood/screenshots/` as the fallback outside JJ. Only copy a screenshot into the report's location if you intend to embed it in the final report.
