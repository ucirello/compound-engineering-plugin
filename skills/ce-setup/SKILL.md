...
4. Check whether `.rocketclaw/config.local.yaml` exists and, if it does, whether it is absent from `jj file list '.rocketclaw/config.local.yaml'` because `.gitignore` excludes it from the working-copy snapshot.
...
Copy `references/config-template.yaml` to `<workspace-root>/.rocketclaw/config.local.example.yaml`, creating the directory if needed. This file belongs in the Jujutsu working-copy snapshot and should always reflect the latest available settings.
