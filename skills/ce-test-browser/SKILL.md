...
2. **Determine test scope** from the argument: a PR number → `gh pr view [number] --json files -q '.files[].path'`; `current` or empty → `jj diff --from main --to @ --name-only`; a bookmark name → `jj diff --from main --to [bookmark] --name-only`.
