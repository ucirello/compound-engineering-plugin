...
1. **JJ activity** — `jj log` over the window (subjects, commit ids, dates, authors), and for the substantial commits, a stat-level view of what they touched. Group obviously-related commits (a feature bookmark's commits, a fix and its follow-ups) rather than listing them flat. Run `jj` with cwd `{repo-root}`.
2. **Merged and open PRs** — only when a PR interface is reachable (a `gh` CLI that responds, a connector/MCP tool). When invoking `gh`, run it with cwd `{repo-root}` and `GIT_DIR=$(jj git root)`. This portion is capability-gated: when no interface is reachable, note "PR evidence unavailable" in one line and move on — never treat the missing interface as an error and never guess PR state from bookmark names.
...
- Why, when a doc or commit body says so — quoted, with the source (`file:line` or commit id)
