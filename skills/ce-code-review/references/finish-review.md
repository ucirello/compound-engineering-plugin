...
**Describe the change when the pre-review tree was clean.** Before applying, note whether the working-copy commit already had changes (`jj diff` non-empty vs parent). The permanence gate is the **push**, not the local description — a local change is private and reversible (`jj undo`).
...
- **Clean before the review:** after applying and verifying, describe the fixes as one isolated change with `jj commit -m "<message composed from the standards above>"`. Labeled and reversible, returning the tree to a known state.
- **Dirty before the review:** apply but do **not** describe/commit — the fixes interleave with the user's in-flight work and ride along with the change they were already going to make. The Applied section lists what changed.
...
2. **Applied (explicit local apply only).** When Stage 5c applied fixes, list them first — before the findings — in an Applied section (see review output template); each entry carries `#`, file, the fix, and reviewer (a multi-file fix is one row with one `#`), then a one-line validation outcome (e.g. "pin tests 4 -> 6; suite 94 pass, lint clean") and commit status (described on a clean tree with the message composed from the standards above, or left undescribed for the user on a dirty one). Flag green-but-unverifiable edits (auth/contract/concurrency) prominently. Omit this section when local apply was not authorized or nothing was applied. Applied findings appear here, not in the severity tables.
...
{
  "run_id": "<run-id>",
  "branch": "<jj bookmark list -r @ -T 'name' at dispatch time>",
