...
# opencode2 is a distinct harness from opencode. The snippet above does not name
# it; when you can identify opencode2 from your own runtime, attest
# XHOST_HARNESS=opencode2 rather than opencode.
```

Pass `XHOST_HARNESS` as `CROSS_MODEL_HOST_HARNESS`; pass `XHOST_FAMILY` as the first worker argument. The snippet is evidence, not the verdict: it resolves the harnesses whose environment markers it already names, and where it yields `unknown` on a harness you can identify from your own runtime, attest what you know instead. Both tokens must be peer keys the worker accepts, never a provider's company name. Family is `codex`, `claude`, `grok`, `composer`, or `unknown`. Harness is `codex`, `claude`, `grok`, `cursor`, `opencode`, `opencode2`, or `unknown`. `opencode2` is not `opencode`. A company name such as `anthropic`, `openai`, or `xai` in either slot makes the worker refuse the job and write no artifact.
...
The peer reviews the **current work tree** (read-only) against `jj diff --from <base-ref>`. Reviewed code/diff content is sent to an external model provider (OpenAI, Anthropic, xAI, or Cursor, depending on the resolved peer). `CROSS_MODEL_PEERS` restricts which providers may receive content.
...
- **opencode2:** distinct from v1 `opencode`. `opencode2 run --standalone --auto --model provider/model#variant --format json --file PROMPT` with cwd at the workspace root and **no `--dir`**. Variant is `#variant` on `--model`, not a `--variant` flag.
