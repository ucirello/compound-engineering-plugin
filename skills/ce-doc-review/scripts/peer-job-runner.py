...
Environment overrides (defaults in parentheses):
  CE_PEER_JOBS_ROOT         base dir (`<jj workspace root>/.tmp`, or `./.tmp`
                            when not a JJ repo). Override still wins when set.
...
the JJ workspace `.tmp` (or local `.tmp` when not a JJ repo). Every read of job state opens the file first (no-follow) and
...
POSIX path is behaviorally unchanged:
  ...
  jobs root defaults to `<jj workspace root>/.tmp` (or `./.tmp` when not a JJ
            repo), owner-private, on POSIX and Windows alike.
...
# `_default_jobs_root`). DEFAULT_ROOT stays None at import so a test that
...
def _jj_workspace_root(cwd=None):
    """Absolute JJ workspace root, or cwd when `jj workspace root` is unavailable."""
    ...
    try:
        ...
        return os.path.abspath(probe_cwd)
...
def _default_jobs_root(cwd=None):
    return os.path.join(_jj_workspace_root(cwd), ".tmp")
