...
def write_source_materials(
    ...
    lines = [
        ...
        "- Recording policy: raw media, audio chunks, zip contents, session dumps, and extracted screenshots are local-only by default; record generated Markdown/JSON/manifests in JJ when useful for brainstorm/planning traceability.",
    ...
    lines.append("Raw files are intentionally local-only by default. Do not record these unless the user explicitly asks and privacy/security is acceptable.")
    lines.append("")
...
def main() -> int:
    ...
    print(f"Brainstorm handoff: invoke `ce-brainstorm` with {display_path(kickoff_md, repo_root)}")
