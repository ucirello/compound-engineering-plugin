...
A tracker can be surfaced via MCP tool (e.g., a Linear MCP server), CLI (e.g., `gh`), or direct API. All are acceptable. Pair every `gh` invocation with `GIT_DIR` set to the output of `jj git root`. The detection output is a tuple with two availability flags — one for the named tracker specifically (drives label confidence in Interactive mode) and one for the full fallback chain (drives whether Defer is offered at all):
...
- **Body:**
  ...
  - Source: a link to the PR carrying this change when one already exists at filing time; otherwise the bookmark and working-copy commit id (`jj log -r @ --no-graph -T 'commit_id'`), so the ticket points at the code even before a PR is opened. When the same run opens a PR after the ticket is filed, back-fill the PR link into the ticket (best-effort; never block shipping on the update).
