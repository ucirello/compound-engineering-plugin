...
**Model-elevation visibility.** Treat a stripped `plan_model:<alias>` token, or a `plan_model` / `plan_harness` config value you have found, as a pending Phase 5.2 input, not a resolved choice. Phase 5.2 resolves the choice from the current conversation, that token, and config immediately before authoring, so later user intent cannot be lost. When `plan_harness` is `opencode2`, the matching `plan_model` is `provider/modelname#variant`.
