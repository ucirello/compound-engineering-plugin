...
# scope; private prompt/result scratch stays under the workspace-local `.tmp` tree.
#
...
#   <run-dir>         existing private dir under <workspace-root>/.tmp; output ->
...
case "$RUN_DIR_RESOLVED/" in "$TEMP_ROOT/"*) ;; *) skip "run-dir must be under '$TEMP_ROOT'" ;; esac
...
SCRATCH_PARENT="${CROSS_MODEL_SCRATCH_PARENT:-$TEMP_ROOT}"
...
case "$SCRATCH_PARENT/" in "$TEMP_ROOT/"*) ;; *) skip "private scratch parent must be under '$TEMP_ROOT'" ;; esac
PEER_WORKDIR=""
for n in 0 1 2 3 4 5 6 7; do
  candidate="$SCRATCH_PARENT/xmodel-pov-peer-$$-$n"
  if (umask 077; mkdir "$candidate") 2>/dev/null; then
    PEER_WORKDIR="$candidate"
    break
  fi
done
...
run_timeout_cmd() {
  ...
  # remain under workspace-local scratch; route adapters separately carry the same root.
