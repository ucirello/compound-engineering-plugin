...
|-------|---------|--------|
| `diff:<revision-or-revset>` | `diff:@-`, `diff:trunk()..@`, `diff:PR#42` | Forces diff mode on that change |
...
- An unrecognized `<word>:<word>` token (including change-description prefixes like `feat:` appearing inside a topic) is not a flag — it passes through verbatim as request text. The same holds for a *recognized* token that fails the reads-as-a-flag test above.
...
- **Diff** — the request names a resolvable change: a change ID, bookmark, revision, PR, "the previous change", "what you just did", or "this change".
