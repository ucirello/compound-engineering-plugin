...
workspace's local `.tmp`. Every read of job state opens the file first (no-follow) and
...
POSIX path is behaviorally unchanged:
  ...
  jobs root defaults under the current workspace's `.tmp`, owner-private.
...
def candidate_jobs_root_bases() -> list:
    """The single workspace-local root where an existing job may live."""
...
def ensure_owned_dirs(base: str, path: str) -> None:
    """Create `path` privately, verifying every component from workspace .tmp down."""
    scratch = os.path.abspath(DEFAULT_SCRATCH_ROOT)
    base = os.path.abspath(base)
    path = os.path.abspath(path)
    try:
        contained = all(os.path.commonpath((scratch, candidate)) == scratch for candidate in (base, path))
    except ValueError:
        contained = False
    if not contained:
        raise RunnerError(f"managed directories must be contained by {scratch}")
    ...
    if IS_WINDOWS:
        if not _win_harden_acl(cur):
            raise RunnerError(f"{cur}: could not harden the managed-directory ACL")
    else:
        os.chmod(cur, 0o700)
...
def write_atomic(path: str, data: bytes) -> None:
    ...
    for _ in range(CLAIM_ATTEMPTS):
        staging = os.path.join(parent, f".staging-{secrets.token_hex(8)}")
        try:
            fd = os.open(staging, os.O_WRONLY | os.O_CREAT | os.O_EXCL | O_NOFOLLOW | O_BINARY, 0o600)
            break
        except FileExistsError:
            continue
    else:
        raise OSError("could not reserve an atomic staging file")
    try:
...
def _require_detach_support() -> None:
    ...
    if not hasattr(os, "fork") or not hasattr(os, "setsid"):
        raise RunnerError(
            ...
            "the Windows detach compatibility contract)."
        )
