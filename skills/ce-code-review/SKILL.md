...
| `mode:report-only` | `mode:report-only` | **Deprecated — ignored.** Former no-artifacts mode; default behavior is review-only without working-copy changes |
...
- **Apply locally; never push.** Never push, open PRs, or file tickets in any mode — push is the outward step the user owns. In **default (interactive)** mode the review applies safe, verified fixes and creates a JJ change when the pre-review working copy was clean (Stage 5c owns the full rule). In **`mode:agent`** it never mutates the working copy — it reports and the caller applies.
...
- **Explicit mutations only.** Never run `gh pr checkout`, `jj edit`, `jj new`, or similar working-copy-switch commands during scope discovery. Passing a PR number, URL, or bookmark name selects **review scope**, not permission to mutate the working copy. To review local work on another bookmark, enter its workspace yourself (or stay in it) and pass `base:` or no target.
- **Smart defaults.** JJ snapshots all non-ignored working-copy files. Plan: use `plan:` when passed; otherwise discover conservatively from PR body or bookmark keywords. Weak advisory P2/P3 from testing/maintainability alone: demote to `testing_gaps` / `residual_risks` per Stage 5.
...
The caller already knows the diff base. Skip all base-bookmark detection and remote resolution. Resolve the common ancestor with the provided value:
...
This path works with any JJ revision — a commit ID, `main@origin`, or a bookmark name. Callers reviewing the current working copy should pass explicit `base:` when auto-detection is unnecessary. **Do not combine `base:` with a PR number or bookmark target.** If both are present, stop with an error: "Cannot use `base:` with a PR number or bookmark target — `base:` implies the current working copy is already correct. Pass `base:` alone, or pass the target alone and let scope detection resolve the base."

**If a PR number or GitHub URL is provided as an argument:**

Do **not** edit the PR bookmark. Scope comes from GitHub read APIs plus optional local alignment when the current JJ change contains the PR head.
...
Set `BASE:` to `pr:<number-or-url>` (logical marker — not a JJ revision). Set `UNTRACKED:` empty: JJ snapshots all non-ignored working-copy files and has no staging/untracked split.
...
1. `jj log -r @ --no-graph -T 'bookmarks'` includes `headRefName`.
2. The PR is **not** cross-repository (`isCrossRepository` is false).
3. `jj log -r '<headRefOid> & ::@' --no-graph -T 'commit_id'` returns the PR head commit. This confirms the working copy carries the PR head (allowing local fixes layered on top) rather than an unrelated same-named bookmark.
...
- **`local-aligned`:** Resolve `<resolved-base-ref>` as `baseRefName@origin` (run `jj git fetch --remote origin --branch <baseRefName>` if needed). Compute `BASE` with `jj log -r 'heads(::@ & ::<resolved-base-ref>)' --no-graph -T 'commit_id ++ "\n"'`, then set `FILES:` from `jj diff --from "$BASE" --to @ --name-only` and `DIFF:` from `jj diff --from "$BASE" --to @ --context 10`. Do **not** call `gh pr diff` or append remote hunks — when local fixes exist, the JJ working copy is canonical. Note in Coverage: `scope: local-aligned (PR; local working-copy diff)`.
- **`pr-remote`:** Set `FILES:` from the PR `files` array. Set `DIFF:` from `gh pr diff <number-or-url> --color=never`. If `gh pr diff` fails, stop with an actionable error — do not change the working copy as a fallback.

When **`pr-remote`**, before Stage 4:

1. Best-effort fetch PR head without changing the working copy: `jj git fetch --remote origin --branch <headRefName>`.
...
3. Best-effort fetch the PR base without changing the working copy: `jj git fetch --remote origin --branch <baseRefName>`. When it succeeds, set `PR_BASE_REF=<baseRefName>@origin` — a real JJ revision reviewers and validators use for file-level diffs (e.g. `data-migration-reviewer` runs `jj diff --from <PR_BASE_REF> --to <PR_HEAD_REF> db/schema.rb`). The `pr:<number-or-url>` marker in `BASE:` stays the scope marker. When the fetch fails, omit `PR_BASE_REF` and note in Coverage — checks fall back to diff hunks only and must **not** assume `main`.
...
If `jj log -r @ --no-graph -T 'bookmarks'` includes `<bookmark>`, use the **standalone (current working copy)** path below — same change, explicit bookmark name; do not use remote-only diff.
...
2. Else run `jj git fetch --remote origin --branch <bookmark>` when needed, then resolve `<bookmark-ref>` as `<bookmark>@origin` or `<bookmark>`.
3. Resolve the default base bookmark (same logic as standalone). Compute `BASE` with `jj log -r 'heads(::<base-ref> & ::<bookmark-ref>)' --no-graph -T 'commit_id ++ "\n"'` and diff with `jj diff --from "$BASE" --to <bookmark-ref> --context 10`.
4. If `<bookmark-ref>` cannot be resolved locally, stop: "Cannot diff bookmark `<bookmark>` without changing the working copy. Enter that bookmark's workspace, pass its open PR URL/number, or review the current working copy with `base:`."
...
```
echo "BASE:$BASE" && echo "FILES:" && jj diff --from "$BASE" --to <bookmark-ref> --name-only && echo "DIFF:" && jj diff --from "$BASE" --to <bookmark-ref> --context 10 && echo "UNTRACKED:"
```

**If no argument (standalone in current workspace):**

Apply the same base-detection logic as bookmark mode above, using bookmarks on `@` (and `gh pr view --json baseRefName,url` with no argument when the current bookmark has a PR).
...
Using `jj diff --from "$BASE" --to @` diffs the common ancestor against the current JJ working-copy change, including the full change stack and current snapshot.

**Untracked file handling:** JJ snapshots all non-ignored working-copy files and has no staging/untracked split, so `UNTRACKED:` is empty. Ignored files remain out of scope; never modify ignore rules.
...
if [ -z "${DIFF_A:-}" ] || ! jj log -r "$DIFF_A" --no-graph -T 'commit_id' >/dev/null 2>&1 \
  ...
  # EXEC_LINES uses JJ's git-format stat, avoiding raw +/- content-line counting.
  if [ -n "${DIFF_B:-}" ]; then
    DIFF_STAT=$(jj diff --from "$DIFF_A" --to "$DIFF_B" --git --stat 2>/dev/null)
  else
    DIFF_STAT=$(jj diff --from "$DIFF_A" --to @ --git --stat 2>/dev/null)
  fi
  EXEC_LINES=$(printf '%s\n' "$DIFF_STAT" | awk '/\|/ && $1 ~ /\.(rb|py|js|jsx|ts|tsx|go|rs|java|swift|kt|c|cc|cpp|cs|php|ex|exs|scala)$/ {s+=$3} END{print s+0}')
...
**Bookmark mode:** Run `jj log -r "$BASE::<bookmark-ref>" --no-graph` using the resolved common ancestor and bookmark revision from Stage 1. Use `<bookmark-ref>` (the resolved `<bookmark>@origin` or local bookmark), not the raw argument, so a remote-only bookmark cannot resolve to a stale local name.

**Standalone (current workspace):** Run:

```
echo "BOOKMARKS:" && jj log -r @ --no-graph -T 'bookmarks ++ "\n"' && echo "CHANGES:" && jj log -r "$BASE::@" --no-graph
```
...
6. Run directory and reviewer name for the artifact file path
...
Persona sub-agents are **read-only** with respect to the project: they review and return structured JSON. They do not edit project files or propose refactors. The one permitted write is saving their full analysis to the workspace-local `{run_dir}` path specified in the output contract.

Read-only here means **non-mutating**, not "no shell access." Reviewer sub-agents may use non-mutating inspection commands when needed to gather evidence or verify scope, including `jj diff`, `jj file show`, `jj file annotate`, `jj log`, and `gh pr view`. In **`pr-remote`** or **`bookmark-remote`** scope (see Stage 1), inspect changed files via `jj file show -r <remote-head-ref> <path>` or diff hunks — do not Read/Grep workspace paths for files in scope. They must not edit project files, edit/create JJ changes, push, create PRs, or otherwise mutate repository state.
...
3. **Spawn validators with bounded parallelism.** One sub-agent per finding, dispatched independently using the validator template and the same bounded scheduler from Stage 4. Each validator receives:
   ...
   - The full diff
   - The scope mode and remote head revision, mirroring the Stage 4 reviewer bundle: inject `<pr-scope-mode>local-aligned | pr-remote | bookmark-remote</pr-scope-mode>` and, when set, `<pr-head-ref>...</pr-head-ref>` or `<bookmark-head-ref>...</bookmark-head-ref>`. The validator template defaults to local-aligned workspace inspection when these are absent, so omitting them in `pr-remote`/`bookmark-remote` makes validators verify findings against the stale working copy — dropping valid findings or confirming false ones on the wrong revision.
...
Severity, confidence, and cross-reviewer agreement tell you what to do first and what to flag loudly — they do not gate the decision. There is no deny-list: downside is controlled after the fact (`jj restore` + visible diff + the change checkpoint), not by a precondition.
...
**Verify, then keep.** After applying, run the affected tests and lint (targeted by default; broaden when fixes span files). If they fail, revert that fix and report it as a finding instead — an unverified fix is not finished. Never leave the working copy with failing checks.

**Review the autofix diff before finishing.** Before describing or reporting applied fixes, use `jj diff --from <pre-apply-change> --to @` to inspect only the changes introduced during Stage 5c. Run one self-review pass over that diff:
...
**Describe an isolated JJ change when the pre-review working copy was clean.** Before applying, capture `jj status`, the current change ID, and the immutable pre-apply commit ID from `jj log -r @ --no-graph -T commit_id`. Use that commit ID as `<pre-apply-change>` in the autofix diff. The permanence gate is `jj git push`, not a local change description; local changes remain private and reversible with JJ operations.

Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards.
Repository-local instructions and `git log` syntax always win. Apply compatible Go quality guidance, including a concise imperative subject and explanatory body when useful, but derive any type, prefix, scope, subject, and body shape dynamically from those sources; do not impose a fixed template.

- **Clean before the review:** after applying and verifying on the empty working-copy change, run `jj describe -m "<message derived from repository-local standards and history>"`, then `jj new` to leave a fresh working-copy change. The isolated review fix remains easy to inspect and abandon.
- **Dirty before the review:** apply but do **not** rewrite the existing change description or create a new change — the fixes interleave with the user's in-flight work. The Applied section lists what changed.
...
2. **Applied (default mode only).** When Stage 5c applied fixes, list them first — before the findings — in an Applied section (see review output template); each entry carries `#`, file, the fix, and reviewer (a multi-file fix is one row with one `#`), then a one-line validation outcome (e.g. "pin tests 4 -> 6; suite 94 pass, lint clean") and JJ change-description status (described on a clean working copy with the dynamically derived message, or left in the existing change on a dirty one). Flag green-but-unverifiable edits (auth/contract/concurrency) prominently. Omit this section in `mode:agent` and when nothing was applied. Applied findings appear here, not in the severity tables.

When rendering or validating that change description, apply this instruction:
Based on https://go.dev/wiki/CommitMessage and on past commit messages that you can see in `git log`, compose commit messages adherent to the present standards.
Repository-local instructions and `git log` syntax always win. Apply compatible Go quality guidance, but derive all prefixes, types, scopes, subjects, and templates dynamically; preserve the semantic requirement to explain the applied review fixes and their verification.
...
Emit **one raw JSON object** as the primary response — a single bare JSON value, **no markdown code fence**. A leading ```` ```json ```` fence makes the response start with backticks and breaks naive `JSON.parse` consumers, so never wrap it. Also write `review.json` under `<workspace-root>/.tmp/rocketclaw/ce-code-review/<run-id>/` with the same payload; if `jj workspace root` fails, use `$PWD/.tmp/rocketclaw/ce-code-review/<run-id>/`. Create the directory first.
...
```json
{
  ...
  "scope": {
    "base": "<common-ancestor revision, pr:NNN marker, or base: revision>",
    "bookmarks": "<bookmarks on the current JJ change>",
    "head_change": "<jj log -r @ --no-graph -T change_id>",
    "pr_url": "<url or null>",
    "files_changed": 0
  },
...
After Stage 6, stop. Never push, open PRs, or file tickets from this skill. In default (interactive) mode, Stage 5c has already applied and (on a clean pre-review working copy) described the isolated JJ change; in `mode:agent` the review mutates nothing — the caller (for example `ce-work`) and the user apply fixes, file tickets, or accept residual risk using the report and artifact.
...
- Include the workspace-local run-artifact path when one was written: `<workspace-root>/.tmp/rocketclaw/ce-code-review/<run-id>/`
...
Always write run artifacts under `<workspace-root>/.tmp/rocketclaw/ce-code-review/<run-id>/`, falling back to `$PWD/.tmp/rocketclaw/ce-code-review/<run-id>/` when `jj workspace root` fails. Create the directory first:
...
```json
{
  ...
  "change_id": "<jj log -r @ --no-graph -T change_id at dispatch time>",
  "verdict": "<Ready to merge | Ready with fixes | Not ready>",
