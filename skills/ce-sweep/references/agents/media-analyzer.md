...
3. **Check whether the issue already appears fixed on the default bookmark.** Once you know the affected surface, use read-only `jj log` (filesets for the files, routes, components the symptom touches) / `gh` (with `GIT_DIR` from `jj git root`, cwd at the workspace root) to see whether a recent change or merged PR already addresses it. Report this as a field in your finding so the orchestrator does not re-file resolved work.
...
- You are read-only except for the ONE write to your scratch artifact path. Read-oriented `jj` / `gh` and running the bundled analyzer are permitted; do not edit project files, check out a different revision, describe or commit a change, push, or open PRs.
